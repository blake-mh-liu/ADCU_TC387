/* Mk_TRICORE_asm_diab.h
 *
 * This file is the toolchain-dependent include file for the diab assembler for Tricore.
 *
 * This file defines the macros used in assembler files in a way such that the assembler files
 * will compile with the diab (WindRiver) assembler.
 *
...
#ifndef MK_TRICORE_ASM_DIAB_H
#define MK_TRICORE_ASM_DIAB_H
...
#define MK_extern			.xref
#define MK_align(b,p2)		.alignn	b
...
*/
#define MK_ASM_SECTION_TEXT				.text
...
*/
#define MK_ASM_SECTION_BOOTWORD			.section	"MK_bootword",4,rx
 ...
 * This could be already defined by the user's memmap, so we only define it if it isn't already defined.
*/
#define MK_ASM_SECTION_RESET			.section	"MK_reset",4,rx
 ...
 * This could be already defined by the user's memmap, so we only define it if it isn't already defined.
*/
#define MK_ASM_SECTION_EXCEPTIONTABLE	.section	"MK_exctable",4,rx
#define MK_ASM_SDECL_EXCEPTIONTABLE

/* The Diab assembler expects the register names to be prefixed with %
