/* Mk_k_syssststartcounter.c
 *
 * This file contains the function MK_SysSstStartCounter()
 *
...
/*
 * MK_SysSstStartCounter() - check parameters and call MK_LocalSstStartCounter()
 *
 * !LINKSTO Microkernel.Function.MK_SysSstStartCounter, 1
 * !doctype src
*/
void MK_SysSstStartCounter(mk_kernelcontrol_t * coreVars)
{
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			mk_tick_t delay = (mk_tick_t)MK_HwGetParameter2(caller->regs);

			errorCode = MK_LocalSstStartCounter(sstCntr, delay);
		}
	}
	...
	{
		osekError = MK_InternReportError(coreVars, MK_sid_SstStartCounter, errorCode, caller, MK_NULL);
	}
