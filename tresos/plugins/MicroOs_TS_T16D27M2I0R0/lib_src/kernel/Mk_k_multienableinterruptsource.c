/* Mk_k_multienableinterruptsource.c
 *
 * This file contains the MK_MultiEnableInterruptSource() function.
 *
...
/*
 * MK_MultiEnableInterruptSource() enables an interrupt source belonging to an ISR.
 *
 * This function is called on multi-core systems to enable a source.
 * The ISR ID is assumed to have been previously validated.
 *
 * !LINKSTO Microkernel.Function.MK_MultiEnableInterruptSource, 2
 * !doctype src
*/
mk_errorid_t MK_MultiEnableInterruptSource
...
)
{
	...
	{
		errorCode = MK_LocalEnableInterruptSource(isrCfg, callerApp);
	}
	else
	{
