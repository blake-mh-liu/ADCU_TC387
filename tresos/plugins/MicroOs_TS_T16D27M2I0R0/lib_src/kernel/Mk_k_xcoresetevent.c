/* Mk_k_xcoresetevent.c
 *
 * This file contains the function MK_XcoreSetEvent().
 *
...
/* MK_XcoreSetEvent() - set events for a task on request from another core.
 *
 * !LINKSTO Microkernel.Function.MK_XcoreSetEvent, 1
 * !doctype src
*/
/* Deviation MISRAC2012-1 <+1>, MISRAC2012-2 <+1> */
void MK_XcoreSetEvent(mk_kernelcontrol_t *coreVars, mk_msgptr_t msg, mk_objectid_t fromCore)
{
	...
	else
	{
		...
		if ( taskCfg->threadCfg.coreIndex == coreVars->coreIndex )
		{
			...
			else
			{
				...
				{
					(void)MK_InternReportError(coreVars, MK_sid_SetEvent, eid, MK_NULL, msg->parameter);
				}
			}
		}
		else
