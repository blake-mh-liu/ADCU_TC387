 ...
 *
 * !LINKSTO Microkernel.Function.MK_SysEnableInterruptSource, 2
 * !doctype src
*/
void MK_SysEnableInterruptSource(mk_kernelcontrol_t *coreVars)
{
	...
	*/
	...
	{
		errorCode = MK_InternEnableInterruptSource(coreVars, (mk_objectid_t)isrId, caller->applicationId);
	}
	else
	{
	...
	{
		osekError = MK_InternReportError(coreVars, MK_sid_EnableInterruptSource, errorCode, caller, MK_NULL);
	}
