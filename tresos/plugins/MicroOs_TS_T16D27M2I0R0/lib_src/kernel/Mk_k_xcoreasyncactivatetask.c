/* Mk_k_xcoreasyncactivatetask.c
 *
 * This file contains the function MK_XcoreAsyncActivateTask().
 *
...
/* MK_XcoreAsyncActivateTask() - activate a task on request from another core.
 *
 * !LINKSTO Microkernel.Function.MK_XcoreAsyncActivateTask, 1
 * !doctype src
*/
/* Deviation MISRAC2012-1 <+1>, MISRAC2012-2 <+1> */
void MK_XcoreAsyncActivateTask(mk_kernelcontrol_t *coreVars, mk_msgptr_t msg, mk_objectid_t unused_fromCore)
{
	...
	*/
	...
	else
	{
		...
		if ( taskCfg->threadCfg.coreIndex == coreVars->coreIndex )
		{
			...
			{
				if ( msg->opcode == (mk_parametertype_t)MK_xcore_AsyncChainTask )
				{
					(void)MK_InternReportError(coreVars, MK_sid_ChainTask, eid, MK_NULL, msg->parameter);
				}
				else
				{
					(void)MK_InternReportError(coreVars, MK_sid_AsyncActivateTask, eid, MK_NULL, msg->parameter);
				}
			}
		}
		else
