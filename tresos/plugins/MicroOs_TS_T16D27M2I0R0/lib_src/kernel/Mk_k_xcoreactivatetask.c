/* Mk_k_xcoreactivatetask.c
 *
 * This file contains the function MK_XcoreActivateTask().
 *
...
/* MK_XcoreActivateTask() - activate a task on request from another core.
 *
 * !LINKSTO Microkernel.Function.MK_XcoreActivateTask, 1
 * !doctype src
*/
/* Deviation MISRAC2012-1 <+1>, MISRAC2012-2 <+1> */
void MK_XcoreActivateTask(mk_kernelcontrol_t *coreVars, mk_msgptr_t msg, mk_objectid_t fromCore)
{
	...
	*/
	...
	else
	{
		...
		{
			/* We can pass MK_APPL_NONE as the application id here because the parameter is
			 * only used to ensure that a task of an application that's in the RESTARTING state
			 * can only be activated by its owner.
			 * An inter-core call can never come from the owner.
			*/
			eid = MK_LocalActivateTask(coreVars, taskId, taskCfg, MK_APPL_NONE);
			...
			{
				(void)MK_InternReportError(coreVars, MK_sid_ActivateTask, eid, MK_NULL, msg->parameter);
			}
		}
		else
		{
