...
#if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON)
 ...
 */
static FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateSynchronousMainCall
(
  void
);
...
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
 ...
 **/
VAR(boolean, CSM_VAR) Csm_MacGenerateIsRunning = FALSE;
 ...
 **/
VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_MacGenerateCurrentConfigId;
 ...
 **/
volatile VAR(Csm_ReturnType, CSM_VAR) Csm_MacGenerate_Ret_CallbackNotification = CSM_E_OK;
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
 ...
 **/
STATIC VAR(boolean, CSM_VAR) Csm_MacGenerate_Flg_ServiceFinishNotification = FALSE;
/** \brief Synchronous mode callback notification flag used for indicating that the primitive has
 **        finished the current start, updated or finish computation.
 **/
STATIC VAR(boolean, CSM_VAR) Csm_MacGenerate_Flg_CallbackNotification = FALSE;
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
...
#if (CSM_MACGENERATE_RTE_INTERFACE_ENABLED == STD_ON)

/*------------------------------------------------------------------------------------------------*/
/*----[Csm_RteMacGenerateStart]-------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------*/

/* !LINKSTO EB_CSM0776_01, 1
 */
FUNC(Std_ReturnType, CSM_CODE) Csm_RteMacGenerateStart
(
...
)
{
  ...
  retVal = (Std_ReturnType) Csm_MacGenerateStart
                            (
                              pdav0,
...
/*------------------------------------------------------------------------------------------------*/
/*----[Csm_RteMacGenerateUpdate]------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------*/

/* !LINKSTO EB_CSM0776_01, 1
 */
FUNC(Std_ReturnType, CSM_CODE) Csm_RteMacGenerateUpdate
(
          Csm_ConfigIdType                            pdav0,
...
)
{
  ...
  retVal = (Std_ReturnType) Csm_MacGenerateUpdate
                            (
                              pdav0,
                              dataBuffer,
                              dataLength
...
/*------------------------------------------------------------------------------------------------*/
/*----[Csm_RteMacGenerateFinish]------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------*/

/* !LINKSTO EB_CSM0776_01, 1
 */
FUNC(Std_ReturnType, CSM_CODE) Csm_RteMacGenerateFinish
(
        Csm_ConfigIdType                            pdav0,
...
)
{
  ...
  retVal = (Std_ReturnType) Csm_MacGenerateFinish
                            (
                              pdav0,
                              resultBuffer,
                              resultLength,
...
#endif /* #if (CSM_MACGENERATE_RTE_INTERFACE_ENABLED == STD_ON) */
...
/* !LINKSTO CSM0108, 1
 */
FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateStart
(
...
)
{
  ...
  /* !LINKSTO CSM0046, 1
   */
  if ( cfgId >= (sizeof(Csm_MacGenerateConfigurations   ) /
                 sizeof(Csm_MacGenerateConfigurations[0])
                )
     )
  {
    RetVal = CSM_E_NOT_OK;
...
#if (CSM_MACGENERATE_IMMEDIATE_RESTART_ENABLED == STD_OFF)
  /* !LINKSTO EB_CSM0017_Busy, 1
   */
  else
#if (CSM_USEPRIORITIES_ENABLED == STD_ON)
  if ( ((Csm_MacGenerateIsRunning == TRUE)
       /* !LINKSTO EB_CSMPrio, 1 */
       && (
        (Csm_MacGenerateConfigurations[cfgId].CsmMacGenerateUsePriorities == FALSE) ||
        (Csm_MacGenerateConfigurations[cfgId].PrimitiveStartFct !=
         Csm_MacGenerateConfigurations[Csm_MacGenerateCurrentConfigId].PrimitiveStartFct))
        )
       && ((Csm_MacGenerateConfigurations[cfgId].CsmMacGenerateEnableRestart == FALSE) ||
        (Csm_MacGenerateCurrentConfigId != cfgId)) )
#else
  if ( (Csm_MacGenerateIsRunning == TRUE) &&
       ((Csm_MacGenerateConfigurations[cfgId].CsmMacGenerateEnableRestart == FALSE) ||
        (Csm_MacGenerateCurrentConfigId != cfgId)) )
  ...
  else
  {
...
#if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON)
    Csm_MacGenerate_Ret_CallbackNotification = CSM_E_NOT_OK;
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */

    /* !LINKSTO CSM0506, 1
     */
    RetVal =
      Csm_MacGenerateConfigurations[cfgId].
      PrimitiveStartFct
      (
        Csm_MacGenerateConfigurations[cfgId].PrimitiveConfigPtr,
        keyPtr
      );
    ...
    if ( RetVal == CSM_E_OK )
    {
      ...
      RetVal = Csm_MacGenerateSynchronousMainCall();
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
    }
    else if ( RetVal != CSM_E_BUSY )
    {
       ...
       */
      Csm_MacGenerateIsRunning = FALSE;
    }
    else
    {
...
/* !LINKSTO CSM0023, 1
 */
/* !LINKSTO EB_CSM0114_01, 1
 */
FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateUpdate
(
          Csm_ConfigIdType                            cfgId,
...
)
{
  ...
  /* !LINKSTO CSM0050, 1
   */
  if ( (Csm_MacGenerateIsRunning       == FALSE) ||
       (Csm_MacGenerateCurrentConfigId != cfgId)
     )
  {
    /* !LINKSTO CSM0657, 1
  ...
  else
  {
     ...
     */
    RetVal =
      Csm_MacGenerateConfigurations[Csm_MacGenerateCurrentConfigId].
      PrimitiveUpdateFct
      (
        dataPtr,
        dataLength
...
/* !LINKSTO CSM0023, 1
 */
/* !LINKSTO EB_CSM0121_01, 1
 */
FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateFinish
(
        Csm_ConfigIdType                            cfgId,
...
#if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_OFF)
/*------------------------------------------------------------------------------------------------*/
/*----[Csm_MacGenerateMainFunction]---------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------*/

FUNC(void, CSM_CODE) Csm_MacGenerateMainFunction
(
  void
)
{
...
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_OFF) */
...
/* !LINKSTO CSM0039, 1
 */
FUNC(void, CSM_CODE) Csm_MacGenerateCallbackNotification
(
...
)
{
    ...
    Csm_MacGenerate_Flg_CallbackNotification = TRUE;
    Csm_MacGenerate_Ret_CallbackNotification = result;

    if (NULL_PTR                                                                                 !=
        Csm_MacGenerateConfigurations[Csm_MacGenerateCurrentConfigId].CallbackFct
       )
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
  {
      /* !LINKSTO EB_CSM_SDS_0003,1 */
      (void) Csm_MacGenerateConfigurations[Csm_MacGenerateCurrentConfigId].
           CallbackFct(result);
  }
}
...
/* !LINKSTO CSM0457, 1
 */
FUNC(void, CSM_CODE) Csm_MacGenerateServiceFinishNotification
(
  void
)
{
  ...
  Csm_MacGenerate_Flg_ServiceFinishNotification = TRUE;

#else /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
...
FUNC(void, CSM_CODE) Csm_MacGenerateInit
(
  void
)
{
  ...
  (void)
{
  ...
  /* !LINKSTO CSM0035, 1
   */
  Csm_MacGenerate_Flg_CallbackNotification = FALSE;
  Csm_MacGenerate_Flg_ServiceFinishNotification = FALSE;

  /* !LINKSTO EB_CSM_SDS_0005,1 */
  while ((Csm_MacGenerate_Flg_CallbackNotification      == FALSE) &&
         (Csm_MacGenerate_Flg_ServiceFinishNotification == FALSE)
        )
  {
    Csm_MacGenerateConfigurations[Csm_MacGenerateCurrentConfigId].
  ...
  returnVal = Csm_MacGenerate_Ret_CallbackNotification;
  return returnVal;
}
#endif /* #if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON) */
...
#else  /* #if (CSM_MACGENERATE_ENABLED == STD_ON) */

/** \brief  Dummy definition preventing this file from being empty, if there is no configuration. */
typedef void Csm_MacGenerate_EmptyTranslationUnit_t;

#endif /* #if (CSM_MACGENERATE_ENABLED == STD_ON) #else */
