...
#ifndef CSM_RANDOMSEED_H
#define CSM_RANDOMSEED_H
 ...
 **
 ** This function requests the start of an random seed computation. The start function of the
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_RandomSeedStart
(
 ...
 **
 ** This function requests the update of the random seed computation for the given data. The
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_RandomSeedUpdate
(
          Csm_ConfigIdType                            cfgId,
 ...
 **
 ** This function requests the finishing of the random seed computation and the storing of the
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_RandomSeedFinish
(
  Csm_ConfigIdType cfgId
);

/** \brief  Used by the random seed primitive to indicate finished computation.
 **
 ** This function has to be called by the random seed primitive to indicate that it has
 ...
 **                      the user.
 **/
extern FUNC(void, CSM_CODE) Csm_RandomSeedCallbackNotification
(
  Csm_ReturnType result
);

/** \brief  Used by the random seed primitive to indicate end of service.
 **
 ** This function has to be called by the random seed primitive to indicate that it has
 ** completely finished the currently computed service. This function then sets the random seed
 ** computation state machine to the idle state.
 **/
extern FUNC(void, CSM_CODE) Csm_RandomSeedServiceFinishNotification
(
  void
);
 ...
 **/
extern FUNC(void, CSM_CODE) Csm_RandomSeedMainFunction
(
  void
);
...
/** \brief  Initialization of the random seed computation interface.
 **
 ** This function initializes the random seed computation interface such that a service can be
 ** requested with Csm_RandomSeedStart().
 **/
extern FUNC(void, CSM_CODE) Csm_RandomSeedInit
(
  void
);
 ...
 **/
extern VAR(boolean, CSM_VAR) Csm_RandomSeedIsRunning;
 ...
 **/
extern VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_RandomSeedCurrentConfigId;

/** \brief Synchronous mode callback notification return value
 **/
#if (CSM_RANDOMSEED_SYNCJOBPROCESSING_ENABLED == STD_ON)
extern volatile VAR(Csm_ReturnType, CSM_VAR) Csm_RandomSeed_Ret_CallbackNotification;
...
#endif /* #if (CSM_RANDOMSEED_ENABLED == STD_ON) */

#endif /* #ifndef CSM_RANDOMSEED_H */
