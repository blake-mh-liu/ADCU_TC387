...
#ifndef CSM_SYMKEYWRAPASYM_H
#define CSM_SYMKEYWRAPASYM_H
 ...
 **
 ** This function requests the start of an asymmetrical key wrapping computation. The start
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymKeyWrapAsymStart
(
 ...
 **
 ** This function requests the update of the asymmetrical key wrapping computation for the
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymKeyWrapAsymUpdate
(
        Csm_ConfigIdType                            cfgId,
 ...
 **
 ** This function requests the finishing of the asymmetrical key wrapping computation and the
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymKeyWrapAsymFinish
(
  Csm_ConfigIdType cfgId
);

/** \brief  Used by the asymmetrical key wrapping primitive to indicate finished computation.
 **
 ** This function has to be called by the asymmetrical key wrapping primitive to indicate that
 ...
 **                      the user.
 **/
extern FUNC(void, CSM_CODE) Csm_SymKeyWrapAsymCallbackNotification
(
  Csm_ReturnType result
);

/** \brief  Used by the asymmetrical key wrapping primitive to indicate end of service.
 **
 ** This function has to be called by the asymmetrical key wrapping primitive to indicate that
 ** it has completely finished the currently computed service. This function then sets the
 ** asymmetrical key wrapping computation state machine to the idle state.
 **/
extern FUNC(void, CSM_CODE) Csm_SymKeyWrapAsymServiceFinishNotification
(
  void
);
 ...
 **/
extern FUNC(void, CSM_CODE) Csm_SymKeyWrapAsymMainFunction
(
  void
);
...
/** \brief  Initialization of the asymmetrical key wrapping computation interface.
 **
 ** This function initializes the asymmetrical key wrapping computation interface such that a
 ** service can be requested with Csm_SymKeyWrapAsymStart().
 **/
extern FUNC(void, CSM_CODE) Csm_SymKeyWrapAsymInit
(
  void
);
 ...
 **/
extern VAR(boolean, CSM_VAR) Csm_SymKeyWrapAsymIsRunning;
 ...
 **/
extern VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_SymKeyWrapAsymCurrentConfigId;

/** \brief Synchronous mode callback notification return value
 **/
#if (CSM_SYMKEYWRAPASYM_SYNCJOBPROCESSING_ENABLED == STD_ON)
extern volatile VAR(Csm_ReturnType, CSM_VAR) Csm_SymKeyWrapAsym_Ret_CallbackNotification;
...
#endif /* #if (CSM_SYMKEYWRAPASYM_ENABLED == STD_ON) */

#endif /* #ifndef CSM_SYMKEYWRAPASYM_H */
