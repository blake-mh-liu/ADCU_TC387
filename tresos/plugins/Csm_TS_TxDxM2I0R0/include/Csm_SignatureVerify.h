...
#ifndef CSM_SIGNATUREVERIFY_H
#define CSM_SIGNATUREVERIFY_H
 ...
 **
 ** This function requests the start of an signature verification computation. The start
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SignatureVerifyStart
(
 ...
 **
 ** This function requests the update of the signature verification computation for the given
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SignatureVerifyUpdate
(
          Csm_ConfigIdType                            cfgId,
 ...
 **
 ** This function requests the finishing of the signature verification computation and the
 ...
 ** \param[in ]  signaturePtr     Holds a pointer to the memory location which holds the
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SignatureVerifyFinish
(
          Csm_ConfigIdType                                cfgId,
...
/** \brief  Used by the signature verification primitive to indicate finished computation.
 **
 ** This function has to be called by the signature verification primitive to indicate that it
 ...
 **                      the user.
 **/
extern FUNC(void, CSM_CODE) Csm_SignatureVerifyCallbackNotification
(
  Csm_ReturnType result
);

/** \brief  Used by the signature verification primitive to indicate end of service.
 **
 ** This function has to be called by the signature verification primitive to indicate that it
 ** has completely finished the currently computed service. This function then sets the
 ** signature verification computation state machine to the idle state.
 **/
extern FUNC(void, CSM_CODE) Csm_SignatureVerifyServiceFinishNotification
(
  void
);
 ...
 **/
extern FUNC(void, CSM_CODE) Csm_SignatureVerifyMainFunction
(
  void
);
...
/** \brief  Initialization of the signature verification computation interface.
 **
 ** This function initializes the signature verification computation interface such that a
 ** service can be requested with Csm_SignatureVerifyStart().
 **/
extern FUNC(void, CSM_CODE) Csm_SignatureVerifyInit
(
  void
);
 ...
 **/
extern VAR(boolean, CSM_VAR) Csm_SignatureVerifyIsRunning;
 ...
 **/
extern VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_SignatureVerifyCurrentConfigId;

/** \brief Synchronous mode callback notification return value
 **/
#if (CSM_SIGNATUREVERIFY_SYNCJOBPROCESSING_ENABLED == STD_ON)
extern volatile VAR(Csm_ReturnType, CSM_VAR) Csm_SignatureVerify_Ret_CallbackNotification;
...
#endif /* #if (CSM_SIGNATUREVERIFY_ENABLED == STD_ON) */

#endif /* #ifndef CSM_SIGNATUREVERIFY_H */
