...
#ifndef CSM_SYMDECRYPT_H
#define CSM_SYMDECRYPT_H
 ...
 **
 ** This function requests the start of an symmetrical decryption computation. The start
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymDecryptStart
(
 ...
 **
 ** This function requests the update of the symmetrical decryption computation for the given
 ...
 ** \param[in    ]  cipherTextPtr       Holds a pointer to the cipher text that shall be
 **                                     decrypted.
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymDecryptUpdate
(
          Csm_ConfigIdType                            cfgId,
 ...
 **
 ** This function requests the finishing of the symmetrical decryption computation and the
 ...
 ** \retval  CSM_E_SMALL_BUFFER  the provided buffer is too small to store the result
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_SymDecryptFinish
(
        Csm_ConfigIdType                            cfgId,
  P2VAR(uint8,            AUTOMATIC, CSM_APPL_DATA) plainTextPtr,
  P2VAR(uint32,           AUTOMATIC, CSM_APPL_DATA) plainTextLengthPtr
...
/** \brief  Used by the symmetrical decryption primitive to indicate finished computation.
 **
 ** This function has to be called by the symmetrical decryption primitive to indicate that it
 ...
 **                      the user.
 **/
extern FUNC(void, CSM_CODE) Csm_SymDecryptCallbackNotification
(
  Csm_ReturnType result
);

/** \brief  Used by the symmetrical decryption primitive to indicate end of service.
 **
 ** This function has to be called by the symmetrical decryption primitive to indicate that it
 ** has completely finished the currently computed service. This function then sets the
 ** symmetrical decryption computation state machine to the idle state.
 **/
extern FUNC(void, CSM_CODE) Csm_SymDecryptServiceFinishNotification
(
  void
);
...
/** \brief  Perform the symmetrical decryption computation tasks.
 **
 ** This function performs the actual symmetrical decryption computation tasks which have been
 ** requested by the service functions. The function calls the main function of the configured
 ** primitive to perform the tasks.
 **/
extern FUNC(void, CSM_CODE) Csm_SymDecryptMainFunction
(
  void
);
...
/** \brief  Initialization of the symmetrical decryption computation interface.
 **
 ** This function initializes the symmetrical decryption computation interface such that a
 ** service can be requested with Csm_SymDecryptStart().
 **/
extern FUNC(void, CSM_CODE) Csm_SymDecryptInit
(
  void
);
 ...
 **/
extern VAR(boolean, CSM_VAR) Csm_SymDecryptIsRunning;
 ...
 **/
extern VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_SymDecryptCurrentConfigId;

/** \brief Synchronous mode callback notification return value
 **/
#if (CSM_SYMDECRYPT_SYNCJOBPROCESSING_ENABLED == STD_ON)
extern volatile VAR(Csm_ReturnType, CSM_VAR) Csm_SymDecrypt_Ret_CallbackNotification;
...
#endif /* #if (CSM_SYMDECRYPT_ENABLED == STD_ON) */

#endif /* #ifndef CSM_SYMDECRYPT_H */
