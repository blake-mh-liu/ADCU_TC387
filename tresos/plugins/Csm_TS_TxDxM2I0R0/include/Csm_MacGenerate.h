...
#ifndef CSM_MACGENERATE_H
#define CSM_MACGENERATE_H
 ...
 **
 ** This function requests the start of an MAC generate computation. The start function of the
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateStart
(
 ...
 **
 ** This function requests the update of the MAC generate computation for the given data. The
 ...
 ** \retval  CSM_E_BUSY    request failed, service is still busy
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateUpdate
(
          Csm_ConfigIdType                            cfgId,
 ...
 **
 ** This function requests the finishing of the MAC generate computation and the storing of the
 ...
 **                                      the result of the MAC generation. If the result does
 **                                      not fit into the given buffer, and truncation is
 ...
 **/
extern FUNC(Csm_ReturnType, CSM_CODE) Csm_MacGenerateFinish
(
        Csm_ConfigIdType                            cfgId,
...
/** \brief  Used by the mac generate primitive to indicate finished computation.
 **
 ** This function has to be called by the mac generate primitive to indicate that it has
 ...
 **                      the user.
 **/
extern FUNC(void, CSM_CODE) Csm_MacGenerateCallbackNotification
(
  Csm_ReturnType result
);

/** \brief  Used by the mac generate primitive to indicate end of service.
 **
 ** This function has to be called by the mac generate primitive to indicate that it has
 ** completely finished the currently computed service. This function then sets the mac
 ** generate computation state machine to the idle state.
 **/
extern FUNC(void, CSM_CODE) Csm_MacGenerateServiceFinishNotification
(
  void
);
 ...
 ** by the service functions. The function calls the main function of the configured primitive
 ** to perform the tasks.
 **/
extern FUNC(void, CSM_CODE) Csm_MacGenerateMainFunction
(
  void
);
...
/** \brief  Initialization of the mac generate computation interface.
 **
 ** This function initializes the mac generate computation interface such that a service can be
 ** requested with Csm_MacGenerateStart().
 **/
extern FUNC(void, CSM_CODE) Csm_MacGenerateInit
(
  void
);
 ...
 **/
extern VAR(boolean, CSM_VAR) Csm_MacGenerateIsRunning;
 ...
 **/
extern VAR(Csm_ConfigIdType, CSM_VAR_NOINIT) Csm_MacGenerateCurrentConfigId;

/** \brief Synchronous mode callback notification return value
 **/
#if (CSM_MACGENERATE_SYNCJOBPROCESSING_ENABLED == STD_ON)
extern volatile VAR(Csm_ReturnType, CSM_VAR) Csm_MacGenerate_Ret_CallbackNotification;
...
#endif /* #if (CSM_MACGENERATE_ENABLED == STD_ON) */

#endif /* #ifndef CSM_MACGENERATE_H */
