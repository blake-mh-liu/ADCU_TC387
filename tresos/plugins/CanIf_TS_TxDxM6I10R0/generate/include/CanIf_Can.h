...
/** \brief This function is called by CAN driver and reports CANIF_SEV_ERRORSTATE_PASSIVE to the IdsM
 **
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC (void, CANIF_CODE) CanIf_ControllerErrorStatePassive
  (
    uint8 ControllerId,
...
/** \brief This function is called by CAN driver and reports CANIF_SEV_TX_ERROR_DETECTED or CANIF_SEV_RX_ERROR_DETECTED to the IdsM
 **
 ** \param[in] ControllerId        Abstracted CanIf ControllerId which is assigned to a
    **                             CAN controller.
 ** \param[in] Can_ErrorType       Reported CAN error
 **
 ** \ServiceId{0x50}
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
extern FUNC (void, CANIF_CODE) CanIf_ErrorNotification
  (
    uint8 ControllerId,
    Can_ErrorType Can_Error
  );
[!ENDIF!] /*CanIfPublicCfg/CanIfEnableSecurityEventReporting = 'true'*/
