...
/* !LINKSTO CanIf.EB.CanIfBusAHeaderFile_Inclusion,1 */
[!IF "node:exists(CanIfPublicCfg/CanIfBusATxIndication) or node:exists(CanIfPublicCfg/CanIfBusARxIndication)"!]
#include <[!"CanIfPublicCfg/CanIfBusAHeaderFile"!].h>    /* Bus Adapter support */
[!ENDIF!]
/*==================[macros]=================================================*/

/** \brief switch reports that PbcfgM uses CanIf */
[!WS "3"!][!IF "node:contains(util:distinct(node:refs(as:modconf('PbcfgM')/PbcfgMBswModules/*/PbcfgMBswModuleRef)), as:modconf('CanIf')) = 'true'"!]
...
/** \brief Multicore wakeup support */
#if (defined CANIF_MULTICORE_WAKEUP_SUPPORT)
#error CANIF_MULTICORE_WAKEUP_SUPPORT already defined!
#endif
#define CANIF_MULTICORE_WAKEUP_SUPPORT         [!WS "3"!][!IF "CanIfPublicCfg/CanIfMulticoreWakeupExtension = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
/** \brief Support of Global Time APIs used when hardware timestamping is supported.
 **
 ** This constant defines whether Global Time APIs are supported.
 ** STD_ON    Global Time APIs are provided
 ** STD_OFF   Global Time APIs are not provided
 */
#if (defined CANIF_GLOBAL_TIME_APIS)
#error CANIF_GLOBAL_TIME_APIS already defined!
#endif /* #if( defined CANIF_GLOBAL_TIME_APIS)*/
#define CANIF_GLOBAL_TIME_APIS          [!WS "3"!][!IF "$canIfGlobalTimeSupport"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
#define CANIF_NM_UPPER_LAYER_INDEX         [!WS "3"!][!IF "(CanIfPublicCfg/CanIfPublicWakeupCheckValidByNM = 'true') and (count(CanIfUpperLayerConfig/*[as:name(.) = 'CAN_NM']) > 0)"!][!"num:i(node:pos(CanIfUpperLayerConfig/*[as:name(.) = 'CAN_NM']))"!]U[!ELSE!]CANIF_NO_UPPER_LAYER_CALLBACK[!ENDIF!]
...
/* !LINKSTO CanIf.EB.CanIfBusATxIndication_GenerateAPI,1 */
[!IF "node:exists(CanIfPublicCfg/CanIfBusATxIndication)"!]
/** \def CANIF_BUS_ADAPTER_TX_SUPPORT
**
** \brief Switch for Bus Adapter Tx extension support.
**
** This switch defines if Bus Adapter Tx extension support is enabled.
*/
#if (defined CANIF_BUS_ADAPTER_TX_SUPPORT)
#error CANIF_BUS_ADAPTER_TX_SUPPORT already defined!
#endif /* #if( defined CANIF_BUS_ADAPTER_TX_SUPPORT)*/
#define CANIF_BUS_ADAPTER_TX_SUPPORT        STD_ON

/** \def CANIF_BUSA_TX_INDICATION(CanId)
**
** \brief Tx Bus Adapter support.
**
** This is a macro for Tx Bus Adapter configured API.
**
** \param[in] txPduId, canId, pduInfo, channelId
**
** \param[out] retVal TRUE or FALSE
*/
#define CANIF_BUSA_TX_INDICATION(txPduId, canId, pduInfo, channelId, retVal)                  \
do                                                                                            \
{                                                                                             \
  retVal = [!"CanIfPublicCfg/CanIfBusATxIndication"!](txPduId, canId, pduInfo, channelId);    \
}while(0U)
[!ELSE!]
/** \def CANIF_BUS_ADAPTER_TX_SUPPORT
**
...
#error CANIF_BUS_ADAPTER_TX_SUPPORT already defined!
#endif /* #if( defined CANIF_BUS_ADAPTER_TX_SUPPORT)*/
#define CANIF_BUS_ADAPTER_TX_SUPPORT        STD_OFF
[!ENDIF!]

/* !LINKSTO CanIf.EB.CanIfBusARxIndication_GenerateAPI,1 */
[!IF "node:exists(CanIfPublicCfg/CanIfBusARxIndication)"!]
/** \def CANIF_BUS_ADAPTER_RX_SUPPORT
**
** \brief Switch for Bus Adapter Rx extension support.
**
** This switch defines if Bus Adapter Rx extension support is enabled.
*/
#if (defined CANIF_BUS_ADAPTER_RX_SUPPORT)
#error CANIF_BUS_ADAPTER_RX_SUPPORT already defined!
#endif /* #if( defined CANIF_BUS_ADAPTER_RX_SUPPORT)*/
#define CANIF_BUS_ADAPTER_RX_SUPPORT        STD_ON

/** \def CANIF_BUSA_RX_INDICATION(CanId)
**
** \brief Rx Bus Adapter support.
**
** This is a macro for Rx Bus Adapter configured API.
**
** \param[in] rxPduId, canId, pduInfo, channelId
**
** \param[out] retVal TRUE or FALSE
*/
#define CANIF_BUSA_RX_INDICATION(rxPduId, canId, pduInfo, channelId, retVal)                       \
do                                                                                                 \
{                                                                                                  \
  retVal = [!"CanIfPublicCfg/CanIfBusARxIndication"!](rxPduId, canId, pduInfo, channelId);         \
}while(0U)
[!ELSE!]
/** \def CANIF_BUS_ADAPTER_RX_SUPPORT
**
...
#error CANIF_BUS_ADAPTER_RX_SUPPORT already defined!
#endif /* #if( defined CANIF_BUS_ADAPTER_RX_SUPPORT)*/
#define CANIF_BUS_ADAPTER_RX_SUPPORT        STD_OFF
[!ENDIF!]
...
#if (defined CANIF_CTRL_SIZE_PART)
#error CANIF_CTRL_SIZE_PART already defined!
#endif /*if (defined CANIF_CTRL_SIZE_PART)*/
#define CANIF_CTRL_SIZE_PART [!"$canIfTxMirrorBufCtrlNum"!]U
[!ENDIF!]

[!IF "(CanIfPublicCfg/CanIfMultiCoreSupport = 'false') or ((CanIfPublicCfg/CanIfMultiCoreSupport = 'true') and (count(CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*) > count(node:refs(CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*/CanIfCtrlCanCtrlRef)/CanControllerEcucPartitionRef)))"!]
#if (defined TX_MIRROR_BUFFER_SIZE)
#error TX_MIRROR_BUFFER_SIZE already defined!
#endif /*#if (defined TX_MIRROR_BUFFER_SIZE)*/
#define TX_MIRROR_BUFFER_SIZE [!"$canIfTxMirrorSize"!]U

#if (defined TX_PAYLOAD_BUFFER_SIZE)
#error TX_PAYLOAD_BUFFER_SIZE already defined!
#endif /*if (defined TX_PAYLOAD_BUFFER_SIZE)*/
#define TX_PAYLOAD_BUFFER_SIZE [!"$canIfTxMirrorBufSize"!]U
[!ENDIF!][!/* (CanIfMultiCoreSupport = 'false') or ((CanIfMultiCoreSupport = 'true') and (count(../CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*) > count(node:refs(../CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*/CanIfCtrlCanCtrlRef)/CanControllerEcucPartitionRef)))*/!]
...
#define CANIF_ENABLE_SECURITY_EVENT_REPORTING     [!WS "3"!][!IF "CanIfPublicCfg/CanIfEnableSecurityEventReporting = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

[!IF "CanIfPublicCfg/CanIfEnableSecurityEventReporting = 'true'"!]
 
#if (defined CANIF_SEV_ERRORSTATE_BUSOFF)
#error CANIF_SEV_ERRORSTATE_BUSOFF already defined!
#endif /* #if( defined CANIF_SEV_ERRORSTATE_BUSOFF)*/
[!IF "node:refvalid(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_ERRORSTATE_BUSOFF)"!]
#define CANIF_SEV_ERRORSTATE_BUSOFF [!"node:value(node:ref(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_ERRORSTATE_BUSOFF)/IdsMInternalEventId)"!]U
[!ELSE!]
#define CANIF_SEV_ERRORSTATE_BUSOFF 0xFFU
[!ENDIF!]

#if (defined CANIF_SEV_ERRORSTATE_PASSIVE)
#error CANIF_SEV_ERRORSTATE_PASSIVE already defined!
#endif /* #if( defined CANIF_SEV_ERRORSTATE_PASSIVE)*/
[!IF "node:refvalid(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_ERRORSTATE_PASSIVE)"!]
#define CANIF_SEV_ERRORSTATE_PASSIVE [!"node:value(node:ref(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_ERRORSTATE_PASSIVE)/IdsMInternalEventId)"!]U
[!ELSE!]
#define CANIF_SEV_ERRORSTATE_PASSIVE 0xFFU
[!ENDIF!]

#if (defined CANIF_SEV_RX_ERROR_DETECTED)
#error CANIF_SEV_RX_ERROR_DETECTED already defined!
#endif /* #if( defined CANIF_SEV_RX_ERROR_DETECTED)*/
[!IF "node:refvalid(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_RX_ERROR_DETECTED)"!]
#define CANIF_SEV_RX_ERROR_DETECTED [!"node:value(node:ref(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_RX_ERROR_DETECTED)/IdsMInternalEventId)"!]U
[!ELSE!]
#define CANIF_SEV_RX_ERROR_DETECTED 0xFFU
[!ENDIF!]

#if (defined CANIF_SEV_TX_ERROR_DETECTED)
#error CANIF_SEV_TX_ERROR_DETECTED already defined!
#endif /* #if( defined CANIF_SEV_TX_ERROR_DETECTED)*/
[!IF "node:refvalid(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_TX_ERROR_DETECTED)"!]
#define CANIF_SEV_TX_ERROR_DETECTED [!"node:value(node:ref(as:modconf('CanIf')[1]/CanIfPublicCfg/CanIfSecurityEventRefs/CANIF_SEV_TX_ERROR_DETECTED)/IdsMInternalEventId)"!]U
[!ELSE!]
#define CANIF_SEV_TX_ERROR_DETECTED 0xFFU
[!ENDIF!]
...
#define CANIF_CFG_SIGNATURE [!"asc:calculateConfigSignature(as:modconf('CanIf')[1], 'PreCompile')"!]UL
...
#define CANIF_PUBLIC_INFO_SIGNATURE  [!"asc:calculateConfigSignature(as:modconf('CanIf')[1]/CommonPublishedInformation, 'PublishedInformation', as:modconf('CanIf')[1]/CommonPublishedInformation/Release)"!]U
