...
#if (defined CANIF_SID_ERRORNOTIFICATION)
#error CANIF_SID_ERRORNOTIFICATION already defined !
#endif
#define CANIF_SID_ERRORNOTIFICATION 0x50U

#if (defined CANIF_SID_CONTROLLERERRORSTATEPASSIVE)
#error CANIF_SID_CONTROLLERERRORSTATEPASSIVE already defined !
#endif
#define CANIF_SID_CONTROLLERERRORSTATEPASSIVE 0x4fU

#if (defined CANIF_CAN_ASR21_11_COMPATIBILITY)
#if (CANIF_GLOBAL_TIME_APIS == STD_ON)

#if (defined CANIF_SID_GETCURRENTTIME)
#error CANIF_SID_GETCURRENTTIME already defined !
#endif
#define CANIF_SID_GETCURRENTTIME           0x51U

#if (defined CANIF_SID_ENABLEEGRESSTIMESTAMP)
#error CANIF_SID_ENABLEEGRESSTIMESTAMP already defined !
#endif
#define CANIF_SID_ENABLEEGRESSTIMESTAMP    0x52U

#if (defined CANIF_SID_GETEGRESSTIMESTAMP)
#error CANIF_SID_GETEGRESSTIMESTAMP already defined !
#endif
#define CANIF_SID_GETEGRESSTIMESTAMP       0x53U

#if (defined CANIF_SID_GETINGRESSTIMESTAMP)
#error CANIF_SID_GETINGRESSTIMESTAMP already defined !
#endif
#define CANIF_SID_GETINGRESSTIMESTAMP      0x54U
...
/** \brief CANIF_E_PARAM_CAN_ERROR */
#if (defined CANIF_E_PARAM_CAN_ERROR)
#error CANIF_E_PARAM_CAN_ERROR already defined!
#endif /* #if( defined CANIF_E_PARAM_CAN_ERROR)*/
...
/** \brief CANIF_E_PDU_INSTANCE_LOST */
#if (defined CANIF_E_PDU_INSTANCE_LOST)
#error CANIF_E_PDU_INSTANCE_LOST already defined!
#endif /* #if( defined CANIF_E_PDU_INSTANCE_LOST)*/
#define CANIF_E_PDU_INSTANCE_LOST           70U
...
/** \brief Check for Can driver wakeup
 **
 ** This function is used to check whether an underlying CAN driver signals a wakeup event.
 **
 ** \param[in] ControllerId - Id of Can controller that has reported an wakeup event
 **
 ** \return Result of Can_CheckWakeup()
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
#if(( CANIF_CAN_WAKEUP_SUPPORT == STD_ON ) && (CANIF_MULTICORE_WAKEUP_SUPPORT == STD_OFF))
...
extern FUNC( Can_ReturnType, CANIF_CODE ) CanIf_CheckDriverWakeup
#else
extern FUNC( Std_ReturnType, CANIF_CODE ) CanIf_CheckDriverWakeup
#endif
    (
        uint8 ControllerId
    );
...
/** \brief Check for Can Transceiver driver wakeup
 **
 ** This is a function that calls CanTrcv_CB_WakeupByBus() function.
 **
 ** \param[in] Transceiver - Id of Transceiver controller that has reported an wakeup event
 **
 ** \return Result of CanTrcv_CB_WakeupByBus()
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
#if(( CANIF_CANTRCV_WAKEUP_SUPPORT == STD_ON ) && (CANIF_MULTICORE_WAKEUP_SUPPORT == STD_OFF))
extern FUNC( Std_ReturnType, CANIF_CODE ) CanIf_CheckTrcvWakeup
  (
    uint8 Transceiver
  );
#endif
...
/** \brief This service calls the corresponding CAN Driver service to retrieve the current time value out of
**   the HW registers.
 **
 ** \param[in] Controller Index of the addresses CAN controller.
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetCurrentTime
  (
    uint8 Controller,
...
/** \brief This service calls the corresponding CAN Driver service to activate egress time stamping on
 ** a dedicated message object.
 **
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC (void, CANIF_CODE) CanIf_EnableEgressTimeStamp
  (
    PduIdType TxPduId
  );


/** \brief This service calls the corresponding CAN Driver service to read back the egress time stamp on
 ...
 **
 ** \ServiceId{0x53}
 ** \Reentrancy{Non Reentrant for the same TxPduId, Reentrant for different TxPduIds}
 ** \Synchronicity{Synchronous}
 */
extern FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetEgressTimeStamp
  (
    PduIdType TxPduId,
    P2VAR( Can_TimeStampType, AUTOMATIC, CANIF_APPL_DATA ) timeStampPtr
  );

/** \brief This service calls the corresponding CAN Driver service to reads back the ingress time stamp
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetIngressTimeStamp
  (
    PduIdType RxPduId,
    P2VAR( Can_TimeStampType, AUTOMATIC, CANIF_APPL_DATA ) timeStampPtr
  );
