...
/** \brief Invalid notification index constant
 **
 ** This constant defines an index value which is interpreted as "invalid
 ** index".
 */
#if (defined CANIF_INVALID_NOTIFY_INDEX)
#error CANIF_INVALID_NOTIFY_INDEX already defined!
#endif /* #if( defined CANIF_INVALID_NOTIFY_INDEX)*/
#define CANIF_INVALID_NOTIFY_INDEX              0xFFFFU
 ...
 **
 ** This constant is used to mark that a payload slot is empty and available.
 */
#if (defined CANIF_MIRR_BUF_EMPTY)
#error CANIF_MIRR_BUF_EMPTY already defined !
#endif
#define CANIF_MIRR_BUF_EMPTY                  0U
...
#if (CANIF_MULTICORE_WAKEUP_SUPPORT == STD_ON)

#if( CANIF_CAN_WAKEUP_SUPPORT == STD_ON )
/** \brief Configuration of Schm_Call_CAN driver(s) function pointers
 **
 ** This array contains the function pointers for all the interfaces
 ** of the Schm_Call CAN driver(s) wakeup.
 */
extern CONST( SchM_CanIf_CheckWakeupFctPtrType, CANIF_APPL_CONST )
    CanIf_CallSchM_CanControllerWakeup[];
#endif

#if (CANIF_TRCV_SUPPORT == STD_ON)
/** \brief Configuration of Schm_Call_CAN transceiver(s) function pointers
 **
 ** This array contains the function pointers for all the interfaces
 ** of the Schm_Call CAN transceiver(s) wakeup.
 */
extern CONST( SchM_CanIf_CBWakeupByBusFctPtrType, CANIF_APPL_CONST )
    CanIf_CallSchM_CanTrcvWakeup[];
#endif
...
#if (( CANIF_BUS_MIRRORING_SUPPORT == STD_ON ) && (CANIF_MULTICORE_SUPPORT == STD_ON))
extern CONST(CanIf_CtrlMirrorBuffType, CANIF_APPL_CONST) CanIf_TxMirrBuffPtr[];
#endif
...
#if ((CANIF_MULTICORE_SUPPORT == STD_OFF) || (CANIF_MULTICORE_SUPPORT == STD_ON) && (CANIF_CTRL_SIZE > CANIF_CTRL_SIZE_PART))
...
#endif /* ((CANIF_MULTICORE_SUPPORT == STD_OFF) || (CANIF_MULTICORE_SUPPORT == STD_ON) && (CANIF_CTRL_SIZE > CANIF_CTRL_SIZE_PART)) */
...
#if (CANIF_DECOUPLED_RXPROCESSING_SUPPORT == STD_ON)
/** \brief Array of Rx queue total number of payload bytes stored */
extern VAR( uint16, CANIF_VAR ) CanIf_MFRxQueueLoad[];
#endif /* CANIF_DECOUPLED_RXPROCESSING_SUPPORT */
 ...
 */
extern VAR( CanIf_MfTxTblRWIdxType, CANIF_VAR )
  CanIf_MfTxTblRWIdx[];
 ...
 */
extern VAR( CanIf_MfRxTblRWIdxType, CANIF_VAR )
  CanIf_MfRxTblRWIdx[];
