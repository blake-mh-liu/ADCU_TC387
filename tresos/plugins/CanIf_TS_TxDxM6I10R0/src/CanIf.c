 ...
 *
 *
 *  MISRAC2012-2) Deviated Rule 10.6 (Required)
 *    The value of a composite expression shall not be assigned to an object with wider
 *    essential type.
 *
 *    Reason:
 *    Can be ignored since copying a variable length data to array of unsigned chars.
 *
 */
 ...
 *
 * TASKING-3) Deviated Rule: W560
 * W560: possible truncation at implicit conversion to type "unsigned char"
 *
 * Reason: False positive due to different return type of CanIf_CheckTrcvWakeup() and CanIf_CheckDriverWakeup()
 ...
 *
 * Reason: For performance reasons variable shall be given to the upper layer without copying its
 *         content, which demands an explicit cast.
 */
/*==================[inclusions]=============================================*/
...
/* patch version check */
#if (CANIF_SW_PATCH_VERSION < 40U)
#error CANIF_SW_PATCH_VERSION wrong (< 40U)
#endif
 ...
 ** configured mirror buffer and payload buffers.
 **
 ...
 ** \param[in] CanIfTxPduId        Identifier of the PDU to be transmitted
 ** \param[in] ControllerId        Identifier of the corresponding Controller
 ** \param[in] skipPduCopy         States if the PDU information from the buffer
 **                                is copied into the provided pointer or not
 ...
 ** configured buffer and payload buffers or a NULL pointer otherwise.
 ** \param[in] ControllerId        Identifier of the corresponding Controller.
 ** \param[in] CanIfTxPduId        Identifier of the PDU to be found.
 ** \param[in] NodeIdxPtr          Index of the Mirror entry of the requested CanTxPduId found in the buffer.
 ...
 ** \param[in] controllerId       Identifier of the corresponding Controller.
 ** \param[in] PduInfoPtr         Pointer to the received PDU (data and length).
 ** \param[in] PayloadIdxPtr      Pointer to the PDU index.
...
(
    uint8 controllerId,
    P2CONST( Can_PduType, AUTOMATIC, CANIF_APPL_DATA ) PduInfoPtr,
    P2VAR( PduIndexType, AUTOMATIC, CANIF_APPL_DATA ) PduIdxPtr
);
 ...
 ** \param[in] PduInfoPtr          Pointer to the received PDU (data and length).
 ** \param[in] CanIfTxPduId        Identifier of the PDU to be found.
 ** \param[in] pduIdx              Index where to start storing the pdu information.
 **
 ** \param[out] PduInfoPtr         Pointer to the received PDU (data and length).
 */
STATIC FUNC(void, CANIF_CODE ) CanIf_AddedPduInMirrorBuff
(
    uint8 controllerId,
    P2CONST( Can_PduType, AUTOMATIC, CANIF_APPL_DATA ) PduInfoPtr,
    PduIdType CanIfTxPduId,
  ...
  )
{
...
#endif /* CANIF_DEV_ERROR_DETECT */
  {
    ...
    for( i = CanIf_ConfigPtr->nrOfStaticTxLPdus;
         ...
         i++ )
    {
      /* !LINKSTO CanIf.SWS_CanIf_00857,1 */
    ...
    for( i = 0U; i < CANIF_GET_NR_OF_CAN_CTRLS(); i++ )
    {
      ...
      CanIf_CanControllerFlags[i] = CANIF_PN_FILTER_ACTIVE_MASK;         /* CanIf.SWS_CanIf_00863: enable PnTxFilter during initialization */
  ...
  )
{
  ...
  Can_IdType BusA_PduId = 0U;  /* This initial value will not be used since id is written before being read.
                                  Only used to avoid false positive MISRA violation. */
            ...
            if( CanTxPduId >= CanIf_ConfigPtr->nrOfStaticTxLPdus )
            {
              ...
              if(CanIf_TxPduConfigPtr[CanTxPduId].getMetadataPduId != CANIF_INVALID_LPDU_INDEX)
              {
                ...
                if(CanIf_TxPduConfigPtr[CanTxPduId].canId != CANIF_INVALID_CANID)
                {
                  ...
                  /* !LINKSTO CanIf.SWS_CanIf_00854,1 */
                  /* !LINKSTO CanIf.SWS_CanIf_00844,1 */
                ...
                else
                {
                  ...
                  /* !LINKSTO CanIf.SWS_CanIf_00855,1 */
...
#if( CANIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
            BusA_PduId = PduInfo.id;  /* this initial value will not be used since id is written before being read */
#endif /* CANIF_BUS_ADAPTER_TX_SUPPORT */
...
#if( CANIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
        CANIF_BUSA_TX_INDICATION(CanTxPduId, BusA_PduId, PduInfoPtr, ControllerId, ret_Val);

        if(ret_Val)
#endif /* CANIF_BUS_ADAPTER_TX_SUPPORT */
        {
          /* Call Can_Write() to transmit the Can message */
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
          ...
          if( CanRetVal != (Can_ReturnType)CAN_OK )
          {
          ...
          else
          {
            ...
            if(CANIF_BUS_MIRRORING_MASK == (CanIf_CanControllerFlags[ControllerId] & CANIF_BUS_MIRRORING_MASK))
            {
              ...
              if(E_NOT_OK != CanIf_FindNodeByID(ControllerId, CanTxPduId, &PduBuffIdx))
              {
               ...
               (void)CanIf_RemovePduMirrBuf(CanTxPduId, ControllerId, TRUE, &MirrPduInfo);
               (void)CanIf_InsertPduMirrBuf(ControllerId,CanTxPduId,&PduInfo);
              }
              else
              {
  ...
  ( PduIdType CanRxPduId )
{
  CanIf_NotifStatusType RetVal = CANIF_NO_NOTIFICATION;
                                                    /* return value */
#if( CANIF_MULTICORE_SUPPORT == STD_ON )
  P2CONST(CanIf_LPduIndexType, AUTOMATIC, CANIF_APPL_DATA) NotifIdxAry;   /* notification idx. */
...
#if( CANIF_MULTICORE_SUPPORT == STD_ON )
    uint8 CtrlIdx; /* loop variable */
#if( CANIF_DEV_ERROR_DETECT == STD_ON )
    boolean validIndex = FALSE;
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
    NotifIdxAry = CanIf_RxPduConfigPtr[CanRxPduId].notifyIndex;

    /* Deviation TASKING-1 */
    for(CtrlIdx = 0U; CtrlIdx < CANIF_CTRL_SIZE; CtrlIdx++)
    {
      /* configuration of L-PDU CanRxPduId */
      if( NotifIdxAry[CtrlIdx] != CANIF_INVALID_NOTIFY_INDEX )
      {
        /* --- read and reset notification ------------------------------------- */
        ...
        if( CANIF_GET_NOTIFICATION_FLAG( CanIf_ControllerRxLPduNotifyFlag[CtrlIdx].RxLPduNotifyFlag, NotifIdxAry[CtrlIdx] ) != 0U )
        {
          RetVal = CANIF_TX_RX_NOTIFICATION;
        }

        /* clear receive notification flag */
        CANIF_CLEAR_NOTIFICATION_FLAG( CanIf_ControllerRxLPduNotifyFlag[CtrlIdx].RxLPduNotifyFlag, NotifIdxAry[CtrlIdx] );
...
#if( CANIF_DEV_ERROR_DETECT == STD_ON )
    if( NotifIdx == CANIF_INVALID_NOTIFY_INDEX )
    {
      /* report DET error */
      CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                              CANIF_INSTANCE_ID,
  ...
  )
{
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    /* !LINKSTO CanIf.SWS_CanIf_00849, 1 */
...
/** \brief Check for Can driver wakeups */

#if( CANIF_CAN_WAKEUP_SUPPORT == STD_ON )

#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
FUNC( Can_ReturnType, CANIF_CODE ) CanIf_CheckDriverWakeup
#else
FUNC( Std_ReturnType, CANIF_CODE ) CanIf_CheckDriverWakeup
#endif
  (
    uint8 ControllerId
  )
...
#endif
  RetVal = CanIf_CanDrvFctConfig[CANIF_GET_CAN_DRIVER_IDX(ControllerId)].CheckWakeup( CANIF_GET_CANDRV_CTRL_ID(ControllerId));
  return RetVal;
}
#endif
...
#if( CANIF_CANTRCV_WAKEUP_SUPPORT == STD_ON )
/** \brief Check for Can driver wakeups */
FUNC( Std_ReturnType, CANIF_CODE ) CanIf_CheckTrcvWakeup
  (
    uint8 Transceiver
  )
{
  Std_ReturnType RetVal;
  RetVal = CanIf_CanTrcvConfig[CANIF_CFG_GET_TRCV_CFG(Transceiver).trcvDrvIdx].chkWakeupByBus( CANIF_CFG_GET_TRCV_CFG(Transceiver).trcvChnlId );
  return RetVal;
}
#endif
  ...
  )
{
...
#endif /* (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY) */
...

  {
    ...
    for( i = 0U; i < CANIF_GET_NR_OF_CAN_CTRLS(); i++ )
    {
      ...
      if(CheckWakeUp == TRUE)
      {
#if (CANIF_MULTICORE_WAKEUP_SUPPORT == STD_OFF)
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
        /* Deviation TASKING-3 */
        if(CanIf_CheckDriverWakeup(i) == (Can_ReturnType)CAN_OK )
#else
        /* Deviation TASKING-3 */
        if(CanIf_CheckDriverWakeup(i) == (Std_ReturnType)CAN_OK )
#endif
#else
        /* !LINKSTO CanIf.EB.CanIfMulticoreWakeupExtension_WakeupCanDriver,1 */
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
        Can_ReturnType Return_val;
#else
        Std_ReturnType Return_val;
#endif
        /* Deviation TASKING-3 */
        (void)CanIf_CallSchM_CanControllerWakeup[i](&Return_val,i);
        if(Return_val == (Can_ReturnType)E_OK )
#endif
        {
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
          /* enter critical section */
...
#if (CANIF_MULTICORE_WAKEUP_SUPPORT == STD_OFF)
          /* Deviation TASKING-3 */
          if( CanIf_CheckTrcvWakeup(i) == E_OK )
#else
          /* !LINKSTO CanIf.EB.CanIfMulticoreWakeupExtension_WakeupCanTransceiver,1 */
          Std_ReturnType Ret_val;
          /* Deviation TASKING-3 */
          (void)CanIf_CallSchM_CanTrcvWakeup[i](&Ret_val,i);
          if(Ret_val == E_OK )
#endif
          {
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
  ...
  else
  {
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
    {
     ...
     ( CANIF_PUBLIC_PN_SUPPORT == STD_ON )                      \
      ...
      {
        /* !LINKSTO CanIf.SWS_CanIf_00905.Provision.CANID,1 */
        /* !LINKSTO CanIf.SWS_CanIf_00905.Provision.Payload,1 */
  ...
  else
  {
...
#endif /* (!defined CANIF_CAN_ASR402_COMPATIBILITY) && (!defined CANIF_CAN_ASR403_COMPATIBILITY) */
    {
...
#endif /* (CANIF_DEV_ERROR_DETECT == STD_ON) */
      {
        ...
        {
          /* !LINKSTO CanIf.SWS_CanIf_00906.Provision.CANID,1 */
          /* !LINKSTO CanIf.SWS_CanIf_00906.Provision.Payload,1 */
          ...
          if(Index != CANIF_INVALID_LPDU_INDEX)
          {
            ...
            RxPduInfo.SduDataPtr = (P2VAR(uint8, AUTOMATIC, AUTOSAR_COMSTACKDATA)) CanSduPtrLcl;
              ...
              if (CANIF_CFG_GET_SORTEDRXLPDU_CFG(Index).setMetadataPduId != CANIF_INVALID_LPDU_INDEX)
              {
                ...
                (void)EcuC_SetMetaDataCanId(CANIF_CFG_GET_SORTEDRXLPDU_CFG(Index).setMetadataPduId, &MetadataPduInfo, ConfCanId);
              }
#endif  /* CANIF_METADATA_SUPPORT == STD_ON */

            {
              ...
              {
                /* add Pdu to queue */
                if(E_NOT_OK != CanIf_InsertRxQueue(&RxPduInfo, ConfCanId, PduIdx, ControllerId))
                {
                  /* forward the call to SchM */
                  /* !LINKSTO CanIf.EB.DecoupledProcessing.TriggerMainFunc,1 */
...
)
{
  ...
  if( CanIf_State == CANIF_INITIALIZED )
  {
    ...
    while( 0U != CanIf_TotalRxQueueSize[Idx] )
    {
      ...
      i = CanIf_MfRxTblRWIdx[Idx].RxQueueReadIdx + StartIdx;

      rxQueue = CanIf_RxPduQueue[i];

      CanIf_MFRxQueueLoad[Idx] -= rxQueue.PayloadLen;
      ...
      CanIf_MfRxTblRWIdx[Idx].RxQueueReadIdx = (CanIf_MfRxTblRWIdx[Idx].RxQueueReadIdx + 1U) % MfTbl.RxQueueSize;
...
)
{
  ...
  if( CanIf_State == CANIF_INITIALIZED )
  {
    ...
    while( 0U != CanIf_TotalTxQueueSize[Idx] )
    {
      ...
      i = StartIdx + CanIf_MfTxTblRWIdx[Idx].TxQueueReadIdx;
      ...
      CanIf_MfTxTblRWIdx[Idx].TxQueueReadIdx = (CanIf_MfTxTblRWIdx[Idx].TxQueueReadIdx + 1U) % MfTbl.TxQueueSize;
...
FUNC( void, CANIF_CODE ) CanIf_ControllerBusOff( uint8 Controller )
{
     ...
     ( CANIF_PUBLIC_TXCONFIRM_RESULT_SUPPORTED == STD_ON ))
      {
        ...
        {
#if (CANIF_SINGLE_CAN_CTRL_OPT ==STD_OFF)
          const uint8 ctrlId = CanIf_TxPduConfigPtr[txPduId].intCtrlIdx;                     /* Controller index */
#else
          const uint8 ctrlId = CANIF_SINGLE_CAN_CTRL_ID;                                     /* Controller index */
#endif
          PduIdType pduIdDiv8 = txPduId / 8U;
          uint8  pduIdMod8 = (uint8)(txPduId % 8U);
          if ((ctrlId == Controller) && (0U != (CanIf_BitMaskByte[pduIdMod8] & CanIf_TxPduNoConf[pduIdDiv8])))
          {
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00918,1 */
     ...
     ( CANIF_PUBLIC_TXCONFIRM_RESULT_SUPPORTED == STD_ON ))
      {
        ...
        for (txPduId = 0U; txPduId < CANIF_MAX_TX_PDUS; txPduId++)
        {
          ...
          PduIdType pduIdDiv8 = txPduId / 8U;
          uint8  pduIdMod8 = (uint8)(txPduId % 8U);
          if ((ctrlId == ControllerId) && (0U != (CanIf_BitMaskByte[pduIdMod8] & CanIf_TxPduNoConf[pduIdDiv8])))
          {
            /* Send Negative Tx Confirmation for pending buffered Tx Pdus */
            /* call Tx confirmation callback with E_NOT_OK */
            /* !LINKSTO CanIf.EB.User_TxConfirmation.Signature.Result.ON, 1 */
  ...
  switch( ControllerMode )
  {
    case CANIF_CS_STOPPED:
    {
      ...
      if(CanControllerMode == CANIF_CS_SLEEP)
      {
        ...
        if (CanIf_CanDrvFctConfig[canIdx].SetControllerMode( canCtrlId, CANIF_T_WAKEUP ) == (Can_ReturnType)CAN_OK)
        {
          RetVal = E_OK;
        }
      }
...
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
        if (CanIf_CanDrvFctConfig[canIdx].SetControllerMode( canCtrlId, CANIF_T_STOP ) == (Can_ReturnType)CAN_OK)
#else
        /* cast to uint8 is done because casting an enum type to another is raising warnings, even if the two types are compatible */
...
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
      if (CanIf_CanDrvFctConfig[canIdx].SetControllerMode( canCtrlId, CANIF_T_START ) == (Can_ReturnType)CAN_OK)
#else
      /* cast to uint8 is done because casting an enum type to another is raising warnings, even if the two types are compatible */
...
#if (!defined CANIF_CAN_ASR431_COMPATIBILITY) && (!defined CANIF_CAN_ASR440_COMPATIBILITY) && (!defined CANIF_CAN_ASR21_11_COMPATIBILITY)
      if (CanIf_CanDrvFctConfig[canIdx].SetControllerMode( canCtrlId, CANIF_T_SLEEP ) == (Can_ReturnType)CAN_OK)
#else
      /* cast to uint8 is done because casting an enum type to another is raising warnings, even if the two types are compatible */
    ...
    if ( doTransmit )
    {
      ...
      if( CanRetVal != CAN_BUSY )
      {
        ...
        /* Block delimited for Bus Mirroring */
        {
          ...
          if(CANIF_BUS_MIRRORING_MASK == (CanIf_CanControllerFlags[controllerId] & CANIF_BUS_MIRRORING_MASK))
          {
             (void)CanIf_InsertPduMirrBuf(controllerId,CanTxPduId, &CanPduInfo);
          }
        } /* end mirroring block */
#endif /* CANIF_BUS_MIRRORING_SUPPORT == STD_ON */
      }
  ...
  )
{
  ...
  const uint32 idx = (uint32)CanIf_MfTxTblRWIdx[MfIndex].TxQueueWriteIdx + MfTbl.TxQueueOffset;

  SchM_Enter_CanIf_SCHM_CANIF_EXCLUSIVE_AREA_0();

  if(CanIf_TxPduQueue[idx].MFIdx == CANIF_INVALID_MF_INDEX)
  {
    CanIf_TxPduQueue[idx].MFIdx = MfIndex;
    CanIf_TxPduQueue[idx].PduId = TxPduId;

    CanIf_TotalTxQueueSize[MfIndex]++;
    CanIf_MfTxTblRWIdx[MfIndex].TxQueueWriteIdx = (CanIf_MfTxTblRWIdx[MfIndex].TxQueueWriteIdx + 1U) % MfTbl.TxQueueSize;
  ...
  )
{
  ...
  const uint16 MfPayloadMaxVal = MfTbl.PayloadQueueSize;
  uint16 MfCurrentSduLength;
  uint16 MfCurrentLoad = CanIf_MFRxQueueLoad[MfIndex];

  SchM_Enter_CanIf_SCHM_CANIF_EXCLUSIVE_AREA_0();
  {
    CONSTP2VAR( CanIf_RxQueueType, AUTOMATIC, CANIF_APPL_DATA) RxPduQueue = &CanIf_RxPduQueue[CanIf_MfRxTblRWIdx[MfIndex].RxQueueWriteIdx + MfTbl.RxQueueOffset];

    MfCurrentSduLength = (uint16)PduInfoPtr->SduLength;

    if((RxPduQueue->MFIdx == CANIF_INVALID_MF_INDEX) &&
      (MfCurrentSduLength <= (MfPayloadMaxVal - MfCurrentLoad)))
    {
      uint16 QueueDataPos;
      uint8 idx;
      uint16 prevPduIdx = MfTbl.RxQueueOffset + (uint16)(((uint32)CanIf_MfRxTblRWIdx[MfIndex].RxQueueWriteIdx + ((uint32)MfTbl.RxQueueSize - 1U)) % (uint32)MfTbl.RxQueueSize);
                          /* instead of substracing one (to reach previous pdu index), add (size-1)%size to avoid negative values */

      if (CanIf_RxPduQueue[prevPduIdx].MFIdx == CANIF_INVALID_MF_INDEX)
      {
        RxPduQueue->PreviousQueuePos = MfTbl.PayloadQueueOffset;
      }
      else
      {
        RxPduQueue->PreviousQueuePos = (uint16)(((uint32)CanIf_RxPduQueue[prevPduIdx].PreviousQueuePos + CanIf_RxPduQueue[prevPduIdx].PayloadLen) % MfPayloadMaxVal);
      }

      RxPduQueue->PayloadLen = (uint8)PduInfoPtr->SduLength;

      /* write data */
      QueueDataPos = RxPduQueue->PreviousQueuePos;
      ...
      CanIf_MFRxQueueLoad[MfIndex] += idx;
      CanIf_TotalRxQueueSize[RxLPduConfig->MFIndex]++;
      CanIf_MfRxTblRWIdx[MfIndex].RxQueueWriteIdx = (CanIf_MfRxTblRWIdx[MfIndex].RxQueueWriteIdx + 1U) % MfTbl.RxQueueSize;
...
( CANIF_PUBLIC_PN_SUPPORT == STD_ON ))
      ...
      {
#if( CANIF_MULTICORE_SUPPORT == STD_ON )
        CanIf_LPduIndexType NotifyIdx = RxLPduConfig->notifyIndex[ControllerId]; /* notification index */
#else
      ...
      /* !LINKSTO CanIf.SWS_CanIf_00896, 1 */
      if((CanIf_CanControllerFlags[ControllerId] & (uint8)(CANIF_PN_FILTER_ACTIVE_MASK)) != 0U)
      {
        DBG_CANIF_CANCONTROLLERFLAGS_GRP(ControllerId,(CanIf_CanControllerFlags[ControllerId]),((CanIf_CanControllerFlags[ControllerId])&((uint8)(~CANIF_PN_FILTER_ACTIVE_MASK))));
        CanIf_CanControllerFlags[ControllerId] &= ~(uint8)(CANIF_PN_FILTER_ACTIVE_MASK);
      }
...
#if (CANIF_BUS_MIRRORING_SUPPORT == STD_ON)
/* !LINKSTO CanIf.SWS_CanIf_00911.Availability,1 */
...
)
{
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00912,1 */
  else if( ControllerId >= CANIF_GET_NR_OF_CAN_CTRLS() )
  {
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    /* !LINKSTO CanIf.SWS_CanIf_91005.ReturnValue,1 */
    retVal = E_OK;
  }
...
)
{
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00898,1 */
  else if( ControllerId >= CANIF_GET_NR_OF_CAN_CTRLS() )
  {
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00899,1 */
...
#endif /* CANIF_DET_REPORT_ERROR == STD_ON */
  {
    ...
    /* !LINKSTO CanIf.SWS_CanIf_91001.ReturnValue.ProperlySupported,1 */
    ...
    TS_PARAM_UNUSED(ControllerId);
#endif
    /* !LINKSTO CanIf.SWS_CanIf_91001.ReturnValue.NotSupportedByDriver,1 */
    retVal = E_OK;
...
)
{
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00909,1 */
  else if( ControllerId >= CANIF_GET_NR_OF_CAN_CTRLS() )
  {
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
  ...
  /* !LINKSTO CanIf.SWS_CanIf_00910,1 */
...
#endif
  {
    ...
    /* !LINKSTO CanIf.SWS_CanIf_91004.ReturnValue.ProperlySupported,1 */
    ...
    TS_PARAM_UNUSED(ControllerId);
#endif
    /* !LINKSTO CanIf.SWS_CanIf_91004.ReturnValue.NotSupportedByDriver,1 */
    retVal = E_OK;
...
)
{
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if((0x1U <=(uint8) Can_Error) && ((uint8)Can_Error <= 0x7U))
    {
#if( CANIF_SEV_TX_ERROR_DETECTED != CANIF_INVALID_SEV_REF )
      /* !LINKSTO CanIf.SWS_CanIf_00915,1 */
    ...
    else if ((0x8U <= (uint8)Can_Error) &&((uint8)Can_Error <= 0xBU))
    {
#if( CANIF_SEV_RX_ERROR_DETECTED != CANIF_INVALID_SEV_REF )
      /* !LINKSTO CanIf.SWS_CanIf_00916,1 */
...
)
{
...
#endif
  {
       ...
       ((RxErrorCounter > 127U) && (RxErrorCounter <= 255U)))
    {
      ...
      /* !LINKSTO CanIf.SWS_CanIf_00917.ErrorCounter_BoundaryValue,1 */
...
#if( CANIF_DEV_RUNTIME_ERROR_DETECT == STD_ON )
      /* !LINKSTO CanIf.SWS_CanIf_00900,1 */
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  Std_ReturnType RetVal = E_NOT_OK;
  PduIndexType PDUIdx = 0U;
  /* Check if the current Pdu fits in the available payload buffer */
  if(E_NOT_OK != CanIf_InsertPayloadInBuffer(controllerId, PduInfoPtr, &PDUIdx))
  {
    /* add PDU in buffer */
    CanIf_AddedPduInMirrorBuff(controllerId, PduInfoPtr, CanTxPduId, PDUIdx);
...
)
{
  ...
  PduIndexType buffSize;
  PayloadIndexType MirrorBuffPayload_Indx = 0U;
  PayloadIndexType buffSizePL = 0U;
  uint8 * buffPayload;
  ...
  if(E_NOT_OK != CanIf_FindNodeByID(ControllerId, CanTxPduId, &PduBuffIdx))
  {
    uint8 len_gt_0 = 0xFFU;;

#if (CANIF_MULTICORE_SUPPORT == STD_OFF)
    MirrorPduPtr = &CanIf_MirrorBuff[PduBuffIdx];
#else
    MirrorPduPtr = &CanIf_TxMirrBuffPtr[ControllerId].buffPtr[PduBuffIdx];
#endif

    len_gt_0 = (MirrorPduPtr->length > 0U) ? 1U : 0U;

    /*Store PDU data and make the slot available*/
    MirrorBuffPayload_Indx = (PayloadIndexType)PduBuffIdx * 8U;
#if (CANIF_MULTICORE_SUPPORT == STD_OFF)
    buffPayload = CanIf_MirrorBuffPayload;
    buffSizePL = TX_PAYLOAD_BUFFER_SIZE;
    buffSize = TX_MIRROR_BUFFER_SIZE;
#else
    buffPayload = CanIf_TxMirrBuffPtr[ControllerId].payloadBuffPtr;
    buffSizePL = CanIf_TxMirrBuffPtr[ControllerId].buffSize * 8U;
    buffSize = CanIf_TxMirrBuffPtr[ControllerId].buffSize;
#endif

    if(skipPduCopy == FALSE)
    {
      /* Check if its a rotated message */
      if((MirrorBuffPayload_Indx + MirrorPduPtr->length) > buffSizePL)
      {
        uint8 pdu_firstchunk = buffSizePL - MirrorBuffPayload_Indx;

        TS_MemCpy(PduInfoPtr->SduDataPtr, &buffPayload[MirrorBuffPayload_Indx], pdu_firstchunk);
        /* Deviation MISRAC2012-2 */
        TS_MemCpy(&PduInfoPtr->SduDataPtr[pdu_firstchunk], &buffPayload[0U], (MirrorPduPtr->length - pdu_firstchunk));
      ...
      {
        TS_MemCpy(PduInfoPtr->SduDataPtr, &buffPayload[MirrorBuffPayload_Indx], MirrorPduPtr->length);
    ...
    for (idx = 0U; idx < ((MirrorPduPtr->length - len_gt_0)/8U + 1U); idx++)
    {
#if (CANIF_MULTICORE_SUPPORT == STD_OFF)
      CanIf_MirrorBuff[(PduBuffIdx+idx)%buffSize].status = CANIF_MIRR_BUF_EMPTY;
#else
      CanIf_TxMirrBuffPtr[ControllerId].buffPtr[(PduBuffIdx+idx)%buffSize].status = CANIF_MIRR_BUF_EMPTY;
#endif
    }
  }
...
)
{
  ...
  {
    if((CanIf_MirrorBuff[Idx].status == 1U) && (CanIf_MirrorBuff[Idx].pduId == CanTxPduId))
    {
      *NodeIdxPtr = Idx;
      RetVal = E_OK;
...
#else
  /* Deviation TASKING-1 */
  for(Idx = 0U; Idx < CanIf_TxMirrBuffPtr[ControllerId].buffSize; Idx++)
  {
    if((CanIf_TxMirrBuffPtr[ControllerId].buffPtr[Idx].status == 1U) && (CanIf_TxMirrBuffPtr[ControllerId].buffPtr[Idx].pduId == CanTxPduId))
    {
      *NodeIdxPtr = Idx;
      RetVal = E_OK;
...
{
  uint8 len_gt_0 = (PduInfoPtr->length > 0U) ? 1U : 0U;
  uint8 cnt = 0U;
  Std_ReturnType RetVal = E_NOT_OK; /* return value */
  PduIndexType idx, MirrorBuff_Indx, buffSize, startIdx;
  PayloadIndexType MirrorBuffPayload_Indx, buffSizePL;
  uint8 * buffPayload;
  CanIf_MirrorBuffType * MirrorPduPtr;

#if (CANIF_MULTICORE_SUPPORT == STD_OFF)
  MirrorBuff_Indx = CanIf_MirrNextFreeIdx;
  buffPayload = CanIf_MirrorBuffPayload;
  MirrorPduPtr = CanIf_MirrorBuff;
  buffSizePL = TX_PAYLOAD_BUFFER_SIZE;
  buffSize = TX_MIRROR_BUFFER_SIZE;
  TS_PARAM_UNUSED(controllerId);
#else
  MirrorBuff_Indx = *CanIf_TxMirrBuffPtr[controllerId].nextFreeIdx;
  buffPayload = CanIf_TxMirrBuffPtr[controllerId].payloadBuffPtr;
  MirrorPduPtr = CanIf_TxMirrBuffPtr[controllerId].buffPtr;
  buffSizePL = CanIf_TxMirrBuffPtr[controllerId].buffSize * 8U;
  buffSize = CanIf_TxMirrBuffPtr[controllerId].buffSize;
#endif

  startIdx = MirrorBuff_Indx % buffSize;
  for (idx = 0U; idx < buffSize; idx++)
  {
    if(CANIF_MIRR_BUF_EMPTY == MirrorPduPtr[(MirrorBuff_Indx + idx) % buffSize].status)
    {
      cnt++;
      if(cnt == (((PduInfoPtr->length - len_gt_0) / 8U) + 1U))
      {
        break;
      }
    }
    ...
    {
      startIdx = (MirrorBuff_Indx + idx + (PduIndexType)1U) % buffSize;
      cnt = 0U;
    }
  }
  ...
  {
    *PduIdxPtr = startIdx;
    MirrorBuffPayload_Indx = (PayloadIndexType)startIdx * 8U;

    /* Rotated Pdu */
    if((MirrorBuffPayload_Indx + PduInfoPtr->length) > buffSizePL)
    {
      PayloadIndexType pdu_firstchunk = buffSizePL - MirrorBuffPayload_Indx;

      TS_MemCpy(&buffPayload[MirrorBuffPayload_Indx], PduInfoPtr->sdu, pdu_firstchunk);
      MirrorBuffPayload_Indx = PduInfoPtr->length - pdu_firstchunk;
      TS_MemCpy(&buffPayload[0U], &(PduInfoPtr->sdu[pdu_firstchunk]), MirrorBuffPayload_Indx);
    }
    else
    {
      TS_MemCpy(&buffPayload[MirrorBuffPayload_Indx], PduInfoPtr->sdu, PduInfoPtr->length);
    }
...
{
  uint8 len_gt_0 = (PduInfoPtr->length > 0U) ? 1U : 0U;
  PduIndexType idx, buffSize;
  PduIndexType * MirrorBuff_Indx;
  CanIf_MirrorBuffType * MirrorPduPtr;
  ...
  for (idx = 0U; idx < ((((PduIndexType)PduInfoPtr->length - len_gt_0) / 8U) + 1U); idx++)
  {
    MirrorPduPtr[(pduIdx + idx)%buffSize].pduId = CanIfTxPduId;
    MirrorPduPtr[(pduIdx + idx)%buffSize].length = PduInfoPtr->length;
    MirrorPduPtr[(pduIdx + idx)%buffSize].status = idx + (PduIndexType)1U;
  }
  *MirrorBuff_Indx = (pduIdx + (((PduIndexType)PduInfoPtr->length - len_gt_0) / 8U) + 1U)%buffSize;
}

#endif


#if (defined CANIF_CAN_ASR21_11_COMPATIBILITY)
#if (CANIF_GLOBAL_TIME_APIS == STD_ON)

FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetCurrentTime
(
  uint8 Controller,
...
)
{
  ...
  DBG_CANIF_GETCURRENTTIME_ENTRY(Controller,timeStampPtr);

#if (CANIF_DEV_ERROR_DETECT == STD_ON)
  if( CanIf_State == CANIF_UNINITIALIZED )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00922,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETCURRENTTIME,
                            CANIF_E_UNINIT
                          );
  }
  else if( Controller >= CANIF_GET_NR_OF_CAN_CTRLS() )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00923,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETCURRENTTIME,
  ...
  {
    /* !LINKSTO CanIf.SWS_CanIf_00924,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETCURRENTTIME,
...
#endif /* CANIF_DET_REPORT_ERROR == STD_ON */
  {
    /* call Can_GetCurrentTime(). */
    retVal = CanIf_CanDrvFctConfig[CANIF_GET_CAN_DRIVER_IDX(Controller)].GetCurrentTime(CANIF_GET_CANDRV_CTRL_ID(Controller), timeStampPtr);
  }

  DBG_CANIF_GETCURRENTTIME_EXIT(retVal,Controller,timeStampPtr);
  return retVal;
}

FUNC (void, CANIF_CODE) CanIf_EnableEgressTimeStamp
(
  PduIdType TxPduId
)
{

  DBG_CANIF_ENABLEEGRESSTIMESTAMP_ENTRY(TxPduId);

#if (CANIF_DEV_ERROR_DETECT == STD_ON)
  if( CanIf_State == CANIF_UNINITIALIZED )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00926,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_ENABLEEGRESSTIMESTAMP,
                            CANIF_E_UNINIT
                          );
  }
  else if( TxPduId >= CanIf_ConfigPtr->nrOfTxLPdus )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00927,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_ENABLEEGRESSTIMESTAMP,
...
#if (CANIF_SINGLE_CAN_CTRL_OPT == STD_OFF)
    const uint8 DriverIdx = CANIF_GET_CAN_DRIVER_IDX(CanIf_TxPduConfigPtr[TxPduId].intCtrlIdx);    /* Driver index */
#else
    const uint8 DriverIdx = CANIF_GET_CAN_DRIVER_IDX(CANIF_SINGLE_CAN_CTRL_ID);                    /* Driver index */
#endif

    /* Call Can_EnableEgressTimeStamp(). */
    CanIf_CanDrvFctConfig[DriverIdx].EnableEgressTimeStamp(CanHth);
  }

  DBG_CANIF_ENABLEEGRESSTIMESTAMP_EXIT(TxPduId);
}

FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetEgressTimeStamp
(
  PduIdType TxPduId,
  P2VAR( Can_TimeStampType, AUTOMATIC, CANIF_APPL_DATA ) timeStampPtr
)
{
  ...
  DBG_CANIF_GETEGRESSTIMESTAMP_ENTRY(TxPduId,timeStampPtr);

#if (CANIF_DEV_ERROR_DETECT == STD_ON)
  if( CanIf_State == CANIF_UNINITIALIZED )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00929,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETEGRESSTIMESTAMP,
                            CANIF_E_UNINIT
                          );
  }
  else if( TxPduId >= CanIf_ConfigPtr->nrOfTxLPdus )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00930,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETEGRESSTIMESTAMP,
  ...
  {
    /* !LINKSTO CanIf.SWS_CanIf_00931,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETEGRESSTIMESTAMP,
...
#endif /* CANIF_DET_REPORT_ERROR == STD_ON */
  {
    ...
    /* Call Can_GetEgressTimeStamp(). */
    retVal = CanIf_CanDrvFctConfig[DriverIdx].GetEgressTimeStamp(TxPduId, CanHth, timeStampPtr);
  }

  DBG_CANIF_GETEGRESSTIMESTAMP_EXIT(retVal,TxPduId,timeStampPtr);
  return retVal;
}


FUNC (Std_ReturnType, CANIF_CODE) CanIf_GetIngressTimeStamp
(
  PduIdType RxPduId,
  P2VAR( Can_TimeStampType, AUTOMATIC, CANIF_APPL_DATA ) timeStampPtr
)
{
  ...
  DBG_CANIF_GETINGRESSTIMESTAMP_ENTRY(RxPduId,timeStampPtr);

#if (CANIF_DEV_ERROR_DETECT == STD_ON)
  if( CanIf_State == CANIF_UNINITIALIZED )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00933,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETINGRESSTIMESTAMP,
                            CANIF_E_UNINIT
                          );
  }
  else if( RxPduId >= CanIf_ConfigPtr->nrOfRxLPdus )
  {
    /* !LINKSTO CanIf.SWS_CanIf_00934,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETINGRESSTIMESTAMP,
  ...
  {
    /* !LINKSTO CanIf.SWS_CanIf_00935,1 */
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
                            CANIF_SID_GETINGRESSTIMESTAMP,
...
#endif /* CANIF_DET_REPORT_ERROR == STD_ON */
  {
    const Can_HwHandleType CanHrh = CanIf_RxPduConfigPtr[RxPduId].hrh;      /* CAN HRH */

#if (CANIF_SINGLE_CAN_CTRL_OPT == STD_OFF)
    const uint8 DriverIdx = CANIF_GET_CAN_DRIVER_IDX(CanIf_RxPduConfigPtr[RxPduId].intCtrlIdx);    /* Driver index */
#else
    const uint8 DriverIdx = CANIF_GET_CAN_DRIVER_IDX(CANIF_SINGLE_CAN_CTRL_ID);                    /* Driver index */
#endif

    /* Call Can_GetIngressTimeStamp(). */
    retVal = CanIf_CanDrvFctConfig[DriverIdx].GetIngressTimeStamp(CanHrh, timeStampPtr);
  }

  DBG_CANIF_GETINGRESSTIMESTAMP_EXIT(retVal,RxPduId,timeStampPtr);
  return retVal;
}


#endif /* (CANIF_GLOBAL_TIME_APIS == STD_ON) */
#endif /* (defined CANIF_CAN_ASR21_11_COMPATIBILITY) */
