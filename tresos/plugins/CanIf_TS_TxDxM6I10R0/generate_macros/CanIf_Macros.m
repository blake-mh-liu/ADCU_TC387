...
*/!][!VAR "userDlcPassedNotifNum" = "num:i(count(CanIfInitCfg/*[1]/CanIfRxPduCfg/*[CanIfUserDlcPassedNotification != '']))"!][!/*
  ...
  */!][!FOR "i" = "1" TO "$canDriverNum"!][!/*
    *** Vendor ID ***
    */!][!VAR "driverVendorId" = "util:distinct(node:refs(CanIfCtrlDrvCfg/*[num:i($i)]/CanIfCtrlDrvNameRef))/../CommonPublishedInformation/VendorId"!][!/*

    *** Api Infix ***
    */!][!VAR "driverApiInfix" = "util:distinct(node:refs(CanIfCtrlDrvCfg/*[num:i($i)]/CanIfCtrlDrvNameRef))/../CommonPublishedInformation/VendorApiInfix"!][!/*

    *** Max HW object Id per driver ***
    */!][!VAR "maxCanHthId" = "0"!][!/*
    */!][!IF "$hthNum > 0"!][!/*
       */!][!VAR "maxCanHthId" = "num:max(util:distinct(node:refs(util:distinct(node:refs(CanIfCtrlDrvCfg/*[num:i($i)]/CanIfCtrlDrvInitHohConfigRef))/CanIfHthCfg/*/CanIfHthIdSymRef))/CanObjectId)"!][!/*
    */!][!ENDIF!][!/*
    */!][!VAR "maxCanHrhId" = "0"!][!/*
    */!][!IF "$hrhNum > 0"!][!/*
       */!][!VAR "maxCanHrhId" = "num:max(util:distinct(node:refs(util:distinct(node:refs(CanIfCtrlDrvCfg/*[num:i($i)]/CanIfCtrlDrvInitHohConfigRef))/CanIfHrhCfg/*/CanIfHrhIdSymRef))/CanObjectId)"!][!/*
    */!][!ENDIF!][!/*
...
*/!][!IF "$canIfPublicMultipleDrvSupport = 'false'"!][!/*
  */!][!IF "node:isconsecutive(util:distinct(node:refs(CanIfInitCfg/*[1]/CanIfInitHohCfg/*[1]/CanIfHrhCfg/*/CanIfHrhIdSymRef))/CanObjectId, 0)"!][!/*
     */!][!IF "node:isconsecutive(util:distinct(node:refs(CanIfInitCfg/*[1]/CanIfInitHohCfg/*[1]/CanIfHthCfg/*/CanIfHthIdSymRef))/CanObjectId, $hrhNum)"!][!/*
...
*/!][!VAR "sortedCtrlCfgs"="text:join(node:paths(util:distinct(node:order(CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*,'CanIfCtrlId'))),',')"!][!/*

*** variable which gives the maximum CAN controller ID ***
*/!][!VAR "maxCanControllerId"="num:max(util:distinct(node:refs(CanIfCtrlDrvCfg/*/CanIfCtrlCfg/*/CanIfCtrlCanCtrlRef))/CanControllerId)"!][!/*
...
*/!][!MACRO "GetControllerIndex", "controllerCfgPath"!][!/*
   ...
   */!][!FOR "i"="0" TO "$controllerNum - 1"!][!/*
      */!][!IF "string($controllerCfgPath) = text:split($sortedCtrlCfgs,',')[position()-1 = $i]"!][!/*
...
*/!][!MACRO "CanDriverWakeupSupport", "idx"!][!/*
    ...
    */!][!IF "$canWakeupSupport = 'true'"!][!/*
      */!][!IF "$canIfPublicMultipleDrvSupport = 'false'"!][!/*
        ...
        */!][!ENDLOOP!][!/*
      */!][!ELSE!][!/*
        */!][!IF "CanIfPublicCfg/CanIfCanDriverCompatibility = 'ASR422' or CanIfPublicCfg/CanIfCanDriverCompatibility = 'ASR431' or CanIfPublicCfg/CanIfCanDriverCompatibility = 'ASR440' or CanIfPublicCfg/CanIfCanDriverCompatibility = 'ASR21_11'"!][!/*
          */!][!LOOP "util:distinct(node:refs(CanIfCtrlDrvCfg/*[num:i($idx + 1)]/CanIfCtrlDrvNameRef))/../CanConfigSet/CanController/*"!][!/*
            */!][!IF "./CanWakeupSupport = 'true'"!][!/*
              */!][!VAR "tempRet" = "'true'"!][!/*
            */!][!ENDIF!][!/*
          */!][!ENDLOOP!][!/*
...
*** prepare for IncCtrlNodifIndex macro ***
*/!][!VAR "ctrlNotifIndex" = "''"!][!/*
*/!][!FOR "i" = "0" TO "$controllerNum - 1"!][!/*
  */!][!VAR "ctrlNotifIndex"!][!"$ctrlNotifIndex"!][!"num:i(0)"!];[!ENDVAR!][!/*
*/!][!ENDFOR!][!/*

*** IncCtrlNodifIndex: Increases the TX notification index per controller ***
*/!][!MACRO "IncCtrlNodifIndex", "ctrl"!][!/*
  */!][!VAR "tempNotifIdx" = "text:split($ctrlNotifIndex, ';')[position() = ($ctrl + 1)]"!][!/*
  */!][!VAR "tmpList" = "''"!][!/*
  */!][!FOR "j" = "0" TO "$controllerNum - 1"!][!/*
    */!][!IF "num:i($j) = num:i($ctrl)"!][!/*
      */!][!VAR "tmpList"!][!"$tmpList"!][!"num:i($tempNotifIdx + 1)"!];[!ENDVAR!][!/*
    */!][!ELSE!][!/*
      */!][!VAR "tmpList"!][!"$tmpList"!][!"text:split($ctrlNotifIndex, ';')[num:i($j + 1)]"!];[!ENDVAR!][!/*
    */!][!ENDIF!][!/*
  */!][!ENDFOR!][!/*
  */!][!VAR "ctrlNotifIndex"!][!"$tmpList"!][!ENDVAR!][!/*
*/!][!ENDMACRO!][!/*
