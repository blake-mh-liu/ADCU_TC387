 ...
 *
 * Reason:
 * Function declaration and the macro exclude each other. This is guarded by compiler
 * switch, which is not correctly analyzed by Astree.
 *
 * MISRAC2012-3) Deviated Rule: 5.5 (required)
 * Identifiers shall be distinct from macro names.
 *
 * Reason:
 * The compiler switch assures that either the macro or the function is enabled.
 * This macro usage is needed when the redirection feature is enabled.
 */
...
/* patch version check */
#if (WDGM_SW_PATCH_VERSION < 7U)
#error WDGM_SW_PATCH_VERSION wrong (< 7U)
#endif
 ...
 * \retval E_OK supervision was ok
 * \retval E_NOT_OK supervision failed */
static FUNC(Std_ReturnType, WDGM_CODE) WdgM_EB_CheckpointReachedExtOptLogical
(
  WdgM_EB_SEIdType IntSEId,
  WdgM_EB_CPIdType CPId,
...
#if((WDGM_EB_MULTICORE_ENABLED == STD_ON) && (WDGM_EB_MASTER_WAIT_SLAVE_MODE_SWITCH != 0U))
...
#if ((WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON) || (WDGM_EB_GET_FIRST_EXPIRED_ENABLED == STD_ON))
 ...
 * \param[in] IntSEId internal SE Id
 * \param[in] CoreInstance  Core instance specific data
 * \param[in] NewLocalStatus  New local status
 * \param[in] PartitionReset  Partition reset switch
 * \retval void
 */
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#if((WDGM_EB_EXT_SATELLITE_ALIVENESS_ENABLED == STD_ON) && (WDGM_EB_SATELLITE_ALIVENESS_CHECK_PERIOD != 0))
/** \brief Updates the alive counter for the Satellite cores
 *
 * \param[in] CoreInstance  Core instance specific data
 * \retval void
 */
static FUNC(void, WDGM_CODE) WdgM_EB_SatelliteAlivenessUpdate
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);
#endif
...
/** \brief stores core-specific SE LocalStatus data
 *
 * \param[out] SELocalStatusPtr pointer to store data
 * \param[in] LocalStatusValue the LocalStatus to store */
static FUNC(void, WDGM_CODE) WdgM_EB_Store_SELocalStatus_Core
(
    P2VAR(WdgM_EB_LocalStatusType, AUTOMATIC, WDGM_VAR) SELocalStatusPtr,
    uint8 LocalStatusValue
);

/** \brief retrieves the core-specific SE local status
 *
 * \param[in] CoreInstance  Core instance specific data
 * \param[in] IntSEId internal SE Id
 * \param[out] LocalStatus pointer to store checked data
 *
 * \return Success of operation
 * \retval E_OK   SELocalStatus data is valid
 * \retval E_NOT_OK   SELocalStatus data is invalid */
static FUNC(WdgM_EB_ResultType, WDGM_CODE) WdgM_EB_GetChecked_SELocalStatus_Core
(
    P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance,
    WdgM_EB_SEIdType IntSEId,
    P2VAR(WdgM_LocalStatusType ,AUTOMATIC, AUTOMATIC) LocalStatus
);

/** \brief retrieves the SE local status
 *
 * \param[in] IntSEId internal SE Id
 * \param[in] ApiId API identifier for unreachable code assert
 * \param[out] LocalStatus pointer to store checked data
 *
 * \return Success of operation
 * \retval E_OK expired SELocalStatus data is valid
 * \retval E_NOT_OK expired SELocalStatus data is invalid */
static FUNC(WdgM_EB_ResultType, WDGM_CODE) WdgM_EB_GetChecked_SELocalStatus
(   WdgM_EB_SEIdType IntSEId,
    uint8 ApiId,
    P2VAR(WdgM_LocalStatusType ,AUTOMATIC, AUTOMATIC) LocalStatus
);
#endif /* WDGM_MIXED_CRITICALITY == STD_ON */

#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
/** \brief Triggers reset if any errors are found during mode switch actions between cores.
 *
 * \param[in] CoreInstance  Core instance specific data
 */
static FUNC(void, WDGM_CODE) WdgM_EB_ModeSwitchHandling
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
);
#endif
...
/** \brief Stores expired counter data or stores error if data being read is corrupted
 *
 * \param[in] CoreInstance  Core instance specific data
 * \param[in] SEId  identifier of the external SE
 */
static FUNC(void, WDGM_CODE) WdgM_EB_CheckAndStoreExpiredCounter
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance,
  WdgM_EB_SEIdType SEId
);
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
/* !LINKSTO WDGM.EB.DesignDecision.InternalVariable35,1 */
static VAR(boolean, WDGM_VAR) WdgM_EB_ModeSwitchInProgress = FALSE;
#endif
...
#define WDGM_START_SEC_VAR_CLEARED_ASIL_D_32
#include <WdgM_MemMap.h>
...
/* Deviation MISRAC2012-1 */
static VAR(uint32, WDGM_VAR) WdgM_EB_ExpiredSupervisionCycleCnt;

#define WDGM_STOP_SEC_VAR_CLEARED_ASIL_D_32
#include <WdgM_MemMap.h>
...
/* !LINKSTO WdgM.SWS_WdgM_00355,1 */
#if (WDGM_EB_EXT_GET_EXPECTED_INIT_STATE_ENABLED == STD_OFF)
/* Deviation MISRAC2012-3, MISRAC2012-2 */
...
)
{
  ...
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00288,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
  ...
  else
  {
    ...
    /* !LINKSTO WdgM.SWS_WdgM_00388,1 */
    /* !LINKSTO WdgM.SWS_WdgM_00407,1 */
...
)
{
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
  /* !LINKSTO WdgM.SWS_WdgM_00256,1,WdgM.ASR40.SWS_WdgM_00004.5,1 */
...
#endif /* WDGM_DEV_ERROR_DETECT */
  {
    ...
    /* !LINKSTO WdgM.ASR40.SWS_WdgM_00110,1 */
...
#endif
{
  ...
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,2 */
  ...
  /* !LINKSTO WDGM.EB.TIMEPM.WDGM020459,1 */
  {
    ...
    /* !LINKSTO WdgM.SWS_WdgM_00020,1,WdgM.ASR40.SWS_WdgM_00004.3,1 */
    else if (Mode >= WdgM_EB_ModeCfgSize)
    {
...
#if (WDGM_EB_CHECK_CALLERID == STD_ON)
      /* !LINKSTO WdgM.ASR40.SWS_WdgM_00245,1 */
...
#endif
      {
        ...
        * WdgM_EB_InitStatus != WDGM_EB_INIT_STATUS_DEINIT)*/
        if (WDGM_EB_INIT_STATUS_DEINIT == WDGM_EB_GET_GLOBAL_INIT_STATUS())
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020457,1,WdgM.ASR40.SWS_WdgM_00021,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
#else
        if ((WDGM_EB_INIT_STATUS_DEINIT == WDGM_EB_GET_GLOBAL_INIT_STATUS())
        ...
        /* !LINKSTO WdgM.SWS_WdgM_00316,1,WdgM.SWS_WdgM_00145,1,WDGM.EB.TIMEPM.WDGM020458,1 */
...
)
{
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
#if (WDGM_EB_MULTICORE_ENABLED == TRUE)
  if (CoreInstance == NULL_PTR)
  {
    WDGM_EB_DET(WDGM_SID_GET_MODE, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else if (WDGM_EB_MASTER_INSTANCE != CoreInstance->CoreID)
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,2 */
    WDGM_EB_DET(WDGM_SID_GET_MODE, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
...
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00253,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
  {
    ...
    /* !LINKSTO WdgM.SWS_WdgM_00254,1,WdgM.ASR40.SWS_WdgM_00004.5,1 */
...
#endif
  {
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
    WdgM_ModeType LocalCoreSwitchMode;
    Std_ReturnType RetVal_Mode;

    RetVal_Mode = WdgM_EB_GetCheckedRedundantData8 (*CoreInstance->WdgM_EB_CoreSwitchMode, &LocalCoreSwitchMode);
    if (RetVal_Mode != E_OK)
    {
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1,WDGM.EB.TIMEPM.WDGM020486,1 */
    ...
    WDGM_EB_DET(WDGM_SID_GET_MODE, WDGM_EB_E_DEMTODET_DATA_CORRUPTION);
#endif
    RetVal = E_NOT_OK;
    }
    else
    {
    *Mode = LocalCoreSwitchMode;
    RetVal = E_OK;
    }
#else
...
)
{
  ...
  /* !LINKSTO WdgM.SWS_WdgM_00278,1,WdgM.ASR40.SWS_WdgM_00004.4,1 */
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
#if (WDGM_EB_MULTICORE_ENABLED == STD_ON)
  if (CoreInstance == NULL_PTR)
  {
...
#endif
  {
    /* !LINKSTO WdgM.ASR40.SWS_WdgM_00279,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
    if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_INIT)
    {
...
#endif
    /* !LINKSTO WdgM.SWS_WdgM_00284,1,WdgM.ASR40.SWS_WdgM_00004.7,1 */
    ...
    * per Supervised Entity are limited to 256 (see WDGM.EB.NumberOfCheckpoints). */
...
#endif
    {
      ...
      /* !LINKSTO WdgM.SWS_WdgM_00208,1 */
#if (WDGM_EB_MULTICORE_ENABLED == STD_ON)
      WdgM_LocalStatusType SELocalStatus;
#if (WDGM_MIXED_CRITICALITY == STD_ON)
      WdgM_EB_ResultType SERetVal;
      SERetVal = WdgM_EB_GetChecked_SELocalStatus_Core(CoreInstance, IntSEId, &SELocalStatus);
      if (SERetVal == E_OK)
      {
#else
      SELocalStatus = CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]];
#endif
...
#endif
      {
        /* !LINKSTO WdgM.ASR40.SWS_WdgM_00319,1,WdgM.ASR40.SWS_WdgM_00004.10,1 */
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
...
#if (WDGM_EB_OPTIMIZED_EXTERNAL_GRAPH_ENABLED == STD_ON)
            /* perform logical supervision */
            if(CPExtLogicalCfg->IsOptimized)
            {
              if (WdgM_EB_CheckpointReachedExtOptLogical (IntSEId, IntCPId, CPExtLogicalCfg) != E_OK)
              {
                CPFailed = TRUE;
              }
            }
...
/* !LINKSTO WdgM.ASR40.SWS_WdgM_00155,1 */
...
)
{
  ...
  if (CoreInstance == NULL_PTR)
  {
    WDGM_EB_DET(WDGM_SID_UPDATE_ALIVE_COUNTER, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
  }
  else
#endif
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00028,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
  {
    if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_INIT)
    {
...
#endif
    {
      ...
      /* !LINKSTO WdgM.ASR40.SWS_WdgM_00290,1,WdgM.ASR40.SWS_WdgM_00004.8,1 */
...

      {
        ...
        /* !LINKSTO WdgM.ASR40.SWS_WdgM_00027,1,WdgM.ASR40.SWS_WdgM_00004.4,1 */
        ...
        else
        {
          ...
          /* !LINKSTO WdgM.ASR40.SWS_WdgM_00318,1 */
          else if (NumOfAliveSupervisions == 1U)
          {
            RetVal = WdgM_CheckpointReached (SEID, FoundCPId);
          }
          /* !LINKSTO WdgM.ASR40.SWS_WdgM_00320,1,WdgM.ASR40.SWS_WdgM_00004.9,1 */
          else
          {
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
...
)
{
  ...
  if (CoreInstance == NULL_PTR)
  {
    WDGM_EB_DET(WDGM_SID_GET_LOCAL_STATUS, WDGM_E_PARAM_WRONG_CORE_ID);
  }
  else
#endif
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00173,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
  {
    if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_INIT)
    {
      WDGM_EB_DET(WDGM_SID_GET_LOCAL_STATUS, WDGM_E_NO_INIT);
    }
    /* !LINKSTO WdgM.SWS_WdgM_00257,1,WdgM.ASR40.SWS_WdgM_00004.5,1 */
...
#endif
    {
      ...
      /* !LINKSTO WdgM.SWS_WdgM_00172,1,WdgM.ASR40.SWS_WdgM_00004.4,1 */
      if (WdgM_EB_SEId2IntSEId (SEID, &IntSEId) != E_OK)
      {
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
      ...
      {
#if ((WDGM_MIXED_CRITICALITY == STD_OFF) || (WDGM_EB_MULTICORE_ENABLED == STD_OFF))
        /* !LINKSTO WdgM.SWS_WdgM_00171,1 */
        *Status = WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, WDGM_SID_GET_LOCAL_STATUS);
        RetVal = E_OK;
#else
        WdgM_LocalStatusType SELocalStatus;
        RetVal = WdgM_EB_GetChecked_SELocalStatus(IntSEId, WDGM_SID_GET_LOCAL_STATUS, &SELocalStatus);
        *Status  = (uint8)SELocalStatus;
#endif
...
)
{
  ...
  else if (WDGM_EB_MASTER_INSTANCE != WDGM_EB_GET_CORE_ID())
  {
    /* !LINKSTO WDGM.EB.SatelliteCall.DetError,2 */
    WDGM_EB_DET(WDGM_SID_GET_GLOBAL_STATUS, WDGM_E_PARAM_WRONG_CORE_ID);
    RetVal = E_NOT_OK;
...
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00176,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
  {
    if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_INIT)
    {
    ...
    /* !LINKSTO WdgM.SWS_WdgM_00258,1,WdgM.ASR40.SWS_WdgM_00004.5,1 */
    else if (Status == NULL_PTR)
    {
...
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00270,1,WdgM.ASR40.SWS_WdgM_00004.1,1  */
  {
    if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_INIT)
    {
...
#endif
    {
...
#endif
      {
        ...
        */
        /* !LINKSTO WdgM.SWS_WdgM_00139,1,WdgM.EB.PerformReset1,1 */
        WdgM_EB_GlobalStatus = WDGM_GLOBAL_STATUS_STOPPED;

        /* !LINKSTO WdgM.SWS_WdgM_00133.1,1 */
#if (WDGM_IMMEDIATE_RESET == STD_ON)
       Mcu_PerformReset();
#endif
...
#if (WDGM_EB_GET_FIRST_EXPIRED_ENABLED == STD_ON)
/* !LINKSTO WdgM.SWS_WdgM_00346,1 */
...
#endif /* WDGM_EB_MULTICORE_ENABLED == TRUE */
  /* !LINKSTO WdgM.SWS_WdgM_00347,1,WdgM.ASR40.SWS_WdgM_00004.5,1 */
  {
    if (SEID == NULL_PTR)
    {
...
/* !LINKSTO WdgM.SWS_WdgM_00159,1,WdgM.ASR40.SWS_WdgM_00060,1 */
FUNC(void, WDGM_CODE) WdgM_MainFunction(void)
{
  ...
  if (CoreInstance == NULL_PTR)
  {
    WDGM_EB_DET(WDGM_SID_MAIN_FUNCTION, WDGM_E_PARAM_WRONG_CORE_ID);
  }
  else
#endif
    ...
    /* MainFunction is executing => active */
  {
    ...
    {
      if(WdgM_EB_GetSatelliteWdgIfSetModeFailed() == WDGM_EB_TRUE)
      {
...

    {
      ...
      const WdgM_EB_InitStatusType DoNotCheckInitRequest = *(CoreInstance->WdgM_EB_InitStatus);
      ...
      /* !LINKSTO WdgM.SWS_WdgM_00063,1,WDGM.EB.TIMEPM.WDGM020420,1,WdgM.ASR40.SWS_WdgM_00292,1  */
      ...
      if (*(CoreInstance->WdgM_EB_InitStatus) != WDGM_EB_INIT_STATUS_DEINIT)
      {
        ...
        /* !LINKSTO WdgM.SWS_WdgM_00325,1,WDGM.EB.MulticoreLocalStatus1,1 */
...
#endif
        {
          ...
          /* !LINKSTO WdgM.SWS_WdgM_00214,1,WdgM.SWS_WdgM_00326,1 */
          /* !LINKSTO WDGM.EB.MulticoreGlobalStatus,2 */
          WdgM_EB_SetGlobalStatus(CoreInstance);

          /* !LINKSTO WdgM.ASR40.SWS_WdgM_00292,1,WdgM.SWS_WdgM_00327,1,WdgM.SWS_WdgM_00328,1 */
    ...
    if(CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
    {
      WdgM_EB_ModeSwitchHandling(CoreInstance);
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020487,1 */
    }
#if((WDGM_EB_EXT_SATELLITE_ALIVENESS_ENABLED == STD_ON) && (WDGM_EB_SATELLITE_ALIVENESS_CHECK_PERIOD != 0U))
    WdgM_EB_SatelliteAlivenessUpdate(CoreInstance);
#endif
#endif
...
)
{
  ...
  /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#if (WDGM_MIXED_CRITICALITY == STD_ON)
    WdgM_EB_StoreRedundantData8 (CoreInstance->WdgM_EB_CoreSwitchMode, WdgM_EB_InitialModeId);
#else
...
)
{
  ...
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00317,1 */
  /* the function doesn't change the global status or the expired reference counter */
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
  if(CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
#endif
  {
    /* !LINKSTO WdgM.SWS_WdgM_00316,1,WdgM.SWS_WdgM_00145,1,WDGM.EB.TIMEPM.WDGM020432,1 */
  ...
  if(E_OK == RetVal)
  {
...
#endif
    {
...
#endif
      {
        ...
        CONSTP2VAR(WdgM_EB_LocalStatusType,AUTOMATIC,AUTOMATIC) SEWData_SELocalStatus =
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#if (WDGM_MIXED_CRITICALITY == STD_OFF)
...
#endif
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020472,2 */
          RetVal = E_NOT_OK;
        }
        else
        {
#endif
#endif
          /* !LINKSTO WdgM.SWS_WdgM_00182,1 */
          /* Local Status of SEs that are active in this mode is not touched */

          if (SEIsActive)
          {
            /* !LINKSTO WDGM.EB.TIMEPM.WDGM020421,1,WDGM.EB.TIMEPM.WDGM020426,1,WdgM.SWS_WdgM_00209,1 */
            /* Note: In case IsNormalOperation is set to FALSE, then old Local Status is ignored. */
              ...
              {
#if (WDGM_MIXED_CRITICALITY == STD_ON)
                WdgM_EB_Store_SELocalStatus_Core(SEWData_SELocalStatus, WDGM_LOCAL_STATUS_OK);
#else
                *SEWData_SELocalStatus = WDGM_LOCAL_STATUS_OK;
#endif
                /* !LINKSTO WdgM.SWS_WdgM_00197,1 */
          ...
          {
            /* !LINKSTO WdgM.SWS_WdgM_00315,1,WDGM.EB.TIMEPM.WDGM020422,1,WDGM.EB.TIMEPM.WDGM020423,1,WdgM.SWS_WdgM_00207,1,WdgM.SWS_WdgM_00291,1 */
            ...
            if (((WDGM_LOCAL_STATUS_OK == SELocalStatus) ||
                  ...
                  (FALSE == IsNormalOperation))
#else /* #if(WDGM_EB_MULTICORE_ENABLED == STD_ON) */
            ...
            {
#if (WDGM_EB_MULTICORE_ENABLED == STD_ON)
              const WdgM_LocalStatusType OldLocalStatus = SELocalStatus;
#else
              const WdgM_LocalStatusType OldLocalStatus = *SEWData_SELocalStatus;
#endif
              /* !LINKSTO WDGM.EB.TIMEPM.WDGM020427,1 */
#if (WDGM_MIXED_CRITICALITY == STD_ON)
              WdgM_EB_Store_SELocalStatus_Core(SEWData_SELocalStatus, WDGM_LOCAL_STATUS_DEACTIVATED);
#else
...
#endif
      {
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020424,1,WDGM.EB.MulticoreTriggerWatchdog,2,WdgM.SWS_WdgM_00186,1 */
...
)
{
  ...
  for (TriggerId = 0U; TriggerId < ModeCfg->NumTriggers; TriggerId++)
  {
...
#endif
    {
      ...
      if (RetVal != E_OK)
      {
        /* !LINKSTO WdgM.ASR40.SWS_WdgM_00006,1,WdgM.SWS_WdgM_00142.1,1,WdgM.ASR40.SWS_WdgM_00004.12,1 */
        ...
        if(CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE)
        {
          ...
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020424,1,WDGM.EB.TIMEPM.WDGM020450,2,WdgM.SWS_WdgM_00186,1 */
          WdgM_PerformReset();
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
        }
        else
        {
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
          *CoreInstance->WdgM_EB_SatelliteWdgIfSetModeFailed = WDGM_EB_TRUE;
        }
#endif
        break;
      }
    ...
    {
      /* For satellite cores which are not triggering the watchdogs return E_OK */
      RetVal = E_OK;
    }
#endif
  }
...
)
{
  ...
  {
    /* !LINKSTO WdgM.SWS_WdgM_00252,1,WdgM.SWS_WdgM_00274,1,WdgM.SWS_WdgM_00323,1 */
    ...
    if (ActiveFlag)
    {
      ...
      else
      {
        ...
        {
          /* !LINKSTO WdgM.SWS_WdgM_00329,1,WdgM.SWS_WdgM_00331,1 */
          ActiveFlag = FALSE;
        }
    ...
    else
    {
      if (CPIntLogicalCfg->NumOfPredCPs == 0U)
      {
        /* !LINKSTO WdgM.SWS_WdgM_00273,1,WdgM.SWS_WdgM_00332,1 */
        ActiveFlag = TRUE;
        RetVal = E_OK;
      }
...
#if (WDGM_EB_OPTIMIZED_EXTERNAL_GRAPH_ENABLED == STD_ON)
/*------------------[WdgM_EB_CheckpointReachedExtOptLogical]-------------------------------------------*/
static FUNC(Std_ReturnType, WDGM_CODE) WdgM_EB_CheckpointReachedExtOptLogical
(
  WdgM_EB_SEIdType IntSEId,
  WdgM_EB_CPIdType CPId,
...
)
{
  ...
  /* !LINKSTO WDGM.EB.Dbg4,1 */
  DBG_WDGM_EB_CHECKPOINTREACHEDINTLOGICAL_ENTRY(IntSEId,CPId,CPExtLogicalCfg);
...
/* This callout is used only for WdgM_ConTest_14_LogSup_multicore test.*/
      ...
      (void)Tst_WdgM_ConcurrentCallout();
#endif
...
)
{
  ...
  if (CPRefCycleCnt == 0U)
  {
    ...
    {
      /* !LINKSTO WdgM.SWS_WdgM_00074,1,WdgM.SWS_WdgM_00115,1,WdgM.SWS_WdgM_00324,1 */
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
          WdgM_EB_Store_SELocalStatus_Core(&CoreInstance->WdgM_EB_SELocalStatus[SEIndex], WDGM_LOCAL_STATUS_OK);
#else
...
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
#if((WDGM_EB_EXT_SATELLITE_ALIVENESS_ENABLED == STD_ON) && (WDGM_EB_SATELLITE_ALIVENESS_CHECK_PERIOD != 0))
/*------------------[WdgM_EB_SatelliteAlivenessUpdate]-------------------------------------------------*/
static FUNC(void, WDGM_CODE) WdgM_EB_SatelliteAlivenessUpdate
(
  P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance
)
{
  ...
  {
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020487,1 */
    WdgM_EB_CheckSatelliteAliveCounter();
  }
  else
  {
#if(WDGM_MIXED_CRITICALITY == STD_ON)
    RetValCheck = WdgM_EB_GetCheckedRedundantData16(*CoreInstance->WdgM_EB_Satellite_AliveCounter, &TempValueRedundantData16);
    if(RetValCheck != E_OK)
    {
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
    ...
    {
      TempValueRedundantData16 += 1U;
      WdgM_EB_StoreRedundantData16(CoreInstance->WdgM_EB_Satellite_AliveCounter, TempValueRedundantData16);
    }
#else
    *CoreInstance->WdgM_EB_Satellite_AliveCounter +=1U;
#endif
  }
}
#endif
...
)
{
  ...
  /* !LINKSTO WdgM.SWS_WdgM_00202,1,WDGM.EB.TIMEPM.WDGM020448,1 */
  ...
  WdgM_LocalStatusType SELocalStatus = *SEWData_SELocalStatus;
#else
  WdgM_EB_ResultType SERetVal;
  WdgM_LocalStatusType NewLocalStatus, SELocalStatus;
  SERetVal = WdgM_EB_GetChecked_SELocalStatus_Core(CoreInstance, IntSEId, &SELocalStatus);
  if (SERetVal != E_OK)
  {
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1 */
  ...
  switch (SELocalStatus)
  {
    ...
    case WDGM_LOCAL_STATUS_FAILED:
    {
      ...
      case WDGM_EB_RESULT_INCORRECT:
      {
        ...
        {
          /* !LINKSTO WdgM.SWS_WdgM_00202,1,WDGM.EB.TIMEPM.WDGM020448,1,WDGM.EB.ASR40.WDGM206,1 */
      ...
      {
        /* !LINKSTO WDGM.EB.ASR40.WDGM300,1,WdgM.SWS_WdgM_00205,1 */
        if (*SEWData_FailedAliveSupervisionRefCycleCnt != 0U)
        {
          /* !LINKSTO WDGM.EB.TIMEPM.WDGM020484,1 */
          (*SEWData_FailedAliveSupervisionRefCycleCnt)--;
        }
        /* !LINKSTO WdgM.SWS_WdgM_00205,1 */
        if (*SEWData_FailedAliveSupervisionRefCycleCnt == 0U)
        {
      ...
      if(DeadlineResult != WDGM_EB_RESULT_CORRECT)
      {
        /* !LINKSTO WdgM.SWS_WdgM_00202,1,WDGM.EB.ASR40.WDGM206,1,WDGM.EB.TIMEPM.WDGM020448,1 */
        NewLocalStatus = NextFailedStatus;
#if (WDGM_PARTITION_RESET == STD_ON)
        PartitionReset = TRUE;
#endif
      ...
      if (NewLocalStatus == WDGM_LOCAL_STATUS_EXPIRED)
      {
...
#if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON)
        WdgM_EB_CheckAndStoreExpiredCounter(CoreInstance, SECfg->SEId);
        ...
        /* !LINKSTO WdgM.SWS_WdgM_00351,1 */
        WdgM_EB_StoreExpiredSEID (&ExpiredSEID, SECfg->SEId);
        WdgM_EB_ExpiredSEID = ExpiredSEID;
      }

#if (WDGM_PARTITION_RESET == STD_ON)
      WdgM_EB_PartitionRestart(IntSEId, CoreInstance, NewLocalStatus, PartitionReset);
#endif
      break;
    }

    /* !LINKSTO WdgM.SWS_WdgM_00201,1 */
  ...
  {
    WdgM_LocalStatusType OldLocalStatus = SELocalStatus;

#if (WDGM_MIXED_CRITICALITY == STD_ON)
    WdgM_EB_Store_SELocalStatus_Core(SEWData_SELocalStatus, NewLocalStatus);
#else
...
)
{
  ...
  if (OldGlobalStatus != NewGlobalStatus)
  {
    ...
    {
      /* !LINKSTO WdgM.ASR40.SWS_WdgM_00129,1,WdgM.ASR40.SWS_WdgM_00004.11,1 */
...
)
{
...
#else
      RetVal = WdgM_EB_GetChecked_SELocalStatus(IntSEId, WDGM_SID_MAIN_FUNCTION, &SELocalStatus);
      if (RetVal == E_NOT_OK)
      {
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
  ...
  switch (OldGlobalStatus)
  {
    ...
    {
      /* !LINKSTO WdgM.SWS_WdgM_00076,1 */
      if (NumSEExpired == 0U)
      {
        if (NumSEFailed > 0U)
        {
          /* !LINKSTO WdgM.SWS_WdgM_00217,1 */
          NewGlobalStatus = WDGM_GLOBAL_STATUS_FAILED;
        }
        else
        {
          /* !LINKSTO WdgM.SWS_WdgM_00078,1,WdgM.SWS_WdgM_00218,1 */
          NewGlobalStatus = WDGM_GLOBAL_STATUS_OK;
        }
      }
      else
      {
        /* !LINKSTO WdgM.SWS_WdgM_00215,1,WDGM.EB.ASR40.WDGM077,1 */
        if (ModeCfg->ExpiredSupervisionCycleTol > 0U)
        {
          NewGlobalStatus = WDGM_GLOBAL_STATUS_EXPIRED;
          WdgM_EB_ExpiredSupervisionCycleCnt = 1U;
        }
        /* !LINKSTO WdgM.SWS_WdgM_00216,1,WdgM.SWS_WdgM_00117,1 */
        else
        {
    ...
    {
      /* !LINKSTO WdgM.SWS_WdgM_00219,2,WdgM.SWS_WdgM_00220,2 */
...
)
{
  ...
  {
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020470,2,WDGM.EB.TIMEPM.WDGM020472,2 */
    RetVal = E_NOT_OK;
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020467,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
    WDGM_EB_DEM(WDGM_EB_DATA_CORRUPTION_DEM_EVENT_ID, DEM_EVENT_STATUS_FAILED);
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
/* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
/*------------------[WdgM_EB_Store_SELocalStatus_Core]--------------------------------------------*/
static FUNC(void, WDGM_CODE) WdgM_EB_Store_SELocalStatus_Core
(
    P2VAR(WdgM_EB_LocalStatusType, AUTOMATIC, WDGM_VAR) SELocalStatusPtr,
    uint8 LocalStatusValue
...
{
  const WdgM_LocalStatusType LocalStatusRedundant = (WdgM_LocalStatusType)(~LocalStatusValue); /* */
  uint16 LocalStatus;

  LocalStatus = LocalStatusValue;
  LocalStatus |= (uint16)(((uint16)LocalStatusRedundant) << 8U);

  *SELocalStatusPtr = LocalStatus;
}

/*------------------[WdgM_EB_GetChecked_SELocalStatus_Core]--------------------------------------------*/
static FUNC(WdgM_EB_ResultType, WDGM_CODE) WdgM_EB_GetChecked_SELocalStatus_Core
(
    P2VAR(WdgM_EB_CoreContextType, AUTOMATIC, WDGM_APPL_CONST) CoreInstance,
    WdgM_EB_SEIdType IntSEId,
...
)
{
  ...
  const uint8 NextFailedStatus = (SECfg->ErrorRecoveryEnabled) ? WDGM_LOCAL_STATUS_FAILED
      : WDGM_LOCAL_STATUS_EXPIRED;
  const WdgM_EB_LocalStatusType UncheckedLocalStatus = CoreInstance->WdgM_EB_SELocalStatus[WdgM_EB_SELocalCoreIdx[IntSEId]];
  const uint8  CheckedLocalStatus = (uint8)(UncheckedLocalStatus & (uint8)0xFFU);
  const uint8  LocalStatusRedundant = (uint8)
          (~((uint8)((uint16)(UncheckedLocalStatus >> 8U) & (uint8)0xFFU)));

  if (CheckedLocalStatus == LocalStatusRedundant)
  {
    RetVal = E_OK;
    *LocalStatus = CheckedLocalStatus;
  }
  else
  {
    *LocalStatus = NextFailedStatus;
    RetVal = E_NOT_OK;
  }
...
/*------------------[WdgM_EB_GetChecked_SELocalStatus]--------------------------------------------*/
static FUNC(WdgM_EB_ResultType, WDGM_CODE) WdgM_EB_GetChecked_SELocalStatus
(   WdgM_EB_SEIdType IntSEId,
    uint8 ApiId,
    P2VAR(WdgM_LocalStatusType ,AUTOMATIC, AUTOMATIC) LocalStatus
)
{
  ...
  const uint8 NextFailedStatus = (SECfg->ErrorRecoveryEnabled) ? WDGM_LOCAL_STATUS_FAILED
      : WDGM_LOCAL_STATUS_EXPIRED;
  const WdgM_EB_LocalStatusType UncheckedLocalStatus = WDGM_EB_GET_SE_LOCAL_STATUS(IntSEId, ApiId);
  const uint8  CheckedLocalStatus = (uint8)(UncheckedLocalStatus & (uint8)0xFFU);
  const uint8  LocalStatusRedundant = (uint8)
          (~((uint8)((uint16)(UncheckedLocalStatus >> 8U) & (uint8)0xFFU)));
...
)
{
  ...
  (void)(CoreInstance);
#else
  if (WdgM_EB_ModeSwitchInProgress == TRUE)
  {
#endif
  ...
  {
    boolean RetVal;
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020476,1 */
    /* go through all core instances and check if all cores switched to the current mode and
     * no data corruption occurred in satellite internal data */
    RetVal = WdgM_EB_AllCoresSameMode();

    if (RetVal == FALSE)
    {
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1,WDGM.EB.TIMEPM.WDGM020477,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
      WDGM_EB_DEM(WDGM_EB_DATA_CORRUPTION_DEM_EVENT_ID, DEM_EVENT_STATUS_FAILED);
#elif (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DET)
      WDGM_EB_DET(WDGM_SID_MAIN_FUNCTION, WDGM_EB_E_DEMTODET_DATA_CORRUPTION);
    ...
    if ((*(CoreInstance->WdgM_EB_InitStatus) == WDGM_EB_INIT_STATUS_INIT))
    {
      ...
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020478,1 */
      RetVal = WdgM_EB_CheckSatelliteInitStatus();

      if (RetVal == FALSE)
      {
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1,WDGM.EB.TIMEPM.WDGM020479,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
        WDGM_EB_DEM(WDGM_EB_DATA_CORRUPTION_DEM_EVENT_ID, DEM_EVENT_STATUS_FAILED);
#elif (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DET)
        WDGM_EB_DET(WDGM_SID_MAIN_FUNCTION, WDGM_EB_E_DEMTODET_DATA_CORRUPTION);
...
{
  Std_ReturnType RetValExpCounter, RetValExpSEID;

  /* !LINKSTO WDGM.EB.GetAllExpiredSEID.MainFunction.Write, 1 */

  WdgM_EB_SEIdType LocalExpiredCounter;
  RetValExpCounter = WdgM_EB_GetCheckedRedundantData8 (*CoreInstance->WdgM_EB_ExpiredSEIDsCounter, &LocalExpiredCounter);

  if (RetValExpCounter != E_OK)
  {
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
    WDGM_EB_DEM(WDGM_EB_DATA_CORRUPTION_DEM_EVENT_ID, DEM_EVENT_STATUS_FAILED);
  ...
  else if (LocalExpiredCounter == 0U)
  {
    WdgM_EB_StoreRedundantData8 (CoreInstance->WdgM_EB_ExpiredSEIDsCounter, 1U);
    WdgM_EB_StoreRedundantData8 (&CoreInstance->WdgM_EB_ExpiredSEIDs[0U], SEId);
  }
  else
  {
    uint8 LocalIndex;
    ...
    for (LocalIndex = 0U; LocalIndex < LocalExpiredCounter; LocalIndex++)
    {
      ...
      RetValExpSEID = WdgM_EB_GetCheckedExpiredSEID (CoreInstance->WdgM_EB_ExpiredSEIDs[LocalIndex], &LocalExpiredSEID);

      if (RetValExpSEID != E_OK)
      {
        /* !LINKSTO WDGM.EB.TIMEPM.WDGM020466,1,WDGM.EB.TIMEPM.WDGM020468,1 */
#if (WDGM_EB_PROD_ERR_HANDLING_DATA_CORRUPTION == WDGM_PROD_ERR_REP_TO_DEM)
        WDGM_EB_DEM(WDGM_EB_DATA_CORRUPTION_DEM_EVENT_ID, DEM_EVENT_STATUS_FAILED);
    ...
    {
      WdgM_EB_StoreRedundantData8 (&CoreInstance->WdgM_EB_ExpiredSEIDs[LocalExpiredCounter], SEId);
      LocalExpiredCounter++;
      WdgM_EB_StoreRedundantData8 (CoreInstance->WdgM_EB_ExpiredSEIDsCounter, LocalExpiredCounter);
    }
  }
}
#endif /* #if (WDGM_EB_GET_ALL_EXPIRED_SE == STD_ON) */
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
  if ((WdgM_EB_IsExpiredSEIdSet == WDGM_EB_FALSE) || (WdgM_EB_IsExpiredSEIdSet == WDGM_EB_TRUE))
#endif
    {
      if (WdgM_EB_IsExpiredSEIdSet == WDGM_EB_FALSE)
      {
        LocalSupEntID = SEID;
        LocalSupEntID |= (WdgM_EB_ExpiredSEIDType)(((WdgM_EB_ExpiredSEIDType)ExpiredSEIDRedundant) << 8U);

      *ExpiredSEID = LocalSupEntID ;
      /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
      WdgM_EB_IsExpiredSEIdSet = WDGM_EB_TRUE;
      }
    }
#if (WDGM_MIXED_CRITICALITY == STD_ON)
  else
...
/*------------------[WdgM_EB_StoreRedundantData8]---------------------------------------------------*/
/* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
FUNC(void, WDGM_CODE) WdgM_EB_StoreRedundantData8
(
  P2VAR(uint16,AUTOMATIC,AUTOMATIC) RedundantData,
  uint8 Data
)
{
  const WdgM_EB_SEIdType ExpiredSEIDRedundant = (WdgM_EB_SEIdType)(~Data);
...
/*------------------[WdgM_EB_StoreRedundantData16]---------------------------------------------------*/
/* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,2 */
FUNC(void, WDGM_CODE) WdgM_EB_StoreRedundantData16
(
  P2VAR(uint32,AUTOMATIC,AUTOMATIC) RedundantData,
  uint16 Data
)
{
  const uint16 DataRealProcessed = (uint16)(~Data);
  uint32 LocalDataRedundant;

  LocalDataRedundant = Data;
  LocalDataRedundant |= (uint32)(((uint32)DataRealProcessed) << 16U);

  *RedundantData = LocalDataRedundant;

}

/*------------------[WdgM_EB_GetCheckedRedundantData8]----------------------------------------------*/
FUNC(Std_ReturnType, WDGM_CODE) WdgM_EB_GetCheckedRedundantData8
(
...
{
  Std_ReturnType RetVal;
  const uint8 DataReal = (uint8)(RedundantData);
  const uint8 DataRedundant = (uint8)
      (~((uint8)(((uint16)(RedundantData >> 8U))
          & (uint8)0xFFU)));

  if (DataReal == DataRedundant)
  {
    RetVal = E_OK;
    *Data = DataReal;
  }
...
/*------------------[WdgM_EB_GetCheckedRedundantData16]----------------------------------------------*/
FUNC(Std_ReturnType, WDGM_CODE) WdgM_EB_GetCheckedRedundantData16
(
...
{
  Std_ReturnType RetVal;
  const uint16 DataReal = (uint16)(RedundantData);
  const uint16 DataRedundant = (uint16)
      (~((uint16)(((uint32)(RedundantData >> 16U))
          & (uint16)0xFFFFU)));
...
)
{
  ...
  for (CPId = 0U; CPId < SECfg->NumOfCPs; CPId++)
  {
...
#if (WDGM_EB_DEADLINE_SUPERVISION_ENABLED == STD_ON)
    {
      ...
      /* !LINKSTO WdgM.ASR40.SWS_WdgM_00314,1 */
  ...
  for (TriggerId = 0U; TriggerId < ModeCfg->NumTriggers; TriggerId++)
  {
...
#endif
    {
      ...
      if (TriggerCfg[TriggerId].WatchdogMode != WDGIF_OFF_MODE)
      {
        ...
        /* !LINKSTO WdgM.SWS_WdgM_00122,1,WdgM.ASR40.SWS_WdgM_00223,1 */
        ...
        /* !LINKSTO WdgM.SWS_WdgM_00119,1,WdgM.SWS_WdgM_00120,1,WdgM.SWS_WdgM_00121,1 */
...
)
{
  ...
  /* !LINKSTO WdgM.ASR40.SWS_WdgM_00245,1,WdgM.ASR40.SWS_WdgM_00004.13,1 */
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
          WdgM_EB_ModeSwitchInProgress = TRUE;
#endif
        }
      }
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
...
#else
            /* delay the satellite instances initialization until the master instance initialization finished */
            /* If the satellites are not triggering the watchdogs the return of this function will always be E_OK */
...
#if (WDGM_DEV_ERROR_DETECT == STD_ON)
    /* !LINKSTO WDGM.EB.TIMEPM.WDGM020437,1,WdgM.ASR40.SWS_WdgM_00004.1,1 */
    else
    {
      /* If init request did not work, the Supervisor shall handle this issue */
...

  {
...
#else
      /* For the value of the master core, there is no need to check for data corruption
       * because the master cannot be corrupted (satellites have only read data access).
       * Data is automatically truncated to uint8 */
       ExpectedWdgMMode = (WdgM_ModeType)WdgM_EB_GetCoreSwitchMode(WDGM_EB_MASTER_INSTANCE);
#endif
...
#endif
    {
      ...
      else
      {
        ...
        WdgM_EB_StoreRedundantData8 (CoreInstance->WdgM_EB_CoreSwitchMode, ExpectedWdgMMode);
#else
...
static FUNC(void, WDGM_CODE) WdgM_EB_CheckModeSwitchTimeFrame (void)
{
  ...
  else
  {
   ...
   WdgM_EB_MasterWaitSlavesModeSwitch = 0U;
#if (WDGM_MIXED_CRITICALITY == STD_ON)
   WdgM_EB_ModeSwitchInProgress = FALSE;
#endif
  }
...
#endif
    {
      ReturnValue = WdgM_EB_GetAllExpiredSEIDsHelper(NoOfExpiredSEID, ExpiredSEID);
      ...
      if (ReturnValue == E_OK)
      {
        ...
        {
          ReturnValue = WdgM_EB_ResetAllExpiredSEIDs();
        }
      }
    }
#if(WDGM_EB_MULTICORE_ENABLED == STD_ON)
