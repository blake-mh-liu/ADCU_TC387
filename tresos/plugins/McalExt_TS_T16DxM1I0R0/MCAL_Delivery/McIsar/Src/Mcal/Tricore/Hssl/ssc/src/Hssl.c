...
#define HSSL_WRITE_PENDING                   (1U)
    ...
    ((HSSL_INTERFACE_MODE == HSSL_MASTER) && (HSSL_MULTI_SLAVE_MODE == STD_ON)))
...
#if ((HSSL_NUM_INSTANCES_CONFIGURRED == 1U) && (HSSL1_CONFIGURRED == STD_ON))
#define HSSL_NUM_0                           (0U)
#endif
...
static void Hssl_lDmaUserNotify(const uint8 Channel, const uint32 Event);
...
LOCAL_INLINE uint8 Hssl_lInstanceIDVerify(const Hssl_InstanceID id,\
        const uint8 ServiceId);
LOCAL_INLINE uint8 Hssl_lChannelVerify(const Hssl_ChannelType *const Channel,\
                                       const uint8 ServiceId);
LOCAL_INLINE uint8 Hssl_ldataSizeVerify(const uint16 DataSize,\
                                        const uint8 ServiceId);
...
#endif
#if ( HSSL_DEV_ERROR_DETECT == STD_ON )
static uint8 Hssl_InterfaceMode[HSSL_MAX_NUMBER_OF_INSTANCES];
static uint8 Hssl_DriverState[HSSL_MAX_NUMBER_OF_INSTANCES];
#endif
...
/* MISRA2012_RULE_8_9_JUSTIFICATION: This variable holds the state of slave
mode in multislave mode, hence it cannot be defined at block scope.*/
static Ifx_HSCT* const HSCT_Reg[] = {NULL_PTR, &MODULE_HSCT1};
static Ifx_HSSL* const HSSL_Reg[] = {NULL_PTR, &MODULE_HSSL1};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_WindowStartAddr[] = {HSSL_NUM_0,\
                                               HSSL_NUM_0,\
                                               HSSL_NUM_0,\
                                               HSSL_NUM_0,\
...
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
static const uint32 HSSL_WindowEndAddr[] = {HSSL_NUM_0,\
                                            HSSL_NUM_0,\
                                            HSSL_NUM_0,\
                                            HSSL_NUM_0,\
...
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
static const uint32 HSSL_AccessMode[] = {HSSL_NUM_0,\
                                         HSSL_NUM_0,\
                                         HSSL_NUM_0,\
                                         HSSL_NUM_0,\
                                         HSSL1_ACCESS_MODE_0,\
...
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_Ch2TxMode[] = {HSSL_NUM_0,\
                                        HSSL1_STREAMING_MODE_TX};

/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_Ch2RxMode[] = {HSSL_NUM_0,\
                                        HSSL1_STREAMING_MODE_RX};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
static const uint32 HSSL_Ch2Mode[] = {HSSL_NUM_0,\
                                      HSSL1_STREAMING_CH_MODE};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_TrgtAddr[] = {HSSL_NUM_0, HSSL1_TIDADD_ADDR};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_RefrenceClock[] = {HSSL_NUM_0,\
                                            HSSL1_REFRENCE_CLK};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
static const uint32 HSSL_SysClkDiv[] = {HSSL_NUM_0,\
                                        HSSL1_SYSCLK_DIV};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
static const uint32 HSSL_MTXSpeed[] = {HSSL_NUM_0,\
                                       HSSL1_MASTER_TX_SPEED};
/* MISRA2012_RULE_8_9_JUSTIFICATION: The array contains the configuration data
for Init function. In order to limit stack usage, all such data is defined
 globally. Hence it can't be defined at block scope.*/
 static const uint32 HSSL_MRXSpeed[] = {HSSL_NUM_0,\
...
**                   Mode                                                     **
**                   1 - Enabled/Init, 2 - Enabled/Run, 3 - Sleep             **
**                                                                            **
** Parameters (out): None                                                     **
**                                                                            **
    ...
    if (ErrorFlag == HSSL_ERR_NOT_FOUND)
    {
                ...
                (HSSL_MODE_INIT == Mode))
        {
            ...
            if (HSSL_MODE_SLEEP == Hssl_InterfaceMode[id])
            {
                ...
                {
                    (void)Det_ReportError(HSSL_MODULE_ID, HSSL_MODULE_INSTANCE,\
...
Std_ReturnType Hssl_Init(const Hssl_ConfigType* const Address)
{
    ...
    {
        (void)Det_ReportError(HSSL_MODULE_ID, HSSL_MODULE_INSTANCE,\
                              HSSL_SID_INIT, HSSL_E_INV_POINTER);
        RetVal = E_NOT_OK;
    }
    else
#endif
    {
#if ((HSSL_NUM_INSTANCES_CONFIGURRED == 1U) && (HSSL1_CONFIGURRED == STD_ON))
        for (Index = 1U; ((Index <= HSSL_NUM_INSTANCES_CONFIGURRED) &&
                          (RetVal == E_OK)); Index++)
#else
                      ...
                      const uint8 TransID, const uint8 AckErr)
{
    ...
    if (HSSL_ERR_FOUND != ErrorFlag)
    {
        ...
        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            ...
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_INIT_CH);
        }
    }
                          ...
                          const uint16 InjectedError)
{
   ...
   /* Check for the proper HSSL instance */
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_WRITE);
        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_WRITE);
            ...
            if (HSSL_ERR_FOUND != ErrorFlag)
            {
                ...
                if (HSSL_ERR_FOUND != ErrorFlag)
                {
                    ...
                    if (HSSL_ERR_FOUND != ErrorFlag)
                    {
                        ...
                        if ((NULL_PTR == WriteData) ||    \
                            (NULL_PTR == WriteData->Data) || \
                            (NULL_PTR == WriteData->Address))
                        {
                            (void)Det_ReportError(HSSL_MODULE_ID,\
                                      ...
                                      const Hssl_ChannelType *const Channel)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_WRITE_ACK);
        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_WRITE_ACK);
            if (HSSL_ERR_FOUND != ErrorFlag)
            {
                /* Verify the mode to be run */
...
#endif
/* CYCLOMATIC_Hssl_Read_JUSTIFICATION: This function performs a direct read.
For better readability, all conditions are kept in the same function*/
/*******************************************************************************
** Traceability    : [cover parentID=][/cover]                                **
**                                                                            **
                         ...
                         const uint16 InjectedError)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_READ);

        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_READ);
                ...
                if (HSSL_ERR_FOUND != ErrorFlag)
                {
                    ...
                    if (HSSL_ERR_FOUND != ErrorFlag)
                    {
                        if ((NULL_PTR == DataAddress) ||       \
                            (NULL_PTR == DataAddress->Data) || \
                            (NULL_PTR == DataAddress->Address))
                        {
                            (void)Det_ReportError(HSSL_MODULE_ID,\
                                                  HSSL_MODULE_INSTANCE,\
                                                  HSSL_SID_READ,\
                       ...
                       const Hssl_ChannelType *const Channel)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_READ_ID);

        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_READ_ID);
                                  ...
                                  const Hssl_ChannelType *const Channel)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_TRIGGER);

        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_TRIGGER);
                                       ...
                                       const Hssl_ChannelType *const Channel)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_READ_REPLY);

        if (HSSL_ERR_FOUND != ErrorFlag)
        {

            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_READ_REPLY);
                                 ...
                                 const uint16 InjectedError)
{
    ...
    {
        /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_STREAM);
        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify the mode to be run */
...
{
    Std_ReturnType RetVal = E_NOT_OK;
    Dma_ConfigUpdateType DmaTxConfig = {0};
    uint8 DmaChNum;
    /* MISRA2012_RULE_19_2_JUSTIFICATION: No side effects foreseen by violating
    this MISRA rule. Union usage is required to access SFR data type and
    implement efficiently for clearing of pending service requrest in Interrupt
    ...
    {
        /* Check for driver initialization */
        ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_WRITE_MULTIPLE);

        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_WRITE_MULTIPLE);
...
#endif
    {
       ...
       There are no other operation performed, hence this convesion
       does not cause any issue */
                    DmaTxConfig.SourceAddress = (uint32)(WriteArray->Data);
       /* MISRA2012_RULE_11_6_JUSTIFICATION:The address of the buffer pointer is
       stores in a local variable to be comapred against 0-NULL.
       There are no other operation performed, hence this convesion
       does not cause any issue */
       ...
       There are no other operation performed, hence this convesion
       does not cause any issue */
                    DmaTxConfig.DestAddress = (uint32)WriteDataRegPtr;
                    DmaTxConfig.Config = (
                    ((uint32)HSSL_DMA_CHCFGR_REGVALUE_WRITE) |
                    ((uint32)(NumCmd) & (uint32)HSSL_DMA_CHCFGR_TREL_MASK));

                    DmaTxConfig.UpdateSourceAddress = 1U;
                    DmaTxConfig.UpdateDestAddress = 1U;
                    DmaTxConfig.UpdateConfig = 1U;
/* MISRA2012_RULE_1_3_JUSTIFICATION: Address of auto variable DmaTxConfig
            ...
            API, hence it is not an issue*/
                    Dma_ChUpdate((uint8)DmaChNum, &DmaTxConfig, NULL_PTR);
                              ...
                              const uint32 *Buffer, const uint16 cmd)
{
    ...
    if (ApiID == HSSL_SID_WRITE_MULTIPLE)
    {
        if ((NULL_PTR == WriteBuffer)       || \
            (NULL_PTR == WriteBuffer->Data) || \
            (NULL_PTR == WriteBuffer->Address))
        {
            (void)Det_ReportError(HSSL_MODULE_ID, \
                                  HSSL_MODULE_INSTANCE, \
                                  ApiID, \
...
**                                                                            **
** Parameters (in) : id : Specifies HSSL instance (0: HSSL0, 1: HSSL1).       **
**                   ReadArray : Hssl_ReadDataTemplateType structure which    **
**                   includes array containing write Address.                 **
                               ...
                               const uint16 InjectedError)
{
    Std_ReturnType RetVal = E_NOT_OK;
    Dma_ConfigUpdateType DmaTxConfig = {0};
    Dma_ConfigUpdateType DmaRxConfig = {0};
    uint8 DmaRxChNum;
    uint8 DmaTxChNum;
    Hssl_ChStateType *ChStateTypePtr = NULL_PTR;
    /* MISRA2012_RULE_19_2_JUSTIFICATION: No side effects foreseen by violating
    ...
    {
         /* Check for driver initialization */
         ErrorFlag = Hssl_lInitCallVerify(id, HSSL_SID_READ_MULTIPLE);
        if (HSSL_ERR_FOUND != ErrorFlag)
        {
            /* Verify Channel Number parameter */
            ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_READ_MULTIPLE);
            /* verify the data size */
            if (HSSL_ERR_FOUND != ErrorFlag)
            {
             ErrorFlag = Hssl_ldataSizeVerify(DataSize, HSSL_SID_READ_MULTIPLE);
                ...
                if (HSSL_ERR_FOUND != ErrorFlag)
                {
                    ErrorFlag = Hssl_lRunModeVerify(id, HSSL_SID_READ_MULTIPLE);
                      ...
                      if (HSSL_ERR_FOUND != ErrorFlag)
                      {
                       ErrorFlag = Hssl_lCheckParam(HSSL_SID_READ_MULTIPLE,\
                                                    NULL_PTR,ReadArray,\
                                                    Buffer,NumCmd);
                      }
                }
...
#endif
    {
       ...
       /* MISRA2012_RULE_11_4_JUSTIFICATION:The address of the buffer pointer is
         ...
         There are no other operation performed, hence this convesion
         does not cause any issue */
                    DmaRxConfig.SourceAddress = (uint32)ReadDataPtr;
      /* MISRA2012_RULE_11_6_JUSTIFICATION:The address of the buffer pointer is
            stores in a local variable to be comapred against 0-NULL.
            There are no other operation performed, hence this convesion
            does not cause any issue */
      ...
      There are no other operation performed, hence this convesion
      does not cause any issue */
                    DmaRxConfig.DestAddress = (uint32)(Buffer);
                    DmaRxConfig.Config = (
                    ((uint32)HSSL_DMA_CHCFGR_REGVALUE_READ) |
                    ((uint32)(NumCmd) & (uint32)HSSL_DMA_CHCFGR_TREL_MASK));

                    DmaRxConfig.UpdateSourceAddress = 1U;
                    DmaRxConfig.UpdateDestAddress = 1U;
                    DmaRxConfig.UpdateConfig = 1U;
/* MISRA2012_RULE_1_3_JUSTIFICATION: Address of auto variable DmaRxConfig
              is used inside Dma_ChUpdate() to
            read the values. The address is not used beyond the context of the
            API, hence it is not an issue*/
                   Dma_ChUpdate((uint8)DmaRxChNum, &DmaRxConfig, NULL_PTR);
       ...
       /* MISRA2012_RULE_11_4_JUSTIFICATION:The address of the buffer pointer is
             ...
             There are no other operation performed, hence this convesion
             does not cause any issue */
                  DmaTxConfig.SourceAddress = (uint32)(ReadArray->Address);
       ...
       /* MISRA2012_RULE_11_4_JUSTIFICATION:The address of the buffer pointer is
         ...
         There are no other operation performed, hence this convesion
         does not cause any issue */
                    DmaTxConfig.DestAddress = (uint32)ReadAddrssPtr;
                    DmaTxConfig.Config = (
                    ((uint32)HSSL_DMA_CHCFGR_REGVALUE_READ) |
                    ((uint32)(NumCmd) & (uint32)HSSL_DMA_CHCFGR_TREL_MASK));
...
/* MISRA2012_RULE_1_3_JUSTIFICATION: Address of auto variable DmaTxConfig
            ...
            read the values. The address is not used beyond the context of the
            API, hence it is not an issue*/
                   Dma_ChUpdate((uint8)DmaTxChNum, &DmaTxConfig, NULL_PTR);
                                    ...
                                    Hssl_ChannelErrorType *const ChannelError)
{
    ...
    if (HSSL_ERR_FOUND != ErrorFlag)
    {
        ...
        {
          /* Verify Channel Number parameter */
           ErrorFlag = Hssl_lChannelVerify(Channel, HSSL_SID_GET_CHANNELERROR);
        }
    }
    /*If no error present*/
    if (HSSL_ERR_FOUND != ErrorFlag)
...
** Reentrancy      : Reentrant                                                **
**                                                                            **
** Parameters (in) : Channel : Structute of type Hssl_ChannelType             **
**                   ServiceId : API Service Id number                        **
**                                                                            **
**                                                                            **
** Parameters (out): None                                                     **
                                       ...
                                       const uint8 ServiceId)
{
    ...
    if (HSSL_ERR_NOT_FOUND == ErrorFlag)
    {
        ...
        if ((Channel->Number) >= HSSL_CHANNEL_INVALID)
        {
            (void)Det_ReportError(HSSL_MODULE_ID, HSSL_MODULE_INSTANCE,
...
** Reentrancy      : Reentrant                                                **
**                                                                            **
** Parameters (in) : DataSize - size of the data                              **
**                   ServiceId - API Service Id number                        **
**                                                                            **
** Parameters (out): None                                                     **
**                                                                            **
        ...
        const uint8 ServiceId)
{
    ...
    {
        ErrorFlag = HSSL_ERR_FOUND;
#if (HSSL_HW_MAX_MODULES == 2u)
        if (HSSL1 == id)
        {
            (void)Det_ReportError(HSSL_MODULE_ID, HSSL_MODULE_INSTANCE, \
                                  ServiceId, HSSL_E_INSTANCE_NOT_CONFIGURED);
        }
        else
        {
            (void)Det_ReportError(HSSL_MODULE_ID, HSSL_MODULE_INSTANCE, \
...
** Description     :                                                          **
*******************************************************************************/
void Hssl_DmaCallout(const uint8 Channel, const uint32 Event)
{
  ...
  {
    /* Notify the DMA User */
    Hssl_lDmaUserNotify(Channel, Event);
  }
}
...
** Description     :                                                          **
*******************************************************************************/
void Hssl_DmaErrCallout(const uint8 Channel, const uint32 Event)
{
    uint32 Hssl_id = (uint32)HSSL0;

    /* Disable DMA Channel */
    Dma_ChStopTransfer(Channel);

    #if (HSSL0_CONFIGURRED == STD_ON)
    if ((Channel == (uint8)HSSL0_DMA_MULTI_WRITE_CHANNEL_USED) || \
        (Channel == (uint8)HSSL0_DMA_MULTI_READ_RX_CHANNEL_USED))

    {
      Hssl_id = (uint32)HSSL0;
    }
    #endif

    #if (HSSL1_CONFIGURRED == STD_ON)
    if ((Channel == (uint8)HSSL1_DMA_MULTI_WRITE_CHANNEL_USED) || \
        (Channel == (uint8)HSSL1_DMA_MULTI_READ_RX_CHANNEL_USED))
    {
        Hssl_id = (uint32)HSSL1;
    }
    #endif
    ...
    /* Notify DMA Error Event to User */
    Hssl_lDmaUserNotify(Channel, Event);
}
/*******************************************************************************
**                                                                            **
** Traceability     :                                                         **
**                                                                            **
** Syntax          : static void Hssl_lDmaUserNotify                          **
**                   (const uint8 Channel, const uint32 Event)                **
**                                                                            **
** Service ID      : None(local API)                                          **
**                                                                            **
...
** Description     :                                                          **
*******************************************************************************/
static void Hssl_lDmaUserNotify(const uint8 Channel,  \
                                const uint32 Event)
{
    ...
    #if (HSSL0_CONFIGURRED == STD_ON)
    if (Channel == (uint8)HSSL0_DMA_MULTI_READ_RX_CHANNEL_USED)
    {
    ...
    #endif
    #if (HSSL1_CONFIGURRED == STD_ON)
    if (Channel == (uint8)HSSL1_DMA_MULTI_READ_RX_CHANNEL_USED)
    {
