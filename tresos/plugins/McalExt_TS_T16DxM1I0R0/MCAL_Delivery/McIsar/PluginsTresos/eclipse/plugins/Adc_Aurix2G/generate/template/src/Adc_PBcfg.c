...
[!VAR "AdcConfigShortName" = "concat('_Config_',$postBuildVariant)"!][!//
[!ELSE!][!//
...
[!VAR "SyncGrpMask" = "num:i(0)"!]
[!VAR "TC32xTC33xDerivate" = "num:i(0)"!] 

[!IF "($rel_derivate = '_TRICORE_TC322') or ($rel_derivate = '_TRICORE_TC323') or ($rel_derivate = '_TRICORE_TC324') or ($rel_derivate = '_TRICORE_TC327') or ($rel_derivate = '_TRICORE_TC332') or ($rel_derivate = '_TRICORE_TC333') or ($rel_derivate = '_TRICORE_TC334') or ($rel_derivate = '_TRICORE_TC336') or ($rel_derivate = '_TRICORE_TC337')"!]
  [!VAR "TC32xTC33xDerivate" = "num:i(1)"!]
[!ENDIF!]

[!LOOP "AdcConfigSet/AdcHwUnit/*"!][!//
  [!VAR "AdcHwId" = "num:i(text:split(AdcHwUnitId,'_ADC')[2])"!]
  [!IF "$TC32xTC33xDerivate = num:i(0)"!][!//
...
ADC Code Generator:  Only one master per synchronization group allowed.
          [!ENDERROR!]
        [!ENDIF!]
        [!VAR "SyncGrpMask" = "bit:or($SyncGrpMask, bit:shl(1,$AdcHwId))"!]
        [!IF "$AdcHwId >= num:i(8)"!]
          [!VAR "SyncGrpAMaster" = "num:i($AdcHwId) - num:i(6)"!]
          [!VAR "SyncGrpA" = "bit:or($SyncGrpA, bit:shl(1,($AdcHwId - num:i(6))))"!]
        [!ELSE!]
          [!VAR "SyncGrpAMaster" = "num:i($AdcHwId)"!]
          [!VAR "SyncGrpA" = "bit:or($SyncGrpA, bit:shl(1,$AdcHwId))"!]
        [!ENDIF!]
...
[!//START - Error check to ensure ACH0 or ACH1 channel used only for Alias feature.
[!IF "((contains($CurrentAnCh,'ACH0') = 'true') and (AdcChannel0Alias = num:i(0))) or ((contains($CurrentAnCh,'ACH1') = 'true') and (AdcChannel1Alias = num:i(1)))"!]
  [!ERROR!]
    ADC Code Generator: Channel [!"$CurrentAnCh"!] is meant to be used only for Alias feature. Configure AdcChannel[!IF "contains($CurrentAnCh,'ACH0') = 'true'"!]0[!ELSE!]1[!ENDIF!]Alias with alias channel instead of default value.
  [!ENDERROR!]
[!ENDIF!]
[!//END - Error check to ensure ACH0 or ACH1 channel used only for Alias feature.
[!ENDFOR!]
[!ENDNOCODE!][!//
[!//
[!VAR "AdcChannelCount" = "num:i(count(AdcGroupDefinition/*))"!][!//
...
[!VAR "SyncGrpMaskPrimary" = "bit:and($SyncGrpMask,num:i(3))"!]
[!VAR "SyncGrpMaskSecondary" = "bit:and(bit:shr($SyncGrpMask,num:i(8)),num:i(3))"!]
[!IF "($SyncGrpMaskPrimary > num:i(0)) and ($SyncGrpMaskSecondary > num:i(0))"!]
  [!IF "../../AdcSyncConvMode = 'ADC_SYNC_MASTER' and ((num:i($AdcHwId) =num:i(8)) or (num:i($AdcHwId) = num:i(9)))"!]
    [!IF "(num:i($AdcGrpAnChNum) > num:i(7)) and (AdcSyncConvChannelEnable = 'true')"!][!//
      [!ERROR!]
ADC Code Generator: Sync Channel CH[!"num:i($AdcGrpAnChNum)"!] of Secondary Master Kernel HWUNIT_ADC[!"num:i($AdcHwId)"!] is not available in Primary Slave Kernel [!IF "num:i($SyncGrpMaskPrimary) < 3"!] HWUNIT_ADC[!"num:i($SyncGrpMaskPrimary - 1)"!] [!ELSE!] HWUNIT_ADC0 and HWUNIT_ADC1 [!ENDIF!][!//
      [!ENDERROR!]
    [!ENDIF!]
  [!ENDIF!]
[!ENDIF!]
...
"EmuxEnable"= "$AdcEmuxEnabledHw","EmuxChSampleTime" ="AdcHwUnitInputClass/*[2]/AdcEmuxChSampleTime", "EmuxChPreChargeClkCycles"= "AdcHwUnitInputClass/*[2]/AdcEmuxChPreChargeClkCycles",
"EmuxChConvMode"="AdcHwUnitInputClass/*[2]/AdcEmuxChConvMode", "EmuxChSESPSEnable"="AdcHwUnitInputClass/*[2]/AdcEmuxChSESPSEnable",
"AdcChClassVal"="$AdcChClassVal"!][!//
...
[!IF "AdcSyncConvMode = 'ADC_SYNC_SLAVE' or AdcSyncConvMode = 'ADC_SYNC_MASTER'"!]
  ...
  [!ELSE!]
    ...
    [!IF "$AdcHwId >= num:i(8)"!]
      [!VAR "AdcSyncHwId" = "(num:i($AdcHwId) - num:i(6))"!][!//
    [!ELSE!]
      [!VAR "AdcSyncHwId" = "num:i($AdcHwId)"!][!//
    [!ENDIF!]
    [!VAR "AdcSyncMaster" = "num:i($SyncGrpAMaster)"!][!//
