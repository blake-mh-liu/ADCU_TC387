...
[!VAR "GptConfigShortName" = "concat('_Config_',$postBuildVariant)"!][!//
[!ELSE!][!//
[!VAR "GptConfigShortName"="'_Config'"!][!//
[!ENDIF!]
...
/*    Note: Elektrobit Automotive GmbH modified the original code provided
  ...
  ASCINFINEON-852 GptNotification should not be a list because it is not readable by StbM
  */
[!ENDCODE!][!// End of Template
...
[!SELECT "as:modconf('Gpt')[1]"!][!//
  [!LOOP "./GptChannelConfigSet/GptChannelConfiguration/*"!][!//
    [!IF "node:exists(GptNotification)"!][!//
      [!IF "not(num:isnumber(GptNotification))"!][!//
...
[!SELECT "as:modconf('Gpt')[1]"!][!//
  ...
  [!ENDIF!][!//
  [!VAR "Readfromallcores" = "GptDriverConfiguration/GptReadAcrossCores"!][!//
...
[!FOR "ConfigCoreIndex" = "num:i(1)" TO "$MaxCoreConfigured"!]
  ...
  [!SELECT "as:modconf('Gpt')[1]"!]
    [!SELECT "node:order(./GptChannelConfigSet/GptChannelConfiguration/*, 'node:value(./GptChannelId)')"!]
      ...
      [!IF "contains($CGModuleMap, $pattern)"!]
        [!IF "contains(GptTimerChannelUsage, '_NORMAL')"!]
          [!VAR "GptChannelOffsetMap" = "concat($GptChannelOffsetMap, "CH", num:i(./GptChannelId), '#_', $GptChannelCounter, '*', $GptCurrentCoreId, ',')"!][!//
          [!IF "node:exists(./GtmTimerOutputModuleConfiguration/*[1])"!]
            [!SELECT "./GtmTimerOutputModuleConfiguration/*[1]"!]
              [!CALL "GPT_CG_GetGTMChannelParam","GtmTimerString" = "GtmTimerUsed"!]
                [!IF "$GtmTimerType = 'ATOM'"!]
              ...
              [!IF "$MaxCoreConfigured > num:i(1)"!]
                [!CODE!]
                  ...
                  [!INDENT "0"!]
                    ...
                    #include "Gpt_MemMap.h"
                    [!CR!]
                    [!/* [cover parentID={9957C443-0499-4a72-A484-A782B91A72A1}] */!][!//
    ...
    [!ENDSELECT!]
  [!ENDSELECT!]
  [!VAR "GptChannelOffsetMap1" = "concat($GptChannelOffsetMap1, $GptChannelOffsetMap)"!]
[!ENDFOR!]

[!IF "$Readfromallcores = 'true'"!][!//
  [!CODE!]
    [!AUTOSPACING!]
    [!INDENT "0"!]
      [!CR!]
      ...
      /* MISRA2012_RULE_5_5_JUSTIFICATION: External identifiers going beyond 32 chars.
        in generated code due to Autosar Naming constraints.*/
      #define GPT_START_SEC_CONST_ASIL_B_GLOBAL_32
      /*MISRA2012_RULE_20_1_JUSTIFICATION: File inclusion after pre-processor
      directives is allowed only for MemMap.h*/
      /* MISRA2012_RULE_4_10_JUSTIFICATION: Memmap header is repeatedly included
        without safegaurd. It complies to Autosar guidelines. */
      #include "Gpt_MemMap.h"
      [!CR!]
      [!/* [cover parentID={C376ECC3-5F81-4d71-B8FE-8350C213C798}] */!][!//
      /* Indexes of channels allocated and the cores to which they are allocated */
      static const uint32 Gpt_ChannelCoreIndex[GPT_MAX_CHANNELS] =
      {
    [!ENDINDENT!]
  [!ENDCODE!]
  [!FOR "ChIndex" = "num:i(1)" TO "num:i(num:i($ChannelMax)-num:i($PredefTimerChannels))"!]
    ...
    [!VAR "matchCh" = "concat( '^.*(', $chPattern, '#).*$' )"!]
    [!VAR "fetchItem" = "concat( '^', $chPattern, '#.*$' )"!]
    [!IF "text:match($GptChannelOffsetMap1, $matchCh)"!] [!// Fetch all strings matching the channel.
      [!VAR "grepData" = "translate(text:grep(text:split($GptChannelOffsetMap1, ','), $fetchItem), '[]', '')"!]
      [!VAR "core" = "num:inttohex(num:i(text:split($grepData,'*')[2]), 2)"!]
    [!ENDIF!]
    [!CODE!]
      [!AUTOSPACING!]
      [!INDENT "2"!]
        [!"$core"!]U,  /*  Core[!"num:hextoint($core)"!]*/
      [!ENDINDENT!]
    [!ENDCODE!]
  [!ENDFOR!]
  [!CODE!]
    ...
    [!INDENT "0"!]
      ...
      /* MISRA2012_RULE_5_5_JUSTIFICATION: External identifiers going beyond 32 chars.
        in generated code due to Autosar Naming constraints.*/
      #define GPT_STOP_SEC_CONST_ASIL_B_GLOBAL_32
      /* MISRA2012_RULE_20_1_JUSTIFICATION: File inclusion after pre-processor
        directives is allowed only for MemMap.h*/
      /* MISRA2012_RULE_4_10_JUSTIFICATION: Memmap header is repeatedly included
        without safegaurd. It complies to Autosar guidelines. */
...
[!/*  [/cover] */!][!//
const Gpt_ConfigType Gpt[!"$GptConfigShortName"!] =
{
