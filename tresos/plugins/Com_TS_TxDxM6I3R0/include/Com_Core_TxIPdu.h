 ...
 * Com_EB_InitTxIPdu and additionally starts TxPduIds without IPduGroup assigned
 * \param[in] TxPduId -IPdu which should be initialized/started
 */
TS_MOD_PRIV_DECL FUNC(void, COM_CODE) Com_EB_InitStartTxIPdu
(
   ComIPduRefType TxPduId
);

#if (COM_TMS_ENABLE == STD_ON)
/** \brief Com_EB_ClearTxSGTMSState - clear/initializes TxSGTMSState flag byte
