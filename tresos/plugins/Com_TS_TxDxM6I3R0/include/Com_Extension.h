 ...
 * COM_RX_DATA_TOUT_ACTION_REPLACE_SUBVAL_INITVAL)
 */
...
#define COM_RX_SIGNAL_SIG_GW_FLAG_ADDRESS(signalPtr) \
   (&Com_gDataMemPtr[(Com_gConfigPtr->ComSigGWRxSigFlagBase + \
         ((ComRAMIdxType)((signalPtr)->ComSignalSigGWFlagBit) / 8U))])
...
#if (defined Com_IsBitGroupSet_8)          /* To prevent double declaration */
#error Com_IsBitGroupSet_8 already defined
#endif
/* Check if a bit is set in a variable of type uint8 */
#define Com_IsBitGroupSet_8(Address,Offset,Mask) \
    (uint8) (((uint8)(*(Address))) >> (uint8)(Offset) & (Mask))
