 ...
 *
 * Reason:
 * Easier associating design transitions and transitions in the source code
 * where pre-compile time switches ensure the distinction between identifiers
 * and similarly named macros.
 *
 */
...
static FUNC(void, COM_CODE) Com_EB_Read_ZS
(
      P2VAR(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr,
      P2CONST(uint8, AUTOMATIC, COM_VAR_CLEARED) dataPtr,
      P2CONST(ComSignalBaseType, AUTOMATIC, AUTOMATIC) Com_EB_ReadPropPtr,
...
static FUNC(uint8, COM_CODE) Com_EB_Read_32_Sign_SetRemainBit
(
  P2VAR(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr,
  P2CONST(ComSignalBaseType, AUTOMATIC, AUTOMATIC) Com_EB_ReadPropPtr
);
...
CONST(Com_ReadExtFuncTableType, AUTOMATIC) COM_READFUNCS[COM_CORE_READ_ARRAY_SIZE] =
{
  ...
  /*  38 */           COM_EB_READ_SET_TABLE( Com_EB_Read_ZS,             Com_EB_ReadReceived_DET,  Com_EB_ReadCopy_DET),
  ...
  /* 111 */ /*  47 */ COM_EB_READ_SET_TABLE( Com_EB_Read_SignalExt,     Com_EB_ReadReceived_DET,   Com_EB_ReadCopyGSig_64_E2),
...
)
{
  ...
  if(Com_EB_ReadPropPtr->ComBitSize > 32U)
  {
    ...
    if(COM_SINT64 == COM_GET_RX_SIGNAL_SIGNAL_TYPE(Com_EB_ReadPropPtr))
    {
      /* signed extension gets applied */
      (void)Com_EB_Read_32_Sign_SetRemainBit(&SignalDataPtr_loc->HiWord, &Com_EB_ReadProp_loc);
    }
  ...
  {
    /* reset all bits in the higher word */
    SignalDataPtr_loc->HiWord = (uint32)(0UL);
    ...
    if(COM_SINT64 == COM_GET_RX_SIGNAL_SIGNAL_TYPE(Com_EB_ReadPropPtr))
    {
      ...
      uint8 const MSB = Com_EB_Read_32_Sign_SetRemainBit(&SignalDataPtr_loc->LoWord, Com_EB_ReadPropPtr);
      if (MSB != 0U)
      {
        /* HiWord gets set according to the MSB so that signed extension gets applied */
        SignalDataPtr_loc->HiWord = (uint32)(0xFFFFFFFFUL);
      }
    }
  }
...
/* Deviation MISRAC2012-2 <+2> */
  ...
  if(Com_EB_ReadPropPtr->ComBitSize > 32U)
  {
    ...
    /* get remainng bits of the high word */
    ComSignalBitSizeType const RemBitSizeHiWord = Com_EB_ReadPropPtr->ComBitSize - 32U;

    /* get the bit count to the next upper byte boundary */
    uint32 const BitCountUpperBytBoundaryStartByt = (7U - ((uint32)Com_EB_ReadPropPtr->ComSignalPositionInBit % 8U));

    /* get the bit pos of the upper byte boundary from the start byte */
    uint32 const BitPosUpperBytBoundaryStartByt = Com_EB_ReadPropPtr->ComSignalPositionInBit + BitCountUpperBytBoundaryStartByt;

    /* byte / bit count to be skipped to the target byte */
    uint32 const CountRowSkip = ((RemBitSizeHiWord + BitCountUpperBytBoundaryStartByt) / 8U);
    uint32 const BitCountRowSkip = CountRowSkip * 8U;

    /* remaining bits from bitsize */
    uint32 const RemBitSize = ((uint32)Com_EB_ReadProp_loc.ComBitSize - 32U + BitCountUpperBytBoundaryStartByt - BitCountRowSkip);

    /* bit pos of upper byte boundary from target byte */
    uint32 const BitPosUpperBytBoundaryTargetByt = BitPosUpperBytBoundaryStartByt + BitCountRowSkip;

    /* signal position of low word */
    ComSignalDescBitType const BitPosLoWord = (ComSignalDescBitType)(BitPosUpperBytBoundaryTargetByt - RemBitSize);

    /* set ComBitSize for higher bytes */
    Com_EB_ReadProp_loc.ComBitSize = RemBitSizeHiWord;
    ...
    /* adapt Com_EB_ReadPropPtr_loc for low 32 bits */
    Com_EB_ReadProp_loc.ComSignalPositionInBit = BitPosLoWord;
    Com_EB_ReadProp_loc.ComBitSize = 32U;
...
)
{
  ...
  Com_EB_Read_64_DL_E1(SignalDataPtr, dataPtr, Com_EB_ReadPropPtr, retvalPtr);
...
)
{
  ...
  Com_EB_Read_64_DL_E2(SignalDataPtr, dataPtr, Com_EB_ReadPropPtr, retvalPtr);
...
)
{
   ...
   {
      /* Deviation MISRAC2012-1 */
      uint8 const MSB = (uint8)((*SignalDataPtrU8) >> (Com_EB_ReadPropPtr->ComBitSize - 1U));
      if (MSB != 0U)
      {
...
)
{
   ...
   {
      /* Deviation MISRAC2012-1 */
      uint8 const MSB = (uint8)((*SignalDataPtrU16) >> (Com_EB_ReadPropPtr->ComBitSize - 1U));
      if (MSB != 0U)
      {
...
)
{
   ...
   (void)Com_EB_Read_32_Sign_SetRemainBit(SignalDataPtr, Com_EB_ReadPropPtr);
...
)
{
  ...
  /* Deviation MISRAC2012-1 */
  uint8 const MSB = (uint8)((*SignalDataPtrU32) >> (Com_EB_ReadPropPtr->ComBitSize - 1U));
  if (MSB != 0U)
  {
...
)
{
  ...
  const uint32 LSB = (helpTemp + (dTemp * 16U)) - shiftBits;
...
)
{
  ...
  CONSTP2VAR(uint8, AUTOMATIC, COM_VAR_CLEARED) pdudataptr =
      &Com_gDataMemPtr[(uint32)IPduPtr->ComIPduValueRef + (uint32)startbyte + BufferOffset];
