 ...
 */
static FUNC(boolean, COM_CODE) Com_EB_CheckTMSforPdu
(
 ...
 * \retval TRUE if I-Pdu shall be sent, else FALSE
 */
static FUNC(boolean, COM_CODE) Com_EB_TMSwitch
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   boolean CurrentTM
...
static FUNC(boolean, COM_CODE) Com_EB_CheckPeriodicSendforPdu
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
...
static FUNC(boolean, COM_CODE) Com_EB_CheckNTimesSendforPdu
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 * \param[out] None
 */
static FUNC(void, COM_CODE) Com_EB_CheckMdtforPdu
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
...
static FUNC(void, COM_CODE) Com_EB_CheckTxDMforPdu
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
#if (COM_MDT_QUEUE_AVAILABLE == STD_ON)
...
static FUNC(void, COM_CODE) Com_EB_HandleDeferredTxConfirm
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 * \param[out] None
 */
static FUNC(void, COM_CODE) Com_EB_Handle_Send_Pdu
(
   ComIPduRefType PduId,
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
 ...
 * \param[out] None
 */
static FUNC(void, COM_CODE) Com_EB_MainFunctionTx_hlp
(
    ComIPduRefType PduId
);
...
)
{
   ...
   if (timeout)
   {
...

      {
         ...
         if ((uint8)COM_TX_MODE_MODE_NONE == AktTM)
#endif /* (COM_TM_AVAILABLE == STD_ON) */
         {
...
)
{
   ...
   if (Com_IsBitSet_8(&(IPduPtr)->ComTxIPduFlags, COM_TXIPDUFLAG_SIGNAL_PROCESSING) == COM_DEFERED)
...
)
{
   ...
   if (Com_InitStatus == COM_EB_INIT)
   {
      ...
      Com_GlobalTimes_Tx[ComMainFunctionsTx_Idx] += 1U;
