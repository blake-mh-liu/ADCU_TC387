...
static FUNC(void, COM_CODE) Com_EB_Read_SignalExt_SigBuffer
(
...
#if (COM_VALUE_REPLACE == STD_ON)
static FUNC(boolean, COM_CODE) Com_EB_Read_SignalReplace
(
   P2CONST(ComRxSignalType, AUTOMATIC, AUTOMATIC) SignalPtr
);
#endif /* (COM_VALUE_REPLACE == STD_ON) */
 ...
 */
static FUNC_P2CONST(ComRxSignalType, COM_APPL_CONST, AUTOMATIC) Com_GetRxSignalCfg
(
 ...
 */
static FUNC(uint8, COM_CODE) Com_EB_ReadValue_Uint8Dyn
(
   P2VAR(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr,
 ...
 */
static FUNC(boolean, COM_CODE) Com_EB_CheckRxSignalDM
(
   P2CONST(ComRxSignalType, AUTOMATIC, AUTOMATIC) SignalPtr
);
...
#if (COM_VALUE_REPLACE == STD_ON)
      boolean const replaceSigVal =
               Com_EB_Read_SignalReplace(SignalPtr);

      if (replaceSigVal == TRUE)
      {
        /* read timeout value */

        SigLen = IPduPtr->ComIPduTimeoutSize - startbyte;
        /* check if the provided buffer is big enough for the timeout value of signal */
        if( (*LengthPtr) >= SigLen )
        {
           /* value from Pdu shall be replaced by the timeout value */

           /* Deviation MISRAC2012-1 <+2> */
           CONSTP2CONST(uint8, AUTOMATIC, COM_VAR_CLEARED) timeoutDataPduPtr =
                    COM_GET_CONFIG_ELEMENT(uint8, IPduPtr->ComTimeoutValueRef, 0U);

           CONSTP2CONST(uint8, AUTOMATIC, COM_VAR_CLEARED) timeoutDataDynSigPtr =
                    &timeoutDataPduPtr[startbyte];

           /* copy init value */
           TS_MemCpy(SignalDataPtr,
                     timeoutDataDynSigPtr,
                     SigLen);
           retval = E_OK;
        }
...
)
{
   ...
   if (COM_GET_RX_SIGNAL_EXT_TIMEOUT_ACTION(SignalExtPtr) == COM_RX_DATA_TOUT_ACTION_REPLACE_SUBVAL_INITVAL)
#endif /* (COM_RX_DATA_TOUT_ACTION == COM_RX_DATA_TOUT_ACTION_CONFIG) */
   {
...
#endif /* (COM_RX_SIGNAL_BASED_DM == STD_ON) */
      {
         ...
         TS_GetBit(&Com_gDataMemPtr[(Com_gConfigPtr->ComRxIPduStatusBase + SignalPtr->ComIPduRef)],
...
)
{
   ...
   boolean replaceSigVal = FALSE;

#if (COM_FILTER_RECEIVER_ENABLE == STD_ON)
   uint8 filterType = SignalPtr->ComSignalBase.ComFilterConf.ComFilterType;
   replaceSigVal = (filterType == COM_F_NEVER);
#endif /* (COM_FILTER_RECEIVER_ENABLE == STD_ON) */

#if (COM_RX_DATA_TOUT_ACTION != COM_RX_DATA_TOUT_ACTION_NONE)
   {
      const boolean rxDMReplace = Com_EB_CheckRxSignalDM(SignalPtr);

      replaceSigVal = replaceSigVal || rxDMReplace;
   }
#endif /* (COM_RX_DATA_TOUT_ACTION != COM_RX_DATA_TOUT_ACTION_NONE) */

   return replaceSigVal;
}
...
)
{
  ...
  CONSTP2CONST(ComRxSignalType, AUTOMATIC, COM_APPL_CONST) SignalPtr =
   ...
   if (replaceSigVal == TRUE)
   {
     ...
     /* replace value with timeout value */
