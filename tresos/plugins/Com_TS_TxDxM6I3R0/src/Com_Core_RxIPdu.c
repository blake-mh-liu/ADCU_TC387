...
#define COM_MULTI_ROUTE_STATUS_LENGTH (uint32)(2U*COM_MAIN_FUNCS_TX)
 ...
 */
static FUNC(uint8, COM_CODE) Com_EB_HandlePduRxDM
(
 ...
 * stored
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxIPdu_EnableRxDM
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
 ...
 * stored
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxIPdu_DisableRxDM
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2VAR(uint8, AUTOMATIC, COM_VAR_CLEARED) IPduFlagBytePtr
);
 ...
 * stored
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxIPdu_Start
(
 ...
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxSignalsDMUBOnly
(
      P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
 ...
 * \param[in] PduId - ID of the I-PDU
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxSig_W_F_A_A_Noti
(
      P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
      ComIPduRefType PduId
);
 ...
 * time out
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxSignalsWithBuffer
(
        P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
        ComIPduRefType PduId,
        uint8 pduRxDMRestat
 ...
 * \param[in] PduId - ID of the I-PDU
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxSignalGroups
(
      P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
      ComIPduRefType PduId
);
 ...
 * \param[in] IPduPtr - pointer to the description of the signal
 */
static FUNC(void, COM_CODE) Com_EB_InitRxSignalsWithFilter
(
      P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
#endif /* (COM_FILTER_RECEIVER_ENABLE == STD_ON) */
 ...
 */
static FUNC(void, COM_CODE) Com_EB_InitRxIPdu
(
 ...
 */
static FUNC(uint8, COM_CODE) Com_EB_CheckRxSignal_Receive_UpdateBitAndDM
(
...
static FUNC(uint8, COM_CODE) Com_EB_CheckRxSignalGroup_Receive_UpdateBitAndDM
(
...
)
{
   ...
   if( Com_IsBitSet_8(&(IPduPtr)->ComRxIPduFlags, COM_RXIPDUFLAG_RX_IPDU_DM) )
   {
...
)
{
   ...
   }                            /* Com_IsBitSet_8(&(IPduPtr)->ComRxIPduFlags, COM_RXIPDUFLAG_RX_IPDU_DM) */
...
)
{
   ...
   {
     /* Deviation MISRAC2012-1 */
     CONSTP2VAR(ComIPduSizeType, AUTOMATIC, COM_VAR_CLEARED) RxIPduSizePtr =
         &COM_RX_IPDU_SIZE_VAR(RxPduId);

     TS_AtomicAssign16(*RxIPduSizePtr , (ComIPduSizeType) 0U );
   }
   else
#endif /* (COM_PBCFG_VARIANT_SUPPORT_ENABLED == STD_ON) */
   {
...
#if (COM_RX_DYN_LENGTH_IPDU_ENABLE == STD_ON)
      {
         ...
         TS_AtomicAssign16(*RxIPduSizePtr , IPduPtr->ComIPduInitSize );
      }
#endif /* (COM_RX_DYN_LENGTH_IPDU_ENABLE == STD_ON) */
...
/** \brief Com_EB_InitStartRxIPdu - initialized by a call to
 * Com_EB_InitRxIPdu and additionally starts RxPduIds without IPduGroup assignment.
 * \param[in] RxPduId -IPdu which should be initialized/started
 */
TS_MOD_PRIV_DEFN FUNC(void, COM_CODE) Com_EB_InitStartRxIPdu
(
   ComIPduRefType RxPduId
)
{
  ...
  /* Deviation MISRAC2012-1 <+2> */
  P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr =
      COM_GET_CONFIG_ELEMENT(ComRxIPduType, Com_gConfigPtr->ComRxIPduRef, RxPduId);

  Com_EB_InitRxIPdu(RxPduId);

  /* start RxPdu if no IPduGroup assigned */
  if(IPduPtr->ComIPduGroupMaskRef == COM_IPDUGROUPMASK_REF_INVALID)
  {
    /* pointer to byte with status flags of the Pdu */
    CONSTP2VAR(uint8, AUTOMATIC, COM_VAR_CLEARED) IPduFlagBytePtr =
             &Com_gDataMemPtr[(Com_gConfigPtr->ComRxIPduStatusBase + RxPduId)];
