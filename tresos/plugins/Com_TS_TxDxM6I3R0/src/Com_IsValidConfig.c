 ...
 *
 * MISRAC2012-4) Deviated Rule: 2.3 (advisory)
 * A project should not contain unused type declarations.
 *
 * Reason:
 * Declared type not used in all test sequences hence some test sequences where
 * it is not used reports violation
 *
 * MISRAC2012-5) Deviated Rule: 11.5 (advisory)
 ...
 *
 * Reason:
 * False-positive: compiler optimizes depending on test configuration settings
 */
...
/*==================[type definitions]=======================================*/
    ...
    * in Com_EB_Read_SignalExt() */
    /* Deviation MISRAC2012-1, MISRAC2012-2, MISRAC2012-3, MISRAC2012-4 */
...
/** \brief Com_EB_CheckIPduGroups - performs sanity checks all for IPduGroups
 ...
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckIPduGroups
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRxSignals
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxSignal
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxSignalExt
(
   P2CONST(ComRxSignalType, AUTOMATIC, COM_APPL_CONST) SignalPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr,
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxSignalG
(
...
/** \brief Com_EB_CheckAllTxSignals - performs sanity checks all for TxSignals
 ** \param[in] CfgPtr - pointer to a post build configuration
 ** \return Function execution success status
 ** \retval E_OK if no error was found
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllTxSignals
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRxSignalGs
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllTxSignalGs
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxSignal
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxSignalG
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr,
 ...
 *         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckGroupSigTMS
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr,
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRxPDUs
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllTxPDUs
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxPdu
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu_RxSigs_W_F_A_A_Noti
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu_RxSignalsDMUBOnly
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu_Callouts
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu_RxSigs
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRxPdu_RxSigGroups
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr,
   ComIPduRefType PduId
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxPdu_TMS
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxPdu_Callouts
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxPdu_TxSigs
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckTxPdu_TxSigGroups
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr,
   ComIPduRefType PduId
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRouteSignals
(
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRouteSignals_GWSourceRefs
(
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRouteSignal_GWSourceRefs
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRouteSignalGroup_GWSourceRefs
(
  P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
  P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRouteSignals_GWDestinationSigRef
(
   P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_Check_DestSignalRef
(
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckRouteGroupSignal_GWSourceRefs
(
  P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
  P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_CheckAllRouteSignals_GWDestinationSigGRef
(
   P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
   P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
 ...
 **         E_NOT_OK if an error was found
 */
static FUNC(Std_ReturnType, COM_CODE) Com_EB_Check_DestSignalGRef
(
  P2CONST(ComGWDestinationType, AUTOMATIC, COM_APPL_CONST) GWDestinationPtr,
  P2CONST(Com_ConfigType, AUTOMATIC, COM_APPL_CONST) CfgPtr
);
...
#if (COM_PRECONDITION_ASSERT_ENABLED == STD_ON)
    /* CHECK: NOPARSE */
                  retval |= Com_EB_CheckIPduGroups(TmpCfgPtr);
...
)
{
  ...
  if(CfgPtr->ComIPduGroupsNum > 0U)
  {

    if ((CfgPtr->ComRxIpduGroupVectorByteEnd == 0U) ||
        (CfgPtr->ComRxIpduGroupVectorByteEnd > COM_IPDU_GROUP_VECTOR_NUM_BYTES))
    {
       retval = E_NOT_OK;
       COM_PRECONDITION_ASSERT_NO_EVAL(
             ((CfgPtr->ComRxIpduGroupVectorByteEnd == 0U) ||
              (CfgPtr->ComRxIpduGroupVectorByteEnd > COM_IPDU_GROUP_VECTOR_NUM_BYTES)),
             COM_INTERNAL_API_ID);
    } else if (CfgPtr->ComTxIpduGroupVectorByteStart >= COM_IPDU_GROUP_VECTOR_NUM_BYTES)
    {
       retval = E_NOT_OK;
       COM_PRECONDITION_ASSERT_NO_EVAL(
             (CfgPtr->ComTxIpduGroupVectorByteStart >= COM_IPDU_GROUP_VECTOR_NUM_BYTES),
             COM_INTERNAL_API_ID);
    }
    else
    {
    ...
    if (CfgPtr->ComRxIpduGroupVectorBitEnd >= (COM_IPDU_GROUP_VECTOR_NUM_BYTES * 8U))
    {
       retval = E_NOT_OK;
       COM_PRECONDITION_ASSERT_NO_EVAL(
           CfgPtr->ComRxIpduGroupVectorBitEnd >= (COM_IPDU_GROUP_VECTOR_NUM_BYTES * 8U),
             COM_INTERNAL_API_ID);
    } else if (CfgPtr->ComTxIpduGroupVectorBitStart >= (COM_IPDU_GROUP_VECTOR_NUM_BYTES * 8U))
    {
       retval = E_NOT_OK;
       COM_PRECONDITION_ASSERT_NO_EVAL(
             CfgPtr->ComTxIpduGroupVectorBitStart >= (COM_IPDU_GROUP_VECTOR_NUM_BYTES * 8U),
             COM_INTERNAL_API_ID);
    }
    else
    {
...
#if ((COM_NOTIFY_RECEIVERS_ENABLE == STD_ON) && (COM_CBK_RX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE))
         if (pduId_check == E_OK)
         {
            /* Deviation MISRAC2012-5 */
            CONSTP2CONST(ComRxSignalExtType, AUTOMATIC, COM_APPL_CONST) SignalExtPtr =
...
#endif /* ((COM_NOTIFY_RECEIVERS_ENABLE == STD_ON) && (COM_CBK_RX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE)) */
         ...
         if (signalGId == COM_SIGNAL_INVALID)
         {
           ...
           if (TRUE == COM_IS_TX_DUMMY_SIGNAL(SignalPtr))
           {
             ...
             if( (SignalPtr->ComSignalBase.ComBufferRef == 0U) &&
                 (SignalPtr->ComIPduRef == 0U) )
             {
               if( (COM_UINT8_DYN == (uint32)COM_GET_TX_SIGNAL_SIGNAL_TYPE(SignalPtr)) &&
                   (SignalPtr->ComSignalBase.ComBitSize == (ComSignalBitSizeType)0U) )
               {
                 /* dyn dummy signal / dyn dummy group signal is valid */
                 sigE_check = E_OK;
               }
               else if( (COM_UINT8_DYN != (uint32)COM_GET_TX_SIGNAL_SIGNAL_TYPE(SignalPtr)) &&
                        (SignalPtr->ComSignalBase.ComBitSize == (ComSignalBitSizeType)COM_SIGNAL_INVALID) )
               {
...
)
{
   ...
   fail_condition =
         (!((COM_GET_COM_TX_MODE_TRUE(IPduPtr->ComTxModeModes) == ((uint8)COM_TX_MODE_MODE_NONE))
               ||
               (COM_GET_COM_TX_MODE_TRUE(IPduPtr->ComTxModeModes) == ((uint8)COM_TX_MODE_MODE_DIRECT)))
         )
         &&
   ...
   fail_condition =
         ...
         (Com_IsBitGroupSet_8(&(IPduPtr)->ComTxIPduFlags,COM_TXIPDUFLAG_OFFSET_TX_IPDU_CLEAR_UPDATE,
                              COM_TXIPDUFLAG_MASK_TX_IPDU_CLEAR_UPDATE) != COM_CL_UB_NEVER)
         &&
   ...
   fail_condition =
         (!((COM_GET_COM_TX_MODE_FALSE(IPduPtr->ComTxModeModes) == ((uint8)COM_TX_MODE_MODE_NONE))
               ||
               (COM_GET_COM_TX_MODE_FALSE(IPduPtr->ComTxModeModes) == ((uint8)COM_TX_MODE_MODE_DIRECT)))
         )
         &&
