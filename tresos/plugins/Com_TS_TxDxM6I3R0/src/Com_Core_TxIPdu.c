 ...
 *
 * Reason:
 * Intended use of bitwise shift and modulo arithmetic operator report the
 ...
 *
 * Reason:
 * TS_MAKEREF2CFG uses offsetof which casts a void pointer to a pointer to
 * struct and then obtaining the address of member within the struct. It
 * returns the address offset to the member as size_t type which is not a
 * dereferenced pointer value. TS_MAKEREF2CFG explicit casts the offsetof
 * value to TS_CfgOffsetType.This means that the comparison does not require
 * a null pointer constant. Instead a zero with an explicit cast to
 * TS_CfgOffsetType is valid.
 *
 */
 ...
 */
static FUNC(void, COM_CODE) Com_EB_HandleMDT
(
 ...
 */
static FUNC(void, COM_CODE) Com_EB_HandleTxIPdu_Start
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   boolean Initialize,
 ...
 * \param[in] IPduId - Id of the I-Pdu
 */
static FUNC(void, COM_CODE) Com_EB_HandleTxIPdu_Stop
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   ComIPduRefType IPduId
);
 ...
 */
static FUNC(void, COM_CODE) Com_EB_InitTxIPdu
(
 ...
 * \param[in] TxPduId -IPdu which should be initialized
 */
static FUNC(void, COM_CODE) Com_EB_InitTxIPdu_PduBuffer
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   ComIPduRefType TxPduId
);
 ...
 * \param[in] IPduPtr pointer to the configuration of the Tx-IPdu
 */
static FUNC(void, COM_CODE) Com_EB_InitTxIPdu_SignalGroups
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 */
static FUNC(void, COM_CODE) Com_EB_InitTxIPdu_HandleFlags
(
      P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
      ComIPduRefType TxPduId
);
...
static FUNC(void, COM_CODE) Com_EB_InitTx_Signal
(
 ...
 * \param[in] IPduPtr pointer to the configuration of the Tx-IPdu
 */
static FUNC(void, COM_CODE) Com_EB_InitTxIPdu_Signals
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);
 ...
 */
static FUNC(void, COM_CODE) Com_EB_StartIPdu
(
   P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
   ComIPduRefType PduId
);
...
)
{
   ...
   (COM_CL_UB_TX_TXCONFIRMATION_ENABLE == STD_OFF)))
         ...
         if (Com_IsBitGroupSet_8(&(IPduPtr)->ComTxIPduFlags,COM_TXIPDUFLAG_OFFSET_TX_IPDU_CLEAR_UPDATE,
                                 COM_TXIPDUFLAG_MASK_TX_IPDU_CLEAR_UPDATE) == COM_CL_UB_TRANSMIT)
#endif
         {
...
)
{
   ...
   {
     /* Deviation MISRAC2012-1 */
     CONSTP2VAR(ComIPduSizeType, AUTOMATIC, COM_VAR_CLEARED) TxIPduSizePtr =
         &COM_TX_IPDU_SIZE_VAR(TxPduId);

     TS_AtomicAssign16(*TxIPduSizePtr , (ComIPduSizeType) 0U );
   }
   else
#endif /* (COM_PBCFG_VARIANT_SUPPORT_ENABLED == STD_ON) */
   {
...
#if (COM_TX_DYN_LENGTH_IPDU_ENABLE == STD_ON)
     {
       ...
       TS_AtomicAssign16(*TxIPduSizePtr , IPduPtr->ComIPduInitSize );
     }
#else
...
)
{
  ...
  if (SignalGPtr->ComSignalGTMSFlagsInitValueRef != (TS_CfgOffsetType)0U)
  {
...
)
{
   ...
   if (Com_IsBitSet_8(&(IPduPtr)->ComTxIPduFlags, COM_TXIPDUFLAG_IPDU_INIT_TM) == TRUE)
   {
   ...
   /* Deviation MISRAC2012-3 */
   if(IPduPtr->ComIPduTMSFlagsInitValueRef != (TS_CfgOffsetType)0U)
   {
      ...
      case COM_TX_MODE_MODE_MIXED_N_TIMES:
...
#if (COM_PERIODIC_QUEUE_AVAILABLE == STD_ON)
         {
            if (TimeToSendFirst == 0U)
            {
               ...
               if (Com_IsBitSet_8(&(IPduPtr)->ComTxIPduFlags, COM_TXIPDUFLAG_DM_AVAILABLE) == COM_TX_DM_ON)
               {
...
/** \brief Com_EB_InitStartTxIPdu - initialized by a call to
 * Com_EB_InitTxIPdu and additionally starts TxPduIds without IPduGroup assignment.
 * \param[in] TxPduId -IPdu which should be initialized/started
 */
TS_MOD_PRIV_DEFN FUNC(void, COM_CODE) Com_EB_InitStartTxIPdu
(
   ComIPduRefType TxPduId
)
{
  ...
  /* Deviation MISRAC2012-1 */
  P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr =
      COM_GET_CONFIG_ELEMENT(ComTxIPduType, Com_gConfigPtr->ComTxIPduRef, TxPduId);

  Com_EB_InitTxIPdu(TxPduId);

  /* start TxPduId if no IPduGroup assigned */
  if(IPduPtr->ComIPduGroupMaskRef == COM_IPDUGROUPMASK_REF_INVALID)
  {
    Com_EB_StartIPdu(IPduPtr, TxPduId);
  }
}                               /* Com_EB_InitStartTxIPdu */
...
{
   boolean TxDM = (Com_IsBitSet_8(&(IPduPtr)->ComTxIPduFlags, COM_TXIPDUFLAG_DM_AVAILABLE)
       == COM_TX_DM_ON) ? TRUE: FALSE;
