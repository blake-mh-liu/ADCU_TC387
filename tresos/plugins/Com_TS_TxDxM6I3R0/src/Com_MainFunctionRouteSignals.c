 ...
 */
static FUNC(void, COM_CODE) Com_EB_SetSendSignalGroup
(
 ...
 * \param[in] ComMainFunctionsTx_Idx - Id of the main function
 */
static FUNC(void, COM_CODE) Com_EB_CheckSendSignalGroup
(
#if (COM_MULTI_ROUTE_MAIN_ENABLE == STD_ON)
   Com_MainFunctionTx_IdxType ComMainFunctionsTx_Idx
#else
 ...
 * \param[in] ComMainFunctionsTx_Idx - Id of the main function
 */
static FUNC(void, COM_CODE) Com_EB_RouteSignalHelper
(
 ...
 */
static FUNC(void, COM_CODE) Com_EB_RouteSignalHelper_Write
(
   P2CONST(ComRxSignalType, AUTOMATIC, COM_APPL_CONST) SrcSignalPtr,
 ...
 * \param[in] ComMainFunctionsTx_Idx - Id of the main function
 */
static FUNC(void, COM_CODE) Com_EB_RouteSG_2_SG
(
...
static FUNC(void, COM_CODE) Com_EB_RouteReceiveSignalGroup
(
 ...
 * else FALSE
 */
static  FUNC(boolean, COM_CODE) Com_EB_RouteSignalGroup
(
 ...
 */
static FUNC(void, COM_CODE) Com_EB_RouteGroupSignal
(
   P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
 ...
 * \param[in] ComMainFunctionsTx_Idx - Id of the main function
 */
static FUNC(void, COM_CODE) Com_EB_RouteSignal
(
   P2CONST(ComGWSourceType, AUTOMATIC, COM_APPL_CONST) GWSourcePtr,
   P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) SrcIPduPtr
#if (COM_MULTI_ROUTE_MAIN_ENABLE == STD_ON)
 ...
 * \param[in] ComMainFunctionsTx_Idx - Id of the main function
 */
static FUNC(void, COM_CODE) Com_MainFunctionRouteSignals_Src_Generic_hlp
(
...
)
{
   ...
   uint8 exit_flag = 1U;
   ComSignalGroupRefType TxSignalGroupQueueEntryPerTxMain = 0U;

   const ComSignalGroupRefType TxSignalGroupQueueSizePerTxMain =
    (ComSignalGroupRefType)(Com_gConfigPtr->ComSigGWTxSignalGroupQueueMax/COM_MAIN_FUNCS_TX);

#if (COM_MULTI_ROUTE_MAIN_ENABLE == STD_ON)
   TxSignalGroupQueueEntryPerTxMain =
         (const ComSignalGroupRefType)(ComMainFunctionsTx_Idx*TxSignalGroupQueueSizePerTxMain);
#endif /* (COM_MULTI_ROUTE_MAIN_ENABLE == STD_ON) */

   {
      ...
      CONSTP2VAR(ComSignalGroupRefType, AUTOMATIC, COM_VAR_CLEARED) TxSignalGroupQueueValPtr =
         &TxSignalGroupQueueBase[TxSignalGroupQueueEntryPerTxMain];

      for (ii = 0U; (ii < TxSignalGroupQueueSizePerTxMain) && (exit_flag == 1U); ii++)
      {

        if (TxSignalGroupQueueValPtr[ii] == SignalGId)
        {
           /* signal group is already in list, exit loop */
           exit_flag = 0U;
        } else if (TxSignalGroupQueueValPtr[ii] == COM_SIGNAL_GROUP_INVALID)
        {
           /* entry in list is free, add signal group */
           TxSignalGroupQueueValPtr[ii] = SignalGId;
...
)
{
...

   {
     ...
     for (ii = 0U; (ii < TxSignalGroupQueueSizePerTxMain) && (exit_flag == 1U); ii++)
     {
          ComSignalGroupRefType const SignalGId  = TxSignalGroupQueueValPtr[ii];
          ...
          if (SignalGId != COM_SIGNAL_GROUP_INVALID)
          {
             ...
             TxSignalGroupQueueValPtr[ii] = COM_SIGNAL_GROUP_INVALID;
...
)
{
   ...
   {                            /* signal group is gated */
      ...
      if (GWSourcePtr->ComGWDestinationNumRef > 0U)
      {
