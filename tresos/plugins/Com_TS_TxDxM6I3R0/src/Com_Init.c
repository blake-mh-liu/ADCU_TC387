...
)
{
...

   {
      ...
      /* Initialize IpduGroupVectorState and IpduGroupVectorStateDM */
      {
        Com_IpduGroupIdType IpduGroup;

        /* Deviation TASKING-1 */
        for(IpduGroup = 0U; IpduGroup < COM_IPDU_GROUP_VECTOR_NUM_BYTES; IpduGroup++)
        {
          /* clear the byte */
          Com_IpduGroupVectorState[IpduGroup] = 0U;
          Com_IpduGroupVectorStateDM[IpduGroup] = 0U;
        }
      }
#endif /* (COM_EB_IPDUGROUPSTARTSTOP_ENABLE == STD_ON) */

      /* Initialize/start Rx-Pdus */
