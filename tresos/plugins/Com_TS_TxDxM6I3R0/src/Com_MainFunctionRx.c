...
static FUNC(void, COM_CODE) Com_EB_CheckRxDMTable
(
...
static FUNC(void, COM_CODE) Com_EB_CheckRxSignalDMTable
(
#if (COM_TIMEBASE == STD_ON)
    void
#else
...
static FUNC(void, COM_CODE) Com_EB_CheckRxSignalGroupDMTable
(
#if (COM_TIMEBASE == STD_ON)
    void
#else
...
DBG_COM_MAINFUNCTIONRX_ENTRY();
   ...
   if (Com_InitStatus == COM_EB_INIT)
   {
...
#endif /* (COM_DEV_ERROR_DETECT == STD_ON) */
     {
      ...
      Com_GlobalTimes_Rx[ComMainFunctionsRx_Idx] += 1U;
