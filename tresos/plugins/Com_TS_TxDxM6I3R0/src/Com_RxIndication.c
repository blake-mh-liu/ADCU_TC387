 ...
 */
static FUNC(void, COM_CODE) Com_EB_HandleRxConfirm
(
 ...
 */
static FUNC(void, COM_CODE) Com_EB_HandleShortPdu_SignalGroups
(
    P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
 ...
 * \param[in] PduInfoPtr - pointer to external buffer and size of buffer
 */
static FUNC(void, COM_CODE) Com_EB_HandleShortPdu_Signals
(
    P2CONST(ComRxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr,
    P2VAR(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPtr
);
          ...
          Com_EB_SchM_Enter_SCHM_COM_EXCLUSIVE_AREA_1();
          {
...
#if (COM_HANDLESMALLERRXPDUS_CPY_SIG == STD_ON)
            if (PduInfoPtr->SduLength < ((uint32)IPduPtr->ComIPduSizeMax + (uint32)COM_HANDLESMALLERRXPDUS_CPY_SIG_TEST) )
            {
...
#if (COM_RX_DYN_LENGTH_IPDU_ENABLE == STD_ON)
            {
               ...
               CONSTP2VAR(ComIPduSizeType, AUTOMATIC, COM_VAR_CLEARED) RxIPduSizePtr =
                   &COM_RX_IPDU_SIZE_VAR(ComRxPduId);

               /* set the size of the I-PDU */
               *RxIPduSizePtr = (ComIPduSizeType)pdulength;
            }
...
)
{
          ...
          if( (Com_IsBitSet_8(&(IPduPtr)->ComRxIPduFlags, COM_RXIPDUFLAG_SIGNAL_PROCESSING)) == COM_DEFERED )
