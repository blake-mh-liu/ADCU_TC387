 ...
 *
 * Reason:
 * The code is under precompile switch so it is excluded from some of the
 * tests based on the requirement.
 *
 * MISRAC2012-4) Deviated Rule: 12.4 (advisory)
...
static FUNC(void, COM_APPL_CODE) Com_EB_HandleTxConfirmations
(
...
static FUNC(uint8, COM_CODE ) Com_EB_ComNTimesQueueBaseRemaining
(
...
#if ((COM_TX_TIMEOUT_FACTOR_AVAILABLE == STD_ON) || (COM_CBK_TX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE))

static FUNC(void, COM_CODE ) Com_EB_StopTxDM_HandleTxConfirmations
(
    PduIdType ComTxPduId,
    P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
);

#endif /* ((COM_TX_TIMEOUT_FACTOR_AVAILABLE == STD_ON) || (COM_CBK_TX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE)) */
...
)
{
   ...
   if (Com_IsBitSet_8(&(IPduPtr)->ComTxIPduFlags, COM_TXIPDUFLAG_SIGNAL_PROCESSING) == COM_IMMEDIATE)
...
static FUNC(void, COM_CODE ) Com_EB_HlpTxConfirmation
(
    PduIdType ComTxPduId,
    P2CONST(ComTxIPduType, AUTOMATIC, COM_APPL_CONST) IPduPtr
)
   ...
   uint8 BitRes;
      ...
      (COM_CL_UB_TX_TRIGGERTRANSMIT_ENABLE == STD_OFF)))
         ...
         if (Com_IsBitGroupSet_8(&(IPduPtr)->ComTxIPduFlags,COM_TXIPDUFLAG_OFFSET_TX_IPDU_CLEAR_UPDATE,
                                 COM_TXIPDUFLAG_MASK_TX_IPDU_CLEAR_UPDATE) == COM_CL_UB_CONFIRMATION)
#endif
         ...
         case COM_TX_MODE_MODE_N_TIMES:
         {
            ...
            {
              uint8 N = Com_EB_ComNTimesQueueBaseRemaining(ComTxPduId, IPduPtr);
              if (N > 0U)
              {
...
#if ((COM_TX_TIMEOUT_FACTOR_AVAILABLE == STD_ON) || (COM_CBK_TX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE))
            Com_EB_StopTxDM_HandleTxConfirmations(ComTxPduId, IPduPtr);
#endif /* ((COM_TX_TIMEOUT_FACTOR_AVAILABLE == STD_ON) || (COM_CBK_TX_ACK_PTR_ARRAY_SIZE_MAX != COM_INDEX_NONE)) */
...
)
{
  ...
  /* calculate new N = N - 1 */
  N = NTimesQueueBaseRemaining[NRamBufIdx] - 1U;
