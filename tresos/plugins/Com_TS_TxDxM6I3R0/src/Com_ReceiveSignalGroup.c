 ...
 *
 * Reason:
 * False-positive: compiler optimizes precondition assert
 */
 ...
 */
static FUNC(boolean, COM_CODE) Com_ReceiveSignalGroup_Replace
(
 ...
 */
static FUNC_P2CONST(ComRxSignalGroupType, COM_APPL_CONST, AUTOMATIC) Com_GetRxSignalGroupCfg
(
 ...
 */
static FUNC(uint8, COM_CODE) Com_SetupRxSignalGroup_DET
(
   Com_SignalGroupIdType SignalGroupId,
 ...
 * to which the signal group belongs to
 */
static FUNC(void, COM_CODE) Com_CopyRxSignalGroupArray_Array
(
 ...
 * to which the signal group belongs to
 */
static FUNC(void, COM_CODE) Com_CopyRxSignalGroupArray
(
...
)
{
   ...
   if (COM_GET_RX_SIGNAL_GROUP_TIMEOUT_ACTION(SignalExtPtr) == COM_RX_DATA_TOUT_ACTION_REPLACE_SUBVAL_INITVAL)
...
#if (COM_DEV_ERROR_DETECT == STD_ON)
   /* in case of COM_EB_SIGNALGROUPARRAY_LENGTH_ENABLE set to STD_OFF check does not apply
   since SignalGroupArrayLength is set to 0xFFFFU which is the maximum range for ComByteSize/ComSignalLength */
...
{
   {
      ...
      if (RxDMReplace == TRUE)
      {
         /* pointer to first byte of timeout value of the signal */
         /* Deviation MISRAC2012-2 <+2> */
         CONSTP2CONST(uint8, AUTOMATIC, COM_APPL_CONST) valPtr =
            COM_GET_CONFIG_ELEMENT(uint8, IPduPtr->ComTimeoutValueRef,
...
)
{
...
#if (COM_EB_SIGNALGROUPARRAY_LENGTH_ENABLE == STD_OFF)
   uint16 SignalGroupArrayLength = 0xFFFFU;
   P2VAR(uint16, AUTOMATIC, COM_APPL_DATA) SignalGroupArrayLengthPtr = &SignalGroupArrayLength;
#endif /* #if(COM_EB_SIGNALGROUPARRAY_LENGTH_ENABLE == STD_OFF) */
   ...
   if (SignalGroupArrayPtr == NULL_PTR)
   {
