...
STATIC FUNC(void, LINIF_CODE) LinTp_Err_Tx_TimerHandling
(
    uint8 currentChannel,
    P2VAR(PduInfoType,AUTOMATIC,LINIF_APPL_DATA) pPduInfo
);
        ...
        if(PhysState != LINTP_PHYS_STATE_TXPENDING)
        {
            ...
            if((ChannelFlags&LINTP_BSWM_SCHEDULE_SWITCH_FALGS_MASK) == 0U)
            {
                  ...
                  )
                {
                    ...
                    {
                        /* Set ExpectedScheduleTableMode to LINTP_DIAG_REQUEST to check,
                         if we will be requested by LINTP_DIAG_REQUEST table */
                        pTpChannel->ExpectedScheduleTableMode = LINTP_DIAG_REQUEST;
               ...
               LINTP_FLAG_PHYS_TRANSMIT_REQUESTED)
            {
                switch(currentState)
                {
                    ...
                    case LINTP_PHYS_STATE_WAITTX:
                    {
                        ...
                        /* !LINKSTO LinIf.SWS_LinIf_00803_2,1 */
...
)
{
    ...
    if((ChannelFlags&LINTP_FLAG_PHYS_RX_BSWM_SCHEDULE_CHANGE_REQUESTED) == LINTP_FLAG_PHYS_RX_BSWM_SCHEDULE_CHANGE_REQUESTED)
    {
        ...
        /* Stop physical transmission Timer */
        LinTp_StopTimer(pPhysData);
    ...
    else if((ChannelFlags&LINTP_FLAG_FUNC_BSWM_SCHEDULE_CHANGE_REQUESTED) == LINTP_FLAG_FUNC_BSWM_SCHEDULE_CHANGE_REQUESTED)
    {
        ...
        uint8 FuncPduRResult = 0U;

        SchM_Enter_LinIf_SCHM_LINIF_EXCLUSIVE_AREA_0();

        TS_AtomicAssign8(ChannelFlags, LinTp_ChannelInfo[currentChannel].ChannelFlags);

        FuncPduRResult = pFuncData->PduRResult;
        ...
        PduR_LinTpTxConfirmation(PduRDestPduHandleId, FuncPduRResult);
    }
    ...
    else if((ChannelFlags&LINTP_FLAG_PHYS_BSWM_SCHEDULE_CHANGE_REQUESTED) == LINTP_FLAG_PHYS_BSWM_SCHEDULE_CHANGE_REQUESTED)
    {
        ...
        uint8 PhysPduRResult = 0U;

        SchM_Enter_LinIf_SCHM_LINIF_EXCLUSIVE_AREA_0();

        TS_AtomicAssign8(ChannelFlags, LinTp_ChannelInfo[currentChannel].ChannelFlags);

        PhysPduRResult = pPhysData->PduRResult;
        ...
        PduR_LinTpTxConfirmation(PduRDestPduHandleId, PhysPduRResult);
    }
    else
    {
        /* do nothing */
...
FUNC(void, LINIF_CODE) LinTp_CheckRequestedDiagSchTable
(
  uint8 currentChannel
)
{
...
#if (LINTP_SCHEDULE_CHANGE_DIAG_API == STD_ON)
  uint8 ExpectedScheduleTableMode;
  ExpectedScheduleTableMode = LinTp_ChannelInfo[currentChannel].ExpectedScheduleTableMode;
  LinTp_ChannelInfo[currentChannel].ExpectedScheduleTableMode = LINTP_INVALID_SCHEDULE_MODE;

  /* Check if LINTP_DIAG_REQUEST table was requested by LinTp via BswM. */
  /* Requested by LinTp_PhysTransmitHandling() and LinTp_FuncTransmitHandling().*/
  /* No need to check if LINTP_SCHEDULE_CHANGE_DIAG option is enabled for this channel as
    ExpectedScheduleTableMode is only changed when this option is enabled.*/
  if(LINTP_DIAG_REQUEST == ExpectedScheduleTableMode)
  {
    CONSTP2VAR(LinIf_MasterChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannelMaster = &LinIf_MasterChannelInfo[currentChannel];
    uint8 ScheduleMode = LINIF_CFG_GET_SCHTABLE[pIfChannelMaster->SchCurrentTable].ScheduleMode;

    /* !LINKSTO LinTp.EB.TpRquestedDaigCheck,1 */
    /* Check if the current schedule mode is not equal to Requested one i.e. LINTP_DIAG_REQUEST. */
    if( LINTP_DIAG_REQUEST != ScheduleMode)
    {
      /* Either no schedule change was requested or wrong schedule table requested. */

      CONSTP2VAR(LinTp_FuncChannelInfoType, AUTOMATIC, LINIF_VAR) pFuncData = &LinTp_ChannelInfo[currentChannel].FuncChannelInfo;
      const LinTp_FuncStates_Type FuncState = pFuncData->State;

      if(LINTP_FUNC_STATE_WAITTX == FuncState)
      {
        /* Request comes from LinTp_FuncTransmitHandling(). */
        CONSTP2CONST(LinTpTxNSduType, TYPEDEF, LINIF_APPL_CONST) pTpTxConfig = &LINTP_CFG_GET_TXNSDU[pFuncData->SduId];
        const PduIdType PduRDestPduHandleId = pTpTxConfig->PduRDestPduHandleId;

        /* Terminate Func request*/
        pFuncData->PduRResult = LINTP_INVALID_PDUR_RESULT;
        pFuncData->State = LINTP_FUNC_STATE_IDLE;

        /* Indicate the PduR that a functional transmission was stopped */
        PduR_LinTpTxConfirmation(PduRDestPduHandleId, E_NOT_OK);
      }
      else
      {
        /* (LINTP_PHYS_STATE_WAITTX == PhysState) */
        /* Request comes from LinTp_PhysTransmitHandling(). */
        CONSTP2VAR(LinTp_PhysChannelInfoType, AUTOMATIC, LINIF_VAR) pPhysData = &LinTp_ChannelInfo[currentChannel].PhysChannelInfo;
        CONSTP2CONST(LinTpTxNSduType, TYPEDEF, LINIF_APPL_CONST) pTpTxConfig = &LINTP_CFG_GET_TXNSDU[pPhysData->SduId];
        const PduIdType PduRDestPduHandleId = pTpTxConfig->PduRDestPduHandleId;

        /* Terminate Physical request*/
        pPhysData->State = LINTP_PHYS_STATE_IDLE;
        pPhysData->PduRResult = LINTP_INVALID_PDUR_RESULT;
...
)
{
...
#endif
    {
        if(PhysState == LINTP_PHYS_STATE_TXPENDING)
        {
            /* !LINKSTO LinIf.SWS_LinIf_00800_4,1 */
            LinTp_StopTimerAS(pPhysData);

            if(MsgLengthRemain == 0U)
            {
                /* message sent completely, end the tx transmission*/
              /* !LINKSTO LinIf.SWS_LinIf_00068.Master.SF,1,
                          LinIf.SWS_LinIf_00068.Master.CF,1,
                          LinIf.SWS_LinIf_00068.Slave.SF,1,
                          LinIf.SWS_LinIf_00068.Slave.CF,1
               */
            ...
            else
            {
                ...
                pPhysData->State = LINTP_PHYS_STATE_WAITTX;
                /* !LINKSTO LinIf.SWS_LinIf_00803_1,1 */
...
)
{
    ...
    switch(currentState)
    {
      case LINTP_PHYS_STATE_WAITRX:
      {
        /* CF was received */
#if (LINIF_SLAVE_SUPPORTED == STD_ON)
        /* !LINKSTO LinIf.SWS_LinIf_00871,1,
                    LinIf.SWS_LinIf_00807,1,
                    LinIf.SWS_LinIf_00798,1
         */
...
#endif /* LINIF_MASTER_SUPPORTED == STD_ON */
        {
          /* !LINKSTO LinIf.SWS_LinIf_00797_1_1,1,
                      LinIf.SWS_LinIf_00797_1_2,1,
                      LinIf.SWS_LinIf_00797_2_1,1,
                      LinIf.SWS_LinIf_00797_2_2,1,
                      LinIf.SWS_LinIf_00798,1
           */
          if(LINIF_FUNCTIONAL_NAD != pPduInfo->SduDataPtr[LINIF_PDU_BYTE_NAD])
          {
...
FUNC(void, LINIF_CODE) LinTp_Status_Err_No_Callout
(
    uint8 currentChannel,
    P2VAR(PduInfoType,AUTOMATIC,LINIF_APPL_DATA) pPduInfo
)
{
    LinTp_Err_Tx_TimerHandling(currentChannel, pPduInfo);
...
)
{
  ...
  TS_PARAM_UNUSED(status);
    ...
    LINIF_LIN_ERROR_NOTIFY(currentChannel, status);
}
#endif /* LINIF_SLAVE_SUPPORTED == STD_ON */
...
)
{
    ...
    if(currentState == LINTP_PHYS_STATE_WAITTX)
    {
        ...
        const uint16 MsgLengthRemain = pPhysData->MsgLengthRemain;
        /* !LINKSTO LinIf.SWS_LinIf_00422_1,1 */
        ...
        switch(GetPduRetCode)
        {
            case LINTP_OK:
                ...
                else
                {
                    ...
                    else
                    {
                      if(LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[currentChannel].CfgFlags & LINIF_NODETYPE_MASK))
                      {
                        /* !LINKSTO LinIf.SWS_LinIf_00800_3,1,
                                    LinIf.SWS_LinIf_00803_2,1
                         */
                        LinTp_StopTimerCS(pPhysData);
            ...
            case LINTP_E_PDUR:
                ...
                /* !LINKSTO LinIf.SWS_LinIf_00803_2,1 */
                LinTp_StopTimerCS(pPhysData);
                /* !LINKSTO LinIf.SWS_LinIf_00073.Slave,1,
                            LinIf.SWS_LinIf_00073.Master,1
                 */
                LinTp_StopTxNotOk(currentChannel);
                break;
            case LINTP_E_NOT_OK:
...
)
{
    ...
    if(retVal == E_OK)
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00797_2_1,1,
                    LinIf.SWS_LinIf_00797_2_2,1,
                    LinIf.SWS_LinIf_00774,1
         */
        if(
            ((LINIF_CH_IS_MASTER == (LINIF_CFG_GET_CHANNEL[currentChannel].CfgFlags & LINIF_NODETYPE_MASK)) && (receivedNAD == expectedNAD)) ||
            ((LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[currentChannel].CfgFlags & LINIF_NODETYPE_MASK)) && (((receivedNAD == expectedNAD) || (receivedNAD == LINIF_FUNCTIONAL_NAD)) || (receivedNAD == LINIF_BROADCAST_NAD)))
          )
#endif /* LINIF_SLAVE_SUPPORTED == STD_OFF */
        {
            /* determine LinTpRxSduId through received NAD */
            /* !LINKSTO LinIf.SWS_LinIf_00076.SRF,1 */
    ...
    if(retVal == E_OK)
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00075.Master.SF,1,
                    LinIf.SWS_LinIf_00075.Master.FF,1,
                    LinIf.SWS_LinIf_00075.Slave.SF,1,
                    LinIf.SWS_LinIf_00075.Slave.FF,1
         */
        ...
        if((BuffRetVal == BUFREQ_OK) || (BuffRetVal == BUFREQ_E_BUSY))
        {
             ...
             if(AvailablePduRBufferLength >= PayloadLength)
             {
                 ...
                 /* !LINKSTO LinIf.SWS_LinIf_00674.MRF.FF,1,
                             LinIf.SWS_LinIf_00674.MRF.SF,1,
                             LinIf.SWS_LinIf_00674.SRF.SF,1,
                             LinIf.SWS_LinIf_00674.SRF.FF,1
                  */
                 ...
                 if(retVal == E_OK)
                 {
...
#endif /* LINIF_MASTER_SUPPORTED == STD_ON */
                     {
                         if(PciType == LINTP_PCI_SF)
                         {
                             ...
                             /* !LINKSTO LinIf.SWS_LinIf_00078.MRF.SF,1 */
                         ...
                         else /* get ready for next frame */
                         {
                             ...
                             /* !LINKSTO LinIf.SWS_LinIf_00323.MRF,1,
                                         LinIf.SWS_LinIf_00323.SRF,1
                              */
                             pPhysData->State = LINTP_PHYS_STATE_WAITRX;

                             /* !LINKSTO LinIf.SWS_LinIf_00805.N_Cr.Start.FF,1 */
        ...
        else
        {
           ...
           /* !LINKSTO LinIf.SWS_LinIf_00676.MRF.E_NOT_OK,1,
                       LinIf.SWS_LinIf_00676.MRF.E_OVFL,1
            */
#if (LINIF_MASTER_SUPPORTED == STD_ON)
...
)
{
    ...
    switch(retPciVal)
    {
        case LINTP_OK:
          {
              ...
              if(currentNAD == lastNAD)
              {
                  ...
                  /* !LINKSTO LinIf.SWS_LinIf_00674.MRF.CF,1,
                              LinIf.SWS_LinIf_00674.MRF.lastCF,1,
                              LinIf.SWS_LinIf_00674.SRF.CF,1,
                              LinIf.SWS_LinIf_00674.SRF.lastCF,1
                   */
                  retVal = LinTp_CopyRxPayloadPduR(pPduInfo,
                                                   PciType,
                                                   PayloadLength,
                  ...
                  if(retVal == E_OK)
                  {
                      ...
                      {
                          /* message received completely, end the rx transmission*/
                          /* !LINKSTO LinIf.SWS_LinIf_00078.MRF.lastCF,1,
                                      LinIf.SWS_LinIf_00078.SRF.lastCF,1
                           */
#if(LINIF_SLAVE_SUPPORTED == STD_ON)
                          if(LINIF_CH_IS_MASTER == (LINIF_CFG_GET_CHANNEL[currentChannel].CfgFlags & LINIF_NODETYPE_MASK))
                          {
                            /* !LINKSTO LinIf.SWS_LinIf_00805.N_Cr.Stop,1 */
                            LinTp_StopTimerCR(pPhysData);
                          }
#endif /* LINIF_SLAVE_SUPPORTED == STD_ON */
                      ...
                      else
                      {
                         /* get ready for the next frame*/
                          ...
                          /* !LINKSTO LinIf.SWS_LinIf_00805.N_Cr.Start.CF,1 */
                          LinTp_StartTimerCR(pTpRxConfig, pPhysData);
                      }
                  }
                  else
        ...
        case LINTP_E_SN:
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00795_1,1 */
        ...
        case LINTP_NEW_RECEPTION:
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00808_1_1,1,
                      LinIf.SWS_LinIf_00808_1_2,1,
                      LinIf.SWS_LinIf_00808_2_1,1,
                      LinIf.SWS_LinIf_00808_2_2,1,
                      LinIf.SWS_LinIf_00774,1
           */
          LinTp_StopTimerCR(pPhysData);
...
)
{
    ...
    if(((((SID <= LINTP_STANDARD_SID_UPPER_LIMIT) || (SID == LINIF_SID_READ_ID)) &&
    ...
    {
        /* !LINKSTO LinIf.EB.RespPendingFrames_ReadByIdentifier,1 */
    ...
    if(PduLength == LINTP_PDU_LENGTH)
    {
        switch(PciTypeByte)
        {
            case LINTP_PCI_CF_FRAME:
                  /* !LINKSTO LinIf.SWS_LinIf_00079.SRF,1,
                              LinIf.SWS_LinIf_00079.MRF,1
                   */
...
)
{
...
#if (LINTP_SCHEDULE_CHANGE_DIAG_API == STD_ON)
    {
      ...
      if((LinTpOptions & LINTP_SCHEDULE_CHANGE_DIAG) == LINTP_SCHEDULE_CHANGE_DIAG)
      {
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00078.SRF.lastCF,1,
                      LinIf.SWS_LinIf_00701.Master.SF.SCD.TRUE,1,
                      LinIf.SWS_LinIf_00701.Master.FF.SCD.TRUE,1
           */
          BswM_LinTp_RequestMode(LINIF_GET_COMM_CHANNEL(currentChannel),
      ...
      {
          /* Inform the PduR that a reception was stopped */
          /* !LINKSTO LinIf.SWS_LinIf_00701.Master.FF.SCD.FALSE,1,
                      LinIf.SWS_LinIf_00701.Master.SF.SCD.FALSE,1
           */
    ...
    /* Inform the PduR that a reception was stopped */
    /* !LINKSTO LinIf.SWS_LinIf_00701.Slave.SF,1,
                LinIf.SWS_LinIf_00701.Slave.FF,1
     */
    PduR_LinTpRxIndication(PduRScrPduHandleId, Result);
    pPhysData->NAD = LINTP_INVALID_NAD;
    pPhysData->SID = LINTP_INVALID_SID;

    /* !LINKSTO LinIf.SWS_LinIf_00414,1 */
    pPhysData->State = LINTP_PHYS_STATE_IDLE;
...
)
{
    ...
    if((LinTpOptions & LINTP_SCHEDULE_CHANGE_DIAG) == LINTP_SCHEDULE_CHANGE_DIAG)
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00701.Master.FF.SCD.TRUE,1 */
        BswM_LinTp_RequestMode(LINIF_GET_COMM_CHANNEL(currentChannel),
                               LINTP_APPLICATIVE_SCHEDULE
                              );
    }
...
)
{
      ...
      )
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00078.SRF.SF,1 */
        BswM_LinTp_RequestMode(LINIF_GET_COMM_CHANNEL(currentChannel),
                               LINTP_DIAG_RESPONSE
                              );
    }
...
)
{
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00688,1 */
    pPhysData->State = LINTP_PHYS_STATE_IDLE;
...
)
{
    ...
    pduBuffer.SduLength = PayloadLength;
     ...
     /* !LINKSTO LinIf.SWS_LinIf_00705,1 */
    ...
    if(PhysTimerState > 0U)
    {
        ...
        if(PhysTimerState == 0U)
        {
            ...
            switch(State)
            {
                case LINTP_PHYS_STATE_WAITRX:
                {
                    /* !LINKSTO LinIf.SWS_LinIf_00806_1,1,
                                LinIf.SWS_LinIf_00806_2,1
                     */
                ...
                case LINTP_PHYS_STATE_WAITTX:
                {
                    ...
                    {
                      /* !LINKSTO LinIf.SWS_LinIf_00801_1,1 */
                      LinTp_StopTxTimeoutAS(currentChannel);
                    }
                    else
#endif /* LINIF_SLAVE_SUPPORTED == STD_ON */
                    {
                      /* !LINKSTO LinIf.SWS_LinIf_00803_3,1 */
                      LinTp_StopTxTimeoutCS(currentChannel);
                    }
                    break;
                }
...
)
{
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00076.MRF,1 */
