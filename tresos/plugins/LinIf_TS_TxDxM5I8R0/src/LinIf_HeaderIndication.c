...
/* !LINKSTO LinIf.SWS_LinIf_00863.Enabled,1 */
...
#if (LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_ON)
#include <TSMem.h>               /* TS_MemCpy() */
#endif /* LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_ON*/
...
STATIC FUNC(Std_ReturnType, LINIF_CODE) LinIf_BusATxIndicationReturn
(
  NetworkHandleType Channel,
  P2VAR(Lin_PduType, AUTOMATIC, AUTOMATIC) PduPtr,
  P2VAR(LinIf_ChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannel,
...
)
{
...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00189,1 */
    if(pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
    {
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00754.HeaderIndication,1 */
      LinIf_StartTimerBI(pIfChConfSlave, pIfChannelSlave);

      /* Confirm wakeup */
      {
        /* !LINKSTO LinIf.SWS_LinIf_00761,1 */
      ...
      {
        /* !LINKSTO LinIf.SWS_LinIf_00846,1,
                    LinIf.SWS_LinIf_00847.ErrorReport,1
         */
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00847.ProcessFrame,1 */
      if(LINIF_PID_SRF == PduPtr->Pid)
      {
        ...
        if(NULL_PTR != PduPtr->SduPtr)
        {
          ...
          {
#if( LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
           frameType = LINIF_FRM_SRF;
           Result = LinIf_BusATxIndicationReturn(Channel, PduPtr, pIfChannel, response, DLFlags, frameType);
#else
            pIfChannel->FrameType = LINIF_FRM_SRF;
            pIfChannel->LastHeaderType = LINIF_FRM_SRF;
            /* !LINKSTO LinIf.SWS_LinIf_00739.Cs,1 */
            PduPtr->Cs = LIN_CLASSIC_CS;
            PduPtr->Drc = LINIF_LIN_RESP_MR_TX;
            /* !LINKSTO LinIf.SWS_LinIf_00739.Dl,1 */
            PduPtr->Dl = 8U;
            Result = E_OK;
#endif /* LINIF_BUS_ADAPTER_TX_SUPPORT */
#if (LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON)
#if (LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_ON)
            pIfChannel->FrameInfo.Pid = PduPtr->Pid;
            pIfChannel->FrameInfo.FrameLength =  8U;
            TS_MemCpy(pIfChannel->FrameInfo.FrameData, PduPtr->SduPtr,8U);
#endif /* LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_ON*/
#endif /* LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON */
        ...
        {
          /* !LINKSTO LinIf.SWS_LinIf_00845,1 */
          (void)Det_ReportError(LINIF_MODULE_ID, LINIF_INSTANCE_ID,
                                LINIF_API_HEADERIND, LINIF_E_PARAM_POINTER);
        }
#endif /* LINIF_DEV_ERROR_DETECT == STD_ON */
      ...
      else
      {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00748.E_NOT_OK,1 */
        if(i < (pIfChConfSlave->FrameStartInd + pIfChConfSlave->FrameNumAssoc))
        {
          ...
          {
#if( LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
            frameType = LINIF_CFG_GET_SLAVEFRTYPE[i].LinIfFrameType;
            Result = LinIf_BusATxIndicationReturn(Channel, PduPtr, pIfChannel, response, DLFlags, frameType);
#else
            /* !LINKSTO LinIf.SWS_LinIf_00733.Dl,1,
                        LinIf.SWS_LinIf_00739.Dl,1
             */
            PduPtr->Dl = DLFlags & LINIF_DL_MASK;
            /* !LINKSTO LinIf.SWS_LinIf_00733.Drc.PreAR44,1,
                        LinIf.SWS_LinIf_00733.Drc.PostAR44,1,
                        LinIf.SWS_LinIf_00739.Drc.PreAR44,1,
                        LinIf.SWS_LinIf_00739.Drc.PostAR44,1
             */
            PduPtr->Drc = response;
#endif /* LINIF_BUS_ADAPTER_TX_SUPPORT */
            /* !LINKSTO LinIf.SWS_LinIf_00733.Cs,1,
                        LinIf.SWS_LinIf_00739.Cs,1
             */
            ...
            /* !LINKSTO LinIf.SWS_LinIf_00763,1 */
      ...
      if(E_OK != Result)
      {
        /* !LINKSTO LinIf.SWS_LinIf_00748.PreAR44,1,
                    LinIf.SWS_LinIf_00748.PostAR44,1,
                    LinIf.SWS_LinIf_00740.PreAR44,1,
                    LinIf.SWS_LinIf_00740.PostAR44,1
         */
...
)
{
  ...
  /* !LINKSTO LinIf.SWS_LinIf_00775,1,
              LinIf.SWS_LinIf_00791,1
   */
  switch(pIfChannelSlave->RSID)
  {
    case LINIF_ASSIGN_NAD_RSID:
    {
      /* !LINKSTO LinIf.SWS_LinIf_00779_2,1,
                  LinIf.SWS_LinIf_00780,1
       */
    ...
    case LINIF_READ_ID_RSID:
    {
      ...
      {
        /* !LINKSTO LinIf.SWS_LinIf_00783,1 */
    ...
    {
      /* !LINKSTO LinIf.SWS_LinIf_00782_2,1,
                  LinIf.SWS_LinIf_00780,1
       */
    ...
    {
      /* !LINKSTO LinIf.SWS_LinIf_00781_2,1,
                  LinIf.SWS_LinIf_00780,1
       */
    ...
    case LINIF_STANDARD_RSID_NEGATIVE_RESPONSE:
    {
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00841,1 */
    ...
    default:
    {
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00776,1 */
      PduInfoType pduInfo;
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00672.Slave,1 */
...
)
{
  ...
  {
    /* !LINKSTO LinIf.SWS_LinIf_00730,1 */
        ...
        *Response = LINIF_LIN_RESP_MR_TX;
        ...
        if(LinIf_FindTxEvtTriggered(Channel, FrameIndex, &pFrameInfo))
        {
          ...
          *DLFlags = pFrameInfo->DLFlags;
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00740.E_NOT_OK,1 */
          Result = LINIF_CFG_GET_UL_TRGTX_FPTR(LINIF_CFG_GET_SLAVEFRTYPE[i].LinIfFrameIdx)(pFrameInfo->ULPduHandleId, &PduInfo);
          /* !LINKSTO LinIf.SWS_LinIf_00731,1 */
...
)
{
    ...
    if(LINIF_PID_SRF == PduPtr->Pid)
    {
        linId = LINIF_PID_SRF & LINIF_PID_PARITY_MASK;
        pduInfo.SduLength= 8U;
        pduInfo.SduDataPtr= PduPtr->SduPtr;
        retVal = BUSA_LINIF_TXINDICATION(linId,&pduInfo,frameType,Channel);
        if(retVal == TRUE)
        {
            /* !LINKSTO LinIf.EB.LinIfBusATxIndication_Slave_True,1 */
            pIfChannel->FrameType = LINIF_FRM_SRF;
            pIfChannel->LastHeaderType = LINIF_FRM_SRF;
            /* !LINKSTO LinIf.SWS_LinIf_00739.Cs,1 */
            PduPtr->Cs = LIN_CLASSIC_CS;
        ...
        {
            /* !LINKSTO LinIf.EB.LinIfBusATxIndication_Slave_False,1 */
            PduPtr->Drc = LINIF_LIN_RESP_S2S_IGN;
            PduPtr->Dl = 0U;
            pIfChannel->LastHeaderType = LINIF_FRM_EMPTY;
            pIfChannel->FrameType = LINIF_FRM_EMPTY;
    ...
    {
        if((frameType == LINIF_FRM_TX)||(frameType == LINIF_FRM_EVENT_TRIGGERED))
        {
          linId = PduPtr->Pid & LINIF_PID_PARITY_MASK;
          pduInfo.SduLength= (PduLengthType)DLFlags & LINIF_DL_MASK;
          pduInfo.SduDataPtr= PduPtr->SduPtr;
          retVal = BUSA_LINIF_TXINDICATION(linId,&pduInfo,frameType,Channel);
        }
        else
...
#endif /* TS_MERGED_COMPILE */
/* !LINKSTO LinIf.SWS_LinIf_00863.Disabled,1 */
