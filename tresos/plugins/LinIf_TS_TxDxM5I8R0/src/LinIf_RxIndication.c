...
/* !LINKSTO LinIf.SWS_LinIf_00864.Enabled,1 */
...
STATIC FUNC(boolean, LINIF_CODE) LinIf_NC_InvalidFrmIdRange
(
  P2CONST(LinIfSlaveChannelType, AUTOMATIC, LINIF_APPL_DATA) pIfChConfSlave,
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) dataPtr
);
...
/* !LINKSTO LinIf.SWS_LinIf_91005_1,1 */
...
)
{
...
#endif /* (LINIF_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    uint8 linId = 0U;
    uint8 frameType = pIfChannel->LastHeaderType;
    ...
    if(pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
    {
      ...
      /* !LINKSTO LinIf.SWS_LinIf_00754.RxIndication,1 */
      ...
      switch(pIfChannel->LastHeaderType)
      {
        case LINIF_FRM_RX:
        {
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00734.Length,1 */
          pdu.SduLength = pIfChannel->LastDl;
          /* !LINKSTO LinIf.SWS_LinIf_00734.DataPtr,1 */
          pdu.SduDataPtr = Lin_SduPtr;

#if( LINIF_BUS_ADAPTER_RX_SUPPORT == STD_ON )
          linId = pIfChannel->FrameInfo.Pid & LINIF_PID_PARITY_MASK;

          retVal = BUSA_LINIF_RXINDICATION(linId, &pdu, frameType, Channel);
          if(retVal == TRUE)
          {
              /* !LINKSTO LinIf.EB.LinIfBusARxIndication_Slave_True,1 */
              LinIf_Status_PduRRxInd(Channel, &pdu);
          }
          else
          {
              /* !LINKSTO LinIf.EB.LinIfBusARxIndication_Slave_False,1 */
...
#if (LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON)
          {
            ...
            {
              /* !LINKSTO LinIf.SWS_LinIf_00838,1 */
        ...
        case LINIF_FRM_MRF:
        {
          /* !LINKSTO LinIf.SWS_LinIf_00750,1 */
          ...
          else
          {
            ...
            {
              /* !LINKSTO LinIf.SWS_LinIf_00785_1,1 */
            ...
            else if(LINIF_NC_HANDLE_FWD_TP == ret)
            {
              ...
              /* !LINKSTO LinIf.SWS_LinIf_00772,1,
                          LinIf.SWS_LinIf_00773,1
               */

#if( LINIF_BUS_ADAPTER_RX_SUPPORT == STD_ON )
              frameType = LINIF_FRM_MRF;
              linId = LINIF_PID_MRF;
            ...
            else if(LINIF_NC_HANDLE_E_NOT_OK == ret)
            {
              ...
              /* !LINKSTO LinIf.SWS_LinIf_00787,1,
...
#if (LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_ON)
            {
              ...
              if (LINIF_CHANNEL_ENABLED == LINIF_MIRROR_CHANNEL_ENABLED(enabledChannels, Channel))
              {
                ...
                pdu.SduLength = 8U;
                pdu.SduDataPtr = Lin_SduPtr;
                /* !LINKSTO LinIf.EB.LinIfMirrorDiagMsgs.MRF-SLAVE_LIN_RX_OK,1,
                            LinIf.EB.LinIfMirrorDiagMsgs.MRF-SLAVE_LIN_RX_OK.Cdd,1 */
                LINIF_UL_REPORTING(LINIF_GET_COMM_CHANNEL(Channel), pIfChannel->FrameInfo.Pid, &pdu, LIN_RX_OK);
              }
            }
        ...
        {
          /* !LINKSTO LinIf.SWS_LinIf_00848,1 */
          break;
        }
        /* CHECK: NOPARSE */
        default:
...
)
{
  ...
  /* !LINKSTO LinIf.SWS_LinIf_00778_2,1 */
    ...
    )
  {
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00778_1,1 */
...
)
{
  ...
  /* !LINKSTO LinIf.SWS_LinIf_00810_1,1 */
  if(E_OK == LinIf_FilterSupplFuncID(Channel, &dataPtr[3U]))
  {
    CONSTP2VAR(LinIf_SlaveChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannelSlave =
      LINIF_GET_CHANNEL_SLAVE(Channel);

    /* !LINKSTO LinIf.SWS_LinIf_00779_1,1 */
...
{
  LinIf_NCHReturnType ret = LINIF_NC_HANDLE_E_NOT_OK;
  /* !LINKSTO LinIf.SWS_LinIf_00812.SaveConfig.ON,1,
              LinIf.SWS_LinIf_00782_1,1
   */
  ...
  else
  {
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00782_3,1 */
  }
  return ret;
}
#endif
...
)
{
  ...
  {
    /* !LINKSTO LinIf.SWS_LinIf_00811,1 */
    uint8 startIdx = dataPtr[3U];
    uint8 idx = 0U;

    /* To avoid writing PIDs and then realizing the frame as a whole is in fact invalid,
       the validity is checked (LinIf_NC_InvalidFrmIdRange) and then proceed with writing. */
    if (FALSE == LinIf_NC_InvalidFrmIdRange(pIfChConfSlave,dataPtr))
    {
      for(idx = 0U; idx < 4U; idx++)
      {
        uint8 offset = (uint8)(startIdx + idx);
        uint8 pid = dataPtr[4U + idx];

        /* !LINKSTO LinIf.SWS_LinIf_00809_1,1,
              LinIf.SWS_LinIf_00809_2,1
         */
        if((pid != 0x3CU) && (pid != 0x3DU) && (pid != LINIF_UNUSED_DATA))
        {
          /* !LINKSTO LinIf.SWS_LinIf_00781_1,1 */
...
)
{
  ...
  if(E_OK == LinIf_FilterSupplFuncID(Channel, &dataPtr[4U]))
  {
    ...
    switch(dataPtr[3U])
    {
      ...
      {
        /* !LINKSTO LinIf.SWS_LinIf_00813,1,
                    LinIf.SWS_LinIf_00783,1
         */
      ...
      {
        /* !LINKSTO LinIf.SWS_LinIf_00840,1,
                    LinIf.SWS_LinIf_00841,1
         */
...
)
{
  ...
  {
    /* !LINKSTO LinIf.SWS_LinIf_00771.NAD,1,
                LinIf.SWS_LinIf_00771.PCI,1,
                LinIf.SWS_LinIf_00771.SID,1,
                LinIf.SWS_LinIf_00778_3,1,
                LinIf.SWS_LinIf_00789,1
     */
    switch(dataPtr[LINIF_PDU_BYTE_SID])
    {
      case LINIF_SID_ASSIGN_NAD:
      {
        ...
        {
          /* !LINKSTO LinIf.SWS_LinIf_00788,1 */
          Result = LINIF_NC_HANDLE_IGNORE;
        }
        else
        {
          ...
          {
            /* !LINKSTO LinIf.SWS_LinIf_00790,1 */
            Result = LINIF_NC_HANDLE_IGNORE;
...
#if(LINIF_NC_SAVECONF_SUPPORTED == STD_ON)
      /* !LINKSTO LinIf.SWS_LinIf_00812.SaveConfig.OFF,1 */
...
)
{
  ...
  for(idx = 0U; idx < 4U; idx++)
  {
    ...
    if((pid != 0x3CU) && (pid != 0x3DU) && (pid != LINIF_UNUSED_DATA))
    {
      if((pIfChConfSlave->FrameStartInd + offset) >= upperLimit)
      {
        ret = TRUE;
      }
    }
...
#endif /* TS_MERGED_COMPILE */
/* !LINKSTO LinIf.SWS_LinIf_00864.Disabled,1 */
