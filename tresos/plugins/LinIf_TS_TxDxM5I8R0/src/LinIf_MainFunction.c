 ...
 *
 * Reason: False-positive from the Tasking 63r1p5 compiler
 */
/*==================[inclusions]============================================*/
...
/* !LINKSTO LinIf.SWS_LinIf_00872,1 */
#include <Mirror.h>
...
#define LINIF_STATUS_USRCALLOUT_OR_IGNORE &LinIf_Status_ErrorCalloutNoDet
...
/** \brief This function preform the callouts.
 *
 *  \param currentChannel  Channel which is currently processed
 *  \param status          Lin_GetStatus return staus
 *  \param pPduInfo         Received data
 */
STATIC FUNC(void, LINIF_CODE) LinIf_PreformCallOuts
(
    uint8 currentChannel,
    Lin_StatusType status,
 ...
 *
 *  \param currentChannel  Channel which is currently processed
 *  \param ReScheduleIsCalled  Flag for not call LinIf_ReSchedule again
 */
STATIC FUNC(void, LINIF_CODE) LinIf_Schedule
(
    uint8 currentChannel,
    boolean ReScheduleIsCalled
);
 ...
 *
 *  \param currentChannel  Channel which is currently processed
 *  \param scheduleDelay   Remaining time to send the next frame
 *
 ...
 *  \param pPduInfo        received data
 */
STATIC FUNC(void, LINIF_CODE) LinIf_Status_ErrorCalloutNoDet
(
    uint8 currentChannel,
    P2VAR(PduInfoType,AUTOMATIC,LINIF_APPL_DATA) pPduInfo
);
...
STATIC CONST(LinIf_StatusFunctionType, LINIF_CONST) LinIf_StatusMatrix[LINIF_STATUS_MATRIX_NUM_HEADERTYPES][LINIF_STATUS_MATRIX_NUM_STATES] =
{
    /* LINIF_FRM_TX */
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00036,1 */
    ...
    /* LINIF_FRM_RX */
    {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00254.RX_BUSY,1 */
        LINIF_STATUS_DET,                      /* LIN_RX_BUSY              */
        /* !LINKSTO LinIf.SWS_LinIf_00254.RX_ERROR,1 */
        LINIF_STATUS_USRCALLOUT_AND_DET,       /* LIN_RX_ERROR             */
        /* !LINKSTO LinIf.SWS_LinIf_00466,1 */
    ...
    /* LINIF_FRM_MRF */
    {
        LINTP_STATUS_ERR_USRCALLOUT_NO_DET, /* LIN_TX_HEADER_ERROR       */
        LINTP_TX_CONFIRMATION,              /* LIN_TX_OK                 */
        /* !LINKSTO LinIf.SWS_LinIf_00465,1 */
        LINTP_STATUS_ERR_NO_USRCALLOUT,     /* LIN_TX_BUSY               */
        /* !LINKSTO LinIf.SWS_LinIf_00036,1 */
        LINTP_STATUS_ERR_USRCALLOUT,        /* LIN_TX_ERROR              */
        LINIF_STATUS_SNH,                   /* LIN_RX_OK                 */
        LINIF_STATUS_SNH,                   /* LIN_RX_BUSY               */
        LINIF_STATUS_SNH,                   /* LIN_RX_ERROR              */
        LINIF_STATUS_SNH,                   /* LIN_RX_NO_RESPONSE        */
    ...
    /* LINIF_FRM_SRF */
    {
        ...
        LINIF_STATUS_DET,                   /* LIN_RX_BUSY               */
        LINIF_STATUS_USRCALLOUT_AND_DET,    /* LIN_RX_ERROR              */
        LINIF_STATUS_USRCALLOUT_OR_IGNORE,  /* LIN_RX_NO_RESPONSE        */
#if (LINIF_REPORTTODEM_ENABLED)
        LINIF_STATUS_IGNORE,                /* LIN_TX_BIT_ERROR           */
        LINIF_STATUS_IGNORE,                /* LIN_RX_CHECKSUM_ERROR      */
        LINIF_STATUS_IGNORE,                /* LIN_RX_NO_RESPONSE_ERROR   */
        ...
        for ( channelIdx = 0U ; channelIdx < (LinIf_ConfigPtr->NumChannelsMaster + LinIf_ConfigPtr->NumSlaveChannels) ; channelIdx++ )
        {
          ...
          if (LINIF_CH_IS_SLAVE == (LINIF_CFG_GET_CHANNEL[channelIdx].CfgFlags & LINIF_NODETYPE_MASK))
          {
            ...
            if (pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
            {
              ...
              /* !LINKSTO LinIf.SWS_LinIf_00751,1 */
              ...
              if(0U < pIfChannelSlave->TimerNas)
              {
                ...
                if(0U == pIfChannelSlave->TimerNas)
                {
                  /* !LINKSTO LinIf.SWS_LinIf_00786,1 */
              ...
              {
                  /* !LINKSTO LinIf.SWS_LinIf_00755,1 */
...
/* !LINKSTO LinIf.SWS_LinIf_00875,1 */
#if (LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON)
/* !LINKSTO LinIf.SWS_LinIf_00876,1 */
...
)
{
...
#if( LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
    uint8 frameType = pIfChannel->FrameType;
    uint8 linId = 0U;
    boolean retVal = FALSE;
    PduInfoType pduInfo;
#endif /* LINIF_BUS_ADAPTER_TX_SUPPORT */
    ...
    if (pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
    {
        ...
        boolean RescheduleThisTB = FALSE;  /* Force rescheduling this timebase */
        uint8 SleepFlags;
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00727_disable,1 */
        /* !LINKSTO LinIf.SWS_LinIf_00727_enable_1,1 */
        /* !LINKSTO LinIf.SWS_LinIf_00727_enable_2,1 */
        if((1U == LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->SchChgNxTB) && (0U == pIfChannelMaster->StatusDelay)
            && ((SleepFlags & LINIF_WAKEUP_PULSE_ONGOING_FLAG) == 0U))
        {
          LinIf_SetupCollisionRes(currentChannel);
          RescheduleThisTB = LinIf_ReSchedule(currentChannel, NextFrameToSendDue);
          if(TRUE == RescheduleThisTB)
          {
            NextFrameToSendDue = 1U;
        ...
        if(NextFrameToSendDue == 1U)
        {
            ...
            if((SleepFlags & LINIF_WAKEUP_PULSE_ONGOING_FLAG) == LINIF_WAKEUP_PULSE_ONGOING_FLAG)
            {
              SleepFlags &= ((uint8) ~LINIF_WAKEUP_PULSE_ONGOING_FLAG);
            ...
            if(ret != E_OK)
            {
                ...
                if(0U == LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->SchChgNxTB) /* LinIf_SetupCollisionRes already called before */
                {
                    LinIf_SetupCollisionRes(currentChannel);
                }
                LinIf_Schedule(currentChannel,RescheduleThisTB);

                if(LinIf_SetupFrame(currentChannel, &LinPduInfo) != LINIF_FRM_EMPTY)
                {
#if( LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON )
                    linId=LinPduInfo.Pid & LINIF_PID_PARITY_MASK;
                    pduInfo.SduLength= LinPduInfo.Dl;
                    pduInfo.SduDataPtr= LinPduInfo.SduPtr;

                    retVal = BUSA_LINIF_TXINDICATION(linId,&pduInfo,frameType,currentChannel);
                    if(retVal == TRUE)
                    {
                        /* !LINKSTO LinIf.EB.LinIfBusATxIndication_Master_True,1 */
                    ...
                    {
                        /* !LINKSTO LinIf.EB.LinIfBusATxIndication_Master_False,1 */
                        /*Nothing to do since the Bus Adapter returned FALSE*/
                    }
#else
                    (void)LinIf_LinDriverConfig[LINIF_GET_LIN_DRIVER_INDEX(currentChannel)].sendFrame(LINIF_GET_LIN_CHANNEL(currentChannel),&LinPduInfo);
#endif /* LINIF_BUS_ADAPTER_TX_SUPPORT */
...
STATIC FUNC(void, LINIF_CODE) LinIf_StatusCheck (uint8 currentChannel)
{
...
#if (LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_OFF)
        /* !LINKSTO LinIf.ASR44.SWS_LinIf_00874.LIN_TX_OK, 1,
                    LinIf.EB.SWS_LinIf_00874.LIN_TX_OK.Cdd, 1 */
        if ((pIfChannel->FrameType != LINIF_FRM_MRF) && (pIfChannel->FrameType != LINIF_FRM_CONFIG))
        {
          LINIF_UL_REPORTING(LINIF_GET_COMM_CHANNEL(currentChannel), pIfChannel->FrameInfo.Pid, pPduInfoTx, status);
        }
        else
        {
          /* do nothing, mirroring Diagnostic frames is disabled */
        }
#else
        /* !LINKSTO LinIf.EB.LinIfMirrorDiagMsgs.MRF-LIN_TX_OK, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.MRF-LIN_TX_OK.Cdd, 1                    */
        LINIF_UL_REPORTING(LINIF_GET_COMM_CHANNEL(currentChannel), pIfChannel->FrameInfo.Pid, pPduInfoTx, status);
#endif /* LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_OFF */
      }
      else
      {
#if (LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_OFF)
        ...
        if ((pIfChannel->FrameType != LINIF_FRM_MRF) &&
            (pIfChannel->FrameType != LINIF_FRM_CONFIG) &&
...
#else
        /* !LINKSTO LinIf.EB.LinIfMirrorDiagMsgs.SRF-LIN_RX_OK, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.SRF-LIN_RX_OK.CDD, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.SRF-LIN_RX_OTHER_THAN_OK, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.SRF-LIN_RX_OTHER_THAN_OK.CDD, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.MRF-LIN_TX_OTHER_THAN_OK, 1,
                    LinIf.EB.LinIfMirrorDiagMsgs.MRF-LIN_TX_OTHER_THAN_OK.Cdd, 1*/
        LINIF_UL_REPORTING(LINIF_GET_COMM_CHANNEL(currentChannel), pIfChannel->FrameInfo.Pid, pPduInfo, status);
#endif /* LINIF_DIAG_MSGS_REPORTING_ENABLE == STD_OFF */
      }
    }
    ...
    else
    {
        ...
        /* AUTOSAR 4.0 Rev 2 SWS_LinIf_00030: Status shall be checked again in later schedules if channel
       is busy and it is not yet the turn of the next schedule */
        ...
        if(PerformCallouts)
        {
            LinIf_PreformCallOuts(currentChannel,status,pPduInfo);
        }
    }
...
#if( LINIF_BUS_ADAPTER_RX_SUPPORT == STD_ON )
            if (LIN_RX_OK == status)
            {
                uint8 frameType = pIfChannel->FrameType;
                uint8 linId = pIfChannel->FrameInfo.Pid & LINIF_PID_PARITY_MASK;
                boolean retVal = BUSA_LINIF_RXINDICATION(linId, pPduInfo, frameType, currentChannel);
                if(retVal == TRUE)
                {
                    /* !LINKSTO LinIf.EB.LinIfBusARxIndication_Master_True,1 */
                    pfunction(currentChannel,pPduInfo);
                }
                else
                {
                   /* !LINKSTO LinIf.EB.LinIfBusARxIndication_Master_False,1 */
                   /*Nothing to do since the Bus Adapter returned FALSE*/
                }
            }
            else
...
#else
        pfunction(currentChannel,pPduInfo);
#endif /* LINIF_BUS_ADAPTER_RX_SUPPORT */
        }
    }
}
...
)
{
  ...
  const NetworkHandleType ComMChannel = LINIF_GET_COMM_CHANNEL(currentChannel);
  ...
  if(status == LIN_CH_SLEEP)
  {
    CONSTP2VAR(LinIf_MasterChannelInfoType, AUTOMATIC, LINIF_VAR) pIfChannelMaster = &LinIf_MasterChannelInfo[currentChannel];
      ...
      if(wakeupRequested)
      {
        /* !LINKSTO LinIf.SWS_LinIf_00460, 2*/
      ...
      if(Result == E_NOT_OK)
      {
        ...
        /* !LINKSTO LinIf.SWS_LinIf_00699, 2*/
        if(wakeupRequested)
        {
      ...
      else
      {
        ...
        /* Lin_Wakeup() will generate a wake up pulse on the bus, which will take from 250 μs to 5 ms. */
        /* !LINKSTO LinIf.EB.SWS_LinIf_00460_LinIfWakeUpTime, 1*/
        pIfChannel->SleepHandling = LINIF_WAKEUP_PULSE_ONGOING_FLAG;
...
)
{
  ...
  {
    /* !LINKSTO LinIf.SWS_LinIf_00756.MRF_GTS,1,
                LinIf.SWS_LinIf_00756.BusIdleTimeout,1
     */
    ...
    if((sleepFlags & LINIF_SLEEP_ONGOING_FLAG) == LINIF_SLEEP_ONGOING_FLAG)
    {
      ...
      else
      {
          ...
          /* !LINKSTO LinIf.SWS_LinIf_00753,1 */
          LinIf_StopTimerBI(LINIF_GET_CHANNEL_SLAVE(currentChannel));

          /* !LINKSTO LinIf.SWS_LinIf_00757,1,
                      LinIf.SWS_LinIf_00759,1
           */
 ...
 * schedule table change from the upper layer or achievement of the schedule end
 */
STATIC FUNC(boolean, LINIF_CODE) LinIf_ReSchedule ( uint8 currentChannel, uint16 scheduleDelay )
{
    ...
    if(LINIF_CFG_GET_SCHTABLE[pIfChannelMaster->SchCurrentTable].EntryStartInd != LINIF_INVALID_ENTRY)
    {
        ...
        {
            /* Restart this time base if schedule table is RUN_ONCE (which mean new ST will be switched to)
            Or schedule table is RUN_CONTINUOUS and LinIfScheduleRestartNextTimeBase is enabled. */
            /* !LINKSTO LinIf.EB.ScheduleRestartNextTimeBase_disable, 1*/
            /* !LINKSTO LinIf.EB.ScheduleRestartNextTimeBase_enable_1, 1*/
            /* !LINKSTO LinIf.EB.ScheduleRestartNextTimeBase_enable_2, 1*/
            /* Deviation TASKING-1 */
            if((LINIF_CFG_GET_SCHTABLE[pIfChannelMaster->SchCurrentTable].RunMode == LINIF_RUN_ONCE) ||
               ((LINIF_CFG_GET_SCHTABLE[pIfChannelMaster->SchCurrentTable].RunMode != LINIF_RUN_ONCE) &&
               ((1U == scheduleDelay) || (1U == LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->SchRestartNxTB))
               )
            )
            {
              Reschedule = TRUE;
...
)
{
    ...
    if(TRUE == ReScheduleIsCalled) /* LinIf_ReSchedule is already called and returned TRUE */
    {
        Reschedule = TRUE;
    }
    else
    {
        Reschedule = LinIf_ReSchedule(currentChannel, 1U);
    }
    ...
    LinTp_CheckRequestedDiagSchTable(currentChannel);
...
)
{
    ...
    if(ret == E_OK)
    {
        ...
        {
#if (LINIF_TP_SUPPORTED == STD_ON)
            /* !LINKSTO LinIf.SWS_LinIf_00672.Master,1 */
            case LINIF_FRM_MRF:
              /* fall through */
#endif /*LINIF_TP_SUPPORTED == STD_ON*/
...
#if (LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON) || ( LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON ) || ( LINIF_BUS_ADAPTER_RX_SUPPORT == STD_ON )
        pIfChannel->FrameInfo.Pid = pLinPduInfo->Pid;
#endif /* LINIF_BUS_MIRRORING_SUPPORT_ENABLE == STD_ON || LINIF_BUS_ADAPTER_TX_SUPPORT == STD_ON || LINIF_BUS_ADAPTER_RX_SUPPORT == STD_ON */
...
)
{
    ...
    switch (pIfChannel->FrameType)
    {
        case LINIF_FRM_TX:
        {
            ...
            /* copy data from PduR */
            /* !LINKSTO LinIf.SWS_LinIf_00706.Sporadic,1,
                        LinIf.SWS_LinIf_00706.Unconditional,1
             */
...
)
{
    ...
    /* !LINKSTO LinIf.SWS_LinIf_00732.EvtTrg,1,
                LinIf.SWS_LinIf_00732.Uncond,1
     */
