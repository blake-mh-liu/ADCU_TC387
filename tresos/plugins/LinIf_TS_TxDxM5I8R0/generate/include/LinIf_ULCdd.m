...
[!LOOP "util:distinct(node:order(as:modconf('LinIf')[1]/LinIfGlobalConfig/*[1]/LinIfChannel/*, 'LinIfChannelId'))"!]
...
[!LOOP "as:modconf('LinIf')[1]/LinIfGlobalConfig/*[1]/LinIfChannel/*/LinIfFrame/*[LinIfFrameType='UNCONDITIONAL']"!]
  ...
  [!FOR "f" = "1" TO "num:i(count(text:split($funcs, ';')))"!]
    ...
    [!"var:set(concat('func', $ulApi), node:when('PDUR' = $type, concat($funcPrefixPduR, $ulApi), node:fallback("->as:modconf('LinIf')[1]/LinIfGeneral/LinIfCddFunctionsUL/*[as:name(.)=node:value(node:ref(concat(node:path(node:current()), '/LinIfPduDirection/', $ulApiField)))]/CddFunctionName", "nopathWrapper")))"!]
  [!ENDFOR!]
