...
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandleDataCycle_Internal(
  FRNM_PDL_SF(const uint8 instIdx));

/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandleVoteCycle_Internal(
  FRNM_PDL_SF(const uint8 instIdx));
...
#endif /*#if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM == STD_ON)*/
    SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
 ...
 * 2) TX_CONFIRMATION_NETWORK_MODE[]/Restart Timeout Timer
 * 3) TX_TIMEOUT_NETWORK_MODE[]/Nn_TxTimoutException();
 * 4) NetworkMode -> BusSleepMode: FAILSAFE_SLEEP_MODE[]/InitInternalVar();
 * 5) NetworkMode -> SynchronizeMode: FAILSAFE_SYNCH_MODE[]/ClearActiveWakeupBit()
 */
...
#if (FRNM_ACTIVE_WAKEUP_BIT_ENABLED == STD_ON)
  /* !LINKSTO FrNm.EB.FrNm_NetworkGwEraRequest.ActiveWakeupBit.SynchronizeState,1 */
...
/* Deviation MISRAC2012-1 */
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfNetworkModeExit(
   FRNM_PDL_SF(const uint8 instIdx))
{
  /* exit action 'Stop Message Timout Timer' */
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON)
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
   ...
   * for TX_CONFIRMATION_NETWORK_MODE[]/...
   * internal transition */
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON)
  FRNM_CHANNEL_STATUS(instIdx).TimeoutTimer = FRNM_CHANNEL_MSGTIMEOUTTIME(instIdx);
  /* CHECK: NOPARSE */
  /* the triggering event for this action can occur only if
   * FRNM_HSM_INST_MULTI_ENABLED is STD_OFF */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
...
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfNetworkModeAction3(
  FRNM_PDL_SF(const uint8 instIdx))
{
  /* action 'Nn_TxTimoutException();'
   * for TX_TIMEOUT_NETWORK_MODE[]/...
   * internal transition */
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON)
#if (FRNM_MULTICORE_ENABLED == STD_OFF)
...
/* Deviation MISRAC2012-1 */
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfNetworkModeAction4(
  FRNM_PDL_SF(const uint8 instIdx))
{
...
/* Deviation MISRAC2012-1 */
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfNetworkModeAction5(
  FRNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* CHECK: NOPARSE */
  /* Event FAILSAFE_SYNCH_MODE is not trigerred if FRNM_PASSIVE_MODE_ENABLED is set to STD_ON */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
 ...
 * Transitions originating from this state:
 * 1) VOTE_CYCLE_TIMESLOT_SEND_IN_RS[]/SendPnsrData()
 * 2) DATA_CYCLE_TIMESLOT_SEND_IN_RS[]/SendPnsrData()
 * 3) ReadySleepState -> BusSleepMode: REP_CYCLE_COMPLETED[ReadySleepTimer == 0]/ClearActiveWakeupBit()
 */
...
{
  /* action 'SendPnsrData()'
   * for VOTE_CYCLE_TIMESLOT_SEND_IN_RS[]/...
   * internal transition */
#if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF)
  /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00126.VotePDU.2,1 */
  FrNm_HandleVoteCycle_Internal(FRNM_PL_SF(instIdx));
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
}
...
/* Deviation MISRAC2012-1 */
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfReadySleepStateAction2(
  FRNM_PDL_SF(const uint8 instIdx))
{
  /* action 'SendPnsrData()'
   * for DATA_CYCLE_TIMESLOT_SEND_IN_RS[]/...
   * internal transition */
#if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF)
  FrNm_HandleDataCycle_Internal(FRNM_PL_SF(instIdx));
  /* CHECK: NOPARSE */
  /* the Sending data on the bus happen only if FRNM_PASSIVE_MODE_ENABLED is set to STD_OFF */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
...
/* Deviation MISRAC2012-1 */
FUNC(boolean, FRNM_CODE) FrNm_HsmFrNmSfReadySleepStateGuard3(
  FRNM_PDL_SF(const uint8 instIdx))
{
...
/* Deviation MISRAC2012-1 */
FUNC(void, FRNM_CODE) FrNm_HsmFrNmSfReadySleepStateAction3(
  FRNM_PDL_SF(const uint8 instIdx))
{
  /* action 'ClearActiveWakeupBit()'
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* !LINKSTO FrNm.SWS_FrNm_00101,1 */
  /* Deviation MISRAC2012-1 */
  FrNm_ReadySleepTimerStart(
    FRNM_PL_TIMER(instIdx, FRNM_CHANNELTIME_CONFIG(instIdx).ReadySleepCnt + 1U));
}
/* Deviation MISRAC2012-1 */
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* CHECK: NOPARSE */
  /* ReadySleepRemoteSleep cannot be entered if FRNM_REMOTE_SLEEP_INDICATION_ENABLED is set to STD_OFF */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
...
#else /* #if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON) */
#if (FRNM_FORCE_VOTE_BIT == STD_OFF)
  TS_AtomicClearBit_8(&FRNM_CHANNEL_STATUS(instIdx).ControlBitVector, FRNM_PDU_VOTE_BIT);
#else /* #if (FRNM_FORCE_VOTE_BIT == STD_OFF) */
  /* !LINKSTO FrNm.EB.VoteBitValueScheduleVariant2,2 */
  /* !LINKSTO FrNm.EB.VoteBitValueScheduleVariant6,1 */
...
#endif /* #if (FRNM_FORCE_VOTE_BIT == STD_OFF) */
  /* Stop timeout timer */
  FRNM_CHANNEL_STATUS(instIdx).TimeoutTimer = 0U;
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
   ...
   * internal transition */
#if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF)
  /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00126.DataPDU,1 */
  FrNm_HandleDataCycle_Internal(FRNM_PL_SF(instIdx));
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* CHECK: NOPARSE */
  /* the triggering event for this action can occur only if
   * FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE is set to STD_OFF */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
...
#endif
  /* CHECK: NOPARSE */
  /* NormalOperationRemoteSleep cannot be entered if FRNM_REMOTE_SLEEP_INDICATION_ENABLED is set to STD_OFF */
#elif (FRNM_HSM_INST_MULTI_ENABLED == STD_ON)
  TS_PARAM_UNUSED(instIdx);
#endif
  /* CHECK: PARSE */
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
   ...
   * Nm_StateChangeNotification(LastState, ActState);
   * => FRNM191 FrNm.SWS_FrNm_00192 CurState = SynchronizeState;
  ...
  FRNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* !LINKSTO FrNm.SWS_FrNm_00340,1, FrNm.SWS_FrNm_00376,1 */
  return (boolean)
  ...
  FRNM_PDL_SF(uint8 instIdx))
{
  ...
  if (FRNM_CHANNEL_STATUS(instIdx).ComUserDataPduPtr != NULL_PTR)
  {
    TS_MemSet(
      FRNM_CHANNEL_STATUS(instIdx).ComUserDataPduPtr,
      0xFFU, (PduLengthType)(FRNM_CHANNEL_USERDATAPDULENGTH(instIdx)));
  }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
        */
