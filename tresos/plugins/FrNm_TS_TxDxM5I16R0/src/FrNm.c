 ...
 */
/* !LINKSTO FrNm.EB.FRNM064.ProvidedFiles,1 */
...
/* patch version check */
#if (FRNM_SW_PATCH_VERSION < 16U)
#error FRNM_SW_PATCH_VERSION wrong (< 16U)
#endif
...
/* !LINKSTO FrNm.EB_Ref.FRNM066.MemMap,1 */
 ...
 **
 ** This function aggregate and filter(based on FrNm_PnFilterMask) the received requests
 **
 ...
 ** reset and the PduR is notified via PduR_FrNmRxIndication callback.
 **
...
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)

#if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0)
/** \brief Handles aggregation of synchronized pnc shut down messages
 **
 ** \param[in] PnInfoDest aggregation buffer
 ** \param[out] PnInfoSrc buffer to input synchronized pnc shut down message
 */
STATIC FUNC(void, FRNM_CODE) FrNm_AggregatePnInfoSyncShutdown
(
  P2VAR(uint8, AUTOMATIC, FRNM_VAR_CLEARED) PnInfoDest,
  P2CONST(uint8, AUTOMATIC, FRNM_VAR_CLEARED) PnInfoSrc
);
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */

/** \brief Handles pnc synchronized shut down operations
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandlePNSyncShutdown(
  FRNM_PDL_SF(uint8 instIdx)
);

#if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)
/** \brief Check if pdu is immediate or decoupled
 **
 ** \param[in] TxVotePduId configured vote pdu index
 */
STATIC FUNC(boolean, FRNM_CODE) FrNm_CheckIfPopulateSentBufferVoteCycle
(
  uint16 TxVotePduId
);

/** \brief Handles pnc synchronized shut down active channels operations
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandlePNSyncShutdownActive(
  FRNM_PDL_SF(uint8 instIdx));

#if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON)
/** \brief Handles pnc synchronized shut down active channels re-transmission operations
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(boolean, FRNM_CODE) FrNm_HandlePNSyncShutdownRetransmissionActive(
  FRNM_PDL_SF(uint8 instIdx));
#endif /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */

/** \brief Check if pdu is immediate or decoupled
 **
 ** \param[in] TxDataPduId configurred vote pdu index
 */
STATIC FUNC(boolean, FRNM_CODE) FrNm_CheckIfPopulateSentBufferDataCycle
(
  uint16 TxDataPduId
);

#endif /* #if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0) */

#if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0)

/** \brief Handles pnc synchronized shut down passive channels operations
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandlePNSyncShutdownPassive(
  FRNM_PDL_SF(uint8 instIdx));

/** \brief Handles pnc synchronized shut down forwarding operations
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_HandlePNSyncShutdownForwardReq(
  FRNM_PDL_SF(uint8 instIdx));
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */


#endif /* #if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) */

#if ((FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON) || \
     ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM  > 0)))
/** \brief Set synchronized pnc shut down events
 **
 ** \param[in] instIdx instance index
 */
/* Deviation MISRAC2012-1 */
STATIC FUNC(void, FRNM_CODE) FrNm_Set_ReadySleepEvents
(
  FRNM_PDL_SF(uint8 instIdx)
);
#endif /* #if ((FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON) || \
        *      ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *       (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM  > 0)))
        */
...
/** \brief Partial networking info: EiraValue and EiraTimer */
#if (FRNM_PN_EIRA_CALC_ENABLED == STD_ON)
VAR(FrNm_PnStatusType, FRNM_VAR) FrNm_PnEiraStatus;
#endif

/** \brief Function pointer to routine scheduled from channel main function */
VAR(FrNm_DoActionType, FRNM_VAR) FrNm_DoActFkp[FRNM_NUMBER_OF_CHANNELS];

#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
STATIC VAR(FrNm_SynchronizedPncShutdownIdsType, FRNM_VAR_CLEARED)
  FrNm_SynchronizedPncShutdownIds[FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM];
#endif

#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0))
STATIC VAR(FrNm_SynchronizedPncShutdownRxIdsType, FRNM_VAR_CLEARED)
  FrNm_SynchronizedPncShutdownRxIds[FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM];
#endif
...
/** \brief Initialization information of FrNm module */
/* !LINKSTO FrNm.SWS_FrNm_00071,1 */
...
/* !LINKSTO FrNm.EB.IsValidApi.ServiceId,1 */
...
/* check if development error detection is enabled */
...
#endif
  {
    ...
    for (instIdx = 0U; instIdx < FRNM_NUMBER_OF_CHANNELS; ++instIdx)
    {
...
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)
      FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags = 0U;
#if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)
      if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)

      {
        FRNM_CHANNEL_STATUS(instIdx).ComUserDataPduPtr =
          &FrNm_Rx_Tx_Buffer[FRNM_CHANNEL_COM_USERDATA_BUFFER_OFFSET(instIdx)];
      }
      else
      {
        FRNM_CHANNEL_STATUS(instIdx).ComUserDataPduPtr = NULL_PTR;
      }
#endif /*#if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)*/
      ...
      if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
      {
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds = &FrNm_SynchronizedPncShutdownIds[ActiveGatewayIdx];
        ActiveGatewayIdx++;
#if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON)
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer = 0;
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentDecoupled, 0U, FRNM_PN_INFO_LENGTH);
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentImmediate, 0U, FRNM_PN_INFO_LENGTH);
#endif /* FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON */
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoRcv, 0U, FRNM_PN_INFO_LENGTH);
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, 0U, FRNM_PN_INFO_LENGTH);
      }
      else
      {
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds = NULL_PTR;
      }
#endif /* #if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0) */

#if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0)
      if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_PASSIVE)
      {
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds = &FrNm_SynchronizedPncShutdownRxIds[PassiveGatewayIdx];
        PassiveGatewayIdx++;
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRx, 0U, FRNM_PN_INFO_LENGTH);
#if (FRNM_MULTICORE_ENABLED == STD_ON)
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg.PnInfo, 0U, FRNM_PN_INFO_LENGTH);
#else
        TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg, 0U, FRNM_PN_INFO_LENGTH);
#endif

      }
      else
      {
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds = NULL_PTR;
      }
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */
#endif /*#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)*/
#else  /* #if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF) */
      ...
      TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).PnEra.PnTempInfo, 0U, FRNM_PN_INFO_LENGTH);
#endif
    ...
    TS_MemSet(FrNm_PnEiraStatus.PnTempInfo, 0x00U, FRNM_PN_INFO_LENGTH);
#endif

    /* !LINKSTO FrNm.EB.FrNmInt.InitStatus,1 */
...
)
{
  ...
  else
  {
...
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)
    FrNm_HandlePNSyncShutdown(FRNM_PL_SF(instIdx));
#endif /* #if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) */
#if (FRNM_HW_VOTE_ENABLE == STD_ON)
...
#endif
    {
      ...
      /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00467.FrNmMainAcrossFrCycle.False,1 */
...
#if ((FRNM_CYCLE_COUNTER_EMULATION == STD_ON) && (FRNM_SYNCH_ERR_EXTENDED == STD_ON))
      /* !LINKSTO FrNm.SWS_FrNm_00378,1, FrNm.SWS_FrNm_00379,1 */
      if (FRNM_CHANNEL_STATUS(instIdx).FrNm_EmulatedCounterValid == TRUE)
      {
    ...
    else
    {
         ...
         )
      {
          ...
          /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00126.VotePDU.1,1 */
     ...
     ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM  > 0)))
      FrNm_Set_ReadySleepEvents(FRNM_PL_SF(instIdx));
#endif
    }
...
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0))
      FrNm_HandlePNSyncShutdownForwardReq(FRNM_PL_SF(instIdx));
#endif
    }
    else
    {
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_PASSIVESTARTUP, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
    ...
    if (FRNM_CHANNEL_STATUS(instIdx).CurState == NM_STATE_BUS_SLEEP)
    {
      ...
      /* SWS_FrNm_00138, FRNM317, Emit event for transition to Synchronise mode */
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_NETWORKREQUEST, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_NETWORKGWERAREQUEST, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
    SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
    /* Set Network Requested status flag */
    /* !LINKSTO FrNm.EB.FrNm_NetworkGwEraRequest.BusSleepMode,1 */
    FRNM_CHANNEL_STATUS(instIdx).ChanStatus |=  (uint8)FRNM_NETWORK_REQUESTED;
#if (FRNM_ACTIVE_WAKEUP_BIT_ENABLED == STD_ON)
    if (FRNM_CHANNEL_STATUS(instIdx).ActiveWakeUp1 == 0U)
    {
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_NETWORKRELEASE, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
...
)
{
  ...
  if (FrNm_InitStatus == FRNM_UNINIT)
  {
    /* !LINKSTO FrNm.EB.FrNm_SetUserData.UninitState,1 */
...
#endif
  {
    ...
    SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
    /* Reserved bytes in the data buffer are being filled just before actual data transmission and not at this place.*/
    /* !LINKSTO FrNm.SWS_FrNm_00447,1 */
    TS_MemCpy(
...
)
{
...
#endif
  {
    ...
    {
      /* !LINKSTO FrNm.ASR403.FRNM111,1 */
      /* !LINKSTO FrNm.SWS_FrNm_00226,1 */
      TS_AtomicSetBit_8(&FRNM_CHANNEL_STATUS(instIdx).ChanStatus, FRNM_REPEAT_MSG_REQUEST_BIT);
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_GETNODEIDENTIFIER, FRNM_E_INVALID_CHANNEL);
  }
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_GETLOCALNODEIDENTIFIER, FRNM_E_INVALID_CHANNEL);
  }
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_REQUESTBUSSYNCHRONIZATION, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_CHECKREMOTESLEEPINDICATION, FRNM_E_INVALID_CHANNEL);
  }
...
#endif
  {
...
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
    if ((FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE) &&
        ((PduInfoPtr->SduDataPtr[0U] & FRNM_CBV_PNSRBITMASK) != 0U))
    {
      /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00504.RuntimeError,1 */
      /* !LINKSTO FrNm.SWS_FrNm_91002,1 */
      FRNM_DET_REPORT_RUNTIME_ERROR(FRNM_INST(instIdx),
                                    FRNM_SERVID_RXINDICATION,
                                    FRNM_E_INVALID_PN_SYNC_SHUTDOWN_REQUEST
                                    );
    }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
    ...
    else
    {
      ...
      if (FRNM_CHANNEL_PNENABLED(instIdx) == TRUE)
      {
        ...
        /* !LINKSTO FrNm.SWS_FrNm_00404,1 */
        if ((PduInfoPtr->SduDataPtr[0U] & FRNM_CBV_PNINFOBIT_MASK) != 0U)
        {
#if (FRNM_PN_EIRA_CALC_ENABLED == STD_ON)
          PnMessage = FrNm_IsValidPnMessage(&PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET]);

          FrNm_AggregatePnInfo(&FrNm_PnEiraStatus,
          ...
          if (FRNM_CHANNELID_CONFIG(instIdx).EraCalcEnabled == STD_ON)
          {
            PnMessage = FrNm_IsValidPnMessage(&PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET]);
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)
            if ((PduInfoPtr->SduDataPtr[0U] & FRNM_CBV_PNSRBITMASK) == 0U)
#endif /* #if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) */
            {
              /* !LINKSTO FrNm.ASR403.FRNM430,1, FrNm.ASR403.FRNM431,1, FrNm.SWS_FrNm_00434,1 */
              FrNm_AggregatePnInfo(&FRNM_CHANNEL_STATUS(instIdx).PnEra,
                                   &PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET]
                                   );
            }
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)
            else
            {
              ...
              if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_PASSIVE)
              {
                FrNm_AggregatePnInfoSyncShutdown(&FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRx[0],
                                                 &PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET]
                                               );
                FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_RX;
              }
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */
            }
#endif /*#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)*/
...
#endif /* #if (FRNM_PN_ERA_CALC_ENABLED == STD_ON) */
        }
        SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
      }
#endif
...
#if ((FRNM_PN_EIRA_CALC_ENABLED == STD_ON) || (FRNM_PN_ERA_CALC_ENABLED == STD_ON))
      if ((PnMessage == TRUE)||
        /* SWS_FrNm_00409 : Partial networking is not supported */
...
)
{
   ...
   * Therefore event to be emitted if the received pdu contains +ve vote OR if the pdu belongs
...
)
{
...
#endif
  {
    ...
    if (FrNm_RootPtr->TxPduMap[TxPduId].PduType == FRNM_VOTE)
    {
     ...
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
      if ((FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE) &&
          (FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN) != 0U)
        {
          PduInfoPtr->SduDataPtr[0U] |= (uint8)FRNM_CBV_PNSRBITMASK;
        }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
        */
    ...
    else
    {
       ...
       * internal transmit buffer.Here pointer address is changing temporarily to
      ...
      /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00541.UserData,1 */
...
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
      /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00542.PncBitVector,1 */
      if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
      {
        SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
        if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN) != 0U)
        {
          PduInfoPtr->SduDataPtr[0U] |= (uint8)(FRNM_CBV_PNSRBITMASK);

          TS_MemCpy(&PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET],
                    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, FRNM_PN_INFO_LENGTH);
          ...
          for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
          {
            ...
            retransmitDecoupled |=
              (FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg[PnIndex]) &
              (uint8)(~(uint8)(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentDecoupled[PnIndex]));
          }
          if (retransmitDecoupled != 0U)
          {
            FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_RESTART_TIMER;
          }
          TS_MemCpy(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentDecoupled,
                    &PduInfoPtr->SduDataPtr[FRNM_PN_INFO_OFFSET], FRNM_PN_INFO_LENGTH);
#else /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */
          TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, 0U, FRNM_PN_INFO_LENGTH);
          FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN));
#endif /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */
        }
        else
        {
          PduInfoPtr->SduDataPtr[0U] &= (uint8)(~(uint8)(FRNM_CBV_PNSRBITMASK));
        }
        SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
      }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
...
/*------------------[FrNm_TxConfirmation]--------------------------------*/
#if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF)
FUNC(void, FRNM_CODE) FrNm_TxConfirmation
(
   PduIdType TxPduId
)
{
...
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
      if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
      {
        SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
        if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN) != 0U)
        {
          if (FrNm_RootPtr->TxPduMap[TxPduId].Immediate == TRUE)
          {
            FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_IMMEDIATE_TX;
          }
          else
          {
            FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_DECOUPLED_TX;
          }
        }
        SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
      }
...
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON)
    FRNM_EMIT(FrNm_RootPtr->TxPduMap[TxPduId].ChannelHandle, FRNM_HSM_FRNM_EV_TX_CONFIRMATION_NETWORK_MODE);
#else
...
)
{
  ...
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_SETSLEEPREADYBIT, FRNM_E_INVALID_CHANNEL);
  }
  else
#endif
  {
...
/* !LINKSTO FrNm.SWS_FrNm_00503,1 */
FUNC(Std_ReturnType, FRNM_CODE) FrNm_RequestSynchronizedPncShutdown
(
  NetworkHandleType nmChannelHandle,
  PNCHandleType pncId
)
{
...
#if (FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON)
#if (((FRNM_HSM_INST_MULTI_ENABLED == STD_ON) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)) || \
     (FRNM_DEV_ERROR_DETECT == STD_ON))
  const uint8 instIdx = FrNm_GetClusterIndex(nmChannelHandle);
#else
  TS_PARAM_UNUSED(nmChannelHandle);
#endif /* #if ((FRNM_HSM_INST_MULTI_ENABLED == STD_ON) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)) || \
        *      (FRNM_DEV_ERROR_DETECT == STD_ON))
        */
#if ((FRNM_DEV_ERROR_DETECT == STD_OFF) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM == 0))
  TS_PARAM_UNUSED(pncId);
#endif /* #if ((FRNM_DEV_ERROR_DETECT == STD_OFF) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM == 0)) */

  DBG_FRNM_REQUESTSYNCHRONIZEDPNCSHUTDOWN_ENTRY(nmChannelHandle,pncId);

#if (FRNM_DEV_ERROR_DETECT == STD_ON)
  if (FrNm_InitStatus == FRNM_UNINIT)
  {
    FRNM_DET_REPORT_ERROR(
      instIdx, FRNM_SERVID_REQUESTSYNCHRONIZEDPNCSHUTDOWN, FRNM_E_UNINIT);
  }
  else if (instIdx >= FRNM_NUMBER_OF_CHANNELS)
  {
    /* !LINKSTO FrNm.SWS_FrNm_00051,1 */
    FRNM_DET_REPORT_ERROR(instIdx, FRNM_SERVID_REQUESTSYNCHRONIZEDPNCSHUTDOWN, FRNM_E_INVALID_CHANNEL);
  }
  else if ((pncId <  (FRNM_PN_INFO_OFFSET * 8U)) ||
           (pncId >= ((FRNM_PN_INFO_OFFSET + FRNM_PN_INFO_LENGTH) * 8U)))
  {
    FRNM_DET_REPORT_ERROR(
      instIdx, FRNM_SERVID_REQUESTSYNCHRONIZEDPNCSHUTDOWN, FRNM_E_INVALID_FUNCTION_ARG);
  }
  else
#endif
  {
#if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)
    /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00511.ComMRequests,1 */
    if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
    {
      const uint8 index = (uint8)(pncId >> FRNM_SHIFT_3_BITS) - FRNM_PN_INFO_OFFSET;

      TS_AtomicSetBit_8(
          &FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoRcv[index],
          (uint8)(pncId & FRNM_MASK_3_BITS)
          );
  ...
  DBG_FRNM_REQUESTSYNCHRONIZEDPNCSHUTDOWN_EXIT(nmChannelHandle,pncId);
...
)
{
...
#if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON)
  uint16 PnIndex = 0;
  uint8  retransmitImmediate = 0U;
#endif

#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
  boolean setSynchronizedPncShutdownSentBuffer = FALSE;
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_OFF)
  boolean sendData = FALSE;
#endif /* #if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_OFF) */
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
        */
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_OFF)
      ...
      )
  {
    ...
    if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
    {
      /* !LINKSTO FrNm.EB.SWS_FrNm_00548.PncBitVector,1 */
      SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
      if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN) != 0U)
      {
        ...
        for(PduIndex = 0U; PduIndex < FRNM_CHANNEL_NOOFTXVOTEPDUS(instIdx); PduIndex++)
        {
           ...
           * is received or not.
           */
          if (FrNm_CheckIfPopulateSentBufferVoteCycle(FRNM_CHANNEL_TXVOTEPDUID(instIdx)[PduIndex]) == TRUE)
          {
            setSynchronizedPncShutdownSentBuffer = TRUE;
          }
        }
#else /* #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
        if(FrNm_CheckIfPopulateSentBufferVoteCycle(FRNM_CHANNEL_TXVOTEPDUID(instIdx)[0]) == TRUE)
        {
          setSynchronizedPncShutdownSentBuffer = TRUE;
        }
#endif /* #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
        if (setSynchronizedPncShutdownSentBuffer == TRUE)
        {
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_OFF)
          sendData = TRUE;
#endif /* #if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_OFF) */
          FRNM_CHANNEL_STATUS(instIdx).TxPduPtr[0U] |= (uint8)(FRNM_CBV_PNSRBITMASK);
          TS_MemCpy(&FRNM_CHANNEL_STATUS(instIdx).TxPduPtr[FRNM_PN_INFO_OFFSET],
            FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, FRNM_PN_INFO_LENGTH);
          ...
          for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
          {
            ...
            retransmitImmediate |=
              (FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg[PnIndex]) &
              (uint8)(~(uint8)(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentImmediate[PnIndex]));
          }
          if (retransmitImmediate != 0U)
          {
            FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_RESTART_TIMER;
          }
          TS_MemCpy(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentImmediate,
                  &FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, FRNM_PN_INFO_LENGTH);
#else /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */
          TS_MemSet(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg, 0U, FRNM_PN_INFO_LENGTH);
          FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN));
      ...
      {
        FRNM_CHANNEL_STATUS(instIdx).TxPduPtr[0U] &= (uint8)(~(uint8)(FRNM_CBV_PNSRBITMASK));
      }
      SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
    }
#endif /* #if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0) */
  ...
  {
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
    boolean setDet = FALSE;
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
        *      (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
        *      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
    ...
    for(PduIndex = 0U; PduIndex < FRNM_CHANNEL_NOOFTXVOTEPDUS(instIdx); PduIndex++)
    {
       ...
       * is received or not.
       */
      if (FrIf_Transmit(FRNM_CHANNEL_TXVOTEPDUID(instIdx)[PduIndex], &FrNm_PduInfo) != E_OK)
      {
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
     (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
        if ((FRNM_CHANNEL_STATUS(instIdx).TxPduPtr[0U] & FRNM_CBV_PNSRBITMASK) != 0U)
        {
          setDet = TRUE;
        }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
...
#else /* #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
    if (FrIf_Transmit(FRNM_CHANNEL_TXVOTEPDUID(instIdx)[0U], &FrNm_PduInfo) != E_OK)
    {
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
    ...
    if (setDet == TRUE)
    {
      /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00512.TransmissionsRequestNotAccepted,1 */
      FRNM_DET_REPORT_RUNTIME_ERROR(instIdx,
                                    FRNM_MAINFUNCTION_ID + instIdx,
                                    FRNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED);
    }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
        *      (FRNM_DET_RUNTIME_CHECKS == STD_ON) &&
...
{
  uint8 PduIndex;
  PduIndex = FrNm_GetPduIndex(TxVotePduId);
  return FrNm_RootPtr->TxPduMap[PduIndex].Immediate;
}
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)) */
      ...
      )
  {
    ...
    if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
    {
      ...
      if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN) != 0U)
      {
        ...
        for(PduIndex = 0U; PduIndex < FRNM_CHANNEL_NOOFTXDATAPDUS(instIdx); PduIndex++)
        {
           ...
           * is received or not.
           */
          if (FrNm_CheckIfPopulateSentBufferDataCycle(FRNM_CHANNEL_TXDATAPDUID(instIdx)[PduIndex]) == TRUE)
          {
            setSynchronizedPncShutdownSentBuffer = TRUE;
          }
        }
#else /*   #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
        if (FrNm_CheckIfPopulateSentBufferDataCycle(FRNM_CHANNEL_TXDATAPDUID(instIdx)[0]) == TRUE)
        {
          setSynchronizedPncShutdownSentBuffer = TRUE;
        }
#endif /* #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
    ...
    boolean setDet = FALSE;
      ...
      for(PduIndex = 0U; PduIndex < FRNM_CHANNEL_NOOFTXDATAPDUS(instIdx); PduIndex++)
      {
         ...
         * is received or not.
         */
        if (FrIf_Transmit(FRNM_CHANNEL_TXDATAPDUID(instIdx)[PduIndex], &FrNm_PduInfo) != E_OK)
        {
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
...
#else /* #if (FRNM_DUAL_CHANNEL_PDU_ENABLE == STD_ON) */
      if (FrIf_Transmit(FRNM_CHANNEL_TXDATAPDUID(instIdx)[0U], &FrNm_PduInfo) != E_OK)
      {
#if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
     (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_OFF) && \
     (FRNM_DET_RUNTIME_CHECKS == STD_ON) && \
...
{
  uint8 PduIndex;
  PduIndex = FrNm_GetPduIndex(TxDataPduId);
  return FrNm_RootPtr->TxPduMap[PduIndex].Immediate;
}
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
        *      (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0))
 ...
 **                   - is set to false in case PduInfoPtr does not refer
...
TS_PARAM_UNUSED(instIdx);
  ...
  if (FrNm_ComUserDataInfo[instIdx].ComUserTxPduEnabled == TRUE)
  {
    ...
    if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
    {
      ...
      iPduInfo.SduLength  = PduInfoPtr->SduLength;
      iPduInfo.SduDataPtr = FRNM_CHANNEL_STATUS(instIdx).ComUserDataPduPtr;

      (void)PduR_FrNmTriggerTransmit(FRNM_CHANNEL_COMUSERTXCONFPDUID(instIdx),&iPduInfo);
...
)
{
  ...
  switch(FRNM_CHANNEL_STATUS(instIdx).CurState)
  {
    case NM_STATE_SYNCHRONIZE:
      ...
      /* On long error handling done via state machine(guard) */
      break;
    /* !LINKSTO FrNm.SWS_FrNm_00384,1, FrNm.SWS_FrNm_00342,1 */
  ...
  if (FRNM_CHANNEL_STATUS(instIdx).CurState == NM_STATE_REPEAT_MESSAGE)
  {
    /* !LINKSTO FrNm.ASR403.FRNM385,1, FrNm.EB.FRNM386.BusSleepState,1 */
    /* Emit event to indicate startup/synch error to enter into bus sleep state */
    FRNM_EMIT(instIdx, FRNM_HSM_FRNM_EV_FAILSAFE_SLEEP_MODE);
  }
  else
#endif
  {
    /* !LINKSTO FrNm.ASR403.FRNM383,1, FrNm.SWS_FrNm_00384,1, FrNm.ASR403.FRNM338,1 */
    /* !LINKSTO FrNm.EB_Ref.FRNM342.StartupError,1, FrNm.EB_Ref.FRNM342.NoGlobalTime,1 */
...
)
{
  ...
  if (FRNM_CHANNEL_STATUS(instIdx).TimeoutTimer > 0U)
  {
    ...
    {
#if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON)
      FRNM_EMIT(instIdx, FRNM_HSM_FRNM_EV_TX_TIMEOUT_NETWORK_MODE);
#else
...
)
{
  ...
  TS_MemCpy(PnStatePtr->PnTempInfo, PnStatePtr->PnInfo, FRNM_PN_INFO_LENGTH);
  ...
  for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
  {
    if ((PnStatePtr->PnTempInfo[PnIndex] & FrNm_RootPtr->PnFilterMask[PnIndex]) != 0U)
    {
      uint8 PnBitPos;
...
)
{
  ...
  {
    uint16 EiraIndex = FrNm_RootPtr->EiraTimerMap[PnIndex];
...
)
{
  ...
  for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
  {
      PnInfoDest[PnIndex] |= (uint8)(PnInfoSrc[PnIndex] & FrNm_RootPtr->PnFilterMask[PnIndex]);
  }
}
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */
...
{
  if ((FRNM_CHANNEL_PNENABLED(instIdx) == TRUE) && (FRNM_CHANNELID_CONFIG(instIdx).EraCalcEnabled == STD_ON))
  {
    if (FRNM_CHANNEL_STATUS(instIdx).CurState != NM_STATE_BUS_SLEEP)
    {
#if (FRNM_PN_GATEWAY_TYPE_ACTIVE_NUM > 0)
      FrNm_HandlePNSyncShutdownActive(FRNM_PL_SF(instIdx));
#endif

#if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0)
      FrNm_HandlePNSyncShutdownPassive(FRNM_PL_SF(instIdx));
#endif /* #if (FRNM_PN_GATEWAY_TYPE_PASSIVE_NUM > 0) */
    }
  }
}
...
)
{
  ...
  if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
  {
    ...
    for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
    {
      ...
      /* !LINKSTO FrNm.SWS_FrNm_00501,1 */
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg[PnIndex] |=
        (uint8)(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoRcv[PnIndex] &
                FrNm_RootPtr->PnFilterMask[PnIndex]);
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoRcv[PnIndex] = 0;
    }
#if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON)
    setDet = FrNm_HandlePNSyncShutdownRetransmissionActive(FRNM_PL_SF(instIdx));
#endif /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */
    ...
    {
      FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)(FRNM_PN_SYNC_SHUTDOWN);
    }
    else
    {
      FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN));
    ...
    SchM_Exit_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
#if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON)
#if (FRNM_DET_RUNTIME_CHECKS == STD_ON)
    if (setDet == TRUE)
    {
      /* !LINKSTO FrNm.ASR21-11.SWS_FrNm_00513,1 */
      FRNM_DET_REPORT_RUNTIME_ERROR(FRNM_INST(instIdx),
                                    FRNM_MAINFUNCTION_ID + FRNM_INST(instIdx),
                                    FRNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED);
    }
#else /* #if (FRNM_DET_RUNTIME_CHECKS == STD_ON) */
    TS_PARAM_UNUSED(setDet);
#endif /* #if (FRNM_DET_RUNTIME_CHECKS == STD_ON) */
#endif /* #if (FRNM_PN_SHUTDOWN_MESSAGE_RETRANSMISSION_ENABLED == STD_ON) */
  }
}
...
{
  boolean MessageSentDecoupled;
  boolean MessageSentImmediate;
  uint16 PnIndex;
  boolean retransmitTimeout = FALSE;

  boolean setDet = FALSE;

  /* !LINKSTO FrNm.ASR21-11.SWS_FrNm_00510,1 */
  if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN_RESTART_TIMER) != 0U)
  {
    FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN_RESTART_TIMER));
    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer =
      FRNM_CHANNELID_CONFIG(instIdx).PnShutdownMessageRetransmissionDuration;
  }
  if (FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer > 0U)
  {
    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer--;
    if (FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer == 0U)
    {
  ...
  /* reset tx flags */
  /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00508.CancelRetransmission,1 */
  /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00509.RetransmissionNextDataCycle,1 */
  if (((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN_DECOUPLED_TX) != 0U) ||
       (retransmitTimeout == TRUE)
     )
  {
    FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN_DECOUPLED_TX));
    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer = 0;
    MessageSentDecoupled = TRUE;
  }
  else
  {
    MessageSentDecoupled = FALSE;
  }

  if (((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN_IMMEDIATE_TX) != 0U) ||
       (retransmitTimeout == TRUE)
     )
  {
    FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN_IMMEDIATE_TX));
    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnRetransmissionTimer = 0;
    MessageSentImmediate = TRUE;
  }
  else
  {
    MessageSentImmediate = FALSE;
  }
  ...
  /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00508.PncStore,1 */
  if (MessageSentDecoupled == TRUE)
  {
    for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
    {
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg[PnIndex] &=
      ...
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentDecoupled[PnIndex] = 0;
    }
  }
  if (MessageSentImmediate == TRUE)
  {
    for (PnIndex = 0U; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex++)
    {
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoAgg[PnIndex] &=
          (uint8)(~(uint8)(FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentImmediate[PnIndex]));
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownIds->PnInfoSentImmediate[PnIndex] = 0;
    }
  }
  return setDet;
}
...
{
  uint16 PnIndex;
  uint8 newForwardSyncShutdown = 0;
  uint8 PnBitPos;
  uint8 PnBitMask;
  if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_PASSIVE)
  {
    SchM_Enter_FrNm_SCHM_FRNM_EXCLUSIVE_AREA_0();
    if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_RX) != 0U)
    {
      FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags &= (uint8)(~(uint8)(FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_RX));

      newForwardSyncShutdown = 0U;
      /* copy rx buffer into aggregate, and remove requests that were received on the bus */
      for (PnIndex = 0; PnIndex < FRNM_PN_INFO_LENGTH; PnIndex ++)
      {

#if (FRNM_MULTICORE_ENABLED == STD_ON)
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg.PnInfo[PnIndex] =
#else
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg[PnIndex] =
#endif
        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRx[PnIndex];

#if (FRNM_MULTICORE_ENABLED == STD_ON)
        newForwardSyncShutdown |= FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg.PnInfo[PnIndex];
#else
        newForwardSyncShutdown |= FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg[PnIndex];
#endif

        FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRx[PnIndex] = 0;
      }
    }
    ...
    if (newForwardSyncShutdown != 0U)
    {
      ...
      FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags |= (uint8)FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_FWD;
    }
...
#if (FRNM_MULTICORE_ENABLED == STD_ON)
          if ((FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg.PnInfo[PnIndex] &
              PnBitMask) != 0U
             )
#else
          if ((FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg[PnIndex] &
              PnBitMask) != 0U
             )
#endif
          {
            FRNM_CHANNEL_STATUS(instIdx).PnEra.PnTimer[FrNm_RootPtr->EiraTimerMap[(PnIndex * 8U) + PnBitPos]] = 1U;
          }
        }
      }
    }
...
{
  if ((FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags & FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_FWD) != 0U)
  {
    TS_AtomicClearBit_8(&FRNM_CHANNEL_STATUS(instIdx).PnSyncShutdownFlags, FRNM_PN_SYNC_SHUTDOWN_PASSIVE_CH_FWD_BIT);
    /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00540.PNSR,1 */
#if (FRNM_MULTICORE_ENABLED == STD_ON)
    FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg.Channel =
        FRNM_CHANNELID_CONFIG(instIdx).ChannelId;

    if (SchM_Send_FrNm_FrNm_ForwardSynchronizedPncShutdown_FrNm_To_Nm_DataSet(
         &FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg
        ) != SCHM_E_OK)
    {
#if (FRNM_DEV_ERROR_DETECT == STD_ON)
      /* !LINKSTO FrNm.EB.SchM_Call_DET,1 */
      /* !LINKSTO FrNm.EB.FrNmSynchronizedPncShutdownMulticoreForwardSynchFails,1 */
      FRNM_DET_REPORT_ERROR(FRNM_INST(instIdx),
                            (FRNM_SERVID_MAINFUNCTION_X + FRNM_INST(instIdx)),
                             FRNM_E_PNSR_FORWARD_SYNCHRONIZED_SHUTDOWN
                             );
#endif
    }
#else
    (void) Nm_ForwardSynchronizedPncShutdown(FRNM_CHANNELID_CONFIG(instIdx).ChannelId,
      FRNM_CHANNEL_STATUS(instIdx).FrNm_SynchronizedPncShutdownRxIds->PnInfoRxAgg
    );
#endif
  }
...
)
{
  ...
  if (FRNM_CHANNELID_CONFIG(instIdx).FrNmTypeOfGateway == FRNM_GATEWAY_TYPE_ACTIVE)
  {
        ...
        )
    {
      ...
      {
          /* Emit event to repeat transmission of vote  */
          FRNM_EMIT(instIdx, FRNM_HSM_FRNM_EV_VOTE_CYCLE_TIMESLOT_SEND_IN_RS);
      }
    }
    if (((FRNM_CHANNEL_PROPERTY(instIdx) & FRNM_PDU_SCHEDULE_VARIANT_MASK) !=
          FRNM_PDU_SCHEDULE_VARIANT_1) &&
       ...
       )
    {
      ...
      {
        /* Emit event to repeat transmission of data */
        FRNM_EMIT(instIdx, FRNM_HSM_FRNM_EV_DATA_CYCLE_TIMESLOT_SEND_IN_RS);
      }
    }
  }
#endif /* #if ((FRNM_PN_SYNCHRONIZED_SHUTDOWN_ENABLED == STD_ON) && \
  ...
  {
    /* Emit event to repeat transmission of vote  */
    /* !LINKSTO FrNm.EB_Ref.SWS_FrNm_00126.VotePDU.2,1 */
    FRNM_EMIT(instIdx, FRNM_HSM_FRNM_EV_VOTE_CYCLE_TIMESLOT_SEND_IN_RS);
  }
#endif /* #if (FRNM_SEND_NEGATIVE_VOTE_IN_READYSLEEP_STATE == STD_ON) */
}
...
/* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.FrNm_MultipleNetworkOperations.Available,1 */
#if (FRNM_MULTICORE_USE_SINGLE_CS == STD_ON)
/* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.FrNm_MultipleNetworkOperations,1 */
FUNC(Std_ReturnType, FRNM_CODE) FrNm_MultipleNetworkOperations
(
  NetworkHandleType nmChannelHandle,
  uint8             operation
)
{
  Std_ReturnType retVal = E_NOT_OK;
  /* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.PassiveStartup,1 */
  if (operation == FRNM_MULTI_CORE_USE_SINGLE_CS_PASSIVE_STARTUP)
  {
    retVal = FrNm_PassiveStartUp(nmChannelHandle);
  }
#if (FRNM_PASSIVE_MODE_ENABLED == STD_OFF)
  /* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.NetworkRequest,1 */
  else if (operation == FRNM_MULTI_CORE_USE_SINGLE_CS_NETWORK_REQUEST)
  {
    retVal = FrNm_NetworkRequest(nmChannelHandle);
  }
  /* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.NetworkRelease,1 */
  else if (operation == FRNM_MULTI_CORE_USE_SINGLE_CS_NETWORK_RELEASE)
  {
    retVal = FrNm_NetworkRelease(nmChannelHandle);
  }
  /* !LINKSTO FrNm.EB.FrNmMultiCoreUseSingleCS.NetworkGwEraRequest,1 */
  else
  {
    retVal = FrNm_NetworkGwEraRequest(nmChannelHandle);
  }
#endif
  return retVal;
}
