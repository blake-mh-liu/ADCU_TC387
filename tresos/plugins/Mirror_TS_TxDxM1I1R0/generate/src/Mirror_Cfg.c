...
[!VAR "CanNWSize" = "num:i(count(as:modconf('Mirror')[1]/MirrorConfigSet/MirrorSourceNetwork/*[node:name(.) = 'MirrorSourceNetworkCan']))"!][!//
...
[!CALL "ValidateNetworkId", "NetworkType" = "node:name(.)" , "NetworkID" = "num:i(./MirrorNetworkId)" , "NetworkName" = "as:name(.)"!][!//
    {
[!IF "num:i($CanNWSize) > 0"!][!//
[!IF "node:name(.) = 'MirrorSourceNetworkCan'"!][!//
        [!"num:i($ElementIdx)"!]U,    /* Can element index within intermediate buffers */
        [!"(num:i((node:value(./MirrorCanFramesPerSecond) * $MainFuncPeriod) +1))"!]U,    /* Can network total number of elements */
        [!VAR "ElementIdx" = "$ElementIdx + (num:i((node:value(./MirrorCanFramesPerSecond) * $MainFuncPeriod) +1))"!][!//
[!IF "node:exists(./MirrorCanFDFramesPerSecond)"!][!//
[!"num:i($FDElementIdx)"!]U,    /* CanFD element index within intermediate buffers */
        [!"(num:i((node:value(./MirrorCanFDFramesPerSecond) * $MainFuncPeriod) +1))"!]U,    /*CanFD network total number of elements*/
        [!VAR "FDElementIdx" = "$FDElementIdx +(num:i((node:value(./MirrorCanFDFramesPerSecond) * $MainFuncPeriod) +1))"!][!//
[!ELSE!][!//
[!IF "node:exists( as:modconf('Mirror')[1]/MirrorConfigSet/MirrorSourceNetwork/*/MirrorCanFDFramesPerSecond)"!][!//
0U,    /* CanFD is not configured for this network */
        0U,    /* CanFD is not configured for this network*/
[!ENDIF!][!//
[!ENDIF!][!//
[!ELSE!][!//
        0U,    /* SourceNetwork is not Can,  element not needed */
        0U,    /* SourceNetwork is not Can,  element not needed */
[!IF "node:exists( as:modconf('Mirror')[1]/MirrorConfigSet/MirrorSourceNetwork/*/MirrorCanFDFramesPerSecond)"!][!//
        0U,    /* SourceNetwork is not CanFD,  element not needed */
        0U,    /* SourceNetwork is not CanFD,  element not needed */
[!ENDIF!][!//
[!ENDIF!][!//
[!ENDIF!][!//
...
/*==================[external data]=========================================*/
/* !LINKSTO Mirror.EB.LinIntermediateBuffer,1 */
...
#include <Mirror_MemMap.h>
[!ENDIF!][!//
/* !LINKSTO Mirror.EB.CanIntermediateBuffer,1 */
/* !LINKSTO Mirror.EB.CanFDIntermediateBuffer,1 */
[!IF "num:i($CanNWSize) > 0"!][!//
...
#include <Mirror_MemMap.h>
[!ENDIF!][!//
/* !LINKSTO Mirror.EB.FlexRayIntermediateBuffer,1 */
[!IF "node:exists(as:modconf('Mirror')[1]/MirrorGeneral/MirrorFrFramesPerSecond)"!][!//
