 ...
 *
 * Reason: False-negative from the Tasking v5.0r2 compiler (TCVX-41885)
 */
...
/* minor version check */
#if (MIRROR_SW_MINOR_VERSION < 1U)
#error MIRROR_SW_MINOR_VERSION wrong (< 1U)
#endif
...
/* patch version check */
#if (MIRROR_SW_PATCH_VERSION < 10U)
#error MIRROR_SW_PATCH_VERSION wrong (< 10U)
#endif
...
/** \brief Mirror_SetAgregateChannelStatus()
 *
 * This function checks channelAStatus, channelBStatus and sets the aggregated channel status for FlexRay channels A and B.
 *
 * \param[in] channelAStatus  Status of FlexRay channel A
 * \param[in] channelBStatus  Status of FlexRay channel B
 *
 *  \return AggStatusByte  aggregated channel status for FlexRay channels A and B of a cluster
 *
 */
STATIC FUNC(uint8, MIRROR_CODE) Mirror_SetAgregateChannelStatus
(
  VAR(uint16, TYPEDEF) channelAStatus,
  VAR(uint16, TYPEDEF) channelBStatus
);

/** \brief Mirror_ChannelASetAggrigateStatus()
 *
 * This function checks channelAStatus and sets the aggregated channel status for FlexRay channels A
 *
 * \param[in] channelAStatus  Status of FlexRay channel A
 * \param[inout] AggStatusByte  aggregated channel status for FlexRay channels A of a cluster
 *
 *  \return
 *
 */

STATIC FUNC(void, MIRROR_CODE) Mirror_ChannelASetAggrigateStatus
(
  VAR(uint16, TYPEDEF) channelAStatus,
  P2VAR(uint8, AUTOMATIC, MIRROR_APPL_DATA) AggStatusByte
);

/** \brief Mirror_ChannelBSetAggrigateStatus()
 *
 * This function checks channelAStatus and sets the aggregated channel status for FlexRay channels B
 *
 * \param[in] channelBStatus  Status of FlexRay channel B
 * \param[inout] AggStatusByte  aggregated channel status for FlexRay channels B of a cluster
 *
 *  \return
 *
 */
STATIC FUNC(void, MIRROR_CODE) Mirror_ChannelBSetAggrigateStatus
(
  VAR(uint16, TYPEDEF) channelBStatus,
  P2VAR(uint8, AUTOMATIC, MIRROR_APPL_DATA) AggStatusByte
);
...
/** \brief Mirror_UpdateQueueBufferHeader()
 *
 * This function finds the time stamp with the min value to update the queue buffer header with it.
 *
 ...
 */
STATIC FUNC (void, MIRROR_CODE) Mirror_UpdateQueueBufferHeader
(
 ...
 *
 * This function performs queue buffer activation.
 *
 * \param[out] MinSrcNwIdx  Netowrk Index of least timestamp.
 * \param[out] MinSrcNwType Netowrk type of least timestamp (CAN or CANFD).
 */
STATIC FUNC (void, MIRROR_CODE) Mirror_ActivateQueueBuffer
(
  P2VAR(uint8, AUTOMATIC, MIRROR_APPL_DATA) MinSrcNwIdx,
  P2VAR(boolean, AUTOMATIC, MIRROR_APPL_DATA) MinSrcNwType
);

#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
/** \brief Mirror_UpdateCanFramesRemaining()
 *
 * This function check all networks if there are elements ready to be sent.
 *
 * \param[out] FramesRemaining  Netowrk Index of least timestamp.
 */
STATIC FUNC (void, MIRROR_CODE) Mirror_UpdateCanFramesRemaining
(
  P2VAR(boolean, AUTOMATIC, MIRROR_APPL_DATA) FramesRemaining
);
 ...
 * and updates MinTimeStampSrcNetIdx with the index of the network of min time stamp
 *
 ...
 *
 */
STATIC FUNC (void, MIRROR_CODE) Mirror_GetMinTimeStamp
(
...
/** \brief Mirror_CheckTimeStamp()
 *
 * This function checks if the time stamps are NULL_PTR and if not,
 * compares the time stamps entered by calling Mirror_TimeStampComparator().
 *
 ...
 *
 * \return TRUE TimeStamp_1 is less than or equal to TimeStamp_2.
 *         FALSE TimeStamp_1 is greater than TimeStamp_2.
 *
 */
STATIC FUNC (boolean, MIRROR_CODE) Mirror_CheckTimeStamp
(
...
/** \brief Mirror_SecondsStampComparator()
 *
 * This function compares the seconds and nanoseconds when secondsHi of both timestamps are equal.
 *
 * \param[in] TimeStamp_1  first time stamp operand.
 * \param[in] TimeStamp_2  second time stamp operand.
 *\param[out] MinTimeStamp  The minimum time stamp between the 2 operands.
 ...
 *
 */
STATIC FUNC (boolean, MIRROR_CODE) Mirror_SecondsStampComparator
(
  P2CONST(MIRROR_TIMESTAMP_TYPE, AUTOMATIC, MIRROR_APPL_DATA) TimeStamp_1,
  P2CONST(MIRROR_TIMESTAMP_TYPE, AUTOMATIC, MIRROR_APPL_DATA) TimeStamp_2,
  P2VAR(MIRROR_TIMESTAMP_TYPE, AUTOMATIC, MIRROR_APPL_DATA) MinTimeStamp
 ...
 *
 */
STATIC FUNC (void, MIRROR_CODE) CopyStatusBytes
(
  void
);
 ...
 *
 * This function copies the can frame of the CanElementReadIdx -the read index of crossponding Can network- to the queue buffer.
 *
 * \param[in] CanElementReadIdx Index of Can Element to be read.
 *
...
(
  VAR(Mirror_ElementIdxType, MIRROR_VAR) CanElementReadIdx,
 ...
 *
 * This function copies the can frame of the CanFDElementReadIdx -the read index of crossponding Can network- to the queue buffer.
 *
 * \param[in] CanFDElementReadIdx Index of CanFD Element to be read.
 *
 * \param[out] TRUE  Frame was copied succussfully.
 *             FALSE Frame wasn't copied as there is no space in the queue buffer.
 *
...
(
  VAR(Mirror_FDElementIdxType, MIRROR_VAR) CanFDElementReadIdx,
  P2VAR(boolean, AUTOMATIC, MIRROR_APPL_DATA) CopySuccessStatus
);
...
#define MIRROR_START_SEC_CODE_CAN
#include <Mirror_MemMap.h>
...
#define MIRROR_STOP_SEC_CODE_CAN
#include <Mirror_MemMap.h>
...
#define MIRROR_START_SEC_CODE_LIN
#include <Mirror_MemMap.h>
...
#define MIRROR_STOP_SEC_CODE_LIN
#include <Mirror_MemMap.h>
...
#define MIRROR_START_SEC_CODE_FLEXRAY
#include <Mirror_MemMap.h>
...
#define MIRROR_STOP_SEC_CODE_FLEXRAY
#include <Mirror_MemMap.h>
...
STATIC VAR(uint8, MIRROR_VAR) MinTimeStampSrcNetIdx;

/** \brief Variable to indicate the buffer type which will write the first data element in the queue buffer. */
STATIC VAR(boolean, MIRROR_VAR) MinTimeStampCanType;

#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
/** \brief Variable to indicate the index of the network of minimum time to be written first in the queue buffer. */
STATIC VAR(Mirror_ElementIdxType, MIRROR_VAR) MinTimeStampCanSrcNetIdx;
#endif /* MIRROR_CANFD_SOURCENETWORK */

#if(MIRROR_CANFD_SOURCENETWORK == STD_ON)
/** \brief Variable to indicate the index of the Can FD network of minimum time to be written first in the queue buffer. */
STATIC VAR(Mirror_FDElementIdxType, MIRROR_VAR) MinTimeStampCanFDSrcNetIdx;
#endif /* MIRROR_CANFD_SOURCENETWORK */
...
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
    /*Check all the CAN networks for elements which are ready to be copied in the main function to the queue.*/
    /* Deviation TASKING-1 */
    for(SrcNtwrkIdx = (uint8)0x00; ((SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS) && (FALSE == CanElementReady || FALSE == CanFDElementReady)); SrcNtwrkIdx += (uint8)0x01)
    {
      Mirror_ElementIdxType CanElementReadIdx=0;
      if((uint8)MIRROR_NT_CAN == Mirror_SourceNetworks[SrcNtwrkIdx].NetworkType)
      {
        CanElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkReadIndex;
        CanElementReady = (boolean)(CanElementReady ||((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanElement[CanElementReadIdx].ElementStatus));
#if(MIRROR_CANFD_SOURCENETWORK == STD_ON)
        /*Check if this Can network supports CanFD*/
        if(0U < Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber)
        {
          Mirror_FDElementIdxType CanFDElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDReadIndex;
          CanFDElementReady = (boolean)(CanFDElementReady || ((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanFDElement[CanFDElementReadIdx].ElementStatus));
        }
#endif /*MIRROR_CANFD_SOURCENETWORK == STD_ON*/
      }
    }
...
#if(MIRROR_LIN_SOURCENETWORK == STD_ON)
    /*Check LIN networks for elements which are ready to be copied in the main function to the queue.*/
    LinElementReady = (boolean)((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_LinElement[Mirror_LinElementReadIndex].ElementStatus);
#endif /* MIRROR_LIN_SOURCENETWORK == STD_ON */

#if(MIRROR_FLEXRAY_SOURCENETWORK == STD_ON)
    /*Check FlexRay networks for elements which are ready to be copied in the main function to the queue.*/
...
)
{
...
#endif /* MIRROR_DEV_ERROR_DETECT */
  {
    ...
    else
    {
      ...
      /*Check if this function is preempted by the same network.*/
      /* !LINKSTO Mirror.EB.CanNestedReportFrameDifferentNetwork,1 */
      else if(FALSE != Mirror_SourceNetworkStatus[SrcNtwrkIdx].ReportingOngoing)
      {
        /* !LINKSTO Mirror.EB.CanNestedReportFrameReportDet,1 */
#if(MIRROR_DEV_ERROR_DETECT == STD_ON)
        MIRROR_DET_REPORT_ERROR(MIRROR_SID_REPORTCANFRAME, MIRROR_E_NESTED_REPORT_FRAMES);
#endif /* MIRROR_DEV_ERROR_DETECT */
        /* !LINKSTO Mirror.EB.CanNestedReportFrameLostFrame,1 */
      ...
      else
      {
        ...
        else
        {
          ...
          {
            Mirror_SourceNetworkStatus[SrcNtwrkIdx].ReportingOngoing = TRUE;
            ...
            else
            {
              ...
              {
                /*Check if this network supports CanFD*/
                if(0U < Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber)
                {
                ...
                {
#if(MIRROR_DEV_ERROR_DETECT == STD_ON)
                  /*This Can network doesn't support CanFD frames.*/
                  /* !LINKSTO Mirror.EB.InvalidCanFDNetworkReportDet,1 */
                  MIRROR_DET_REPORT_ERROR(MIRROR_SID_REPORTCANFRAME, MIRROR_E_INVALID_CANFD_NETWORK);
#endif /* MIRROR_DEV_ERROR_DETECT */
                }
              }
              else
            ...
            Mirror_SourceNetworkStatus[SrcNtwrkIdx].ReportingOngoing = FALSE;
          }
        }
      }
    }
...
)
{
...
#endif /* MIRROR_DEV_ERROR_DETECT */
  {
    ...
    {
      /* !LINKSTO Mirror.EB.NestedReportFrameReportDet,2 */
#if(MIRROR_DEV_ERROR_DETECT == STD_ON)
      MIRROR_DET_REPORT_ERROR(MIRROR_SID_REPORTLINFRAME, MIRROR_E_NESTED_REPORT_FRAMES);
#endif /* MIRROR_DEV_ERROR_DETECT */
      /* !LINKSTO Mirror.EB.NestedReportFrameLostFrame,2 */
      MIRROR_SET_LOST_FRAME_BIT(Mirror_SourceNetworkStatus[SrcNtwrkIdx].SourceNetworkCurrentStatusByte);
    }
    else
    {
...
)
{
  ...
  Mirror_ChannelASetAggrigateStatus(channelAStatus,&AggStatusByte);

  Mirror_ChannelBSetAggrigateStatus(channelBStatus,&AggStatusByte);

  return AggStatusByte;
}
...
#endif /* MIRROR_DEV_ERROR_DETECT */
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          uint8 FlexRayChannelCurrentStatusByte = Mirror_SetAgregateChannelStatus(channelAStatus, channelBStatus);
          Mirror_FlexRaySrcAggStatus[Mirror_SourceNetworks[SrcNtwrkIdx].FlexRayNetworkIdx].FlexRayChannelCurrentStatusByte = FlexRayChannelCurrentStatusByte;
        }
      }
    }
  }
...
#if (MIRROR_CAN_SOURCENETWORK == STD_ON)
  Mirror_ElementIdxType NetworkCounter;
#endif /* MIRROR_CAN_SOURCENETWORK == STD_ON */
#if (MIRROR_LIN_SOURCENETWORK == STD_ON)
  /* Deviation TASKING-1 */
...
#if (MIRROR_CAN_SOURCENETWORK == STD_ON)
  /* Deviation TASKING-1 */
  for(NetworkCounter = 0U; NetworkCounter<MIRROR_NO_OF_SOURCENETWORKS; NetworkCounter++)
  {
    Mirror_SourceNetworkStatus[NetworkCounter].NetworkReadIndex = Mirror_SourceNetworks[NetworkCounter].Mirror_ElementIdx;
    Mirror_SourceNetworkStatus[NetworkCounter].NetworkWriteIndex = Mirror_SourceNetworks[NetworkCounter].Mirror_ElementIdx;
    Mirror_SourceNetworkStatus[NetworkCounter].ElementCounter = Mirror_SourceNetworks[NetworkCounter].Mirror_ElementTotalNumber;
    Mirror_SourceNetworkStatus[NetworkCounter].ReportingOngoing = FALSE;
#if (MIRROR_CANFD_SOURCENETWORK == STD_ON)
   if(0U < Mirror_SourceNetworks[NetworkCounter].Mirror_FDElementTotalNumber)
   {
     Mirror_SourceNetworkStatus[NetworkCounter].NetworkFDReadIndex = Mirror_SourceNetworks[NetworkCounter].Mirror_FDElementIdx;
     Mirror_SourceNetworkStatus[NetworkCounter].NetworkFDWriteIndex = Mirror_SourceNetworks[NetworkCounter].Mirror_FDElementIdx;
     Mirror_SourceNetworkStatus[NetworkCounter].ElementFDCounter = Mirror_SourceNetworks[NetworkCounter].Mirror_FDElementTotalNumber;
   }
#endif /* MIRROR_CANFD_SOURCENETWORK == STD_ON */
  }
#endif /* MIRROR_CAN_SOURCENETWORK == STD_ON */
...
)
{
...
#if(MIRROR_CAN_SOURCENETWORK == STD_OFF)
  TS_PARAM_UNUSED(MinSrcNwType);
  TS_PARAM_UNUSED(SrcNtwrkIdx);
  TS_PARAM_UNUSED(CanElementReady);
#endif  /* MIRROR_CAN_SOURCENETWORK == STD_OFF */
#if(MIRROR_CANFD_SOURCENETWORK == STD_OFF)
  TS_PARAM_UNUSED(CanFDElementReady);
#endif  /* MIRROR_CANFD_SOURCENETWORK == STD_OFF */

#if(MIRROR_LIN_SOURCENETWORK == STD_OFF)
  TS_PARAM_UNUSED(LinElementReady);
#endif  /* MIRROR_LIN_SOURCENETWORK == STD_OFF */

#if(MIRROR_FLEXRAY_SOURCENETWORK == STD_OFF)
  TS_PARAM_UNUSED(FlexRayElementReady);
#endif  /* MIRROR_FLEXRAY_SOURCENETWORK == STD_OFF */
  ...
  for(SrcNtwrkIdx = (uint8)0x00; ((SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS) && (FALSE == CanElementReady || FALSE == CanFDElementReady)); SrcNtwrkIdx += (uint8)0x01)
  {
    ...
    if((uint8)MIRROR_NT_CAN == Mirror_SourceNetworks[SrcNtwrkIdx].NetworkType)
    {
      ...
      CanElementReady = (boolean)(CanElementReady ||((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanElement[CanElementReadIdx].ElementStatus));
#if(MIRROR_CANFD_SOURCENETWORK == STD_ON)
      /*Check if this Can network aslo supports CanFD*/
      if(0U < Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber)
      {
        Mirror_FDElementIdxType CanFDElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDReadIndex;
        CanFDElementReady = (boolean)(CanFDElementReady || ((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanFDElement[CanFDElementReadIdx].ElementStatus));
  ...
  Mirror_UpdateQueueBufferHeader(CanElementReady, CanFDElementReady, LinElementReady, FlexRayElementReady);
  *MinSrcNwIdx = (uint8)MinTimeStampSrcNetIdx;
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
  *MinSrcNwType = (boolean)MinTimeStampCanType;
#endif /*MIRROR_CAN_SOURCENETWORK == STD_ON*/
}
...
)
{
...
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
  MIRROR_TIMESTAMP_TYPE OutputTimeStamp;
  uint8 SrcNtwrkIdx = (uint8)0x00U;
  OutputTimeStamp.secondsHi = 0xFFFFU;
  OutputTimeStamp.seconds = 0xFFFFFFFFU;
  OutputTimeStamp.nanoseconds = 0xFFFFFFFFU;
   ...
   for(SrcNtwrkIdx = (uint8)0x00; SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS; SrcNtwrkIdx += (uint8)0x01)
     {
       ...
       {
         CanElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkReadIndex;
         if((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanElement[CanElementReadIdx].ElementStatus)
         {
           CanTimeStamp = &Mirror_CanTimeStamp[CanElementReadIdx];
           if(FALSE == Mirror_CheckTimeStamp(&OutputTimeStamp, CanTimeStamp, &OutputTimeStamp))
           {
             /*Update the index of minimum Can timestamp */
             MinTimeStampCanSrcNetIdx = SrcNtwrkIdx;
           }
         }
       }
     }
     /*update the Can timestamp pointer with the minimum */
     CanTimeStamp->secondsHi = OutputTimeStamp.secondsHi;
     CanTimeStamp->seconds = OutputTimeStamp.seconds;
     CanTimeStamp->nanoseconds = OutputTimeStamp.nanoseconds;
  }
  else
  {
    /* Do nothing */
  ...
  if(TRUE == CanFDFrameReady)
  {
    ...
    for(SrcNtwrkIdx = (uint8)0x00; SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS; SrcNtwrkIdx += (uint8)0x01)
    {
       Mirror_ElementIdxType CanElementFDReadIdx = 0U;
       if((uint8)MIRROR_NT_CAN == Mirror_SourceNetworks[SrcNtwrkIdx].NetworkType)
       {
         /* Check if this Can network supports CanFD */
         if(0U < Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber)
         {
           CanElementFDReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDReadIndex;
           if((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanFDElement[CanElementFDReadIdx].ElementStatus)
           {
             CanFDTimeStamp = &Mirror_CanFDTimeStamp[CanElementFDReadIdx];
             if(FALSE == Mirror_CheckTimeStamp(&OutputTimeStamp, CanFDTimeStamp, &OutputTimeStamp))
             {
               /*Update the index of minimum CanFD timestamp */
               MinTimeStampCanFDSrcNetIdx = SrcNtwrkIdx;
             }
           }
         }
       }
    ...
    /*update the CanFD timestamp pointer with the minimum */
    CanFDTimeStamp->secondsHi = OutputTimeStamp.secondsHi;
    CanFDTimeStamp->seconds = OutputTimeStamp.seconds;
    CanFDTimeStamp->nanoseconds = OutputTimeStamp.nanoseconds;
  }
  else
  {
    /* Do nothing */
  ...
  Mirror_GetMinTimeStamp(CanTimeStamp, CanFDTimeStamp, LinTimeStamp, FlexRayTimeStamp, &RefTimeStamp);
...
{
  uint8 SrcNtwrkIdx = (uint8)0x00;
  /* Check all network if there are ready frames to be sent */
  for(SrcNtwrkIdx = 0U; (SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS) && (FALSE == *FramesRemaining); SrcNtwrkIdx += (uint8)0x01)
  {
    if((uint8)MIRROR_NT_CAN == Mirror_SourceNetworks[SrcNtwrkIdx].NetworkType)
    {
      Mirror_ElementIdxType CanElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkReadIndex;
      /* Deviation TASKING-2 */
      *FramesRemaining = (*FramesRemaining || (((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanElement[CanElementReadIdx].ElementStatus) && (Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementCounter > (Mirror_ElementIdxType)0x00U)));
#if(MIRROR_CANFD_SOURCENETWORK == STD_ON)
      /*Check if this network supports CanFD*/
      if(0U < Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber)
      {
        Mirror_FDElementIdxType CanFDElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDReadIndex;
        /* Deviation TASKING-1 */
        *FramesRemaining = (*FramesRemaining || (((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanFDElement[CanFDElementReadIdx].ElementStatus) && (Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementFDCounter > (Mirror_FDElementIdxType)0x00U)));
      }
#endif /* MIRROR_CANFD_SOURCENETWORK == STD_ON */
    }
    else
...
{
  if(TRUE == Mirror_CheckTimeStamp(CanTimeStamp, CanFDTimeStamp, OutputTimeStamp))
  {
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
    /* The data element with the min time stamp is a Can data element */
    MinTimeStampSrcNetIdx = MinTimeStampCanSrcNetIdx;
    MinTimeStampCanType = (boolean)MIRROR_CAN_NETWORK_TYPE;
#endif /* MIRROR_CAN_SOURCENETWORK == STD_ON */
  }
  else
  {
    ...
    MinTimeStampSrcNetIdx = MinTimeStampCanFDSrcNetIdx;
    MinTimeStampCanType = (boolean)MIRROR_CANFD_NETWORK_TYPE;
#endif /* MIRROR_CANFD_SOURCENETWORK == STD_ON */
  }
  if(TRUE == Mirror_CheckTimeStamp(OutputTimeStamp, LinTimeStamp, OutputTimeStamp))
  {
  ...
  else
  {
    ...
    MinTimeStampSrcNetIdx = MIRROR_LIN_FIRSTNETWORKINDEX;
#endif /* MIRROR_LIN_SOURCENETWORK == STD_ON */
  }
  if(TRUE == Mirror_CheckTimeStamp(OutputTimeStamp, FlexRayTimeStamp, OutputTimeStamp))
  {
      /* Do nothing as the data element with the min time stamp is the same as above */
  }
  else
  {
#if(MIRROR_FLEXRAY_SOURCENETWORK == STD_ON)
    /* The data element with the min time stamp is a FlexRay data element */
    MinTimeStampSrcNetIdx = MIRROR_FLEXRAY_FIRSTNETWORKINDEX;
#endif /* MIRROR_FLEXRAY_SOURCENETWORK == STD_ON */
  }
}
...
)
{
  ...
  {
    retVal = Mirror_SecondsStampComparator(TimeStamp_1, TimeStamp_2,MinTimeStamp);
  }
  return retVal;
}
...
)
{
  ...
  {
    retVal = Mirror_TimeStampComparator(TimeStamp_1, TimeStamp_2, MinTimeStamp);
  }
  return retVal;
}
...
{
  uint8 SrcNtwrkIdx = (uint8)0U;
  uint8 MinSrcNwIdx = (uint8)MinTimeStampSrcNetIdx;
  boolean MinSrcNwType = (uint8)MinTimeStampCanType;
  ...
  while ((E_OK == QueueBuffAvailable) && (TRUE == FramesRemaining))
  {
    ...
    {
      Mirror_ActivateQueueBuffer(&MinSrcNwIdx,&MinSrcNwType);
    }
    else
    {
    ...
    while ((TRUE == CopySuccess) && (TRUE == FramesRemaining))
    {
      ...
      for(SrcNtwrkIdx = (uint8)MinSrcNwIdx; SrcNtwrkIdx < (uint8)MIRROR_NO_OF_SOURCENETWORKS; SrcNtwrkIdx += (uint8)0x01)
      {
        switch (Mirror_SourceNetworks[SrcNtwrkIdx].NetworkType)
        {
          case (uint8)MIRROR_NT_CAN:
          {
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
            if((boolean)MIRROR_CAN_NETWORK_TYPE == MinSrcNwType)
            {
              Mirror_ElementIdxType CanElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkReadIndex;
              CanElementReady = (boolean)(((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanElement[CanElementReadIdx].ElementStatus) && (Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementCounter > (Mirror_ElementIdxType)0x00U));
              if((TRUE == CanElementReady) && (TRUE == CopySuccess))
              {
                QueueBuffAvailable = Mirror_CopyCurrentCanFrame(CanElementReadIdx,&CopySuccess);
                if(TRUE == CopySuccess)
                {
                  Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementCounter -= (Mirror_ElementIdxType)0x01;
                }
                else
                {
            ...
            {
              Mirror_FDElementIdxType CanFDElementReadIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDReadIndex;
              CanFDElementReady = (boolean)(((uint8)MIRROR_ELEMENT_WRITING_FINISHED == Mirror_CanFDElement[CanFDElementReadIdx].ElementStatus) && (Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementFDCounter > (Mirror_FDElementIdxType)0x00U));
              if((TRUE == CanFDElementReady) && (TRUE == CopySuccess))
              {
                QueueBuffAvailable = Mirror_CopyCurrentCanFDFrame(CanFDElementReadIdx,&CopySuccess);
                if(TRUE == CopySuccess)
                {
                  Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementFDCounter -= (Mirror_FDElementIdxType)0x01;
                }
                else
                {
                  /* Do nothing as the frame isn't copied to the counter shouldn't be decremented */
...
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
        MinSrcNwType = (boolean)MIRROR_CAN_NETWORK_TYPE;
#endif /*MIRROR_CAN_SOURCENETWORK == STD_ON*/
      }
      /* Reset index to start from network 0 */
      MinSrcNwIdx = 0x0U;
      /* Check all network if there are ready frames to be sent */
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
      /*Check for ready Can and CanFD elements*/
      Mirror_UpdateCanFramesRemaining(&FramesRemaining);
#endif /*MIRROR_CAN_SOURCENETWORK == STD_ON*/
#if(MIRROR_LIN_SOURCENETWORK == STD_ON)
      /* Deviation TASKING-2 */
...
#if(MIRROR_CAN_SOURCENETWORK == STD_ON)
  /* Restore element counter values */
  /* Deviation TASKING-1 */
  for(SrcNtwrkIdx = 0U; SrcNtwrkIdx<MIRROR_NO_OF_SOURCENETWORKS; SrcNtwrkIdx++)
  {
    Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementCounter = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_ElementTotalNumber;
#if (MIRROR_CANFD_SOURCENETWORK == STD_ON)
     Mirror_SourceNetworkStatus[SrcNtwrkIdx].ElementFDCounter = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber;
#endif /* MIRROR_CANFD_SOURCENETWORK == STD_ON */
  }
#endif /* MIRROR_CAN_SOURCENETWORK */
...
#else /* MIRROR_FLEXRAY_SOURCENETWORK == STD_ON */
    if((FALSE == Mirror_SourceNetworkStatus[Index].IsStatusReported) &&
       (Mirror_SourceNetworkStatus[Index].SourceNetworkCurrentStatusByte != Mirror_SourceNetworkStatus[Index].SourceNetworkOldStatusByte))
#endif /* MIRROR_FLEXRAY_SOURCENETWORK == STD_ON */
...
)
{
  ...
  boolean TimeStampBelowMax = FALSE;
  uint8 SourceNetworkIdx = Mirror_CanElement[CanElementReadIdx].ElementSrcNtwrk;
  Mirror_ElementIdxType CanElementIdx = Mirror_SourceNetworks[SourceNetworkIdx].Mirror_ElementIdx;
  Mirror_ElementIdxType CanElementsTotal = Mirror_SourceNetworks[SourceNetworkIdx].Mirror_ElementTotalNumber;

  /* Check if network status is started to check the status byte for the current data element */
  if(TRUE == Mirror_SourceNetworkStarted[SourceNetworkIdx])
  {
    /* The source network that reported this frame is active so a check to the status byte is needed */
    if((Mirror_SourceNetworkStatus[SourceNetworkIdx].SourceNetworkCurrentStatusByte) ==
    (Mirror_SourceNetworkStatus[SourceNetworkIdx].SourceNetworkOldStatusByte))
    {
  ...
  DataElementLength += (uint16)Mirror_CanElement[CanElementReadIdx].ElementLength;
  ...
  DataElementTimeStamp = Mirror_TimeStampDiff(&(Mirror_CanTimeStamp[CanElementReadIdx]), &TimeStampBelowMax);
  if((TRUE == *CopySuccessStatus) && (TRUE == TimeStampBelowMax))
  {
    ...
    /* Network ID copying*/
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00076,1 */
    TxBuffer[BuffIndex] = Mirror_SourceNetworks[SourceNetworkIdx].MirrorNetworkId;
    BuffIndex += (uint32)1U;
    ...
    if(TRUE == NetworkStatusAvailable)
    {
      ...
      /* !LINKSTO Mirror.ASR44.SWS_Mirror_00077,1 */
      /* !LINKSTO Mirror.ASR44.SWS_Mirror_00078,1 */
      Mirror_SourceNetworkStatus[SourceNetworkIdx].SourceNetworkOldStatusByte = Mirror_SourceNetworkStatus[SourceNetworkIdx].SourceNetworkCurrentStatusByte;
      TxBuffer[BuffIndex] = Mirror_SourceNetworkStatus[SourceNetworkIdx].SourceNetworkOldStatusByte;
      BuffIndex += (uint32)1U;
    }
    else
    {
    ...
    Mirror_SourceNetworkStatus[SourceNetworkIdx].IsStatusReported = TRUE;
    /* FrameID copying */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00095,1 */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00097,1 */
    Mirror_MemCpy32(Mirror_CanElement[CanElementReadIdx].FrameID, &TxBuffer[BuffIndex]);
    BuffIndex += (uint32)4U;
    /* Payload length copying */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00109,1 */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00110,1 */
    TxBuffer[BuffIndex] = Mirror_CanElement[CanElementReadIdx].ElementLength;
    BuffIndex += (uint32)1U;
    /* Payload copying */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00111,1 */
    TS_MemCpy(&TxBuffer[BuffIndex], &(Mirror_CanElement[CanElementReadIdx].DataElement[0U]), Mirror_CanElement[CanElementReadIdx].ElementLength);
    /* Change the status of the Can data element to available */
    Mirror_CanElement[CanElementReadIdx].ElementStatus = (uint8)MIRROR_ELEMENT_AVAILABLE;
    CanElementReadIdx -= CanElementIdx;
    CanElementReadIdx = (CanElementReadIdx + 1U) % (CanElementsTotal);
    CanElementReadIdx = CanElementReadIdx + CanElementIdx;
    Mirror_SourceNetworkStatus[SourceNetworkIdx].NetworkReadIndex = CanElementReadIdx;
    ...
    InsertSourceNetworkIndex(SourceNetworkIdx);
  }
  else
  {
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00049,1 */
...
)
{
  ...
  boolean NetworkStatusAvailable = FALSE;
  boolean TimeStampBelowMax = FALSE;
  uint8 SourceNetworkIdx = Mirror_CanFDElement[CanFDElementReadIdx].ElementSrcNtwrk;
  Mirror_FDElementIdxType CanFDElementIdx = Mirror_SourceNetworks[SourceNetworkIdx].Mirror_FDElementIdx;
  Mirror_FDElementIdxType CanFDTotalElemets = Mirror_SourceNetworks[SourceNetworkIdx].Mirror_FDElementTotalNumber;
  ...
  DataElementLength += (uint16)Mirror_CanFDElement[CanFDElementReadIdx].ElementLength;
  /* if the current buffer can't store the current element then check if the next queue can store the element. */
  if((Mirror_QueueBufferStatus[Mirror_ActiveQueueBuffer].FrameLength + DataElementLength) >
    Mirror_DestNetworks[Mirror_ActiveDestination].DestNetworkPduLength)
  {
  ...
  DataElementTimeStamp = Mirror_TimeStampDiff(&(Mirror_CanFDTimeStamp[CanFDElementReadIdx]), & TimeStampBelowMax);
  if((TRUE == *CopySuccessStatus) && (TRUE == TimeStampBelowMax))
  {
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00064,1 */
    /* The Frame can be copied so Set the availability status byte and start the copying The data to the queue buffer*/
    ...
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00095,1 */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00097,1 */
    Mirror_MemCpy32(Mirror_CanFDElement[CanFDElementReadIdx].FrameID, &TxBuffer[BuffIndex]);
    BuffIndex += (uint32)4U;
    /* Payload length copying */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00109,1 */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00110,1 */
    TxBuffer[BuffIndex] = Mirror_CanFDElement[CanFDElementReadIdx].ElementLength;
    BuffIndex += (uint32)1U;
    /* Payload copying */
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00111,1 */
    TS_MemCpy(&TxBuffer[BuffIndex], &(Mirror_CanFDElement[CanFDElementReadIdx].DataElement[0U]), Mirror_CanFDElement[CanFDElementReadIdx].ElementLength);
    /* Change the status of the CanFD data element to available */
    Mirror_CanFDElement[CanFDElementReadIdx].ElementStatus = (uint8)MIRROR_ELEMENT_AVAILABLE;
    CanFDElementReadIdx -= CanFDElementIdx;
    CanFDElementReadIdx = (CanFDElementReadIdx + 1U) % (CanFDTotalElemets);
    CanFDElementReadIdx = CanFDElementReadIdx + CanFDElementIdx;
    Mirror_SourceNetworkStatus[SourceNetworkIdx].NetworkFDReadIndex = CanFDElementReadIdx;
    Mirror_QueueBufferStatus[Mirror_ActiveQueueBuffer].FrameLength += DataElementLength;
    /* Overwrite the Frame length in the dest frame with the new length after adding the frame*/
    /* !LINKSTO Mirror.ASR44.SWS_Mirror_00062,1 */
    Mirror_MemCpy16((uint16)(Mirror_QueueBufferStatus[Mirror_ActiveQueueBuffer].FrameLength - (uint16)MIRROR_FRAME_HEADER_SIZE), &TxBuffer[MIRROR_DEST_FRAME_DATALENGTH_POSITION]);
...
)
{
  ...
  if(FALSE == Mirror_CheckTimeStamp(&RefTimeStamp, TimeStamp, &TempTimeStamp))
  {
    /* The difference is 0 */
    *BelowMax = TRUE;
  }
...
{
  MIRROR_USERDATA_TYPE Mirror_UserData;
  Mirror_ElementIdxType CanElementIdx = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_ElementIdx;
  Mirror_ElementIdxType CanTotalElements = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_ElementTotalNumber;
  Mirror_ElementIdxType CanWriteIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkWriteIndex;

  if((uint8)MIRROR_ELEMENT_AVAILABLE == Mirror_CanElement[CanWriteIdx].ElementStatus)
  {
    Mirror_CanElement[CanWriteIdx].ElementStatus = (uint8)MIRROR_ELEMENT_WRITING_IN_PROGRESS;
    MIRROR_GET_TIME_STAMP(&Mirror_CanTimeStamp[CanWriteIdx], &Mirror_UserData);
    Mirror_CanElement[CanWriteIdx].ElementSrcNtwrk = SrcNtwrkIdx;
    Mirror_CanElement[CanWriteIdx].FrameID = (uint32)canId;
    Mirror_CanElement[CanWriteIdx].ElementLength = length;
    TS_MemCpy(&(Mirror_CanElement[CanWriteIdx].DataElement[0U]), payload, length);
    Mirror_CanElement[CanWriteIdx].ElementStatus = (uint8)MIRROR_ELEMENT_WRITING_FINISHED;
    CanWriteIdx = CanWriteIdx - CanElementIdx;
    CanWriteIdx = ((CanWriteIdx + 1U) % (CanTotalElements));
    CanWriteIdx = CanWriteIdx + CanElementIdx;
    Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkWriteIndex = CanWriteIdx;
  }
  else
  {
#if(MIRROR_DEV_ERROR_DETECT == STD_ON)
    ...
    MIRROR_CAN_LOST_FRAME_CBK(Mirror_SourceNetworks[SrcNtwrkIdx].MirrorNetworkId);
...
{
  MIRROR_USERDATA_TYPE Mirror_UserData;
  Mirror_FDElementIdxType CanFDTotalElements = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementTotalNumber;
  Mirror_FDElementIdxType CanFDElementIdx = Mirror_SourceNetworks[SrcNtwrkIdx].Mirror_FDElementIdx;
  Mirror_FDElementIdxType CanFDWriteIdx = Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDWriteIndex;

  if((uint8)MIRROR_ELEMENT_AVAILABLE == Mirror_CanFDElement[CanFDWriteIdx].ElementStatus)
  {
    Mirror_CanFDElement[CanFDWriteIdx].ElementStatus = (uint8)MIRROR_ELEMENT_WRITING_IN_PROGRESS;
    MIRROR_GET_TIME_STAMP(&Mirror_CanFDTimeStamp[CanFDWriteIdx], &Mirror_UserData);
    Mirror_CanFDElement[CanFDWriteIdx].ElementSrcNtwrk = SrcNtwrkIdx;
    Mirror_CanFDElement[CanFDWriteIdx].FrameID = (uint32)canId;
    Mirror_CanFDElement[CanFDWriteIdx].ElementLength = length;
    TS_MemCpy(&(Mirror_CanFDElement[CanFDWriteIdx].DataElement[0U]), payload, length);
    Mirror_CanFDElement[CanFDWriteIdx].ElementStatus = (uint8)MIRROR_ELEMENT_WRITING_FINISHED;
    CanFDWriteIdx = CanFDWriteIdx - CanFDElementIdx;
    CanFDWriteIdx = ((CanFDWriteIdx + 1U) % (CanFDTotalElements));
    CanFDWriteIdx = CanFDWriteIdx + CanFDElementIdx;
    Mirror_SourceNetworkStatus[SrcNtwrkIdx].NetworkFDWriteIndex = CanFDWriteIdx;
  }
  else
  {
#if(MIRROR_DEV_ERROR_DETECT == STD_ON)
    ...
    MIRROR_CANFD_LOST_FRAME_CBK(Mirror_SourceNetworks[SrcNtwrkIdx].MirrorNetworkId);
...
)
{
  ...
  else
  {
    ...
    MIRROR_LIN_LOST_FRAME_CBK(Mirror_SourceNetworks[SrcNtwrkIdx].MirrorNetworkId);
...
)
{
  ...
  else
  {
    ...
    MIRROR_FLEXRAY_LOST_FRAME_CBK(Mirror_SourceNetworks[SrcNtwrkIdx].MirrorNetworkId);
