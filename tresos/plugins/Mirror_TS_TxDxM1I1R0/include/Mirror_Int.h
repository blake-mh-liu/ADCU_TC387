...
#define MIRROR_INVALID_MIRROR_ID 0xFFU
...
#define MIRROR_CHECK_CANID_TYPE(CanId) (((Can_IdType)(CanId &(Can_IdType) MIRROR_CANID_FD_MASK)) >>(Can_IdType)30U)
...
#define MIRROR_FLEXRAY_IS_AGGRIGATED_SYNTAXERROR_SET(Fr_ChannelStatus) ((uint8)0U != (Fr_ChannelStatus &(uint8)0x02) ? TRUE : FALSE)
...
#define MIRROR_FLEXRAY_IS_AGGRIGATED_CONTENTERROR_SET(Fr_ChannelStatus) ((uint8)0U != (Fr_ChannelStatus &(uint8)0x04) ? TRUE : FALSE)
...
#define MIRROR_FLEXRAY_IS_AGGRIGATED_BVIOLATION_SET(Fr_ChannelStatus) ((uint8)0U != (Fr_ChannelStatus &(uint8)0x10) ? TRUE : FALSE)
...
#define MIRROR_FLEXRAY_SET_AGGRIGATED_STATUS_CHANNEL_A(AggStatusByte) ((AggStatusByte) &=(uint8)(0x0F))
...
{\
  AggStatusByte &= (uint8)0xF0;\
  AggStatusByte = (AggStatusByte >>((uint8)4U));\
}
...
{\
  uint8 Temp = (AggStatusByte &(uint8)0x0F);\
  ((AggStatusByte) &= (uint8)(0xF0));\
  ((AggStatusByte) = (AggStatusByte >>(uint8)4U));\
  ((AggStatusByte) |= (uint8)(Temp));\
}
...
{\
  ((OldAggStatusByte) &= (uint8)(0xF0)); \
  ((OldAggStatusByte) |= ((AggStatusByte) &(uint8)0x0F)); \
}
...
{\
  ((OldAggStatusByte) &= (uint8)(0x0F)); \
  ((OldAggStatusByte) |= ((AggStatusByte) &(uint8)0xF0)); \
}
