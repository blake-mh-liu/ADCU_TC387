...
/* !LINKSTO Crc.ASR40.SWS_Crc_00022,1 */
...
/* !LINKSTO Crc.ASR40.SWS_Crc_00005,1 */
...
/* patch version check */
#if (CRC_SW_PATCH_VERSION < 20U)
#error CRC_SW_PATCH_VERSION wrong (< 20U)
#endif
...
#if (defined CRC_POLYNOMIAL16ARC)
#error CRC_POLYNOMIAL16ARC already defined
#endif
/** \brief definition of key width CRC_POLYNOMIAL16ARC polynomial [SWS_Crc_00067]
 **
 ** The polynomial 0x8005 is specified to be used.
 **
 ...
 ** polynomial of the polynomial 0x8005.
 **
...
#if (defined CRC_POLYNOMIAL32P4)
#error CRC_POLYNOMIAL32P4 already defined
#endif
/** \brief definition of key width CRC_POLYNOMIAL32P4 polynomial [SWS_Crc_00056]
 **
 ** The polynomial 0xFAACFB13 is specified to be used.
 **
 ** In that standard, the reflection of all input bytes is specified.
 ** We use an optimized algorithm where we do not reflect the input
 ** but the polynomial.
 ** So the polynomial 0xC8DF352F specified below is the reflected
 ** polynomial of the polynomial 0xFAACFB13.
 **
 ** See the "A Painless Guide to CRC Error Detection Algorithms",
 ** R. Williams, 1993. */
...
#if (defined CRC_INITIAL_VALUE16ARC)
#error CRC_INITIAL_VALUE16 already defined
#endif
/** \brief Definition of the initial value of crc16ARC */
...
#if (defined CRC_INITIAL_VALUE32P4)
#error CRC_INITIAL_VALUE32P4 already defined
#endif
/** \brief Definition of the initial value of crc32p4 */
#define CRC_INITIAL_VALUE32P4   0xFFFFFFFFU
...
#if (CRC_16_ARC_TABLE_SIZE > 0U) /* CRC16 generation via table */
...
/* Deviation MISRAC2012-1 */
STATIC CONST(uint16, CRC_CONST) Crc_Table16ARC[CRC_16_TABLE_SIZE] =
{
#if (CRC_16_TABLE_SIZE == 16U)
...
#endif /* CRC_16_ARC_TABLE_SIZE > 0U */
...
#if (CRC_32P4_TABLE_SIZE > 0U) /* CRC32 generation via table */
 ...
 * 0xF4ACFB13 */
 /* Deviation MISRAC2012-1 */
STATIC CONST(uint32, CRC_CONST) Crc_Table32P4[CRC_32P4_TABLE_SIZE] =
{
#if (CRC_32P4_TABLE_SIZE == 16U)
    0x00000000U, 0x2B2C2BEEU, 0x565857DCU, 0x7D747C32U, 0xACB0AFB8U,
    0x879C8456U, 0xFAE8F864U, 0xD1C4D38AU, 0xC8DF352FU, 0xE3F31EC1U,
    0x9E8762F3U, 0xB5AB491DU, 0x646F9A97U, 0x4F43B179U, 0x3237CD4BU,
    0x191BE6A5U,
...
)
{
  ...
  DBG_CRC_CALCULATECRC16ARC_ENTRY(Crc_DataPtr,Crc_Length,Crc_StartValue16,Crc_IsFirstCall);

  if (TRUE == Crc_IsFirstCall)
  {
    Crc_StartValue16 = CRC_INITIAL_VALUE16ARC;
  }

  /* Process all data (byte wise) */
  while (Crc_Length != 0U)
  {
#if (CRC_16_ARC_TABLE_SIZE == 16U) /* CRC16 generation with small table */

    /* Process low nibble of actual data */
    Crc_StartValue16
      = Crc_Table16ARC[0x0FU & (Crc_StartValue16 ^ *Crc_DataPtr)]
      ^ ((uint16)(Crc_StartValue16 >> 4U));

    /* Process high nibble of actual data */
    Crc_StartValue16
      = Crc_Table16ARC[0x0FU & (Crc_StartValue16 ^ ((uint16)*Crc_DataPtr >> 4U))]
      ^ ((uint16)(Crc_StartValue16 >> 4U));

#elif (CRC_16_ARC_TABLE_SIZE == 256U) /* CRC16 generation with large table */

    /* Process one byte of data */
    Crc_StartValue16
      = Crc_Table16ARC[((uint8)Crc_StartValue16 ) ^ *Crc_DataPtr]
      ^ ((uint16)(Crc_StartValue16 >> 8U));
    ...
    Crc_StartValue16 ^= *Crc_DataPtr;

    /* calculate crc bit by bit */
    for (i = 0U; i < 8U; ++i)
    {
      /* if lowest bit set to zero */
      if ((Crc_StartValue16 & 1U) == 0U)
      {
        /* no need to xor the zero bit with the polynomial, just shift */
        Crc_StartValue16 >>= 1U;
      }
      ...
      {
        /* bit was set to one: xor it with the CRC16ARC polynomial */
        Crc_StartValue16
          = ((uint16)(Crc_StartValue16 >> 1U)) ^ CRC_POLYNOMIAL16ARC;
      }
    }
  ...
  DBG_CRC_CALCULATECRC16ARC_EXIT(Crc_StartValue16,Crc_DataPtr,Crc_Length,Crc_StartValue16,Crc_IsFirstCall);
  return Crc_StartValue16;
}
...
)
{
  ...
  while (Crc_Length != 0U)
  {
    ...
    for (i = 0U; i < 8U; ++i)
    {
      /* Test value of the lowest bit.  Note that the CRC32 works on
...
)
{
  ...
  DBG_CRC_CALCULATECRC32P4_ENTRY(Crc_DataPtr ,Crc_Length, Crc_StartValue32, Crc_IsFirstCall);

  if (TRUE == Crc_IsFirstCall)
  {
    Crc_StartValue32 = CRC_INITIAL_VALUE32P4;
  }
  else
  {
    /* undo the XOR on the start value */
  ...
  while (Crc_Length != 0U)
  {
#if (CRC_32P4_TABLE_SIZE == 16U) /* CRC32P4 generation via small table */

    /* Process low nibble of actual data */
    Crc_StartValue32
      = Crc_Table32P4[0x0FU & (Crc_StartValue32 ^ *Crc_DataPtr)]
      ^ (Crc_StartValue32 >> 4U);
...
#elif (CRC_32P4_TABLE_SIZE == 256U) /* CRC32P4 generation via large table */

    /* Process one byte of data */
    Crc_StartValue32
      = Crc_Table32P4[((uint8)Crc_StartValue32) ^ *Crc_DataPtr]
      ^ (Crc_StartValue32 >> 8U);

#else /* CRC32P4 generation at runtime */
    ...
    for (i = 0U; i < 8U; ++i)
    {
      /* Test value of the lowest bit.  Note that the CRC32P4 works on
      ...
      {
        /* bit was set to one: xor it with the reflected CRC32P4
         * polynomial */
        Crc_StartValue32 = (Crc_StartValue32 >> 1U) ^ CRC_POLYNOMIAL32P4;
      }
    }
  ...
  DBG_CRC_CALCULATECRC32P4_EXIT(Crc_StartValue32, Crc_DataPtr, Crc_Length, Crc_StartValue32, Crc_IsFirstCall);
  return Crc_StartValue32;
}
