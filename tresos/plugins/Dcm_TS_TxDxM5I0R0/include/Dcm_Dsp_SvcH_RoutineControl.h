...
/* In case configuration contains routines, then the sub-services are called and they need to be
 * available. Also in case configuration does not have any routines, but default interface is
 * enabled the sub-service need to be available because sub-service interface will call
 * the corresponded default API. */
 ...
 **                                        WARNING: DCM does not control response length. It is the
 **                                        callout's responsibility not to overflow the buffer.
