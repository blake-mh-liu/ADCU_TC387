...
#if (defined DCM_DID_TYPE_AVAILABILITY)
/** \brief Did is an availability DID within data stream range. */
  #error "DCM_DID_TYPE_AVAILABILITY is already defined"
#endif /* (defined DCM_DID_TYPE_AVAILABILITY) */
#define DCM_DID_TYPE_AVAILABILITY ((uint8)9U)
...
/* !LINKSTO Dcm.Dsn.Type.Dcm_DspDidUsedType,1 */
/** \brief Type of a Dcm_DspDidUsed array */
typedef uint8 Dcm_DspDidUsedType;
#endif
...
#endif
  /** \brief TRUE if all the signals of the DID are synchronous */
  boolean IsDidSync;
} Dcm_DidConfigType;
...
/** \brief Checks if requested Did is available in configuration.
 **
 ** \param [in]    DidIdx  Requested Did.
 **
 ** \retval TRUE   DID is available.
 ** \retval FALSE  DID is not available.
 */
#if (DCM_DSP_DIDSERVICES_NUM_DIDS > 0U)
extern FUNC(boolean, DCM_CODE) Dcm_Dsp_DidServices_ValidateDidUsed(
 ...
 ** \param [in]  Did  Requested Did.
 **
 ** \retval TRUE   A matching Did is considered to be synchronous.
 ** \retval FALSE  A matching Did is considered to be asynchronous.
 */
extern FUNC(boolean, DCM_CODE) Dcm_Dsp_DidServices_IsDidSynchronous(
  uint16 Did);
...
#if (DCM_DSP_DIDSERVICES_NUM_DIDS > 0)
/** \brief Global array holding the availability configuration for the Dids */
/* !LINKSTO Dcm.Dsn.DidServices.ConfigurationData.DcmDspDidUsed,1 */
/* Deviation MISRAC2012-2 */
extern CONST(Dcm_DspDidUsedType, DCM_CONST_CALIBRATION)
  Dcm_DspDidUsed[DCM_DSP_DID_USED_BUF_SIZE];
#endif
...
#if (DCM_DID_AVAILABILITY_CALCULATION == STD_ON)
extern CONST(Dcm_DidConfigIdxType, DCM_CONST) Dcm_FirstObdDidConfigIndex;
extern CONST(Dcm_DidConfigIdxType, DCM_CONST) Dcm_FirstMonitorDidConfigIndex;
extern CONST(Dcm_DidConfigIdxType, DCM_CONST) Dcm_FirstInfoTypeDidConfigIndex;
#endif
#endif
