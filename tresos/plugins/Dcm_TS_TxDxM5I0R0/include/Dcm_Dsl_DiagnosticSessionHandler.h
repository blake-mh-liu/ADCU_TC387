 ...
 *  \param[in] isStartedByReception Identifies this call as a result of the reception of a request (internal or external).
 *                                  Timers will not be decremented by the first Dcm_MainFunction() cycle if they have been
 *                                  just started during this same Dcm_MainFunction() cycle. This indicates to the
 *                                  Dcm_Dsl_DiagnosticSessionHandler_P2TimerHandlingMainFunction() to skip a decrement and
 *                                  only begin decrementing in the next Dcm_MainFunction() cycle.
 *  \return void
 */
...
/** \brief This function announces the diagnostic session handler that it should not decrement any P2 timer started
 *         during this same Dcm_MainFunction() call.
 *  \return void
 */
extern FUNC(void, DCM_CODE) Dcm_Dsl_DiagnosticSessionHandler_StartOfMainFunctionCycle(void);
