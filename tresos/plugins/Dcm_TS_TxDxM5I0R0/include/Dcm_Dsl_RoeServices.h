 ...
 ** \Synchronicity{Synchronous}
 **/
extern FUNC(void, DCM_CODE) Dcm_Dsl_RoeServices_TriggerOnDTCStatus(uint8 DTCStatusOld, uint8 DTCStatusNew);
 ...
 **
 ** \param[inout] MsgLen      The number of data bytes that can be written (as input
 **                           value) and is written (as output value) to the message
 **                           buffer.
 **
 ** \return Std_ReturnType
 ** \retval DCM_E_OK          The request is processed in the regular way.
 ** \retval DCM_E_ABORT       Response does not fit to the output message buffer.
 **
 ** \Reentrancy{Non-Reentrant}
 ** \Synchronicity{Synchronous}
 **/
extern FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsl_RoeServices_ReportActivatedEvents(
