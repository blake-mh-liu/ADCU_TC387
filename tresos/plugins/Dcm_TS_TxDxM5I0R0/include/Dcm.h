 ...
 *
 *    Reason:
 *    The function declaration is generated in SchM_DCM.h and is protected for re-declaration
 *    through macro guarding. Astree does not take into consideration this protection.
 *
 */
...
#endif /* DCM_PDUR_INTERFACE_AUTOSAR_R21_11 */
...
/** \brief Sets a new role in deauthenticated state.
 **
 ** This function sets a new role used in deauthenticated state for
 ** a connection. The set role is valid until the connection switches into
 ** authenticated state or the ECU is reset.
 **
 ** \param[in] connectionId  Unique connection identifier identifiying the
 ** connection for which a deauthenticated roles is set
 ** \param[in] deauthenticatedRole  New deauthenticated role that is assigned
 ** to that connection
 **
 ** \return
 **         - ::E_OK - Always returned
 **
 ** \ServiceID{::DCM_SVCID_SETDEAUTHENTICATEDROLE}
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
...
/** \brief Sets the authenticated state to DCM_DEAUTHENTICATED.
 **
 ** This function sets the authenticated state to DCM_DEAUTHENTICATED for
 ** a connection.
 **
 ** \param[in] connectionId  Unique connection identifier identifiying the
 ** connection for which a deauthenticated state is set
 **
 ** \return
 **         - ::E_OK - - Always returned
 **
 ** \ServiceID{::DCM_SVCID_RESETAUTHENTICATION}
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
extern FUNC(Std_ReturnType, DCM_CODE) Dcm_ResetAuthentication(
  uint16 connectionId
);
...
#if (DCM_ENABLE_R22CALLOUT_REQUESTDOWNLOAD == STD_ON)
/* !LINKSTO Dcm.EB.ProcessRequestDownload.AUTOSAR.R2211,2 */
 ...
 ** \param[in]     MemorySize Uncompressed size of memory to be downloaded
...
#elif (DCM_DATATRANSFER_SERVICES_ASR_VERSION == DATA_TRANSFER_SERVICES_AUTOSAR_403)
/* !LINKSTO Dcm.EB.ProcessRequestDownload.AUTOSAR403,3 */
/** \brief Callout for RequestDownload
 **
 **  This callout allows the user to implement the negotiation of a data transfer
 ** from the tester to the ECU.
...
/* !LINKSTO Dcm.EB.ProcessRequestDownload.AUTOSAR430,4 */
/** \brief Callout for RequestDownload
 **
 **  This callout allows the user to implement the negotiation of a data transfer
 ** from the tester to the ECU.
...
extern FUNC(Std_ReturnType, DCM_APPL_CODE) Dcm_ProcessRequestDownload(
  ...
  P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode);
#endif /*(DCM_ENABLE_R22CALLOUT_REQUESTDOWNLOAD == STD_ON)*/
...
/* !LINKSTO Dcm.EB.RequestTransferExit.AUTOSAR403,3 */
...
/* !LINKSTO Dcm.EB.RequestTransferExit.AUTOSAR422,3 */
/** \brief Callout for termination of an upload or download process.
 **
 ** This callout allows the user to terminate the data transfer between the ECU and the tester.
 **
 ...
 ** \param[in,out] transferResponseParameterRecordSize When the function is called this parameter
...
#elif (DCM_DATATRANSFER_SERVICES_ASR_VERSION == DATA_TRANSFER_SERVICES_AUTOSAR_430)

/* !LINKSTO Dcm.EB.RequestTransferExit.AUTOSAR430,3 */
/** \brief Callout for termination of an upload or download process.
 **
 ** This callout allows the user to terminate the data transfer between the ECU and the tester.
 **
...
/** \brief Callout for storing the programming conditions.
 **
 ...
 ** \Synchronicity{Asynchronous}
 */
/* !LINKSTO Dcm.EB.ServiceProcessing.ASR403.SetProgConditions, 1 */
 ...
 ** \param[in]  OpStatus       Operation Status: DCM_INITIAL, DCM_PENDING, DCM_CANCEL, DCM_FORCE_RCRRP_OK
 **
 ** \return - E_OK:              Conditions have correctly been set
 **         - E_NOT_OK:          Conditions cannot be set
 **         - DCM_E_PENDING:     Conditions set is in progress, a further call to this API is needed to end the setting
 ...
 ** \Synchronicity{Asynchronous}
 */
/* !LINKSTO Dcm.EB.ServiceProcessing.ASR22-11.SetProgConditions, 1 */
extern FUNC(Std_ReturnType, DCM_CODE) Dcm_SetProgConditions(
  Dcm_OpStatusType OpStatus,
 ...
 ** \Synchronicity{Synchronous}
 */
/* !LINKSTO Dcm.EB.ServiceProcessing.ASR403.GetProgConditions, 1 */
/* !LINKSTO Dcm.EB.ServiceProcessing.ASR22-11.GetProgConditions, 1 */
