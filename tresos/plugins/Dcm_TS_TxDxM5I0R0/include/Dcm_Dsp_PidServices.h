...
/** \brief Function to lock the PidServices when they need to be used exclusively by one user
 **
 ** \retval DCM_E_OK                Service successfully locked
 ** \retval DCM_E_RESOURCE_LOCKED   Service locked and not available
 **/
extern FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_PidServices_LockService(void);

/** \brief Function to unlock the PidServices after they were locked
 **/
extern FUNC(void, DCM_CODE) Dcm_Dsp_PidServices_UnlockService(void);
...
extern CONST(uint8,DCM_CONST_CALIBRATION) Dcm_DspPidUsed[DCM_DSP_PID_USED_BUF_SIZE];
...
/* Deviation MISRAC2012-1 */
extern CONST(Dcm_DspPidConfigType,DCM_CONST_CALIBRATION) Dcm_DspPidConfig[DCM_NUM_PID_CFG];
