...
FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_VinServices_GetAvailabilityInfoTypeId(
  ...
  P2VAR(uint8, AUTOMATIC, DCM_VAR) AvailabilityData)
{
  ...
  DBG_DCM_DSP_VINSERVICES_GETAVAILABILITYINFOTYPEID_ENTRY(InfoTypeID, AvailabilityData);
  ...
  for (VehInfoIndex = 0U; VehInfoIndex < DCM_DSP_VEHINFO_NUM; VehInfoIndex++)
  {
    if (DCM_IS_VEHINFOUSED_IDX_SET(VehInfoIndex))
    {
      ...
      if (Dcm_OBDSupport_UpdateAvailableInfoWithID(
            currentVehInfoInfoType, InfoTypeID, AvailabilityData) == DCM_E_OK)
      {
        RetVal = DCM_E_OK;
      }
    }
...
#else
  TS_PARAM_UNUSED(InfoTypeID);
  TS_PARAM_UNUSED(AvailabilityData);
#endif /* (DCM_DSP_VEHINFO_NUM > 0) */

  DBG_DCM_DSP_VINSERVICES_GETAVAILABILITYINFOTYPEID_EXIT(RetVal, InfoTypeID, AvailabilityData);
