...
/* The functions provided and implemented by this unit are accessed through OBD service 0x08 handling
 *  or UDS service 0x31 when an OBD Routine ID is requested and OBD mirroring is enabled. */
...
#if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON)
/** \brief Mutex controlling access to the access to RequestControl() */
STATIC VAR(Dcm_MutexType, DCM_VAR ) Dcm_Dsp_RequestControl_Lock = DCM_MUTEX_UNLOCKED;
#endif
...
#if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON)
/** \brief Function to lock the access to RequestControl() when they need to be used exclusively by one user
 **
 ** \retval DCM_E_OK                Service successfully locked
 ** \retval DCM_E_RESOURCE_LOCKED   Service locked and not available
 **/
STATIC FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_RequestControl_LockServices(void);

/** \brief Function to unlock the access to RequestControl() after they were locked
 **/
STATIC FUNC(void, DCM_CODE) Dcm_Dsp_RequestControl_UnlockServices(void);
#endif /* #if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON) */
  ...
  P2VAR(uint8, AUTOMATIC, DCM_VAR) availabilityData)
{
  ...
  {
    /* Only consider TIDs that are active */
    if (Dcm_DspRequestControlUsedConfig[idx] == TRUE)
    {
      ...
      /* !LINKSTO Dcm.ISO-15031-5_8.8.2_2,1 */
      const Std_ReturnType UpdateAvailableInfoWithIDRetVal =
          Dcm_OBDSupport_UpdateAvailableInfoWithID(Dcm_DspRequestControlConfig[idx].TestId, ReqAvailTid, availabilityData);
  ...
  P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) Nrc)
{
  ...
  for (idx=0U; idx<DCM_DSP_NUM_REQUESTCONTROL_CFG; idx++)
  {
    if (Dcm_DspRequestControlConfig[idx].TestId == InBuffer[0U])
    {
      ...
      {
#if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON)
        /* If access to Xxx_RequestControl() needs to be protected */
        const Std_ReturnType LockStatus = Dcm_Dsp_RequestControl_LockServices();
...
#endif /* #if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON) */
        {
          ...
          retVal = Dcm_DspRequestControlConfig[idx].RequestControlFnc(&(OutBuffer[1U]), &(InBuffer[1U]));

#if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON)
          /* Unlock ASAP */
          Dcm_Dsp_RequestControl_UnlockServices();
#endif /* #if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON) */
      ...
      else
      {
        ...
        /* !LINKSTO Dcm.EB.RoutineControl.DcmDspEnableObdMirrorEnabled.ConfiguredOBDRID.UnusedOBDRID.ConfiguredNonAvailabityTID.DisabledByCalibrationNonAvailabityTID.TransmitNRC0x31,2 */
        /* !LINKSTO Dcm.EB.RoutineControl.DcmDspEnableObdMirrorEnabled.UnconfiguredOBDRID.ConfiguredNonAvailabityTID.DisabledByCalibrationNonAvailabityTID.TransmitNRC0x31,2 */
  ...
  P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) Nrc)
{
  ...
  *ResponseLength = ResponsePos;
  ...
  if (ResponsePos == 0U)
  {
    /* The requestOutOfRange(0x31) nrc can be transmitted the following scenarios:
     * 1) When the buffer is not big enough, still the NRC RESPONSETOOLONG is not transmitted
     * in the UDS case, due to the fact that the minimum configured response buffer size is 8 bytes
     * and the response for an availability RID via RoutineControl(0x31) service
     * cannot exceed 8 bytes.
     * (1byte serviceId + 1 byte subServiceId + 2 bytes routineId + 4 bytes availabilityBitMap)
     *
     * 2) When configuration parameter DcmDspRidAvailabilityCalculation is set to True the 0x31
     * (requestOutOfRange) NRC is suppressed from the RequestControlOfOnBoardSysTstComp unit
     * and sent from the RoutineControl unit if no RIDs are supported in the requested
     * availability range.
     *
     * 3) When configuration parameter DcmDspRidAvailabilityCalculation is set to False 0x31
     * (requestOutOfRange) NRC is sent from the RequestControlOfOnBoardSysTstComp unit if no TIDs
     * are supported in the requested availability range. */
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  else
  {
    ...
    else if (DCM_E_BUSYREPEATREQUEST == Nrc)
    {


        /* !LINKSTO Dcm.EB.RequestControlOfOnBoardSysTstComp.RoutineControlAlreadyRunningOnDifferentStack.BRREnabled.TransmitNRC0x21,1 */
        /* !LINKSTO Dcm.EB.RequestControlOfOnBoardSysTstComp.RequestControlOfOnBoardSysTstCompAlreadyRunningOnDifferentStack.BRREnabled.TransmitNRC0x21,1 */
...
#else
      /* !LINKSTO Dcm.EB.RequestControlOfOnBoardSysTstComp.RoutineControlAlreadyRunningOnDifferentStack.BRRDisabled.Discard,1 */
      /* !LINKSTO Dcm.EB.RequestControlOfOnBoardSysTstComp.RequestControlOfOnBoardSysTstCompAlreadyRunningOnDifferentStack.BRRDisabled.Discard,1 */
      /* no response */
...
STATIC FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_RequestControl_LockServices(void)
{
  ...
  LockStatus = Dcm_Internal_GetLock(&Dcm_Dsp_RequestControl_Lock);
...
STATIC FUNC(void, DCM_CODE) Dcm_Dsp_RequestControl_UnlockServices(void)
{
  ...
  Dcm_Internal_ReleaseLock(&Dcm_Dsp_RequestControl_Lock);

}
#endif /* #if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON) */

FUNC(Std_ReturnType, DCM_CODE) Dcm_DspInternal_ReqCtrlViaRoutineCtrl_UDSShell(
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  Std_ReturnType retVal = E_NOT_OK;
  ...
  else
  {
#if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON)
#if (DCM_RESPOND_WITH_BUSYREPEATREQUEST == STD_ON)
    /* !LINKSTO Dcm.EB.RoutineControl.RequestControlOfOnBoardSysTstCompAlreadyRunningOnDifferentStack.BRREnabled.TransmitNRC0x21,1 */
    /* set negative response code */
#else
    if (DCM_E_BUSYREPEATREQUEST == Nrc)
    {
      /* !LINKSTO Dcm.EB.RoutineControl.RequestControlOfOnBoardSysTstCompAlreadyRunningOnDifferentStack.BRRDisabled.Discard,1 */
      /* no response */
      pMsgContext->msgAddInfo.suppressPosResponse = TRUE;
    }
    else
...
#endif /* #if (DCM_DSP_REQUESTCONTROL_USE_LOCKING == STD_ON) */
    {
      ...
      /* In case an availability routine is requested and requestOutOfRange (0x31) NRC is set,
       * the NRC shall not be send from the OBD sub-system, in order to allow checking if there is
       * also any routine identifier configured within the OBDonUDS range (0xE000 - 0xE1FF), in
       * case no routine is found in the requested availability range, a requestOutOfRange (0x31)
       * NRC shall be sent from the UDS system.
      */
      if ((DCM_IS_AVAILABILITY_OBDSUBID(pMsgContext->reqData[2U])) == FALSE)
      ...
      if(retVal != E_NOT_OK)
      {
        ...
        DCM_DET_REPORT_ERROR(DCM_SVCID_REQCTRLVIAROUTINECTRL_UDSSHELL, DCM_E_INTERFACE_RETURN_VALUE);
      }
