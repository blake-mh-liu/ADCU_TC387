                                            ...
                                            (Dcm_MsgLenType)DCM_ADDRESSANDLENGTHFORMATID_SIZE))
          {
            ...
            if(retVal == DCM_E_OK)
            {
              ...
              else
              {
                ...
                const uint32 RequiredBufSize = (uint32)Dcm_WriteMemAddLength +
                                               (uint32)Dcm_WriteMemSizeLength +
                                               (uint32)DCM_ADDRESSANDLENGTHFORMATID_SIZE;

                /* Check if adequate buffer is configured to write the data (RequiredBufSize + 1 byte for service ID) */
                if(RequiredBufSize < (uint32)(DCM_HSM_INFO(InstIdx).TxBuffer.Size))
                {
                  Dcm_Dsp_MemoryServices_WriteMemory_Setup(
                    ...
                    writeMemAddress,
                    writeMemSize,
                    &(pMsgContext->reqData[Dcm_WriteMemAddLength + Dcm_WriteMemSizeLength + DCM_ADDRESSANDLENGTHFORMATID_SIZE]));
                ...
                else
                {
                  ...
                  /* !LINKSTO Dcm.WriteMemoryByAddress.InterfaceBufferOverflow.ReportDETError,1 */
            ...
            {
              /* !LINKSTO Dcm.EB.0x3D.AddressAndLengthFormatIdentifier.OutOfRangeAdressFormat,1 */
              /* !LINKSTO Dcm.EB.0x3D.AddressAndLengthFormatIdentifier.OutOfRangeLengthFormat,1 */
              Dcm_ExternalSetNegResponse(pMsgContext, DCM_E_REQUESTOUTOFRANGE);
              enableResponseCheck = FALSE;
              serviceProcessStatus = DCM_E_DONE;
            }
