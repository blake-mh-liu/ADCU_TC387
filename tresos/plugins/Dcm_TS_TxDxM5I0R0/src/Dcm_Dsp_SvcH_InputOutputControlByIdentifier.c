...
/** \brief This function determines the access interface to be used.
 **
 ** \param[in]    DidData AccessInterface defined in configuration
 ** \retval       DidData AccessInterface calculated
 */
STATIC FUNC(Dcm_DidDataAccesType, DCM_CODE)
  Dcm_Dsp_InputOutputControlByIdentifier_ReturnControlToECU_DetermineAccessType(Dcm_DidDataAccesType InAccessTypeToUse);
#endif /* #if (DCM_DEFAULT_ASR_SERVICE_API != DCM_SERVICE_API_ASR40) */
...
#if (DCM_DID_SIGNALS_COUNT > 0U)
/** \brief This function handles the configured ReturnControlToECU callout functions
           for all configured Signals.
 **
 ** \param[in] curNrc        Negative response code variable
 ** \param[in] dataCfg       Holds the configuration of signal being processed
 ** \param[in] AccessInfoExt Holds the access information of signal being processed
 **
 ** \retval    E_OK          Request was successful.
 */
STATIC FUNC(Std_ReturnType, DCM_CODE)
  Dcm_Dsp_InputOutputControlByIdentifier_ConfiguredDID_CallReturnControlToEcuSyncUnion(
...
#endif /* (DCM_REQUEST_SUPPLIER_DEFAULT_DID_READ == STD_ON) */
    {
      ...
      if(ReturnVal == DCM_E_OK)
      {
        ...
        if ( (((pMsgContext->reqData[2U] == DCM_RETURNCONTROLTOECU) &&
             ...
             (Dcm_DidConfig[DidInfoContext.DidInfoContext.ConfigIdx].NumSignals != 0x00U)
           )
        {
  ...
  Dcm_OpStatusType OpStatus)
{
  ...
  if(FALSE == Dcm_IoControlDefaultStatus.IsDefaultDidReadReturnPending)
  {
    ...
    ServiceProcessStatus = Dcm_DefaultDIDShortTermAdjustment(
                            ...
                            (uint8*)(&pMsgContext->reqData[DCM_DID_ID_SIZE + 0x01U]),
                            /* Length = Total Request Size - (Did Id Size + Sub-Function Size). */
                            (pMsgContext->reqDataLen - (DCM_DID_ID_SIZE + 0x01U)),
                            &Nrc);
...
STATIC FUNC(Dcm_DidDataAccesType, DCM_CODE)
  Dcm_Dsp_InputOutputControlByIdentifier_ReturnControlToECU_DetermineAccessType(Dcm_DidDataAccesType InAccessTypeToUse)
{
  Dcm_DidDataAccesType RetAccessTypeToUse;

  RetAccessTypeToUse = InAccessTypeToUse;

  /* !LINKSTO Dcm.ReturnControlToECU.AsyncCS.ASR42,1 */
  /* !LINKSTO Dcm.ReturnControlToECU.AsyncFnc.ASR42,1 */

#if (DCM_INCLUDE_RTE == STD_ON)
    if (InAccessTypeToUse == DCM_USE_DATA_ASYNCH_CLIENT_SERVER)
    {
      RetAccessTypeToUse = DCM_USE_DATA_SYNCH_CLIENT_SERVER;
    }
#endif /* DCM_INCLUDE_RTE == STD_ON */

    if (InAccessTypeToUse == DCM_USE_DATA_ASYNCH_FNC)
    {
      RetAccessTypeToUse = DCM_USE_DATA_SYNCH_FNC;
    }

  return RetAccessTypeToUse;
    ...
    P2CONST(Dcm_DidIoctlAccessInfoExtType, AUTOMATIC, DCM_CONST) AccessInfoExt)
{
...
#if (DCM_CONTROL_MASK_EXTERNAL_ENABLED == STD_OFF)
  TS_PARAM_UNUSED(AccessInfoExt);
#endif /* #if (DCM_CONTROL_MASK_EXTERNAL_ENABLED == STD_OFF) */
  ...
  RetVal = Dcm_Dsp_ReturnControlToEcuWithAllOnesMask(dataCfg,
                                                     &curNrc,
                                                     AccessInfoExt->DidControlMask,
                                                     AccessInfoExt->DidControlMaskSize);
#else /* #if (DCM_CONTROL_MASK_EXTERNAL_ENABLED == STD_ON) */
  RetVal = Dcm_Dsp_ReturnControlToEcuWithAllOnesMask(dataCfg,
                                                     &curNrc);
#endif
...
/* Check if the corresponding Did is available */
    ...
    boolean DidFound = Dcm_Dsp_DidServices_ValidateDidUsed(Dcm_ReturnControlToEcuContext.configIdx);
#else
    boolean DidFound = Dcm_Dsp_DidServices_ValidateDidUsed((uint8)Dcm_ReturnControlToEcuContext.configIdx);
#endif
    if ((AccessInfoIdx != DCM_DIDIOCTLACCESSINFOIDX_INVALID) && (DidFound == TRUE))
    {
      P2CONST(Dcm_DidIoctlAccessInfoExtType, AUTOMATIC, DCM_CONST) AccessInfoExt =
...
#if(DCM_DEFAULT_ASR_SERVICE_API == DCM_SERVICE_API_ASR40)
          /* !LINKSTO Dcm.ReturnControlToECU.AsyncCS.ASR40,3 */
...
#endif /* #if(DCM_DEFAULT_ASR_SERVICE_API == DCM_SERVICE_API_ASR40) */
          {
            ...
            if( (dataCfg->AccessInterface == DCM_USE_DATA_SYNCH_FNC) ||
                ...
                (((dataCfg->AccessInterface == DCM_USE_DATA_ASYNCH_FNC) ||
                  (dataCfg->AccessInterface == DCM_USE_DATA_ASYNCH_CLIENT_SERVER)) &&
                 (DCM_DEFAULT_ASR_SERVICE_API != DCM_SERVICE_API_ASR40) )
              )
            {
              RetVal = Dcm_Dsp_InputOutputControlByIdentifier_ConfiguredDID_CallReturnControlToEcuSyncUnion(
                        curNrc,
                        dataCfg,
                        AccessInfoExt);
            }
  ...
  Dcm_OpStatusType OpStatus)
{
  ...
  Dcm_DidDataAccesType AccessTypeToUse;
         ...
         (pIOControlDIDContext->RequestProcessingSt != DCM_DID_READ_PENDING))
  {
    ...
    /* !LINKSTO Dcm.ReturnControlToECU.AsyncFnc.ASR40,1 */
#if (DCM_DEFAULT_ASR_SERVICE_API != DCM_SERVICE_API_ASR40)
    /*Determine the type of access interface to be used for access control */
    AccessTypeToUse = Dcm_Dsp_InputOutputControlByIdentifier_ReturnControlToECU_DetermineAccessType(dataCfg->AccessInterface);
#else
    AccessTypeToUse = dataCfg->AccessInterface;
#endif /* #if (DCM_DEFAULT_ASR_SERVICE_API != DCM_SERVICE_API_ASR40) */

    /* The interface type of the currently processed signal data */
    switch (AccessTypeToUse)
    {
#if (DCM_INCLUDE_RTE == STD_ON)
...
#if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON)
      case DCM_USE_BLOCK_ID:
        /* !LINKSTO Dcm.EB.InputOutputControlByIdentifier.ReturnControlToECU.UseBlockId.NRC,1 */
        nrc = DCM_E_REQUESTOUTOFRANGE;
        break;
#endif /* #if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON) */
...
#if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON)
      case DCM_USE_BLOCK_ID:
        /* !LINKSTO Dcm.EB.InputOutputControlByIdentifier.ResetToDefault.UseBlockId.NRC,1 */
        nrc = DCM_E_REQUESTOUTOFRANGE;
        break;
#endif /* #if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON) */
...
#if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON)
      case DCM_USE_BLOCK_ID:
        /* !LINKSTO Dcm.EB.InputOutputControlByIdentifier.FreezeCurrentState.UseBlockId.NRC,1 */
        nrc = DCM_E_REQUESTOUTOFRANGE;
        break;
#endif /* #if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON) */
  ...
  Dcm_OpStatusType OpStatus)
{
  ...
  DidDataLength = (uint16)DCM_GET_BYTES(LastSignalCfg->DataPos + LastSignaldataCfg->DataLength);


#if (DCM_RW_DID_BLOCK_ID_CONFIGURED == STD_ON)
  /* Check if the signal buffer is available */
  if (pDidSignalBufferContext->NvMService == DCM_DIDNVMOPERATION_NONE)
  {
#endif /* if (DCM_RW_DID_BLOCK_ID_CONFIGURED == STD_ON) */
    /* Dcm581: In case of more than one supported I/O signal per DataIdentifier, the DCM shall
       internally consider the parameter controlEnableMaskRecord and control only the
       included signals in the request message */
    while (   (pIOControlDIDContext->SignalInd < numSignals)
     ...
     */
      ...
      if (numSignals > 1U)
      {
        ...
        /* Convert to octets */
        signalLength = (uint16)DCM_GET_BYTES(signalLength);
      }
      else
      {
#if (DCM_CONTROL_MASK_EXTERNAL_ENABLED == STD_ON)
...
#if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON)
        case DCM_USE_BLOCK_ID:
          /* !LINKSTO Dcm.EB.InputOutputControlByIdentifier.ShortTermAdjustment.UseBlockId.NRC,1 */
          nrc = DCM_E_REQUESTOUTOFRANGE;
          break;
#endif /* #if (DCM_CONTROL_DID_BLOCK_ID_CONFIGURED == STD_ON) */
...
#endif /* #if (DCM_CONTROL_MASK_EXTERNAL_ENABLED == STD_ON) */
{
  ...
  if (DidControlMask == DCM_CONTROLMASK_EXTERNAL)
  {
    if(pMsgContext->reqData[2U] == DCM_SHORTTERMADJUSTMENT)
    {
      ...
      Dcm_MsgLenType MaxLength = (Dcm_MsgLenType)DCM_GET_BYTES((Dcm_MsgLenType)SignalCfg->DataPos + (Dcm_MsgLenType)dataCfg->DataLength);
      Dcm_MsgLenType MinLength = (Dcm_MsgLenType)DCM_GET_BYTES((Dcm_MsgLenType)SignalCfg->DataPos);
  ...
  */
  {
    ...
    if (numSignals > 1U)
    {
      ControlMaskLength = (uint16)DCM_GET_BYTES(numSignals);
    }
    ...
    if(pMsgContext->reqData[2U] == DCM_SHORTTERMADJUSTMENT)
    {
      DidSignalsDataLength = (uint32)DCM_GET_BYTES((uint32)(SignalCfg->DataPos) + (uint32)(dataCfg->DataLength));
    }
    ...
    {
      uint32 MinDidSignalDataLength = (uint32)DCM_GET_BYTES((uint32)(SignalCfg->DataPos));
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  /* Deviation TASKING-2 <+5> */
  /* Deviation TASKING-3 <+4> */
