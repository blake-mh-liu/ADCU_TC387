...
/** \brief This variable holds if the ReadScalingDataByIdentifier service has locked
 **         the DidServices unit.
 **  DCM_TRYING_LOCK       - ReadScalingDataByIdentifier service has the lock over DidServices unit.
 **  DCM_EXECUTING_SERVICE - ReadScalingDataByIdentifier does not have the lock over DidServices unit.
 */
STATIC VAR(uint8, DCM_VAR) Dcm_ReadScalingDataByIdentifier_LockStatus = DCM_TRYING_LOCK;
...
/* Initialize the internal data of the ReadScalingDataByIdentifier service */
FUNC(void, DCM_CODE) Dcm_Dsp_ReadScalingDataByIdentifier_Init(void)
{
  DBG_DCM_DSP_READSCALINGDATABYIDENTIFIER_INIT_ENTRY();

#if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_ON)
  Dcm_ReadScalingDataByIdentifier_LockStatus = DCM_TRYING_LOCK;
#endif /* #if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_ON) */

  DBG_DCM_DSP_READSCALINGDATABYIDENTIFIER_INIT_EXIT();
}
...
#if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_ON)
  switch(Dcm_ReadScalingDataByIdentifier_LockStatus)
  {
    case DCM_TRYING_LOCK:
    {
         ...
         (OpStatus != DCM_CONFIRMED_NOK))
      {
        ...
        else
        {
          ...
          Dcm_ReadScalingDataByIdentifier_LockStatus = DCM_EXECUTING_SERVICE;
...
#else /* if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_ON) */
