  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  else
  {
    ...
    if(DataTranContextTypePtr->TransState == DCM_TRANSFER_UPLOAD)
    {
      /* For the upload request the MemTransfered is updated before calling the Dcm_ReadMemory() for
         each TransferData request with a valid blockSequence counter, in order obtain the
         uploaded data size at this point, CurrMemTransfered shall also be considered */
      if (
          ((DataTranContextTypePtr->MemTransfered + DataTranContextTypePtr->CurrMemTransfered) == DataTranContextTypePtr->MemSize) &&
    ...
    /* !LINKSTO Dcm.EB.TransferData.Upload.StartBlockSequenceCounter,1 */
    else if (((DataTranContextTypePtr->CurrMemTransfered == 0U) && (BlockSeqCounter != 1U)) ||
             /* The request with repeated block sequence count is also supported.
                ...
                equal to the next value of internal count */
            ((DataTranContextTypePtr->BlockSequenceCounter > 0U) &&
             (BlockSeqCounter != DataTranContextTypePtr->BlockSequenceCounter) &&
             (BlockSeqCounter != (DataTranContextTypePtr->BlockSequenceCounter - 1U))) ||
             /* After block sequence counter reset for the next transferRequest
                the block sequence counter should be zero */
             ((DataTranContextTypePtr->BlockSequenceCounter == 0U) &&
  ...
  Dcm_OpStatusType OpStatus)
{
  ...
  if (OpStatus == DCM_INITIAL)
  {
    ...
    /* If the request has a new sequence counter, consider the previous transfer acknowledged */
