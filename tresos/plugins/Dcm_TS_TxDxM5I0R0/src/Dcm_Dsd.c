...
/** \brief Used to search in the current service to check subservice used status
 **
 ** \param[in] ServiceTableIndex Current service table Id
 ** \param[in] sidIdxExt Current service Id
 ** \param[in] curSidConfig Current ServiceId config
 ** \param[in] subSidId Subservice Id to be found
 ** \param[in] subSidIdxExt The sub-service Id
 **
 ** \retval FALSE    SubserviceUsed status is set to FALSE
 ** \retval TRUE     SubserviceUsed status is set to TRUE
 */
STATIC FUNC(boolean, DCM_CODE) Dcm_DsdInternal_GetSubServiceUsedStatus(
  uint8 ServiceTableIndex,
  uint8 sidIdxExt,
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) curSidConfig,
  uint8 subSidId,
  P2VAR(uint8, AUTOMATIC, DCM_VAR) subSidIdxExt);
#endif /* #if (DCM_DSP_SUBSERVICE_USED_BUF_SIZE > 0) */
...
#if (DCM_AUTHENTICATION_CONNECTION == STD_ON)
/** \brief Used to check authentication permission for service
 ** functions
 **
 ** \param[in] InstIdx    Current HSM instance Id
 ** \param[in] SidConfig  Current ServiceId config
 **
 ** \retval E_OK                          Authentication permission for service is granted
 ** \retval DCM_E_AUTHENTICATIONREQUIRED  Authentication permission for service is not granted.
 */
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_Dsd_AuthenticationCheckOnServiceLevel(
  uint8 InstIdx,
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) SidConfig);

/** \brief Used to check authentication permission for sub-service
 ** functions
 **
 ** \param[in] InstIdx       Current HSM instance Id
 ** \param[in] SidConfig     Current ServiceId config
 ** \param[in] SubSidIdxExt  Current Sub-service Id
 **
 ** \retval E_OK                          Authentication permission for sub-service is granted
 ** \retval DCM_E_AUTHENTICATIONREQUIRED  Authentication permission for sub-service is not granted.
 */
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_Dsd_AuthenticationCheckOnSubServiceLevel(
  uint8 InstIdx,
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) SidConfig,
  uint8 SubSidIdxExt);
#endif /* #if (DCM_AUTHENTICATION_CONNECTION == STD_ON) */

/** \brief Used to check security level permission for service
 ** functions
 **
 ** \param[in] SidConfig     Current ServiceId config
 **
 ** \retval E_OK                        Security permission for service is granted
 ** \retval DCM_E_SECURITYACCESSDENIED  Security permission for service is not granted.
 */
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_Dsd_SecurityLevelCheckOnService(
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) SidConfig);
  ...
  DCM_PDL_DISPATCH_SERVICE_REQ(uint8 instIdx))
{
  ...
  boolean ServiceExecutionAllowed;
  ...
  */
...
#if (DCM_NUM_UDS_STACKS > 0)
  ServiceExecutionAllowed = Dcm_Dsp_ServiceExecutionAllowed(
      (Dcm_ProtocolIndexType)(DCM_CUR_INST_ID(instIdx)),
      (Dcm_ServiceIDType)(curHsmInfo->serviceId),
      (Dcm_RequestTypeType)(curHsmInfo->RequestType));
#endif /* #if (DCM_NUM_UDS_STACKS > 0) */

  /* check if Tx buffer is available in order to process this request
   * and mark it as 'in use' */
  if (
      (TxBufferAllocationResult != BUFREQ_OK)
#if (DCM_NUM_UDS_STACKS > 0)
  /* and reject second request with same service Id as it is pointed for reduced functionality
   * of parallel UDS to UDS request processing */
      ||
  ...
  /* if all preconditions are ok fulfilled then continue process */
    ...
    /* !LINKSTO Dcm.EB.UDSServices.ReadGenericInformation.AcceptedForDispatching.ReadGenericInformationAlreadyRunningOnDifferentStack.NotSameDemClient.BRREnabled.TransmitBRR,1 */
...
#endif
    {
      ...
      /* !LINKSTO Dcm.EB.UDSServices.ReadGenericInformation.AcceptedForDispatching.ReadGenericInformationAlreadyRunningOnDifferentStack.NotSameDemClient.BRRDisabled.NoRCRRPSent.Discard,1 */
    ...
    else
    {
      ...
      /* !LINKSTO Dcm.EB.UDSServices.ReadGenericInformation.AcceptedForDispatching.ReadGenericInformationAlreadyRunningOnDifferentStack.NotSameDemClient.BRRDisabled.RCRRPSent.TransmitGR,1 */
...
#endif /* #if (DCM_REQUEST_MANUFACTURER_NOTIFICATION_ENABLED == STD_ON ) */
    {
      ...
      if (DCM_E_OK != curNrc)
      {
        ...
        Dcm_SEVReportingServices_IdsMSetSecurityEvent_NRC(
                                            ...
                                            curHsmInfo->negativeResponseCode);
#endif /* #if (DCM_SECURITY_EVENT_REPORTING == STD_ON) */
      }
    }
  }
...
FUNC(void, DCM_CODE) Dcm_DsdInternal_ProcessPage(
  Dcm_MsgLenType FilledPageLen,
  Dcm_ProtocolIndexType ProtocolIdx)
{
  DBG_DCM_DSDINTERNAL_PROCESSPAGE_ENTRY(FilledPageLen, ProtocolIdx);
...
#endif /* DCM_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    DCM_HSM_INFO(ProtocolIdx).sentLength += FilledPageLen;

    /* ENTER critical section */
    SchM_Enter_Dcm_SCHM_DCM_EXCLUSIVE_AREA_0();

    /* update length information of transmission buffer */
    DCM_HSM_INFO(ProtocolIdx).TxBuffer.FilledLength += FilledPageLen;
    ...
    (void)DCM_HSMEMITINST_WRAPPER(&Dcm_HsmScDcm, ProtocolIdx, DCM_HSM_DCM_EV_PROCESS_PAGE);
  }

  DBG_DCM_DSDINTERNAL_PROCESSPAGE_EXIT(FilledPageLen, ProtocolIdx);
}
#endif
  ...
  P2VAR(Dcm_SidTabEntryConfigPtrType, AUTOMATIC, DCM_CONST) curSidConfig)
{
  ...
  if (curNrc == DCM_E_OK)
  {
    /* Verification of the service access control based on the current authentication state */
    curNrc = Dcm_Dsd_AuthenticationCheckOnServiceLevel(instIdx, sidConfig);
  }
#endif /* (DCM_AUTHENTICATION_CONNECTION == STD_ON) */

  /*
   * check if service request is allowed in current session and security levels
   * and mode rule has not failed
   */
  /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceRole.CheckPass.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceRole.CheckPass.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDataByIdentifier.ServiceRole.CheckPass.CheckActiveSession,1 */
  if (curNrc == DCM_E_OK)
  {
    /*
    ...
    if ((curNrc == DCM_E_OK) && (sidConfig->ModeRuleSrvFunction != NULL_PTR))
    {
      ...
      (void)sidConfig->ModeRuleSrvFunction(&modeRuleNrc);
...
STATIC FUNC(boolean, DCM_CODE) Dcm_DsdInternal_GetSubServiceUsedStatus(
  ...
  P2VAR(uint8, AUTOMATIC, DCM_VAR) subSidIdxExt)
{
  boolean subserviceEnabled = FALSE;
  uint8 subSidIdxExtLocal = *subSidIdxExt;

  /* search list of configured subservices */
  while (subSidIdxExtLocal < curSidConfig->NumSubServices)
  {
    if (curSidConfig->SubServices[subSidIdxExtLocal].SubServiceId == subSidId)
    {
      const uint8 subserviceEnabledByteIdx = subSidIdxExtLocal / 8U;
      const uint8 subserviceEnabledBitMask = (1U << (subSidIdxExtLocal % 8U));

      /* Check if the corresponding subservice is enabled */
      if ((Dcm_DsdSubServiceUsed[ServiceTableIndex][sidIdxExt][subserviceEnabledByteIdx] &
          subserviceEnabledBitMask) != 0U)
      {
        subserviceEnabled = TRUE;
      }
      break;
    }
    subSidIdxExtLocal++;
  ...
  *subSidIdxExt = subSidIdxExtLocal;
  return subserviceEnabled;
}
#endif /* #if (DCM_DSP_SUBSERVICE_USED_BUF_SIZE > 0)*/

/* !LINKSTO Dcm.Dsn.Dsd.IB.ValidateSubSvcReq,1 */
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_DsdInternal_ValidateSubSvcReq(
  uint8 instIdx,
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) curSidConfig)
{
  ...
  if(curMsgContext->reqDataLen > 0U)
  {
    ...
    if ((curSidTabConfig->Entries[curHsmInfo->sidTabEntryInd]).ServiceId == DCM_SID_TESTER_PRESENT)
    {
      /* !LINKSTO Dcm.EB.Calibration.DcmDsdSubServiceUsed.TesterPresent.zeroSubFunction.SkipCheck,1 */
      /* !LINKSTO Dcm.EB.Calibration.DcmDsdSubServiceUsed.TesterPresent.zeroSubFunction.SupressResponse.SkipCheck,1 */
      if ((curMsgContext->reqData[0] == 0x00U) || (curMsgContext->reqData[0] == 0x80U))
      {
        subserviceEnabled = TRUE;
      }
    }
    if (subserviceEnabled == FALSE)
    {
      subserviceEnabled = Dcm_DsdInternal_GetSubServiceUsedStatus(ServiceTableIndex, curHsmInfo->sidTabEntryInd, curSidConfig, curMsgContext->reqData[0], &subSidIdxExt);
    }
#endif /* #if (DCM_DSP_SUBSERVICE_USED_BUF_SIZE > 0) */
    ...
    if (curSidConfig->ServiceId != DCM_SID_ROUTINE_CONTROL)
    {
      ...
      if (subserviceEnabled == FALSE)
      {
        /* !LINKSTO Dcm.EB.Calibration.DcmDsdSubServiceUsed.False.TransmitNRC,1 */
        curNrc = DCM_E_SUBFUNCTIONNOTSUPPORTED;
      }
      else
      {
        /* !LINKSTO Dcm.EB.Calibration.DcmDsdSubServiceUsed.True.CheckPass,1 */
        curNrc = DCM_E_OK;
      }
#if (DCM_DSP_USE_SERVICE_0X31 == STD_ON)
    }
    ...
    {
      /* !LINKSTO Dcm.EB.Calibration.DcmDsdSubServiceUsed.RoutineControl.CheckPass,1 */
      curNrc = DCM_E_OK;
    }
  ...
  {
    /* !LINKSTO Dcm.EB.RequestVerification.MinimumLengthCheckFailure.TransmitNRC0x13,1 */
    curNrc = DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT;
  }
  ...
  if (curSidConfig->ServiceId != DCM_SID_ROUTINE_CONTROL)
  {
    ...
    if(curNrc == DCM_E_OK)
    {
      curNrc = Dcm_Dsd_AuthenticationCheckOnSubServiceLevel(instIdx, curSidConfig, subSidIdxExt);
    }
#endif /* #if (DCM_AUTHENTICATION_CONNECTION == STD_ON) */
    /* Deviation TASKING-3 */
    if(curNrc == DCM_E_OK)
    {
      /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceAndSubfunctionRole.CheckPass.CheckActiveSession,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceAndSubfunctionRole.CheckPass.CheckActiveSession,1 */
       ...
       (curSidConfig->SubServices[subSidIdxExt].ModeRuleSubSrvFunction != NULL_PTR))
    {
      ...
      (void)curSidConfig->SubServices[subSidIdxExt].ModeRuleSubSrvFunction(&modeRuleNrc);
...
#if ((DCM_INCLUDE_RTE == STD_OFF) || (DCM_SERVICE_REQUEST_NOTIFICATION_WITH_NULL_PTR_ENABLED == STD_ON))
  if (DataSize == 0x00U)
  {
    /* !LINKSTO Dcm.EB.RequestVerification.DcmServiceRequestNotificationWithNullPtrEnabled.RteOn.OneByteRequest.ManufacturerNotification.NullPointer,1 */
    RequestData = NULL_PTR;
  }
  else
#endif /* #if ((DCM_INCLUDE_RTE == STD_OFF) || (DCM_SERVICE_REQUEST_NOTIFICATION_WITH_NULL_PTR_ENABLED == STD_ON)) */
  {
    /* !LINKSTO Dcm.EB.RequestVerification.DcmServiceRequestNotificationWithNullPtrDisabled.RteOn.OneByteRequest.ManufacturerNotification.NonNullPointer,1 */
...
/*Verification of the Supplier Application environment/permission */
  ...
  if (DataSize == 0x00U)
  {
    /* !LINKSTO Dcm.EB.RequestVerification.DcmServiceRequestNotificationWithNullPtrEnabled.RteOn.OneByteRequest.SupplierNotification.NullPointer,1 */
    RequestData = NULL_PTR;
  }
  else
#endif /* #if ((DCM_INCLUDE_RTE == STD_OFF) || (DCM_SERVICE_REQUEST_NOTIFICATION_WITH_NULL_PTR_ENABLED == STD_ON)) */
  {
    /* !LINKSTO Dcm.EB.RequestVerification.DcmServiceRequestNotificationWithNullPtrDisabled.RteOn.OneByteRequest.SupplierNotification.NonNullPointer,1 */
    RequestData = &CurHsmInfo->RxBuffer.BufferPtr[1];
  }
...
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_Dsd_AuthenticationCheckOnServiceLevel(
  ...
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) SidConfig)
{
  Dcm_NegativeResponseCodeType AuthenticationNrc = E_OK;
  /* get the context information current instance */
  P2VAR(Dcm_HsmInfoType, AUTOMATIC, DCM_VAR) CurHsmInfo = &DCM_HSM_INFO(InstIdx);
  ...
  /* !LINKSTO Dcm.EB.RequestVerification.TesterPresent.NoAuthenticationCheck.OnServiceLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.Authentication.NoAuthenticationCheck.OnServiceLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.OBDServices.NoAuthenticationCheck.OnServiceLevel.CheckActiveSession,1 */
  if ((!DCM_IS_OBD_SVCID(CurHsmInfo->serviceId))
       &&
      (CurHsmInfo->serviceId != DCM_SID_AUTHENTICATION) && (CurHsmInfo->serviceId != DCM_SID_TESTER_PRESENT)
       &&
      (DCM_NUM_AUTHENTICATION_CONNECTIONS !=
       Dcm_MainConnectionConfig[Dcm_RxPduIdConfig[CurHsmInfo->RxConnectionId].MainConnectionIndex].AuthenticatedIndex))
  {
    /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.OnServiceLevel,1 */
    if (E_OK != Dcm_DiagnosticPolicyHandler_CheckAuthentication(
                  Dcm_MainConnectionConfig[Dcm_RxPduIdConfig[CurHsmInfo->RxConnectionId].MainConnectionIndex].AuthenticatedIndex,
                  SidConfig->Role))
    {
      /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceRole.NoRole.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceRole.CheckFailed.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDataByIdentifier.ServiceRole.CheckFailed.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDataByIdentifier.ServiceRole.NoRole.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceRole.NoRole.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceRole.CheckFailed.NRC_0x34,1 */
      AuthenticationNrc = DCM_E_AUTHENTICATIONREQUIRED;
    }
  }

  return AuthenticationNrc;
}
  ...
  uint8 SubSidIdxExt)
{
  ...
  /* !LINKSTO Dcm.EB.RequestVerification.deAuthenticate.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.verifyCertificateUnidirectional.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.verifyCertificateBidirectional.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.proofOfOwnership.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.authenticationConfiguration.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1
     !LINKSTO Dcm.EB.RequestVerification.zeroSubFunction.NoAuthenticationCheck.OnServiceAndSubfunctionLevel.CheckActiveSession,1 */
  if ((SidConfig->ServiceId != DCM_SID_AUTHENTICATION) && (SidConfig->ServiceId != DCM_SID_TESTER_PRESENT)
       &&
      (DCM_NUM_AUTHENTICATION_CONNECTIONS !=
       Dcm_MainConnectionConfig[Dcm_RxPduIdConfig[CurHsmInfo->RxConnectionId].MainConnectionIndex].AuthenticatedIndex))
  {
    /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.OnServiceAndSubfunctionLevel,2 */
    if (E_OK != Dcm_DiagnosticPolicyHandler_CheckAuthentication(
                  Dcm_MainConnectionConfig[Dcm_RxPduIdConfig[CurHsmInfo->RxConnectionId].MainConnectionIndex].AuthenticatedIndex,
                  SidConfig->SubServices[SubSidIdxExt].Role))
    {
      /* !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceAndSubfunctionRole.NoRole.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceAndSubfunctionRole.CheckFailed.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceAndSubfunctionRole.CheckFailed.NRC_0x34,1
         !LINKSTO Dcm.EB.RequestVerification.AuthenticationCheck.ServiceToRespondTo.ReadDTCInformation.ServiceAndSubfunctionRole.NoRole.NRC_0x34,1 */
      AuthenticationNrc = DCM_E_AUTHENTICATIONREQUIRED;
    }
  }
...
STATIC FUNC(Dcm_NegativeResponseCodeType, DCM_CODE) Dcm_Dsd_SecurityLevelCheckOnService(
  P2CONST(Dcm_SidTabEntryConfigType, AUTOMATIC, DCM_CONST) SidConfig)
{
  Dcm_NegativeResponseCodeType SecurityCheckNrc = E_OK;
  uint8 i = 0U;
  Dcm_SecLevelType CurSecLevel;

  DBG_DCM_DSD_SECURITYLEVELCHECKONSERVICE_ENTRY();
  ...
  if (CurSecLevel != DCM_SEC_LEV_LOCKED)
  {
    ...
    {
      SecurityCheckNrc = DCM_E_SECURITYACCESSDENIED;
    }
  }
  else
  {
  ...
  DBG_DCM_DSD_SECURITYLEVELCHECKONSERVICE_EXIT(SecurityCheckNrc);

  return SecurityCheckNrc;
}
