...
#if (defined DCM_DID_OBD_RANGE_LOW_LIMIT)
#error DCM_DID_OBD_RANGE_LOW_LIMIT already defined
#endif /* if (defined DCM_DID_OBD_RANGE_LOW_LIMIT) */
/** \brief OBD Did lower limit for high byte */
#define DCM_DID_OBD_RANGE_LOW_LIMIT (0xF4U)

#if (defined DCM_DID_OBD_RANGE_UP_LIMIT)
#error DCM_DID_OBD_RANGE_UP_LIMIT already defined
#endif /* if (defined DCM_DID_OBD_RANGE_UP_LIMIT) */
/** \brief OBD Did upper limit for high byte */
#define DCM_DID_OBD_RANGE_UP_LIMIT (0xF7U)
 ...
 ** \param[in]    UseFastPath Boolean parameter that defines path for DID processing.
 **
...
/** \brief Internal handler for UDS service 0x22 request consistensy check
 **
 ** \param[inout] pMsgContext Message related information for one diagnostic protocol identifier.
 **
 ** \retval TRUE         Request is correct
 ** \retval FALSE        Request is not correct
 */
STATIC FUNC(boolean, DCM_CODE) Dcm_Dsp_ReadDataByIdentifier_IsRequestLengthCorrect(
  Dcm_MsgContextPtrType pMsgContext);

#if (DCM_DSP_OBD_RANGE == STD_ON)
/** \brief Internal handler for UDS service 0x22 request consistensy check
 **
 ** \param[in] DidList   List id DIDs.
 ** \param[in] DidCount  Number of DIDs.
 **
 ** \retval TRUE         Request is correct
 ** \retval FALSE        Request is not correct
 */
STATIC FUNC(boolean, DCM_CODE) Dcm_Dsp_ReadDataByIdentifier_IsFastPathAllowed(
 ...
 ** \param[inout] pMsgContext Message related information for one diagnostic protocol identifier.
 ** \param[in]    ReadDidListContext Pointer to the context to be processed.
 **
 ** \retval DCM_E_DONE         Operation completed
 ** \retval DCM_E_PENDING      Operation did not complete, it must be called again
 */
...
STATIC FUNC(void, DCM_CODE) Dcm_Dsp_ReadDataByIdentifier_ReadDidList_Reset(
  P2VAR(Dcm_ReadDidListContextType, AUTOMATIC, DCM_VAR) ReadDidListContext);
...
#if (DCM_DSP_OBD_RANGE == STD_ON)
STATIC VAR(Dcm_ReadDidListContextType, DCM_VAR) Dcm_ReadDidListContextFastPath;
#endif
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  {
#if (DCM_NUM_UDS_STACKS > 1)
    if ((Dcm_ReadDataByIdentifier_LockStatus == DCM_EXECUTING_INTERNAL_SERVICE) &&
        (pMsgContext->msgAddInfo.requestOrigin == DCM_EXTERNAL))
    {
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NonOBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.TYPE2ROEEventAlreadyRunningOnDifferentStack.CancelTYPE2ROE.ProcessNonOBDOnUDSRequest,2 */
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.OBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.TYPE2ROEEventAlreadyRunningOnDifferentStack.CancelTYPE2ROE.ProcessOBDOnUDSRequest,2 */
      (void)Dcm_Dsp_ReadDataByIdentifier_ServiceExecution(DCM_CANCEL, pMsgContext);
    }
#endif /* #if (DCM_NUM_UDS_STACKS > 1) */

    if (Dcm_Dsp_ReadDataByIdentifier_IsRequestLengthCorrect(pMsgContext) != TRUE)
    {
    ...
    else
    {
      ...
      const uint32 didCount = (pMsgContext->reqDataLen) / DCM_DID_ID_SIZE;
      const boolean IsFastPathAllowed = Dcm_Dsp_ReadDataByIdentifier_IsFastPathAllowed(pMsgContext->reqData, (uint16)didCount);

#if (DCM_ASYNC_SERVICE_HANDLER == STD_ON)
      const boolean IsServiceExecutionTypeAsync = Dcm_Dsp_IsServiceExecutionTypeAsync(pMsgContext->dcmRxPduId);
      if (
          (IsServiceExecutionTypeAsync == FALSE) &&
          (IsFastPathAllowed == TRUE) &&
          (pMsgContext->msgAddInfo.requestOrigin != DCM_INTERNAL_ROE))
#else
      if (
          (IsFastPathAllowed == TRUE) &&
          (pMsgContext->msgAddInfo.requestOrigin != DCM_INTERNAL_ROE))
  ...
  {
#if (DCM_DSP_OBD_RANGE == STD_ON)
    if (FastPathAllowed == TRUE)
    {
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.OBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.NonOBDOnUDSRequestAlreadyRunningOnDifferentStack.ProcessInParallel,2 */
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.OBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.OBDOnUDSRequestAlreadyRunningOnDifferentStack.ProcessInParallel,2 */
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NewTYPE2ROEEvent.DefaultSession.OBDOnUDSRequestAlreadyRunningOnDifferentStack.ProcessInParallel,2 */
      ServiceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_SvcStart(pMsgContext, TRUE);
    }
    else
#endif /* #if (DCM_DSP_OBD_RANGE == STD_ON) */

    {
      ...
      switch(Dcm_ReadDataByIdentifier_LockStatus)
      {
        case DCM_TRYING_LOCK:
        {
             ...
             (OpStatus != DCM_CONFIRMED_NOK))
          {
            ...
            if (LockStatus == DCM_E_RESOURCE_LOCKED)
            {
              ...
              /* !LINKSTO Dcm.EB.ReadDataById.BusyWithRequestVehicleInformation.Postpone,1 */
              /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NonOBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.NonOBDOnUDSRequestAlreadyRunningOnDifferentStack.Postpone,2 */
            ...
            else
            {
              ...
              /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NonOBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.OBDOnUDSRequestAlreadyRunningOnDifferentStack.ProcessInParallel,2 */
        ...
        {
          /* If the current external request has locked DidServices unit then continue processing,
           * otherwise return pending until the locked is removed.
           */
          if ((pMsgContext->msgAddInfo.requestOrigin == DCM_EXTERNAL) &&
              (pMsgContext->dcmRxPduId == Dcm_ReadDidListContext.ReadDidListContext.RxPduId))
          {
            ServiceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_ServiceExecution(OpStatus, pMsgContext);
          }
          else if ((OpStatus == DCM_CONFIRMED_OK) || (OpStatus == DCM_CONFIRMED_NOK) || (OpStatus == DCM_CANCEL))
          {
            ServiceProcessStatus = DCM_E_DONE;
          }
          else
          {
            ...
            /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NewTYPE2ROEEvent.DefaultSession.NonOBDOnUDSRequestAlreadyRunningOnDifferentStack.Postpone,2 */
            /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NonOBDOnUDSRequest.RxIndication.ResultNTFRSLT_OK.DefaultSession.NonOBDOnUDSRequestAlreadyRunningOnDifferentStack.Postpone,2 */
            ServiceProcessStatus = DCM_E_PENDING;
          }
        }
  ...
  boolean UseFastPath)
{
  ...
  if (pMsgContext->reqData == pMsgContext->resData)
  {
    ...
    TS_MemCpy(Dcm_ReadDIDBuffer, pMsgContext->reqData, pMsgContext->reqDataLen);
  ...
  if (UseFastPath == TRUE)
  {
    ReadDidListContext = &Dcm_ReadDidListContextFastPath;
  }
#endif /*#if (DCM_DSP_OBD_RANGE == STD_ON) */
  ...
  serviceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_SvcContinue(DCM_INITIAL, pMsgContext, ReadDidListContext, UseFastPath);
  ...
  uint16 DidCount)
{
  ...
  boolean FastPathAllowed = TRUE;
  uint16 DidIdx = 0;
  uint16 DidPos = 0;

  while((DidIdx < DidCount) && (FastPathAllowed == TRUE))
  {
    /* Getting the requested DID from request buffer */
    Did = (uint16) DCM_COMPOSE_16BIT_WORD_FROM_MSB_AND_LSB((uint8)DidList[DidPos],
                                                           (uint8)DidList[DidPos + 1U]);

    if ((((uint8)DidList[DidPos] >= DCM_DID_OBD_RANGE_LOW_LIMIT) &&
         ((uint8)DidList[DidPos] <= DCM_DID_OBD_RANGE_UP_LIMIT)) ||
        (Did == 0xF810U))
    {
      /* Check if DID synchronous */
      FastPathAllowed = Dcm_Dsp_DidServices_IsDidSynchronous(Did);
      DidIdx++;
      DidPos = DCM_DID_ID_SIZE * DidIdx;
    }
    else
    {
...
STATIC FUNC(boolean, DCM_CODE) Dcm_Dsp_ReadDataByIdentifier_IsRequestLengthCorrect(
  Dcm_MsgContextPtrType pMsgContext)
{
  boolean RequestLengthIsCorrect;
  ...
  {
    /* !LINKSTO Dcm.UDSServices.ReadDataByIdentifier.TotalMinimumLengthCheck.Failed.TransmitNRC0x13,1 */
    /* !LINKSTO Dcm.UDSServices.ReadDataByIdentifier.TotalLengthCheck.Failed.TransmitNRC0x13,1 */
    Dcm_ExternalSetNegResponse(pMsgContext, DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
    RequestLengthIsCorrect = FALSE;
  }
  /* If DCM_READ_DID_MAX is zero no need to check the request Dids limit.*/
  /* Check if the requested Dids do not exceed the allowed limits.*/
  else if (didCount > DCM_READ_DID_MAX)
  {
#if (DCM_NRC_POLICY_ISO_VERSION == DCM_ISO14229_2013)
    /* !LINKSTO Dcm.UDSServices.ReadDataByIdentifier.TotalMaximumLengthCheck.ISO14229_2013.Failed.TransmitNRC0x13,1 */
    Dcm_ExternalSetNegResponse(pMsgContext, DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
#else
    /* !LINKSTO Dcm.UDSServices.ReadDataByIdentifier.TotalMaximumLengthCheck.ISO14229_2006.Failed.TransmitNRC0x31,1 */
  ...
  {
    RequestLengthIsCorrect = TRUE;
  }

  return RequestLengthIsCorrect;
}
  ...
  boolean UseFastPath)
{
...
#if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_OFF)
  TS_PARAM_UNUSED(UseFastPath);
#endif
  ...
  DCM_DSP_CONTEXT_OPSTATUS(DCM_DSP_CONTEXT(ReadDidListContext)) = OpStatus;
  /* execute ReadDidList */
  serviceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_ReadDidList_Execute(ReadDidListContext);

  if (DCM_E_OK == serviceProcessStatus)
  {
    CONSTP2VAR(Dcm_DspMsgBufferCfgType, AUTOMATIC, DCM_VAR) BufferCfg =
      &(ReadDidListContext->ReadDidListContext.BufferCfg);
    ...
    serviceProcessStatus = DCM_E_DONE;
    /* reset ReadDidList */
    Dcm_Dsp_ReadDataByIdentifier_ReadDidList_Reset(ReadDidListContext);
  ...
  {
    if ((pMsgContext->msgAddInfo.requestOrigin == DCM_INTERNAL_ROE) &&
        (ReadDidListContext->BaseContext.nrc == DCM_E_REQUESTCORRECTLYRECEIVEDRESPONSEPENDING))
    {
      /* set PENDING for ROE in case if MID lock is in place */
      /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.NewTYPE2ROEEvent.RequestOnBoardMonitoringTestResultsAlreadyRunning.Async.Postpone,1 */
      serviceProcessStatus = DCM_E_PENDING;
      ReadDidListContext->BaseContext.nrc = DCM_E_OK;
    }
    else
    {
      if (ReadDidListContext->BaseContext.nrc == DCM_E_REQUESTCORRECTLYRECEIVEDRESPONSEPENDING)
      {
        /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.OBDOnUDSRequest.AcceptedForDispatching.DefaultSession.RequestOnBoardMonitoringTestResultsAlreadyRunningOnDifferentStack.Async.TransmitNRC0x22,1 */
        /* !LINKSTO Dcm.EB.UDSServices.ReadDataByIdentifier.OBDOnUDSRequest.AcceptedForDispatching.DefaultSession.ReadDataByIdentifierAlreadyRunningOnDifferentStack.Async.TransmitNRC0x22,1 */
        ReadDidListContext->BaseContext.nrc = DCM_E_CONDITIONSNOTCORRECT;
      }
      Dcm_ExternalSetNegResponse(
        pMsgContext, DCM_DSP_CONTEXT_NRC(DCM_DSP_CONTEXT(ReadDidListContext)));
      /* reset ReadDidList */
      Dcm_Dsp_ReadDataByIdentifier_ReadDidList_Reset(ReadDidListContext);
  ...
  P2VAR(Dcm_ReadDidListContextType, AUTOMATIC, DCM_VAR) ReadDidListContext)
{
  ...
  /* reset ReadDidList */
  ReadDidList->DidCount = 0U;
  ReadDidList->DidsRead = 0U;
  ReadDidList->DidIdx   = 0U;
  ReadDidList->RxPduId  = DCM_NUM_RXPDUIDS;
}
  ...
  Dcm_Dsp_ContextPtr context)
{
  Dcm_DidOpType DidOp;
#if (DCM_DSP_OBD_RANGE == STD_ON)
  if ((DCM_DSP_READDIDLIST_CONTEXT(context).RxPduId == Dcm_ReadDidListContextFastPath.ReadDidListContext.RxPduId) &&
      (Dcm_ReadDidListContextFastPath.ReadDidListContext.DidCount > 0U))
  {
    DidOp = DCM_DID_OP_READ_OBD;
  }
  else
#endif /* #if (DCM_DSP_OBD_RANGE == STD_ON) */
  {
...
#if (DCM_DSP_OBD_RANGE == STD_ON)
  PduIdType RxPduId =
    DCM_DSP_READDIDLIST_CONTEXT(context).RxPduId;
#endif /*#if (DCM_DSP_OBD_RANGE == STD_ON) */
...
#if (DCM_DSP_OBD_RANGE == STD_ON)
      if ((RxPduId == Dcm_ReadDidListContextFastPath.ReadDidListContext.RxPduId) &&
          (Dcm_ReadDidListContextFastPath.ReadDidListContext.DidCount > 0U))
      {
        if ((DCM_DSP_READDIDLIST_CONDFAILED(Dcm_ReadDidListContextFastPath) == TRUE) ||
            (DCM_DSP_READDIDLIST_MODERULEFAILED(Dcm_ReadDidListContextFastPath) == TRUE))
        {
          ForceStopProcessing = TRUE;
          DCM_DSP_READDIDLIST_CONDFAILED(Dcm_ReadDidListContextFastPath) = FALSE;
          DCM_DSP_READDIDLIST_MODERULEFAILED(Dcm_ReadDidListContextFastPath) = FALSE;
        }
        DCM_DSP_READDIDLIST_CONDFAILED(Dcm_ReadDidListContext) = FALSE;
      }
      else
#endif /*#if (DCM_DSP_OBD_RANGE == STD_ON) */
      {
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  switch (OpStatus)
  {
    ...
    case DCM_INITIAL:
    {
      ServiceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_SvcStart(pMsgContext, FALSE);
    }
    break;
    ...
    case DCM_PENDING:
    {
      ServiceProcessStatus = Dcm_Dsp_ReadDataByIdentifier_SvcContinue(OpStatus, pMsgContext, &Dcm_ReadDidListContext, FALSE);
    }
    break;

    /* !LINKSTO Dcm.EB.ReadDataById.ReadMemory.CallRCRRP,1 */
    ...
    case DCM_CANCEL:
    {
      ...
      /* reset ReadDidList */
      Dcm_Dsp_ReadDataByIdentifier_ReadDidList_Reset(&Dcm_ReadDidListContext);
#if(DCM_DSP_DIDSERVICES_LOCK_MECHANISM_ENABLED == STD_ON)
      /* Unlock DidServices */
      Dcm_Dsp_DidServices_UnlockService();
      Dcm_ReadDataByIdentifier_LockStatus = DCM_TRYING_LOCK;
