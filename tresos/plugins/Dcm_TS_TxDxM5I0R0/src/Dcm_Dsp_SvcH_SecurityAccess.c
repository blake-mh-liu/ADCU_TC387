...
/** \brief Handle the result of the compare key operation
 **
 ** \param[in]    SendKeyStatus Return value of the SecurityCompareKey operation
 ** \param[out]   Nrc Negative reponse code
 ** \param[inout] MsgContextPtr Message related information for one diagnostic protocol identifier
 **
 ** \retval DCM_E_DONE           Operation completed
 ** \retval DCM_E_PENDING        Operation did not complete, it must be called again
 */
STATIC FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_SecurityAccess_CompareKey_HandleKeyResult(
...
#if ((DCM_SECURITY_EVENT_REPORTING == STD_ON) && (DIAG_SEV_SECURITY_ACCESS_NUMBER_OF_ATTEMPTS_EXCEEDED != DCM_SEV_INVALID_SEV_REF))
          Dcm_SEVReportingServices_IdsMSetSecurityEvent_AttemptCounter(
                                            AttemptCounter);
#endif /* #if ((DCM_SECURITY_EVENT_REPORTING == STD_ON) && (DIAG_SEV_SECURITY_ACCESS_NUMBER_OF_ATTEMPTS_EXCEEDED != DCM_SEV_INVALID_SEV_REF)) */
        }
        /* CHECK: NOPARSE */
        else
        {
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) MsgContextPtr)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if ((OpStatus != DCM_CANCEL) && (GetSeedStatus == DCM_E_NOT_OK) && (*Nrc == 0x00U))
      {
        *Nrc = DCM_E_CONDITIONSNOTCORRECT;
      }
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) MsgContextPtr)
{
  ...
  if (OpStatus != DCM_CANCEL)
  {
    ServiceProcessStatus = Dcm_Dsp_SecurityAccess_CompareKey_HandleKeyResult(SendKeyStatus, Nrc, MsgContextPtr);
  }
  else
  {
    ServiceProcessStatus = DCM_E_DONE;
