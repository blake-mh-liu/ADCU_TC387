...
STATIC FUNC (boolean, DCM_CODE)
  Dcm_Dsl_Supervisor_ProtocolHandling_BlockingProtocolOnDifferentStack(
    Dcm_ProtocolIndexType RxProtocolIndex
);
#endif
...
/** \brief Function checks the priority of the requested protocol against the current protocol
 **
 ** \param[in]  RequestOrigin                               Origin of the request
 ** \param[in]  CurrentProtocolIndex                        Index of the current protocol
 ** \param[in]  RequestedProtocolIndex                      Index of the requested protocol
 **
 ** \retval     DCM_SUPERVISOR_ARBITRATION_ACCEPT           Request accepted
 ** \retval     DCM_SUPERVISOR_ARBITRATION_REJECT_PRIORITY  Request rejected
 */
STATIC FUNC (Dcm_ReceptionArbitrationResultType, DCM_CODE)
  Dcm_Dsl_Supervisor_ProtocolHandling_CheckCurrentProtocolState(
...
/** \brief Function handles the cancellation activities of a protocol
 **
 ** \param[in]  CurrentProtocolIndex Index of the current protocol
 **
 ** \retval     NONE
...
STATIC FUNC (void, DCM_CODE)
  Dcm_Dsl_Supervisor_ProtocolHandling_CancelCurrentProtocol(
    Dcm_ProtocolIndexType CurrentProtocolIndex
);
...
STATIC VAR(Dcm_ProtocolIndexType, DCM_VAR) Dcm_LatestExternalUDSProtocolIndex[DCM_NUM_UDS_STACKS];
...
)
{
 ...
 /* Deviation TASKING-1 */
  ...
  {
    Dcm_LatestExternalUDSProtocolIndex[StackContextIdx] = DCM_INVALID_PROTOCOL_INDEX;
  }
#endif /* #if (DCM_NUM_UDS_STACKS > 0) */
...
)
{
  ...
  for (StackContextIdx = 0U;
       ((StackContextIdx < DCM_NUM_UDS_STACKS) && (GetProtocolConfigResult == DCM_E_NOT_OK));
       StackContextIdx++)
  {
    if ((Dcm_StackContextIndex[StackContextIdx] == StackNumber) &&
        (Dcm_LatestExternalUDSProtocolIndex[StackContextIdx] != DCM_INVALID_PROTOCOL_INDEX))
    {
      *CurrentProtocolConfig = DCM_PROTOCOL_CONFIG(Dcm_LatestExternalUDSProtocolIndex[StackContextIdx]);
...
)
{
  ...
  Dcm_ProtocolIndexType CurrentProtocolIndex;
  Dcm_ProtocolStackNumberType StackNumber = Dcm_ProtocolConfig[NextProtocolIndex].StackNumber;
...
)
{
  ...
  */
  ...
  if (DCM_INVALID_PROTOCOL_INDEX != CurrentProtocolIndex)
  {
    ...
    /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.RxIndication.ResultNTFRSLT_OK.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.ExternalUDS.HigherPriority.CurrentProtocolBusy.CancelProcessing,1 */
    /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.ApplicationRequest.Dispatch.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.ImmediateApplicationUDS.HigherPriority.CurrentProtocolBusy.CancelProcessing,1 */
    /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.ApplicationRequest.Dispatch.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.ImmediateApplicationUDS.HigherPriority.CurrentProtocolBusy.CancelTransmission,1 */
    ...
    Dcm_Dsl_Supervisor_ProtocolHandling_CancelCurrentProtocol(CurrentProtocolIndex);
  }
}
...
)
{
  ...
  /* OBD Protocols want the Session to stay the DefaultSession. Therefore, while any
     ...
     Parallel UDS Protocols want the Session to stay the DefaultSession. Therefore,
     while more than one UDS Protocol is Running, the Session won't switch.
   */
  /* Deviation TASKING-1 <+2> */
       ...
       ProtocolIndex++)
  {
       ...
       )
    {
      ...
      if (
          (DCM_PROTOCOL_OBD_GROUP == ProtocolGroup) &&
         ...
         )
      {
         ...
         */
        IsSessionClearedToSwitch = DCM_SUPERVISOR_SESSION_SWITCH_BUSY_RETRY;

      }
      else
      {
        if ((ProtocolIndex != ProcessingProtocolIdx) && (DCM_DEFAULT_SESSION != NewSession))
        {
          /* Busy with more than one UDS and the request is to switch to non
             default means we discard the request to switch the session. */
          IsSessionClearedToSwitch = DCM_SUPERVISOR_SESSION_SWITCH_BUSY_RETRY;
      ...
      {
        /* Break the search loop. Doesn't make sense to go on further. */
        break;
      }
    }
  }
...
)
{
  ...
  switch (Dcm_ProtocolStatus[ProtocolIndex].ProtocolState)
  {
    ...
    case DCM_SUPERVISOR_PROTOCOL_STOPPING_CANCELLING_TYPE2_ROE:
      /* Protocol finished its task. It becomes the StoppingIdle.
         This branch is only reachable via TYPE2 ROE Processing.
         No MainConnection to stop. */
      Dcm_ProtocolStatus[ProtocolIndex].ProtocolState =
          DCM_SUPERVISOR_PROTOCOL_STOPPING_IDLE;
      break;
...
)
{
...
#if (DCM_DSP_USE_ROESERVICES == STD_ON)
  boolean BlockingProtocolOnDifferentStack = FALSE;
#endif
  ...
  /* Next, if the request is for a Protocol that's already busy (Active or Presumptive),
    ...
    BlockingProtocolOnDifferentStack = Dcm_Dsl_Supervisor_ProtocolHandling_BlockingProtocolOnDifferentStack(RequestedProtocolIndex);

  if(
     (TRUE == ProtocolPresumptive) ||
     (TRUE == ProtocolBusy) ||
     ((TRUE == BlockingProtocolOnDifferentStack) && (DCM_INTERNAL_ROE == RequestOrigin))
    )
#else
  if(
     (TRUE == ProtocolPresumptive) ||
  ...
  else
  {
    /* The RxProtocol is not busy. There's a limitation that if there's another protocol of the same
       kind running on a different stack in non-default session or another OBD protocol is processing
       in default session, we need to decline (it is allowed to process UDS requestzs in parallel if
       they are on different stacks). Of course, if this Protocol of the same kind would be running
       on this same stack, we'd simply follow prioritization rules.
     */
    const boolean BlockingProtocolOfSameTypeOnDifferentStack =
      Dcm_Dsl_Supervisor_ProtocolHandling_BlockingProtocolOfSameTypeOnDifferentStack(RequestedProtocolIndex);

    if (TRUE == BlockingProtocolOfSameTypeOnDifferentStack)
    {
    ...
    else
    {
        ...
        */
            ...
            Dcm_ProtocolConfig[RequestedProtocolIndex].Priority)
        {
             ...
             )
          {
            ...
            /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.StartOfReception.NotOnCurrentProtocol.RequestedProtocolHigherPrio.DefaultSession.CurrentProtocolRunningTYPE2ROE.BufferFree.AcceptRx,1 */
      ...
      else
      {
        ...
        if (DCM_INVALID_PROTOCOL_INDEX != CurrentProtocolIndex)
        {
          ...
          ArbitrationResult = Dcm_Dsl_Supervisor_ProtocolHandling_CheckCurrentProtocolState(
                                RequestOrigin,
                                CurrentProtocolIndex,
                                RequestedProtocolIndex);
        }
        else
        {
...
)
{
  ...
  for (RxConnectionIndex = 0;
       ...
       RxConnectionIndex++)
  {
    Dcm_ProtocolStackNumberType ReceptionStackNumber;
...
)
{
  ...
  const Dcm_ProtocolStackNumberType NextStackNumber =
    Dcm_ProtocolConfig[NextProtocolIndex].StackNumber;
  const Dcm_ProtocolGroupType NextProtocolGroup =
    Dcm_Dsl_Supervisor_ProtocolHandling_GetProtocolGroupTypeForProtocol(NextProtocolIndex);
  ...
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.RxIndication.ResultNTFRSLT_OK.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.ExternalUDS.HigherPriority.CurrentProtocolBusy.CancelTransmission,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.RxIndication.ResultNTFRSLT_OK.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.ExternalUDS.HigherPriority.CurrentProtocolBusyTYPE1Periodic.CancelTransmission,1 */

  /* Start with presumptive protocol for non-default session. */
  if (DCM_SUPERVISOR_PROTOCOL_QUEUE_IDLE != Dcm_ProtocolQueue[NonDefaultSessionProtocolIndex].ProtocolQueueState)
  {
    Dcm_Dsl_Supervisor_ProtocolHandling_CancelPresumptiveProtocol(NonDefaultSessionProtocolIndex);
  }
  ...
  for (ProtocolIndex = 0U; ProtocolIndex < DCM_NUM_PROTOCOLS; ProtocolIndex++)
  {
    ...
    if (((Dcm_ProtocolConfig[ProtocolIndex].StackNumber != NextStackNumber) &&
         (DCM_PROTOCOL_UDS_GROUP == NextProtocolGroup)) ||
        (DCM_PROTOCOL_OBD_GROUP == NextProtocolGroup))
    {
    if(DCM_PROTOCOL_UDS_GROUP == ProtocolGroup)
    {
      ...
      switch (Dcm_ProtocolStatus[ProtocolIndex].ProtocolState)
      {
        ...
        case DCM_SUPERVISOR_PROTOCOL_STARTED_ACTIVE_TYPE1_ROE:
          ...
          /* Since there is only one active protocol in non-default session TYPE1 ROE
             request is cancelled either by external request or as presumptive by TYPE1
             periodic. */
          DCM_UNREACHABLE_CODE_ASSERT(DCM_INTERNAL_API_ID);
          break;
        /* CHECK: PARSE */
...
{
  Dcm_ProtocolIndexType latestExternalUDSProtocolIndex = DCM_INVALID_PROTOCOL_INDEX;
  ...
  /* Deviation TASKING-1 <+2> */
  for (StackContextIdx = 0U;
       ((StackContextIdx < DCM_NUM_UDS_STACKS) && (latestExternalUDSProtocolIndex == DCM_INVALID_PROTOCOL_INDEX));
       StackContextIdx++)
  {
    if (Dcm_StackContextIndex[StackContextIdx] == StackNumber)
    {
      latestExternalUDSProtocolIndex = Dcm_LatestExternalUDSProtocolIndex[StackContextIdx];
    }
  }
#else
  TS_PARAM_UNUSED(StackNumber);
#endif /* #if (DCM_NUM_UDS_STACKS > 0) */

  return latestExternalUDSProtocolIndex;
}

FUNC (boolean, DCM_CODE)
  Dcm_Dsl_Supervisor_ProtocolHandling_CheckIfExternalUDSProtocolExist(
    void
)
{
  boolean ExternalUDSProtocolExist = FALSE;
  ...
  /* Deviation TASKING-1 <+2> */
  for (StackContextIdx = 0U;
       ((StackContextIdx < DCM_NUM_UDS_STACKS) && (ExternalUDSProtocolExist == FALSE));
       StackContextIdx++)
  {
    if (Dcm_LatestExternalUDSProtocolIndex[StackContextIdx] != DCM_INVALID_PROTOCOL_INDEX)
    {
      ExternalUDSProtocolExist = TRUE;
    }
  }
#endif /* #if (DCM_NUM_UDS_STACKS > 0) */

  return ExternalUDSProtocolExist;
}
...
{
  uint8_least ProtocolIndex;
  Dcm_ProtocolGroupType OtherProtocolGroup;
  ...
  boolean BlockingProtocolOfSameTypeOnDifferentStack = FALSE;
  ...
  /* External UDS requests in default session don't count as they're allowed to run in parallel. */
  if (DCM_DEFAULT_SESSION == CurrentSession)
  {
    /* Deviation TASKING-1 */
    for (ProtocolIndex = 0; ProtocolIndex < DCM_NUM_PROTOCOLS; ProtocolIndex++)
    {
      OtherProtocolGroup = Dcm_Dsl_Supervisor_ProtocolHandling_GetProtocolGroupTypeForProtocol(
            (Dcm_ProtocolIndexType)(ProtocolIndex)
          );

      if (
          /* OBD Protocol is on a different stack ... check protocol status */
          (Dcm_ProtocolConfig[ProtocolIndex].StackNumber != StackNumber) &&
          (OtherProtocolGroup == RxProtocolGroup) && (DCM_PROTOCOL_OBD_GROUP == RxProtocolGroup) &&
          (
           (
            /* ... being active... */
            (DCM_SUPERVISOR_PROTOCOL_STARTED_ACTIVE_POST_BOOT == Dcm_ProtocolStatus[ProtocolIndex].ProtocolState) ||
         ...
         )
      {
        ...
        BlockingProtocolOfSameTypeOnDifferentStack = TRUE;
        break;
      }
    }
  }
  ...
  {
    ProtocolIndex = Dcm_Dsl_DiagnosticSessionHandler_GetNonDefaultSessionProtocolIndex();
    DCM_PRECONDITION_ASSERT(DCM_INVALID_PROTOCOL_INDEX != ProtocolIndex,
                            DCM_INTERNAL_API_ID);

    OtherProtocolGroup = Dcm_Dsl_Supervisor_ProtocolHandling_GetProtocolGroupTypeForProtocol(
          (Dcm_ProtocolIndexType)(ProtocolIndex));
    DCM_PRECONDITION_ASSERT(DCM_PROTOCOL_OBD_GROUP != OtherProtocolGroup,
                            DCM_INTERNAL_API_ID);

    if (
        /* UDS Protocol is on a different stack in non-default session... compare priorities */
        (Dcm_ProtocolConfig[ProtocolIndex].StackNumber != StackNumber) &&
        (OtherProtocolGroup == RxProtocolGroup) && (DCM_PROTOCOL_UDS_GROUP == RxProtocolGroup) &&
        !(Dcm_ProtocolConfig[ProtocolIndex].Priority > Dcm_ProtocolConfig[RxProtocolIndex].Priority)
       )
    {
  ...
  return BlockingProtocolOfSameTypeOnDifferentStack;
}
...
)
{
  ...
  else
  {
    ...
    *ProtocolIndex = Dcm_Dsl_Supervisor_GetTYPE2PresumptiveProtocolOnStack(StackNumber);
    ...
    if (DCM_INVALID_PROTOCOL_INDEX != *ProtocolIndex)
    {
      if ((DCM_SUPERVISOR_PROTOCOL_STARTED_IDLE == Dcm_ProtocolStatus[*ProtocolIndex].ProtocolState) ||
          (DCM_SUPERVISOR_PROTOCOL_STOPPED_IDLE == Dcm_ProtocolStatus[*ProtocolIndex].ProtocolState))
      {
        RequestCanBeAccepted = DCM_E_OK;
      }
      else
      {
        /* Defensive branch.
           This branch cannot be reached, since in order to reach the 'ELSE' condition, the
           protocol has to be in a state different from "STARTED_IDLE", or "STOPPED_IDLE".
           However, at the moment when the request is enqueued by calling the function
           "Dcm_Dsl_Supervisor_ProtocolHandling_EnqueueAsPresuntive()", the arbitration
           process already ensures that the state of the protocol is either "STARTED_IDLE"
           or "STOPPED_IDLE", making the conditions for the "ELSE" statement to be
           impossible to meet.
         */
        DCM_UNREACHABLE_CODE_ASSERT(DCM_INTERNAL_API_ID);
      }
...
)
{
  ...
  for (ProtocolIndex = 0; ProtocolIndex < DCM_NUM_PROTOCOLS; ProtocolIndex++)
  {
       ...
       )
    {
      ...
      BlockingProtocolOnDifferentStack = TRUE;
      break;
    }
  }

  return BlockingProtocolOnDifferentStack;
}
#endif /* #if (DCM_DSP_USE_ROESERVICES == STD_ON) */
...
)
{
  ...
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.RxIndication.ResultNTFRSLT_OK.AcceptedForProcessing.UDSRequestUDSAlreadyRunningOnDifferentStack.DefaultSession.ProcessInParallel,1 */
  ...
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.ImmediateApplicationRequest.Dispatch.AcceptedForProcessing.UDSRequestUDSAlreadyRunningOnDifferentStack.DefaultSession.ProcessInParallel,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.DelayedApplicationRequest.Dispatch.AcceptedForProcessing.UDSRequestUDSAlreadyRunningOnDifferentStack.DefaultSession.ProcessInParallel,1 */
  ...
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.RxIndication.ResultNTFRSLT_OK.AcceptedForProcessing.NonDefaultSession.TYPE2PeriodicRunningOnDifferentStack.ProcessInParallel,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.RequestedProtocolNotStarted.StartProtocol.ReturnsE_OK.ProcessRequest,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolBusy.RequestedProtocolAlreadyStarted.ProcessRequest,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolIdle.RequestedProtocolAlreadyStarted.ProcessRequest,1 */
  ...
  switch (Dcm_ProtocolQueue[ProtocolIndex].ProtocolQueueState)
  {
    ...
    case DCM_SUPERVISOR_PROTOCOL_QUEUE_EXTERNAL_QUEUED:
    {
      ...
      Dcm_Dsl_Supervisor_MainConnectionHandling_InitiateNRC(
            ...
            NRCTimeout);
#else
      if (0U != Dcm_ProtocolQueue[ProtocolIndex].RCRRPsConfirmed)
      {
        Dcm_Dsl_Supervisor_MainConnectionHandling_InitiateNRC(
            Dcm_ProtocolQueue[ProtocolIndex].RxConnectionId,
            Dcm_ProtocolQueue[ProtocolIndex].RequestBuffer,
      ...
      )
    {
...
#endif /* #if (DCM_CALLBACK_REQUEST_SIZE > 0) */
      {
        ...
        switch (Dcm_ProtocolStatus[ProtocolIndex].ProtocolState)
        {
          ...
          /* CHECK: NOPARSE */
          /* A periodic type1 transmission can not find another current protocol
             in a started running ROE Type2. The code is not reachable. */
          case DCM_SUPERVISOR_PROTOCOL_STOPPING_ACTIVE_TYPE2_ROE:
            Dcm_ProtocolStatus[ProtocolIndex].ProtocolState =
              DCM_SUPERVISOR_PROTOCOL_STOPPED_ACTIVE_TYPE2_ROE;
            break;
...
)
{
  ...
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolBusy.RequestedProtocolNotStarted.StartProtocol.Call,1 */
  /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolIdle.RequestedProtocolNotStarted.StartProtocol.Call,1 */
       ...
       StartOperationIndex++)
  {
    ...
    else if (DCM_E_OK != Result)
    {
      ...
      /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.RequestedProtocolNotStarted.StartProtocol.ReturnsUnsupportedCode.ReportDETError,1 */
  ...
  if (DCM_INVALID_PROTOCOL_INDEX != TYPE1PresumptiveProtocolIndex)
  {
    if (TRUE == Dcm_Dsl_Supervisor_ProtocolHandling_StackCanAcceptTYPE1Request(StackNumber))
    {
...
#endif /* #if (DCM_CALLBACK_REQUEST_SIZE > 0) */
      {
        ...
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.RequestedProtocolNotStarted.StartProtocol.ReturnsE_OK.ResetToDefaultSession,1 */
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolBusy.RequestedProtocolAlreadyStarted.ResetToDefaultSession,1 */
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolIdle.RequestedProtocolAlreadyStarted.ResetToDefaultSession,1 */
        ...
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.RequestedProtocolNotStarted.StartProtocol.ReturnsE_OK.LockSecurityLevel,1 */
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolBusy.RequestedProtocolAlreadyStarted.LockSecurityLevel,1 */
        /* !LINKSTO Dcm.EB.DiagnosticProtocolHandling.UDSRequest.HigherPriority.AcceptedForProcessing.NonDefaultSession.UDSRequestUDSAlreadyRunningOnDifferentStack.CurrentProtocolIdle.RequestedProtocolAlreadyStarted.LockSecurityLevel,1 */
    ...
    if (DCM_E_NOT_OK != StackCanAcceptRequest)
    {
            ...
            Dcm_Dsl_Supervisor_GetProtocolGroupTypeForProtocol(TYPE1PresumptiveProtocolIndex))
      {
             ...
             StackContextIdx++)
        {
          if (Dcm_StackContextIndex[StackContextIdx] == StackNumber)
          {
            Dcm_LatestExternalUDSProtocolIndex[StackContextIdx] = TYPE1PresumptiveProtocolIndex;
  ...
  Dcm_ProtocolIndexType ProtocolIndex)
{
  ...
  else
  {
    ...
    /* This function cannot be executed as a result of a request type DCM_JUMP_FROM_BL_RESPONSE_REQUEST
       or DCM_JUMP_FROM_BL_NO_RESPONSE_REQUEST, therefore there is no need to stop and free the P2Timer
      ...
      */
      ...
      if (
          /* ... for an External or PostBoot request, give the permission for reception since
             an external reception shall cancel an ROE processing in default session. */
          (
           ((DCM_EXTERNAL == RequestOrigin) ||
           (DCM_INTERNAL_POST_BOOT_NO_ANSWER == RequestOrigin) ||
           (DCM_INTERNAL_POST_BOOT_WITH_ANSWER == RequestOrigin)
          ...
          /* If the session is a non-default session or an internal request is appeared,
             consider the priority schema. */
          (Dcm_ProtocolConfig[CurrentProtocolIndex].Priority >
           Dcm_ProtocolConfig[RequestedProtocolIndex].Priority)
         )
      {
