    ...
    P2VAR(Dcm_DspReqVehInfoContextType, AUTOMATIC, DCM_VAR) svcContext)
{
  ...
  if (VehInfoIdx < DCM_DSP_VEHINFO_NUM)
  {
         ...
         VehInfoDataRelIndex++)
    {
      ...
      if ((pVehInfo->DcmDspVehInfoNODIProvResp == TRUE) && (svcContext->service22 == TRUE))
      {
        /* !LINKSTO Dcm.ReadDataById.ReadVehicleInformation.DIDNotConfigured.NonAvailability.GetInfotypeValueDataReturnsE_OK.PositiveResponse,1 */
        uint8 BufferValue;
        /* The first byte of the callout data is the NODI byte that we does not need here
           BufferOccupied is always bigger than zero, because we have either the DID or the INFOTYPE ID */
        DCM_PRECONDITION_ASSERT(BufferOccupied > 1U, DCM_INTERNAL_API_ID);
        /* save the value from the buffer that will be overwritten by the NODI */
        BufferValue = pBuffer[BufferOccupied - 1U];
        InterfaceReturnValue = pGetInfotypeValueData(OperationStatus, &(pBuffer[BufferOccupied - 1U]), &DataValueBufferSize);
        /* copy back the overwritten value by the NODI */
        pBuffer[BufferOccupied - 1U] = BufferValue;
        /* decrease the response size because of the stripped NODI byte */
        DataValueBufferSize -= 1U;
      }
      else
      {
      ...
      DataValueBufferSize = (uint8)(svcContext->BufferCfgPtr->Size - BufferOccupied);
      if (DataValueBufferSize >= (uint8)(pVehInfoData->VehInfoDataSize))
      {
        /* call getter function */
        /* !LINKSTO Dcm.RequestVehicleInformation.NonAvailabilityInfoType.DcmDspVehInfoDataUsePortFalse,1 */
        /* !LINKSTO Dcm.RequestVehicleInformation.NonAvailabilityInfoType.DcmDspVehInfoDataUsePortTrue,1 */
      ...
      else
      {
        ...
        /* !LINKSTO Dcm.EB.RequestVehicleInformation.DcmDspVehInfoASRVersion403.NonAvailabilityInfoType.BufferSizeOverflow.ReportDET,1 */
#if (DCM_DEV_ERROR_DETECT == STD_ON)
        DCM_DET_REPORT_ERROR(DCM_SVCID_GETINFOTYPEVALUEDATA, DCM_E_INTERFACE_BUFFER_OVERFLOW);
      ...
      switch (InterfaceReturnValue)
      {
        case E_OK:
          ...
          /* !LINKSTO Dcm.ISO-15031-5_6.2.4.3.7_Table11_y,1 */
          {
            ...
            /* NODI is stripped from responses to service 0x22 */
            if ((pVehInfo->DcmDspVehInfoNODIProvResp == TRUE) && (svcContext->service22 == FALSE))
            {
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  switch (OpStatus)
  {
     ...
     if (reqDataLen >= DCM_OBD_MIN_REQ_VEHINFO_SERVICE_09)
      {
       const uint8 firstReqVehInfo = pMsgContext->reqData[0U];
        ...
        /* !LINKSTO Dcm.RequestVehicleInformation.NonAvailabilityInfoType.DcmDspVehInfoDataUsePortTrue,1 */
        {
          ...
          {
#if (DCM_DSP_VEHINFO_AUTOSAR_VERSION == DCM_DSP_VEHINFO_AUTOSAR_440)
            /* The service context is service $09, so reset the service22 to FALSE */
            Dcm_DspReqVehInfoContext.service22 = FALSE;
#endif
    ...
    /* Deviation MISRA-1, MISRAC2012-1, MISRAC2012-2 */
  ...
  switch (serviceProcessStatus)
  {
    ...
    case DCM_E_NOT_OK:
...
#endif /* (DCM_DSP_VEHINFO_AUTOSAR_VERSION == DCM_DSP_VEHINFO_AUTOSAR_440) */
      {
        ...
        /* !LINKSTO Dcm.EB.RequestVehicleInformation.DcmDspVehInfoASRVersion403.NonAvailabilityInfoType.BufferSizeOverflow.NoResponse,1 */
