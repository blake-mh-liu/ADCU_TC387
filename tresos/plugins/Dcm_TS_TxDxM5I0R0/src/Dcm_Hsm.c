 ...
 *    which is not correctly analyzed by Astree.
 *
 *  MISRAC2012-6) Deviated Rule: 10.4 (required)
 *    Both operands of an operator in which the usual arithmetic conversions are performed shall have
 *    the same essential type category.
 *
 *    Reason:
 *    Function printf() is not used in production (it is only used for debugging purposes).
 *    Ternary operation guardCond?"TRUE":"FALSE" is used on the context of printf() and MISRA tool
 *    sees "TRUE" as char[5] and "FALSE" as a char[6], therefore the essential type category difference.
 *
 */
 ...
 *
 *    Reason:
 *    __NO_ISOCEXT macro is required to be set, to include only the basic functionality of
 *    the functions that are required from the standard libraries.
 */
...
/* Switch which activates some of options/definitions in HSM for debug */
...
#endif
/* __NO_ISOCEXT is a switch which activates only the basic functionality of the functions
 * which are required from the standard libraries */
/* Deviation CERTC-1 */
#define __NO_ISOCEXT
...
#if (DCM_HSM_INST_MULTI_ENABLED == STD_ON)
/* Deviation MISRAC2012-4, MISRAC2012-5 */
      ...
      for (i = 0U; (i < state->numTrans) && (FALSE == triggerFound); ++i)
      {
        ...
        if (event == trans->event)
        {
          ...
          if (trans->guardFnIdx != DCM_HSM_GIDX_INVALID)
          {
            ...
            /* Deviation MISRAC2012-2, MISRAC2012-6 */
            /* Deviation MISRA-2 */
            DCM_HSM_PRINT5(
