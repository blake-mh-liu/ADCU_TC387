 ...
 ** \param[in]    ProtocolIdx Protocol index.
 */
...
/* Flags that inhibit the decrementation of the P2 timer for one cycle. Used in conjunction with
   the flag signaling a reception during the Dcm_MainFunction(). */
STATIC VAR(boolean, DCM_VAR) Dcm_P2TimerDecrementInhibited[DCM_NUM_PROTOCOLS];

/* Flag signaling a reception during the Dcm_MainFunction(). */
STATIC volatile VAR(boolean, DCM_VAR) Dcm_TimerActionDuringMainFunction;
...
/* Session timer value */
/* !LINKSTO Dcm.EB.SessionStateHandling.S3Server.Handling,2 */
STATIC VAR(Dcm_TimerValueType, DCM_VAR) Dcm_S3TimerValue;
...
FUNC(void, DCM_CODE) Dcm_Dsl_DiagnosticSessionHandler_Init(void)
{
  ...
  for (ProtocolIdx = 0U; ProtocolIdx < DCM_NUM_PROTOCOLS; ProtocolIdx++)
  {
    ...
    Dcm_P2TimerDecrementInhibited[ProtocolIdx] = FALSE;
  }
...
{
#if (DCM_AUTHENTICATION_CONNECTION == STD_ON)
  boolean S3Timeout_Occured = FALSE;
  ...
  boolean CurrentSessionIsNotDefaultSession = FALSE;
...
#if (DCM_AUTHENTICATION_IDLECONNECTIONTIMER == STD_ON)
  /* Preparation of data required prior to enter Dcm_AuthenticationHandler_IdleConnectionTimerStart()*/
  /* Get AuthenticationConnectionIndex or invalid value DCM_NUM_MAIN_CONNECTIONS. */
  AuthenticationConnectionIndex = Dcm_Authentication_GetAuthenticationConnectionIndex(
                                    Dcm_DiagnosticSessionMainConnectionIndex);
  if (Dcm_Dsl_DiagnosticSessionHandler_GetCurrentSession() != DCM_DEFAULT_SESSION)
  {
    CurrentSessionIsNotDefaultSession = TRUE;
  }
#endif /* #if (DCM_AUTHENTICATION_IDLECONNECTIONTIMER == STD_ON) */
  /* enter critical section */
  SchM_Enter_Dcm_SCHM_DCM_EXCLUSIVE_AREA_0();

  /* session timer has runned out */
  ...
  /* Skip MainConnection which does not support authentication, i.e. is not referenced in authentication configuration.
     Check that transition is from non-default to default session and it is not because of S3Timeout. */
  if ((CurrentSessionIsNotDefaultSession == TRUE) &&
      (AuthenticationConnectionIndex != DCM_NUM_MAIN_CONNECTIONS) &&
      (S3Timeout_Occured == FALSE))
    {
      /* !LINKSTO Dcm.EB.AuthenticationStateHandling.Authenticated.Idle.NonDefaultSession.ProtocolPreemption.StartIdleConnectionTimer,1 */
      /* !LINKSTO Dcm.EB.AuthenticationStateHandling.Authenticated.Idle.NonDefaultSession.ResetToDefaultSessionAPI.StartIdleConnectionTimer,1 */
  ...
  /* Notify Authentication unit about S3 timeout */
#if (DCM_AUTHENTICATION_CONNECTION == STD_ON)
  if (S3Timeout_Occured == TRUE)
  {
    ...
    Dcm_Authentication_ResetOnTimeout(Dcm_DiagnosticSessionMainConnectionIndex);
  }
#endif /* if (DCM_AUTHENTICATION_CONNECTION == STD_ON) */

  if (Dcm_DiagnosticSessionResetFlag == TRUE)
  {
    Dcm_Dsl_DiagnosticSessionHandler_DoSwitch(DCM_DEFAULT_SESSION, DCM_INVALID_PROTOCOL_INDEX);
  }
...
FUNC(void, DCM_CODE) Dcm_Dsl_DiagnosticSessionHandler_P2TimerHandlingMainFunction(void)
{
  ...
  {
    /* P2timer is running */
    if ((FALSE == Dcm_P2TimerDecrementInhibited[ProtocolIdx])&&(Dcm_P2TimerValue[ProtocolIdx] > 0U))
    {
    ...
    /* Should the P2Timer had been started during the same Dcm_MainFunction() cycle, allow decrementing it in
       the next Dcm_MainFunction() cycles. */
    Dcm_P2TimerDecrementInhibited[ProtocolIdx] = FALSE;
  }

  /* Any P2 timer started between this point and the next Dcm_MainFunction()
     cycle will be decremented in the next Dcm_MainFunction() cycle.*/
  Dcm_TimerActionDuringMainFunction = FALSE;
  ...
  Dcm_ProtocolIndexType ProtocolIdx)
{
  ...
  DBG_DCM_DSL_DIAGNOSTICSESSIONHANDLER_SWITCHCURRENTSESSION_ENTRY(NewSession, ProtocolIdx);

  IsSessionClearedToSwitch = Dcm_Dsl_Supervisor_IsSessionClearedToSwitch(NewSession, ProtocolIdx);
  ...
  /* !LINKSTO Dcm.EB.SessionStateHandling.DefaultSession.DiagnosticSessionControlRequestRunning.UDSRequestUDSAlreadyRunningOnDifferentStack.SPRMIB.False.DiscardSessionSwitch,1 */
  /* !LINKSTO Dcm.EB.SessionStateHandling.DefaultSession.DiagnosticSessionControlRequestRunning.UDSRequestUDSAlreadyRunningOnDifferentStack.SPRMIB.True.DiscardSessionSwitch,1 */
      ...
      (FALSE == ResetToDefaultSessionOccured))
  {
    ...
    AuthenticationConnectionIndex =
      Dcm_Authentication_GetAuthenticationConnectionIndex(Dcm_DiagnosticSessionMainConnectionIndex);

    /* Skip MainConnection which does not support authentication, i.e. is not referenced in authentication configuration.
       Start IdleConnectionTimer only on transition from a non-default to the default session */
    if ((Dcm_Dsl_DiagnosticSessionHandler_GetCurrentSession() != DCM_DEFAULT_SESSION) &&
        (AuthenticationConnectionIndex != DCM_NUM_MAIN_CONNECTIONS) &&
        (NewSession == DCM_DEFAULT_SESSION))
    {
      /* enter critical section */
      SchM_Enter_Dcm_SCHM_DCM_EXCLUSIVE_AREA_0();

      /* Check if S3Timer is running, i.e. connection is idle */
      if (Dcm_S3TimerValue > 0U)
      {
        /* !LINKSTO Dcm.EB.AuthenticationStateHandling.Authenticated.Idle.NonDefaultSession.UDSService0x10.StartIdleConnectionTimer,2 */
        (void)Dcm_AuthenticationHandler_IdleConnectionTimerStart(AuthenticationConnectionIndex);
      }
...
#endif /* #if (DCM_AUTHENTICATION_IDLECONNECTIONTIMER == STD_ON) */
    Dcm_Dsl_DiagnosticSessionHandler_DoSwitch(NewSession, ProtocolIdx);
  }
...
FUNC(void, DCM_CODE) Dcm_Dsl_DiagnosticSessionHandler_StartP2Timer(
  ...
  boolean IsStartedByReception)
{
  DBG_DCM_DSL_DIAGNOSTICSESSIONHANDLER_STARTP2TIMER_ENTRY(UserId,ProtocolIndex,Timeout, IsStartedByReception);
  ...
  /* If the timer is started by an RxConnection during the run of a Dcm_MainFunction() cycle,
     avoid decrementing it during the same Dcm_MainFunction() cycle. */
    Dcm_P2TimerDecrementInhibited[ProtocolIndex] = (boolean)((Dcm_TimerActionDuringMainFunction) && (IsStartedByReception));
...
FUNC(void, DCM_CODE)Dcm_Dsl_DiagnosticSessionHandler_StartOfMainFunctionCycle(void)
{
  Dcm_TimerActionDuringMainFunction = TRUE;
}
  ...
  Dcm_ProtocolIndexType ProtocolIdx)
{
  ...
  DBG_DCM_DSL_DIAGNOSTICSESSIONHANDLER_DOSWITCH_ENTRY(NewSession, ProtocolIdx);
...
#if (DCM_DSP_USE_DATATRANSFERSERVICES == STD_ON)
    Dcm_Dsl_DataTransferServices_SessionChange(NewSession);
#endif  /* (DCM_DSP_USE_DATATRANSFERSERVICES == STD_ON) */
