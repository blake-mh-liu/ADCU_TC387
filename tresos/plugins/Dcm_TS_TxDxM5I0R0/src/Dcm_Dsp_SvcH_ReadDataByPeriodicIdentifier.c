...
FUNC(void, DCM_CODE) Dcm_Dsp_ReadDataByPeriodicIdentifier_Schedule(void)
{
  ...
  {
    /* !LINKSTO Dcm.ReadDataByPeriodicIdentifier.Dsn.SchedData,1 */
     ...
     * expires */
    /* !LINKSTO Dcm.ReadDataByPeriodicIdentifier.Dsn.SchedProcess,1 */
...
FUNC(void, DCM_CODE) Dcm_Dsp_ReadDataByPeriodicIdentifier_ProcessPDID(uint8 InstIdx)
{
  ...
  if (Dcm_PeriodicDIDContext.NumberOfTransmittableSamples > 0U)
  {
    ...
    if (BUFREQ_OK == TxBufferAllocationResult)
    {
      ...
      /* !LINKSTO Dcm.ReadDataByPeriodicIdentifier.Dsn.CancelProcess,1 */
      ...
      /* !LINKSTO Dcm.ReadDataByPeriodicIdentifier.Dsn.SchedDidDone,1 */
      Dcm_Dsp_ReadDataByPeriodicIdentifier_RemoveSample(TxIdx, DCM_PDID_TRANSRING_REMOVE_FIRST);
    }
    else
    {
  ...
  do
  {
        ...
        )
    {
      ...
      if (Dcm_PeriodicDIDContext.SchedulerTable[IndexSchedTable].PeriodicTimer == 0U)
      {
        ...
        /* !LINKSTO Dcm.EB.ReadDataByPeriodicIdentifier.SessionAndSecurity,1 */
           ...
           )
        {
          ...
          /* !LINKSTO Dcm.EB.ReadDataByPeriodicIdentifier.SameTimeWithReadDID,1 */
...
#endif /* #if (DCM_DSP_SERVICE_0X22_ASYNC == STD_ON) */
          {
            ...
            * but the lower 8 bits of the periodic DID */
            ...
            if (E_OK == serviceProcessStatus)
            {
              ...
              /* !LINKSTO Dcm.EB.ReadDataByPeriodicIdentifier.PDID.Sampling.OrderOfTransmission,1 */
  ...
  )
{
  ...
  for (IndexPDIDs = 0U; IndexPDIDs < NumberOfPDIDs; IndexPDIDs++)
  {
    ...
    /* !LINKSTO Dcm.ReadDataByPeriodicIdentifier.Dsn.SchedEntry,1 */
...
STATIC FUNC(uint8, DCM_CODE) Dcm_Dsp_ReadDataByPeriodicIdentifier_RemoveDuplicates(
  ...
  uint8 NumberOfPDIDs)
{
  /* !LINKSTO Dcm.Service.ReadDataByPeriodicIdentifier.ReqDuplicateDid,1 */
...
STATIC FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_ReadDataByPeriodicIdentifier_ValidateOnePDID(
  ...
  boolean InitialPDIDConfig)
{
  /* !LINKSTO Dcm.Service.ReadDataByPeriodicIdentifier.SecLvlNotOk,1 */
  /* !LINKSTO Dcm.Service.ReadDataByPeriodicIdentifier.ModeRuleNotOk,1 */
  ...
  uint8 NumberOfPDIDs)
{
  ...
  for (DidIdx = 0U; DidIdx < NumberOfPDIDs; DidIdx++)
  {
    ...
    else
    {
      ...
      /* If the attempt to read Periodic DID during the ReadDataByPeriodicIdentifier request
         results in PENDING, the DID is excluded from periodic reporting.
         DIDs that cannot be read in the current session are ignored also. */
    }
  }

  /* See if no PDID in this request message can be read.
   * There should be at least one DID in the request message that can be read */
  /* !LINKSTO Dcm.Service.ReadDataByPeriodicIdentifier.SessionNotOk,1 */
  /* !LINKSTO Dcm.EB.ReadDataByPeriodicIdentifier.DIDConfigured.UseBlockId.NRC,1 */
