  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  ...
  if (pMsgContext->reqDataLen == 1U)
  {
    ...
    Dcm_MsgLenType resLength = pMsgContext->resMaxDataLen - 1U; /* one byte is reserved for service ID */

    serviceProcessStatus = Dcm_Dsl_RoeServices_ReportActivatedEvents(&(pMsgContext->resData[1U]),
                                                                     &resLength);
    ...
    {
      /* !LINKSTO Dcm.EB.ROE.ReportActivatedEvents.BufferFull.TransmitNRC0x14,1 */
