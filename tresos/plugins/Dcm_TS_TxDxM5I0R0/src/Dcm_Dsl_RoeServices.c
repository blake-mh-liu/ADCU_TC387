  ...
  Dcm_SesCtrlType OldSession, Dcm_SesCtrlType NewSession)
{
  ...
  DBG_DCM_DSL_ROESERVICES_SESSIONCHANGE_ENTRY(OldSession, NewSession);
    ...
    /* !LINKSTO Dcm.EB.DefaultSessionToNondefaultSessionChange.StopROE,1 */
    /* !LINKSTO Dcm.EB.NondefaultSessionToNondefaultSessionChange.OldSessionNewSessionSame.StopROE,1 */
    /* !LINKSTO Dcm.EB.NondefaultSessionToNondefaultSessionChange.OldSessionNewSessionDifferent.StopROE,1 */
    /* !LINKSTO Dcm.EB.NondefaultSessionToDefaultSessionChange.StopROE,1 */
  if ( ( (OldSession != NewSession) ||
         (NewSession != DCM_DEFAULT_SESSION) )&&
       (TRUE == Dcm_Dsl_RoeServices_Initialized) )
  {
...
FUNC(void, DCM_CODE) Dcm_Dsl_RoeServices_TriggerOnDTCStatus(uint8 DTCStatusOld, uint8 DTCStatusNew)
{

  DBG_DCM_DSL_ROESERVICES_TRIGGERONDTCSTATUS_ENTRY(DTCStatusOld, DTCStatusNew);
  if (TRUE == Dcm_Dsl_RoeServices_Initialized)
  {
    uint8 PreconfIdx;
    /* !LINKSTO SWS_Dcm_00601,1 */
        ...
        (Dcm_RoeInterMsgCycle == 0U))
    {
      ...
      if(((DTCStatusNew ^ DTCStatusOld) & DTCStatusNew & Dcm_RoeEventPreconfList[PreconfIdx].DTCStatusMask) != 0U)
      {
        Dcm_Dem_RoeServices_CommonTriggerProcedure(DCM_ROE_EVENT_DTC);
      }
    }
  }
  DBG_DCM_DSL_ROESERVICES_TRIGGERONDTCSTATUS_EXIT(DTCStatusOld, DTCStatusNew);
}
...
FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsl_RoeServices_ReportActivatedEvents(
    Dcm_MsgType ResData, P2VAR(Dcm_MsgLenType, AUTOMATIC, DCM_VAR) MsgLen)
{
  ...
  Std_ReturnType SubServiceProcessStatus = DCM_E_OK;
  Dcm_RoeEventTypeType EventType;

  DBG_DCM_DSL_ROESERVICES_REPORTACTIVATEDEVENTS_ENTRY(ResData, MsgLen);

  for (EventType=DCM_ROE_EVENT_DTC; (EventType<DCM_ROE_EVENT_MAX) && (SubServiceProcessStatus == DCM_E_OK); ++EventType)
  {
    if (Dcm_Dsl_RoeServices_Runtime_Data[EventType].State == DCM_ROE_STARTED)
    {
      ...
      switch (event.EventType)
      {
        case DCM_ROE_EVENT_DID:
          ...
          if (offset + 6U < *MsgLen) /* 7 bytes can be written to the response buffer in case of DID event */
          {
          ...
          {
            SubServiceProcessStatus = DCM_E_ABORT;
          }
          break;
        case DCM_ROE_EVENT_DTC:
          /* eventType (subFunction) */
          /* Since DTC event length is 5 bytes and min buffer size is 8 bytes, response always
             fits into the buffer for DTC event because it is first in the list of events */
  ...
  *MsgLen = offset;
  ...
  DBG_DCM_DSL_ROESERVICES_REPORTACTIVATEDEVENTS_EXIT(SubServiceProcessStatus, ResData, MsgLen);
  return SubServiceProcessStatus;
}
  ...
  if(DTCEventPreconfigured == TRUE)
  {
         ...
         ++PreconfEventWindowIndex)
    {
      if( EventWindowTime == Dcm_RoeEventWindowTimePreconfList[PreconfEventWindowIndex].EventWindowTime)
      {
        ...
        if ((Dcm_RoeEventWindowTimePreconfList[PreconfEventWindowIndex].StorageState == TRUE) &&
            (EventWindowTime == DCM_ROE_EVENT_WINDOW_CURRENT_CYCLE) &&
            (StoreEvent == TRUE))
        {
          /* !LINKSTO SWS_Dcm_01076.OnDTCStatusChange.StorageStateSet.EventWindowTimeCurrentCycle.DcmDspRoeStorageStateTrue,1 */
          RetVal = DCM_E_REQUESTOUTOFRANGE;
        }
#if (DCM_ROE_PERSISTENCE == STD_ON)
        /* Overwrite the storage parameter, if needed. */
        /* !LINKSTO Dcm.EB.ECUC_Dcm_00983,1 */
        /* !LINKSTO SWS_Dcm_01076.OnDTCStatusChange.StorageStateSet.EventWindowTimeCurrentCycle.DcmDspRoeStorageStateFalse,1  */
    ...
    /* Event Window has been found and  */
