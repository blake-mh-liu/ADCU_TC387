...
/* patch version check */
#if (FRSM_SW_PATCH_VERSION < 27U)
#error FRSM_SW_PATCH_VERSION wrong (< 27U)
#endif
...
STATIC FUNC(void,FRSM_CODE) FrSM_FE_DEM_SYNC_LOSS_Func
(
    CONSTP2CONST(FrSM_ClstCfgType,AUTOMATIC,FRSM_APPL_CONST) pClstCfg
);

/**
 * \brief Functional element FE_DEM_SYNC_LOSS of state machine
 *
...
#define FrSM_FE_DEM_SYNC_LOSS FrSM_FE_DEM_SYNC_LOSS_Func
...
STATIC FUNC(void,FRSM_CODE) FrSM_FE_DEM_SYNC_LOSS_PASSED_Func
(
    CONSTP2CONST(FrSM_ClstCfgType,AUTOMATIC,FRSM_APPL_CONST) pClstCfg
);
...
#define FrSM_FE_DEM_SYNC_LOSS_PASSED FrSM_FE_DEM_SYNC_LOSS_PASSED_Func
...
#if (FRSM_FRIF_GETWAKEUPRXSTATUS_ENABLE == STD_ON)
...
#endif /* FRSM_FRIF_GETWAKEUPRXSTATUS_ENABLE == STD_ON */
    ...
    )
{
    ...
    else
    {
        ...
        {
        /* No possibility to check if a Wakeup is received on Channel B in case that
         * FrIf_GetWakeupRxStatus is deactivated in FrIf
         */
#if (FRSM_FRIF_GETWAKEUPRXSTATUS_ENABLE == STD_ON)
