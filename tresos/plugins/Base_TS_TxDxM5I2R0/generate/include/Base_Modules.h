...
[!/* check if the module shall be initialized by the PbcfgM */!][!//
    [!IF "count(as:modconf('PbcfgM')[1]/PbcfgMBswModules/eb-list::*[node:refvalid(PbcfgMBswModuleRef) and (as:path(as:ref(PbcfgMBswModuleRef)) = $InModuleConfigPath)])>0"!][!//
...
/** \brief Address of the first multiple configuration container for [!"$moduleName"!] ([!"$InModuleConfigPath"!]) */
    [!IF "@type = 'MAP'"!]
      [!VAR "configShortName"="name(eb-list::*[1])"!][!//
    [!ELSE!][!//
...
[!/* special handling for Xfrm and Cdd modules */!][!//
  ...
  [!IF "($moduleName = 'Xfrm') or ($moduleName = 'Cdd')"!][!//
    ...
    [!ELSEIF "contains($moduleVariant, 'TS_')"!][!//
      [!VAR "apiServicePrefix"="text:split($moduleVariant, '_')[last()]"!][!//
...
[!LOOP "util:distinct(node:order(as:modconfs(), 'text:split(as:path(as:dtos(.)), &apos;/&apos;)[position()=last()]'))"!]
...
[!/* check if there is an implementationConfigVariant element. - If not fall back to
  ...
  [!ENDIF!]
  [!VAR "ConfigCount"="count(*[node:getSpecialDataEntryAsBoolean(node:dtos(.), 'EB-TRESOS/CUSTOM-ATTRIBUTES', 'MULTIPLE-CONFIGURATION-CONTAINER') = true()])"!]
  [!IF "$ConfigCount > 0 "!]
      [!SELECT "*[node:getSpecialDataEntryAsBoolean(node:dtos(.), 'EB-TRESOS/CUSTOM-ATTRIBUTES', 'MULTIPLE-CONFIGURATION-CONTAINER') = true()]"!]
  ...
  [!ELSE!]
    [!/* select the multiple configuration container of the module if there is any */!][!//
      [!SELECT "*[node:dtos(.)/@type = 'MULTIPLE-CONFIGURATION-CONTAINER']"!]
        ...
        [!IF "(count(eb-list::*)>1) and (@type = 'MAP')"!]
