...
TRESOS2_GENERATOR     := $(if $(IS_DOS_SHELL),call ,)$(TRESOS_BIN)/tresos_cmd.$(if $(IS_DOS_SHELL),bat,sh)

#################################################################
# path of folder for plugin configuration (optional)
TRESOS2_CONFIGURATION_FOLDER ?=
...
STUB_VERSIONS		 ?= 40 41 42 43 44 45 46 11
$(eval TRESOS2_CONF_STUBS_IDS = $(foreach m,$(TRESOS2_CONF_STUBS),$$(addprefix $(m)_,$$(firstword $($(m)Stub_VARIANT) $(patsubst %,$$($(m)Stub_%_VARIANT),$(STUB_VERSIONS))))))
...
ifeq (,$(TRESOS2_PROJECT_NAME))
  ...
  # the order of adding the lists into the list with all files is essential for the legacy generation!
  # - first the static sysd files of the project
  # - second the generated sysd files
  # - third the provided generate_swcd sysd files of the project
  # - last any other provided sysd files
...
ifeq ($(TRESOS2_GENERATOR_MODE),CALL_GEN)
  ...
  ifneq (,$(TRESOS2_USE_TMP_WORKSPACE))
    ...
    ifeq (,$(TRESOS2_PROJECT_NAME))
      TRESOS2_WORKSPACE_DIR := $(TRESOS2_OUTPUT_PATH)/.ws
    else
      TRESOS2_WORKSPACE_DIR := $(dir $(PROJECT_ROOT)).$(PROJECT_NAME)
