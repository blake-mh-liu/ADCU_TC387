...
ifneq (,$(USERBUILD_SHELL))
  ...
  # for indirectly recursive make calls (via scripts)
...
# verify Make version
ifneq (4.3,$(firstword $(sort 4.3 $(MAKE_VERSION))))
  $(error make version $(MAKE_VERSION) detected! expected Make version >= 4.3)
endif
...
# 'constant' macros for special characters (commented are still unused, but reserved)
override EMPTY		:=
override SPACE		:= $(EMPTY) $(EMPTY)
override BLANK		:= $(SPACE)
override COMMA		:= ,
override COLON		:= :
override SEMICOL	:= ;
override DOLLAR		:= $$
...
override LPAREN		:= (
override RPAREN		:= )
# deprecated / in quarantine: conflicting with previous mis-naming / not used, yet
override LBRACK		 = [$(error Use of outdated LBRACK for $(RPAREN) - use LPAREN)
override RBRACK		 = ]$(error Use of outdated RBRACK for $(RPAREN) - use RPAREN)
#override LBRACE	:= {
#override RBRACE	:= }
...
override GEN_OUTPUT_PATH	:= $(call cygpath_u,$(subst \,/,$(GEN_OUTPUT_PATH)))
...
TRESOS_BIN		:= $(TRESOS_BASE)/bin
ifneq (,$(IS_DOS_SHELL))
  # re-normalize: one of the few paths in backslash-notation, because it's solely used to execute the tresos_cmd.bat
  TRESOS_BIN		:= $(subst /,\,$(abspath $(subst \,/,$(TRESOS_BIN))))
endif
...
ifeq (,$(IS_DOS_SHELL))
  ...
  cygpath_m		= $(foreach p,$1,$(if $(findstring $(MOUNT),$p),$(call _cygpath_m,$(subst /, ,$p)),$p))
  fixCmdPaths		= $(subst /,\,$(call cygpath_m,$1))
  ...
  _MOVE_EXE		 = $(_MV_EXE)$(error DEPRECATED use of _MOVE_EXE - use MV instead)
  MV			:= $(_MV_EXE) -f
  MOVE			 = $(MV)$(error DEPRECATED use of MOVE - use MV instead)
...
# DEPRECATED
  ...
  # ... to be fixed
  COPY			 = $(CP)$(error DEPRECATED use of COPY - use CP instead)
  RMTREE		:= $(_RM_EXE) -Rf
  # deprecated - use RMTREE instead!
  RMDIR			 = $(RMTREE)$(error DEPRECATED use of RMDIR - use RMTREE instead)
...
# add autosar version conversion postfix to non-XDM files for legacy convert
get_legacy_convert_target = $(1)$(if $(findstring .xdm,$(1)),,@asc:4.4.0)
