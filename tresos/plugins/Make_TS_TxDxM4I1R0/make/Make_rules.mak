...
ASM_FILES_TO_BUILD	:= $(call cygpath_m,$(call uniq,$(subst \,/,$(ASM_FILES_TO_BUILD))))
CC_FILES_TO_BUILD	:= $(call cygpath_m,$(call uniq,$(subst \,/,$(CC_FILES_TO_BUILD))))
CC_INCLUDE_PATH		:= $(call cygpath_m,$(call uniq,$(subst \,/,$(CC_INCLUDE_PATH))))
LIB_INCLUDE_PATH	:= $(call cygpath_m,$(call uniq,$(subst \,/,$(LIB_INCLUDE_PATH))))
USER_INCLUDE_PATH	:= $(call cygpath_m,$(call uniq,$(subst \,/,$(USER_INCLUDE_PATH))))
LOCAL_INC_DIR		:= $(call cygpath_m,$(call uniq,$(subst \,/,$(LOCAL_INC_DIR))))
...
ALL_INC_DIR		 = $(addprefix -I,$(call cygpath_m,$(call uniq,$(realpath $(USER_INCLUDE_PATH) $(GEN_INC_DIR) $(filter-out $(GEN_INC_DIR), $(LOCAL_INC_DIR) $(CC_INCLUDE_PATH))))))
HDR_PATH		 = $(call cygpath_m,$(USER_INCLUDE_PATH) $(filter-out $(USER_INCLUDE_PATH),$(CC_INCLUDE_PATH)))
...
OUTPUT_PATH_LIBS	 = $(patsubst %,$(LIB_DIR)/$(LIB_FILE_PREFIX)%.$(LIB_FILE_SUFFIX),$(LIBRARIES_TO_BUILD))
...
ifneq (,$(GLOBAL_LIB_ENABLE))
  ...
  STATIC_LIBS_TO_BUILD		:= $(filter-out $(basename $(notdir $(realpath $(patsubst %,$(GLOBAL_LIB_DIR)/$(LIB_FILE_PREFIX)%.$(LIB_FILE_SUFFIX),$(STATIC_LIBS))))),$(STATIC_LIBS))
else
...
ifeq (,$(GLOBAL_LIB_ENABLE))
	...
	$(if $(findstring $(basename $(@F)), $(SRC_LIBS_TO_BUILD) $(PBCONFIG_LIBS_TO_BUILD)),\
          ...
          $(if $(realpath $(GLOBAL_LIB_DIR)/$(@F)),\
            $(info Using global lib $(@F))$(CP) $(GLOBAL_LIB_DIR)/$(@F) $@ >$(DEVNULL),\
            $(build-lib-file)))
	$(if $(findstring $(basename $(@F)), $(STATIC_LIBS)),\
          $(if $(realpath $(GLOBAL_LIB_DIR)/$(@F)),\
            ,\
            $(CP) $@ $(GLOBAL_LIB_DIR)/$(@F).tmp.$(PROJECT_NAME) >$(DEVNULL) &&\
            $(call ifnotexist,$(info Creating global lib $(@F))$(GLOBAL_LIB_DIR)/$(@F),$(MV) $(GLOBAL_LIB_DIR)/$(@F).tmp.$(PROJECT_NAME) $(GLOBAL_LIB_DIR)/$(@F) >$(DEVNULL))))
endif
...
$(SRC_LIST): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file > $(SRC_LIST),$(subst $(BLANK),$(\n),$(UNIQUE_SOURCES)))

$(HDR_LIST): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file > $(HDR_LIST),$(subst $(BLANK),$(\n),$(HDR_PATH)))

check_dups: $(SRC_LIST) $(HDR_LIST)
	$(EB_VERBOSE)$(PERL) $(CHECK_DUPS) --hdr=$(HDR_LIST) --src=$(SRC_LIST) && $(ECHO) === Check finished ===
...
# each option is put on its own line
_DEPEND_INC ?= $(filter -D%, $(CC_OPT)) $(cppGetPreProcessorDefines) $(ALL_INC_DIR)

$(DEPEND_INC): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file >$@,$(subst $(BLANK)-,$(\n)-,-D_EMPTY $(_DEPEND_INC)))

_MAKE_INC_DEFAULT ?= $(CC_OPT) $(call cppGetPreProcessorDefines) $(ALL_INC_DIR)

$(MAKE_INC_DEFAULT): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file >$@,$(subst $(BLANK)-,$(\n)-,-D_EMPTY $(_MAKE_INC_DEFAULT)))

_MAKE_INCPP ?=$(CPP_OPT) $(call cppGetPreProcessorDefines) $(ALL_INC_DIR)

$(MAKE_INCPP): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file >$@,$(subst $(BLANK)-,$(\n)-,-D_EMPTY $(_MAKE_INCPP)))

_ASM_INC ?= $(ASM_OPT) $(PFLAGS) $(call cppGetPreProcessorDefines) $(ALL_INC_DIR)

$(ASM_INC): $(SOURCE_MAKEFILES)
	$(info Building $(@F))
	$(file >$@,$(subst $(BLANK)-,$(\n)-,-D_EMPTY $(_ASM_INC)))

_LNK_INC ?= $(LINK_OPT) $(addprefix -L,$(LIB_INCLUDE_PATH)) $(call cygpath_m,$(LINK_ONLY_OBJ) $(OBJECTS_LINK_ONLY) $(OUTPUT_PATH_LIBS))
...
_LIB_TO_OBJ ?= $(foreach LIB, $(TOTAL_LIBS_TO_BUILD)\
		,$(\n)$$(LIB_DIR)/$(LIB_FILE_PREFIX)$(LIB).$(LIB_FILE_SUFFIX):\
...
$(call log_verbose,variables dump:$(foreach v,\
 ...
 ,$(\n)# $(v)=$(subst $(PLUGINS_BASE),PLUGINS_BASE,$(subst $(PROJECT_ROOT),PROJECT_ROOT,$(subst $(SPACE),$(\n)#$(\t), $($(v))))))$(\n))
...
ifneq (,$(filter single_lib%,$(MAKECMDGOALS)))
  ...
  SINGLE_LIB_FILE_NAME		 = $(LIB_DIR)/$(LIB_FILE_PREFIX)$(SL).$(LIB_FILE_SUFFIX)
