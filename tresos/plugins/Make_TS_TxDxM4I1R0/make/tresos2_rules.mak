...
# newer target used for ACG9
tresos_generate: 01_tresos2_call_generator
...
TRESOS2_CONTINUE_ON_ERROR ?= false
...
ifeq (,$(filter-out false,$(TRESOS_EARLY_BATCH_GENERATE)))
  ifeq (,$(TRESOS2_PROJECT_NAME))
	...
	-$(EB_VERBOSE)$(TRESOS2_GENERATOR) $(call cygpath_m,$(if $(TRESOS2_CONFIGURATION_FOLDER),-configuration $(TRESOS2_CONFIGURATION_FOLDER) )$(TRESOS2_WORKSPACE_OPTION) batch $(if $(filter true,$(TRESOS2_CONTINUE_ON_ERROR)),-continueOnError) $(TRESOS2_BATCH_FILE))
  else
	...
	$(if $(findstring $(TRESOS2_WORKSPACE_DIR)/,$(TRESOS2_OUTPUT_PATH)),,& $(MV) -v $(TRESOS2_WORKSPACE_DIR) $(TRESOS2_OUTPUT_PATH))\
	& exit 1 )
  endif
endif
...
ifeq (,$(filter-out false,$(TRESOS_EARLY_BATCH_GENERATE)))
  ifneq (,$(TRESOS2_PROJECT_NAME))
	...
	-$(EB_VERBOSE)$(RMTREE) $(TRESOS2_WORKSPACE_OUT) >$(DEVNULL) 2>&1 \
	 && $(call ifexist,$(TRESOS2_WORKSPACE_OUT), $(RMTREE) $(TRESOS2_WORKSPACE_OUT) >$(DEVNULL) 2>&1 ) \
	 && $(MV) -v $(TRESOS2_WORKSPACE_DIR) $(TRESOS2_OUTPUT_PATH)
    endif
  endif
...
TRESOS2_OPTIONS				:=$(call cygpath_m,$(TRESOS2_OPTIONS))
TRESOS2_SWCD_OPTIONS			:=$(call cygpath_m,$(TRESOS2_SWCD_OPTIONS))
TRESOS2_GENERATE_SWCD_MODE_OPTION	:=$(call cygpath_m,$(TRESOS2_GENERATE_SWCD_MODE_OPTION))
TRESOS2_USER_OPTIONS			:=$(call cygpath_m,$(TRESOS2_USER_OPTIONS))
TRESOS2_CONVERT_OPTIONS			+= -DValidate=FALSE

ifeq (,$(TRESOS2_PROJECT_NAME))
  ################################################################
  # batch commands for legacy mode tests
  LEGACY_BATCH_MERGE_XDM         = \
	$(TRESOS2_CONVERT_OPTIONS) $(TRESOS2_OPTIONS) legacy convert $(call cygpath_m,$(TRESOS2_CONF_FILES) $(TRESOS2_MERGED_XDM_FILENAME))
  LEGACY_BATCH_CONVERT           =\
	$(TRESOS2_CONVERT_OPTIONS) $(TRESOS2_OPTIONS) legacy convert $(call cygpath_m,$(TRESOS2_SYSD_ALL_FILES) $(TRESOS2_SYSTEM_MODEL40_FILENAME))
  LEGACY_BATCH_GENERATE_SWCD     = \
	...
	legacy make $(TRESOS2_GENERATE_SWCD_MODE_OPTION) $(call cygpath_m,$(TRESOS2_SYSD_GENSWCD_FILES) $(TRESOS2_CONF_INPUT_FILES))\
	$(PLUGINS_TO_GENERATE:%=-g %) -o $(call cygpath_m,$(TRESOS2_OUTPUT_PATH)) -u
  LEGACY_BATCH_GENERATE_CONTRACT =\
	-DEcuResourceModuleIds=$(TRESOS2_CONF_ECU_RESOURCEMODULE_IDS) -DValidate=TRUE -DvalidationVariant=standard -Dosek.orti=TRUE\
	-Dosek.html=FALSE -Dtarget=$(CPU) -Dderivate=$(DERIVATE) -Decuid=EB_legacy_ECU -Dproject=$(PROJECT_NAME)\
	$(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) -Dcontract.merge=true\
	legacy make contract $(call cygpath_m,$(TRESOS2_SYSD_ALL_FILES)) -g Rte_$(Rte_VARIANT)\
	$(PLUGINS_TO_GENERATE:%=-g %) -o $(call cygpath_m,$(TRESOS2_OUTPUT_PATH)/include) -u
  LEGACY_BATCH_GENERATE          =\
	...
	legacy make generate $(call cygpath_m,$(TRESOS2_SYSTEM_MODEL_INPUT_FILES) $(TRESOS2_CONF_INPUT_FILES))\
	$(PLUGINS_TO_GENERATE:%=-g %) -o $(call cygpath_m,$(TRESOS2_OUTPUT_PATH)) -u
  ...
  PROJECT_BATCH_IMPORT_PROJECT	= $(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) importProject $(call cygpath_m,$(PROJECT_ROOT))
  PROJECT_BATCH_GENERATE_SWCD	= $(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) make generate_swcd $(call cygpath_m,$(TRESOS2_PROJECT_NAME))
  PROJECT_BATCH_MERGE_BSWMD	= $(TRESOS2_CONVERT_OPTIONS) $(TRESOS2_OPTIONS) legacy convert $(call cygpath_m,$(TRESOS2_MERGE_BSWMD_FILES) $(call get_legacy_convert_target,$(lastword $(TRESOS2_MERGE_BSWMD_FILES))))
  PROJECT_BATCH_GENERATE	= $(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) make generate $(TRESOS2_PROJECT_NAME) $(PLUGINS_TO_GENERATE:%=-g %)
  PROJECT_BATCH_VERIFY		= $(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) make $(call cygpath_m,$(TRESOS2_VERIFY_TARGET)) $(TRESOS2_PROJECT_NAME)
  PROJECT_BATCH_DELETE		= $(TRESOS2_OPTIONS) $(TRESOS2_USER_OPTIONS) deleteProject $(call cygpath_m,$(PROJECT_ROOT))
...
define backup_emitter
	$(call ifnotexist,$(1).orig, $(EB_VERBOSE)$(CP) $(1) $(1).orig > $(DEVNULL))
...
$(\n)-DMapOptionalAsList=false $(TRESOS2_CONVERT_OPTIONS) $(TRESOS2_OPTIONS) legacy convert $(call cygpath_m,$(TRESOS2_$(plugin)_CONF_FILES) $(call get_legacy_convert_target,$(firstword $(TRESOS2_$(plugin)_CONF_FILES)))))))
...
# however MOVE doesn't work on linux, and our GNU tools collection was updated

01_revert_merged_xdm:
	$(foreach xdmfile, $(wildcard $(PROJECT_ROOT)/config/*.xdm.orig), \
	  $(call SC_CMD_AR_HELPER, \
	    -$(EB_VERBOSE)$(MV) -f $(xdmfile) $(basename $(xdmfile)) > $(DEVNULL) \
	))
...
ifneq (,$(filter-out false,$(TRESOS_EARLY_BATCH_GENERATE)))
  ...
  _listsplit=$(if $(2),$(2) $(call _listsplit,$(1),$(filter-out $(1),$(firstword $(3))),$(wordlist 2,100,$(3))))
  # 1: split-token  3: rest of list
  # split-token is included in the returned sub-list
  listsplit=$(call _listsplit,$(1),$(filter-out $(1),$(firstword $(2))),$(wordlist 2,100,$(2)))$(1)
  SORTED_MAKE_EARLY_GENERATE_RULES:= $(call listsplit,01_tresos2_call_generator,$(SORTED_MAKE_GENERATE_RULES))
...
$(MAKE_INFO_FILE): $(GOALS_GENERATE)

$(GOALS_BUILD): $(MAKE_INFO_FILE)

$(GOALS_BUILD) $(GOALS_RUN): $(PROJECT_BINARY) $(MAKE_INFO_FILE) # $(SORTED_MAKE_GENERATE_RULES)

$(info DIRECT: MAKECMDGOALS=$(MAKECMDGOALS)		GOALS_BUILD=$(GOALS_BUILD)		GOALS_RUN=$(GOALS_RUN) DIRECT=$(DIRECT)!)

    endif #ifneq (,$(GOALS_BUILD))
  endif

  # it's not the fast case: switch this make to sequential 'generate', defer all other targets to a recursive make
  ifeq (,$(DIRECT))
$(info tresos2/indirect: $(foreach v,GOALS_GENERATE GOALS_BUILD GOALS_RUN DIRECT, $(v)= $($(v))))
...
#all $(filter postbuild,$(MAKECMDGOALS)): recurse
    GOALS_RECURSE:= $(GOALS_BUILD) $(GOALS_RUN)
    MAKECMDGOALS:= recurse $(filter-out $(GOALS_RECURSE),$(MAKECMDGOALS))
    # remove delegated targets
    GOALS_BUILD:=
    GOALS_RUN:=
# but make them triggers
$(GOALS_RECURSE): recurse

recurse: $(GOALS_GENERATE)
	$(info +$(MAKE) -f $(firstword $(MAKEFILE_LIST)) SHELL=$(USERBUILD_SHELL) $(USR_MAKEFLAGS) GOALS_RECURSE=$(GOALS_RECURSE))
	+$(MAKE) -f $(firstword $(MAKEFILE_LIST)) SHELL=$(USERBUILD_SHELL) $(USR_MAKEFLAGS) $(GOALS_RECURSE) $(if $(filter 1%,$(IS_AN_AUTOMATIC_TEST)),,-j$(NUMBER_OF_PROCESSORS))

$(info RECURSIVE: MAKECMDGOALS=$(MAKECMDGOALS)		GOALS_BUILD=$(GOALS_BUILD)		GOALS_RUN=$(GOALS_RUN) DIRECT=$(DIRECT)!)

  endif #ifneq (,$(DIRECT))
endif #ifneq (,$(GOALS_BUILD)$(GOALS_RUN))
