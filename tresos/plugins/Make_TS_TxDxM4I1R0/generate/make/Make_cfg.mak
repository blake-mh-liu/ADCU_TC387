...
[!LOOP "text:order(text:split($enabledModules))"!]
  ...
  [!VAR "pluginPath" = "util:relativePath($tresosBase, asc:getPluginLocationById(.), '$(TRESOS_BASE)/')"!]
  [!IF "$pluginPath = ''"!]
    [!VAR "pluginName"!][!"$moduleName"!]_[!CALL "GetModuleVariant", "PluginName"="."!][!ENDVAR!]
    [!VAR "pluginPath" = "util:relativePath($tresosBase, asc:getPluginLocationById($pluginName), '$(TRESOS_BASE)/')"!]
  [!ENDIF!]
  [!IF "$moduleVariant != ''"!]
...
[!"$moduleName"!]_CORE_PATH :=[!WS!][!"$pluginPath"!]
  [!ENDIF!]
[!ENDLOOP!]
