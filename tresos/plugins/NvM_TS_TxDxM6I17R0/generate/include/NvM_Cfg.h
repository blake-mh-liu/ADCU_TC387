...
/* !LINKSTO NvM.SWS_NvM_00030,1 */
[!INCLUDE "NvM_Checks.m"!][!//
#ifndef NVM_CFG_H
#define NVM_CFG_H
/*==================[inclusions]============================================*/

#include <NvM_Types.h>        /* contains NvM specific types*/
#include <Std_Types.h>
[!IF "(count(as:modconf('NvM')[1]/NvMCommon/NvMEcucPartitionRef/eb-list::*) > 1)"!]
/* Os support is needed for OsApplication symbolic names */
#include <Os.h>
[!ENDIF!]
...
[!LOOP "NvMBlockDescriptor/eb-list::*[(NvMNvramBlockIdentifier > 1) and (NvMNvBlockNum > 1)]"!][!/*
*/!][!IF "NvMNvramBlockIdentifier = '1'"!][!/*
*/!][!VAR "Block1Size"="NvMNvBlockLength"!][!/*
*/!][!ENDIF!][!/*
*/!][!VAR "blockDesc"="NvMNvBlockNum"!][!/*
*/!][!IF "(node:exists(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfFeeBlock))"!][!/*
*/!][!VAR "blockNumber"="node:ref(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfFeeBlock)/FeeBlockNumber"!][!/*
*/!][!ELSEIF "(node:exists(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfEaBlock))"!][!/*
*/!][!VAR "blockNumber"="node:ref(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfEaBlock)/EaBlockNumber"!][!/*
*/!][!ENDIF!][!/*
*/!][!FOR "I"="0" TO "$blockDesc - 1"!][!/*
*/!][!IF "(node:exists(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfFeeBlock)) and
          (count(node:ref(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfFeeBlock)/../../FeeBlockConfiguration/eb-list::*[FeeBlockNumber = ($blockNumber + $I)]) != 1)"!][!/*
*/!][!ERROR!]There should exist exactly one block in Fee with Block number = [!"num:i($blockNumber + $I)"!] for [!"node:name(.)"!][!ENDERROR!][!/*
*/!][!ELSEIF "(node:exists(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfEaBlock)) and
              (count(node:ref(NvMTargetBlockReference/eb-choice::*[1]/NvMNameOfEaBlock)/../../EaBlockConfiguration/eb-list::*[EaBlockNumber = ($blockNumber + $I)]) != 1)"!][!/*
...
[!IF "node:existsAndTrue(FeeDynamicBlockLength)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
[!ENDSELECT!]

#if (defined NVM_BSW_DISTRIBUTION_ENABLED)
#error NVM_BSW_DISTRIBUTION_ENABLED is already defined
#endif
/** \brief Macro to enable Bsw distribution
 **/
/* !LINKSTO NvM.BswDistribution.OnePartition,1 */
#define NVM_BSW_DISTRIBUTION_ENABLED     [!//
[!IF "(count(NvMCommon/NvMEcucPartitionRef/eb-list::*) > 1)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!][!/*

*/!][!IF "(count(NvMCommon/NvMEcucPartitionRef/eb-list::*) > 1)"!]

#if (defined NVM_NUMBER_OF_SATELLITE_PARTITIONS)
#error NVM_NUMBER_OF_SATELLITE_PARTITIONS is already defined
#endif
/** \brief Macro to define the number of partitions NvM is distributed on
 **/
#define NVM_NUMBER_OF_SATELLITE_PARTITIONS    [!//
[!"num:i(count(NvMCommon/NvMEcucPartitionRef/eb-list::*) - 1)"!]U

#if (defined NVM_OS_APPLICATION_ID_MASTER)
#error NVM_OS_APPLICATION_ID_MASTER is already defined
#endif
/** \brief Macro to define the master partition id
 **/
#define NVM_OS_APPLICATION_ID_MASTER    (os_applicationid_t)[!//
[!"name(as:modconf('Os')[1]/OsApplication/eb-list::*[OsAppEcucPartitionRef = as:modconf('NvM')[1]/NvMCommon/NvMMasterEcucPartitionRef])"!]
...
#define NVM_MEMACC_USAGE                [!//
[!IF "node:existsAndTrue(NvMCommon/NvMMemAccUsage)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 **/
#define NVM_BUFFER_ALIGNMENT_BITS [!//
[!IF "node:exists(NvMCommon/NvMBufferAlignmentValue)"!][!//
[!"substring-before(substring-after(NvMCommon/NvMBufferAlignmentValue, 'ALIGN_'), '_BITS')"!]U
[!ELSE!][!//
8U
[!ENDIF!][!//
...
[!IF "node:existsAndTrue(NvMCommon/NvMDevErrorDetect)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMRteUsage)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(NvMCommon/NvMSetRamBlockStatusApi)) and (count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBlockUseSetRamBlockStatus))]) != 0)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

/** \brief Macro to check if Block Checks is configured for any block
 **/
#define NVM_BLOCK_CHECK_ENABLED     [!//
[!IF "(count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMEnBlockCheck))]) != 0)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

/** \brief Number of blocks configured with Block Check.
 **/
#define NVM_BC_NR_OF_BLOCKS     [!"num:i(count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMEnBlockCheck))]))"!]U

 /** \brief Number of blocks configured with Block Check of type REDUNDANT.
**/
#define NVM_BC_NR_OF_BLOCKS_REDUNDANT     [!"num:i(count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMEnBlockCheck)) and (NvMBlockManagementType  = 'NVM_BLOCK_REDUNDANT')]))"!]U
...
[!IF "(count(NvMBlockDescriptor/eb-list::*) != num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier)) and (num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier) div (count(NvMBlockDescriptor/eb-list::*) + 1) <= 10)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

/** \brief Macro to check if Block Id-s are consecutive. Otherwise a binary search is needed
 **/
#define NVM_BLOCK_INDEX_SEARCH     [!//
[!IF "(num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier) div (count(NvMBlockDescriptor/eb-list::*) + 1) > 10)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

/** \brief Macro to check if Block Checks Set API is configured for any block
 **/
#define NVM_BLOCK_CHECK_SET_API_ENABLED     [!//
[!IF "(count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBcEnSetAPI))]) != 0)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMCancelInternalOperations)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMVersionInfoApi)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
/** \brief Disables or enables the API NvM_VersionInfoApi()
 **/
#define NVM_CLEAR_RAMBLOCK_AFTER_RESET             [!//
[!IF "node:existsAndTrue(NvMCommon/NvMResetRamBlockAfterReset)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "num:i(count(NvMBlockDescriptor/eb-list::*[node:existsAndTrue(NvMAdvancedRecovery)])) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMEnableLegacySymbolicNames)"!][!//
...
#if (defined NvMConf_NvMBlockDescriptor_[!"as:name(.)"!])
#error NvMConf_NvMBlockDescriptor_[!"as:name(.)"!] already defined
#endif
/** \brief Symbolic name for Block [!"NvMNvramBlockIdentifier"!] */
#define NvMConf_NvMBlockDescriptor_[!"as:name(.)"!]   [!"NvMNvramBlockIdentifier"!]U
...
[!"num:i(count(NvMBlockDescriptor/eb-list::*) + 1)"!]U
...
[!"num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier)"!]U

[!IF "node:existsAndTrue(NvMCommon/NvMExportBlockLengths)"!]
[!LOOP "NvMBlockDescriptor/eb-list::*"!][!//
#if (defined NvMConf_[!"as:name(.)"!]_Length)
#error NvMConf_[!"as:name(.)"!]_Length already defined
#endif
/** \brief Export length of Block [!"NvMNvramBlockIdentifier"!] using symbolic name */
#define NvMConf_[!"as:name(.)"!]_Length    [!"NvMNvBlockLength"!]U
...
[!IF "node:existsAndTrue(NvMCommon/NvMJobPrioritization)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 */!][!IF "NvMNvramBlockIdentifier = 1"!][!/*
   */!][!IF "(node:exists(NvMBlockCrcType)) and (node:existsAndTrue(NvMBlockUseCrc))"!][!/*
...
[!IF "(node:existsAndFalse(NvMCommon/NvMMemAccUsage)) and (node:existsAndTrue(NvMCommon/NvMDrvModeSwitch))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 **/
[!VAR "NumberOfNvMResistantToChangedSwBlocks" = "count(NvMBlockDescriptor/eb-list::*[(node:existsAndFalse(NvMResistantToChangedSw))])"!][!//
 ...
 **/
[!VAR "NumberOfNvMSelectBlockForReadAllBlocks" = "count(NvMBlockDescriptor/eb-list::*[(NvMNvramBlockIdentifier > 1) and (node:exists(NvMSelectBlockForReadAll)) and (node:existsAndTrue(NvMSelectBlockForReadAll))])"!][!//
 ...
 **/
[!VAR "NumberOfNvMSelectBlockForFirstInitAllBlocks" = "count(NvMBlockDescriptor/eb-list::*[(NvMNvramBlockIdentifier > 1) and (node:exists(NvMSelectBlockForFirstInitAll)) and (node:existsAndTrue(NvMSelectBlockForFirstInitAll))])"!][!//
 ...
 **        the configuration parameter NvMSelectBlockForWriteAll set to true.
  **/
[!VAR "NumberOfWriteAllBlocks" =  "count(NvMBlockDescriptor/eb-list::*[(node:exists(NvMSelectBlockForWriteAll)) and (node:existsAndTrue(NvMSelectBlockForWriteAll))])"!][!//
 ...
 **/
[!VAR "NumberOfValidateAllBlocks" =  "count(NvMBlockDescriptor/eb-list::*[(node:exists(NvMBlockUseAutoValidation)) and (node:existsAndTrue(NvMBlockUseAutoValidation))])"!][!//
 ...
 **/
[!VAR "NumberOfNvMDatasetBlocks" = "count(NvMBlockDescriptor/eb-list::*[(NvMBlockManagementType  = 'NVM_BLOCK_DATASET')])"!][!//
 ...
 **/
[!VAR "NumberOfNvMInitBlockCallback" = "count(NvMBlockDescriptor/eb-list::*[(node:exists(NvMInitBlockCallback)) and (NvMInitBlockCallback  != '') and (NvMInitBlockCallback  != 'NULL_PTR')])"!][!//
[!VAR "NumberOfNvMInitBlockCallback" =  "$NumberOfNvMInitBlockCallback + count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMProvideRteInitBlockPort))])"!][!//
 ...
 **/
[!VAR "NumberOfNvMSingleBlockCallback" = "count(NvMBlockDescriptor/eb-list::*[(node:exists(NvMSingleBlockCallback)) and (NvMSingleBlockCallback  != '' )])"!][!//
[!VAR "NumberOfNvMSingleBlockCallback" = "$NumberOfNvMSingleBlockCallback + count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMProvideRteJobFinishedPort))])"!][!//
 ...
 **/
[!VAR "NumberOfCalcCrc8Blocks" = "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBlockUseCrc)) and (node:exists(NvMBlockCrcType)) and (NvMBlockCrcType = 'NVM_CRC8')])"!][!//
#define NVM_NUMBER_OF_CALC_CRC8_BLOCKS                 [!//
[!"num:i($NumberOfCalcCrc8Blocks)"!]U
 ...
 **/
[!VAR "NumberOfCalcCrc16Blocks" = "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBlockUseCrc)) and (node:exists(NvMBlockCrcType)) and (NvMBlockCrcType = 'NVM_CRC16')])"!][!//
#define NVM_NUMBER_OF_CALC_CRC16_BLOCKS                 [!//
[!"num:i($NumberOfCalcCrc16Blocks)"!]U
 ...
 **/
[!VAR "NumberOfCalcCrc32Blocks" = "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBlockUseCrc)) and (node:exists(NvMBlockCrcType)) and (NvMBlockCrcType = 'NVM_CRC32')])"!][!//
#define NVM_NUMBER_OF_CALC_CRC32_BLOCKS                 [!//
[!"num:i($NumberOfCalcCrc32Blocks)"!]U
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBlockUseCRCCompMechanism))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMBswMMultiBlockJobStatusInformation)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMBswMBlockStatusInformation))]) > 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMWriteVerification))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMPreWriteDataComp))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMStaticBlockIDCheck))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 **        in ASR 20-11 format.
 **/
#define NVM_CBK2011ACTIVE                              [!//
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(ASR2011CallbackEnabled))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!"num:i(count(NvMBlockDescriptor/eb-list::*[(NvMBlockJobPriority = 0)]))"!]U
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMWriteBlockOnce))]) != 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!"num:i(count(NvMBlockDescriptor/eb-list::*[(NvMMaxNumOfWriteRetries > 0)]))"!]U
...
[!"num:i(count(NvMBlockDescriptor/eb-list::*[(NvMMaxNumOfReadRetries > 0)]))"!]U
...
[!IF "node:existsAndTrue(NvMCommon/NvMPollingMode)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMDynamicConfiguration)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(NvMCommon/NvMReadBlockHook)"!][!//
...
[!IF "node:existsAndTrue(NvMCommon/NvMWriteBlockHook)"!][!//
 ...
 **/
#define NVM_MC_CALLOUTS_ENABLED
#endif
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMProvideRteAdminPort))]) > 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "count(NvMBlockDescriptor/eb-list::*[(node:existsAndTrue(NvMProvideRteServicePort))]) > 0"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(count(NvMBlockDescriptor/eb-list::*) != num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier))"!]
#define NVM_START_SEC_CONFIG_DATA_16
#include <NvM_MemMap.h>

extern CONST(uint16, NVM_CONST) NvM_RedirBlockId[[!"num:i((num:max(NvMBlockDescriptor/eb-list::*/NvMNvramBlockIdentifier)) + 1)"!]U];
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled)) and (node:existsAndTrue(NvMPrecondAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled)) and (node:existsAndTrue(NvMPostcondAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled)) and (node:existsAndTrue(NvMUnreachAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled)) and (node:existsAndTrue(NvMInvariantAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(../NvMCommon/NvMDevErrorDetect)) and (node:existsAndTrue(NvMDefProgEnabled)) and (node:existsAndTrue(NvMStaticAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
