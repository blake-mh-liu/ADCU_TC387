...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
typedef Atomic_t NvM_Atomic_t;
typedef Atomic_value_t NvM_Atomic_value_t;
#endif

/**  \brief Administrative information for each block which
 **         is changed at run time.
 **/
/* !LINKSTO NvM.SWS_NvM_00134,1 */
typedef struct
{
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  NvM_Atomic_t MultiPartitionInfo;
#endif
