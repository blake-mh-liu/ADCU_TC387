 ...
 **/
extern FUNC( Std_ReturnType,NVM_CODE ) NvM_Queue_CheckFullStandard
(
...
/** \brief Indicates the current number of standard job requests accepted on satellites. */
extern VAR(uint16, NVM_VAR_CLEARED) NvM_Std_Initiated_On_Satellite;
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
/** \brief Indicates the current number of immediate job requests accepted on satellites. */
extern VAR(uint16, NVM_VAR_CLEARED) NvM_Imm_Initiated_On_Satellite;
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
#endif
