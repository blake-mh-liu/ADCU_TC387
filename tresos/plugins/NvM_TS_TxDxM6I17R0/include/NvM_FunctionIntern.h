 ...
 *
 * Reason: Because of the NvM inclusion structure currently there are includes paths where not all macros are visible. There is no functional impact as if this would be the case a warning would be present.
 */
/*==================[inclusions]============================================*/
                                  /* standard types                         */
#include <NvM_Cfg.h>
...
/** \brief Macro function that requests the Os Application Id either the real one or the test one,
 **        depending on the definition of TST_NVM_BSW_DISTRIBUTION in the included user header
 **/
#ifdef TST_NVM_BSW_DISTRIBUTION
#define NVM_GET_OS_APPLICATION_ID() Tst_GetApplicationId()
#else
#define NVM_GET_OS_APPLICATION_ID() GetApplicationID()
#endif

/** \brief Handles the result of an asynchronous request in the context of BSW distribution
 ** \param[in]  ReqResult: The result of the API
 ** \param[in]  BlockIndex: the index of the block being requested
 **
 **/
#define HANDLE_SATELLITE_TO_MASTER_REQUEST_RESULT(ReqResult, BlockIndex)\
do{\
  if(ReqResult == E_OK)\
  {\
    /* Set the satellite job state */\
    NVM_SET_SATELLITE_JOB_ACCEPTED_FLAG(BlockIndex);\
  }\
  else\
  {\
    /* The job could not be sent to the master because SchM refused it */\
    NVM_ASYNC_ASYNC_LOCK_RELEASE(BlockIndex);\
    NVM_ASYNC_SYNC_LOCK_RELEASE(BlockIndex); /* set atomic flags */\
  }\
}while(0);
#endif
...
extern FUNC(void,NVM_CODE) NvM_Report_Queue_Overflow(uint16 BlockIndex, uint8 ApiId);
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
/** \brief Implementation of the generic satellite main function in multi-partition environment.
 **/
extern FUNC( void,NVM_CODE ) NvM_Satellite_MainFunction( uint8 OsApplicationId );
/** \brief Returns the index of the corresponding element of the array NvM_SchMInterPartitionCalls
 * for the given Os application id.
 **/
extern FUNC(uint8, NVM_CODE) NvM_GetPartitionIndex
(
  os_applicationid_t OsAppId
);
/** \brief If it's a satellite job, reports the job as failed or waits for the coming cancellation
 **/
extern FUNC(void, NVM_CODE) NvM_ManageUnQueuedJobs
(
  uint16 BlockIndex,
  uint8 JobType
);
