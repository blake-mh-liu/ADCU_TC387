...
#if ( defined NVM_E_WRONG_PARTITION ) /* to prevent double declaration */
#error NVM_E_WRONG_PARTITION is already defined
#endif /* if ( defined NVM_E_WRONG_PARTITION ) */
/** \brief A job is requested on a satellite partition the block is not mapped to */
#define NVM_E_WRONG_PARTITION        0x1DU
 ...
 ** must not be queued (requirement NvM.SWS_NvM_00420) NvM_Cancel-WriteAll()
