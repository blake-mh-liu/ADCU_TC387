...
#if !defined(NVM_RESTOREPRAMBLOCKDEFAULTS_CALLOUT) && (NVM_BSW_DISTRIBUTION_ENABLED != STD_ON)
...
#endif
  {
    /* !LINKSTO NvM.ASR42.SWS_NvM_00227,1 */
...
#endif
  {
    /* !LINKSTO NvM.ASR42.SWS_NvM_00819,1 */
    NvM_SetPermanentRamBlockInvalid();
    NvM_SetPermanentRamBlockUnchanged();
  }
  NvM_CurrentFunctionPtr[NvM_GlobalCallLevel] = &NvM_RestoreBlockDefaults_AscSt0;
...
FUNC(void,NVM_CODE) NvM_RestoreBlockDefaults_AscSt1(void)
{
  ...
  if (CurrentRamAddress != NULL_PTR)
  {
    ...
    /* !LINKSTO NvM.InitCallback2011.Port,1 */
        ...
        NvM_InitBlockRequestType InitBlockRequest = NVM_INIT_RESTORE_BLOCK_DEFAULTS;
        if (NVM_BD_AR2011CBK(NvM_CurrentBlockDescriptorPtr->blockDesc))
        {
          if (NvM_CurrentServiceId == NVM_READ_BLOCK_API_ID)
          {
            /* !LINKSTO NvM.InitBlockCallback2011.READ_BLOCK,1 */
            InitBlockRequest = NVM_INIT_READ_BLOCK;
          }
          else if (NvM_CurrentServiceId == NVM_READ_ALL_API_ID)
          {
            /* !LINKSTO NvM.InitBlockCallback2011.READ_ALL_BLOCK,1 */
            InitBlockRequest = NVM_INIT_READ_ALL_BLOCK;
          }
          else
          {
            /* !LINKSTO NvM.InitBlockCallback2011.RESTORE_BLOCK_DEFAULTS,1 */
            InitBlockRequest = NVM_INIT_RESTORE_BLOCK_DEFAULTS;
          }
          /* Deviation MISRAC2012-2 */
          InitCallbackResult = ((NvM_InitBlockCallbackTypeAR2011)*(NvM_CurrentBlockDescriptorPtr->initBlockCallback))(InitBlockRequest);
        }
        else
        {
          /* Deviation MISRAC2012-2 */
          InitCallbackResult = ((NvM_InitBlockCallbackType)*(NvM_CurrentBlockDescriptorPtr->initBlockCallback))();
          TS_PARAM_UNUSED(InitBlockRequest);
        }
#else
        /* Deviation MISRAC2012-2 */
        InitCallbackResult = ((NvM_InitBlockCallbackType)*(NvM_CurrentBlockDescriptorPtr->initBlockCallback))();
...
FUNC(void,NVM_CODE) NvM_RestoreBlockDefaults_AscSt4(void)
{
...
#endif
  {
     ...
     * RAM Block valid and changed. NvM.SWS_NvM_00366,SWS_NvM_00228,SWS_NvM_00367.
     */
    /* !LINKSTO NvM.SWS_NvM_00228,1 */
...
/* !LINKSTO NvM.SWS_NvM_00150,1 */
   ...
   * be set to NVM_REQ_OK. NvM.SWS_NvM_00354.
   */
