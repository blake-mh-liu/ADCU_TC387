...
VAR(uint16, NVM_VAR_CLEARED) NvM_Std_Initiated_On_Satellite;
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
VAR(uint16, NVM_VAR_CLEARED) NvM_Imm_Initiated_On_Satellite;
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
#endif
...
)
{
  ...
  {
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    if (NvM_Std_Initiated_On_Satellite < ( NVM_SIZE_STANDARD_JOB_QUEUE - 2U ))
#endif
    {
      Result = E_OK;
    }
...
)
{
  ...
  if ( NvM_BlockDescriptorTable[BlockIndex].blockJobPriority == NVM_BLOCK_IMMEDIATE_PRIORITY )
  {
     ...
     */
#if ( NVM_SIZE_IMMEDIATE_JOB_QUEUE > 0U )
    /* If BswDistribution is ON: Lock the queue to protect it from paralel access */
    NVM_QUEUE_WAIT_TO_LOCK();
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    if(NVM_GET_SATELLITE_JOB_ACCEPTED_FLAG(BlockIndex) != 0U)
    {
      /* It has been incremented by satellite. Decremented back. */
      NvM_Imm_Initiated_On_Satellite--;
      Result = E_OK;
    }
    /* The job has come on master. */
    else
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
...
/* Index is already 0 in case NVM_SIZE_IMMEDIATE_JOB_QUEUE is 1 */
#if ( NVM_SIZE_IMMEDIATE_JOB_QUEUE > 1U )
    ...
    {
      /* If BswDistribution ON : Release the lock as fast as possible */
      NVM_QUEUE_LOCK_RELEASE();
    }
#endif
