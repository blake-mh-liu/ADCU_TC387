...
FUNC(void,NVM_CODE) NvM_StateMachine_SetNextRequest(void)
{
...
#endif
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    if(NVM_CANCEL_OPERATION_LOCK(NvM_GlobalBlockNumber))
    {
      if(NVM_GET_SATELLITE_JOB_TO_BE_CANCELLED_FLAG(NvM_GlobalBlockNumber))
      {
        /* The job for this block was requested to be cancelled. */
        NvM_GlobalCallLevel = 0U;
      }
      else
      {
        if(NVM_GET_SATELLITE_JOB_ACCEPTED_FLAG(NvM_GlobalBlockNumber))
        {
          NvM_SatelliteJobType = NVM_SATELLITE_DO_IMM_JOB;
        ...
        {
          /* The job is performed on Master */
        }
      }
      NVM_CANCEL_OPERATION_LOCK_RELEASE(NvM_GlobalBlockNumber);
    }
    else
    {
      /* The job for this block is currently being canceled. Don't perform the job. */
      NvM_GlobalCallLevel = 0U;
    }
...
#endif
  {
    if ( NVM_STD_QUEUE_NOT_EMPTY() )
    {
      ...
      if(NVM_CANCEL_OPERATION_LOCK(NvM_GlobalBlockNumber))
      {
        ...
        else
        {
          if(NVM_GET_SATELLITE_JOB_ACCEPTED_FLAG(NvM_GlobalBlockNumber))
          {
            NvM_SatelliteJobType = NVM_SATELLITE_DO_STD_JOB;
            NVM_SET_SATELLITE_JOB_ONGOING_FLAG(NvM_GlobalBlockNumber);
            /* Set the token for a satellite */
            NVM_ATOMIC_STORE( &NvM_MultiPartitionToken, (NvM_Atomic_value_t)NvM_BlockDescriptorTable[NvM_GlobalBlockNumber].OsAppId);
          }
    ...
    {
      /* It's the case for SetRamBlockStatus CRC job and it's treated outside of exclusive area, below. */
    }
  }
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    StartCrcJob = FALSE;
    /* The CRC calculation job at this point is just like an asynchronous normal job, therefore the block must be locked */
    if(NVM_ASYNC_ASYNC_TRY_LOCK(NvM_GlobalBlockNumber))
    {
      if(NVM_GET_SATELLITE_BGND_CRC_QUEUED_FLAG(NvM_GlobalBlockNumber))
      {
        NvM_SatelliteJobType = NVM_SATELLITE_DO_CRC_JOB;
        NVM_SET_SATELLITE_JOB_ONGOING_FLAG(NvM_GlobalBlockNumber);
        /* Set the token for a satellite */
        NVM_ATOMIC_STORE( &NvM_MultiPartitionToken, (NvM_Atomic_value_t)NvM_BlockDescriptorTable[NvM_GlobalBlockNumber].OsAppId);
      }
      ...
      {
        /* The job will be processed on Master */
      }
      StartCrcJob = TRUE;
    }
    else
    {
      /* Some user is requesting an asynchronous job for this block. Do nothing on this cycle. */
    }
    if(StartCrcJob == TRUE)
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
