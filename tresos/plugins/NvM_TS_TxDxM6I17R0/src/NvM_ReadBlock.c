...
/* To check whether Det/Dem error shall be reported */
...
#if !defined(NVM_READPRAMBLOCK_CALLOUT) && (NVM_BSW_DISTRIBUTION_ENABLED != STD_ON)
...
FUNC(void,NVM_CODE) NvM_ReadBlock_Async(void)
{
   ...
   * and unchanged.
   */
  /* !LINKSTO NvM.ASR42.SWS_NvM_00198,1 */
...
FUNC(void,NVM_CODE) NvM_ReadPRAMBlock_Async(void)
{
   ...
   * and unchanged.
   */
  /* !LINKSTO NvM.ASR42.SWS_NvM_00767,1 */
#if (NVM_SYNC_MECHANISM == STD_ON)
  if (!(NVM_CHECK_WORKING_STATUS(NVM_DYN_MIRROR_MASK)))
#endif
  {
...
{
  NvM_PtrToApplDataType DataAddress = NvM_CalcCrc_GetBlockCrcAddress();
  /* Do CRC check */
  NvM_GlobalErrorStatus = NvM_CheckCrc(DataAddress,
...
FUNC(void,NVM_CODE) NvM_ReadBlock_Async_State6(void)
{
  ...
  uint16 Length =(NvM_BlockDescriptorTable[NvM_GlobalBlockNumber].nvBlockLength +
  ...
  if (!NVM_CHECK_WORKING_STATUS(NVM_DYN_WBID_MASK))
  {
...
#endif
    {
      ...
      if ((NVM_BD_USEINTERNALBUFFER(NvM_CurrentBlockDescriptorPtr->blockDesc) || NvM_UseInternalBufferAlignment)
          &&
          (process_block == TRUE)
         )
#else
...
#endif
      {
            ...
            (NVM_CHECK_WORKING_STATUS(NVM_DYN_UPRB_MASK)))
        {
          ...
          TS_MemCpy(NvM_GetUserRamBlockAddress(), DataBufPtr,Length);
        }
        else
        {
...
#endif
    {
       ...
       * RAM Block valid and unchanged. NvM.SWS_NvM_00200, ( Autosar Bugzilla #20953 ),
       * NvM.SWS_NvM_00251.
...
FUNC(void,NVM_CODE) NvM_ReadBlock_Async_StateError3(void)
{
     ...
     )
  {
    if ( NvM_GlobalBlockNumber > 1U)
    {
...
#endif
      {
         ...
         * must be set to NVM_REQ_INTEGRITY_FAILED. (NVM358, NvM.SWS_NvM_00204, NvM.SWS_NvM_00360, NvM.SWS_NvM_00295).
         */
        /* !LINKSTO NvM.SWS_NvM_00358,2 */
  ...
  else if ( NVM_CHECK_WORKING_STATUS( NVM_DYN_BRVE_MASK ) )
  {
     ...
     * job result. NvM.SWS_NvM_00341, NvM.SWS_NvM_00342, NVM299.
    */
  ...
  else
  {
     ...
     * the request result is set to NVM_REQ_NOT_OK. NVM359, NvM.SWS_NvM_00361.
...
)
{
  ...
  for(ParseIdx = 0U; (ParseIdx < BlkLength) && (boBreakLoop == FALSE); ParseIdx++)
  {
    if(*pBuf != *(pBuf-1U))
    {
      boBreakLoop = TRUE;
  ...
  for(Idx2 = 1U; Idx2 < CrcLength; Idx2++)
  {
    pBuf--;
  }
  PadValue = ~(*pBuf);
  for(Idx2 = 0U; Idx2 < (CrcLength + ParseIdx); Idx2++)
  {
    *pBuf = PadValue;
     pBuf++;
  }
...
STATIC FUNC(void, NVM_CODE) NvM_ReportReadProductionErrors(void)
{
       ...
       NVM_BD_BLOCKMNGTTYPE_REDUNDANT)
  {
    ...
    {
      /* Report Dem error as failed (SWS_NvM_00595, NvM.SWS_NvM_00655) if one copy failed. */
        ...
        (NVM_CHECK_EXT_STATUS(NVM_ES_REDUNDANT_ERROR_MASK)))
    {
      if (((NVM_BD_CRCTYPE(NvM_CurrentBlockDescriptorPtr->blockDesc) != NVM_BD_CRCTYPE_NONE) &&
          ...
          (NVM_CHECK_WORKING_STATUS(NVM_DYN_BRBI_MASK)))
      {
        /* The error must be reported to DEM (SWS_NvM_00360, NVM358, NvM.SWS_NvM_00203) */
#if ( NVM_PROD_ERR_HANDLING_INTEGRITY_FAILED == TS_PROD_ERR_REP_TO_DEM )
