 ...
 *
 * Reason: Needed a typedef in SchM generation. In the end it's the same base type.
 */
...
/* patch version check */
#if (NVM_SW_PATCH_VERSION < 36U)
#error NVM_SW_PATCH_VERSION wrong (< 36U)
#endif
...
/** \brief Extracts a job from the queues if there is no job ongoing or
 *         the current job must be interrupted by a more prior one.
 *         Returns true if the current/new job needs execution. */
STATIC FUNC(boolean ,NVM_CODE) NvM_QueueJobNeedsProcessing(void);

/** \brief Finish the current job. */
STATIC FUNC(void ,NVM_CODE) NvM_CurrentJobFinished(void);

#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
/** \brief Manages the token's ownership. */
STATIC FUNC(boolean ,NVM_CODE) NvM_MasterOwnsToken(void);
...
/** \brief NvM_PerformQueueJobCancelation removes canceled jobs from queue. */
STATIC FUNC(void, NVM_CODE) NvM_PerformQueueJobCancelation(uint16 BlockIndex);
...
#include <NvM_MemMap.h>
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
/** \brief Stores the state of token being taken or not by a satellite
 **/
STATIC VAR( uint8,NVM_VAR ) NvM_Satellite_TokenTaken[NVM_NUMBER_OF_SATELLITE_PARTITIONS];
#endif
...
/* !LINKSTO NvM.SWS_NvM_00193,1, NvM.SWS_NvM_00091,1, NvM.SWS_NvM_00157,1 */
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  NvM_Std_Initiated_On_Satellite = 0U;
#endif
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  NvM_Imm_Initiated_On_Satellite = 0U;
#endif
#endif

#endif /* #if (NVM_API_CONFIG_CLASS != NVM_API_CONFIG_CLASS_1) */
#if (NVM_NUMBER_OF_SELECT_BLOCK_FOR_FIRSTINITALL_BLOCKS > 0U)
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    NVM_ATOMIC_INIT(&CurrentAdminBlockPtr->MultiPartitionInfo, 0U);
#endif
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  NVM_ATOMIC_INIT(&NvM_MultiPartitionToken, NVM_OS_APPLICATION_ID_MASTER);
  NVM_ATOMIC_INIT(&NvM_MultiPartitionGeneral, 0U);
  NvM_SatelliteJobType = 0U;
#endif
  DBG_NVM_INIT_EXIT();
}/* end of function  NvM_Init */

/* !LINKSTO NvM.SWS_NvM_00464,1 */
FUNC(void,NVM_CODE) NvM_MainFunction(void)
{
  /* Indicates whether a job is currently being processed.(starting with the extraction from queue) */
  boolean JobProcessing = FALSE;
  /* Indicates whether the main function's functionality must be executed on this cycle */
  boolean ExecuteMainFunction = FALSE;
  ...
  /* !LINKSTO NvM.SWS_NvM_00349,1 */
  if (ExecuteMainFunction == TRUE)
  {
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    /* Check whether Master can take back the token from Satellite */
    if ( NvM_MasterOwnsToken() == TRUE )
#endif /* #if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
#if (NVM_BLOCK_CHECK_ENABLED == STD_ON)
      ...
      /* Check whether a job needs to be fetched from the queues or the current job needs further processing. */
      JobProcessing = NvM_QueueJobNeedsProcessing();
#if (NVM_BLOCK_CHECK_ENABLED == STD_ON)
      if (JobProcessing == FALSE)
      {
        /* There is no job to be processed. */
        if(NvM_BcGlobalDisableFlag == STD_OFF)
        {
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
      /* process request */
      /* the atomic read is not needed here as the value can only change in this while */
      while( ( NvM_MultiPartitionToken == NVM_OS_APPLICATION_ID_MASTER ) && ( NvM_GlobalCallLevel > 0U ) && ( NvM_GlobalErrorStatus != NVM_REQ_PENDING ) )
#else
      ...
      if ( ( JobProcessing == TRUE ) && ( NvM_GlobalCallLevel == 0U ) )
      {
        NvM_CurrentJobFinished();
      }/* if ( ( JobProcessing == TRUE ) && ( NvM_GlobalCallLevel == 0U ) ) */

    } /* if ( NvM_SatelliteJobOngoing() == FALSE ) */
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
FUNC( void,NVM_CODE ) NvM_Satellite_MainFunction( uint8 OsApplicationId )
{
  uint8 Token = (uint8)NVM_ATOMIC_LOAD(&NvM_MultiPartitionToken);
  uint8 PartitionIdx = NvM_GetPartitionIndex(OsApplicationId);

  if(Token == OsApplicationId)
  {
    NvM_Satellite_TokenTaken[PartitionIdx] = TRUE;
  }

  /* If the token has been already taken before or it has just been taken */
  if (NvM_Satellite_TokenTaken[PartitionIdx] == TRUE)
  {
    NvM_GlobalErrorStatus = NVM_REQ_NOT_OK;
    /* process request */
    while( ( NvM_GlobalCallLevel > 0U ) && ( NvM_GlobalErrorStatus != NVM_REQ_PENDING ) )
    {
      ...
      if(NVM_GET_SATELLITE_MULTIBLOCK_FINALIZING_FLAG())
      {
        /* The multi-block job finished the current block. Master will take on. */
        NvM_Satellite_TokenTaken[PartitionIdx] = FALSE;
        break;
      }
    }/* while( ( NvM_GlobalCallLevel > 0U )&&( NvM_GlobalErrorStatus != NVM_REQ_PENDING ) ) */
    ...
    ( NVM_NUMBER_OF_SINGLE_BLOCK_CALLBACK > 0U ) )
      ...
      /* Give the control back to Master in order to handle the job finalization */
      NVM_SET_SATELLITE_JOB_FINALIZING_FLAG(NvM_GlobalBlockNumber);
      NvM_Satellite_TokenTaken[PartitionIdx] = FALSE;
    }
    else
    {
      /* Satellite job still ongoing */
    }
  }
  else
  {
    /* This satellite currently doesn't have a job. */
  }

  /* If Master wants Satellite to stop the processing of the current job */
  if((Token == (uint8)NVM_NOBODY_S_TOKEN) && (OsApplicationId == NvM_BlockDescriptorTable[NvM_GlobalBlockNumber].OsAppId))
  {
    /* A more prior job must be executed, therefore stop the Satellite execution and give back the control to the Master */
    /* The job will be restarted by Master (from scratch) */
    NVM_CLEAR_SATELLITE_JOB_ONGOING_FLAG(NvM_GlobalBlockNumber);
    NvM_Satellite_TokenTaken[PartitionIdx] = FALSE;
  }
}
#endif /* #if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
...
STATIC FUNC(void, NVM_CODE) NvM_ProcessCanceledJob(void)
{
  ...
  if (ExecuteBlockCallback == TRUE) /* $$ daca asta ii true se da job de single block callback la satelit */
  {
    ...
    else
    {
      ...
      for (CurrentBlock = 1U; CurrentBlock < NVM_TOTAL_NUMBER_OF_NVRAM_BLOCKS; CurrentBlock++)
      {
        ...
        {
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
          if(NvM_BlockDescriptorTable[CurrentBlock].OsAppId != NVM_OS_APPLICATION_ID_MASTER)
          {
            uint8 PartitionIdx = NvM_GetPartitionIndex(NvM_BlockDescriptorTable[CurrentBlock].OsAppId);
            (void)NvM_SchMInterPartitionCalls[PartitionIdx].satelliteJobEndNotification(CurrentBlock,
...
STATIC FUNC(boolean, NVM_CODE) NvM_ProcessImmediateBlock(void)
{
   ...
   * According to NvM.SWS_NvM_00381 and NvM.SWS_NvM_00567, multi-block requests shall not be interrupted.
   *
   * Note: The only exception to the rule given in [SWS_NvM_00381, NvM.SWS_NvM_00567] is a write job with
   * immediate priority which shall preempt the running read / write job [SWS_NvM_00182].
   * The preempted job shall subsequently be resumed / restarted by the NvM module.
   */
  /* !LINKSTO NvM.SWS_NvM_00381,1, NvM.SWS_NvM_00567,1, NvM.SWS_NvM_00300,1 */
...
#if (NVM_CANCEL_INTERNAL_JOB == STD_ON) /*$ bullshit feature */
      for(Idx=0U; Idx < NVM_NUM_USED_DEVICES; Idx++)
      {
...
/* Check if block status info reporting to BswM is enabled */
    ...
    {
      SingleJobResult = NvM_AdminBlockTable[NvM_GlobalBlockNumber].NvMResult;
#if(defined(NVM_VARIABLE_BLOCK_LENGTH_EN))
#if (NVM_VARIABLE_BLOCK_LENGTH_EN == STD_ON)
      if ((SingleJobResult == NVM_REQ_OK_BLK_INCREASED) || (SingleJobResult == NVM_REQ_OK_BLK_DECREASED))
      {
        /* !LINKSTO NvM.VariableBlockLength.BswMSingleIncreased,1 */
        /* !LINKSTO NvM.VariableBlockLength.BswMSingleDecreased,1 */
        SingleJobResult = NVM_REQ_OK;
      }
      else
      {
...
#if ( ( NVM_BSWM_MULTI_BLOCK_JOB_STATUS_INFO == STD_OFF ) || \
    ( NVM_NUMBER_OF_SINGLE_BLOCK_CALLBACK > 0U ) )
FUNC(void, NVM_CODE) NvM_ProcessBlockCallback(void)
{
#if (NVM_BSWM_MULTI_BLOCK_JOB_STATUS_INFO == STD_OFF)
...
#elif (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) /*defined(NVM_MC_CALLOUTS_ENABLED)*/
    uint8 OsApplicationId = (uint8)NVM_GET_OS_APPLICATION_ID();
    /* !LINKSTO NvM.BswDistribution.ErrorStatus,1 */
    if(OsApplicationId != NVM_OS_APPLICATION_ID_MASTER)
    {
      if(NVM_GET_SATELLITE_JOB_ACCEPTED_FLAG(BlockIndex))
      {
        *RequestResultPtr = NVM_REQ_PENDING;
      }
      else
      {
        /* Satellite requests the status and the job is not pending. */
...
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
      /* Master should know the real status. */
      *RequestResultPtr = NvM_AdminBlockTable[BlockIndex].NvMResult;
    }
...
#if ( defined(NVM_MC_CALLOUTS_ENABLED) && ( NVM_NUMBER_OF_DATASET_BLOCKS > 0U ))
  if (((uint8)(NvM_AdminBlockTable[BlockIndex].NvMExtendedStatus & (NVM_ES_MULTICORE_LOCK_MASK)) != (NVM_ES_MULTICORE_LOCK_MASK)))
#endif /*MC CALLOUTS ENABLED AND DATASET ENABLED*/
#endif /* NVM_DEV_ERROR_DETECT == STD_ON */
  {
/* !LINKSTO NvM.SWS_NvM_00150,1 */
    ...
    if ( NVM_BD_BLOCKMNGTTYPE(NvM_BlockDescriptorTable[BlockIndex].blockDesc)
         == NVM_BD_BLOCKMNGTTYPE_DATASET )
    {
      /* Enter inter-Partition exclusive area if the block is not being used */
      NVM_SYNC_SYNC_WAIT_TO_LOCK(BlockIndex);

      /* NvM.SWS_NvM_00704 */
      /* Check if an other request for block BlockIndex is already pending. */
      /* Enter inter-Partition exclusive area if the block is not being used */
      if(NVM_ASYNC_SYNC_TRY_LOCK(BlockIndex))
      {
        ...
        /* Exit inter-Partition exclusive area if the block is not being used */
      ...
      /* Exit inter-Partition exclusive area if the block is not being used */
      NVM_SYNC_SYNC_LOCK_RELEASE(BlockIndex);
    }
#endif
  }
...
{
#if (( NVM_JOB_PRIORITIZATION == STD_ON ) || (!defined(NVM_ERASENVBLOCK_CALLOUT)) || (NVM_BSW_DISTRIBUTION_ENABLED != STD_ON))
  uint16 BlockIndex = NVM_TOTAL_NUMBER_OF_NVRAM_BLOCKS;
#endif
  Std_ReturnType Result = E_NOT_OK; /* Result of this function */
...
)
{
...
#endif
  {
        ...
        (NVM_BD_BLOCKSTATUSINFO(NvM_BlockDescriptorTable[BlockIndex].blockDesc)))
    {
       ...
       * block status to BswM */
      BswM_NvM_CurrentBlockMode(NvM_BlockDescriptorTable[BlockIndex].nvBlockIdentifier, NVM_REQ_PENDING);
    }
#endif
...
#endif /* NVM_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if(NVM_ASYNC_SYNC_TRY_LOCK(BlockIndex))
    {
      ...
      /* Enabling/Disabling of the write protection is not allowed when
         the NvMWriteBlockOnce is TRUE (SWS_NvM_00577, NvM.SWS_NvM_00398). */
      ...
      /* Exit inter-Partition exclusive area */
      NVM_ASYNC_SYNC_LOCK_RELEASE(BlockIndex);
    }
    /* Exit inter-Partition exclusive area */
    NVM_SYNC_SYNC_LOCK_RELEASE(BlockIndex);
...
)
{
  ...
  else if ( NVM_CHECK_GLOBAL_GENERIC_STATUS_FALSE( NVM_GENERIC_INIT_FLAG_MASK ) )
  {
    ...
    /* !LINKSTO NvM.SWS_NvM_00758,1 */
  else if (NVM_CHECK_DYN_STATUS(BlockIndex, NVM_DYN_LOCK_MASK))
  {
    NVM_DET_REPORT_ERROR(NVM_SET_RAM_BLOCK_STATUS_API_ID, NVM_E_BLOCK_LOCKED);
  }
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  else if ((OsApplicationId != NVM_OS_APPLICATION_ID_MASTER) && (OsApplicationId != NvM_BlockDescriptorTable[BlockIndex].OsAppId))
  {
    /* This block is not mapped to this partition */
    NVM_DET_REPORT_ERROR( NVM_SET_RAM_BLOCK_STATUS_API_ID, NVM_E_WRONG_PARTITION );
  }
#endif
#if ( defined(NVM_MC_CALLOUTS_ENABLED) )
  else if (((uint8)(NvM_AdminBlockTable[BlockIndex].NvMExtendedStatus & (NVM_ES_MULTICORE_LOCK_MASK)) == (NVM_ES_MULTICORE_LOCK_MASK)))
      ...
      if ((NVM_BD_USERAMBLOCKSTATUS(NvM_CurrentBlockDescriptorLocalPtr->blockDesc)) &&
...
#endif /* NVM_SYNC_MECHANISM = STD_ON */
        {
          ...
          if(NVM_ASYNC_SYNC_TRY_LOCK(BlockIndex))
          {
            ...
            {
              /* !LINKSTO NvM.SWS_NvM_00406,1 */
              /* Block data has changed --> validate block. NvM.SWS_NvM_00406. */
                      ...
                      NVM_ES_RAM_BLOCK_CRC_CALC_MASK) != NVM_ES_RAM_BLOCK_CRC_CALC_MASK))
              {
                ...
                NVM_CRC_QUEUE_WAIT_TO_LOCK();
                /* A Ram block CRC calculation request is pending */
                NvM_CalcCrc_EnqueueElement(BlockIndex);
                NVM_CRC_QUEUE_LOCK_RELEASE();

#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
                if(OsApplicationId != NVM_OS_APPLICATION_ID_MASTER)
                {
                  /* Set the satellite job state */
                  NVM_SET_SATELLITE_BGND_CRC_QUEUED_FLAG(BlockIndex);
                }
                else
                {
                  /* The request came on Master */
                }
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
              }
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  else if(OsApplicationId != NVM_OS_APPLICATION_ID_MASTER)
  {
    /* This block is not mapped to this partition */
    NVM_DET_REPORT_ERROR( NVM_SET_BLOCK_LOCK_STATUS_API_ID, NVM_E_WRONG_PARTITION );
  }
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
#if (NVM_WRITE_BLOCK_ONCE == STD_ON)
...
#endif /* NVM_SYNC_MECHANISM == STD_ON */
    {
      ...
      if(NVM_ASYNC_SYNC_TRY_LOCK(BlockIndex))
      {
        if (BlockLocked == TRUE)
        {
           ...
           * NvM_InvalidateNvBlock or NvM_EraseNvBlock. (SWS_NvM_00751)
           */
...
FUNC(Std_ReturnType, NVM_CODE) NvM_ASR40_CancelJobs(NvM_ASR40_BlockIdType BlockId)
{
  uint16 BlockIndex;
  Std_ReturnType Result = E_OK; /* Result of this function */

  DBG_NVM_ASR40_CANCELJOBS_ENTRY(BlockId);

#if !defined(NVM_CANCELJOBS_CALLOUT) && (NVM_BSW_DISTRIBUTION_ENABLED != STD_ON)
  ...
  {
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    if(NVM_GET_SATELLITE_JOB_TO_BE_CANCELLED_FLAG(BlockIndex))
    {
      SchM_Enter_NvM_SCHM_NVM_EXCLUSIVE_AREA_0();
      /* The job must be in queue by now because schM-Cancel always come after the schm-Job */
      NvM_PerformQueueJobCancelation(BlockIndex);
      SchM_Exit_NvM_SCHM_NVM_EXCLUSIVE_AREA_0();
    }
    else
      /* If the job came from Satellite, here on Master it should be cancelled only if already queued,
       * in order to be consistent with GetErroStatus. */
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
      ...
      /* !LINKSTO NvM.BswDistribution.MasterCancel,1 */
    ...
    /* If the job is cancelled clear the locks */
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    /* If the job is cancelled clear the locks */
    if(Result == E_OK)
    {
      /* Clear all the BswDistribution flags related to this block except the one set by SetRamBlockStatus */
      NVM_CLEAR_SATELLITE_JOB_ACCEPTED_FLAG(BlockIndex);
      NVM_CLEAR_SATELLITE_JOB_TO_BE_CANCELLED_FLAG(BlockIndex);
      NVM_CLEAR_SATELLITE_JOB_ONGOING_FLAG(BlockIndex);
      NVM_CLEAR_SATELLITE_JOB_FINALIZING_FLAG(BlockIndex);
      NVM_ASYNC_ASYNC_LOCK_RELEASE(BlockIndex);
      NVM_ASYNC_SYNC_LOCK_RELEASE(BlockIndex);
    }
#endif /* (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
        ...
        (NVM_BD_BLOCKSTATUSINFO(NvM_BlockDescriptorTable[BlockIndex].blockDesc)))
    {
       ...
       * status to BswM */
      BswM_NvM_CurrentBlockMode(NvM_BlockDescriptorTable[BlockIndex].nvBlockIdentifier, NVM_REQ_CANCELED);
    }
  #endif
  }
#if !defined(NVM_CANCELJOBS_CALLOUT) && (NVM_BSW_DISTRIBUTION_ENABLED != STD_ON)
...
STATIC FUNC(boolean ,NVM_CODE) NvM_QueueJobNeedsProcessing(void)
{
  boolean JobNeedsProcessing = FALSE;
  ...
  else
  {
    ...
    * priority to process an immediate write request.
     */
    if (TRUE == NvM_ProcessImmediateBlock())
    {
      /* An immediate job needs to be processed now. */
    }
    else
#endif /* #if ((NVM_JOB_PRIORITIZATION == STD_ON) &&
              (NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U)) */
   ...
   (NVM_RAMBLOCK_CRC_BUFFER_SIZE > 0U))
    ...
    /* A the current job or the new job needs processing */
    JobNeedsProcessing = TRUE;
  }/* end of else-if( NvM_GlobalCallLevel == 0U ) */

#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  /* A job that has been cancelled on Satellite is not started.
   * It must wait to be un-queued. */
  if ( NvM_GlobalCallLevel == 0U )
  {
    JobNeedsProcessing = FALSE;
  }
#endif

  return JobNeedsProcessing;
}
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
    /* Process the single block and multi block callbacks. */
    if (NVM_GET_SATELLITE_JOB_FINALIZING_FLAG(NvM_GlobalBlockNumber))
    {
      /* The single block callback has been already called on satellite */
    }
    else
#endif /* #if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
    {
...
#if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON)
  NvM_SatelliteJobType = NVM_SATELLITE_NO_JOB;
  /* Clear all the locks and all the satellite job related flags for this block */
  NVM_CLEAR_SATELLITE_JOB_ACCEPTED_FLAG(NvM_GlobalBlockNumber);
  NVM_CLEAR_SATELLITE_JOB_TO_BE_CANCELLED_FLAG(NvM_GlobalBlockNumber);
  NVM_CLEAR_SATELLITE_JOB_ONGOING_FLAG(NvM_GlobalBlockNumber);
  NVM_CLEAR_SATELLITE_JOB_FINALIZING_FLAG(NvM_GlobalBlockNumber);
  NVM_ASYNC_ASYNC_LOCK_RELEASE(NvM_GlobalBlockNumber);
  NVM_ASYNC_SYNC_LOCK_RELEASE(NvM_GlobalBlockNumber);
  /* Clear also the background CRC flag */
  if(NVM_SET_RAM_BLOCK_STATUS_API_ID == NvM_CurrentServiceId )
  {
    NVM_CLEAR_SATELLITE_BGND_CRC_QUEUED_FLAG(NvM_GlobalBlockNumber);
  }
#endif /* #if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
...
STATIC FUNC(boolean ,NVM_CODE) NvM_MasterOwnsToken(void)
{
  boolean MasterOwnsToken = TRUE;

  NvM_Atomic_value_t CurrentJobPartitionId = NVM_ATOMIC_LOAD(&NvM_MultiPartitionToken);

  if (CurrentJobPartitionId != NVM_OS_APPLICATION_ID_MASTER)
  {
    if(NVM_GET_SATELLITE_JOB_FINALIZING_FLAG(NvM_GlobalBlockNumber))
    {
      /* Finish the satellite job */
      NvM_CurrentJobFinished();

      /* Get the token back from Satellite. */
      NVM_ATOMIC_STORE(&NvM_MultiPartitionToken, NVM_OS_APPLICATION_ID_MASTER);
    }
    else if(NVM_GET_SATELLITE_JOB_ONGOING_FLAG(NvM_GlobalBlockNumber) == FALSE)
    {
      NVM_CLEAR_SATELLITE_MULTIBLOCK_FINALIZING_FLAG();
      /* Satellite stopped the job processing as requested. Get the token back. */
      NVM_ATOMIC_STORE(&NvM_MultiPartitionToken, NVM_OS_APPLICATION_ID_MASTER);
    }
    else if(NVM_GET_SATELLITE_MULTIBLOCK_FINALIZING_FLAG() == TRUE)
    {
      NVM_CLEAR_SATELLITE_MULTIBLOCK_FINALIZING_FLAG();
      /* Satellite stopped the job processing as requested. Get the token back. */
      NVM_ATOMIC_STORE(&NvM_MultiPartitionToken, NVM_OS_APPLICATION_ID_MASTER);
...
#if (( NVM_JOB_PRIORITIZATION == STD_ON ) && ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U ) )
      /* Check if an immediate request is available */
      if (NVM_IMMEDIATE_QUEUE_NOT_EMPTY()  && (NvM_SatelliteJobType != NVM_SATELLITE_DO_IMM_JOB))
      {
        /* Tell the satellite to stop the execution of the current job */
        NVM_ATOMIC_STORE(&NvM_MultiPartitionToken, (NvM_Atomic_value_t)NVM_NOBODY_S_TOKEN);
      }
      else
#endif
#endif
...
#if ((NVM_SET_RAM_BLOCK_STATUS_API == STD_ON) && \
    (NVM_RAMBLOCK_CRC_BUFFER_SIZE > 0U))
        /* Check if an Crc request is available */
        if (NVM_STD_QUEUE_NOT_EMPTY() && (NvM_SatelliteJobType == NVM_SATELLITE_DO_CRC_JOB))
        {
          /* Tell the satellite to stop the execution of the current job */
          NVM_ATOMIC_STORE(&NvM_MultiPartitionToken, (NvM_Atomic_value_t)NVM_NOBODY_S_TOKEN);
          /* Reset the call level so that NvM can fetch the standard job on this main function */
          NvM_GlobalCallLevel = 0U;

          /* Crc calculation locked the block. Unlock the block so that the users might request Std/Imm jobs for it */
          NVM_ASYNC_ASYNC_LOCK_RELEASE(NvM_GlobalBlockNumber);
        }
        else
        {
          /* Continue the current satellite job. */
        }
#endif
      }
      /* Until the Satellite either stops on request or finishes the job, Master must not run. */
      MasterOwnsToken = FALSE;
    }
  }
  return MasterOwnsToken;
}
#endif /* #if (NVM_BSW_DISTRIBUTION_ENABLED == STD_ON) */
