...
STATIC VAR(uint8, NVM_VAR_POWER_ON_CLEARED) NvM_RamBlockCrcValues[NVM_RAMBLOCK_CRC_BUFFER_SIZE];
 ...
 **/
/* Deviation MISRAC2012-2 */
STATIC VAR(uint8, NVM_VAR_POWER_ON_CLEARED) NvM_NvBlockCrcValues[NVM_NVBLOCK_CRC_BUFFER_SIZE];
...
{
  NvM_PtrToApplDataType ret = NvM_GetDataAddress();
  ret = &(ret[NvM_CurrentBlockDescriptorPtr->nvBlockLength]);
  return ret;
}
...
{
#if(NVM_NUMBER_OF_CRC_BLOCKS > 1U)
