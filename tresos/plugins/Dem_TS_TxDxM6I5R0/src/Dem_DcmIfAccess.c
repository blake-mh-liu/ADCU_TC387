 ...
 *
 * Reason:
 * The expression "(EventId + DEM_MAX_NUM_CHECK_CLEAR_EVENTS_PER_CYCLE)" keep changing based on
 * EventId value. In other configurations, the code coverage reaches sections where
 * (EventId + DEM_MAX_NUM_CHECK_CLEAR_EVENTS_PER_CYCLE) is lesser or equal than DEM_MAX_EVENTID.
 *
 * TASKING-4) Deviated Rule: W549
 * W549: condition is always false
 *
 * Reason:
 * "LastEventId" depends on DEM_MAX_NUM_CHECK_CLEAR_EVENTS_PER_CYCLE. In other configurations,
 * the code coverage reaches sections where EventId is greater than LastEventId.
 *
 * TASKING-5) Deviated Rule: W549
 * W549: condition is always false
 *
 * Reason:
 * "FractionalNumEvents" is equal to 0 and "RemainingEventsToProcess" can't be less than 0 because
 * it is a unit16. In other configurations, the code coverage reaches sections where
 * "RemainingEventsToProcess" is less than "FractionalNumEvents".
 *
 * TASKING-6) Deviated Rule: W549
 * W549: condition is always false
 *
 * Reason:
 * "EmissionRelDTCs" is always FALSE if DEM_EMISSION_REL_DTCS_GROUP_EXIST == STD_OFF.
 * In other configurations, the code coverage reaches sections where EmissionRelDTCs
 * is equal to TRUE.
 */
...
/** \brief Function to trigger the RGP notification
 **
 ** This function calls the relevant RGP clear notification
 ** with the relevant parameter.
 */
STATIC FUNC(void, DEM_CODE) Dem_TriggerRGPNotification(void);
#endif
...
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ClearCmbEvents(
  ...
  P2VAR(uint16, AUTOMATIC, AUTOMATIC)                    RemainingEventsToProcess,
  P2VAR(Dem_CmbDTCEvLnkType, AUTOMATIC, DEM_VAR_CLEARED) NextCmbEvIdx,
...
VAR(Dem_DTCSelectionType, DEM_VAR_CLEARED) Dem_ClearDTCSelection;
  ...
  Dem_DTCOriginType DTCOrigin)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    Dem_SetDTCSelectionCriteria(DEM_VIRTUAL_CLIENT_CDD,
                                ...
                                DTCFormat,
                                DTCOrigin,
                                DEM_DTC_ORIGIN_MAPPING_ASR40_IF);
  ...
  if (DEM_PENDING != Result)
  {
    ...
    /* !LINKSTO dsn.Dem.OBD.Mode04.Master.ConditionsForNotifyingSwC.0001, 1 */
      ...
      if ( (DTC == DEM_DTC_GROUP_ALL_DTCS) &&
           ...
           (DTCOrigin == DEM_DTC_ORIGIN_PRIMARY_MEMORY) )
      {
        /* !LINKSTO Dem.ASR42.SWS_Dem_00721,1 */
...
#if ((DEM_OBD_Support == STD_ON) && (DEM_READINESS_CALCULATION_SUPPORT == STD_ON))
    /* Notify readiness group processor */
    Dem_TriggerRGPNotification();
#endif
...
#endif
    {
      /* set the Result to be reported to the client */
      Dem_SetClearDTCResult(Result);
  ...
  Dem_DTCOriginType DTCOrigin)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    Dem_SetDTCSelectionCriteria(ClientId,
                                ...
                                DTCFormat,
                                DTCOrigin,
                                DEM_DTC_ORIGIN_MAPPING_ASR43_IF);
    Result = E_OK;
  }
  ...
  Dem_DTCOriginMappingType MappingVariant)
{
  ...
  if ( (MappingVariant == DEM_DTC_ORIGIN_MAPPING_ASR43_IF) ||
  ...
  P2VAR(Dem_EventIdType, AUTOMATIC, DEM_VAR) EventId)
{
  ...
  if (NewSelectorStatus == DEM_DTC_SELECTOR_NOT_PROCESSED)
  {
    ...
    if (!DEM_IS_DTCFMT_SUPPORTED(DTCFormat))
    {
...
#endif
    else /* single DTC in either UDS or OBD_3BYTE format requested */
    {
      Dem_GetDTCFctPtrType GetVisibleDTCFctPtr;
      Dem_EventIdType EvId;

#if (DEM_OBDONUDS_SUPPORT && (DEM_OBD_DTCVALUE3BYTE_SUPPORT == STD_ON))
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01372,1 */
      if (DTCFormat == DEM_DTC_FORMAT_OBD_3BYTE)
      {
        GetVisibleDTCFctPtr = Dem_GetVisibleObdDTCValue3Byte;
      }
      else
#endif
      {
        GetVisibleDTCFctPtr = Dem_GetVisibleUdsDTC;
      }
      ...
      {
        if (DTC == GetVisibleDTCFctPtr(EvId))
        {
          /* check DTC origin */
  ...
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) GroupIdx)
{
  ...
  if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
  {
    ...
    if ( ((Dem_DTCGroupType)DTC == DEM_DTC_GROUP_ALL_DTCS) ||
         (DTC == DEM_DTC_FUNCTIONAL_GROUP_EMISSION) ||
  ...
  uint8 ClientId)
{
  ...
  DEM_ENTER_EXCLUSIVE_AREA();
  if (Dem_ClearDTCResult[ClientId] != DEM_CLEAR_NO_RESULT)
  {
    ...
    Result = Dem_ClearDTCResult[ClientId];
    if (Result != DEM_PENDING)
    {
      /* clearing operation done, reset result */
      Dem_ClearDTCResult[ClientId] = DEM_CLEAR_NO_RESULT;
    }
    IsNewClearing = FALSE;
  }
  DEM_EXIT_EXCLUSIVE_AREA();

  /* no pending Dem_ClearDTCResult -> this is a new clearing process */
...
#endif
    /* Check if the DTCFormat is supported */
    else if (!DEM_IS_DTCFMT_SUPPORTED(DTCSelection.DTCInfo.DTCFormat))
    {
      Result = DEM_WRONG_DTC;
    }
    else
  ...
  uint8 ClientId)
{
  ...
  {
    /* set the Result to be reported by InternalClearDTC() */
    Dem_ClearDTCResult[ClientId] = Result;

    /* !LINKSTO Dem.ASR43.SWS_Dem_00661,1 */
...
#if (DEM_OBDONUDS_SUPPORT)
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01179,1 */
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01178,1 */
    if( (Dem_ClearDTCSelection.DTCInfo.DTC == DEM_DTC_FUNCTIONAL_GROUP_EMISSION) &&
        ( (Dem_ClearDTCSelection.DTCInfo.DTCFormat == DEM_DTC_FORMAT_UDS) ||
          (Dem_ClearDTCSelection.DTCInfo.DTCFormat == DEM_DTC_FORMAT_OBD_3BYTE) ) )
    {
      Dem_ClearDTCSelection.DTCInfo.DTC = DEM_DTC_GROUP_ALL_DTCS;
    }
#endif
    ...
    if (DEM_DTC_GROUP_ALL_DTCS == Dem_ClearDTCSelection.DTCInfo.DTC)
    {
      ...
      if (Dem_ClearDTCSelection.DTCInfo.DTCOrigin == DEM_DTC_ORIGIN_PRIMARY_MEMORY)
      {
        Dem_RgpNotifyClearReadiness(DEM_SID_ClearDTC, DEM_CLEAR_START, 0U);
      }
#endif
    }
#endif
  ...
  {
    /* !LINKSTO Dem.ASR43.SWS_Dem_00664,1 */
...
STATIC FUNC(void, DEM_CODE) Dem_NotifyNewDTCSelection(uint8 ClientId)
{
  ...
  /*
   * ENTER critical section to protect client ClearDTC result
   *       call-context: Dcm (Task), Rte (Task)
   */
  DEM_ENTER_EXCLUSIVE_AREA();

  /* DEM_CLEAR_NO_RESULT indicates Client not interested in return result anymore, hence
   * setting such value here causes the setter function Dem_SetClearDTCResult to
   * ignore any updates for this result till the clearing process is finished */
   /* !LINKSTO Dem.SWS_Dem_01304,1 */
  Dem_ClearDTCResult[ClientId] = DEM_CLEAR_NO_RESULT;
...
STATIC FUNC(void, DEM_CODE) Dem_ResetOBDCounters(void)
{
...
#if (DEM_OBD_CENTRALIZED_PID31_HANDLING == STD_OFF)
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.DistanceTraveledSinceDTCsCleared,1 */
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.DistanceTraveledSinceDTCsCleared,1 */
    /* !LINKSTO Dem.OBD.ClearResetEmissionRelatedDiagInfo.DistanceTraveledSinceDTCsCleared,1 */
    /* !LINKSTO Dem.ClearDiagnosticInformation.AllDTCs.DistanceTraveledSinceDTCsCleared,1 */
    /* Clear the counter for the distance traveled. The counter can be read via PID $31 */
  ...
  if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
  {
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.DistanceTraveledWithMILOn,1 */
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.DistanceTraveledWithMILOn,1 */
    /* Check if it was a Dcm service $14 request for clearing/resetting the
     * emission-related diagnostic information */
    if ( (Dem_ClearDTCSelection.DTCInfo.DTCFormat != DEM_DTC_FORMAT_J1939) &&
         (Dem_ClearDTCSelection.DTCInfo.DTCFormat != DEM_DTC_FORMAT_OBD) &&
...
#endif /* (DEM_OBDONUDS_SUPPORT) */
  {
    ...
    /* Check if it was a Dcm service $04 request for clearing/resetting the
     * emission-related diagnostic information */
    ...
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.WarmupsSinceDTCsCleared,1 */
    /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.WarmupsSinceDTCsCleared,1 */
    /* !LINKSTO Dem.OBD.ClearResetEmissionRelatedDiagInfo.WarmupsSinceDTCsCleared,1 */
    /* !LINKSTO Dem.ClearDiagnosticInformation.AllDTCs.WarmupsSinceDTCsCleared,1 */
    /* Clear the warm-up cycle counter. The counter can be read via PID $30 */
      ...
      else
      {
        ...
        switch (Dem_DoDTCSelection(ClientId, &EventId))
        {
          ...
          case DEM_DTC_SELECTOR_SINGLE_DTC_EVID_CACHED:
            /* !LINKSTO Dem.SWS_Dem_01256,1 */
            Result = E_OK;
            break;
          case DEM_DTC_SELECTOR_WRONG_DTC:
            /* !LINKSTO Dem.SWS_Dem_01257,1 */
...
#pragma warning default
  ...
  for (; EventId <= LastEventId; ++EventId)
  {
    ...
    if (RetVal == E_OK)
    {
      ...
      if (0U == *NextCmbEvIdx)
      {
        /* !LINKSTO Dem.ASR42.SWS_Dem_00515,1 */
      ...
      /* !LINKSTO Dem.ASR40.SWS_Dem_00516,1 */
...
#endif
        {
          /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.ConfirmedAndPendingDTCs,1 */
          /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.ConfirmedAndPendingDTCs,1 */
          ...
          /* !LINKSTO Dem.ASR42.SWS_Dem_00680,1 */
      ...
      {
        /* !LINKSTO Dem.ASR42.SWS_Dem_00668,1 */
        /* !LINKSTO Dem.ASR42.SWS_Dem_00667,1 */

        /* !LINKSTO Dem.SWS_Dem_01332,1 */
        /* ( (RetVal == E_OK) && (Allowed == FALSE) ) */
        /* partially clear the event and the callback function
           Dem_CbInitMonitorForEvent shall not be called */
        if (DEM_ONLY_THIS_CYCLE_AND_READINESS == Behavior)
        {
          /* !LINKSTO Dem.ASR42.SWS_Dem_00669,1 */
#if (DEM_NUM_CMB_DTCS > 0U)
          if (DEM_IS_EVENT_COMBINED(EventId))
          {
            Result =
...
#endif
          {
            /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.IMReadinessBits,1 */
            /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.IMReadinessBits,1 */
  ...
  if (EventId > DEM_MAX_EVENTID)
  {
    ...
    else/* else: status already set to E_OK */
    {
      ...
      if (DEM_DTC_GROUP_ALL_DTCS == (Dem_DTCGroupType)DTC)
      {
        ...
        DEM_CLR_BIT_8(DEM_OVFIND_FLAGS, OriginIdx);
        /*
         * LEAVE critical section
         */
        DEM_EXIT_EXCLUSIVE_AREA();
  ...
  P2VAR(Dem_EventModifiedType, AUTOMATIC, AUTOMATIC)     AnyEventModified)
{
  ...
  /* If the number of the combined events fit in a single MainFucntion
   * Or multiple MainFunctions without remainder, then wait for the next
   * MainFunction. Otherwise start clearing the combined events */
  /* Deviation TASKING-5 */
  /* Deviation TASKING-1 */
  if ( (0U == *NextCmbEvIdx) &&
  ...
  else
  {
    ...
    for (; CmbEvIdx <= LastCmbEvIdx; CmbEvIdx++)
    {
...
#endif
      {
        ...
        /* !LINKSTO Dem.OBDonUDS.CombinedUDSDTCStatus.PDTC.ResetByClear,1 */
        /* !LINKSTO Dem.OBDonUDS.CombinedUDSDTCStatus.CDTC.ResetByClear,1 */
#if (DEM_OBDONUDS_COMBINATION_ENABLED == STD_ON)
        /*reset combined PDTC status when DEM_OBDONUDS_COMBINATION_ENABLED is enabled
         * and clear request is received*/
    ...
    if(Result != DEM_PENDING)
    {
      /* Loop over all the events of the combined DTC and call InitMonitorForEvent */
      Dem_CmbDTCEvLnkType InitMonitorCmbEvIdx = Dem_CmbDTCEvLnk[DTCConfIdx];
      for (; InitMonitorCmbEvIdx <= LastCmbEvIdx; InitMonitorCmbEvIdx++)
      {
        Dem_EventIdType CurrentEventId = Dem_CmbDTCEvents[InitMonitorCmbEvIdx];

        /* !LINKSTO Dem.ASR42.SWS_Dem_00680,1 */
        Dem_CbInitMonitorForEvent(CurrentEventId, DEM_INIT_MONITOR_CLEAR);
      }
    }
#endif
  }
  ...
  P2VAR(Dem_EventModifiedType, AUTOMATIC, AUTOMATIC) AnyEventModified)
{
...
#else
  {
    ...
    for (LinkIdx = 0U; LinkIdx < DEM_NUM_INDICATOR_LINKS; ++LinkIdx)
    {
      if (EventId == Dem_IndicatorLink[LinkIdx].EventId)
      {
        ...
        /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.MILAndNumberOfDTCs,1 */
        /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.MILAndNumberOfDTCs,1 */
        /* !LINKSTO dsn.Dem.IndicatorCycleCounters.Clear,1 */
        /* clear indicator link-related data */
        DEM_CLR_BIT_IN_ARRAY(
  ...
  {
    const Dem_EventIdType PFFIdx = Dem_GetPFFIndex(EventId);
    /* mark prestored freeze frame as unused */
    DEM_CLR_BIT_IN_ARRAY(Dem_PFFEntryValid, PFFIdx);
  }
#endif /* DEM_MAX_NUMBER_PRESTORED_FF > 0U */

  /* reset debouncing counters for event (Dem.ASR40.SWS_Dem_00343) */
...
#endif
  {
    ...
    if (Dem_SearchForEntry(EventId, &EvMemEntry, &EvMemEntryIdx) == TRUE)
    {
      ...
      /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.SnapshotDataDTCs,1 */
      /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.EmissionsGroup.SnapshotData,1 */
      /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.SnapshotDataDTCs,1 */
      /* !LINKSTO Dem.OBDonUDS.ClearResetEmissionRelatedDiagInfo.AllDTCs.SnapshotData,1 */
...
#endif
    {
...
#endif /* (DEM_EVENT_DISABLING_SUPPORT == STD_ON) */
      {
             ...
             (Dem_ClearDTCSelection.DTCInfo.DTCFormat == DEM_DTC_FORMAT_OBD_3BYTE) )
        {
          ...
          {
            uint32 DTCOfEvent = DEM_NO_DTC;
#if (DEM_OBDONUDS_SUPPORT && (DEM_OBD_DTCVALUE3BYTE_SUPPORT == STD_ON))
            if (Dem_ClearDTCSelection.DTCInfo.DTCFormat == DEM_DTC_FORMAT_OBD_3BYTE)
            {
              DTCOfEvent = Dem_GetObdDTCValue3Byte(EventId);
            }
            else
#endif
            {
              DTCOfEvent = Dem_GetObdDTC(EventId);
            }
            if (DTCOfEvent == DTC)
            {
              EntryDTC = TRUE;
...
#if (DEM_NUM_CMB_DTCS > 0U)
  if (Dem_ClearDtcAsyncStatus.NextCmbEvIdx != 0U)
  {
    Result = E_OK;
  }
  else
...
STATIC FUNC(void, DEM_CODE) Dem_TriggerRGPNotification(void)
{
  DBG_DEM_TRIGGERRGPNOTIFICATION_ENTRY();

#if ((DEM_J1939_READINESS1_CALCULATION_SUPPORT == STD_ON) && \
     (DEM_J1939_CLEAR_DTC_SUPPORT == STD_ON))
  if((Dem_ClearDtcAsyncStatus.ClientId == DEM_VIRTUAL_CLIENT_J1939) &&
     (Dem_ClearDtcAsyncStatus.Dem_J1939DTCTypeFilter == DEM_J1939DTC_CLEAR_ALL))
  {
    Dem_RgpNotifyClearReadiness(DEM_SID_J1939DcmClearDTC,
          ...
          (DEM_J1939_CLEAR_DTC_SUPPORT == STD_ON) */
  {
    if ( (Dem_ClearDTCSelection.DTCInfo.DTC == DEM_DTC_GROUP_ALL_DTCS) &&
         (Dem_ClearDTCSelection.DTCInfo.DTCOrigin == DEM_DTC_ORIGIN_PRIMARY_MEMORY) )
    {
      Dem_RgpNotifyClearReadiness(DEM_SID_ClearDTC, DEM_CLEAR_FINISH, 0U);
    }
  }

  DBG_DEM_TRIGGERRGPNOTIFICATION_EXIT();
}
#endif
  ...
  uint8                                                ServiceId)
{
  ...
  /* !LINKSTO Dem.ASR40.SWS_Dem_00570,1 */
