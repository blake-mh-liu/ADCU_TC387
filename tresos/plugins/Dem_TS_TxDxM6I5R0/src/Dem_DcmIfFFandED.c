 ...
 *
 * Reason:
 * Value for "SelectedRecNum" depends on the Client requested RecNum. In other configurations,
 * the code coverage reaches sections where the SelectedRecNum is DEM_ALL_OBD_ED_RECORD_NUM.
 *
 * TASKING-5) Deviated Rule: W549
 * W549: condition is always false
 *
 * Reason:
 * "isDevFFIdxValid" it will be TRUE only if EvMemEntry->DevFFEntryIdx[FFKind] < Dem_SizeDevFFMem[FFKind].
 * In other configurations, the code coverage reaches sections where the isDevFFIdxValid is
...
#if (defined DEM_TOTAL_SIZE_CLASSIC_OBDFF )
#error DEM_TOTAL_SIZE_CLASSIC_OBDFF  already defined
#endif
/** \brief Macro to get whole size of OBD Freeze Frame */
#define DEM_TOTAL_SIZE_CLASSIC_OBDFF  \
  (DEM_SIZE_CLASSIC_OBD_FF + (2U * DEM_NUM_OF_PIDS_IN_OBDFF) + 2U)

#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
#if (defined DEM_TOTAL_SIZE_OBDONUDS_FF)
#error DEM_TOTAL_SIZE_OBDONUDS_FF already defined
#endif
/** \brief Macro to get whole size of OBDonUDS Freeze Frame */
#define DEM_TOTAL_SIZE_OBDONUDS_FF \
  (DEM_SIZE_OBDONUDS_OBD_FF + (2U * DEM_NUM_OF_DIDS_IN_OBDONUDS_FF) + 2U)
#endif
/*==================[type definitions]======================================*/
 ...
 **
 ** \return Number of FF bytes which are copied to DestBuffer
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(uint16, DEM_CODE) Dem_CopyFFInfo(
...
STATIC FUNC(boolean, DEM_CODE) Dem_FindNextFFRecForReporting(
  ...
  P2VAR(Dem_FFKindType, AUTOMATIC, DEM_APPL_DATA) FFKind);
    ...
    (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
 ...
 **
 ** \param[in] EventId              Id of the event whose extended data records
 **                                 are searched for
...
VAR(Dem_EventIdType, DEM_VAR_CLEARED) Dem_EvIdOfLockedMemoryEntry[DEM_NUMBER_OF_CLIENTS];
...
/** \brief Index of the next available entry in DTC record locking arrays */
VAR(uint8, DEM_VAR_CLEARED) Dem_DTCRecordLockingNextFreeIndex;
#endif
 ...
 **  enabled (unlocked).
 */
STATIC VAR(uint8, DEM_VAR_CLEARED) Dem_ClientIdLockingDTCRecordUpdate[DEM_NUMBER_OF_CLIENTS];
#endif
...
#if (DEM_NUMBER_OF_CLIENTS > 1U)
  Dem_DTCRecordLockingNextFreeIndex = 0U;

  while(MemoryIndex < DEM_NUMBER_OF_CLIENTS)
  {
    Dem_ClientIdLockingDTCRecordUpdate[MemoryIndex] = DEM_CLIENT_ID_INVALID;

    Dem_EvIdOfLockedMemoryEntry[MemoryIndex] = DEM_EVENT_ID_INVALID;

    MemoryIndex++;
  }
#else
  Dem_EvIdOfLockedMemoryEntry[0U] = DEM_EVENT_ID_INVALID;
#endif
        ...
        case DEM_DTC_SELECTOR_SINGLE_DTC_EVID_CACHED:
          {
            ...
            while(ClientIndex < Dem_DTCRecordLockingNextFreeIndex)
            {
              if(Dem_ClientIdLockingDTCRecordUpdate[ClientIndex] == ClientId)
              {
                ClientIsLocking = TRUE;
            ...
            if (ClientIsLocking == TRUE)
            {
              if((Dem_EvIdOfLockedMemoryEntry[ClientIndex] != MasterEventId))
              {
              ...
              {
                /* the current client disabled record update for given DTC, nothing to be done */
                Result = E_OK;
              }
            }
            else
            {
              ...
              Dem_ClientIdLockingDTCRecordUpdate[Dem_DTCRecordLockingNextFreeIndex] = ClientId;
              Dem_EvIdOfLockedMemoryEntry[Dem_DTCRecordLockingNextFreeIndex] = MasterEventId;

              Dem_DTCRecordLockingNextFreeIndex++;
...
#else
            if ( (Dem_EvIdOfLockedMemoryEntry[0U] == DEM_EVENT_ID_INVALID) ||
                 (Dem_EvIdOfLockedMemoryEntry[0U] == MasterEventId) )
            {
              /* DTC record update is either not disabled or already disabled
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
...
#endif /* DEM_DEV_ERROR_DETECT */
    {
...
#if (DEM_NUMBER_OF_CLIENTS > 1U)
      while(Index < Dem_DTCRecordLockingNextFreeIndex)
      {
        if(Dem_ClientIdLockingDTCRecordUpdate[Index] == ClientId)
        {
          break;
        }
        Index++;
...
#endif
      LockedEvId = Dem_EvIdOfLockedMemoryEntry[Index];
      ...
      if (DEM_EVENT_ID_INVALID == LockedEvId)
      {
        ...
        /* Requesting client did not previously disable any DTC update record.
         * Report a DET if there are other clients already locking DTCs */
        /* !LINKSTO Dem.ASR431.EnableDTCRecordUpdate.DetReporting,1 */
        if (Dem_DTCRecordLockingNextFreeIndex > 0U)
        {
          Result = E_NOT_OK;
        }
#endif
      ...
      else
      {
        ...
        /* !LINKSTO Dem.ASR40.SWS_Dem_00271,3 */
#if (DEM_NUMBER_OF_CLIENTS > 1U)
        /* unlock DTC event entry and close array gaps if any */
        Dem_EvIdOfLockedMemoryEntry[Index] =
          Dem_EvIdOfLockedMemoryEntry[Dem_DTCRecordLockingNextFreeIndex - 1U];
        Dem_EvIdOfLockedMemoryEntry[Dem_DTCRecordLockingNextFreeIndex - 1U] = DEM_EVENT_ID_INVALID;

        Dem_ClientIdLockingDTCRecordUpdate[Index] =
          Dem_ClientIdLockingDTCRecordUpdate[Dem_DTCRecordLockingNextFreeIndex - 1U];
        Dem_ClientIdLockingDTCRecordUpdate[Dem_DTCRecordLockingNextFreeIndex - 1U] =
          DEM_CLIENT_ID_INVALID;
...
#if ( (DEM_USE_AGING == STD_ON) && \
      (DEM_EXTENDED_DATA_CAPTURE != DEM_TRIGGER_ON_FDC_THRESHOLD) )
        /* checking that event ID is unlocked from every client, this is necessary as there might
         * be other clients locking the same event ID */
#if (DEM_NUMBER_OF_CLIENTS > 1U)
        IsLockedEvId = DEM_RECORD_UPDATE_LOCKED(LockedEvId);
#endif
        /* clear disabled event from event memory if it was aged and "globally" unlocked */
        if (DEM_AGING_ALLOWED(LockedEvId))
        {
#if (DEM_NUMBER_OF_CLIENTS > 1U)
          if (IsLockedEvId == FALSE)
#endif
          {
#if (DEM_USE_IMMEDIATE_NV_STORAGE == STD_ON)
      ...
      (DEM_EXTENDED_DATA_CAPTURE != DEM_TRIGGER_ON_FDC_THRESHOLD) )
      /* Deviation TASKING-1 */
      if ((Result == E_OK) && (IsLockedEvId == FALSE))
      {
#if ( (DEM_USE_CB_TRIG_ON_STATUS_CHANGE == STD_ON) || \
      (DEM_NUM_CMB_DTCS > 0U) )
  ...
  P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA) BufSize)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    if (Dem_RecordSelection[ClientId].Status == DEM_DATA_RECORD_SELECTOR_FF)
    {
      ...
      uint8 NextAvailableRecNum = DEM_OBD_FF_RECORD_NUM_FIRST;
      ...
      while (Dem_RecordSelection[ClientId].NextRecNum != DEM_END_OF_RECORD_SEARCH)
      {
        ...
        /* !LINKSTO Dem.SWS_Dem_00576_Implicit1,1 */
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
        /* !LINKSTO dsn.Dem.CheckRecordNumber.SearchForEntry,3 */
        /* !LINKSTO Dem.SWS_Dem_00576,1 */
        /* Handling of DEM_OBD_FF_RECORD_NUM_FIRST (0x00) is intentionally kept separately,
           because the algorithm in the else-path can not handle a 0 value */
        if (NextRecNum == DEM_OBD_FF_RECORD_NUM_FIRST)
        {
          if (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS)
          {
            /* collect OBD FF record data header info and update result */
            /* !LINKSTO Dem.SWS_Dem_00071,1 */
            /* !LINKSTO Dem.SWS_Dem_00630,1 */
...
#endif /* ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON)) */
        {
                                                     ...
                                                     &FFKind))
          {
            ...
            {
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
              /* no run-time check of the OBD variant needed here, since
               * it is done inside Dem_FindNextFFRecForReporting() */
              if (FFKind == DEM_OBDONUDS_FF)
              {
                /* collect OBDonUDS FF last occurrence record and update result */
                /* !LINKSTO Dem.OBDonUDS.OBDFF.RetrieveLastOccurrence,1 */
                Result = Dem_CollectOBDFreezeFrameInfo(EventId,
                                                       NextAvailableRecNum,
                                                       BufSize,
                                                       DestBuffer);
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    if (Dem_RecordSelection[ClientId].Status == DEM_DATA_RECORD_SELECTOR_FF)
    {
      ...
      */
...
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
      if ( (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS) &&
           ( (SelectedRecNum == DEM_OBD_FF_RECORD_NUM_FIRST) ||
             ( (SelectedRecNum == DEM_OBD_FF_RECORD_NUM_LAST) &&
      ...
      {
        /* !LINKSTO Dem.SWS_Dem_00074_Implicit1,1 */
        /* !LINKSTO dsn.Dem.DevFF.IB.GetSizeOfFreezeFrameSelection,1 */
        ...
        if (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS)
        {
          uint16 OBDSize = Dem_SizeOfOBDFreezeFrameInfo(EventId);
          SizeOfFF += OBDSize;
#if(DEM_OBDONUDS_FF_SUPPORT == STD_ON)
          /* Deviation MISRAC2012-1 */
          if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
          {
            /* For OBDonUDS Last occurrence FF, another instance of FF record exists
             * and hence add the OBD FF size again. */
            SizeOfFF += OBDSize;
          }
#endif
        }
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
   ...
   (DEM_NUM_CAPTURE_ON_RETRIEVAL_DATAELEMENTS > 0U))
              ...
              else
              {
                ...
                /* !LINKSTO Dem.DataElementCaptureOnRetrieval.CallBackEventId.GetNextEDR,1 */
...
#endif
      {
             ...
             (SelectedRecNum == DEM_ALL_RECORD_NUM) )
        {
          ...
          for (SegIndex = 0U; (SegIndex < EDClass->NumEDSegs); ++SegIndex)
          {
            ...
            /* !LINKSTO Dem.SWS_Dem_00076_Implicit1,1 */
            /* !LINKSTO Dem.SWS_Dem_00076_Implicit2,1 */
            if ( (SelectedRecNum == DEM_ALL_RECORD_NUM) ||
                 ( (SelectedRecNum == DEM_ALL_OBD_ED_RECORD_NUM) &&
                 ...
                 (SelectedRecNum == EDSegment->RecNum) )
            {
              ...
              /* !LINKSTO Dem.OBDonUDS.MAR.MiniNumerator.Report,1 */
              /* !LINKSTO Dem.OBDonUDS.MAR.MonitorActivityRatio.Report,1 */
...
#if ((DEM_USE_MAPPED_INT_DATA_ELEMENTS == STD_ON) || \
     (DEM_NUM_CAPTURE_ON_RETRIEVAL_DATAELEMENTS > 0U))
                  SizeOfEDR += Dem_SizeOfNonStoredDEsInSegment(EDSegment);
#endif
                  SizeOfEDR += Dem_SizeOfStoredDEsInSegment(EDSegment);
                }
              }
            }
          }
  ...
  P2VAR(Dem_FFKindType, AUTOMATIC, DEM_APPL_DATA) FFKind)
{
  ...
  *FoundRecordNumber = DEM_MAX_FF_RECORD_NUM + 1U;
  ...
  /* exclude DEM_OBD_FF_RECORD_NUM_FIRST for this case, even though this function is
   * not expected to be called with it */
  if ( (RecordNumberToSearch <= Dem_GbiMaxNumFFRecs(EventId)) &&
       (RecordNumberToSearch != DEM_OBD_FF_RECORD_NUM_FIRST)
     )
  {
  ...
  if ((DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
      (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS) &&
      (RecordNumberToSearch <= DEM_OBD_FF_RECORD_NUM_LAST) &&
      (DEM_OBD_FF_RECORD_NUM_LAST < *FoundRecordNumber))
  {
    *FoundRecordNumber = DEM_OBD_FF_RECORD_NUM_LAST;
    *FFKind = DEM_OBDONUDS_FF;
    Result = TRUE;
  }
#endif
  ...
  if (EntryExists == TRUE)
  {
       ...
       )
    {
      ...
      if (NeededFFSize <= *BufSize)
      {
        ...
        (void)Dem_CopyFFInfo(FFKind, FFIndClass, FFEntryData, &DestBuffer[2]);
  ...
  uint8                                   RecordNumber)
{
  ...
  if (EntryExists == TRUE)
  {
    ...
    if (FFIdxValid == TRUE)
    {
      ...
      SizeOfFFKind = 2U;
      SizeOfFFKind = SizeOfFFKind + Dem_SizeOfFF(FFKind, FFIndClass);
      SizeOfFFKind = SizeOfFFKind +
    ...
    P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA)  DestBuffer)
{
  ...
  if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
  {
    /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressFirstOccurrence.GetNextFreezeFrameData,1 */
    /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressLastOccurrence.GetNextFreezeFrameData,1 */
    if ( Dem_IsAnyDTCStatusBitSet(EventId,
           (DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_PDTC)) == TRUE )
    {
      EntryExists = Dem_SearchForEntry(EventId , NULL_PTR, &SelectedEntryIdx);
    }
  }
  /* Deviation MISRAC2012-1 */
  else
  ...
  OBDFFEventId = Dem_EventMem[(DEM_DTC_ORIGIN_PRIMARY_MEMORY - 1U)][SelectedEntryIdx].EventId;
  ...
  OBDFFEventId = DEM_OBDFF_STORED_EVENT_ID;
#endif /* (DEM_OBDFF_SUPPORT_KIND == DEM_OBDFF_MULTIPLE) */

  if ( (EntryExists == TRUE) &&
       (DEM_GET_MASTER_EVID(EventId) == DEM_GET_MASTER_EVID(OBDFFEventId)) )
  {
    /* check if the possible biggest OBD FF (or OBD on UDS FF if enabled) fits
     * into the buffer,do not care about disabled PIDs or DIDs*/
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
    if ( ((DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
          (*BufSize >= DEM_TOTAL_SIZE_OBDONUDS_FF)) ||
         ((DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBD_CLASSIC) &&
          (*BufSize >= DEM_TOTAL_SIZE_CLASSIC_OBDFF )) )
#else
    if (*BufSize >= DEM_TOTAL_SIZE_CLASSIC_OBDFF )
#endif
    {
      /* write data for one record: */
      /*  byte 0: record number */
      ...
      if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
      {
        uint16 OdbonUdsFFSize = 0U;
        /* !LINKSTO Dem.OBDonUDS.OBDFF.RetrieveFirstOccurrence,1 */
        DestBuffer[1] = Dem_GetNumFFSegs(DEM_OBDONUDS_FF,
                                        &Dem_FFClass[DEM_OBDONUDS_FF_CLASS_IDX]);
        if(RecordNumber == DEM_OBD_FF_RECORD_NUM_FIRST)
        {
          OdbonUdsFFSize = Dem_CopyFFInfo(DEM_OBDONUDS_FF,
                             &Dem_FFClass[DEM_OBDONUDS_FF_CLASS_IDX],
                             DEM_OBDFF_ENTRY_FFDATA_LINK(SelectedEntryIdx), &DestBuffer[2]);
        }
        if(RecordNumber == DEM_OBD_FF_RECORD_NUM_LAST)
        {
          OdbonUdsFFSize = Dem_CopyFFInfo(DEM_OBDONUDS_FF,
                             &Dem_FFClass[DEM_OBDONUDS_FF_CLASS_IDX],
                             DEM_OBDFF_ENTRY_FFDATA_2_LINK(SelectedEntryIdx), &DestBuffer[2]);
        }
        /* add size for record number and segments */
        *BufSize = OdbonUdsFFSize + 2U;
      }
      /* Deviation MISRAC2012-1 */
      else
#endif
    ...
    Dem_EventIdType                         EventId)
{
  ...
  if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
  {
    /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressFirstOccurrence.GetSizeOfFreezeFrameSelection,1 */
    /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressLastOccurrence.GetSizeOfFreezeFrameSelection,1 */
    if ( Dem_IsAnyDTCStatusBitSet(EventId,
           (DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_PDTC)) == TRUE )
    {
      EntryExists = Dem_SearchForEntry(EventId , NULL_PTR, &SelectedEntryIdx);
  ...
  OBDFFEventId = Dem_EventMem[0][SelectedEntryIdx].EventId;
       ...
       (DEM_GET_MASTER_EVID(EventId) == DEM_GET_MASTER_EVID(OBDFFEventId)) )
  {
    ...
    if(DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS)
    {
      ...
      ObdFFSize  = 2U;
      ObdFFSize += Dem_SizeOfFF(DEM_OBDONUDS_FF,
                               &Dem_FFClass[DEM_OBDONUDS_FF_CLASS_IDX]);
      ObdFFSize += (2U * (uint16)Dem_GetNumFFSegs(DEM_OBDONUDS_FF,
                                                 &Dem_FFClass[DEM_OBDONUDS_FF_CLASS_IDX]));
#else
      ObdFFSize = DEM_TOTAL_SIZE_OBDONUDS_FF;
#endif /* (DEM_CALIBRATION_WITHOUTEVCOMB_SUPPORT == STD_ON) */
    }
    /* Deviation MISRAC2012-1 */
    else
#endif
    ...
    ObdFFSize += 2U;
#else
    ObdFFSize = DEM_TOTAL_SIZE_CLASSIC_OBDFF ;
#endif /* (DEM_CALIBRATION_WITHOUTEVCOMB_SUPPORT == STD_ON) */
...
#endif
    {
      /* map PID to DID according Dem.ASR42.SWS_Dem_00686, DID --> 0xF4 + PID */
  ...
  P2VAR(Dem_EDStartByteType, AUTOMATIC, DEM_APPL_DATA)       EDStartByte)
{
  ...
  DBG_DEM_FINDNEXTEDRECFORREPORTING_ENTRY(EventId, EDClass,
...
#if (DEM_MAR_SUPPORT == STD_OFF)
  TS_PARAM_UNUSED(EventId);
#endif

  DBG_DEM_FINDNEXTEDRECFORREPORTING_EXIT(Result, EventId,
