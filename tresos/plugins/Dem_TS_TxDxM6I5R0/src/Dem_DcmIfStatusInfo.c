 ...
 *
 * Reason:
 * Values for the "NumStoredFFs" depends on each event memory entry, the code coverage
 * reaches sections where the NumStoredFFs is greater than ClientFFRecordFilter->RecNumIdx.
 *
 * TASKING-4) Deviated Rule: W549
 * W549: condition is always false
 *
 * Reason:
 * "FFRecordFound" remains FALSE only if freeze frame records not found in event memory.
 * In other configurations, the code coverage reaches sections where the FFRecordFound is
 * different from the value FALSE.
 */
...
#if (defined DEM_DTC_FILTER_INVALID)
#error DEM_DTC_FILTER_INVALID already defined
#endif
/** \brief Macro for set DTC filter API sets during initialization */
#define DEM_DTC_FILTER_INVALID        0U

#if (defined DEM_DTC_FILTER_DTCFILTER)
#error DEM_DTC_FILTER_DTCFILTER already defined
#endif
/** \brief Macro for API Dem_SetDTCFilter */
#define DEM_DTC_FILTER_DTCFILTER      1U

#if (defined DEM_DTC_FILTER_EXTDATAFILTER)
#error DEM_DTC_FILTER_EXTDATAFILTER already defined
#endif
/** \brief Macro for API Dem_SetDTCFilterByExtendedDataRecordNumber */
#define DEM_DTC_FILTER_EXTDATAFILTER  2U

#if (defined DEM_DTC_FILTER_READINESSFILTER)
#error DEM_DTC_FILTER_READINESSFILTER already defined
#endif
/** \brief Macro for API Dem_SetDTCFilterByReadinessGroup */
#define DEM_DTC_FILTER_READINESSFILTER 3U

#if (defined DEM_DTCSEVERITY_INFO_MASK)
#error DEM_DTCSEVERITY_INFO_MASK already defined
#endif
/** \brief Mask for separating DTC Severity */
#define DEM_DTCSEVERITY_INFO_MASK (DEM_SEVERITY_MAINTENANCE_ONLY   | \
                                   DEM_SEVERITY_CHECK_AT_NEXT_HALT | \
                                   DEM_SEVERITY_CHECK_IMMEDIATELY)

#if (defined DEM_DTCCLASS_INFO_MASK)
#error DEM_DTCCLASS_INFO_MASK already defined
#endif
/** \brief Mask for separating DTC Class */
#define DEM_DTCCLASS_INFO_MASK (DEM_SEVERITY_WWHOBD_CLASS_NO_CLASS | \
...
/** \brief Performs the internal processing of the
 **        Dem_GetNextFilteredDTCAndSeverity API
 **
 ** \param[in]  ClientId           Client ID for which the next filtered
 **                                DTC and Severity will be retrieved.
 ** \param[out] DTC                Receives the DTC value in respective format
 **                                of the filter returned by this function. If
 **                                the return value of thefunction is other
 **                                than E_OK this parameter does not contain
 **                                valid data.
 ** \param[out] DTCStatus          Receives the status value returned by the
 **                                function. If the return value is other
 **                                than E_OK this parameter does not contain
 **                                valid data.
 ** \param[out] DTCSeverity        Receives the severity value returned by the
 **                                function. If the return value is other
                                   than E_OK this parameter does not contain
 **                                valid data.
 ** \param[out] DTCFunctionalUnit  Receives the functional unit value returned
 **                                by the function. If the return value is other
 **                                than E_OK this parameter does not contain
 **                                valid data.
 **
 ...
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_GetDTCAndSeverityInternal(
  uint8                                                ClientId,
 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(boolean, DEM_CODE) Dem_ApplyEDRNFilter(
  uint8                                                    ClientId,
  Dem_EventIdType                                          EventId,
  P2VAR(Dem_DTCStatusMaskType, AUTOMATIC, DEM_APPL_DATA)   DTCStatus,
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                  DTC);

/** \brief Helper function used to count the DTCs filtered with
 **        Dem_SetDTCFilter() API
 **
 ** \param[in] ClientId
 **
 ** \return uint16 value, representing the number of matching DTCs
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(uint16, DEM_CODE) Dem_GetNumOfDTCByFilter(uint8 ClientId);

/** \brief Helper function used to count the DTCs filtered with
 **        Dem_SetDTCFilterByExtendedDataRecordNumber() API
 **
 ** \param[in] ClientId
 **
 ** \return uint16 value, representing the number of matching DTCs
 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(uint16, DEM_CODE) Dem_GetNumOfDTCByEDRN(uint8 ClientId);

/** \brief Helper function used to retrieve the next DTC filtered with
 **        Dem_SetDTCFilter() API
 **
 ** \param[in]    ClientId
 ** \param[inout] DTCStatus
 ** \param[inout] DTC
 **
 ** \return Std_ReturnType, indicates if a new matching DTC was retrieved
 ** \retval E_OK:                Successfuly returned next DTC.
 ** \retval DEM_NO_SUCH_ELEMENT: No further DTC that matches the filter
                                 criteria was found.
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_GetNextDTCByFilter(
  uint8                                                  ClientId,
  P2VAR(Dem_DTCStatusMaskType, AUTOMATIC, DEM_APPL_DATA) DTCStatus,
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                DTC);
 ...
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_GetNextDTCByEDRN(
  uint8                                                  ClientId,
  P2VAR(Dem_DTCStatusMaskType, AUTOMATIC, DEM_APPL_DATA) DTCStatus,
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                DTC);
#endif
...
/** \brief Function to check extended data record number is supported
 **
 **
 ** \param[in] EvId
 ** \param[in] ExtendedDataRecordNumber
 **
 ** \return Boolean value, if the ED exist for the specified event
 ** \retval TRUE  ED is supported
 ** \retval FALSE ED is not supported
 **
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(boolean, DEM_CODE) Dem_IsEDRNSupported(
...
/** \brief Function to update the flags indicating whether an event is assigned
 ** or not to a readiness group
 **
 ** The function is invoked at the first call made for readiness group NONE
 ** of the Dem_SetDTCFilterByReadinessGroup() API
 **
 ** \Reentrancy{Not reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(void, DEM_CODE) Dem_UpdateRGEventsFlags(void);
#endif
...
/* !LINKSTO Dem.ASR40.SWS_Dem_00410_Implicit1,1, Dem.ASR40.SWS_Dem_00410_Implicit2,1 */
...
STATIC VAR(Dem_EventIdType, DEM_VAR_CLEARED) Dem_OrderedEventId
  [DEM_NUMBER_OF_CLIENTS][DEM_MAX_NUM_ORDERED_EVENT_IDS];
...
FUNC(void, DEM_CODE) Dem_InitDTCFilter(void)
{
  ...
  {
    Dem_DTCFilter[Idx].CurrentFilterType = DEM_DTC_FILTER_INVALID;
  }
}
  ...
  boolean                    FilterForFaultDetectionCounter)
{
  ...
  /* for backwards compatibility, report Det if defined DTC format J1939 is selected */
  else if (!DEM_IS_DTCFMT_DEFINED(DTCFormat) || (DTCFormat == DEM_DTC_FORMAT_J1939))
  {
  ...
  {
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_00057,1 */
    ...
    Dem_DTCOriginType MappedDTCOrigin = Dem_GetMappedDTCOrigin(
        DTCOrigin, DEM_DTC_ORIGIN_MAPPING_ASR43_IF);

    /* reset event to read next */
    /* !LINKSTO dsn.Dem.ReportingOrderDTC.SetDtcFilter.InitNextItem,1 */
    /* !LINKSTO Dem.SetDTCFilter.Reset,3 */
    ClientDTCFilter->NextItem = 1U;
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01363,1 */
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01266,1 */
    ClientDTCFilter->CurrentFilterType = DEM_DTC_FILTER_INVALID;
    ...
    if(MappedDTCOrigin != DEM_DTC_ORIGIN_INVALID)
    {
      ...
      else if (!DEM_IS_DTCFMT_SUPPORTED(DTCFormat))
      {
        /* !LINKSTO Dem.ASR431.SetDTCFilter.InvalidDTCFormat,1 */
        /* exit with E_NOT_OK */
        /* For backwards compatibility, E_NOT_OK is returned for "unsupported" DTCFormat values */
      }
      else
      {
        ...
        /* !LINKSTO Dem.ASR40.SWS_Dem_00410_Implicit2, 1 */
        /* save info if:
        ...
        /* !LINKSTO dsn.Dem.ReportingOrderDTC.SetDtcFilter.InitNextItem,1 */
        ClientDTCFilter->NextItem = 1U;
        ClientDTCFilter->CurrentFilterType = DEM_DTC_FILTER_DTCFILTER;
  ...
  uint8                      ExtendedDataRecordNumber)
{
  ...
  DBG_DEM_SETDTCFILTERBYEXTDATARECNUM_ENTRY(
    ClientId, DTCFormat, ExtendedDataRecordNumber);

#if (DEM_DEV_ERROR_DETECT == STD_ON)
  /* check of Dem_InitializationState would be not necessary, but if this API
   * is called before full initialization, the system design is dubious
   * (and Dem_GetNextFilteredDTC() is then not usable anyway) */
  if (Dem_InitializationState != DEM_INITIALIZED)
  {
    DEM_REPORT_ERROR(DEM_SID_DTCFilterByExtDataRecordNumber, DEM_E_UNINIT);
  }
#if (DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON)
  else if (GetCoreID() != DEM_MASTER_INSTANCE)
  {
    /* !LINKSTO Dem.BSWDistribution.ContextCheckForNonDistributedFunctionalities,1 */
    DEM_REPORT_ERROR(DEM_SID_DTCFilterByExtDataRecordNumber, DEM_E_WRONG_CONTEXT);
  }
#endif /* DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON */
  else if (ClientId >= DEM_NUMBER_OF_CLIENTS)
  {
    DEM_REPORT_ERROR(DEM_SID_DTCFilterByExtDataRecordNumber, DEM_E_WRONG_CONFIGURATION);
  }
  /* check if the DTCFormat is invalid */
  /* !LINKSTO Dem.ASR21-11.SWS_Dem_01373,1 */
  else if (!DEM_IS_UDS_DTCFMT_DEFINED(DTCFormat))
  {
    DEM_REPORT_ERROR(DEM_SID_DTCFilterByExtDataRecordNumber, DEM_E_PARAM_DATA);
  }
  else
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* !LINKSTO dsn.Dem.ReportingOrderDTC.SetDtcFilter.InitNextItem,1 */
    ClientDTCFilter->NextItem = 1U;
    /* !LINKSTO Dem.SetDTCFilterByExtendedDataRecordNumber.Reset,1 */
#if (DEM_ORDERED_DTC_REPORTING_USED == STD_ON)
    ClientDTCFilter->OccOrdering = FALSE;
    ClientDTCFilter->EventsInMemory = 0U;
#endif
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01266,1 */
    ClientDTCFilter->CurrentFilterType = DEM_DTC_FILTER_INVALID;

#if (DEM_NUM_EDCLS > 1)
    /* !LINKSTO Dem.SetDTCFilterByEDRecordNumber.InvalidEDRecordNumber,2 */
    if ((ExtendedDataRecordNumber > 0x00U) && (ExtendedDataRecordNumber < 0xFEU))
    {
      /* !LINKSTO Dem.SetDTCFilterByEDRecordNumber.MemoryOrigin,1 */
      /* save filter information in internal data structure */
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01377,1 */
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01378,1 */
      ClientDTCFilter->DTCFormat = DTCFormat;
      ClientDTCFilter->DTCOrigin = DEM_DTC_ORIGIN_PRIMARY_MEMORY;
      ClientDTCFilter->ExtendedDataRecordNumber = ExtendedDataRecordNumber;

      /* Save the current filter type in internal data structure */
      ClientDTCFilter->CurrentFilterType = DEM_DTC_FILTER_EXTDATAFILTER;

      Result = E_OK;
    }
#else
    TS_PARAM_UNUSED(DTCFormat);
    TS_PARAM_UNUSED(ExtendedDataRecordNumber);
#endif /* DEM_NUM_EDCLS > 1 */
  }
  DBG_DEM_SETDTCFILTERBYEXTDATARECNUM_EXIT(
    Result, ClientId, DTCFormat, ExtendedDataRecordNumber);
  return Result;
}
  ...
  Dem_EventOBDReadinessGroupType ReadinessGroupNumber)
{
  ...
  DBG_DEM_SETDTCFILTERBYREADINESSGROUP_ENTRY(
    ClientId, DTCFormat, ReadinessGroupNumber);
  ...
  if (Dem_InitializationState != DEM_INITIALIZED)
  {
    DEM_REPORT_ERROR(DEM_SID_SetDTCFilterByReadinessGroup, DEM_E_UNINIT);
  }
#if (DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON)
  else if (GetCoreID() != DEM_MASTER_INSTANCE)
  {
    /* !LINKSTO Dem.BSWDistribution.ContextCheckForNonDistributedFunctionalities,1 */
    DEM_REPORT_ERROR(DEM_SID_SetDTCFilterByReadinessGroup, DEM_E_WRONG_CONTEXT);
  }
#endif /* DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON */
  else if (ClientId >= DEM_NUMBER_OF_CLIENTS)
  {
    DEM_REPORT_ERROR(DEM_SID_SetDTCFilterByReadinessGroup,
                     DEM_E_WRONG_CONFIGURATION);
  }
  /* check if the DTCFormat is invalid */
  /* !LINKSTO Dem.ASR21-11.SWS_Dem_01373,1 */
  /* !LINKSTO Dem.ASR21-11.SWS_Dem_01376,1 */
...
#endif
  {
    DEM_REPORT_ERROR(DEM_SID_SetDTCFilterByReadinessGroup, DEM_E_PARAM_DATA);
  }
  else
#endif /* DEM_DEV_ERROR_DETECT */
  {
...
#endif
    {
      /* reset event to read next */
      /* !LINKSTO dsn.Dem.ReportingOrderDTC.SetDtcFilter.InitNextItem,1 */
      /* !LINKSTO Dem.SetDTCFilterByReadinessGroup.Reset,1 */
      ClientDTCFilter->NextItem = 1U;
#if (DEM_ORDERED_DTC_REPORTING_USED == STD_ON)
      ClientDTCFilter->OccOrdering = FALSE;
#endif

      /* !LINKSTO Dem.SetDTCFilterByReadinessGroup.InvalidReadinessGroupNumber,1 */
      if (ReadinessGroupNumber <= DEM_RGP_MAX_READINESS_GROUP_ID_ISO)
      {
        /* save filter information in internal data structure */
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01374,1 */
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01375,1 */
        ClientDTCFilter->DTCFormat = DTCFormat;
        ClientDTCFilter->ReadinessGroupNumber = ReadinessGroupNumber;
        ClientDTCFilter->CurrentFilterType = DEM_DTC_FILTER_READINESSFILTER;

#if (DEM_READINESS_CALIBRATION_SUPPORT == STD_ON)
        /* the first call of Dem_SetDTCFilterByReadinessGroup() API made for
         * readiness group NONE after Dem initialization will invoke
         * Dem_UpdateRGEventsFlags() helper function to synchronize the flags
         * held in Dem_RGEvents[] bit-array with the calibratable values from
         * Dem_OBDReadinessGroup[] array */
        if ( (ReadinessGroupNumber == DEM_OBD_RDY_NONE) &&
             (DEM_ARE_RG_FLAGS_UPDATED == FALSE) )
        {
          /* the first bit of the array is set in order to indicate that the RG flags
           * are sychronized with the calibratable values from Dem_OBDReadinessGroup[]
           * after Dem restart */
          /* the first bit acts also as locking mechanism for Dem_RGEvents[] update */
          DEM_SET_BIT_IN_ARRAY(Dem_RGEvents, 0U);
          /* synchronize the flags from Dem_RGEvents[] with calibratable values from
           * Dem_OBDReadinessGroup[] array */
          Dem_UpdateRGEventsFlags();
        }
#endif
  ...
  DBG_DEM_SETDTCFILTERBYREADINESSGROUP_EXIT(
    Result, ClientId, DTCFormat, ReadinessGroupNumber);
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    /* exit with E_NOT_OK if DTCFormat is DEM_DTC_FORMAT_J1939 */
    if (DTCFormat != DEM_DTC_FORMAT_J1939)
    {
...
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
...
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON) && (DEM_OBD_VARIANT_SELECTABLE == STD_ON)
      /* do nothing, if ONLY OBDonUDS FF is enabled */
      ...
      for (Index = 0U; Index < SizeEventMem; ++Index)
      {
        ...
        {
          /* event in memory entry is only considered if a DTC is assigned and not suppressed
           * (to match with Dem.ASR40.SWS_Dem_00225) */
          /* since an entry exists, the event should always be available, implicitly checked
           * by reading the DTC visibility */
          if (Dem_GetVisibleCmbDTC(DTCFormat, EvId) != DEM_NO_DTC)
          {
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
            /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressFirstOccurrence.SetFreezeFrameRecordFilter,1 */
            /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressLastOccurrence.SetFreezeFrameRecordFilter,1 */
            if ( (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
                 (Dem_GbiDTCKind(EvId) == DEM_DTC_KIND_EMISSION_REL_DTCS) &&
                 (Dem_IsAnyDTCStatusBitSet(EvId,
                      (DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC)) == TRUE) )
            {
              StoredOBDFFEventId = EvId;
              /* when OBDonUDS FF is enabled, an increment by 2 is needed (first and last
               * occurrence records) and hence the extra increment. */
              NumOfFilteredRecs++;
            }
#endif
...
#endif
            {
              const Dem_FFIdxType MaxNumFF = Dem_GbiMaxNumFFRecs(EvId);
              NumOfFilteredRecs +=
                (uint8)DEM_NUMFF(&Entry[Index], MaxNumFF);
            }
      ...
      /* !LINKSTO Dem.SWS_Dem_00650,1 */
      /* reset freeze frame record-filter */
      ClientFFRecordFilter->EntryIdx  = 0U;
      ClientFFRecordFilter->RecNumIdx = 0U;

#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
      ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_FIRST] = FALSE;
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
      ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_LAST]  = FALSE;
#endif
#endif
#if (DEM_DEV_FREEZE_FRAME_USED == STD_ON)
        ...
        case DEM_DTC_SELECTOR_SINGLE_DTC_EVID_CACHED:
          {
            ...
            if (DEM_IS_DTC_COMBINED(DTCConfIdx))
            {
              ...
              /* !LINKSTO Dem.SWS_Dem_00059,1 */
              *DTCStatus =
                Dem_CmbDTCStatus[DTCConfIdx] & DEM_DTC_STATUS_AVAILABILITY_MASK;
            }
            else
#endif
            {
  ...
  P2VAR(Dem_DTCSeverityType, AUTOMATIC, DEM_APPL_DATA) DTCSeverityMask)
{
  ...
  DBG_DEM_GETDTCSEVERITYAVAILABILITYMASK_ENTRY(ClientId, DTCSeverityMask);
  ...
  if (Dem_InitializationState != DEM_INITIALIZED)
  {
    DEM_REPORT_ERROR(DEM_SID_GetDTCSeverityAvailabilityMask, DEM_E_UNINIT);
  }
#if (DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON)
  else if (GetCoreID() != DEM_MASTER_INSTANCE)
  {
    /* !LINKSTO Dem.BSWDistribution.ContextCheckForNonDistributedFunctionalities,1 */
    DEM_REPORT_ERROR(DEM_SID_GetDTCSeverityAvailabilityMask, DEM_E_WRONG_CONTEXT);
  }
#endif /* DEM_BSW_DISTRIBUTION_FOR_BSW_EVENT_REPORTING == STD_ON */
  else if (ClientId > DEM_MAX_CLIENTID)
  {
    /* !LINKSTO Dem.SWS_Dem_01252,1 */
    DEM_REPORT_ERROR(DEM_SID_GetDTCSeverityAvailabilityMask, DEM_E_WRONG_CONFIGURATION);
  }
  else if (DTCSeverityMask == NULL_PTR)
  {
    DEM_REPORT_ERROR(
      DEM_SID_GetDTCSeverityAvailabilityMask, DEM_E_PARAM_POINTER);
  }
  else
#endif /* DEM_DEV_ERROR_DETECT */
  {
#if (DEM_CALIBRATION_SUPPORT == STD_ON)
    /* retrieve the cached value of the severity mask */
    Dem_DTCSeverityType SevAvailabilityMask = Dem_SeverityAvailabilityMask;
    /* the first call of Dem_GetDTCSeverityAvailabilityMask() API made after
     * Dem initialization requires re-calculation of DTC severity availability
     * mask, to make sure that it is synchronized with corresponding calibratable
     * values from Dem_DTCDesc[] and Dem_OBDConfig[] arrays */
    /* Dem_IsSeverityMaskUpdated acts as locking mechanism for mask's update */
    if (Dem_IsSeverityMaskUpdated == FALSE)
    {
      Dem_EventIdType EventId;
      Dem_DTCSeverityType SevMask = 0U;
      Dem_DTCSeverityType DtcClassMask = 0U;

      /* reset severity availability mask before calculation */
      SevAvailabilityMask = 0U;

      /* in case of calibration, the severity availability mask has to be
       * determined at runtime */
      for (EventId = 1U; EventId <= DEM_MAX_EVENTID; EventId++)
      {
        /* get the severity of the event */
        SevMask = Dem_GbiDTCSeverity(EventId);
#if (DEM_DTC_WWHOBD_CLASS_SUPPORT)
        /* get WWHOBD DTC Class of the event */
        DtcClassMask = Dem_GbiDTCClass(EventId);
#endif
        /* update the severity availability mask with the event's mask */
        /* !LINKSTO Dem.ASR21-11.SWS_Dem_01182,1 */
        SevAvailabilityMask = SevAvailabilityMask | SevMask | DtcClassMask;
      }
      /* cache the calculated mask */
      Dem_SeverityAvailabilityMask = SevAvailabilityMask;
      Dem_IsSeverityMaskUpdated = TRUE;
    }

    /* update the out parameter with the calculated or cached value */
    /* !LINKSTO Dem.SWS_Dem_01181,1 */
    *DTCSeverityMask = SevAvailabilityMask;
#else
    /* when calibration is disabled, the severity availability mask is taken
     * from configuration */
    /* !LINKSTO Dem.SWS_Dem_01181,1 */
    *DTCSeverityMask = DEM_SEVERITY_AVAILABILITY_MASK;
#endif /* (DEM_CALIBRATION_SUPPORT == STD_ON) */

    Result = E_OK;
  }

  DBG_DEM_GETDTCSEVERITYAVAILABILITYMASK_EXIT(Result, ClientId, DTCSeverityMask);
  return Result;
}
    ...
    P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA) NumberOfFilteredDTC)
{
  ...
  else if ((Dem_DTCFilter[ClientId].CurrentFilterType != DEM_DTC_FILTER_DTCFILTER) &&
           (Dem_DTCFilter[ClientId].CurrentFilterType != DEM_DTC_FILTER_EXTDATAFILTER) &&
           (Dem_DTCFilter[ClientId].CurrentFilterType != DEM_DTC_FILTER_READINESSFILTER))
  {
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01300,1 */
    /* !LINKSTO Dem.ASR21-11.SWS_Dem_01266,1 */
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* filter DTCs based on different filter types */
    switch (ClientDTCFilter->CurrentFilterType)
    {
      case DEM_DTC_FILTER_DTCFILTER:
      {
        NrOfFilteredDTCs = Dem_GetNumOfDTCByFilter(ClientId);
        break;
      }
      case DEM_DTC_FILTER_EXTDATAFILTER:
      {
        NrOfFilteredDTCs = Dem_GetNumOfDTCByEDRN(ClientId);
        break;
      }
#if (DEM_OBDONUDS_SUPPORT)
      case DEM_DTC_FILTER_READINESSFILTER:
      {
        /* !LINKSTO Dem.ASR21-11.SWS_Dem_01364,1 */
        NrOfFilteredDTCs = Dem_RgpGetNumDTCForRG(
                             ClientDTCFilter->ReadinessGroupNumber,
                             ClientDTCFilter->DTCFormat);
        break;
      }
#endif
      /* CHECK: NOPARSE */
      default: /* should never be reached */
        DEM_UNREACHABLE_CODE_ASSERT(DEM_SID_GetNumberOfFilteredDTC);
        break;
      /* CHECK: PARSE */
    }
...
STATIC FUNC(uint16, DEM_CODE) Dem_GetNumOfDTCByFilter(uint8 ClientId)
{
  ...
  DBG_DEM_GETNUMOFDDTCBYFILTER_ENTRY(ClientId);
  ...
  Result = NrOfFilteredDTCs;

  DBG_DEM_GETNUMOFDDTCBYFILTER_EXIT(Result, ClientID);
  return Result;
}
...
STATIC FUNC(uint16, DEM_CODE) Dem_GetNumOfDTCByEDRN(uint8 ClientId)
{
  ...
  DBG_DEM_GETNUMOFDDTCBYEDRN_ENTRY(ClientId);
...
#endif
    {
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01362,1 */
      if (Dem_ApplyEDRNFilter(ClientId,
                              (Dem_EventIdType)EvId,
                              &IdleDTCStatus,
                              &IdleDTC) == TRUE)
      {
  ...
  DBG_DEM_GETNUMOFDDTCBYEDRN_EXIT(Result, ClientId);
  return Result;
}

/*----------------[Dem_GetNextFilteredDTC]----------------------------------*/

/* !LINKSTO Dem.SWS_Dem_00653,1 */
  ...
  P2VAR(Dem_DTCStatusMaskType, AUTOMATIC, DEM_APPL_DATA) DTCStatus)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* filter matching DTC based on different filter type */
    switch (ClientDTCFilter->CurrentFilterType)
    {
      case DEM_DTC_FILTER_DTCFILTER:
      {
        Result = Dem_GetNextDTCByFilter(ClientId, &DTCStatusValue, &DTCValue);
        break;
      }
#if (DEM_NUM_EDCLS > 1)
      case DEM_DTC_FILTER_EXTDATAFILTER:
      {
        Result = Dem_GetNextDTCByEDRN(ClientId, &DTCStatusValue, &DTCValue);
        break;
      }
#endif
#if (DEM_OBDONUDS_SUPPORT)
      ...
      {
        /* !LINKSTO Dem.ASR21-11.SWS_Dem_01364,1 */
        Result = Dem_RgpGetNextDTCForRG(ClientDTCFilter->ReadinessGroupNumber,
                                        ClientDTCFilter->DTCFormat,
      ...
      /* CHECK: NOPARSE */
      default: /* should never be reached */
        DEM_UNREACHABLE_CODE_ASSERT(DEM_SID_GetNextFilteredDTC);
        break;
      /* CHECK: PARSE */
    }
...
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_GetNextDTCByFilter(
  ...
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                DTC)
{
  /* !LINKSTO Dem.ASR21-11.SWS_Dem_00216_Implicit1,1 */
  ...
  DBG_DEM_GETNEXTDTCBYFILTER_ENTRY(ClientId, DTCStatus, DTC);
            ...
            (ClientDTCFilter->NextItem <= DEM_MAX_NUMBER_EVENT_ENTRY_PER) )
    {
      ...
      * only be processed within the while loop */
           ...
           (NvStatus == DEM_PERMANENT_MEMORY_NV_STORED) )
      {
        ...
        FilterResult = Dem_ApplyDTCFilter(ClientId, NextEventId,
          DTCStatus, &IdleFDC, DTC);
...
#endif
    {
...
#endif
      {
        FilterResult = Dem_ApplyDTCFilter(ClientId,
            NextEventId, DTCStatus, &IdleFDC, DTC);
  ...
  DBG_DEM_GETNEXTDTCBYFILTER_EXIT(Result, ClientId, DTCStatus, DTC);
  return Result;
}
  ...
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                DTC)
{
  ...
  DBG_DEM_GETNEXTDTCBYEDRN_ENTRY(ClientId, DTCStatus, DTC);
          ...
          (FilterResult == FALSE) )
  {
    ...
    {
      /* !LINKSTO Dem.ASR21-11.SWS_Dem_01362,1 */
      FilterResult = Dem_ApplyEDRNFilter(ClientId,
          NextEventId, DTCStatus, DTC);
  ...
  DBG_DEM_GETNEXTDTCBYEDRN_EXIT(Result, ClientId, DTCStatus, DTC);
  return Result;
}
#endif
    ...
    P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)      DTC)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* !LINKSTO Dem.ASR40.SWS_Dem_00219,1 */
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    if (ClientFFRecordFilter->RecordFilterIsSet != FALSE)
    {
...
#if (DEM_NUM_OF_PIDS_IN_OBDFF > 0U)
      {
        ...
        StoredOBDFFEventId = DEM_OBDFF_STORED_EVENT_ID;
#endif /* (DEM_OBDFF_SUPPORT_KIND == DEM_OBDFF_MULTIPLE) */
#endif /* ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))  */
      ...
      while (ClientFFRecordFilter->EntryIdx < SizeEventMem)
      {
        ...
        if (EventId != DEM_EVENT_ID_INVALID)
        {
          ...
          const uint32 DTCValue =
            Dem_GetVisibleCmbDTC(ClientFFRecordFilter->DTCFormat, EventId);
          ...
          if (DTCValue != DEM_NO_DTC)
          {
            ...
            /* !LINKSTO dsn.Dem.FreezeFrameRecordFilter.OrderOfRetrievingFF,2 */
            /* !LINKSTO Dem.SWS_Dem_00225,1 */
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
#if (DEM_OBDONUDS_FF_SUPPORT == STD_ON)
            /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressFirstOccurrence.GetNextFilteredRecord,1 */
            /* !LINKSTO dsn.Dem.OBDonUDS.OBDFF.SuppressLastOccurrence.GetNextFilteredRecord,1 */
            if ( (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
                 (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS) &&
                 (Dem_IsAnyDTCStatusBitSet(EventId,
                    (DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_PDTC)) == TRUE) )
            {
              StoredOBDFFEventId = EventId;
            }
#endif
            if ( (StoredOBDFFEventId == EventId) &&
                 (FALSE == ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_FIRST]) )
            {
              /* event memory entry with demanded freeze frame record reached */
              *DTC = DTCValue;
              /* OBDFF record number */
              *RecordNumber = DEM_OBD_FF_RECORD_NUM_FIRST;

              /* set the flag */
              ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_FIRST] = TRUE;
            ...
            if ( (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
                 (StoredOBDFFEventId == EventId) &&
                 (FALSE == ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_LAST]) &&
                 (FALSE == FFRecordFound))
            {
              /* event memory entry with demanded freeze frame record reached */
              *DTC = DTCValue;
              /* OBDFF record number */
              *RecordNumber = DEM_OBD_FF_RECORD_NUM_LAST;

              /* set the flag */
              ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_LAST] = TRUE;
...
#pragma warning default
...
#if ((DEM_MAX_NUMBER_DEVFFA_ENTRY > 0U) || (DEM_MAX_NUMBER_DEVFFB_ENTRY > 0U))
            {
              ...
              for (IdxDevFFKind = DEM_DEVFF_A;
                   ...
                   ++IdxDevFFKind)
              {
                /* !LINKSTO dsn.Dem.GetNextFilteredRecord.FFRecRecognizedFlags,3 */
...
#pragma warning default
        ...
        /* !LINKSTO dsn.Dem.GetNextFilteredRecord.FFRecRecognizedFlags.Reset,3 */
#if ((DEM_NUM_OF_PIDS_IN_OBDFF > 0U) || (DEM_OBDONUDS_FF_SUPPORT == STD_ON))
        ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_FIRST] = FALSE;
#if(DEM_OBDONUDS_FF_SUPPORT == STD_ON)
        ClientFFRecordFilter->ObdFFConsidered[DEM_OBD_FF_CONSIDERED_LAST]  = FALSE;
...
/* !LINKSTO Dem.SWS_Dem_00228_Implicit1,1 */
...
/* !LINKSTO Dem.SWS_Dem_00287_Implicit1,1 */
FUNC(Std_ReturnType, DEM_CODE)
  Dem_GetNextFilteredDTCAndSeverity(
    uint8                                                ClientId,
    P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)              DTC,
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    Result = Dem_GetDTCAndSeverityInternal(ClientId,
                                           DTC,
                                           DTCStatus,
                                           DTCSeverity,
                                           DTCFunctionalUnit);
    ...
    P2VAR(Dem_DTCSeverityType, AUTOMATIC, DEM_APPL_DATA) DTCSeverity)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
...
#endif /* DEM_DEV_ERROR_DETECT */
    {
      ...
      {
        case DEM_DTC_SELECTOR_SINGLE_DTC_EVID_CACHED:
          /* !LINKSTO Dem.SWS_Dem_01291,1 */
          /* !LINKSTO Dem.SWS_Dem_01292,1 */
          /* get the severity mapped to event Id */
          *DTCSeverity = Dem_GbiDTCSeverity(EventId);
#if (DEM_DTC_WWHOBD_CLASS_SUPPORT)
          /* get the WWH-OBD DTC Class mapped to event Id */
          *DTCSeverity |= Dem_GbiDTCClass(EventId);
#endif
          Result = E_OK;
          break;
  ...
  P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA)                 DTCFunctionalUnit)
{
  ...
  DBG_DEM_GETDTCANDSEVERITYINTERNAL_ENTRY(ClientId, DTC, DTCStatus, DTCSeverity, DTCFunctionalUnit);
        ...
        )
  {
     ...
     * otherwise the cmb DTC might be returned for each cmb event linked to
     * this DTC */
    if (NextEventId == DEM_GET_MASTER_EVID(NextEventId))
#endif
    {
...
#if (DEM_DTC_WWHOBD_CLASS_SUPPORT)
        /* get the WWH-OBD DTC Class mapped to event Id */
        *DTCSeverity |= Dem_GbiDTCClass(NextEventId);
#endif
        /* !LINKSTO Dem.SWS_Dem_01293,1 */
        /* get the functional unit mapped to event Id */
  ...
  DBG_DEM_GETDTCANDSEVERITYINTERNAL_EXIT(ClientId, DTC, DTCStatus, DTCSeverity, DTCFunctionalUnit, Result);
         ...
         )
      {
...
#endif
        {
          ...
          if ( (ClientDTCFilter->FilterWithSeverity == TRUE)       &&
               ...
               (ClientDTCFilter->DTCKind == DEM_DTC_KIND_EMISSION_REL_DTCS) )
          {
            /* !LINKSTO Dem.OBDonUDS.DTCSeverityMaskRecord.WithSeverityMask,1 */
            /* check if any bit of DTCClass matches */
            if ( ((ClientDTCFilter->DTCStatusMask   & *DTCStatus) != 0U) &&
                 ((ClientDTCFilter->DTCSeverityMask & DTCSeverity & DEM_DTCCLASS_INFO_MASK) != 0U) )
            {
              /* if any bit in optional severity info mask is set, check if any
              bit in severity info matches */
              /* !LINKSTO Dem.OBDonUDS.DTCSeverityMaskRecord.WithoutSeverityMask,1 */
              if ( ((ClientDTCFilter->DTCSeverityMask & DEM_DTCSEVERITY_INFO_MASK) == 0U) ||
                   ((ClientDTCFilter->DTCSeverityMask & DTCSeverity & DEM_DTCSEVERITY_INFO_MASK) != 0U) )
              {
                Result = TRUE;
              }
            }
  ...
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)                  DTC)
{
  ...
  DBG_DEM_APPLYEDRNDTCFILTER_ENTRY(ClientId, EventId, DTCStatus, DTC);

#if ((DEM_MAX_NUMBER_EVENT_ENTRY_SEC != 0U) || (DEM_MAX_NUMBER_EVENT_ENTRY_MIR != 0U))
  if (ClientDTCFilter->DTCOrigin == Dem_GbiDTCOrigin(EventId))
#endif
  {
    *DTC = Dem_GetVisibleCmbDTC(ClientDTCFilter->DTCFormat, EventId);
    ...
    {
#if (DEM_NUM_EDSEGS > 0U)
      if (Dem_IsEDRNSupported(EventId,
          ClientDTCFilter->ExtendedDataRecordNumber) == TRUE)
      {
        /* access DTC status only once centrally to ensure consistent value for
         * that filter services, which return this value */
#if (DEM_NUM_CMB_DTCS > 0U)
...
#else
      TS_PARAM_UNUSED(DTCStatus);
#endif /* (DEM_NUM_EDSEGS > 0U) */
    }
  }

  DBG_DEM_APPLYEDRNDTCFILTER_EXIT(Result, ClientId, EventId, DTCStatus, DTC);
  return Result;
}
  ...
  uint8           ExtendedDataRecordNumber)
{
  ...
  DBG_DEM_ISEDRNSUPPORTED_ENTRY(EvId, ExtendedDataRecordNumber);

#if (DEM_MAR_SUPPORT == STD_ON)
  /* !LINKSTO Dem.OBDonUDS.MAR.FilteringForEDRN0x93,1 */
  if ( (ExtendedDataRecordNumber != DEM_MAR_EDRN) ||
       ( (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) &&
         (Dem_GbiDTCKind(EvId) == DEM_DTC_KIND_EMISSION_REL_DTCS) ) )
#endif
  {
    /* get extended data class layout */
    EDClass = &Dem_EDClass[Dem_GbiEDClassIdx(EvId)];
    for (SegIndex = 0U; SegIndex < EDClass->NumEDSegs; ++SegIndex)
    {
      uint8 EDSegRecNum = Dem_EDSegment[EDClass->EDSegIdx[SegIndex]].RecNum;

      if (ExtendedDataRecordNumber == EDSegRecNum)
      {
        Result = TRUE;
        break;
      }
  ...
  DBG_DEM_ISEDRNSUPPORTED_EXIT(Result, EvId, ExtendedDataRecordNumber);
  return Result;
}
#endif

#if (DEM_ORDERED_DTC_REPORTING_USED == STD_ON)
/* !LINKSTO Dem.ASR40.SWS_Dem_00410_Implicit1,1 */
/* !LINKSTO dsn.Dem.ReportingOrderDTC.DtcOrderImage,1 */
STATIC FUNC(void, DEM_CODE) Dem_ApplyDTCOrder(uint8 ClientId,
                                              Dem_DTCOriginType DTCOrigin)
{
...
STATIC FUNC(void, DEM_CODE) Dem_UpdateRGEventsFlags(void)
{
  ...
  DBG_DEM_UPDATERGEVENTSFLAGS_ENTRY();

  for (Iterator = 0U; Iterator < DEM_RGP_MAX_NUM_READINESS_EVENTS; Iterator++)
  {
    EventId = Dem_OBDReadinessGroup[Iterator];

    if (EventId != DEM_EVENT_ID_INVALID)
    {
      DEM_SET_BIT_IN_ARRAY(Dem_RGEvents, EventId);
    }
  }

  DBG_DEM_UPDATERGEVENTSFLAGS_EXIT();
}
#endif
