...
[!VAR "NumEventsWithDemDTCClassRefAndImmNvStorage" = "count(DemConfigSet/eb-list::*/DemEventParameter/eb-list::*[node:refexists(DemDTCClassRef) and node:existsAndTrue(as:ref(DemDTCClassRef)/DemImmediateNvStorage)])"!][!//
...
[!IF "($NumEventsWithDemDTCClassRefAndImmNvStorage > 0) or (node:existsAndTrue(DemGeneral/DemImmediateStorageInternalEvents)) or (DemGeneral/DemClearDTCBehavior != 'DEM_CLRRESP_VOLATILE') or node:exists(DemGeneral/DemCalloutDynamicDTCFnc)"!]
#include <Dem_Cfg.h>                             /* declaration of callouts */
[!ENDIF!]

[!IF "($NumEventsWithDemDTCClassRefAndImmNvStorage > 0) or (node:existsAndTrue(DemGeneral/DemImmediateStorageInternalEvents)) or (DemGeneral/DemClearDTCBehavior != 'DEM_CLRRESP_VOLATILE')"!]
/* Implementation hint:
...
[!ENDIF!]
 ...
 /* !LINKSTO Dem.ObdVariantSwitch.CalloutObdVariantFnc,1 */
...
/* Implement this function to provide the activated OBD variant to the Dem.
 * The Dem calls this function on ECU start-up during Dem_Init(). When a
 * OBD protocol variant change is requested, the new OBD variant must only
 * be activated and reported to the Dem if all Dem NV blocks were
 * successfully invalidated before NvM_ReadAll() is called.
 * Otherwise the last activated OBD variant must be reported to the Dem.
 *
 * FUNC(uint8, DEM_APPL_CODE) [!"DemGeneral/DemCalloutObdVariantFnc"!](void)
 * {
 *   uint8 ObdVariant;
 *
 *   * Set the active OBD variant. It must be either
 *   *   - DEM_OBD_VARIANT_OBD_CLASSIC in accordance to SAE_J1979
 *   *     or
 *   *   - DEM_OBD_VARIANT_OBDONUDS in accordance to SAE_J1979-2
 *
 *   return ObdVariant;
 * }
 */
