...
/* !LINKSTO Nm.EB_Ref.SWS_Nm_00123.3,1 */
...
#if (defined NM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN)
#error NM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN already defined
#endif
/** \brief Service ID for function Nm_RequestSynchronizedPncShutdown() */
#define NM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN  36U

#if (defined NM_SID_NM_FORWARDSYNCHRONIZEDPNCSHUTDOWN)
#error NM_SID_NM_FORWARDSYNCHRONIZEDPNCSHUTDOWN already defined
#endif
/** \brief Service ID for function Nm_ForwardSynchronizedPncShutdown() */
#define NM_SID_NM_FORWARDSYNCHRONIZEDPNCSHUTDOWN  40U
...
#endif
/* !LINKSTO Nm.EB_Ref.SWS_Nm_00232.NM_E_UNINIT,1 */
...
#endif
/* !LINKSTO Nm.EB_Ref.SWS_Nm_00232.NM_E_HANDLE_UNDEF,1 */
/** \brief Development Error Code
 **
...
#endif
/* !LINKSTO Nm.EB_Ref.SWS_Nm_00232.NM_E_PARAM_POINTER,1 */
/** \brief Development Error Code
 **
...
#if (defined NM_E_FORWARD_SYNCHRONIZED_PNC_SHUTDOWN)
#error NM_E_FORWARD_SYNCHRONIZED_PNC_SHUTDOWN already defined
#endif
/** \brief  Error id reported in case SchM call fails for Request Synchronized Pnc Shut Down */
#define NM_E_FORWARD_SYNCHRONIZED_PNC_SHUTDOWN       253U

#if (defined NM_E_REQUEST_SYNCHRONIZED_PNC_SHUTDOWN)
#error NM_E_REQUEST_SYNCHRONIZED_PNC_SHUTDOWN already defined
#endif
/** \brief  Error id reported in case SchM call fails for Request Synchronized Pnc Shut Down */
#define NM_E_REQUEST_SYNCHRONIZED_PNC_SHUTDOWN       252U
...
#if (((NM_MULTICORE_ENABLED == STD_ON) && (NM_GENERICNM_USED == STD_ON)) || (NM_MULTICORE_ENABLED == STD_OFF))
/** \brief Function Pointer for standard Nm functions with return type
 ** Std_ReturnType and parameter of NetworkHandleType and PncId. */
typedef P2FUNC(Std_ReturnType, NM_APPL_CODE, Nm_TS_BusNm_ReqSynchShutDownFPtrType)
(
  NetworkHandleType nmChannelHandle,
  PNCHandleType PncId
);
#endif /* #if (((NM_MULTICORE_ENABLED == STD_ON) && (NM_GENERICNM_USED == STD_ON)) || (NM_MULTICORE_ENABLED == STD_OFF)) */

#if (NM_MULTICORE_ENABLED == STD_ON)
#if ((NM_CANNM_USED == STD_ON) || (NM_FRNM_USED == STD_ON) || (NM_UDPNM_USED == STD_ON))
/** \brief Function Pointer for Nm Synchronized Pnc shut down functions when multicore is enabled
 ** Std_ReturnType and parameter of NetworkHandleType and PncId. */
typedef P2FUNC(Std_ReturnType, NM_APPL_CODE, Nm_TS_BusNm_ReqSynchShutDownMcFPtrType)
(
  P2CONST(Nm_PnSyncShutdown_Nm_To_BusNm_DataType, AUTOMATIC, NM_APPL_DATA) data
);
...
#if (NM_SYNCHRONIZED_PNC_SHUTDOWN_ENABLED == STD_ON)
#if (NM_MULTICORE_ENABLED == STD_ON)
  /** \brief Points to the BusNm_RequestSynchronizedPncShutdown() function of the lower layer
   * modules */
#if ((NM_CANNM_USED == STD_ON) || (NM_FRNM_USED == STD_ON) || (NM_UDPNM_USED == STD_ON))
  Nm_TS_BusNm_ReqSynchShutDownMcFPtrType RequestSynchronizedPncShutdownMulticore;
#endif /* #if ((NM_CANNM_USED == STD_ON) || (NM_FRNM_USED == STD_ON) || (NM_UDPNM_USED == STD_ON)) */
#if (NM_GENERICNM_USED == STD_ON)
  Nm_TS_BusNm_ReqSynchShutDownFPtrType RequestSynchronizedPncShutdownMcOffOrGeneric;
 ...
 ** functionality (Nm.SWS_Nm_00020). However, specific implementation may not need it
 ** (SWS_Nm_00093).
 **
 ** Preconditions:
...
/** \brief Passive start of Network Management.
 **
 ** This function calls the BusNm_RequestSynchronizedPncShutdown function (e.g.
 ** CanNm_RequestSynchronizedPncShutdown function is called if channel is configured as CAN).
 **
 ** Preconditions:
 ** - The channel handle should be valid and the module should have been
 **    initialized for this channel (checked).
 **
 ** \param[in] NetworkHandle Identification of the NM-channel.
 ** \param[in] PncId Identification of the Pnc.
 **
 ** \return Standard Return Code
 ** \retval E_OK No Error.
 ** \retval E_NOT_OK  BusNm_RequestSynchronizedPncShutdown of network management has failed.
 ** NetworkHandle does not exist (development only) module not yet initialized
 ** development only).
 **
 ** \ServiceID{36}
 ** \Reentrancy{Non-Reentrant for the same NetworkHandle. Reentrant otherwise}
 ** \Synchronicity{Synchronous} */
extern FUNC(Std_ReturnType, NM_CODE) Nm_RequestSynchronizedPncShutdown
(
  NetworkHandleType NetworkHandle,
  PNCHandleType PncId
);
...
extern FUNC(Std_ReturnType, NM_CODE) Nm_MultipleNetworkOperations
(
  NetworkHandleType nmNetworkHandle,
  uint8 operation
);
