 ...
 *
 * Reason:
 * Instead of the standard AUTOSAR Crc module, a Safety CRC module named SCrc is used.
 *
...
/** \brief Initial value used for Crc calculation with polynomial 0xF4ACFB13 */
...
/** \brief Xor-value for Crc calculation with polynomial 0xF4ACFB13 */
...
)
{
    ...
    for (Idx=0U; Idx<SCrc_Length; ++Idx)
    {
        SCrc_StartValue8 = SCrc_Table8[(uint8)(SCrc_StartValue8 ^ SCrc_DataPtr[Idx])];
    }
...
)
{
    ...
    for (Idx=0U; Idx<SCrc_Length; ++Idx)
    {
        SCrc_StartValue8H2F = SCrc_Table8H2F[(uint8)(SCrc_StartValue8H2F ^ SCrc_DataPtr[Idx])];
    }
...
)
{
    ...
    for (Idx=0U; Idx<SCrc_Length; ++Idx)
    {
        ...
        SCrc_StartValue16 =
                SCrc_Table16[((uint8)(SCrc_StartValue16 >> 8U)) ^ SCrc_DataPtr[Idx]]
...
)
{
    ...
    for (Idx=0U; Idx<SCrc_Length; ++Idx)
    {
        /* Process one byte of data */
        SCrc_StartValue32P4
          = (SCrc_Table32P4[((uint8)(SCrc_StartValue32P4 & 0xFFU)) ^ SCrc_DataPtr[Idx]]
