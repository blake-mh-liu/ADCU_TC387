...
[!ENDIF!]
/** \brief Number of partitions where Csm events are executed
 */
/* !LINKSTO CSM.Dsn.Api/Macros/00027,1 */
#if (defined CSM_PRT_COUNT)
#error CSM_PRT_COUNT already defined
#endif
#define CSM_PRT_COUNT [!"num:i(count(node:refs(as:modconf('Csm')/CsmMainFunction/*/CsmMainFunctionPartitionRef)))"!]U
...
/** \brief  List of configured Csm callback contexts
 **/
extern CONST(Csm_CallbackNotificationCtxType, CSM_CONST) Csm_Callbacks[CSM_CBK_COUNT];
[!ENDIF!][!//
...
/** \brief  List of additional internal data sets per Csm callback notification
 **/
extern VAR(Csm_CallbacksExtraDataType, CSM_APPL_DATA) Csm_CallbacksExtraData[CSM_CBK_COUNT];
[!ENDIF!][!//
