...
*
* AS 4.3.0 specifies:
*   Csm<Service>AlgorithmFamiliy
*   Csm<Service>AlgorithmFamilyCustom
*   Csm<Service>AlgorithmSecondaryFamily
*   Csm<Service>AlgorithmSecondaryFamilyCustom
*   Csm<Service>AlgorithmMode
****************************************************************************************************
*/!]
[!MACRO "GETALGORITHM", "_path", "_type", "_srv"!]
  [!VAR "locParNameBase" = "concat($_path, '/', $_srv, 'Algorithm', $_type)"!]
  [!IF "$_type = 'Familiy'"!]
    [!VAR "locParNameCust" = "concat($_path, '/', $_srv, 'Algorithm', text:replace($_type, 'iliy', 'ily'), 'Custom')"!]
  [!ELSE!]
    [!VAR "locParNameCust" = "concat($_path, '/', $_srv, 'Algorithm', $_type, 'Custom')"!]
  [!ENDIF!]
  [!VAR "locMcrNameCust" = "concat('CRYPTO_ALGO', text:toupper(text:replace(text:replace($_type, 'Secondary', ''), 'ily|iliy', '')), '_CUSTOM')"!]
  [!VAR "locMcrNameNotS" = "concat('CRYPTO_ALGO', text:toupper(text:replace(text:replace($_type, 'Secondary', ''), 'ily|iliy', '')), '_NOT_SET')"!]
  [!IF "node:exists($locParNameBase)"!]
    [!IF "node:value($locParNameBase) = $locMcrNameCust"!]
      [!IF "node:exists($locParNameCust)"!]
        [!IF "$_type = 'Familiy' or $_type = 'SecondaryFamily'"!]
          [!WS "0"!][!"text:replace($locMcrNameCust, '_CUSTOM', '')"!]_[!"text:toupper(text:replace(node:value($locParNameCust), 'CRYPTO_ALGOFAM_', ''))"!][!//
        [!ELSE!]
          [!WS "0"!][!"text:replace($locMcrNameCust, '_CUSTOM', '')"!]_[!"text:toupper(text:replace(node:value($locParNameCust), 'CRYPTO_ALGOMODE_', ''))"!][!//
        [!ENDIF!]
      [!ELSE!]
        [!WS "0"!][!"$locMcrNameNotS"!][!//
      [!ENDIF!]
...
#include <Csm_Int.h>
[!IF "(node:value(as:modconf('Csm')/CsmEbGeneral/CsmEbMisc/CsmEbCorrectionCsmJobBasedCallbacknotificationPorts) = 'false' and $Csm_Rte_Enabled_Callback = 'true') or (node:value(as:modconf('Csm')/CsmEbGeneral/CsmEbMisc/CsmEbCorrectionCsmJobBasedCallbacknotificationPorts) = 'true' and $Csm_Rte_Enabled_CallbackR2011 = 'true') or (count(as:modconf('Csm')/CsmMainFunction/*) != 0)"!][!//
#include <SchM_Csm.h>
[!ENDIF!]
...
CONST(Crypto_PrimitiveInfoType, CSM_CONST) Csm_PI_[!"node:name(.)"!]_[!"node:name(node:ref(./CsmJobPrimitiveRef))"!] =
{
  [!SELECT "node:ref(./CsmJobPrimitiveRef)/*[1]/*"!]
    ...
    [!WS "2"!][!/* service           */!][!"text:toupper(text:replace(text:replace($vService,'Csm','CRYPTO_'),'_Job','_'))"!],
    [!WS "2"!][!/* algorithm         */!]{
    [!IF "$vService = 'CsmJobRandomSeedConfig' or $vService = 'CsmJobKeySetValidConfig'"!]
    [!WS "2"!][!/*   family          */!]  [!CALL "GETALGORITHM", "_path"="node:path(node:current())", "_srv"="$vService", "_type"="'Family'         "!],
    [!ELSE!]
...
[!LOOP "node:order(./CsmJobs/CsmJob/*,'node:value(./CsmJobId)')"!][!//
  {
    ...
    [!IF "   node:value(./../../../CsmEbGeneral/CsmEbMisc/CsmEbAutosarApiVersion) = 'CSM_API_VERSION_431'
          or node:value(./../../../CsmEbGeneral/CsmEbMisc/CsmEbAutosarApiVersion) = 'CSM_API_VERSION_440'
    "!]
      [!WS "6"!][!/*   input64                                    */!]  0U,
    [!ENDIF!]
    ...
    [!ELSE!]
      [!WS "4"!][!/* cryptoKeyId                                    */!]CSM_KEY_EMPTY,
      [!WS "4"!][!/* jobRedirectionInfoRef                          */!]NULL_PTR,
      [!WS "4"!][!/* targetCryptoKeyId                              */!]CSM_KEY_EMPTY
    [!ENDIF!]
  },
[!ENDLOOP!]
};
...
[!IF "(node:value(as:modconf('Csm')/CsmEbGeneral/CsmEbMisc/CsmEbAutosarApiVersion) != 'CSM_API_VERSION_440') and (node:value(as:modconf('Csm')/CsmEbGeneral/CsmEbMisc/CsmEbCorrectionCsmJobBasedCallbacknotificationPorts) = 'false')"!]
  [!IF "$Csm_Rte_Enabled_Callback = 'true'"!]
    ...
    [!LOOP "as:modconf('Csm')/CsmCallbacks/CsmCallback/*[node:empty(./CsmCallbackFunc)]"!]
      [!VAR "vCbk" = "node:name(.)"!]
FUNC(void, CSM_CODE) Csm_[!"$vCbk"!]_CallbackNotification(void)
{
  Csm_Int_CallbackNotification
  (
    Csm_CallbacksExtraData[CSM_CBK_[!"text:toupper(node:name(.))"!]].job,
    Csm_CallbacksExtraData[CSM_CBK_[!"text:toupper(node:name(.))"!]].result
  );
}
  ...
  [!ENDIF!]
[!ELSE!][!//
  [!IF "$Csm_Rte_Enabled_CallbackR2011 = 'true'"!][!//
    ...
    [!LOOP "as:modconf('Csm')/CsmJobs/CsmJob/*[CsmJobUsePort = 'true']"!]
      [!IF "node:exists(node:ref(./CsmJobPrimitiveRef)/*) = 'true' and node:value(node:ref(./CsmJobPrimitiveRef)/*/*/*[contains(node:name(.),'Processing')]) = 'CSM_ASYNCHRONOUS'"!]
        [!VAR "vJob" = "node:name(.)"!][!//
FUNC(void, CSM_CODE) Csm_CallbackNotification_[!"$vJob"!](void)
{
  Csm_Int_CallbackNotification
  (
    &Csm_JobConfigurations[[!"num:i(./CsmJobId)"!]],
    Csm_JobExtraData[[!"num:i(./CsmJobId)"!]].callbackSWCResult
  );
}
...
{
#if (CSM_API_ENABLED_SERVICE_ASYNCHRONOUS == STD_ON)
[!LOOP "as:modconf('Csm')/CsmQueues/CsmQueue/*"!][!//
  Csm_Main(CSM_QUE_[!"text:toupper(node:name(.))"!]);
[!ENDLOOP!][!//
...
[!LOOP "as:modconf('Csm')/CsmMainFunction/*"!][!//
  [!VAR "MainFunc" = "node:name(.)"!][!//
  [!IF "num:i(0) < num:i(count(node:refs(as:modconf('Csm')/CsmJobs/CsmJob/*[node:name(node:ref(node:ref(./CsmJobQueueRef)/CsmQueueMainFunctionRef)) = $MainFunc]/CsmJobPrimitiveRef)/*/*/*[contains(node:name(.),'Processing') and node:value(.) = 'CSM_ASYNCHRONOUS']))"!][!//
/* --- Csm_Mainfunction_[!"$MainFunc"!] --- */

/* !LINKSTO CSM.Dsn.SWS_Csm_00479_2,1 */

FUNC(void, CSM_CODE) Csm_MainFunction_[!"$MainFunc"!]
(
  void
)
{
  /* !LINKSTO CSM.Req.Gen/CsmMulticore/MainFunction/00002,1, Csm.ASR20-11.SWS_Csm_91065,1 */
  [!LOOP "as:modconf('Csm')/CsmQueues/CsmQueue/*"!][!//
    [!IF "$MainFunc = node:name(as:ref(./CsmQueueMainFunctionRef)) and
          num:i(0) < num:i(count(node:refs(./../../../CsmJobs/*/*[node:contains(./../../../CsmQueues/CsmQueue/*[node:name(node:ref(./CsmQueueMainFunctionRef)) = $MainFunc],node:ref(./CsmJobQueueRef))]/CsmJobPrimitiveRef)/*/*/*[contains(node:name(.),'Processing') and node:value(.) = 'CSM_ASYNCHRONOUS']))"!][!//
  Csm_Main(CSM_QUE_[!"text:toupper(node:name(.))"!]);
    [!ENDIF!][!//
  [!ENDLOOP!][!//
}
