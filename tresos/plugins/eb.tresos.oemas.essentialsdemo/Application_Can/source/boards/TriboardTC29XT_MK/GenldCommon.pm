...
=item C<addUniqueArrayEntry(\@array, $type, $argidx)>
...
name currently exists in the array. Therefore only one object may exist by that name.
...
{
	my ($array, $type, $argidx) = @_;
	usage("$type requires a name!") unless $argidx<@ARGV;

	my $value = $ARGV[$argidx];
	if(grep {$_->{name} eq $value} @$array) {
		usage("Multiple definitions of $type $value\n");
	}
...
sub parseArgs
{
	...
	for(my $i=0; $i < @ARGV; $i++)
	{
		...
		{
			$sysdesc{$a} ||= [];
			$curapp = addUniqueArrayEntry($sysdesc{$a}, 'OS-Application', ++$i);
		...
		elsif($a eq '-task' || $a eq '-isr' || $a eq '-memregsym')
		{
			...
			$curcontainer = addUniqueArrayEntry($curapp->{$a}, ucfirst(substr($a, 1)), ++$i);
		}
		...
		{
			$sysdesc{$a} = [ ] unless defined $sysdesc{$a};
			$curstack = addUniqueArrayEntry($sysdesc{$a}, 'OS-Stack', ++$i);
