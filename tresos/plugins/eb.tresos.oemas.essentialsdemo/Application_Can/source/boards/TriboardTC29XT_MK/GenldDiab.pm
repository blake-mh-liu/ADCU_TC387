...
sub printOutputSectionSimple
{
	...
	my $align_minus_1 = $ot->{align}-1;
	GenldCommon::printOut(". = ((. + ($align_minus_1)) & (~($align_minus_1)));");

	my $diabTypedescStr = ($ot->{_diab_typedesc} eq '-')? "" : "($ot->{_diab_typedesc})";
	GenldCommon::printOut("$osecName $diabTypedescStr ALIGN($ot->{align}) $loadaddr $extraattr :");
	GenldCommon::printOut("{");
	...
	if(defined $endSymbol or defined $initEndSymbol)
	{
...
sub printOutputSection
{
	...
	if ($mixInSDA) {
		...
		$self->printOutputSectionSimple($prefix.$osecName, $sOsecType, $inFiles, undef,
			$startSymbol, $sdaEndSymbol, $initSymbol, $sdaInitEndSymbol);
		$self->printOutputSectionSimple($osecName, $osecType, $inFiles, undef,
			$ndaStartSymbol, $endSymbol, $ndaInitSymbol, $initEndSymbol);
	} else {
