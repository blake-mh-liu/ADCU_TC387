 ...
 *
 * Reason:
 *  The function OS_DoTryToGetSpinlock uses if-elseif-else-cascades to select the
 *  the proper actions. Additonally, the PA implementation of the cache functions
 *  have a special handling, which is hidden in function like macros, but still
 *  increases the VG value. Moving this special handling to dedicated functions
 *  would not increase the code quality but only result in a overall performance
 *  drop for PA.
*/
...
OS_TOOL_FUNCTION_ATTRIB_PRE
static os_autosarspinlockid_t OS_CheckNestedSpinlock(os_objectid_t lastSpinlockIn, os_autosarspinlockid_t lockId)
OS_TOOL_FUNCTION_ATTRIB_POST
{
	os_objectid_t lastSpinlock = lastSpinlockIn;
