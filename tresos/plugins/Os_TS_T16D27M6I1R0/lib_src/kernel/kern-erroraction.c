...
OS_TOOL_FUNCTION_ATTRIB_POST
{
		 ...
		 (act == OS_ACTION_QUARANTINE) )
	{
		...
		*/
		(void)(*OS_killTaskFunc)(kernel_data->taskCurrent,
			(os_taskstate_t) ((act==OS_ACTION_KILL) ? OS_TS_SUSPENDED
													: OS_TS_QUARANTINED));
	}
	else
...
/* Deviation MISRAC2012-1 */
OS_TOOL_FUNCTION_ATTRIB_PRE
os_result_t OS_ErrorAction(os_erroraction_t act, os_result_t resIn)
OS_TOOL_FUNCTION_ATTRIB_POST
{
	os_result_t res = resIn;
	os_kerneldata_t * const kernel_data = OS_GetKernelData();
