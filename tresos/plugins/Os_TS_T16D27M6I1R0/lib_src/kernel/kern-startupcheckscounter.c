...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	for ( i = 0; i < OS_nCounters; i++ )
	{
		...
		/* !LINKSTO Kernel.Feature.StartupChecks.Counter.NoOwningApplication, 1 */
		if ( OS_AppIsNull(app) )
		{
		...
		if (OS_GetCounterCoreId(ctr) == myCoreId)
		{
			...
			/* !LINKSTO Kernel.Feature.StartupChecks.Counter.NoApplicationPermission, 1 */
			...
			if ( ctr->hwt != OS_NULL )
			{
				 ...
				 * !LINKSTO Kernel.Feature.StartupChecks.Counter.RangeTooLow, 1
				*/
