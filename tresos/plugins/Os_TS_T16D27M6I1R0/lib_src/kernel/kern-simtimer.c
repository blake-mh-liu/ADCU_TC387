...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	{
		OS_simTimer[tmr].channel[chan].isr = (os_isrid_t) (isrId + 1u);
	...
	else
	{
		...
		else
		{
			...
			*/
			...
			{
				if ( ( (tmrp->channel[c].control & OS_SIMTIMERCTRL_PENDING) != 0u ) ||
					 ( ((tmrp->channel[c].compare - old) & tmrp->mask) <= incr ) )
				{
					if ( (tmrp->channel[c].control & OS_SIMTIMERCTRL_ENABLED) != 0u )
					{
						if ( tmrp->channel[c].isr != 0u )
						{
							(*OS_isrTableBase[tmrp->channel[c].isr - 1u].func)();
						}
