 ...
 *
 * Reason:
 * Function OS_QuarantineApplication is called indirectly by OS_Error and OS_ErrorAction.
...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	*/
	...
	for ( alarmid = 0; alarmid < OS_nAlarms; alarmid++ )
	{
		if ( OS_GET_APP(alarm->app) == app )
		{
			...
			(void)(*OS_killAlarmFunc)(alarmid, OS_ALARM_QUARANTINED);
		}
	...
	*/
	...
	for ( scheduleid = 0; scheduleid < OS_nSchedules; scheduleid++ )
	{
		...
		{
			(void)(*OS_killScheduleFunc)(scheduleid, OS_ST_QUARANTINED);
		}
		/* Possible diagnostic TOOLDIAG-1 <2> */
		schedule++;
	}
	...
	*/
	...
	for ( taskid = 0; taskid < OS_nTasks; taskid++ )
	{
		...
		{
			(void)(*OS_killTaskFunc)(task, OS_TS_QUARANTINED);
		}
		task++;
	}
