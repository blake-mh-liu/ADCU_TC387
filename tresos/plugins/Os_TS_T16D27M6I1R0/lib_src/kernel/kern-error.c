...
static void OS_Error_CallErrorHook(
...
static void OS_Error_CallErrorHookApp(
	os_kerneldata_t * const kernel_data,
	os_result_t res,
	os_uint8_t ehNestingSave,
	...
	)
{
	...
	*/
	if ( (OS_AppIsNull(app)) ||
		 (kernel_data->appsStarted == 0u) ||
		 (app->errorHook == OS_NULL) ||
		 (kernel_data->errorHookNesting != 0u) )
	{
		/* Use No Hooks! */
	}
	else
...
static void OS_Error_CallProtectionHook(
	os_kerneldata_t * const kernel_data,
	os_result_t res,
	os_uint8_t ehNestingSave)
{
...
OS_TOOL_FUNCTION_ATTRIB_POST
{
	...
	*/
	if ( OS_IS_ACTION_HOOK(hooks) )
	{
		...
		*/
		if ( (hooks & OS_ACTION_ERRORHOOK_APP) != 0u )
		{
		...
		*/
		if ( ((hooks & (OS_ACTION_ERRORHOOK | OS_ACTION_ERRORHOOK_EB)) != 0u) &&
			 ((kernel_data->errorHookNesting & OS_EHN_ERR) == 0u) )
		{
			OS_Error_CallErrorHook(kernel_data, res, ehNestingSave);
		}

		if ( (hooks & OS_ACTION_ERRORHOOK_APP) != 0u )
		{
			OS_Error_CallErrorHookApp(kernel_data, res, ehNestingSave, app);
		}
		...
		*/
		if ( ((hooks & OS_ACTION_PROTECTIONHOOK) != 0u) &&
			 ((kernel_data->errorHookNesting & OS_EHN_PROT) == 0u) )
		{
			OS_Error_CallProtectionHook(kernel_data, res, ehNestingSave);
		}
