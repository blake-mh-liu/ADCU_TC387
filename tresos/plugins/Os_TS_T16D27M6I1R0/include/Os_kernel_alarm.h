 ...
 *
 * Reason:
 * The macro is defined to store parameters passed to a function.
 * If a parameter is a pointer to void, it has to be cast to os_paramtype_t.
 * The conversion ensures, that no bits are lost.
 *
 * MISRAC2012-5) Deviated Rule: 12.4 (advisory)
...
#define OS_CtrStart(hwt, b, d) \
	do {													\
		...
		(void)OS_HwtStart((hwt), (b), (OS_hwtStartDelta));	\
	} while (0)
...
/* Deviation MISRAC2012-1 <+2>, MISRAC2012-3 <+2> */
...
#define OS_ExtraCheck(x)	(((OS_configMode & OS_EXTRACHECK) != OS_U(0)) && (x))
#endif
