/* Os_tool_TRICORE_clang.h - Tricore macros for clang
 *
...
#ifndef OS_TOOL_TRICORE_CLANG_H
#define OS_TOOL_TRICORE_CLANG_H

#include <TRICORE/Os_tool_TRICORE_inline.h>



#define OS_OFFSETOF(type, member) ((os_address_t)((os_char_t const volatile *)&((type *)0)->member))

/*	Expansion and stringification of a macro. (see Os_tool_TRICORE_gnu)
*/
...
/*	The register naming convention for assembler is %register_name. See HighTec TriCore Development Platform User Guide
*/
#define a0		%a0
#define a1		%a1
#define a2		%a2
...
/*	The clang integrated assembler does not expect any special prefix for
 *	immediate operands.
*/
#define _IMM(p,x)		x
...
/*	The clang integrated assembler uses EABI syntax for the bit specification in jz.t etc.
*/
#define _REGBIT(r,b)	r, b

/*	The clang integrated assembler uses hi:x and lo:x to extract the
 *	high and low words from a literal.
*/
#define _hiword(x)		 hi:x
#define _loword(x)		 lo:x

/*	The clang integrated assembler uses .type, .globl and .extern as normal.
*/
...
/*	 clang integrated assembler has standard .text and .data sections
*/
#define	_TEXT	.text
#define _DATA	.data

/*	The clang integrated assembler uses "power of 2" notation for alignment
*/
#define _align(n,p)		.align p

/* The clang integrated assembler has the .hword directive for 16-bit words */
#define _HWORD .hword

/* These macros implement the *16.* and *32.* opcodes
 * that are provided by some assemblers. The clang integrated assembler
 ...
 * use _mfcr intrinsic, which also uses braces in the expression to
...
/* The core id register has a constant value (on a core).
 */
OS_QUAL_INLINE os_uint32_t OS_MFCR_OS_CORE_ID(void) OS_ATTR_INLINE;
...
OS_QUAL_INLINE void* OS_MFA8(void) OS_ATTR_INLINE;
...
/* A buggy compiler uses an address register when storing 32-bit data. The st.a instructions
