 ...
 *
 *
 * MISRAC2012-7) Deviated Rule: 13.5 (required)
 * The right hand operand of a logical && or || operator shall not contain
 * persistent side effects
 *
 * Reason:
 * The left hand operand checks if the given parameter is valid.
 *
 *
 * MISRAC2012-8) Deviated Rule: 11.5 (required)
 * A conversion should not be performed from pointer to void into pointer to object.
 *
 * Reason:
 * The conversion in question is part of the OS_KernelStackRemaining() macro.
 * It is used to calculate the remaining bytes of the stack.
 * As the results of the conversions are just subtracted and not dereferenced,
 * there is no risk performing them.
 *
 *
 * MISRAC2012-9) Deviated Rule: 5.5 (required)
 * Identifiers shall be distinct from macro names.
 *
 * Reason:
 * For function-call kernels, the OS_UserTaskReturn function is directly replaced by
 * OS_KernTaskReturn using a macro.
 *
 *
 * MISRAC2012-10) Deviated Rule: 5.8 (required)
...
#define OS_ActivationPending(td)			((td)->nAct > 0u)

#define OS_CurrentLink(tp, td)				((os_tasklink_t) ((tp)->linkBase + (td)->cAct))
#define OS_ClearCurrentActivation(td)		(td)->cAct = 0u
#define OS_Enqueue(tp)						OS_EnqueueLink((tp)->linkBase, (tp)->queuePrio)
#define OS_IncCurrentActivation(tp, td)							\
	do {														\
		if (((td)->nAct == 0u) || ((td)->cAct >= (tp)->maxAct))	\
		{														\
			(td)->cAct = 0u;									\
		}														\
		else													\
...
#if (OS_N_CORES == 1)
/* Deviation MISRAC2012-1 <+1>, MISRAC2012-3 <+1> */
 ...
 *
 * In the microkernel-based system these are the internal interfaces between
 * the counter subssystem and the microkernel. For a standard system we must
 * construct them in terms of internal functions.
*/
...
#define OS_IsrLevelToPrio(p)	((os_prio_t) ((p) + OS_NULLISRPRIO))
 ...
 */
/* Deviation MISRAC2012-5 <+1>, MISRAC2012-6 <+1>, MISRAC2012-8 <+1>  */
...
#define OS_IS_ACTION_HOOK(h)		((h) != 0u)
#endif
...
#define OS_CALLAPPSTARTUPHOOKS() \
	do {																	\
		os_kerneldata_t * const OS_kd = OS_GetKernelData();					\
		os_coreid_t const OS_CallAppHooks_myCoreId = OS_GetMyCoreId();		\
		...
		{																	\
			if ((casuhApp->startupHook != OS_NULL)							\
				&& (OS_GetAppCoreId(casuhApp) == OS_CallAppHooks_myCoreId))	\
			{																\
	...
	do {																	\
		...
		{																	\
			if ((casdhApp->shutdownHook != OS_NULL)							\
				&& (OS_GetAppCoreId(casdhApp) == OS_CallAppHooks_myCoreId))	\
			{																\
...
#define OS_ERRORCODE_CHECK(e)	((os_errorresult_t)(e))
#endif

#ifndef OS_ERRORCODE_NOCHECK
#define OS_ERRORCODE_NOCHECK(e)	((os_errorresult_t)(e))
#endif
...
#define OS_ErrorHandlingForVoidApi()	((OS_configMode & OS_VOIDERROR) != 0u)
#endif
...
#define OS_CacheInvalidate(addr, size)	do {	\
		if (OS_cacheManagementDisabled != 0u)	\
		{										\
...
#elif (OS_CACHE_MODE == OS_CACHE_MODE_BARRIER)

#define OS_CacheInvalidate(addr, size)	OS_CacheHardwareMemBarrier(addr, size)
#define OS_CacheFlush(addr, size)		OS_CacheHardwareMemBarrier(addr, size)

#elif (OS_CACHE_MODE == OS_CACHE_MODE_LINE)
...
#else
#error "OS_CACHE_MODE is not correctly set. Check the architecture/derivative headers"
#endif	/* OS_CACHE_MODE */
 ...
 */
/* Deviation MISRAC2012-9 <+5>, MISRAC2012-10 <+5> */
#if (OS_KERNEL_TYPE==OS_FUNCTION_CALL)
