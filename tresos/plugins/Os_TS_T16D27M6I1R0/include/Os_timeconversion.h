 ...
 *
 * Factor is 4160000/1000000000
 *         = 416/100000
 *         = 13/3125
 * 3125 = 5*5*5*5*5 and 25>13, so 25 is the first division factor
*/
#define OS_NsToTicks_4160000(ns)	OS_DivMulDiv((ns), 25u, 13u, 125u)
#define OS_TicksToNs_4160000(tx)	OS_SatMulDivMul((tx), 125u, 13u, 25u, 17867063u)
 ...
 *         = 12/1000
 *         = 3/250
 * 250 = 5*50, and 5>3, so 5 is the first divisor and 50 the second.
*/
#define OS_NsToTicks_12000000(ns)	OS_DivMulDiv((ns), 5u, 3u, 50u)
#define OS_TicksToNs_12000000(tx)	OS_SatMulDivMul((tx), 50u, 3u, 5u, 51539607u)
 ...
 *         = 15/1000
 *         = 3/200
 * 200 = 4*50, and 50>3, so 4 is the first division factor.
*/
 ...
 * Factor is 3000000/1000000000
 *         = 3/100
 * 100 = 4 * 25 and 4>3, so 4 is the first division factor
*/
#define OS_NsToTicks_30000000(ns)	OS_DivMulDiv((ns), 4u, 3u, 25u)
#define OS_TicksToNs_30000000(tx)	OS_SatMulDivMul((tx), 25u, 3u, 4u, 128849016u)
 ...
 *
 * Factor is 74000000/1000000000
 *         = 74/1000
 *         = 37/500
 * 500 = 100*5, and 100>37, so 100 is the first division factor
*/
#define OS_NsToTicks_74000000(ns)	OS_DivMulDiv((ns), 100u, 37u, 5u)
#define OS_TicksToNs_74000000(tx)	OS_SatMulDivMul((tx), 5u, 37u, 100u, 317827572u)
...
/* Clock frequency 148 MHz
 *
 * Factor is 148000000/1000000000
 *         = 74/500
 *         = 37/250
 * 50 = 50*5, and 50>37, so 50 is the first division factor.
*/
#define OS_NsToTicks_148000000(ns)	OS_DivMulDiv((ns), 50u, 37u, 5u)
#define OS_TicksToNs_148000000(tx)	OS_SatMulDivMul((tx), 5u, 37u, 50u, 635655153u)
 ...
 *
 * Factor is 350000000/1000000000
 *         = 7/20
 * No mul-div-mul needed.
*/
#define OS_NsToTicks_350000000(ns)	(((ns)/20u)*7u)
#define OS_TicksToNs_350000000(tx)	(((tx) > 1503238553u) ? 0xffffffffu : (((tx)/7u)*20u))
 ...
 *
 * Factor is 500000000/1000000000
 *         = 1/2
 * No mul-div-mul needed.
*/
#define OS_NsToTicks_500000000(ns)	(((ns)/2u)*1u)
#define OS_TicksToNs_500000000(tx)	(((tx) > 2147483647u) ? 0xffffffffu : (((tx)/1u)*2u))

/* Clock frequency 600 MHz
*
* Factor is 600000000/1000000000
*         = 3/5
* No mul-div-mul needed.
*/
#define OS_NsToTicks_600000000(ns)	(((ns)/5u)*3u)
#define OS_TicksToNs_600000000(tx)	(((tx) > 2576980377u) ? 0xffffffffu : (((tx)/3u)*5u))
 ...
 *
 * Factor is 900000000/1000000000
 *         = 9/10
 * No mul-div-mul needed.
*/
#define OS_NsToTicks_900000000(ns)	(((ns)/10u)*9u)
#define OS_TicksToNs_900000000(tx)	(((tx) > 3865470565u) ? 0xffffffffu : (((tx)/9u)*10u))

/* Clock frequency 1000 MHz
 *
 * Factor is 1000000000/1000000000
 *         = 1
 * No mul-div-mul needed.
*/
#define OS_NsToTicks_1000000000(ns)	(((ns)/1u)*1u)
#define OS_TicksToNs_1000000000(tx)	(((tx) > 4294967295u) ? 0xffffffffu : (((tx)/1u)*1u))

/* Clock frequency 1060 MHz
 *
 * Factor is 1060000000/1000000000
 *         = 53/50
 * MAx value of counter
*/
#define OS_NsToTicks_1060000000(ns)	OS_DivMulDiv((ns), 1u, 53u, 50u)
#define OS_TicksToNs_1060000000(tx)	OS_SatMulDivMul((tx), 50, 53u, 1u, 4294967295u)
