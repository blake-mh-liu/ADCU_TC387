...
#if MK_SW_MAJOR_VERSION == 2 && (MK_SW_MINOR_VERSION == 0 || MK_SW_MINOR_VERSION == 1)
 ...
 * When using the microkernel 2.x the AutoCore OS activities are sequenced in a single thread per core,
...
#define OS_ArchCanWrite(o,s)		(OS_TRUE)
...
#define OS_ForceIntRestore(p)		( ((p) == (os_intstatus_t) MK_E_OK) ? MK_UsrReleaseLock(MK_resLockCat2) : (p) )

/* AutoCore OS part of TerminateApplication
*/
#ifndef OS_ASM
...
*/
#ifndef OS_ASM
os_result_t OS_MkiReportError(mk_serviceid_t, os_errorresult_t, os_paramtype_t *);
#endif
...
#define OS_ERROR(e, p)				OS_MkiReportError(OS_SID, (e), (os_paramtype_t*)(p))
...
/* Determine the application mode in Safety OS.
*/
#ifndef OS_ASM
os_appmodeid_t OS_MkiGetAppMode(void);
#endif

/* Determine the core ID of an object in Safety OS
*/
#if (OS_N_CORES > 1)
#define OS_GetCoreIdFromTaskId(tsx)				MK_LibGetTaskCoreId(tsx)
#define OS_GetCoreIdFromAlarmId(asx)			MK_LibGetAlarmCoreId(asx)
#define OS_GetCoreIdFromCounterId(csx)			MK_LibGetCounterCoreId(csx)
#define OS_GetCoreIdFromScheduleTableId(ssx)	MK_LibGetScheduleTableCoreId(ssx)
#endif
...
#define OS_APPISACCESSIBLE(app)		(MK_GetAppStateForCtrSub(app) == APPLICATION_ACCESSIBLE)
#else
...
/* Currently Safety OS doesn't perform calling context or permission checks for calls to the counter subsystem
 ...
 * Currently there is no AutoCore OS service in the Safety OS, which doesn't return a StatusType.
*/
...
/* Lookup-table for core local AutoCore OS panic code variables.
*/
#ifndef OS_ASM
...
#error "This version of AutoCore OS only supports Safety"
#endif
