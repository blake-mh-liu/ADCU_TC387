...
Options:
  ...
  -ctrsub-data-size <size-core0>[,<size-core1> ...]:
   Size of the core-local portion of the QMOS data/bss region; on single cores, this contains just one value
  -kern-data-size <size>:
   Size of the Microkernel data/bss region
  -kern-data-size-core <size-core0>[,<size-core1> ...]:
   Size of the core-local portion of the microkernel data/bss region
  -mpu-cache-size <size>:
  ...
  @<filename>: Add options given in <filename> to arguments.
...
sub checkIntArg
{
	...
	my $intArg = $sysdesc->{$arg};
	if ($intArg !~ /^((0[xX])([0-9a-fA-F]+)|(\d+))$/)
	{
		usage("$arg expects a non-negative decimal or hexadecimal integer");
	}
...
sub parseArgs
{
	...
	for(my $i=0; $i < @ARGV; $i++)
	{
		...
		elsif(($a eq '-ctrsub-data-size') or ($a eq '-qmos-data-size'))
		{
			setArg(\%sysdesc, $a, ++$i);
			$sysdesc{$a} = [ split(',', $sysdesc{$a}) ];
			if(grep { !/(^[1-9]\d*$)|(^0$)/ } @{$sysdesc{$a}})
		...
		elsif($a eq '-kern-data-size-core')
		{
			setArg(\%sysdesc, $a, ++$i);
			$sysdesc{$a} = [ split(',', $sysdesc{$a}) ];
			if(grep { !/(^[1-9]\d*$)|(^0$)/ } @{$sysdesc{$a}})
		...
		{
			my $optionfile = substr($a, 1);
			my @optionarray;
			open(my $fh, "<", $optionfile) or usage("Can't open < $optionfile: $!");
			while(my $line = <$fh>){
			    chomp $line;
			    push(@optionarray, split(" ", $line));
			}
			close($fh);
			splice @ARGV, $i--, 1, @optionarray;
		}
		else
		{
