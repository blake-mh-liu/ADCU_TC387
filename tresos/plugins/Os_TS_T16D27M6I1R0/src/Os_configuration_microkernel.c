...
*/
#include <memmap/Os_mm_const_begin.h>
volatile os_panic_t * const OS_panicCodePtrs[OS_N_CORES_MAX] = { OS_MAKE_COREARRAY_FROM(OS_CTRSUB_PANICCODE_PTR_CORE) };
#include <memmap/Os_mm_const_end.h>

/* Assertion */
#if (OS_N_CORES_MAX != MK_MAXCORES)
#error "Number of cores assumed to be provided by the hardware, differs between counter subsystem and microkernel."
