 ...
 */
 ...
 /* !LINKSTO LinSM.ASR40.SWS_LinSM_00078,1 */
[!AUTOSPACING!]
...
*/!][!MACRO "calcTimerValue", "Timer"!][!/*
  ...
  */!][!ELSE!][!/*
    ...
    */!][!IF "(($dValue - $ticks) > 0.00001) or (($dValue - $ticks) < -0.00001)"!][!/*
      */!][!WARNING!][!/*
        */!]["!$Timer!"] value of channel [!"as:name(.)"!] [!/*
...
{
[!LOOP "util:distinct(node:order(LinSMConfigSet/*[1]/LinSMChannel/*,'as:ref(LinSMComMNetworkHandleRef)/ComMChannelId'))"!]
  { /* channel [!"as:name(.)"!] */
  [!IF "$multiCoreSupport = 'true'"!]
...
[!WS "4"!][!IF "LinSMNodeType = 'SLAVE'"!]TRUE[!ELSE!]FALSE[!ENDIF!], /* is node SLAVE (TRUE) or MASTER (FALSE) */
  [!ENDIF!]
  [!WS "4"!][!IF "LinSMWakeupAndSleepIndication = 'true'"!]TRUE[!ELSE!]FALSE[!ENDIF!],  /* Wakeup And Sleep Indication */
  },
[!ENDLOOP!]
};

/* Empty configuration structure to be passed to LinSM_Init() */
CONST(LinSM_ConfigType, LINSM_APPL_CONST) [!"as:name(LinSMConfigSet/*[1])"!] = 0U;
