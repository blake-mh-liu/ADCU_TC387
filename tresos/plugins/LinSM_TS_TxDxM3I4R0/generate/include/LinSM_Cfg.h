...
#if (defined LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION) /* to prevent double declaration */
#error LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION already defined
#endif /* if (defined LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION) */

/** \brief Switch for Extend LinSM to notify BswM with LINSM_WAKEUP and LINSM_GOTO_SLEEP states. */
[!IF "LinSMConfigSet/*/LinSMChannel/*/LinSMWakeupAndSleepIndication = 'true'"!][!//
#define LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION  STD_ON
[!ELSE!][!//
#define LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION  STD_OFF
[!ENDIF!][!//
...
#if (defined LinSMConf_LinSMSchedule_[!"as:name(.)"!]) /* to prevent double declaration */
#error LinSMConf_LinSMSchedule_[!"as:name(.)"!] already defined
#endif /* if (defined LinSMConf_LinSMSchedule_[!"as:name(.)"!]) */

/** \brief Export symbolic name value for parameter LinIfScheduleIndex */
#define LinSMConf_LinSMSchedule_[!"as:name(.)"!] 0U

[!ELSE!]
[!IF "node:refvalid(LinSMScheduleIndexRef)='true'"!]
[!VAR "SymbolName" = "as:name(as:ref(LinSMScheduleIndexRef))"!]
/*---------------------------[!"as:name(.)"!]-------------------------------------*/
...
/** \brief Export symbolic name value for parameter LinIfScheduleIndex */
#define LinSMConf_LinSMSchedule_[!"as:name(.)"!] [!"concat('LinIfConf_LinIfScheduleTable_', $SymbolName)"!]
...
extern CONST(LinSM_ConfigType, LINSM_APPL_CONST) [!"as:name(LinSMConfigSet/*[1])"!];
