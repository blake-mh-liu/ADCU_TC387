...
/* !LINKSTO LinSM.EB.LinSMWakeupAndSleepIndication_Conf,1 */
/** \brief return if a given LinSM channel supports BSWM notification for LINSM_WAKEUP and LINSM_GOTO_SLEEP */
#define LINSM_BSWM_WAKEUPANDSLEEPINDICATION(id)  \
        (LinSM_ChannelConfig[(id)].WakeupAndSleepIndication)
...
[!IF "$numSlaveNodes > 0"!]
  boolean  ChIsSlave;
[!ENDIF!]
  boolean  WakeupAndSleepIndication;
