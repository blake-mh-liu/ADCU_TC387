...
/** \brief LinSM_ComMode_FULL_COMMUNICATION().
 *
 * In Case the mode is COMM_FULL_COMMUNICATION.
 * \param[in] id                LinSM channel id of channel to operate on
 */
STATIC FUNC(void, LINSM_CODE) LinSM_ComMode_FULL_COMMUNICATION
(
  LinSM_ChannelIdType id
);
...
)
{
...
#endif
  {
    ...
    switch (mode)
    {
      ...
      {
        LinSM_ComMode_FULL_COMMUNICATION(id);
        retval = E_OK;
        break;
      }
      ...
      case COMM_NO_COMMUNICATION:
      {
             ...
             )
          {
            ...
            {
              /* !LINKSTO LinSM.SWS_LinSM_00230_EB,1 */
          ...
          else
          {
            ...
            {
              LinSM_ProcessSlaveGotoSleep( network, mode, id);
            }
            else
#endif
            {
...
)
{
  ...
  uint8 currentChannelState = 0xFF;
#endif
...
#if (LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION == STD_ON)
    /* !LINKSTO LinSM.EB.LinSM_BswM_LINSM_WAKEUP_Notification,1 */
  if (LINSM_BSWM_WAKEUPANDSLEEPINDICATION(id) != FALSE)
  {
    BswM_LinSM_CurrentState(LINSM_CHANNEL_COMM_NW(id), LINSM_WAKEUP);
  }
#endif
  ...
  {
#if (LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION == STD_ON)
    currentChannelState = LINSM_CHANNEL_STATE(id);
#endif
    /* LinIf denied the request: reset channel state to old value */
    /* enter critical section */
...
#if (LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION == STD_ON)
    /* !LINKSTO LinSM.EB.LinSM_BswM_LINSM_WAKEUP_LinIfWakepFailure,1 */
    if ((LINSM_BSWM_WAKEUPANDSLEEPINDICATION(id) != FALSE) && (retval != E_OK) && (currentChannelState == LINSM_WAKEUP))
    {
      BswM_LinSM_CurrentState(LINSM_CHANNEL_COMM_NW(id), currentState);
    }
#endif
}
...
)
{
  ...
  /* save current state */
  currentState = LINSM_CHANNEL_STATE(id);
  /* !LINKSTO LinSM.SWS_LinSM_00302,1 */
...
#if (LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION == STD_ON)
  /* !LINKSTO LinSM.EB.LinSM_BswM_LINSM_GOTO_SLEEP_Notification,1 */
  if (LINSM_BSWM_WAKEUPANDSLEEPINDICATION(id) != FALSE)
  {
    BswM_LinSM_CurrentState(LINSM_CHANNEL_COMM_NW(id), LINSM_GOTO_SLEEP);
  }
#endif
...
#if (LINSM_BSWM_WAKEUP_AND_SLEEP_INDICATION == STD_ON)
  /* !LINKSTO LinSM.EB.LinSM_BswM_LINSM_GOTO_SLEEP_LinIfGoToSleepFailure,1 */
  if ((LINSM_BSWM_WAKEUPANDSLEEPINDICATION(id) != FALSE) && (retval != E_OK) && (currentChannelState == LINSM_GOTO_SLEEP))
  {
    BswM_LinSM_CurrentState(LINSM_CHANNEL_COMM_NW(id), currentState);
  }
#endif
  ...
  if (LINSM_CHANNEL_IS_SLAVE(id) == TRUE)
  {
    if (retval == E_OK)
    {
      /* !LINKSTO LinSM.SWS_LinSM_00233_EB,2 */
...
)
{
  ...
  {
    /* !LINKSTO LinSM.SWS_LinSM_00237,1 */
#if (LINSM_SLAVE_NODES_ENABLED == STD_ON)
...
#if (LINSM_SLEEPSUPPORT_TYPE == LINSM_ALL)
      /* !LINKSTO LinSM.SWS_LinSM_00047,1 */
      LinSM_WakeUp(id);
#elif (LINSM_SLEEPSUPPORT_TYPE == LINSM_NONE)
      /* Deviation MISRAC2012-1 */
