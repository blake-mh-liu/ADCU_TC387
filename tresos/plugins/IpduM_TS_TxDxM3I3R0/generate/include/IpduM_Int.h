 ...
 *
 * MISRAC2012-2) Deviated Rule: 11.3
 ...
 *
 * MISRAC2012-3) Deviated Rule: 10.7
 * If a composite expression is used as one operand of an operator in which the usual arithmetic conversions
 * are performed then the other operand shall not have wider essential type.
 *
 * Reason:
 * For post build configuration this is needed.
 *
 */
...
/* !LINKSTO IpduM.SWS_IpduM_00247,1 */
...
/** \brief macro to check if the ContainerTxPdu has the CanFd padding service activated */
#if (defined IPDUM_CNTRTX_HAS_CANFD_PADDING)
#error IPDUM_CNTRTX_HAS_CANFD_PADDING already defined!
#endif /* #if (defined IPDUM_CNTRTX_HAS_CANFD_PADDING) */
#define IPDUM_CNTRTX_HAS_CANFD_PADDING(cfgFlag) (boolean)((((cfgFlag) & 0x40U) != 0U)?TRUE:FALSE)

/** \brief macro to check if the ContainerTxPdu has MetaData configured */
#if (defined IPDUM_CNTRTX_HAS_METADATA)
#error IPDUM_CNTRTX_HAS_METADATA already defined!
#endif /* #if (defined IPDUM_CNTRTX_HAS_METADATA) */
#define IPDUM_CNTRTX_HAS_METADATA(cfgFlag) (boolean)((((cfgFlag) & 0x80U) != 0U)?TRUE:FALSE)
...
/** \brief macro to check if the ContainedTxPdu has MetaData */
#if (defined IPDUM_CNTND_HAS_METADATA)
#error IPDUM_CNTND_HAS_METADATA already defined!
#endif /* #if (defined IPDUM_CNTND_HAS_METADATA)*/
#define IPDUM_CNTND_HAS_METADATA(cfgFlag) (boolean)((((cfgFlag) & 0x04U) != 0U)?TRUE:FALSE)
...
#define IPDUM_GET_CNTR_TXDATA(ContainerTxIndex, idx)  (P2VAR(uint8, AUTOMATIC, IPDUM_VAR_CLEARED))&IpduM_GDataMemPtr[IPDUM_PBCFG_ACCESS(uint32, IpduM_ConfigPtr->TxCntrInst)[((sint32)IPDUM_PBCFG_ACCESS(IpduM_ContainerTxPduType,IpduM_ConfigPtr->ContainerTxPdu)[(ContainerTxIndex)].InstFirstIdx + (idx))]]
...
[!IF "$IpduMMultiMF = 'true'"!]
/* Deviation MISRAC2012-2 */
#define IPDUM_GET_DEFERRED_LIST(mfIdx)  (P2VAR(uint32, AUTOMATIC, IPDUM_VAR_CLEARED))&IpduM_GDataMemPtr[(IPDUM_RX_DATA_CNTR_OFFSET + ((uint32)sizeof(uint32) * (uint32)[!"$IpduMRxBFSizeU32"!]U * ((uint32)mfIdx)))]
/* Deviation MISRAC2012-2 */
#define IPDUM_GET_TX_CNTR_CH_BF(mfIdx)  (P2VAR(uint32, AUTOMATIC, IPDUM_VAR_CLEARED))&IpduM_GDataMemPtr[(IPDUM_TX_DATA_CNTR_OFFSET + ((uint32)sizeof(uint32) * (uint32)[!"$IpduMBFSizeTxU32"!]U * (uint32)(mfIdx)))]
[!ELSE!]
/* Deviation MISRAC2012-2 */
...
#if (defined IPDUM_TX_ADVANCE_READ_BY_ONE)
#error IPDUM_TX_ADVANCE_READ_BY_ONE already defined!
#endif
#define IPDUM_TX_ADVANCE_READ_BY_ONE(qPtr, NumInstances) \
 do \
 { \
   (qPtr)->ReadIdx = ((qPtr)->ReadIdx + 1U) % (NumInstances); \
 } while (0)
...
#if (defined IPDUM_TXCNTR_EV_Q_OVFL)
#error IPDUM_TXCNTR_EV_Q_OVFL already defined!
#endif
#define IPDUM_TXCNTR_EV_Q_OVFL           0x80U
...
#if (defined IPDUM_TXCNTR_TX_INT_BITIDX)
#error IPDUM_TXCNTR_TX_INT_BITIDX already defined!
#endif
#define IPDUM_TXCNTR_TX_INT_BITIDX       4U

#if (defined IPDUM_TXCNTR_TX_INT_MSK)
#error IPDUM_TXCNTR_TX_INT_MSK already defined!
#endif
#define IPDUM_TXCNTR_TX_INT_MSK          16U
...
[!IF "$MetaDataEn = 'true'"!]
[!VAR "ContainerRxMetaDataMaxLength" = "num:i(1)"!]
[!LOOP "as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerRxPdu/*"!]
    [!IF "node:exists(as:ref(IpduMContainerRxPduRef)/MetaDataTypeRef) = 'true'"!]
      [!IF "num:i($ContainerRxMetaDataMaxLength) < num:i(sum(as:ref(as:ref(IpduMContainerRxPduRef)/MetaDataTypeRef)/MetaDataItem/*/MetaDataItemLength))"!]
        [!VAR "ContainerRxMetaDataMaxLength" = "num:i(sum(as:ref(as:ref(IpduMContainerRxPduRef)/MetaDataTypeRef)/MetaDataItem/*/MetaDataItemLength))"!]
      [!ENDIF!]
    [!ENDIF!]
[!ENDLOOP!]
#if (defined IPDUM_CONTAINERRXMETADATAMAXLENGTH)
#error IPDUM_CONTAINERRXMETADATAMAXLENGTH already defined!
#endif /* #if (defined IPDUM_CONTAINERRXMETADATAMAXLENGTH)*/
#define IPDUM_CONTAINERRXMETADATAMAXLENGTH [!"$ContainerRxMetaDataMaxLength"!]
[!ENDIF!][!// MetaDataEn

/** \brief Used for masking the priority in the MetaData as well as initial value */
#if (defined IPDUM_METADATA_PRIO_MSK)
#error IPDUM_METADATA_PRIO_MSK already defined!
#endif /* #if (defined IPDUM_METADATA_PRIO_MSK)*/
#define IPDUM_METADATA_PRIO_MSK 0x[!"node:when($MetaDataTxPrioEn = 'true', '1C000000UL', '0U')"!]

[!IF "$MetaDataTxPrioEn = 'true'"!]
/** \brief Used for setting the MetaData priority and handling it as uint8 */
#if (defined IPDUM_MTDT_PRIO_SHIFT_VAL)
#error IPDUM_MTDT_PRIO_SHIFT_VAL already defined!
#endif /* #if (defined IPDUM_MTDT_PRIO_SHIFT_VAL)*/
#define IPDUM_MTDT_PRIO_SHIFT_VAL 26U

[!IF "$MetaDataEn = 'false'"!]
/** \brief Used for resetting the instance MetaData priority in IDLE */
#if (defined IPDUM_MTDT_PRIO_LOWEST_VAL)
#error IPDUM_MTDT_PRIO_LOWEST_VAL already defined!
#endif /* #if (defined IPDUM_MTDT_PRIO_LOWEST_VAL)*/
#define IPDUM_MTDT_PRIO_LOWEST_VAL 7U
[!ENDIF!]
[!ENDIF!][!// MetaDataTxPrioEn

[!IF "node:value(as:modconf('IpduM')[1]/IpduMGeneral/IpduMCanFdPaddingSupport) = 'true'"!]
/*------------------[macros for CanFd padding]--------------------------------*/

/** \brief Maximum number of CanFd DLC intervals */
#if (defined IPDUM_NUMBER_OF_CANFD_INTERVALS)
#error IPDUM_NUMBER_OF_CANFD_INTERVALS is already defined
#endif
#define IPDUM_NUMBER_OF_CANFD_INTERVALS 7U

/** \brief Invalid CanFd DLC interval */
#if (defined IPDUM_INVALID_CANFD_INTERVAL)
#error IPDUM_INVALID_CANFD_INTERVAL is already defined
#endif
#define IPDUM_INVALID_CANFD_INTERVAL 0xFFU

/** \brief Invalid CanFd DLC interval */
#if (defined IPDUM_CANFD_PADDING_VALUE)
#error IPDUM_CANFD_PADDING_VALUE is already defined
#endif
#define IPDUM_CANFD_PADDING_VALUE 0xAAU

/** \brief Minimum between two values */
#if (defined IPDUM_MIN)
#error IPDUM_MIN is already defined
#endif
#define IPDUM_MIN(a, b) (((a) < (b)) ? (a) : (b))
[!ENDIF!]
