...
#endif
[!IF "node:empty(IpduMGeneral/IpduMHeaderByteOrder) or node:value(IpduMGeneral/IpduMHeaderByteOrder) = 'IPDUM_LITTLE_ENDIAN'"!]
#define IPDUM_HDR_ID_OFFST 0
[!ELSE!]
#define IPDUM_HDR_ID_OFFST 1
[!ENDIF!]

#if (defined IPDUM_USE_METADATA)
#error IPDUM_USE_METADATA is already defined
#endif
/** \brief This Macro determines if the MetaData option is enabled or not.
 **/
#define IPDUM_USE_METADATA STD_[!"node:when($MetaDataEn = 'true', 'ON', 'OFF')"!]

#if (defined IPDUM_METADATA_TX_PRIO_SUPPORT)
#error IPDUM_METADATA_TX_PRIO_SUPPORT is already defined
#endif
/** \brief This Macro determines if the MetaData Priority option is enabled or not.
 **/
#define IPDUM_METADATA_TX_PRIO_SUPPORT STD_[!"node:when($MetaDataTxPrioEn = 'true', 'ON', 'OFF')"!]

#if (defined IPDUM_CANFD_PADDING_SUPPORT)
#error IPDUM_CANFD_PADDING_SUPPORT is already defined
#endif
/** \brief This Macro determines if the CanFd padding service is enabled or not.
 **/
#define IPDUM_CANFD_PADDING_SUPPORT STD_[!"node:when(./IpduMGeneral/IpduMCanFdPaddingSupport = 'true', 'ON', 'OFF')"!]
...
[!IF "node:contains(util:distinct(node:refs(as:modconf('PbcfgM')/PbcfgMBswModules/*/PbcfgMBswModuleRef)), as:modconf('IpduM')) = 'true'"!]
...
#endif
#define IPDUM_PBCFGM_SUPPORT_ENABLED STD_[!IF "node:contains(util:distinct(node:refs(as:modconf('PbcfgM')/PbcfgMBswModules/*/PbcfgMBswModuleRef)), as:modconf('IpduM')) = 'true'"!]ON[!ELSE!]OFF[!ENDIF!]
...
#define IPDUM_CFG_SIGNATURE [!"asc:calculateConfigSignature(as:modconf('IpduM')[1], 'PreCompile')"!]U
...
#define IPDUM_PUBLIC_INFO_SIGNATURE [!"asc:calculateConfigSignature(as:modconf('IpduM')[1]/CommonPublishedInformation, 'PublishedInformation')"!]U
