 ...
 *   cache miss.
 *
 * MISRAC2012-8) Deviated Rule: 5.5 (required)
 * Identifiers shall be distinct from macro names.
 *
 * Reason:
 * As stated by IpduM.ASR40.SWS_IpduM_00085 the module IpduM should realize this API as a function or a macro.
 *
 * MISRAC2012-9) Deviated Rule: 5.8 (required)
...
/* !LINKSTO IpduM.ASR40.SWS_IpduM_00148_3,1 */
#include <PduR_IpduM.h>
/* !LINKSTO IpduM.ASR40.SWS_IpduM_00148_2,1 */
#include <IpduM_Cbk.h>
...
/* !LINKSTO IpduM.ASR40.SWS_IpduM_00148_5,1 */
#include <Det.h>
#endif

#if ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
#include <EcuC.h>
#endif
...
/* patch version check */
#if (IPDUM_SW_PATCH_VERSION < 57U)
#error IPDUM_SW_PATCH_VERSION wrong (< 57U)
#endif
...
/* typedef for the CanFd intervals const struct */
typedef struct /* IpduM_CanfdIntervals */ {
    VAR( uint8, TYPEDEF ) Start;
    VAR( uint8, TYPEDEF ) End;
} IpduM_CanfdIntervalsType;
...
(
  ...
  P2VAR(sint32, AUTOMATIC, AUTOMATIC) Contained_Rx_Idx,
...
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_TX_ST_TRIGGERED
(
  P2CONST(PduInfoType, AUTOMATIC, IPDUM_APPL_DATA) PduInfoPtr,
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_OFF)
  ...
  P2VAR(Std_ReturnType, AUTOMATIC, AUTOMATIC) RetVal,
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
  uint32 MetaDataValue,
#endif
  PduIdType ContainerIdx,
  PduIdType IdxContained
);

IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_TransmitReadyInst
(
  P2CONST(IpduM_ContainerTxPduType, AUTOMATIC, IPDUM_APPL_CONST) TxCntrPtr,
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) Event,
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
...
IPDUM_STATIC FUNC(Std_ReturnType, IPDUM_CODE) IpduM_GetTrgTxPayload
(
  P2VAR(PduInfoType, AUTOMATIC, IPDUM_APPL_DATA) PduInfoPtr,
  P2CONST(IpduM_ContainerTxPduType, AUTOMATIC, IPDUM_APPL_CONST) TxCntrPtr,
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
...
#if ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
  ,uint32 MetaDataValue
#endif /* (IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) */
);
...
/** \brief Checks is the contained PDU is new to the container.
 *
 * \param[in] InstPtr     Instance pointer
 * \param[in]  ContainerIdx  Idx of the Container
 * \param[in]  IdxContained  Idx of the LastIsBest ContainedPdu
 *
 * \return boolean
 * \retval TRUE  PDU is new to the container.
 *
 * \retval FALSE  PDU already exists in the container.
  */
IPDUM_STATIC FUNC(boolean, IPDUM_CODE) IpduM_IsPduNewToContainer
(
  volatile P2CONST(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
  PduIdType ContainerIdx,
  PduIdType IdxContained
);
...
/** \brief Advances the instance read index
 *
 * Used when overflow has to be handled during instance dequeuing
 *
 * \param[inout]  QPtr          Pointer to queue
 * \param[in]     ContainerIdx  Idx of the Container
 * \param[in]     NumInstances  Number of instances
 *
 * \return void
 */
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_TxAdvanceReadWOVFL
(
  P2VAR(IpduM_CntrQueueType, AUTOMATIC, IPDUM_VAR_CLEARED) QPtr,
  PduIdType ContainerIdx,
  uint8 NumInstances
...
#if (IPDUM_CANFD_PADDING_SUPPORT == STD_ON)
/** \brief Handles the CanFd padding service
 *
 * Used when padding is needed for a to-be-transmitted container
 * up to the closest CanFD standard Data Length Code.
 *
 * \param[inout]  pduInfoPtr    PDU to be transmitted
 * \param[in]     TxCntrPtr     Pointer to the container configuration structure
 *
 * \return void
 */
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_CanFdPaddingService
(
    P2VAR(PduInfoType, AUTOMATIC, AUTOMATIC) pduInfoPtr,
    P2CONST(IpduM_ContainerTxPduType, AUTOMATIC, IPDUM_APPL_CONST) TxCntrPtr
);

/** \brief Searches for the CanFd DLC data interval
 *
 * Used to search in which CanFd length interval the PduLength resides
 *
  * \param[in]     pduLength     Length of the Pdu to be transmitted
 *
 * \return Interval index or an invalid value if no padding is needed
 */
IPDUM_STATIC FUNC(uint8, IPDUM_CODE) IpduM_FindInterval (PduLengthType pduLength);

#endif /* IPDUM_CANFD_PADDING_SUPPORT == STD_ON */

#if (IPDUM_USE_METADATA == STD_ON)

/** \brief Evaluates trigger condition
 *
 * Used to establish whether the Container has to be triggered
 * due to new MetaData (ignoring the priority)
 *
 * Priority is handled only if IpduMMetaDataTxPrioritySupport is enabled
 *
 * \param[inout]  InstPtr            Instance pointer
 * \param[in]     ContainedMetaData  Meta Data Value of the ContainedPdu
 * \param[in]     CntrHasMetadata    Boolean value
 *
 * \return boolean
 * \retval TRUE   New MetaData differs from the stored one, transmit container
 * \retval FALSE  New MetaData is the same as the stored one, no trigger condition
 */
IPDUM_STATIC FUNC(boolean, IPDUM_CODE) IpduM_EvalTrgMetaData
(
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
  uint32 ContainedMetaData,
  boolean CntrHasMetadata
);
#endif /* IPDUM_USE_METADATA == STD_ON */

#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
/** \brief Stores the priority (MetaData)
 *
 * Used to store and update the priority
 *
 * \param[inout]  InstPtr            Instance pointer
 * \param[in]     ContainedMetaData  Meta Data Value of the ContainedPdu
 * \param[in]     CntdHasMetadata    Boolean value
 *
 * \return void
 */
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_SetInstMtdtPrio
(
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
  uint32 ContainedMetaData,
  boolean CntdHasMetadata
);
#endif /* IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON */

#if((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
/** \brief Sets the MetaData in EcuC (type CAN_ID_32)
 *
 * Used to set the MetaData with the updated priority
 *
 * \param[in]     InstPtr            Instance pointer
 * \param[in]     TxCntrPtr          Pointer to the container configuration structure
 *
 * \return void
 */
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_SetEcucCanMetadata
(
  volatile P2VAR(IpduM_CntrTxInstType, AUTOMATIC, IPDUM_VAR_CLEARED) volatile InstPtr,
  P2CONST(IpduM_ContainerTxPduType, AUTOMATIC, IPDUM_APPL_CONST) TxCntrPtr
);
...
#if (IPDUM_CANFD_PADDING_SUPPORT == STD_ON)
IPDUM_STATIC CONST(IpduM_CanfdIntervalsType, IPDUM_APPL_CONST) IpduM_CanfdIntervals[IPDUM_NUMBER_OF_CANFD_INTERVALS] =
{
    { 9u, 12u},
    {13u, 16u},
    {17u, 20u},
...
/* !LINKSTO IpduM.ASR40.SWS_IpduM_00075,1 */
...
#endif /* IPDUM_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    else
    {
      if(0U < PduInfoPtr->SduLength)
      {
...
#endif /* IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON */
#if ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
        /* !LINKSTO IpduM.EB.MetaData.Tx.Priority.Transmit.NoContainedPrio,1 */
        uint32 PduMetaData = IPDUM_METADATA_PRIO_MSK;
        /* Get MetaData of the Contained Pdu in Case the Container is configured with MetaData */
        if(IPDUM_CNTRTX_HAS_METADATA(IPDUM_CFG_GET_CNTR_TX(ParentContainerIndex)->CfgFlags))
        {
          P2CONST(IpduM_ContainedTxPduType, AUTOMATIC, IPDUM_APPL_CONST) ContainedPdu = IPDUM_CFG_GET_CNTD_TX(ContainedPduIndex);

          if(IPDUM_CNTND_HAS_METADATA(ContainedPdu->CfgFlags))
          {
            (void) EcuC_GetMetaDataCanId(ContainedPdu->TxContainedMetaDataPduId, PduInfoPtr, &PduMetaData);
          }
#if (IPDUM_USE_METADATA == STD_ON)
          /* Calculate MetaData even if no metadata is configured for that Contained Pdu (as some modules don't support metadata, such as Com).
             In addition, overwrite some parts from the gotten MetaData as CanId32 type has four bytes, only the first two least bytes are of interest,
             (SA & DA) and the remaining should be discarded => SAE J1939-22
           */
          PduMetaData = (PduMetaData & ContainedPdu->IpduMTxMetaDataMask) | (ContainedPdu->IpduMTxMetaDataValue & (~ContainedPdu->IpduMTxMetaDataMask));
#endif /* IPDUM_USE_METADATA == STD_ON */
        }
#endif /* (IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) */
        ...
        {
#if ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
          RetValue = IpduM_CntrTransmitLB(PduInfoPtr, ParentContainerIndex, ContainedPduIndex, PduMetaData);
#else
          RetValue = IpduM_CntrTransmitLB(PduInfoPtr, ParentContainerIndex, ContainedPduIndex);
#endif /* (IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) */
        }
      }
      else
    ...
    if(TxPduId < IpduM_ConfigPtr->TxPathWayLen)
    {
      ...
      if (PduInfoPtr->SduLength >= IPDUM_GET_BYTE_SIZE(TxPathWay->Size))
      {
...
#if (IPDUM_ENABLE_JIT_UPDATE == STD_ON)
        {
          ...
          /* !LINKSTO IpduM.SWS_IpduM_00172,1 */
...
/* !LINKSTO IpduM.SWS_IpduM_00252,1 */
...
)
{
...
#endif /* (IPDUM_DEFENSIVE_PROGRAMMING_ENABLED == STD_ON) */
  {
    ...
    if (0U == TxData[TxPartT->TxConfirmationPduId].ConfirmationTimeoutCounter)
    {
      /* No request pending */
    ...
    if (TxFlag == TRUE)
    {
      ...
      if (RetValUpdatePdu != E_OK)
      {
        ...
        /* !LINKSTO IpduM.SWS_IpduM_00171.NoTransmission,2 */
        TxFlag = FALSE;
      }
    }
#endif /* (IPDUM_ENABLE_JIT_UPDATE == STD_ON) */
    ...
    /* !LINKSTO IpduM.SWS_IpduM_00171.RetVal,2 */
...
)
{
  ...
  sint32 right = (left + (sint32)ContainerPdu->ContainedRxPduNum) - 1;
...
#if (IPDUM_USE_METADATA == STD_ON)
  uint8 PduMetaData[IPDUM_CONTAINERRXMETADATAMAXLENGTH];
  PduInfoType LocalPduForMetadata = {NULL_PTR, 0U};

  /* !LINKSTO IpduM.EB.MetaData.Rx.GetMetadata,1 */
  if(ContainerPdu->ContainerMetaDataLength > 0U)
  {
    (void) EcuC_CopyMetaDataTypeFromPdu(ContainerPdu->ContainerMetaDataPduId, &LocalPduForMetadata, &PduMetaData[0]);
  }
#endif /* IPDUM_USE_METADATA == STD_ON */
    ...
    if(RemainLen >= HeaderSize)
    {
      ...
      if(PduHeaderId != 0U)
      {
        if ((ContainedDLCLength + HeaderSize) > ContainedDLCLength)
        {
          ...
          {
            /*this value will never be used, IpduM_RxCntd_BinSearch() will set the value of outGoingPduId*/
            PduIdType outGoingPduId = 0xFFFFU;
            sint32 Contained_Rx_Idx = 0;
#if (IPDUM_USE_METADATA == STD_ON)
            P2CONST(IpduM_ContainedRxPduType, AUTOMATIC, IPDUM_APPL_CONST) ContainedPdu;
#endif

            if(E_OK == IpduM_RxCntd_BinSearch(ContainerPdu, &outGoingPduId, &Contained_Rx_Idx,  IPDUM_SWAPU32_IF_MUST(PduHeaderId)))
            {
...
#if (IPDUM_USE_METADATA == STD_ON)
              ContainedPdu = IPDUM_CFG_GET_CNTD_RX(Contained_Rx_Idx);

              /* !LINKSTO IpduM.EB.MetaData.Rx.SetMetadata,1 */
              if(ContainerPdu->ContainerMetaDataLength > 0U && ContainedPdu->ContainedMetaDataLength > 0U)
              {
                (void) EcuC_CopyMetaDataTypeToPdu(ContainedPdu->ContainedMetaDataPduId, &LocalPduForMetadata, &PduMetaData[0]);
              }
#endif

              /* forward matching received PDUs to PduR */
              PduR_IpduMRxIndication(outGoingPduId, &LocalPdu);
...
)
{
  ...
  for(idx = startIdx; idx < stopIdx; idx++)
  {
    ...
    {
      /* Only that last contained PDU can have variable size */
      if(idx ==  (stopIdx - 1U))
      {
...
)
{
  if(PduInfoPtr->SduLength <= IPDUM_CFG_GET_CNTR_RX(ContainerRxIndex)->PduLength)
  {
    ...
    /* Deviation MISRAC2012-1 */
    P2VAR(IpduM_CntrQueueType,  AUTOMATIC, AUTOMATIC) rxQueue = IPDUM_GET_RX_QUEUE(ContainerRxIndex);
    uint8 writeIdx = (cntrPtr->NumInstances > 1U) ? rxQueue->WriteIdx : 0U;
#else
...
{
#if(IPDUM_STATIC_CONTAINER_PDU_HANDLING_ENABLE == STD_ON)
  IpduM_NumCntdType cntdPdusNum = InstPtr->NumCntd;
#endif
...
#else
  if (((Length + TxCntrPtr->HeaderSize) >= TxCntrPtr->PduLength)
  || ((IPDUM_HEADERTYPE_NONE == TxCntrPtr->HeaderSize) && (TxCntrPtr->NumCntdMax == cntdPdusNum)))
#endif
  {
...
)
{
  ...
  if(NumInstances > 1U)
  {
    ...
    /* Deviation MISRAC2012-1 */
    nextWIdxTemp = ((IPDUM_GET_IDX_TX_INST(ContainerIdx, InstPtr) + 1) % ((sint32)NumInstances));
    nextWIdx = (uint8)nextWIdxTemp;
...
#if (IPDUM_DEV_ERROR_DETECT == STD_ON)
    while(0U == (*Event & IPDUM_TXCNTR_EV_TRANSMIT))
#else
    while(IPDUM_TXCNTR_EV_TRANSMIT != *Event)
#endif
    {
      state = nextInst->State;
      ...
      switch(state)
      {
        ...
        {
          if(QPtr->ReadIdx == QPtr->WriteIdx)
          {
            IpduM_TxAdvanceReadWOVFL(QPtr, ContainerIdx, NumInstances);
          }
          /* Instance has overflown without being transmitted, drop it and store PDU */
          nextInst->State = IPDUM_TXCNTR_ST_IDLE;
#if (IPDUM_DEV_ERROR_DETECT == STD_ON)
          *Event = (uint8)(IPDUM_TXCNTR_EV_TRANSMIT | IPDUM_TXCNTR_EV_Q_OVFL);
#else
          *Event = IPDUM_TXCNTR_EV_TRANSMIT;
#endif /* IPDUM_DEV_ERROR_DETECT == STD_ON */
          break;
        ...
        {
          TS_AtomicClearBit_8(&nextInst->InstLock, IPDUM_TXCNTR_CNTD_STR_BTIDX);
          /* TriggerTransmit or TxConfirmation pending, jump over */
          nextWIdx = (nextWIdx + 1U) % NumInstances;
          /* Deviation MISRAC2012-1 */
          nextInst = IPDUM_GET_TX_INST(ContainerIdx, nextWIdx);
          TS_AtomicSetBit_8(&nextInst->InstLock, IPDUM_TXCNTR_CNTD_STR_BTIDX);
          QPtr->WriteIdx = nextWIdx;
          /* Event remains IPDUM_TXCNTR_EV_NEW_INST */
          break;
        }
        /* CHECK: NOPARSE */
        default:
    ...
    if(0U != (InstPtr->InstLock & IPDUM_TXCNTR_TX_INT_MSK))
    {
      IPDUM_PRECONDITION_ASSERT(IPDUM_TXCNTR_ST_IDLE == InstPtr->State, IPDUM_SID_TRANSMIT);

      IpduM_TxAdvanceReadWOVFL(QPtr, ContainerIdx, NumInstances);
      TS_AtomicClearBit_8(&InstPtr->InstLock, IPDUM_TXCNTR_TX_INT_BITIDX);
      *QFull = FALSE;
    }
  }
  else
  ...
  if(0U == (instPtr->InstLock & IPDUM_TXCNTR_CNTD_STR_MSK))
  {
    ...
    while(IPDUM_TXCNTR_EV_NONE != event)
    {
      ...
      switch(state)
      {
        ...
        {
          PduLengthType buffOffset;
          boolean triggerCntr = FALSE;
          boolean added = FALSE;
#if (IPDUM_USE_METADATA == STD_ON)
          boolean triggerCntrDiffMetaData = IpduM_EvalTrgMetaData(instPtr, MetaDataValue, IPDUM_CNTRTX_HAS_METADATA(txCntrPtr->CfgFlags));
#endif

          IPDUM_AA_PDULEN(buffOffset, instPtr->PduLength);

          /* Does the PDU already exist in the container? */
          if(TRUE == IpduM_IsPduNewToContainer(instPtr,ContainerIdx,IdxContained))
          {
            /* Does it fit ? */
#if (IPDUM_USE_METADATA == STD_ON)
            if((containedPtr->PduLength + (PduLengthType)txCntrPtr->HeaderSize) <= (txCntrPtr->PduLength - buffOffset) && (triggerCntrDiffMetaData == FALSE))
#else
...
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
              /* Added to current instance, update priority */
              IpduM_SetInstMtdtPrio(instPtr, MetaDataValue, IPDUM_CNTND_HAS_METADATA(IPDUM_CFG_GET_CNTD_TX(IdxContained)->CfgFlags));
#endif
            }
            else
            {
                triggerCntr = TRUE; /* Either exceeded the size or received different Metadata Values */
            }
          }
          ...
          if(added)
          {
            ...
            IpduM_EvalTxCntrTrgCond(txCntrPtr, containedPtr, instPtr, &event, &RetVal, currentLength);
          }
          else if(triggerCntr)
          {
          ...
          {
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
              /* Already in the current instance, update priority */
              IpduM_SetInstMtdtPrio(instPtr, MetaDataValue, IPDUM_CNTND_HAS_METADATA(IPDUM_CFG_GET_CNTD_TX(IdxContained)->CfgFlags));
#endif
        ...
        {
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_OFF)
          IpduM_TX_ST_TRIGGERED(PduInfoPtr, txCntrPtr, &event, &qFull, &RetVal, instPtr, ContainerIdx, IdxContained);
#else
          IpduM_TX_ST_TRIGGERED(PduInfoPtr, &event, &qFull, &RetVal, instPtr, MetaDataValue, ContainerIdx, IdxContained);
#endif
        ...
        case IPDUM_TXCNTR_ST_WAIT_CONF:
        {
          ...
          if(0U == (instPtr->InstLock & IPDUM_TXCNTR_SEALED_MSK))
          {
            ...
            if(TRUE == IpduM_IsPduNewToContainer(instPtr,ContainerIdx,IdxContained))
            {
              if((containedPtr->PduLength + (PduLengthType)txCntrPtr->HeaderSize) <= (txCntrPtr->PduLength - instLen))
              {
                boolean added;
      ...
      if(IPDUM_TXCNTR_EV_NEW_INST == event)
      {
        SchM_Enter_IpduM_SCHM_IPDUM_EXCLUSIVE_AREA_0();
        {
          uint8 InstIdx = IpduM_TxCntrNewInst(instPtr, qPtr, &event, &qFull, ContainerIdx, txCntrPtr->NumInstances);
          /* Deviation MISRAC2012-1 */
        ...
        if(0U != (event & IPDUM_TXCNTR_EV_Q_OVFL))
        {
          IPDUM_DET_REPORT_ERROR(IPDUM_SID_TRANSMIT, IPDUM_E_QUEUEOVFL);
          event &= (uint8)~IPDUM_TXCNTR_EV_Q_OVFL;
        }
#endif /* IPDUM_DEV_ERROR_DETECT == STD_ON */
      }
    }
...
#if (IPDUM_USE_METADATA == STD_ON)
  InstPtr->MetaDataValue = 0U;
#else
#if (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
  /* !LINKSTO IpduM.EB.MetaData.Tx.Priority.Transmit.NoContainedPrio,1 */
  InstPtr->MetaDataPrio = IPDUM_MTDT_PRIO_LOWEST_VAL;
#endif /* IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON */
#endif /* IPDUM_USE_METADATA == STD_ON */
...
)
{
  ...
  if(IPDUM_TXCNTR_EV_DEQUEUE == (*Event & IPDUM_TXCNTR_EV_DEQUEUE))
  {
    ...
    {
      IpduM_TransmitReadyInst(TxCntrPtr,Event,InstPtr,qPtr,cntrRamData,ContainerIdx,instIdx);
    }
  ...
  else
  {
    ...
    {
#if(IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON)
      boolean isLastIsBest = IPDUM_CNTRTX_IS_LAST_IS_BEST(TxCntrPtr->CfgFlags);
      boolean processNew = isLastIsBest ? IpduM_IsPduNewToContainer(InstPtr,ContainerIdx,IdxContained) : (boolean)TRUE;
      if(TRUE == processNew)
#else
      /* Does the PDU already exist in the container? */
      if(TRUE == IpduM_IsPduNewToContainer(InstPtr,ContainerIdx,IdxContained))
      ...
      {
#if(IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON)
        PduLengthType cntdLen = isLastIsBest ? IPDUM_CFG_GET_CNTD_TX(IdxContained)->PduLength : PduInfoPtr->SduLength;
#else
        PduLengthType cntdLen = IPDUM_CFG_GET_CNTD_TX(IdxContained)->PduLength;
#endif
        PduLengthType buffOffset;
        IPDUM_AA_PDULEN(buffOffset, InstPtr->PduLength);

        if((cntdLen + (PduLengthType)TxCntrPtr->HeaderSize) <= (TxCntrPtr->PduLength - buffOffset))
        {
#if(IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON)
...
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
          /* CQ or LB added to current instance, update priority */
          IpduM_SetInstMtdtPrio(InstPtr, MetaDataValue, IPDUM_CNTND_HAS_METADATA(IPDUM_CFG_GET_CNTD_TX(IdxContained)->CfgFlags));
#endif
          *Event = IPDUM_TXCNTR_EV_NONE;
          *RetVal = E_OK;
      ...
      {
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
        /* LB is already in the current instance, update priority */
        IpduM_SetInstMtdtPrio(InstPtr, MetaDataValue, IPDUM_CNTND_HAS_METADATA(IPDUM_CFG_GET_CNTD_TX(IdxContained)->CfgFlags));
#endif
    ...
    {
      *Event = IPDUM_TXCNTR_EV_NEW_INST;
      /* Single instance retriggered, no TX */
      *QFull = TRUE;
    }
  }

#if(IPDUM_CNTND_COLLECT_QUEUED_TX == STD_OFF)
...
)
{
  ...
  uint16 timeoutVal = TxCntrPtr->IpduMContainerTxConfirmationTimeout;
  PduInfoType pduInfo;
  ...
  if(E_OK == IpduM_GetTrgTxPayload(&pduInfo, TxCntrPtr, InstPtr, ContainerIdx, instIdx))
  {

#if (IPDUM_CANFD_PADDING_SUPPORT == STD_ON)
    /* Check if container has the CANFD padding service enabled */
    if(IPDUM_CNTRTX_HAS_CANFD_PADDING(TxCntrPtr->CfgFlags) && (IPDUM_CNTRTX_IS_TXMODE_DIRECT(TxCntrPtr->CfgFlags)))
    {
      /* Perform the padding service */
      IpduM_CanFdPaddingService(&pduInfo, TxCntrPtr);
    }
#endif  /* IPDUM_CANFD_PADDING_SUPPORT == STD_ON */
...
#if ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON))
    /* Set MetaData/Priority for Tx Container */
    IpduM_SetEcucCanMetadata(InstPtr, TxCntrPtr);
#endif /* (IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) */
    ...
    if(E_OK == PduR_IpduMTransmit(TxCntrPtr->IpduMContainerTxHandleId, &pduInfo))
    {
      if(IPDUM_CNTRTX_IS_TXMODE_DIRECT(TxCntrPtr->CfgFlags))
      {
        if(0U == timeoutVal)
        {
          ...
          IPDUM_PRECONDITION_ASSERT(0U != ((IPDUM_TXCNTR_EV_DEQUEUE | IPDUM_TXCNTR_EV_NEW_INST) & *Event), IPDUM_SID_TRANSMIT);

          if(0U != (IPDUM_TXCNTR_EV_NEW_INST & *Event))
          {
            IPDUM_TX_ADVANCE_READ_BY_ONE(qPtr, TxCntrPtr->NumInstances);
          }
        }
        else
        {
          /* For CQ, already set for LB */
          TS_AtomicSetBit_8(&InstPtr->InstLock, IPDUM_TXCNTR_SEALED_BTIDX);
        }
      }
    }
...
{
  Std_ReturnType retCanTransmit;

  /* !LINKSTO IpduM.SWS_IpduM_00193.LB,1 */
  if(IPDUM_CNTRTX_IS_TXMODE_DIRECT(TxCntrPtr->CfgFlags))
  {
...
#endif
    {
      ...
      PduInfoPtr->SduDataPtr = IPDUM_GET_CNTR_TXDATA(ContainerIdx, 0);
      /* !LINKSTO IpduM.SWS_IpduM_00220,1 */
      retCanTransmit = IpduM_PackLBtoBuff(PduInfoPtr, ContainerIdx, InstPtr->NumCntd, instIdx);

      TS_AtomicSetBit_8(&InstPtr->InstLock, IPDUM_TXCNTR_ASSEMBLED_BTIDX);

      IPDUM_AA_PDULEN(InstPtr->PduLength, PduInfoPtr->SduLength);
    }
#if(IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON)
    else
    {
      /* Deviation MISRAC2012-1 */
      PduInfoPtr->SduDataPtr = IPDUM_GET_CNTR_TXDATA(ContainerIdx, IPDUM_GET_IDX_TX_INST(ContainerIdx, InstPtr));
      retCanTransmit = E_OK;
    }
#endif /* IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON */
  }
...
)
{
  ...
  if(!IPDUM_CNTRTX_IS_LAST_IS_BEST(TxCntrPtr->CfgFlags))
  {
    ...
    pduInfo.SduDataPtr = IPDUM_GET_CNTR_TXDATA(ContainerIdx, (sint32)InstIdx);
...
#endif
  {
    ...
    {
      TS_AtomicSetBit_8(&InstPtr->InstLock, IPDUM_TXCNTR_SEALED_BTIDX);
      pduInfo.SduDataPtr = IPDUM_GET_CNTR_TXDATA(ContainerIdx, 0);
  ...
  if((InstPtr->State == IPDUM_TXCNTR_ST_TRIGGERED) && (E_OK == canTransmit))
  {
    if(isDirect)
    {
...
#if (IPDUM_CANFD_PADDING_SUPPORT == STD_ON)
      /* Check if container has the CANFD padding service enabled */
      if(IPDUM_CNTRTX_HAS_CANFD_PADDING(TxCntrPtr->CfgFlags))
      {
        /* Perform the padding service */
        IpduM_CanFdPaddingService(&pduInfo, TxCntrPtr);
      }
    ...
    if(E_OK == PduR_IpduMTransmit(TxCntrPtr->IpduMContainerTxHandleId, &pduInfo))
    {
      if(0U == TxCntrPtr->IpduMContainerTxConfirmationTimeout)
      {
        ...
        InstPtr->State = IPDUM_TXCNTR_ST_IDLE;
        IpduM_TxAdvanceReadWOVFL(QPtr, ContainerIdx, TxCntrPtr->NumInstances);
    ...
    while(IPDUM_TXCNTR_EV_NONE != event)
    {
      ...
      switch(state)
      {
...
#if (IPDUM_USE_METADATA == STD_ON)
          if( ( (PduInfoPtr->SduLength + (PduLengthType)txCntrPtr->HeaderSize) <= (txCntrPtr->PduLength - buffOffset) ) && (triggerCntrDiffMetaData == FALSE) )
#else
...
#if (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
            IpduM_SetInstMtdtPrio(instPtr, MetaDataValue, IPDUM_CNTND_HAS_METADATA(containedPtr->CfgFlags));
#endif
          }
          else
          {
            triggerCntr = TRUE; /* Either exceeded the size or receive different Metadata Values */
          }
        ...
        case IPDUM_TXCNTR_ST_WAIT_CONF:
        {
          ...
          {
            /* !LINKSTO IpduM.EB.CntrTx.DequeueOrder.OverFlow,2 */
      ...
      case IPDUM_TXCNTR_ST_WAIT_CONF:
      {
        ...
        if(PduInfoPtr->SduLength >= txCntrPtr->PduLength)
        {
          ...
          if(!IPDUM_CNTRTX_IS_LAST_IS_BEST(txCntrPtr->CfgFlags))
          {
            ...
            PduInfoPtr->SduLength = instPtr->PduLength;
            /* !LINKSTO IpduM.SWS_IpduM_00193.CQ,1 */
            TS_MemCpy(PduInfoPtr->SduDataPtr, IPDUM_GET_CNTR_TXDATA(ContainerIdx, (sint32)instIdx), PduInfoPtr->SduLength);
          ...
          if(IPDUM_CNTRTX_IS_TT_NOWAITCONF(txCntrPtr->CfgFlags))
          {
            ...
            /* !LINKSTO IpduM.dsn.Transition.Container.Process.Idle,1 */
            instPtr->State = IPDUM_TXCNTR_ST_IDLE;
            cntrRamData->TxConfTimeout = 0U;
            IpduM_TxAdvanceReadWOVFL(qPtr, ContainerIdx, txCntrPtr->NumInstances);

            if((0U != (instPtr->InstLock & IPDUM_TXCNTR_CNTD_STR_MSK))
            && (qPtr->ReadIdx == qPtr->WriteIdx))
            {
              /* Head instance; Transmission interrupted before new instance creation */
              TS_AtomicSetBit_8(&instPtr->InstLock, IPDUM_TXCNTR_TX_INT_BITIDX);
            }
...
)
{
  ...
  /* Deviation MISRAC2012-1 */
  uint8 instIdx = IPDUM_GET_IDX_TX_INST(ContainerIdx, InstPtr);
  P2VAR(uint8, AUTOMATIC, IPDUM_VAR_CLEARED) dataBuff = (P2VAR(uint8, AUTOMATIC, IPDUM_VAR_CLEARED))(&(IPDUM_GET_CNTR_TXDATA(ContainerIdx, (sint32)instIdx))[BuffOffset]);
  /* Deviation MISRAC2012-1 */
  P2VAR(PduIdType, AUTOMATIC, IPDUM_VAR_CLEARED) txConfList = IPDUM_GET_TX_CONFLIST(ContainerIdx, instIdx);
...
)
{
...
#endif /* IPDUM_STATIC_CONTAINER_PDU_HANDLING_ENABLE == STD_ON || IPDUM_CONTAINEDTXPDU_PRIORITY_HANDLING_ENABLE == STD_ON */
  ...
  *AddedOutPtr = FALSE;
  ...
  if(IPDUM_CNTRTX_HAS_PRIORITY(cntrPtr->CfgFlags))
  {
    ...
    for(idx = numCntd; idx > 0U; idx--)
    {
      /* !LINKSTO IpduM.SWS_IpduM_00249,1 */
...
)
{
  ...
  P2CONST(PduIdType, AUTOMATIC, IPDUM_VAR_CLEARED) txConfList;
  /* Deviation MISRAC2012-1 */
  txConfList = IPDUM_GET_TX_CONFLIST(ContainerIdx, instIdx);

  for(idx = 0U; idx < numCntd; idx++)
  {
  ...
  while(IPDUM_TXCNTR_EV_NONE != event)
  {
    ...
    switch(state)
    {
      ...
      case IPDUM_TXCNTR_ST_WAIT_CONF:
      {
        ...
        /* !LINKSTO IpduM.EB.CntrTx.DequeueOrder.NoOverFlow,2 */
        /* !LINKSTO IpduM.EB.CntrTx.DequeueOrder.OverFlow,2 */
...
{
  uint16 trgIdx = 0xFFFFU;
  uint8 i;

  if(QPtr->WriteIdx > QPtr->ReadIdx)
  {
    for(i = (QPtr->WriteIdx + 1U); i < NumInstances; i++)
    {
      /* Deviation MISRAC2012-1 */
      if(IPDUM_TXCNTR_ST_TRIGGERED == (IPDUM_GET_TX_INST(ContainerIdx, i))->State)
      {
        trgIdx = i;
        break;
      }
  ...
  {
    for(i = (QPtr->WriteIdx + 1U); i < QPtr->ReadIdx; i++)
    {
      /* Deviation MISRAC2012-1 */
      if(IPDUM_TXCNTR_ST_TRIGGERED == (IPDUM_GET_TX_INST(ContainerIdx, i))->State)
      {
    ...
    for(i = (QPtr->ReadIdx + 1U); ((trgIdx == 0xFFFFU) && (i < NumInstances)); i++)
    {
      /* Deviation MISRAC2012-1 */
      if(IPDUM_TXCNTR_ST_TRIGGERED == (IPDUM_GET_TX_INST(ContainerIdx, i))->State)
      {
  ...
  /* Single pass, would be 0->ReadIdx; ReadIdx->WriteIdx */
  for(i = 0U; (trgIdx == 0xFFFFU) && (i < QPtr->WriteIdx); i++)
  {
    /* Deviation MISRAC2012-1 */
    if(IPDUM_TXCNTR_ST_TRIGGERED == (IPDUM_GET_TX_INST(ContainerIdx, i))->State)
    {
...
/* !LINKSTO IpduM.EB.CanFdPadding.Behavior,1 */
IPDUM_STATIC FUNC(void, IPDUM_CODE) IpduM_CanFdPaddingService
(
    P2VAR(PduInfoType, AUTOMATIC, AUTOMATIC) pduInfoPtr,
    P2CONST(IpduM_ContainerTxPduType, AUTOMATIC, IPDUM_APPL_CONST) TxCntrPtr
)
{
  ...
  /* Find the next CanFd DLC */
  interval = IpduM_FindInterval(pduInfoPtr->SduLength);

  /* Check if padding is required */
  if (interval != IPDUM_INVALID_CANFD_INTERVAL)
  {
    /* Set the new length as the minimum between the CanFD DLC and the container max length. */
    PduLengthType newLength = IPDUM_MIN(IpduM_CanfdIntervals[interval].End, TxCntrPtr->PduLength);

    /* Check if header + AA padding is required */
    if((PduLengthType)(newLength - pduInfoPtr->SduLength) > 3U)
    {
      /* Set the 00 header */
      TS_MemSet(&pduInfoPtr->SduDataPtr[pduInfoPtr->SduLength], 0U, 3U);

      /* Update length */
      pduInfoPtr->SduLength = pduInfoPtr->SduLength +3U;

      /* Set the AA padding */
      TS_MemSet(&pduInfoPtr->SduDataPtr[pduInfoPtr->SduLength], IPDUM_CANFD_PADDING_VALUE, (PduLengthType)(newLength - pduInfoPtr->SduLength));

      /* Update length */
      pduInfoPtr->SduLength = newLength;
    }
    else
    {
      /* Set the 00 header */
      TS_MemSet(&pduInfoPtr->SduDataPtr[pduInfoPtr->SduLength], 0U, (PduLengthType)(newLength - pduInfoPtr->SduLength));
      /* Update length */
      pduInfoPtr->SduLength = newLength;
    }
  }
...
IPDUM_STATIC FUNC(uint8, IPDUM_CODE) IpduM_FindInterval (PduLengthType pduLength)
{
  sint8 mid;
  sint8 low = 0U;
  sint8 high = IPDUM_NUMBER_OF_CANFD_INTERVALS - 1u;
  uint8 returnValue = IPDUM_INVALID_CANFD_INTERVAL;

  /* Binary search */
  while (low <= high)
  {
    /* Find the mid element */
    mid = ((low + high) / (sint8)2);
    /* If element is found */
    if ((pduLength >= (PduLengthType)IpduM_CanfdIntervals[mid].Start) && (pduLength < (PduLengthType)IpduM_CanfdIntervals[mid].End))
    {
      returnValue = (uint8)mid;
      break;
    }
    /* Check in first half */
    else if (pduLength < (PduLengthType)IpduM_CanfdIntervals[mid].Start)
    {
      high = (mid - (sint8)1);
    }
...
{
  boolean triggerCond = FALSE;

  if(CntrHasMetadata)
  {
    if(InstPtr->NumCntd == 0U)
    {
      /* Empty Container, set MetaData for that Container by the first transmit request */
      InstPtr->MetaDataValue = ContainedMetaData;
    }
    else
    {
      /* Non Empty Container: check if the new request has the same metadata or not
       * In case of different MetaData (ignoring the priority), Trigger the current Container Instance for transmission
       * and add the new request with new MetaData value to a new Container instance. */
#if(IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)
      if((InstPtr->MetaDataValue & (~IPDUM_METADATA_PRIO_MSK)) != (ContainedMetaData & (~IPDUM_METADATA_PRIO_MSK)))
#else
      if(InstPtr->MetaDataValue != ContainedMetaData)
#endif /* IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON */
      {
        triggerCond = TRUE; /* Trigger the Cntr as different MetaData */
      }
    }
  }
...
)
{
  if(CntdHasMetadata)
  {
    /* !LINKSTO IpduM.EB.MetaData.Tx.Priority.Establish,1 */
#if(IPDUM_USE_METADATA == STD_ON)
    uint32 metaDataVal = InstPtr->MetaDataValue;
    if((metaDataVal & IPDUM_METADATA_PRIO_MSK) > (ContainedMetaData & IPDUM_METADATA_PRIO_MSK))
    {
      InstPtr->MetaDataValue = (metaDataVal & (~IPDUM_METADATA_PRIO_MSK)) | (ContainedMetaData & IPDUM_METADATA_PRIO_MSK);
    }
#else
    uint8 newPrio = (uint8)((ContainedMetaData & IPDUM_METADATA_PRIO_MSK) >> IPDUM_MTDT_PRIO_SHIFT_VAL);
    if(InstPtr->MetaDataPrio > newPrio)
    {
      InstPtr->MetaDataPrio = newPrio;
    }
#endif /* IPDUM_USE_METADATA == STD_ON */
  }
}
...
{
  /* Set MetaData/Priority for Tx Container */
  if(IPDUM_CNTRTX_HAS_METADATA(TxCntrPtr->CfgFlags))
  {
    PduInfoType LocalPduForMetadata = {NULL_PTR, 0U};

#if((IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) && (IPDUM_USE_METADATA == STD_OFF))
    uint32 cntrMetaData;
    (void) EcuC_GetMetaDataCanId(TxCntrPtr->TxContainerMetaDataPduId, &LocalPduForMetadata, &cntrMetaData);

    /* !LINKSTO IpduM.EB.MetaData.Tx.Priority.Transmit,1 */
    cntrMetaData = (cntrMetaData & (~IPDUM_METADATA_PRIO_MSK)) | (uint32)((uint32)InstPtr->MetaDataPrio << IPDUM_MTDT_PRIO_SHIFT_VAL);
    (void) EcuC_SetMetaDataCanId(TxCntrPtr->TxContainerMetaDataPduId, &LocalPduForMetadata, cntrMetaData);
#else
    (void) EcuC_SetMetaDataCanId(TxCntrPtr->TxContainerMetaDataPduId, &LocalPduForMetadata, InstPtr->MetaDataValue);
#endif /* (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON) && (IPDUM_USE_METADATA == STD_OFF) */
  }
}
#endif /* ((IPDUM_USE_METADATA == STD_ON) || (IPDUM_METADATA_TX_PRIO_SUPPORT == STD_ON)) */
