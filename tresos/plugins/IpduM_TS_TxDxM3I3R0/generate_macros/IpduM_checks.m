...
[!SELECT "IpduMConfig/*[1]"!][!//
  [!IF "node:exists(IpduMRxPathway/*)"!][!//
    [!LOOP "IpduMRxPathway/*"!][!//
      [!IF "contains($IpduMRxIndicationNames, concat(' ', as:name(IpduMRxIndication), ' '))"!][!//
         [!INFO!]The SHORT-NAMEs of the IpduMRxIndication are not unique. The SHORT-NAME " [!"as:name(IpduMRxIndication)"!] "  is used multiple times.[!ENDINFO!][!//
      [!ELSE!][!//
        [!VAR "IpduMRxIndicationNames"= "concat($IpduMRxIndicationNames, as:name(IpduMRxIndication), ' ')"!][!//
      [!ENDIF!][!//
    [!ENDLOOP!][!//
  [!ENDIF!][!//
[!ENDSELECT!][!/*
...
<!-- !LINKSTO IpduM.SWS_IpduM_00245,1 -->
*******************************************************************************
...
[!LOOP "util:distinct(node:order(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedTxPdu/* [node:path(node:value(IpduMContainedTxInContainerPduRef)) = string($parentNode)], 'node:IpduMContainedPduOffset'))"!]
...
[!LOOP "as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerTxPdu/* [IpduMContainerHeaderSize = 'IPDUM_HEADERTYPE_NONE']"!][!//
    ...
    [!FOR "i" = "1" TO "$noOfrefCntdsTxOffset - 1"!]
      ...
      [!IF "$Start + $Length > $End"!]
        [!ERROR!][!"as:name(as:ref($NextCntd))"!]: Overlapping with [!"as:name(as:ref($Cntd))"!]. Please check IpduMContainedPduOffset or PduLength for both contained PDUs.[!ENDERROR!]
      [!ENDIF!]
    [!ENDFOR!]
    ...
    [!IF "$Start + $Length > $End"!]
      [!ERROR!][!"as:name(as:ref($Cntd))"!]: ContainedPdu exceeds container's [!"as:name(as:ref(as:path($Container)))"!] PduLength. Please check IpduMContainedPduOffset / PduLength for contained PDU or PduLength for container PDU.[!ENDERROR!]
    [!ENDIF!]
    ...
    [!IF "not(node:exists(IpduMUnusedAreasDefault))"!]
      [!FOR "i" = "1" TO "$noOfrefCntdsTxOffset"!]
        ...
        [!VAR "End" = "as:ref(as:ref(as:path($Container))/IpduMContainerTxPduRef)/PduLength"!]
        [!IF "$BytePos >= $End"!]
          [!ERROR!][!"as:name(as:ref($Cntd))"!]: IpduMPduUpdateBitPosition [Byte: [!"$BytePos"!]] exceeds container's [!"as:name(as:ref(as:path($Container)))"!] PduLength. Please check IpduMPduUpdateBitPosition for contained PDU or PduLength for container PDU.[!ENDERROR!]
        [!ENDIF!]
        ...
        [!FOR "j" = "1" TO "$noOfrefCntdsTxOffset"!]
          ...
          [!IF "$BytePos >= $Start and $BytePos < $End"!]
            [!ERROR!][!"as:name(as:ref($Cntd))"!]: IpduMPduUpdateBitPosition [Byte: [!"$BytePos"!]] overlaps contained PDU [!"as:name(as:ref($Cntd2))"!].[!ENDERROR!]
          [!ENDIF!]
          [!IF "$i != $j and $BitPos = $BitPos2"!]
            [!ERROR!][!"as:name(as:ref($Cntd))"!]: IpduMPduUpdateBitPosition has the same value as the one for [!"as:name(as:ref($Cntd2))"!].[!ENDERROR!]
          [!ENDIF!]
        [!ENDFOR!]
      [!ENDFOR!]


      [!VAR "maxCntdOffset" = "num:i(num:max(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedTxPdu/*/IpduMContainedPduOffset [node:exists(../IpduMContainedTxInContainerPduRef) and as:name(as:ref(../IpduMContainedTxInContainerPduRef)) = as:name(node:current())]))"!]
      [!VAR "maxCntdUpdateBytePos" = "num:i(num:max(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedTxPdu/*/IpduMPduUpdateBitPosition [node:exists(../IpduMContainedTxInContainerPduRef) and as:name(as:ref(../IpduMContainedTxInContainerPduRef)) = as:name(node:current())]) div 8)"!]
      [!IF "$maxCntdUpdateBytePos > $maxCntdOffset"!]
        [!ERROR!][!"as:name(as:ref(as:path($Container)))"!]: Container PDU must not end with a byte used for storing IpduMUpdateBits. It must have a contained PDU at its end.[!ENDERROR!]
      [!ENDIF!]
    [!ENDIF!]
[!ENDLOOP!]
...
[!LOOP "util:distinct(node:order(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedRxPdu/* [node:exists(IpduMContainedRxInContainerPduRef) and node:path(node:value(IpduMContainedRxInContainerPduRef)) = string($parentNode)], 'node:IpduMContainedPduOffset'))"!]
...
[!LOOP "as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerRxPdu/* [IpduMContainerHeaderSize = 'IPDUM_HEADERTYPE_NONE']"!][!//
    ...
    [!IF "num:i(count(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedRxPdu/*/IpduMPduUpdateBitPosition [node:exists(../IpduMContainedRxInContainerPduRef) and as:name(as:ref(../IpduMContainedRxInContainerPduRef)) = as:name(node:current())])) > 0"!]
      [!VAR "maxCntdOffset" = "num:i(num:max(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedRxPdu/*/IpduMContainedPduOffset [node:exists(../IpduMContainedRxInContainerPduRef) and as:name(as:ref(../IpduMContainedRxInContainerPduRef)) = as:name(node:current())]))"!]
      [!VAR "maxCntdUpdateBytePos" = "num:i(num:max(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainedRxPdu/*/IpduMPduUpdateBitPosition [node:exists(../IpduMContainedRxInContainerPduRef) and as:name(as:ref(../IpduMContainedRxInContainerPduRef)) = as:name(node:current())]) div 8)"!]
      [!IF "$maxCntdUpdateBytePos > $maxCntdOffset"!]
        [!ERROR!][!"as:name(as:ref(as:path($Container)))"!]: Container PDU must not end with a byte used for storing IpduMUpdateBits. It must have a contained PDU at its end.[!ENDERROR!]
      [!ENDIF!]
    [!ENDIF!]
