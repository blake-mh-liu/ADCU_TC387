...
[!IF "node:exists(as:modconf('IpduM')[1]/IpduMGeneral/IpduMHeaderByteOrder) and node:value(as:modconf('IpduM')[1]/IpduMGeneral/IpduMHeaderByteOrder) = 'IPDUM_BIG_ENDIAN'"!]
  [!VAR "IHBO" = "'BE'"!]
[!ENDIF!]
[!IF "asc:getEndianness() != string($IHBO)"!]
...
[!LOOP "util:distinct(node:order(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerRxPdu/*[IpduMContainerPduProcessing='IPDUM_PROCESSING_DEFERRED'], 'node:IpduMContainerRxHandleId'))"!]
  ...
  [!ENDIF!]
  [!VAR "rxQLookupStr" = "concat($rxQLookupStr, as:name(.), ':', num:i($qOfs), ':', num:i($instOfs), '|')"!]
  [!CALL "IpduMGetCntrLength", "cntrPath" = "as:path(.)"!]
  [!VAR "rxPduBuffLookupStr" = "concat($rxPduBuffLookupStr, as:name(.), ':', $nInst, ';', $IpduMCntrLen, '|')"!]
[!ENDLOOP!]
...
[!CALL "IpduMGetCntrLength", "cntrPath" = "as:path(util:distinct(node:order(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerRxPdu/*[IpduMContainerPduProcessing='IPDUM_PROCESSING_DEFERRED'], 'node:IpduMContainerRxHandleId'))[last()])"!]
...
[!LOOP "util:distinct(node:order(as:modconf('IpduM')/IpduMConfig/*[1]/IpduMContainerTxPdu/*[node:exists(IpduMContainerTxHandleId)], 'node:IpduMContainerTxHandleId'))"!]
  [!VAR "IpduMTxContainersLst"!][!"$IpduMTxContainersLst"!];[!"as:path(.)"!]:[!"$IpduMCntrIdx"!][!ENDVAR!]
  [!VAR "IpduMCntrIdx" = "num:i($IpduMCntrIdx + 1)"!]
[!ENDLOOP!]
[!LOOP "util:distinct(node:order(as:modconf('IpduM')/IpduMConfig/*[1]/IpduMContainerTxPdu/*[not(node:exists(IpduMContainerTxHandleId))], 'as:name(.)'))"!]
  [!VAR "IpduMTxContainersLst"!][!"$IpduMTxContainersLst"!];[!"as:path(.)"!]:[!"$IpduMCntrIdx"!][!ENDVAR!]
  [!VAR "IpduMCntrIdx" = "num:i($IpduMCntrIdx + 1)"!]
[!ENDLOOP!]
...
[!VAR "prios" = "text:join((../../IpduMContainedTxPdu/*[node:exists(IpduMContainedTxPduPriority) and node:path(node:value(./IpduMContainedTxInContainerPduRef)) = string($Container)]/IpduMContainedTxPduPriority), ':')"!]
...
[!SELECT "as:ref(text:split(string(text:split($IpduMTxContainersLst,';')[position()-1 = $c]),':')[1])"!]
  ...
  [!/* LAST_IS_BEST or HEADERTYPE_NONE */!]
    ...
    [!ENDIF!]
  [!ENDIF!]
  [!VAR "txLookupStr" = "concat($txLookupStr, as:name(.), ':', $qOfs, ':', $ramDataOfs, ':', $instOfs, ':', $txCnfNumElem, ':', num:i($nInst), ':', num:i($nBuffInst), '|')"!]
  [!/* All containers have an entry otherwise can't check what grep returns */!]
  [!VAR "txBuffLookupStr" = "concat($txBuffLookupStr, as:name(.), ':', $pduBuffLen, ':', $nBuffInst, '|')"!]
  [!VAR "txConfListLookupStr" = "concat($txConfListLookupStr, as:name(.), ':', num:i($nInst * $txCnfNumElem), '|')"!]
[!ENDSELECT!]
[!ENDFOR!]
...
[!IF "$valPathNode != ''"!]
  ...
  [!VAR "mVal" = "num:i(ceiling(node:value($shortNameToXPath) div $txTBase))"!]
[!ELSE!]
  [!VAR "mVal" = "num:i(ceiling($directValue div $txTBase))"!]
[!ENDIF!]
[!ENDNOCODE!][!ENDMACRO!]
...
[!LOOP "as:modconf('IpduM')[1]/IpduMGeneral/IpduMRxProcessing/*"!]
  [!VAR "mfRXalloc"!][!"$mfRXalloc"!];[!"num:i(node:pos(.) + 1)"!]#[!"as:name(.)"!]#IpduM_MainFunctionRx_[!"as:name(.)"!][!ENDVAR!]
[!ENDLOOP!]
[!ENDNOCODE!][!ENDMACRO!]
[!/*
****************************************************************************************************
...
[!VAR "mfTxPathsMFGenPWs" = "text:join(text:difference(as:paths(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMTxPathway/*), as:paths(util:distinct(node:refs(as:modconf('IpduM')[1]/IpduMGeneral/IpduMTxProcessing/*/IpduMTxPduRef/*)))),'|')"!]
[!VAR "mfTXalloc"!];[!"num:i(0)"!]# #IpduM_MainFunctionTx# [!"$mfTxPathsMFGenPWs"!][!ENDVAR!]
[!VAR "mfTxAllocTiming" = "concat($mfTxPathsMFGenPWs, '|', text:join(text:difference(as:paths(as:modconf('IpduM')[1]/IpduMConfig/*[1]/IpduMContainerTxPdu/*), as:paths(util:distinct(node:refs(as:modconf('IpduM')[1]/IpduMGeneral/IpduMTxProcessing/*/IpduMTxPduRef/*)))),'|'), ':', as:modconf('IpduM')[1]/IpduMGeneral/IpduMTxTimeBase)"!]
...
[!LOOP "as:modconf('IpduM')[1]/IpduMGeneral/IpduMTxProcessing/*"!]
  [!VAR "mfTXalloc"!][!"$mfTXalloc"!];[!"num:i(node:pos(.) + 1)"!]#[!"as:name(.)"!]#IpduM_MainFunctionTx_[!"as:name(.)"!]# [!"text:replaceAll(text:join(node:filter(./IpduMTxPduRef/*, "text:split(as:path(as:dtos(as:ref(.))), '/')[last()] = 'IpduMTxPathway'"),'|'),'ASPath\:','')"!][!ENDVAR!]
