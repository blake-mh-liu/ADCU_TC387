...
/* patch version check */
#if (FRTP_SW_PATCH_VERSION < 35U)
#error FRTP_SW_PATCH_VERSION wrong (< 35U)
#endif
...
STATIC FUNC_P2VAR(FrTp_ChannelType, AUTOMATIC, FRTP_CODE)
    FrTp_FindTxChannel(VAR(PduIdType,AUTOMATIC) TxPduId, VAR(uint16,AUTOMATIC) localAddress, VAR(uint16,AUTOMATIC) remoteAddress);
#else
STATIC FUNC_P2VAR(FrTp_ChannelType, AUTOMATIC, FRTP_CODE)
...
#endif
{
    ...
    do
    {
        ...
        if(ConnectionIdx == (uint8)TxPduId)
        {
            ...
            {
#if (FRTP_METADATA_HANDLING_ENABLE == STD_ON)
                if ((pChannel->LocalAddress == localAddress) &&
                    (pChannel->RemoteAddress == remoteAddress))
                {
...

        {
            ...
            else
            {
                ...
                if(pConnection->ConnectionLimitConfigIdx < FRTP_NUMBER_CONNECTION_LIMITS)
                {
                    ...
                    const uint8 connectionLimit = pConnectionLimitConfig->Limit;
                ...
                else
                {
...
#if FRTP_METADATA_HANDLING_ENABLE == STD_ON
                    /* Early retrieval of addresses:
                        1. No external API call within exclusive area
                        2. No immediate freeing of channel if retrieval fails */
                    uint16 localAddress = pConnection->LocalAddress;
                    uint16 remoteAddress = pConnection->RemoteAddress;
                    Std_ReturnType ecucRetVal = E_OK;

                    if(FrTp_Con_IsMetaDataLa(pConnection))
                    {
                        ecucRetVal = EcuC_GetMetaDataSourceAddr(pConnection->getMetaDataPduId, NULL_PTR, &localAddress);
                    }

                    if((ecucRetVal == E_OK) && FrTp_Con_IsMetaDataRa(pConnection))
                    {
                        ecucRetVal = EcuC_GetMetaDataTargetAddr(pConnection->getMetaDataPduId, NULL_PTR, &remoteAddress);
                    }
...
#endif
                    {
                        ...
                        /* get pointer to right channel structure */
#if FRTP_METADATA_HANDLING_ENABLE == STD_ON
                        pChannel = FrTp_FindTxChannel(FrTpTxPduId, localAddress, remoteAddress);
#else
                        pChannel = FrTp_FindTxChannel(FrTpTxPduId);
#endif

#if (FRTP_LIMIT_NUMBER_CONNECTIONS == STD_ON) && (FRTP_NUMBER_CONNECTION_LIMITS > 0)
...
#endif /* (FRTP_LIMIT_NUMBER_CONNECTIONS == STD_ON) && (FRTP_NUMBER_CONNECTION_LIMITS > 0)*/
...
#if FRTP_METADATA_HANDLING_ENABLE == STD_ON
                            pChannel->LocalAddress = localAddress;
                            pChannel->RemoteAddress = remoteAddress;
...
FUNC(void,FRTP_CODE) FrTp_MainFunction(void)
{
    ...
    if (FrTp_InitStatus == FRTP_INIT)
    {
        ...
        P2VAR(FrTp_ChannelType, AUTOMATIC, FRTP_VAR) ActiveChannelTable[FRTP_CHAN_NUM] = {0};
...
)
{
    ...
    )
    {
        ...
        for(
            ...
            iBuffer = (iBuffer + 1U) % pBufferInfo->BufferSize
            )
        {
