...
STATIC FUNC(Std_ReturnType, FRTP_CODE) FrTp_TxSm_TxBufferHandling
(
    P2VAR(FrTp_ChannelType,AUTOMATIC,FRTP_VAR) pChannel,
...
)
{
    ...
    /* reset separation cycle settings (immediate transmission, no separation cycles) */
    ...
    /* initialize bandwidth control (disabled in the beginning) */
...
)
{
    ...
    if((pChannel->Tx_CtrTxPdusPending > 0U) && (NewCycle != 0U))
    {
        fMoreDataAvailable = FALSE;
    }
...
)
{
    ...
    if(localTx_State == FRTP_CHNL_TX_DATA)
    {
        ...
        PduRRetVal = PduR_FrTpCopyTxData( pConnection->TxSduIdx,
                                            &TxDestinationInfo,
                                            RetryInfoPtr,
                                            &AvailableData
                                        );
        /* Proceed according to the PduR buffer availability */
        RetVal = FrTp_TxSm_TxBufferHandling(pChannel,
                                            TxPduIdx,
                                            PduRRetVal
                                            );
    }
...
)
{
    ...
    {
        if ((pChannel->Tx_CtrDataAck == 0U) && (bufferPointer == 0U))
        {
            /* no CTS yet received
...
)
{
    ...
    (
        (pChannel->Tx_State != FRTP_CHNL_TX_IDLE) &&
        ((pChannel->Tx_State & FRTP_CHNL_STATE_DIRECTION_RX) == 0U) &&
...
#endif /* (FRTP_FULLDUPLEX_ENABLE == STD_OFF) */
    {
        ...
        (
            FrTp_ActiveConnectionsCnt[pConnectionFinished->ConnectionLimitConfigIdx] > 0U,
