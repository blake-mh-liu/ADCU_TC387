...
#define FRTP_CONNECTION_HASBCLIMIT      0x04U  /* bitmask for 'bandwidth limitation' config switch */
#define FRTP_CONNECTION_HASMETADATALA   0x08U  /* bitmask for 'metadata local address' config switch */
#define FRTP_CONNECTION_HASMETADATARA   0x10U  /* bitmask for 'metadata remote address' config switch */
...
#if FRTP_METADATA_HANDLING_ENABLE == STD_ON
#define FrTp_Con_IsMetaDataRa(pCon) ((pCon->TpOptions & FRTP_CONNECTION_HASMETADATARA) != 0U)
#define FrTp_Con_IsMetaDataLa(pCon) ((pCon->TpOptions & FRTP_CONNECTION_HASMETADATALA) != 0U)
#define FrTp_Con_GetLa(pCon, pChan) (pChan->LocalAddress)
#define FrTp_Con_GetRa(pCon, pChan) (pChan->RemoteAddress)
#define FrTp_Con_IsAddressMatch(pCon, pChan, localAddress, remoteAddress) \
    ((pChan->LocalAddress == localAddress) && (pChan->RemoteAddress == remoteAddress))
#else
#define FrTp_Con_IsMetaDataRa(pCon) FALSE
#define FrTp_Con_IsMetaDataLa(pCon) FALSE
#define FrTp_Con_GetLa(pCon, pChan) (pCon->LocalAddress)
#define FrTp_Con_GetRa(pCon, pChan) (pCon->RemoteAddress)
#define FrTp_Con_IsAddressMatch(pCon, pChan, localAddress, remoteAddress) \
    ((pCon->LocalAddress == localAddress) && (pCon->RemoteAddress == remoteAddress))
#endif /* FRTP_METADATA_HANDLING_ENABLE == STD_ON */
...
#define Rx_CtrRetry         Tx_CtrTxPdusPending
  ...
  #error Rx_State already defined
#endif
#define Rx_State            Rx_State_mem

#ifdef Rx_Timer1
  #error Rx_Timer1 already defined
#endif
#define Rx_Timer1           Rx_Timer1_mem

#ifdef Rx_Timer2
  #error Rx_Timer2 already defined
#endif
#define Rx_Timer2           Rx_Timer2_mem

#ifdef Rx_PCI
  #error Rx_PCI already defined
#endif
#define Rx_PCI              Rx_PCI_mem

#ifdef Rx_ML
  #error Rx_ML already defined
#endif
#define Rx_ML               Rx_ML_mem

#ifdef Rx_CtrDataAck
  #error Rx_CtrDataAck already defined
#endif
#define Rx_CtrDataAck       Rx_CtrDataAck_mem

#ifdef Rx_BfS
  #error Rx_BfS already defined
#endif
#define Rx_BfS              Rx_BfS_mem

#ifdef Rx_CtrBlockReceived
  #error Rx_CtrBlockReceived already defined
#endif
#define Rx_CtrBlockReceived  Rx_CtrBlockReceived_mem

#ifdef Rx_CtrWait
  #error Rx_CtrWait already defined
#endif
#define Rx_CtrWait          Rx_CtrWait_mem

#ifdef Rx_CtrRetry
  #error Rx_CtrRetry already defined
#endif
#define Rx_CtrRetry         Rx_CtrRetry_mem
...
#define FrTp_Header_Offset_BC               5U   /* bandwidth control */
...
{
    /* TX related members */
    VAR(uint16,TYPEDEF) Tx_Timer1;              /**< Tx Timer 1 counter variable (Timer AS) */
    VAR(uint16,TYPEDEF) Tx_Timer2;              /**< Tx Timer 2 counter variable (Timer BS) */
    VAR(uint16,TYPEDEF) Tx_ML;                  /**< Tx message length */
    VAR(uint16,TYPEDEF) Tx_CtrDataAck;          /**< Tx bytes already acknowledged by receiver */
    VAR(uint16,TYPEDEF) Tx_BfS;                 /**< Tx current block size */
    VAR(uint16,TYPEDEF) Tx_BfSPduR;             /**< Tx PduR buffer size */
    VAR(uint16,TYPEDEF) Tx_CtrBlockTransmit;    /**< Tx number of bytes transmitted for current block */
    VAR(uint16,TYPEDEF) Tx_CtrBlockTriggerTransmit;
    VAR(uint16,TYPEDEF) Tx_Retry;               /**< >0 means that channel is in retry state (next frame is a retry) and
                                                      value is byte position received in RETRY frame*/
    VAR(uint16,TYPEDEF) Tx_CtrBufReq;           /**< Counts number of retries to PduR_FrTpCopyTxData
                                                     in case the buffer is temporarily unavailable*/

    /* RX related members */
    VAR(uint16,TYPEDEF) Rx_Timer3;               /**< Rx Timer 1 counter variable (Timer BR) */

    VAR(uint16,TYPEDEF)  Tx_BCMinCycleDistance;  /**< Tx minimum cycle distance
                                                     between 2 burst transmissions */

    VAR(uint16,TYPEDEF)  Tx_BCCtrCycleDistance;  /**< number of cycles for which this channel
                                                     must not transmit */

#if (FRTP_METADATA_HANDLING_ENABLE == STD_ON)
    VAR(uint16, TYPEDEF) LocalAddress; /**< Local address if metadata-handling is enabled */
    VAR(uint16, TYPEDEF) RemoteAddress; /**< Remote address if metadata-handling is enabled */
#endif /* FRTP_METADATA_HANDLING_ENABLE */

#if (FRTP_FULLDUPLEX_ENABLE == STD_ON)
    /* suffix _mem added to prevent MISRA Rule 5.5 warning */
    VAR(uint16,TYPEDEF) Rx_Timer1_mem;               /**< Rx Timer 1 counter variable (Timer AR) */
    VAR(uint16,TYPEDEF) Rx_Timer2_mem;               /**< Rx Timer 1 counter variable (Timer CR) */
    VAR(uint16,TYPEDEF) Rx_ML_mem;                   /**< Rx message length */
    VAR(uint16,TYPEDEF) Rx_CtrDataAck_mem;           /**< Rx number of bytes received
                                                     and to be acknowledged */
    VAR(uint16,TYPEDEF) Rx_BfS_mem;                  /**< Rx current block size */
    VAR(uint16,TYPEDEF) Rx_CtrBlockReceived_mem;     /**< Rx Number of bytes received within the
                                                     current block */
    VAR(uint16,TYPEDEF) Rx_CtrWait_mem;              /**< Rx number of wait FC messages
                                                     already transmitted */
    VAR(uint8,TYPEDEF)  Rx_PCI_mem;                  /**< Rx consecutive frame PCI (CF1 vs. CF2, SN)*/
    VAR(uint8,TYPEDEF)  Rx_CtrRetry_mem;             /**< Rx number of retry messages already
                                                     transmitted for this block */
    VAR(uint8,TYPEDEF)  Rx_State_mem;                /**< Rx State machine state variable */

#endif /* FRTP_FULLDUPLEX_ENABLE */

    /* TX related members */
    VAR(uint8,TYPEDEF)  Tx_State;               /**< Tx State machine state variable */
    VAR(uint8,TYPEDEF)  Tx_PCI;                 /**< Tx consecutive frame PCI (CF1 vs. CF2, SN)*/
    VAR(uint8,TYPEDEF)  Tx_CtrTxPdusPending;    /**< Tx number of Pdus pending for transmission */

    VAR(uint8,TYPEDEF)  Tx_FrIfRetry;            /**< Number of Tx retries to FrIf_Transmit in case
                                                     it returns E_NOT_OK for SF, CF and LF */

    VAR(uint8,TYPEDEF)  TxFc_FrIfRetry;          /**< Number of Tx retries to FrIf_Transmit in case
                                                     it returns E_NOT_OK for FC frames. */

    VAR(uint8,TYPEDEF)  Tx_BCMaxPduPerCycle;    /**< maximum number of Pdus to
                                                    transmit per cycle */
    VAR(uint8,TYPEDEF)  Tx_BCCtrPdu;            /**< number of TxPdus which are
                                                    transmitted for this cycle */

    VAR(uint8,TYPEDEF)  RxHasAck;                /**< This message expects an acknowledge */

    /* common (TX/RX)) members */
    VAR(uint8,TYPEDEF) ConnectionIdx;           /**< index of connection associated
                                                     with this channel */
#if (FRTP_COPY_STF_LOCALBUFFER == STD_ON)
    VAR(uint8,TYPEDEF) STFCopied;                /**< Total bytes of STF frame payload copied */
...
extern VAR(uint8, FRTP_VAR) FrTp_ActiveConnectionsCnt[1U];
