...
)
{
    ...
    /* CHECK: PARSE */
    {
        ...
        uint16 RemoteAddress = FrTp_Con_GetRa(pConnection, pChannel);
        uint16 LocalAddress = FrTp_Con_GetLa(pConnection, pChannel);

        /* set address information into tx-buffer */
        FrTp_SetAI(pPduInfo->SduDataPtr, RemoteAddress, LocalAddress);
...
)
{
    ...
    {
        /* FPL is consistent */
        PduLengthType Bfs = 0U;  /* variable stores the buffer size */

        /* indicate the start of a  new reception (and obtain the existing buffer size */
        BufReq_ReturnType RetCode = BUFREQ_E_NOT_OK;

        if (stf_retransmission_flag == FALSE)
        {

#if FRTP_METADATA_HANDLING_ENABLE == STD_ON
            Std_ReturnType EcuCRetCode = E_OK;
            if(FrTp_Con_IsMetaDataRa(pConnection))
            {
                PduInfoType PduInfo;
                EcuCRetCode = EcuC_SetMetaDataSourceAddr(pConnection->setMetaDataPduId, &PduInfo, FrTp_GetAI_Source(pData));
            }
            if((EcuCRetCode == E_OK) && (FrTp_Con_IsMetaDataLa(pConnection)))
            {
                PduInfoType PduInfo;
                EcuCRetCode = EcuC_SetMetaDataTargetAddr(pConnection->setMetaDataPduId, &PduInfo, FrTp_GetAI_Target(pData));
            }
        ...
        else
        {
            ...
            FRTP_UL_CALLOUT_RX_INDICATION(pConnection->RxSduIdx, FrTp_Con_GetLa(pConnection, pChannel), FrTp_Con_GetRa(pConnection, pChannel), FRTP_PCITYPE_STFU, &pduInfo);
    ...
    if(FrTp_Chnl_GetRxState(RxState) == FRTP_CHNL_RX_DATA)
    {
          ...
          )
        {
            ...
            else
            {
                ...
                /* copy the payload into the receive buffer */
                /* an error should not happen here, since the buffer was previously guaranteed */
                FRTP_UL_CALLOUT_RX_INDICATION(pConnection->RxSduIdx, FrTp_Con_GetLa(pConnection, pChannel), FrTp_Con_GetRa(pConnection, pChannel), FRTP_PCITYPE_CF1, &PduInfo);
                retval = PduR_FrTpCopyRxData(pConnection->RxSduIdx, &PduInfo, &Bfs);
...
)
{
    ...
    if(FrTp_Chnl_GetRxState(RxState) == FRTP_CHNL_RX_DATA)
    {
        ...
        else
        {
            ...
            /* copy the payload into the receive buffer */
            /* an error should not happen here, since the buffer was previously guaranteed */
            FRTP_UL_CALLOUT_RX_INDICATION(pConnection->RxSduIdx, FrTp_Con_GetLa(pConnection, pChannel), FrTp_Con_GetRa(pConnection, pChannel), FRTP_PCITYPE_LF, &PduInfo);
