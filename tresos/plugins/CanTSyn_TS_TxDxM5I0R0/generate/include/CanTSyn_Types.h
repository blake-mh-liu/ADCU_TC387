...
/* !LINKSTO CanTSyn.SWS_CanTSyn_00092,1 */
...
/* !LINKSTO CanTSyn.SWS_CanTSyn_00091,1 */
...
[!IF "$isIcvGenerationSupported = 'true'"!]
  CONST(uint32, TYPEDEF) IcvDataStartIndex; /* Used to find the starting point for this time domain inside the CanTSyn_IcvData array */
  CONST(uint32, TYPEDEF) IcvGenerationJobId; /* Identifier of the Csm job */
[!IF "$isFvUsedInIcvGeneration = 'true'"!]
  CONST(uint32, TYPEDEF) IcvGenerationFvLength; /* Length of freshness value */
  CONST(uint32, TYPEDEF) IcvGenerationFvTruncLength; /* Truncated length of freshness value */
[!ENDIF!]
  CONST(uint32, TYPEDEF) IcvDataLength; /* Stores the length of the data used for computing the ICV */
[!ENDIF!]
...
[!IF "$isIcvGenerationSupported = 'true'"!]
  CONST(uint16, TYPEDEF) ComputedIcvStartIndex; /* Used to index the computed Icv global array */
  CONST(uint16, TYPEDEF) IcvGenerationTimeout; /* Timeout of ICV generation (respective CSM job completion in asynchronous behaviour). */
[!IF "$isFvUsedInIcvGeneration = 'true'"!]
  CONST(uint16, TYPEDEF) IcvGenerationFvId; /* Identifier of the freshness value */
  CONST(boolean, TYPEDEF) IsFvUsed; /* Flag to determine whether FV is used in ICV generation or not */
[!ENDIF!]
  CONST(uint8, TYPEDEF) IcvGenerationBase; /* Configuration parameter to select Mac or Signature crypthography for the ICV generation. */
  CONST(uint8,TYPEDEF) IcvTxLength; /* Length of ICV to be transmitted within Follow_Up Message on the bus (in bytes). */
[!ENDIF!]
...
[!ENDIF!]
  CONST(uint8, TYPEDEF) IcvGenerationSupport; /* Configuration parameter that controls whether or not ICV generation shall be supported. */
[!IF "$isIcvGenerationSupported = 'true'"!]
  CONST(boolean, TYPEDEF) IcvGenerationCsmProcessing; /* Configuration parameter to determine if Csm job is in synchronous or asynchronous behavoiur. */
[!ENDIF!]
...
[!IF "$isIcvVerificationSupported = 'true'"!]
  CONST(uint32, TYPEDEF) IcvDataStartIndex; /* Used to find the starting point for this time domain inside the CanTSyn_IcvData array */
  CONST(uint32, TYPEDEF) IcvVerificationJobId; /* Identifier of the Csm job */
[!IF "$isFvUsedInIcvVerification = 'true'"!]
  CONST(uint32, TYPEDEF) IcvVerificationFvLength; /* Length of freshness value */
  CONST(uint32, TYPEDEF) IcvVerificationFvTruncLength; /* Truncated length of freshness value */
[!ENDIF!]
  CONST(uint32, TYPEDEF) IcvDataLength; /* Stores the length of the data used for computing the ICV */
[!ENDIF!]
[!IF "as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DEM'"!]
  CONST(Dem_EventIdType, TYPEDEF) EventIdSyncFailed;      /**< Event Id to be passed to Dem */
[!ENDIF!]
[!IF "not(as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DISABLE')"!]
  CONST(uint16, TYPEDEF) ReceiveSyncPairTimeout;
[!ENDIF!]
  CONST(uint16, TYPEDEF) RxDebounceTime; /* This represents the configuration of a RX debounce time for the Sync and FUP, OFS and OFNS. */
  CONST(uint16, TYPEDEF) RxFollowUpOffsetTimeout; /* Time offset between SYNC and related FUP messages in MainFunction invocations */
  CONST(uint16, TYPEDEF) SyncTimeBaseId; /* StbMSynchronizedTimeBaseIdentifier needed for StbM */
[!IF "$isIcvVerificationSupported = 'true'"!]
  CONST(uint16, TYPEDEF) ComputedIcvStartIndex; /* Used to index the computed Icv global array */
  CONST(uint16, TYPEDEF) IcvVerificationTimeout; /* Timeout of ICV verification (respective CSM job completion in asynchronous behaviour). */
  CONST(uint16, TYPEDEF) IcvVerificationAttempts; /* Number of ICV verification attempts that are to be carried out when the verification of the ICV failed */
[!IF "$isFvUsedInIcvVerification = 'true'"!]  
  CONST(uint16, TYPEDEF) IcvVerificationFvId; /* Identifier of the freshness value */
  CONST(boolean, TYPEDEF) IsFvUsed; /* Flag to determine whether FV is used in ICV verification or not */
[!ENDIF!]  
  CONST(uint8, TYPEDEF) IcvRxLength; /* Length of ICV to be used for verification of received ICV within FUP Message in Bytes. */
  CONST(uint8, TYPEDEF) IcvVerificationBase; /* Configuration parameter to select Mac or Signature crypthography for the ICV verification. */
[!ENDIF!]
  CONST(uint8, TYPEDEF) TimeDomainId; /* Global Time Domain Handle Id */
  CONST(uint8, TYPEDEF) RxCrcValidated; /* Configuration parameter that activates/deactivates the CRC support */
  CONST(uint8, TYPEDEF) TimeScHysteresis; /* number of consecutive valid message pairs until a Time Tuple is forwarded to the StbM */
  CONST(uint8, TYPEDEF) ScJumpWidth; /* Maximum allowed gap between two SYNC/ two OFS messages */
  CONST(uint8, TYPEDEF) HysteresisIndex; /* Hysteresis array index */
...
[!IF "$isSlaveTimeValidationSupported = 'true'"!]
  ...
  CONST(uint8, TYPEDEF) NetworkSegmentId; /* Global Time Network Segment ID */
[!ENDIF!]
  CONST(uint8, TYPEDEF) IcvVerificationType; /* Configuration parameter that controls whether or not ICV verification shall be supported */
[!IF "$isIcvVerificationSupported = 'true'"!]
  CONST(uint8, TYPEDEF) ExpectedSecurityPayloadLength; /* FV + ICV */
  CONST(boolean, TYPEDEF) IcvVerificationCsmProcessing; /* Configuration parameter to determine if Csm job is in synchronous or asynchronous behavoiur. */
[!ENDIF!]
...
[!IF "(node:exists(as:modconf('CanTSyn')[1]/CanTSynGeneral/CanTSynMaxNumberOfTransmitRetries)) and (node:value(as:modconf('CanTSyn')[1]/CanTSynGeneral/CanTSynMaxNumberOfTransmitRetries) > '0')"!]
  VAR(uint8, TYPEDEF) SyncTransRetriesCounter; /* value indicating the number of transmition retries */
[!ENDIF!]
[!IF "$isIcvGenerationSupported = 'true'"!]
  VAR(uint8, TYPEDEF) IcvIndex; /* Index inside CanTSyn_IcvMaster array. */
[!ENDIF!]
...
[!IF "(node:exists(as:modconf('CanTSyn')[1]/CanTSynGeneral/CanTSynMaxNumberOfTransmitRetries)) and (node:value(as:modconf('CanTSyn')[1]/CanTSynGeneral/CanTSynMaxNumberOfTransmitRetries) > '0')"!]
  VAR(uint8, TYPEDEF) OfsTransRetriesCounter; /* value indicating the number of transmition retries */
[!ENDIF!]
[!IF "$isIcvGenerationSupported = 'true'"!]
  VAR(uint8, TYPEDEF) IcvIndex; /* Index inside CanTSyn_IcvMaster array. */
[!ENDIF!]
...
typedef struct
{
  ...
  VAR(uint16, TYPEDEF) RxDebounceTime; /* Time to wait between any two incoming messages. */
  VAR(uint16, TYPEDEF) TimeRxFollowUpOffset; /* Time between the SYNC and the related FUP or between OFS and OFNS */
[!IF "not(as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DISABLE')"!]
  VAR(uint16, TYPEDEF) SyncPairTimer; /* Timer for unreceived Sync frame pairs */
[!ENDIF!]
...
[!IF "as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DET'"!]
  VAR(uint8, TYPEDEF) SyncPairsDroppedCnt; /* Dropped Sync frame pairs */
[!ENDIF!]
[!IF "$isIcvVerificationSupported = 'true'"!]
  VAR(uint8, TYPEDEF) IcvIndex; /* Index inside CanTSyn_IcvSlave array. */
[!ENDIF!]
}CanTSyn_SyncSlaveType;


/** \brief Defines the type of CanTSyn slave shared variables used for the OFS/OFNS messages. */
typedef struct
{
...
[!IF "not(as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DISABLE')"!]
  VAR(uint16, TYPEDEF) OfsPairTimer; /* Timer for unreceived Ofs frame pairs */
[!ENDIF!]
  VAR(uint8, TYPEDEF) SeqCount; /* The sequence counter for the Slave */
  VAR(uint8, TYPEDEF) RxStatus; /* Status of message reception of the Slave */
...
[!IF "as:modconf('CanTSyn')[1]/CanTSynReportError/CanTSynSyncFailedReportError = 'DET'"!]
  VAR(uint8, TYPEDEF) OfsPairsDroppedCnt; /* Dropped Ofs frame pairs */
[!ENDIF!]
[!IF "$isIcvVerificationSupported = 'true'"!]
  VAR(uint8, TYPEDEF) IcvIndex; /* Index inside CanTSyn_IcvSlave array. */
[!ENDIF!]
