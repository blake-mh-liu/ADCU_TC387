...
#if ((XCP_BSW_DISTRIBUTION_ENABLED == STD_ON) && ((XCP_BSW_DISTRIBUTION_MEMORY_ACCESS_AREAS == STD_ON) || (XCP_REMOTE_DESTINATIONS == STD_ON)))
#include <Rte_Type.h>
#endif
...
#endif /* XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON */

#if (defined XCP_MEMORY_REQUEST_READ)
#error XCP_MEMORY_REQUEST_READ already defined
#endif
/** \brief Value for Xcp_MemoryRequestType. Used for upload commands and modify bits. */
#define XCP_MEMORY_REQUEST_READ 0U

#if (defined XCP_MEMORY_REQUEST_WRITE)
#error XCP_MEMORY_REQUEST_WRITE already defined
#endif
/** \brief Value for Xcp_MemoryRequestType. Used for download commands and modify bits. */
#define XCP_MEMORY_REQUEST_WRITE 1U

#if (defined XCP_MEMORY_REQUEST_CRC)
#error XCP_MEMORY_REQUEST_CRC already defined
#endif
/** \brief Value for Xcp_MemoryRequestType. Used for the BUILD_CHECKSUM command. */
#define XCP_MEMORY_REQUEST_CRC 2U

#if (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON)
typedef uint32 Xcp_MemoryRequestSizeType;
#else
typedef uint8 Xcp_MemoryRequestSizeType;
#endif
...
#endif
/** \brief  The maximum between XCP_MAX_PROGRAM_BYTES, XCP_MAX_DOWNLOAD_MEM_SIZE and 255
            in case slave block mode support is enabled */
#if (XCP_MAX_PROGRAM_BYTES > XCP_MAX_DOWNLOAD_MEM_SIZE)
  #if (XCP_SLAVE_BLOCK_MODE == XCP_SLAVE_BLOCK_MODE_MASK)
    #define XCP_MAX_MEMORY_WRITE_BYTES 255U
  #else
...
#if (defined XCP_FLX_BUF_PACKET_CMD_BIT)
#error XCP_FLX_BUF_PACKET_CMD_BIT already defined
#endif
/** \brief Gets the bit for the CMD bit in the XCP_PACKET
  */
#define XCP_FLX_BUF_PACKET_CMD_BIT                       0U
...
/** \brief Gets the mask for the CMD bit in the XCP_PACKET
  */
#define XCP_FLX_BUF_PACKET_CMD_MASK                      (1U << XCP_FLX_BUF_PACKET_CMD_BIT)
...
#if (defined XCP_FLX_BUF_PACKET_RES_ERR_BIT)
#error XCP_FLX_BUF_PACKET_RES_ERR_BIT already defined
#endif
/** \brief Gets the bit for the RES_ERR bit in the XCP_PACKET
  */
#define XCP_FLX_BUF_PACKET_RES_ERR_BIT                   2U
...
/** \brief Gets the mask for the RES_ERR bit in the XCP_PACKET
  */
#define XCP_FLX_BUF_PACKET_RES_ERR_MASK                  (1U << XCP_FLX_BUF_PACKET_RES_ERR_BIT)
...
#if (defined XCP_BUFFER_DIRECTION_TX_BIT)
#error XCP_BUFFER_DIRECTION_TX_BIT already defined
#endif
/** \brief Bit set means that the buffer direction is Tx, else Rx
  */
#define XCP_BUFFER_DIRECTION_TX_BIT                      0U
...
/** \brief Bit set means that the buffer direction is Tx, else Rx
  */
#define XCP_BUFFER_DIRECTION_TX_MASK                     (1U << XCP_BUFFER_DIRECTION_TX_BIT)

#if (defined XCP_BUFFER_PRECONFIGURED_MASK_BIT)
#error XCP_BUFFER_PRECONFIGURED_MASK_BIT already defined
#endif
/** \brief Bit set means that the buffer is preconfigured
  */
#define XCP_BUFFER_PRECONFIGURED_MASK_BIT                1U

#if (defined XCP_BUFFER_PRECONFIGURED_MASK)
#error XCP_BUFFER_PRECONFIGURED_MASK already defined
#endif
/** \brief Mask set means that the buffer is preconfigured
  */
#define XCP_BUFFER_PRECONFIGURED_MASK                    (1U << XCP_BUFFER_PRECONFIGURED_MASK_BIT)

#if (defined XCP_BUFFER_FIXED_BIT)
#error XCP_BUFFER_FIXED_BIT already defined
#endif
/** \brief Bit set means that the buffer is fixed and cannot be changed at runtime
  */
#define XCP_BUFFER_FIXED_BIT                            2U
...
/** \brief Bit set means that the buffer is fixed and cannot be changed at runtime
  */
#define XCP_BUFFER_FIXED_MASK                            (1U << XCP_BUFFER_FIXED_BIT)
...
#endif /* #if (XCP_DET_RUNTIME_CHECKS == STD_ON) */
...
#define XCP_GET_FLAG_BYTE_INDEX(FlagId) ((uint8)((FlagId) % 8U))
...
#if (defined XCP_IS_FLAG_MASK_SET)
#error XCP_IS_FLAG_MASK_SET already defined
#endif
...
#define XCP_IS_FLAG_MASK_SET(FlagMask, BitArray, BitArrayType) \
  ((((BitArray) & (BitArrayType)(FlagMask)) != 0U ) ? TRUE : FALSE)

#define XCP_IS_FLAG_BIT_SET(FlagId, BitArray, BitArrayType) \
  ((((BitArray) & (BitArrayType)(XCP_GET_FLAG_BYTE_MASK((FlagId)))) >> XCP_GET_FLAG_BYTE_INDEX(FlagId)))
...
#define SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS() Xcp_SchMExclusiveAreas[GetCoreID()].enterCriticalSection()
...
#define SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS() Xcp_SchMExclusiveAreas[GetCoreID()].exitCriticalSection()
#endif
...
typedef struct
{
  ...
  Xcp_DaqPrioQueueIndexType NoOfEntries;
...
typedef struct
{
  ...
  Xcp_DescriptorCalloutType Callout;
#endif
}Xcp_MemoryAreaType;
...
{
  uint16 WriteIdx;
  uint16 NoOfEntries[XCP_MAX_EVENT_CHANNEL];
...
typedef struct
{
  XcpSchMExclusiveAreaFuncPtr enterCriticalSection;
  XcpSchMExclusiveAreaFuncPtr exitCriticalSection;
...
/** \brief Data type used by the calibration memory access functions.
 ** The possible values are XCP_MEMORY_REQUEST_READ, XCP_MEMORY_REQUEST_WRITE and XCP_MEMORY_REQUEST_CRC.
 */
typedef uint8 Xcp_MemoryRequestType;
 ...
 ** inactive DAQ Lists so that the transmission of the DTOs
 ** is stopped.
 **/
 ...
 **
 **/
extern FUNC(uint8, XCP_CODE) Xcp_CalculateCRC
(
...
#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
extern FUNC(void, XCP_CODE) Xcp_CalculateCRCAsync(void);
#endif
 ...
 **
 ** The copy of the remote address is done with/without resetting the port value.
 **
 ...
 ** \param[in] DTOMirrorWritePos  The position from Mirror where the data will be written/read
 ** \param[in] AppId  The ApplicationId where the memory access shall be done
...
/** \brief Performs a memory access in a bsw distribution context.
 **
 ** Depending on the configuration, the memory access can either be a direct memory access or
 ** a RAM callout function.
 **
 ** This function is designed to be called either from the same context as the one from which the
 ** calibration command originated or the memory proxy.
 **
 ** In case a RAM callout function is used the return value of the function is ignored as, by design,
 ** the Xcp_ApplReadDataFromRAMFinished or Xcp_ApplWriteDataToRAMFinished callbacks are required
 ** to indicate the end of the memory access.
 **
 ** \param[inout] memory Pointer to data that will either be read or written
 ** \param[in] size The number of bytes to be processed
 ** \param[in] requestType Type of request, XCP_MEMORY_REQUEST_READ or XCP_MEMORY_REQUEST_WRITE
 ** \param[in] memoryAreaIdx the index of the memory access area corresponding to the requested data
 ** \param[in] SessionState the value of Xcp_MemoryProxyState
 ** \return XCP_E_OK if the request completed, XCP_E_BUSY if the request is in progress or XCP_E_NOT_OK
            if the request failed
 */
extern FUNC(Xcp_ErrorType, XCP_CODE) Xcp_BswDistributionMemoryAccess
(
...
/** \brief Verifies if the receive processor has any pending data.
 **  */
extern FUNC(boolean, XCP_CODE) Xcp_IsRxBufferFull(void);
 ...
 **/
extern VAR(uint8, XCP_VAR_CLEARED) Xcp_MemoryProxyErrorPending;

/** \brief Flags signaling if a Cyclic Event has been set while the response from
 ** Memory Proxy was not received
 **/
extern VAR(uint8, XCP_VAR_CLEARED) Xcp_CyclicEventOverloaded;
#endif
 ...
 ** */
extern VAR(Xcp_RequestToMemProxyType, XCP_VAR_CLEARED) Xcp_RequestBuffer[XCP_MAX_SIZE_OF_REQUEST_BUFFER];
/** \brief Variable used for holding the status of request for every event
 ** */
extern VAR(Xcp_RequestForEventType, XCP_VAR_CLEARED) Xcp_RequestForEvent[XCP_MAX_EVENT_CHANNEL];
/** \brief Variable used for holding information about the Daq Prio information of the
 ** requested event
 ** */
extern VAR(Xcp_DaqPrioQueueOfRequestType, XCP_VAR_CLEARED) Xcp_DaqPrioQueueOfRequest;
#endif

#if (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON)
/** \brief Variable holding data used for processing of the RequestBuffer
 ** */
extern VAR(Xcp_RequestToMemProxyStructType, XCP_VAR_CLEARED) Xcp_RequestToMemProxy;
#endif

#define XCP_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include <Xcp_MemMap.h>

#define XCP_START_SEC_VAR_CLEARED_ALL_CORES_ACCESS_UNSPECIFIED
#include <Xcp_MemMap.h>
...
#define XCP_STOP_SEC_VAR_CLEARED_ALL_CORES_ACCESS_UNSPECIFIED
#include <Xcp_MemMap.h>
