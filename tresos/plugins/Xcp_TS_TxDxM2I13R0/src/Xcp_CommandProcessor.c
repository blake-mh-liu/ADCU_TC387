 ...
 *
 * Reason:
 * The RAM optimization feature requires packing ODT entries in the Xcp_DaqLists.Xcp_DynamicArea. In order to get the address
 * of the first ODT entry a memory reinterpretation is required as Xcp_DaqLists.Xcp_DynamicArea is an uint8 array.
 * This is safe because:
 * - Xcp_AllDaqListsType (underlying type of Xcp_DaqLists) contains a dummy 4 byte DummyVar which ensures that the linker will
 *   always map this variable to a 4 byte aligned address.
 * - Both Xcp_DaqType and Xcp_OdtType contain a pointer so their size will always be a multiple of 4
 */
 ...
 *
 * Reason:
 * This is due to specific module configurations that lead to different preprocessor expansions.
 * The true branch is covered by different module configurations.
 *
 ...
 * Reason:
 * This is due to specific module configurations that lead to different preprocessor expansions.
 * The false branch is covered by different module configurations.
 *
 */
...
#if ((XCP_USER_TIMESTAMP_SUPPORTED == STD_OFF) && (XCP_BSW_DISTRIBUTION_MEMORY_ACCESS_AREAS == STD_OFF))
#include <Os.h>                  /* Os counter is required */
#endif
...
#endif
/** \brief GET_SEGMENT_INFO command response maximum length */
...
/* If a CRC user callout function is used then the callout return type will be stored in the internal
 * buffer used for Xcp_ProcessMemoryRequest. By convention, the callout return type will always be
 * written on the first position of the buffer.
 */
#if (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON)
#ifdef XCP_CRC_RESPONSE_TYPE_INDEX
#error XCP_CRC_RESPONSE_TYPE_INDEX already defined
#endif
#ifdef XCP_CRC_RESPONSE_INDEX
#error XCP_CRC_RESPONSE_INDEX already defined
#endif
#if (XCP_BUILD_CHECKSUM_TYPE == XCP_CRC_USER_CALLOUT)
/** \brief The index of the CRC response type in the internal buffer used for Xcp_ProcessMemoryRequest */
#define XCP_CRC_RESPONSE_TYPE_INDEX 1U
#else
#define XCP_CRC_RESPONSE_TYPE_INDEX 0U
#endif
/** \brief The index of the CRC response in the internal buffer used for Xcp_ProcessMemoryRequest */
#define XCP_CRC_RESPONSE_INDEX (XCP_CRC_RESPONSE_TYPE_INDEX + 1U)
#endif
...
typedef struct
{
  ...
  boolean Pending;                          /* Flag indicating that the current write operation is
                                               pending due to RAM callouts returning XCP_APPL_BUSY */
} Xcp_MemoryWriteType;
...
typedef struct {
  ...
  Xcp_ErrorType Error; /* store the memory request status */
  Xcp_CTOType Cto; /* CTO object used to store the error response */
...
/** \brief Generic function to get the relevant pointer to identification data.
 **
 ** \param[in] IdentificationType - identification type from the GET_ID command
 ** \param[out] LengthPtr - length of the provided identification information
 **
 ** \return Pointer to identification data.
 */
STATIC FUNC_P2CONST(uint8, AUTOMATIC, XCP_CODE) Xcp_GetIdentificationData
(
  uint8 IdentificationType,
  P2VAR(uint8, AUTOMATIC, XCP_APPL_DATA) LengthPtr
);

/** \brief Function to reset the variables used for various calibration commands.
 **
 ...
 **  */
STATIC FUNC(void, XCP_CODE) Xcp_ResetCalibrationHelperVars(void);
...
/** \brief Helper function that is used by Xcp_ProcessCmdStartStopDAQList.
**
** \param[out] ResponseCTO Pointer to the response object
** \param[in] DaqMode The requested mode for the DAQ list
** \param[in] DaqListNumber The DAQ list identifier
**/
STATIC FUNC(void, XCP_CODE) Xcp_ProcessStartStopDaqList
(
  P2VAR(Xcp_CTOType, AUTOMATIC, AUTOMATIC) ResponseCTO,
  uint8 DaqMode,
  uint16 DaqListNumber
);

/** \brief Helper function used by commands when they require
**  to stop a DAQ list
**
** \param[in] DaqListNumber DAQ list identifier
**/
STATIC FUNC(void, XCP_CODE) Xcp_StopDaqList(uint16 DaqListNumber);
/** \brief Helper function used by commands when they require
**  to start a DAQ list
**
** \param[in] DaqListNumber DAQ list identifier
**/
STATIC FUNC(void, XCP_CODE) Xcp_StartDaqList(uint16 DaqListNumber);
 ...
 **
 ** \param[in] OdtEntryPtr     Address of first ODT entry to be cleared
 ** \param[in] NoOfODTEntries  Number of ODT entries that need to be cleared
 */
...
STATIC FUNC(void, XCP_CODE) Xcp_SendCmdPendingEventCto(Xcp_ErrorType ret);
     ...
     (XCP_RESOURCE_STIM == XCP_RESOURCE_STIM_MASK))
/**
 ** \brief This function is used to reset Xcp_DAQPointer to the default value.
 **  */
STATIC FUNC(void, XCP_CODE) Xcp_ResetDAQPointer(void);
#endif
...
/* This command is available only for static DAQ lists */
  ...
  /* 0xFA - GET_ID */
  {
    ...
    2U,                                   /* Base Length */
    XCP_CMD_GETID_RES_LENGTH_MAX          /* Command maximum response length */
  },
...
STATIC VAR(uint32, XCP_VAR_CLEARED) Xcp_ModifyBitsValue;
...
STATIC VAR(boolean, XCP_VAR) Xcp_UploadInternalCopyFinished = FALSE;

#if (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON)
STATIC VAR(boolean, XCP_VAR) Xcp_CRCMemoryRequestFinished = FALSE;
#endif

#if (XCP_MODIFY_BITS_SUPPORTED == STD_ON)
STATIC VAR(boolean, XCP_VAR) Xcp_ModifyBitsReadFinished = FALSE;
#endif
...
#if ((XCP_GET_ID_TYPE1_CALLOUT_ENABLED == STD_ON) || (XCP_GET_ID_GENERIC_TYPE_CALLOUT_ENABLED == STD_ON))
/** \brief Buffer where the Identification information retrieved from user callout
 *  is stored. */
/* Deviation MISRAC2012-5 <START> */
STATIC VAR(uint8, XCP_VAR) Xcp_GetIdBuffer[XCP_GET_ID_MAX_LENGTH];
/* Deviation MISRAC2012-5 <STOP> */
#endif
...
STATIC VAR(Xcp_FlxBufRuntimeType, XCP_VAR_CLEARED) Xcp_FlxDaqList[XCP_NO_OF_FLX_BUF_CONFIGURED];
...
#include <Xcp_MemMap.h>
     ...
     ((XCP_BSW_DISTRIBUTION_ENABLED == STD_ON) && (XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON) && (XCP_NUMBER_OF_MEMORY_AREAS == 0)) || \
     (XCP_BSW_DISTRIBUTION_ENABLED == STD_ON && XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_OFF)) \
     || \
     (XCP_BSW_DISTRIBUTION_CALIBRATION_MEMORY_ACCESS_AREAS == STD_ON) \
    )
STATIC FUNC(Xcp_ApplReturnType, XCP_CODE) Xcp_DirectMemoryAccess(
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  P2VAR(uint16, AUTOMATIC, XCP_VAR) outputSize,
  Xcp_MemoryRequestSizeType size,
  Xcp_MemoryRequestType requestType
)
{
  Xcp_ApplReturnType applRet = XCP_APPL_OK;
  if(requestType == XCP_MEMORY_REQUEST_READ)
  {
    TS_MemCpy(memory, XCP_APPL_GET_ADDRESS( 0U, (P2VAR(void, AUTOMATIC, XCP_APPL_DATA)) Xcp_MTA.Address.Write ), size);
    *outputSize = (uint16) size;
  }
  else if (requestType == XCP_MEMORY_REQUEST_WRITE)
  {
    TS_MemCpy(XCP_APPL_GET_ADDRESS(0U, Xcp_MTA.Address.Write), memory, size);
    *outputSize = (uint16) size;
  }
  else
  {
#if (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON)
    uint8* addr = (uint8*) XCP_APPL_GET_ADDRESS(0U, (P2VAR(void, AUTOMATIC, XCP_APPL_DATA)) Xcp_MTA.Address.Write);
    applRet = Xcp_CalculateCRC(addr, (uint32) size, &memory[XCP_CRC_RESPONSE_INDEX], &memory[XCP_CRC_RESPONSE_TYPE_INDEX]);
#if (XCP_BUILD_CHECKSUM_TYPE == XCP_CRC_USER_CALLOUT)
    memory[0] = applRet;
    *outputSize = (uint16) 6U;
#else
    *outputSize = (uint16) 5U;
#endif
#else
    /* Nothing to do */
#endif
  ...
  return applRet;
     ...
     (XCP_BSW_DISTRIBUTION_ENABLED == STD_ON && XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_OFF)) \
     && \
     ((XCP_WRITE_READ_MEMORY_RAM == STD_ON) || (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON)) \
    )
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_HandleMemoryRequestResponse
(
  P2VAR(Xcp_CTOType, AUTOMATIC, AUTOMATIC) ResponseCTO,
  Xcp_ApplReturnType response
)
{
  Xcp_ErrorType RetValue = XCP_E_NOT_OK;
  switch(response)
  {
...
STATIC FUNC(Xcp_ApplReturnType,XCP_CODE) Xcp_TriggerRAMCallout
(
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  uint8 size,
  Xcp_MemoryRequestType requestType,
  Xcp_RAMCalloutType callout,
  uint16 SessionState
)
{
  Xcp_ApplReturnType calloutResponse;
#if (XCP_BSW_DISTRIBUTION_ENABLED == STD_OFF)
  TS_PARAM_UNUSED(SessionState);
#endif
  if(requestType == XCP_MEMORY_REQUEST_READ)
  {
    calloutResponse = callout(
                         memory,
...
FUNC(Xcp_ErrorType, XCP_CODE) Xcp_BswDistributionMemoryAccess
(
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  P2VAR(uint16, AUTOMATIC, XCP_VAR) outputSize,
  Xcp_MemoryRequestSizeType size,
...
{
  Xcp_ErrorType ret = XCP_E_OK;
#if(XCP_WRITE_READ_MEMORY_RAM == STD_ON)
  if (requestType != XCP_MEMORY_REQUEST_CRC)
  {
    /* !LINKSTO Xcp.BswDistribution.CalibrationCommands.UserDefinedCallouts.MemProxy,1 */
    (void)Xcp_TriggerRAMCallout(
      memory,
      (uint8) size,
      requestType,
      ((requestType == XCP_MEMORY_REQUEST_READ) ? Xcp_MemoryAreas[memoryAreaIdx].Callout.readFromRAMCallout : Xcp_MemoryAreas[memoryAreaIdx].Callout.writeToRAMCallout),
      SessionState
    );
    ret = XCP_E_BUSY;
    *outputSize = (uint16) size;
...
#endif
  {
    ...
    TS_PARAM_UNUSED(SessionState);
    (void) Xcp_DirectMemoryAccess(memory, outputSize, size, requestType);
  }
  return ret;
}

STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessMemoryRequestBswDistribution(
  P2VAR(Xcp_CTOType, AUTOMATIC, AUTOMATIC) ResponseCTO,
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  Xcp_MemoryRequestSizeType size,
  Xcp_MemoryRequestType requestType
...
{
  Xcp_ErrorType retValue = XCP_E_BUSY;
  ...
  if (XCP_GET_MEMORY_PROXY_STATE() == XCP_MEMORY_PROXY_AVAILABLE)
  {
    uint8 memoryAreaIdx;
    const Xcp_MemoryAreaType* memoryArea;
    /* !LINKSTO Xcp.dsn.BSWDistribution.CalibrationCommands.MemProxyLocked,1 */
    Xcp_MemoryProxySetNewState(XCP_MEMORY_PROXY_LOCKED);
    /* !LINKSTO Xcp.dsn.BSWDistribution.CalibrationCommands.ProtectedMemoryLocations,1 */
    memoryArea = Xcp_FindMemoryArea(
                   XCP_APPL_GET_ADDRESS(0, Xcp_MTA.Address.Write),
                   (uint32) size,
                   ((requestType == XCP_MEMORY_REQUEST_WRITE) ? XCP_MEMORY_AREA_TYPE_WRITE : XCP_MEMORY_AREA_TYPE_READ),
                   XCP_MEMORY_AREA_SCOPE_CALIBRATION,
                   &memoryAreaIdx
                 );
    if(memoryArea !=  NULL_PTR)
    {
      if (requestType == XCP_MEMORY_REQUEST_WRITE)
      {
        TS_MemCpy(&Xcp_MemoryProxyBuffer, memory, size);
      }
      /* !LINKSTO Xcp.BswDistribution.Calibration.DOWNLOAD.Optimization,1 */
      if(memoryArea->ApplicationId == (uint8)GetApplicationID())
      {
        /* !LINKSTO Xcp.dsn.BSWDistribution.CalibrationCommands.MemProxyReady.ContinueProcessing, 1 */
        retValue = Xcp_BswDistributionMemoryAccess(
                    &Xcp_MemoryProxyBuffer[0],
                    &Xcp_MemoryProxyResponseLength,
                    size,
                    requestType,
                    memoryAreaIdx,
                    (uint16)(Atomics_Load(&Xcp_MemoryProxyState))
                  );
      }
      ...
      else
      {
        ...
        Xcp_MemoryProxyData.address = Xcp_MemoryProxyBuffer;
        Xcp_MemoryProxyData.size = size;
        Xcp_MemoryProxyData.sessionState = (uint16)(Atomics_Load(&Xcp_MemoryProxyState));
#if (XCP_RAM_CALLOUT_PROXY_ENABLED == STD_ON)
        Xcp_MemoryProxyData.MemoryAreaIdx = memoryAreaIdx;
#endif
        schmResult = Xcp_SendMemoryProxyData[ApplicationId][Xcp_MemoryAreas[memoryAreaIdx].ApplicationId](&Xcp_MemoryProxyData);
        if (schmResult != SCHM_E_OK)
        {
          /* !LINKSTO Xcp.dsn.BSWDistribution.CalibrationCommands.SchMError.MemProxyResetState,1 */
          Xcp_MemoryProxySetNewState(XCP_MEMORY_PROXY_AVAILABLE);
#if (XCP_DET_RUNTIME_CHECKS == STD_ON)
        ...
        {
          /* Nothing to do, XCP_E_BUSY returned by default */
        }
      }
    }
    else
  ...
  if (retValue != XCP_E_BUSY)
  {
    Xcp_MemoryProxySetNewState(XCP_MEMORY_PROXY_AVAILABLE);
    if ((requestType == XCP_MEMORY_REQUEST_READ || requestType == XCP_MEMORY_REQUEST_CRC) && (retValue == XCP_E_OK))
    {
      TS_MemCpy(memory, &Xcp_MemoryProxyBuffer, Xcp_MemoryProxyResponseLength);
    }
  }
  ...
  if (XCP_GET_MEMORY_PROXY_STATE() == XCP_MEMORY_PROXY_READY)
  {
    retValue = XCP_E_OK;
    ...
    Xcp_MemoryProxySetNewState(XCP_MEMORY_PROXY_AVAILABLE);
    if (requestType == XCP_MEMORY_REQUEST_READ || requestType == XCP_MEMORY_REQUEST_CRC)
    {
      TS_MemCpy(memory, &Xcp_MemoryProxyBuffer, Xcp_MemoryProxyResponseLength);
    }
  }
  ...
  return retValue;
     ...
     ((XCP_BSW_DISTRIBUTION_ENABLED == STD_ON) && (XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON) && (XCP_NUMBER_OF_MEMORY_AREAS == 0)) || \
     (XCP_BSW_DISTRIBUTION_ENABLED == STD_ON && XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_OFF))
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessMemoryRequestLocal(
  P2VAR(Xcp_CTOType, AUTOMATIC, AUTOMATIC) ResponseCTO,
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  Xcp_MemoryRequestSizeType size,
  Xcp_MemoryRequestType requestType
...
#endif
  {
#if((XCP_WRITE_READ_MEMORY_RAM == STD_ON) || (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON))
    Xcp_ApplReturnType applRet;
#endif
#if (XCP_WRITE_READ_MEMORY_RAM == STD_ON)
    if (requestType != XCP_MEMORY_REQUEST_CRC)
    {
      applRet = Xcp_TriggerRAMCallout(
        memory,
        (uint8) size,
        requestType,
        ((requestType == XCP_MEMORY_REQUEST_READ) ? Xcp_DescriptorCalloutTypePtr.readFromRAMCallout : Xcp_DescriptorCalloutTypePtr.writeToRAMCallout),
        0
      );
    }
    else
#endif
    {
      uint16 outputSize;
#if((XCP_WRITE_READ_MEMORY_RAM == STD_ON) || (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON))
      applRet = Xcp_DirectMemoryAccess(memory, &outputSize, size, requestType);
#else
      (void)Xcp_DirectMemoryAccess(memory, &outputSize, size, requestType);
#endif
    }
#if((XCP_WRITE_READ_MEMORY_RAM == STD_ON) || (XCP_BUILD_CHECKSUM_SUPPORT == STD_ON))
    RetVal = Xcp_HandleMemoryRequestResponse(ResponseCTO, applRet);
#else
    TS_PARAM_UNUSED(ResponseCTO);
#endif
  }
  return RetVal;
}
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessMemoryRequest(
  P2VAR(Xcp_CTOType, AUTOMATIC, AUTOMATIC) ResponseCTO,
  P2VAR(uint8, AUTOMATIC, XCP_VAR) memory,
  Xcp_MemoryRequestSizeType size,
  Xcp_MemoryRequestType requestType
)
{
  Xcp_ErrorType RetVal = XCP_E_OK;
#if (XCP_BSW_DISTRIBUTION_ENABLED == STD_OFF)
  RetVal = Xcp_ProcessMemoryRequestLocal(ResponseCTO, memory, size, requestType);
#else
#if (XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON)
#if (XCP_NUMBER_OF_MEMORY_AREAS > 0)
#if (XCP_BSW_DISTRIBUTION_CALIBRATION_MEMORY_ACCESS_AREAS == STD_ON)
  RetVal = Xcp_ProcessMemoryRequestBswDistribution(ResponseCTO, memory, size, requestType);
#else
  TS_PARAM_UNUSED(memory);
  TS_PARAM_UNUSED(size);
  TS_PARAM_UNUSED(requestType);
  XCP_ERROR_RESPONSE((*ResponseCTO), XCP_ERR_ACCESS_DENIED, XCP_ERR_ACCESS_DENIED_LENGTH);
  RetVal = XCP_E_NOT_OK;
#endif /* XCP_BSW_DISTRIBUTION_CALIBRATION_MEMORY_ACCESS_AREAS == STD_ON */
#else /* XCP_NUMBER_OF_MEMORY_AREAS > 0 */
  RetVal = Xcp_ProcessMemoryRequestLocal(ResponseCTO, memory, size, requestType);
#endif
#else /* XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON */
  RetVal = Xcp_ProcessMemoryRequestLocal(ResponseCTO, memory, size, requestType);
...
FUNC(void, XCP_CODE) Xcp_SetAbortProgrammingSession(void)
{
  ...
  if ((Xcp_CmdReqStatus & XCP_CMD_PROGRAMMING_ACTIVE_MASK) != 0U)
  {
    ...
    /* Because an error has occurred and the entire programming sequence shall be restarted, a DET
...
/* If event packet transmission enabled */
  ...
  Xcp_ResetCalibrationHelperVars();
...
FUNC(void, XCP_CODE) Xcp_ProcessCommandQueue(void)
{
  ...
  if (XCP_IS_CMD_BUSY_ABORTED() == TRUE)
  {
    ...
    Xcp_MemoryProxySetAvailableNewSession();
#endif
    XCP_CMD_PROCESSOR_STATE_TRANS(XCP_CMDPROCESSOR_IDLE);
  }
  /* If SYNCH command is pending */
  else if (TRUE == XCP_IS_SYNCH_CMD_PENDING())
  {
    /* Initialize command request status flags */
    Xcp_CmdReqStatus = 0U;
    /* Flush command queue */
...
)
{
  ...
  if(XCP_IS_FLAG_BIT_SET(XCP_MAX_DLC_REQUIRED_BIT, Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Properties, uint8) == TRUE)
  {
...
)
{
  ...
  if(Xcp_EventCTOQueue.NoOfElements < XCP_EV_CTO_QUEUE_SIZE)
  {
        ...
        EvCtoQueueIndex = (EvCtoQueueIndex + 1U) % XCP_EV_CTO_QUEUE_SIZE)
    {
       ...
       * the ID of Event response package, which is the same for all Ev CTOs.*/
...
)
{
  ...
  if ((uint8)XCP_MASTER_APPLICATION == partition)
  {
...
FUNC(void, XCP_CODE) Xcp_PrepareStoredDaqLists(void)
{
  ...
  /* Deviation TASKING-1 */
  for (DaqListNumber = 0U; (DaqListNumber < XCP_GET_NO_OF_CONFIGURED_DAQ_LISTS(Xcp_DaqIdType)); DaqListNumber++)
  {
...
#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
FUNC(void, XCP_CODE) Xcp_CalculateCRCAsync(void)
{
  uint8 buffer[6];
  Xcp_BuildChecksumData.Error = Xcp_ProcessMemoryRequestLocal(&Xcp_BuildChecksumData.Cto, &buffer[0],(Xcp_MemoryRequestSizeType)Xcp_BuildChecksumData.BlockSize, XCP_MEMORY_REQUEST_CRC);
  Xcp_BuildChecksumData.ChecksumType = buffer[XCP_CRC_RESPONSE_TYPE_INDEX];
  TS_MemCpy(&Xcp_BuildChecksumData.ChecksumResult, &buffer[XCP_CRC_RESPONSE_INDEX], 4);
  SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
  Xcp_BuildChecksumData.IsRequestCRCActive = FALSE;
  SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
}
...
/*--------------------------[Xcp_CalculateCRC]-----------------------------------*/
FUNC(uint8, XCP_CODE) Xcp_CalculateCRC
(
  P2VAR(uint8, AUTOMATIC, XCP_VAR) data,
  uint32 size,
  P2VAR(uint8, AUTOMATIC, XCP_VAR) resultPtr,
  P2VAR(uint8, AUTOMATIC, XCP_VAR) resultTypePtr
)
{
  Xcp_ApplReturnType applRet = XCP_APPL_OK;
#if (XCP_BUILD_CHECKSUM_TYPE == XCP_CRC_16_CITT)
  {
    uint32 ret;
    ret = (uint32) Crc_CalculateCRC16(data, size, XCP_CRC_16_CITT_STARTVALUE, TRUE);
    *resultTypePtr = 0x08U;
    TS_MemCpy(resultPtr, &ret, 4);
  }
#elif (XCP_BUILD_CHECKSUM_TYPE == XCP_CRC_32)
  {
    uint32 ret;
    ret = Crc_CalculateCRC32(data, size, XCP_CRC_32_STARTVALUE, TRUE);
    *resultTypePtr = 0x09U;
    TS_MemCpy(resultPtr, &ret, 4);
  }
#else
  {
    uint32 checksum;
    applRet = Xcp_ApplBuildChecksum(data, size, &checksum, resultTypePtr);
    TS_MemCpy(resultPtr, &checksum, 4);
  }
#endif
  return applRet;
}
  ...
  for (i = 0; i< XCP_NO_OF_FLX_BUF_CONFIGURED; i++)
  {
    if (Xcp_FlxBufConfig[i].FlxPduIdConfIdx == PduId)
    {
      ...
      if ((PacketType == XCP_SUPPORT_FOR_DAQ_DTO_PDU) &&
          (XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[i].Flags,uint8) == TRUE))
      {
        PacketTypeStatus = TRUE;
      }
      else if ((PacketType == XCP_SUPPORT_FOR_STIM_DTO_PDU) &&
               (XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[i].Flags,uint8) == FALSE))
      {
        PacketTypeStatus = TRUE;
      }
      else
...
#if ((XCP_MEMORY_AREAS_ACCESS_CALLOUT == STD_OFF) && (XCP_NUMBER_OF_MEMORY_AREAS > 0))
...
(
  ...
  P2VAR(uint8, AUTOMATIC, AUTOMATIC) MemoryAreaIdx
)
{
  const Xcp_MemoryAreaType* ret = NULL_PTR;
  uint8 i;
    ...
    if (
         ...
         (
          /* !LINKSTO   Xcp.ProtectedMemoryLocations.AccessType,1 */
           (Xcp_MemoryAreas[i].MemoryAccessType == XCP_MEMORY_AREA_TYPE_READ_WRITE) ||
           (Xcp_MemoryAreas[i].MemoryAccessType == MemoryAreaType)
         ) &&
         (
           /* Deviation MISRAC2012-3 <START> */
           /* Deviation CERTC-1 <START> */
         ...
         (
           ...
           /* Deviation CERTC-1 <STOP> */
       )
    {
       /* We found a suitable memory range, no need to continue */
       ret = &Xcp_MemoryAreas[i];
       if(MemoryAreaIdx != NULL_PTR)
       {
         *MemoryAreaIdx = i;
       }
       break;
...
{
#if ((XCP_MEMORY_AREAS_ACCESS_CALLOUT == STD_OFF) && ((XCP_NUMBER_OF_MEMORY_AREAS > 0)))
  return (NULL_PTR != Xcp_FindMemoryArea(MemoryAreaAddress, MemoryAreaLength, MemoryAreaType, MemoryAreaScope,NULL_PTR));
  ...
  return Xcp_ApplIsMemoryAreaAccessible(MemoryAreaAddress, MemoryAreaLength, MemoryAreaType, MemoryAreaScope);
#else
...
FUNC(PduLengthType, XCP_CODE) Xcp_GetCtoLength(void)
{
  ...
  RetVal = (Xcp_CtoQueue.Queue[(Xcp_CtoQueue.Head + XCP_RESP_CTO_QUEUE_MULTICAST_SIZE) % XCP_CTO_QUEUE_SIZE]);
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessCmd(void)
{
  ...
  {
    /* Execute the command */
    Xcp_ErrorType cmdProcError = Xcp_ExecuteCommand(Command);
    /* Enter critical section.*/
    SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
...
{
  P2CONST(uint8, AUTOMATIC, XCP_VAR) RetVal = NULL_PTR;
  uint8 localLength = 0;
#if ((XCP_GET_ID_TYPE1_CALLOUT_ENABLED == STD_ON) || (XCP_GET_ID_GENERIC_TYPE_CALLOUT_ENABLED == STD_ON))
  Xcp_ApplReturnType applRetVal = XCP_APPL_OK;
#endif

  /* ASAM-MC2 filename without path and extension */
  if (IdentificationType == XCP_IDENTIFICATION_TYPE1)
  {
    ...
    applRetVal = Xcp_ApplGetIdentification(Xcp_GetIdBuffer, &localLength);
    RetVal = Xcp_GetIdBuffer;
#else /* (XCP_GET_ID_TYPE1_CALLOUT_ENABLED == STD_OFF) */

#if (XCP_GET_ID_TYPE1_ASAM_FILENAME_LENGTH > 0)
    RetVal = Xcp_IdentificationTypeOne;
    localLength = XCP_GET_ID_TYPE1_ASAM_FILENAME_LENGTH;
#endif
#endif /* (XCP_GET_ID_TYPE1_CALLOUT_ENABLED == STD_ON) */
  }

#if (XCP_GET_ID_GENERIC_TYPE_CALLOUT_ENABLED == STD_ON)
  if ((IdentificationType == XCP_IDENTIFICATION_TYPE0) ||
      (IdentificationType == XCP_IDENTIFICATION_TYPE2) ||
      (IdentificationType == XCP_IDENTIFICATION_TYPE3) ||
      (IdentificationType == XCP_IDENTIFICATION_TYPE4) ||
      (IdentificationType >= 128U))
  {
    /* !LINKSTO Xcp.GET_ID.GenericUserCallout,1 */
    /* !LINKSTO Xcp.GET_ID.GenericUserCallout.ReturnValue.APPL_OK,1 */
    applRetVal = Xcp_ApplGenericGetIdentification(IdentificationType, Xcp_GetIdBuffer, &localLength);
    RetVal = Xcp_GetIdBuffer;
  }
#endif

#if ((XCP_GET_ID_TYPE1_CALLOUT_ENABLED == STD_ON) || (XCP_GET_ID_GENERIC_TYPE_CALLOUT_ENABLED == STD_ON))
  if (applRetVal == XCP_APPL_OK)
  {
    *LengthPtr = localLength;
  }
  else
  {
    /* !LINKSTO Xcp.GET_ID.UserCallout.ReturnValue.NegativeResponse,1 */
    /* !LINKSTO Xcp.GET_ID.GenericUserCallout.ReturnValue.NegativeResponse,1 */
    *LengthPtr = 0U;
  }
#else
  *LengthPtr = localLength;
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE)  Xcp_ProcessCmdGetId(void)
{
  ...
  Xcp_CTOType ResponseCTO;
  uint8 IdLength;
  P2CONST(uint8, AUTOMATIC, XCP_VAR) IdentificationData;
  ...
  IdentificationData = Xcp_GetIdentificationData(IdentificationType, &IdLength);

  /* Deviation TASKING-3 */
#pragma warning 549
  if (IdentificationData == NULL_PTR)
#pragma warning default
  ...
  else
  {
    ...
    if ((IdLength + XCP_CMD_GETID_RES_LENGTH_MIN) > Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Cto)
    {
      ...
      /* Set MTA */
      Xcp_MTA.Type         = XCP_MTATYPE_ADDRESS;
      Xcp_MTA.Address.Read = IdentificationData;
      Xcp_MTA.IsSet        = TRUE;
  ...
  else
  {
    ...
    else /* DAQ List number is valid */
    {
      ...
      else
      {
        ...
        Xcp_ProcessStartStopDaqList(&ResponseCTO, DaqMode, DaqListNumber);
      }
    }
  }
...
)
{
  ...
  case XCP_SSDL_MODE_START:
    Xcp_StartDaqList(DaqListNumber);
    /* Prepare response CTO */
    ResponseCTO->CTO[XCP_RES_PID_INDEX] = XCP_RES_PID;
    /* If DAQ Packet Identification is of type "Absolute PID" */
#if (XCP_IDENTIFICATION_FIELD_TYPE == XCP_ABSOLUTE_IF_MASK)
...
STATIC FUNC (void, XCP_CODE) Xcp_StopDaqList(uint16 DaqListNumber)
{
  ...
  if (XCP_IS_DAQ_LIST_RUNNING(DaqListPtr) == TRUE)
  {
    ...
    /* Update the total number of DAQ Lists running */
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdShortUpload(void)
{
  Xcp_ErrorType ret = XCP_E_OK;
  Xcp_CTOType ResponseCTO;
  const uint8 NoOfDataBytes = (uint8)( XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE1_INDEX) * XCP_ADDRESS_GRANULARITY_SIZE );
  ...
  {
    Xcp_MTA.Type = XCP_MTATYPE_ADDRESS;
    TS_MemCpy(&Xcp_MTA.Address.Write, &(Xcp_CommandQueue.Queue[Xcp_CommandQueue.Head].CTO[4U]), XCP_MTA_ADDRESS_LENGTH);
    Xcp_MTA.IsSet = TRUE;
    ret = Xcp_ProcessMemoryRequest(&ResponseCTO, &(ResponseCTO.CTO[XCP_ADDRESS_GRANULARITY_SIZE]), NoOfDataBytes, XCP_MEMORY_REQUEST_READ);

    /* Deviation TASKING-2, TASKING-3 */
    if (ret == XCP_E_OK)
    {
      ResponseCTO.CTO[XCP_RES_PID_INDEX] = XCP_RES_PID;
...
#if ((XCP_RAM_CALLOUT_PROXY_ENABLED == STD_OFF) && (XCP_EVENT_PACKET_ENABLED == STD_ON))
  Xcp_SendCmdPendingEventCto(ret);
#endif

  SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();

  DBG_XCP_PROCESSCMDSHORTUPLOAD_EXIT(ret);
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdUpload(void)
{
...
#else
    else if ( (NoOfDataBytesRequested > (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Cto - XCP_ADDRESS_GRANULARITY_SIZE)) ||
  ...
  if (RetVal == XCP_E_OK)
  {
    if (Xcp_UploadInternalCopyFinished == FALSE)
    {
      RetVal = Xcp_ProcessMemoryRequest(&ResponseCTO, &Xcp_MemoryWrite.Buffer[0], NoOfDataBytesRequested, XCP_MEMORY_REQUEST_READ);

      /* Deviation TASKING-2, TASKING-3 */
      if (RetVal == XCP_E_OK)
      {
        Xcp_UploadInternalCopyFinished = TRUE;
        Xcp_RemainingDataBytesToUpload = NoOfDataBytesRequested;
      }
    }
    /* allow to send the first packet in the same MF cycle */
    if (Xcp_UploadInternalCopyFinished == TRUE)
    {
      ...
      * bytes to be uploaded in one message  */
      ...
      TS_MemCpy(&ResponseCTO.CTO[XCP_ADDRESS_GRANULARITY_SIZE], &Xcp_MemoryWrite.Buffer[NoOfDataBytesRequested - Xcp_RemainingDataBytesToUpload], NoOfDataBytesToUploadNow);
      ResponseCTO.CTO[XCP_RES_PID_INDEX] = XCP_RES_PID;
      ...
      {
        Xcp_UploadInternalCopyFinished = FALSE;
      }
    }
  }
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdDownload(void)
{
  ...
  if ((NoOfDataElementsRequested == 0U) || (NoOfDataElementsRequested > Xcp_Max_Download_Elements))
  {
    XCP_ERROR_RESPONSE(ResponseCTO, XCP_ERR_OUT_OF_RANGE, XCP_ERR_OUT_OF_RANGE_LENGTH);
  }
  else
  {
    if((XCP_GET_RESOURCE_PROTECTION_STATUS() & XCP_RESOURCE_CAL_PAG_MASK) == XCP_RESOURCE_CAL_PAG_MASK)
    {
      /* Calibration commands are protected; return ERR_ACCESS_LOCKED */
      XCP_ERROR_RESPONSE(ResponseCTO, XCP_ERR_ACCESS_LOCKED, XCP_ERR_ACCESS_LOCKED_LENGTH);
    }
    else
    {
      ...
      {
        /* Set the variable holding the number of elements to download with the requested number found in the command */
        if (Xcp_MemoryWrite.Pending == FALSE)
        {
          Xcp_MemoryWrite.ElementsLeftToWrite = NoOfDataElementsRequested;
        }
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdDownloadNext(void)
{
  ...
  if (
      (Xcp_MemoryWrite.ElementsLeftToWrite != XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE1_INDEX)) &&
      (FALSE == Xcp_MemoryWrite.Pending) &&
     /* the command processor state can be in busy state meaning that the memory proxy has not yet finished */
      (Xcp_CmdProcessorState == XCP_CMDPROCESSOR_IDLE)
     )
  {
...
)
{
  ...
  if (Xcp_MemoryWrite.ElementsLeftToWrite == 0U)
  {
    ...
    uint8 NumberOfBytesToDownload = (uint8)(Xcp_MemoryWrite.ElementsWrittenSoFar * XCP_ADDRESS_GRANULARITY_SIZE);
    ...
    /* CHECK: PARSE */
    {
      ...
      RetValue = Xcp_ProcessMemoryRequest(ResponseCTO, &(Xcp_MemoryWrite.Buffer[0U]), NumberOfBytesToDownload, XCP_MEMORY_REQUEST_WRITE);

      /* Deviation TASKING-2, TASKING-3 */
      if (RetValue == XCP_E_OK)
      {
      ...
      /* Deviation TASKING-2 */
      else if(RetValue == XCP_E_BUSY)
      {
        Xcp_MemoryWrite.Pending = TRUE;
      }
      else
      {
        /* Nothing additional to do, simply return RetValue */
      }
    }
  }
  return RetValue;
...
/* !LINKSTO Xcp.Ethernet.Udp.GETSLAVEID,1 */
        ...
        case XCP_SUBCMD_GET_SLAVE_ID_PID:
        {
          ...
          if(XCP_IS_BYTE_FLAG_SET(Xcp_ConnectionStatus.ConnectionId,
                &Xcp_Connections_IPv6[Xcp_ConnectionStatus.ConnectionId / 8U]) == TRUE)
          {
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        /* check if Odt entries have been set */
        if (RequestedDaqList->OdtList == NULL_PTR)
        {
#if (XCP_DET_RUNTIME_CHECKS == STD_ON)
          XCP_DET_REPORT_RUNTIME_ERROR(XCP_SID_MAIN_FUNCTION, XCP_E_WRONG_SET_DAQ_PTR_CMD_SEQUENCE_ERROR);
#endif
          ResponseCTO.Length = 0U;
        }
        else
...
)
{
  ...
  P2VAR(Xcp_DaqType, AUTOMATIC, XCP_VAR_CLEARED) PointedDaqListPtr =
    XCP_GET_DAQ_LIST_PTR(Xcp_DAQPointer.DAQList);
  /* Check whether the bit position (BIT_OFFSET) is valid */
...
#if (XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON)
#if ((XCP_BSW_DISTRIBUTION_ENABLED == STD_OFF) || (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON))
...
#endif /* (XCP_BSW_DISTRIBUTION_ENABLED == STD_ON) || (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON) */
#endif /* XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON */
        {
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE)  Xcp_ProcessCmdGetSeed(void)
{
  ...
  uint8 SeedLength = Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Cto - 1U;
  ...
  else
  {
    ...
    else
    {
       ...
       */
      if ((E_OK != errorCode) ||
         (SeedLength > (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Cto - 2U))
        )
      {
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdBuildChecksum(void)
{
  Xcp_CTOType ResponseCTO = { 0 };
  Xcp_ErrorType RetVal = XCP_E_OK;
  uint32 crcSize = XCP_UINT32_FROM_BYTE_ARRAY(&XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE4_INDEX));

#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
  if (Xcp_CmdProcessorState == XCP_CMDPROCESSOR_BUSY)
  {
    SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
    if (Xcp_BuildChecksumData.IsRequestCRCActive == TRUE)
    {
      RetVal = XCP_E_BUSY;
    }
    else
    {
      Xcp_CRCMemoryRequestFinished = TRUE;
    }
    SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
  }
  else
#endif
  {
    ...
    else
    {
      ...
      SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
      Xcp_BuildChecksumData.BlockSize = crcSize;
      RetVal = XCP_E_BUSY;
#else
      RetVal = Xcp_ProcessMemoryRequest(&ResponseCTO, &Xcp_MemoryWrite.Buffer[0], (Xcp_MemoryRequestSizeType)crcSize, XCP_MEMORY_REQUEST_CRC);
  ...
  if (Xcp_CRCMemoryRequestFinished == TRUE)
  {
#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
    if (Xcp_BuildChecksumData.Error != XCP_E_OK)
    {
      ResponseCTO.Length = Xcp_BuildChecksumData.Cto.Length;
      TS_MemCpy(&ResponseCTO.CTO, &Xcp_BuildChecksumData.Cto.CTO, sizeof(ResponseCTO.CTO));
    }
    else
#elif (XCP_BUILD_CHECKSUM_TYPE == XCP_CRC_USER_CALLOUT)
    if (Xcp_MemoryWrite.Buffer[0] != XCP_APPL_OK)
    {
      ResponseCTO.CTO[XCP_RES_PID_INDEX] = XCP_ERR_PID;
      ResponseCTO.CTO[XCP_RES_ERRCODE_INDEX] = Xcp_MemoryWrite.Buffer[0];
      ResponseCTO.Length = 2;
    }
    else
#endif
    {
...
#else
      ResponseCTO.CTO[1U] = Xcp_MemoryWrite.Buffer[XCP_CRC_RESPONSE_TYPE_INDEX];
#endif
      ResponseCTO.CTO[2U] = XCP_RESERVED_BYTE;
      ResponseCTO.CTO[3U] = XCP_RESERVED_BYTE;
#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
      TS_MemCpy(&ResponseCTO.CTO[4U], &Xcp_BuildChecksumData.ChecksumResult, 4U);
#else
      TS_MemCpy(&ResponseCTO.CTO[4U], &Xcp_MemoryWrite.Buffer[XCP_CRC_RESPONSE_INDEX], 4U);
#endif
      ResponseCTO.Length = XCP_CMD_BUILD_CHECKSUM_RES_LENGTH;
      XCP_INCREMENT_MTA_BY_SIZE_IN_AG(crcSize);

    }

    Xcp_CRCMemoryRequestFinished = FALSE;
  }

  if (ResponseCTO.Length > 0U)
  {
    /* special handling for XCP_ERR_OUT_OF_RANGE, as specified by ASAM */
    if (ResponseCTO.CTO[0] == XCP_ERR_PID && ResponseCTO.CTO[1] == XCP_ERR_OUT_OF_RANGE)
    {
      ResponseCTO.Length = 8;
      ResponseCTO.CTO[2U] = XCP_RESERVED_BYTE;
      ResponseCTO.CTO[3U] = XCP_RESERVED_BYTE;
#if (XCP_ASYNCH_CRC_SUPPORT == STD_ON)
      TS_MemCpy(&ResponseCTO.CTO[4U], &Xcp_BuildChecksumData.ChecksumResult, 4U);
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdModifyBits(void)
{
  ...
  const uint16 And_Mask = XCP_UINT16_FROM_BYTE_ARRAY(&XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE2_INDEX));
  const uint16 Xor_Mask = XCP_UINT16_FROM_BYTE_ARRAY(&XCP_READ_CMDPACKET_BYTE(XCP_CMD_BYTE4_INDEX));
  Xcp_ErrorType RetValue = XCP_E_OK;
  ...
  {
    if(Xcp_ModifyBitsReadFinished == FALSE)
    {
      RetValue = Xcp_ProcessMemoryRequest(&ResponseCTO, (uint8*)&Xcp_ModifyBitsValue, XCP_MODIFY_VALUE_SIZE, XCP_MEMORY_REQUEST_READ);

      /* Deviation TASKING-2 */
      if (RetValue == XCP_E_OK)
      {
        uint32 Temp_Value_1, Temp_Value_2;
        Xcp_ModifyBitsReadFinished = TRUE;
        Temp_Value_1 = (uint32)((uint16)(~And_Mask)) << Shift_Value;
        Temp_Value_2 = (uint32)Xor_Mask << Shift_Value;
        Xcp_ModifyBitsValue = (Xcp_ModifyBitsValue & (uint32)(~Temp_Value_1)) ^ Temp_Value_2;
      }
    }

    if (Xcp_ModifyBitsReadFinished == TRUE)
    {

      RetValue = Xcp_ProcessMemoryRequest(&ResponseCTO, (uint8*) &Xcp_ModifyBitsValue, XCP_MODIFY_VALUE_SIZE, XCP_MEMORY_REQUEST_WRITE);
      ...
      if (RetValue == XCP_E_OK)
      {
        ...
        Xcp_ModifyBitsReadFinished = FALSE;
      }
    }
  }
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdSetRequest(void)
{
  ...
  else if( (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].ConnectionType == XCP_CONNECTION_OVER_TCPIP) ||
           (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].ConnectionType == XCP_CONNECTION_OVER_UDPIP) )
  {
    /* Prepare error CTO with negative response ERR_CMD_UNKNOWN */
    XCP_ERROR_RESPONSE(ResponseCTO, XCP_ERR_CMD_UNKNOWN, XCP_ERR_CMD_UNKNOWN_LENGTH);
  }
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessCmdUserDefined(void)
{
  ...
  if (StatusVal == XCP_E_OK)
  {
    if (Xcp_UserCommandConf[SubCommandIndex].SubCmdRespLength >
        (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Cto - 1U))
    {
      XCP_ERROR_RESPONSE(ResponseCTO, XCP_ERR_OUT_OF_RANGE, XCP_ERR_OUT_OF_RANGE_LENGTH);
    ...
    else
    {
      ...
      else
      {
...
#endif
        {
          ...
          if (applicationId != (uint8)XCP_MASTER_APPLICATION)
          {
            (void) Xcp_SendNvMRequest[applicationId](applicationId);
          }
          else
          {
            (void)NvM_WriteBlock(XCP_STORE_DAQ_NVM_BLOCK_ID, NULL_PTR);
        ...
        else
        {
          ...
          /* We must restore the selected status and the resume mode (if needed) for the selected DAQ lists */
          /* Deviation TASKING-1 */
          for (i = 0U; i< XCP_GET_NO_OF_CONFIGURED_DAQ_LISTS(Xcp_DaqIdType); i++)
          {
            /* Get the DAQ list */
...
STATIC FUNC(Xcp_ErrorType, XCP_CODE) Xcp_ProcessSubCmdFlxAssign(void)
{
  ...
  else
  {
    if (Xcp_FlxAssignParametersAreValidated() == TRUE)
    {
      ...
      {
        if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_FIXED_BIT, Xcp_FlxBufConfig[CmdParamFlxBuf].Flags,uint8) == FALSE)
        {
          ...
          if (Xcp_FlxBufConfig[CmdParamFlxBuf].FlxBufLengthProperties != XCP_FLX_BUF_VALUE_FIXED)
          {
            /* Enter critical section */
            SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
            if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[CmdParamFlxBuf].Flags,uint8) == TRUE)
            {
...
STATIC FUNC(Std_ReturnType, XCP_CODE) Xcp_FlxRestoreBuffer(uint8 FlxBuff)
{
  ...
  if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, buffer.Flags,uint8) == TRUE)
  {
#if (XCP_RESOURCE_DAQ == XCP_RESOURCE_DAQ_MASK)
    /* enable/disable the DAQ packet type */
    Xcp_SetPacketEnableStatusDaq(Xcp_TxLPduIdConf[buffer.FlxPduIdConfIdx].IntPduBufId,
        (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_DAQ_DTO_PDU_BIT, Xcp_TxLPduIdConf[buffer.FlxPduIdConfIdx].Properties, uint8)));
#endif
#if (XCP_EVENT_PACKET_ENABLED == STD_ON)
    /* enable/disable the EVSERV packet type */
    Xcp_SetPacketEnableStatusEvServ(Xcp_TxLPduIdConf[buffer.FlxPduIdConfIdx].IntPduBufId,
        (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_EVSERV_CTO_PDU_BIT, Xcp_TxLPduIdConf[buffer.FlxPduIdConfIdx].Properties, uint8)));
#endif
  }
...
#if (XCP_RESOURCE_STIM == XCP_RESOURCE_STIM_MASK)
  if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, buffer.Flags,uint8) == FALSE)
  {
    /* enable/disable the STIM packet type */
    Xcp_SetPacketEnableStatusStim(Xcp_RxLPduIdConf[buffer.FlxPduIdConfIdx].IntPduBufId,
        (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_STIM_DTO_PDU_BIT, Xcp_RxLPduIdConf[buffer.FlxPduIdConfIdx].Properties, uint8)));
  }
...
STATIC FUNC(boolean, XCP_CODE) Xcp_AllowTxPacket (uint8 FlxBuff, uint8 XcpPacket)
{
  ...
  if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_RESERR_CTO_PDU_BIT, Xcp_TxLPduIdConf[buffer.FlxPduIdConfIdx].Properties, uint8) == TRUE)
  {
    if (XCP_IS_FLAG_BIT_SET(XCP_FLX_BUF_PACKET_RES_ERR_BIT, XcpPacket, uint8) == FALSE)
    {
      retValue = FALSE;
    }
  }
  ...
  {
    if (XCP_IS_FLAG_BIT_SET(XCP_FLX_BUF_PACKET_RES_ERR_BIT, XcpPacket, uint8) == TRUE)
    {
      retValue = FALSE;
    }
  }
...
STATIC FUNC(boolean, XCP_CODE) Xcp_AllowRxPacket (uint8 FlxBuff, uint8 XcpPacket)
{
  ...
  if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_CMD_CTO_PDU_BIT, Xcp_RxLPduIdConf[buffer.FlxPduIdConfIdx].Properties, uint8) == TRUE)
  {
    if (XCP_IS_FLAG_BIT_SET(XCP_FLX_BUF_PACKET_CMD_BIT, XcpPacket, uint8) == TRUE)
    {
      retValue = TRUE;
    }
  }
  ...
  {
    if (XCP_IS_FLAG_BIT_SET(XCP_FLX_BUF_PACKET_CMD_BIT, XcpPacket, uint8) == FALSE)
    {
      retValue = TRUE;
    }
  }
...
{
  uint8 retValue;
  if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[FlxBuff].Flags,uint8) == TRUE)
  {
...
#if(XCP_EVENT_PACKET_ENABLED == STD_OFF)
...
#if ((XCP_BUILD_CHECKSUM_SUPPORT == STD_ON) && (XCP_ASYNCH_CRC_SUPPORT == STD_ON))
...
STATIC FUNC(void, XCP_CODE) Xcp_ResetDAQPointer(void)
{
  Xcp_DAQPointer.DAQList = XCP_MAX_DAQ;
  Xcp_DAQPointer.ODT = XCP_MAXIMUM_ODT;
  Xcp_DAQPointer.ODTEntry = XCP_MAX_ENTRIES_PER_ODT;
}
#endif
