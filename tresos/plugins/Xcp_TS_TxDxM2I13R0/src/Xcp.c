 ...
 * When the length of the NvM block used to store the DAQ lists is configured large enough to
 * exceed the size of the data in the XCP structure, then the condition is always false.
 ...
 * Reason:
 * This is due to specific module configurations that lead to different preprocessor expansions.
 * When the length of the NvM block used to store the DAQ lists is configured with a small value that does not
 * exceed the size of the data in the XCP structure, then the condition is always true.
...
/* minor version check */
#if (XCP_SW_MINOR_VERSION < 13U)
#error XCP_SW_MINOR_VERSION wrong (< 13U)
#endif
...
/* patch version check */
#if (XCP_SW_PATCH_VERSION < 5U)
#error XCP_SW_PATCH_VERSION wrong (< 5U)
#endif
...
VAR(uint8, XCP_VAR_CLEARED) Xcp_MemoryProxyErrorPending;
VAR(uint8, XCP_VAR_CLEARED) Xcp_CyclicEventOverloaded;
#endif
...
{
#if (XCP_BSW_DISTRIBUTION_ENABLED == STD_ON)
  if ((uint8)GetApplicationID() == (uint8)XCP_MASTER_APPLICATION)
  {
...
FUNC(void, XCP_CODE) Xcp_Control( uint8 command )
{
  ...
  if ((XCP_NUM_OF_OS_APPS == partition) || ((uint8)XCP_MASTER_APPLICATION == partition))
  {
...
/* !LINKSTO Xcp.SWS_Xcp_00848,1 */
#if (XCP_SUPPRESS_TX_SUPPORT == STD_ON)
/*----------------------------------[Xcp_SetTransmissionMode]----------------------------*/

/* !LINKSTO Xcp.SWS_Xcp_00844,1 */
...
STATIC FUNC(Std_ReturnType, XCP_CODE) Xcp_InitSoAdConnections(void)
{
  ...
  for(Iterator = 0U; Iterator < XCP_NO_OF_CONNECTIONS; Iterator++)
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_AUTO_OPEN_SOCON_BIT,
...
STATIC FUNC(void, XCP_CODE) Xcp_CloseSoAdConnections(void)
{
  ...
  for(Iterator = 0U; Iterator < XCP_NO_OF_CONNECTIONS; Iterator++)
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_AUTO_OPEN_SOCON_BIT, ConnectionCfgPtr->Properties,uint8) == TRUE)
    {
