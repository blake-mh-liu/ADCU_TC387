...
/** \brief Function used to unpack the XCP Pdus from the Rx Pdu buffer.
 **
...
FUNC(void, XCP_CODE) Xcp_InitReceiveProcessor(void)
{
  ...
  {
    if(XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[Iterator].Flags,uint8) == FALSE)
    {
...
)
{
  ...
  if (Xcp_ConnectionStatus.ConnectionId != RxLPduIdConfPtr->IntConnectionId)
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_TP_CONFIGURATION_DONE_BIT, Xcp_ConnectionStatus.Flags, uint8) == FALSE)
    {
...
#endif /* (XCP_DYNAMIC_PDU_SUPPORTED == STD_ON) */
  {
    ...
    if (TRUE == XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_RX_FROM_INDICATION_BIT,
...
#endif
  {
...
#endif /* (XCP_NO_OF_CONNECTIONS > 1U) */
    {
      ...
      for(RxPduId = 0U; RxPduId < ConnectionCfgPtr->RxPduMax; RxPduId++)
      {
        ...
        boolean PduLocalSts = FALSE;
        /* if PDU is dynamic and enabled or PDU is not dynamic */
        if ((TRUE == XCP_IS_FLAG_BIT_SET(XCP_DYNAMIC_RX_PDU_BIT,
        ...
        else if (FALSE == XCP_IS_FLAG_BIT_SET(XCP_DYNAMIC_RX_PDU_BIT,
                   Xcp_RxLPduIdConf[Xcp_RxLPduData.LPduIntId[RxPduId]].Properties,uint8))
        {
          PduLocalSts = TRUE;
        }
  ...
  for(Iterator = 0U; Iterator < XCP_NO_OF_RX_PDU_IDS; Iterator++)
  {
    if (Xcp_RxLPduIdConf[Iterator].IntConnectionId == ConnectionId)
    {
      ...
      if ((TRUE == XCP_IS_FLAG_BIT_SET
                    (XCP_DYNAMIC_RX_PDU_BIT, Xcp_RxLPduIdConf[Iterator].Properties,uint8)) &&
          (TRUE == XCP_IS_FLAG_BIT_SET
                    (XCP_ENABLE_STATE_DYNAMIC_RX_PDU_BIT, Xcp_RxLPduIdConf[Iterator].Properties,uint8)))
      {
      ...
      {
        if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_STIM_DTO_PDU_BIT, Xcp_RxLPduIdConf[Iterator].Properties, uint8) == TRUE)
        {
...
)
{
  ...
  if (DataLength >= minimumStimLength)
  {
    ...
    {
      /* Check if the packet STIM is allowed (read from runtime bit field) */
...
#endif /* (XCP_ON_FLEXRAY_ENABLED == STD_ON) && (XCP_FLEXRAY_TP_COMMANDS_SUPPORTED == STD_ON)*/
    {
      ...
      retValue = (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_STIM_DTO_PDU_BIT,
...
#if(XCP_NO_OF_CONNECTIONS > 1U)
      if((ConnectionCfgPtr->ConnectionType == XCP_CONNECTION_OVER_FLEXRAY) &&
         (XCP_IS_FLAG_BIT_SET(XCP_MULTI_PDUS_IN_FRAME_BIT,
                        ConnectionCfgPtr->Properties,
                        uint8) == TRUE))
#endif /* (XCP_NO_OF_CONNECTIONS > 1U) */
      {
...
)
{
  ...
  if(XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_CMD_CTO_PDU_BIT,
...
/*-----------------------------[Xcp_IsRxBufferFull]---------------------*/
FUNC(boolean, XCP_CODE) Xcp_IsRxBufferFull(void)
{
  uint8 i;
  boolean RetVal = FALSE;

  /* Deviation TASKING-1 */
  for (i = 0; i < XCP_RX_PDU_FLAG_SIZE; i++)
  {
    if (Xcp_RxLPduData.IsBufferFullFlag[i] != 0U)
    {
      RetVal = TRUE;
    }
  }
