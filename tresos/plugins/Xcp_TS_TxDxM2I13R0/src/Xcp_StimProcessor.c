 ...
 **
 ** This function calculates and retrieves the DTO package information:
 ** - data header size
 ** - data size
 **
 ...
 ** stimulate the data associated with the ODT and remove the
 ...
 ** stimulate the data associated with the ODTs and remove the
...
)
{
  ...
  else
  {
    ...
    /*Previous stimulation data exists and therefore new entry cannot be added.
  ...
  for (OdtEntryId = 0U; OdtEntryId < NrOfOdtEntries; OdtEntryId++)
  {
...
#if (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON)
#if (XCP_ENABLE_DAQ_OPTIMIZATION == STD_OFF)
      MemoryArea = Xcp_FindMemoryArea(XCP_APPL_GET_ADDRESS( 0U, OdtEntryPtr[OdtEntryId].Address ), OdtEntryLength, XCP_MEMORY_AREA_TYPE_WRITE, XCP_MEMORY_AREA_SCOPE_DAQ_STIM, NULL_PTR);
#else
      /* Deviation CERTC-1 */
      MemoryArea = Xcp_FindMemoryArea(XCP_APPL_GET_ADDRESS( 0U, (uint8*)(*((uint32*)Xcp_GetOdtEntryData(OdtEntryPtr,OdtEntryId,XCP_ODT_ENTRY_FIELD_ADDRESS)))), OdtEntryLength, XCP_MEMORY_AREA_TYPE_WRITE, XCP_MEMORY_AREA_SCOPE_DAQ_STIM, NULL_PTR);
#endif
    ...
    ((XCP_ENABLE_DAQ_OPTIMIZATION == STD_ON) && (XCP_ENABLE_BIT_OFFSET == STD_ON)))
        ...
        if (bitOffset != XCP_BIT_OFFSET_IGNORE_VALUE)
        {
          ...
          if ((*XCP_GET_MIRROR_PTR(DaqID, DTOMirrorWritePos) & bitMask) > 0U)
          {
        ...
        Xcp_MemoryProxyEnqueueRequest(DaqID, DaqListPtr->EventId, Size, DTOMirrorWritePos, MemoryArea->ApplicationId, Type, XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress ), bitMask);
