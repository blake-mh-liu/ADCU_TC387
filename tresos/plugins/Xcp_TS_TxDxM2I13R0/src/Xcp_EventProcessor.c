 ...
 *
 * Reason:
 * RetVal value change is followed by a break in the loop, so the condition is obsolete.
 * If the condition were always true then an infinite loop would occur.
 *
 */
...
VAR(Xcp_RequestToMemProxyStructType, XCP_VAR_CLEARED) Xcp_RequestToMemProxy;
VAR(Xcp_DaqPrioQueueOfRequestType, XCP_VAR_CLEARED) Xcp_DaqPrioQueueOfRequest;
VAR(Xcp_RequestToMemProxyType, XCP_VAR_CLEARED) Xcp_RequestBuffer[XCP_MAX_SIZE_OF_REQUEST_BUFFER];
VAR(Xcp_RequestForEventType, XCP_VAR_CLEARED) Xcp_RequestForEvent[XCP_MAX_EVENT_CHANNEL];
...
#if (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON)
STATIC VAR(Xcp_RequestQueueType, XCP_VAR_CLEARED) Xcp_RequestQueue;
#endif
...
FUNC(void, XCP_CODE) Xcp_ProcessEvents(uint16 EventId)
{
  ...
  if (EventProcessingStatus == XCP_OK)
  {
                         ...
                         &(Xcp_Event[EventId].EventStatusFlag)) == TRUE)
    {
      ...
      else if ((XCP_GET_MEMORY_PROXY_STATE() == XCP_MEMORY_PROXY_AVAILABLE) &&
...
#if (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_OFF)
...
{
  Xcp_DaqPrioQueueIndexType QueueIndex = 0U;
  Xcp_DaqIdType DaqID;
  ...
  while (QueueIndex < Xcp_DaqPrioQueue.NoOfEntries)
  {
    ...
    if (XCP_IS_DAQ_LIST_RUNNING(DaqListPtr) != TRUE)
    {
      ...
      /* Iterator */
      Xcp_DaqPrioQueueIndexType DaqPrioQueueIndex;
...
)
{
  ...
  /* Deviation TASKING-1 */
  for (DaqIDLocal = 0U; DaqIDLocal < XCP_GET_NO_OF_CONFIGURED_DAQ_LISTS(Xcp_DaqIdType); DaqIDLocal++)
  {
    /* Get the DAQ list */
...
/* !LINKSTO Xcp.ProtectedMemoryLocations.General,1 */
...
#if (XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON)
  /* Deviation TASKING-1 */
  for(OdtEntryCount = 0U; ((OdtEntryCount < DaqListPtr->OdtList[OdtIndex].NrOfOdtEntries) && (MemoryIsAccessible == TRUE)); OdtEntryCount++)
#else
...
#endif /* XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON */
  {
...
#if (XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_ON)
#if (XCP_ENABLE_DAQ_OPTIMIZATION == STD_OFF)
      MemoryArea = Xcp_FindMemoryArea(XCP_APPL_GET_ADDRESS( 0U, OdtEntryPtr[OdtEntryCount].Address ), OdtEntryLength, XCP_MEMORY_AREA_TYPE_READ, XCP_MEMORY_AREA_SCOPE_DAQ_STIM, NULL_PTR);
#else
      /* Deviation CERTC-1 */
      MemoryArea = Xcp_FindMemoryArea(XCP_APPL_GET_ADDRESS( 0U, (uint8*)(*((uint32*)Xcp_GetOdtEntryData(OdtEntryPtr,OdtEntryCount,XCP_ODT_ENTRY_FIELD_ADDRESS))) ), OdtEntryLength, XCP_MEMORY_AREA_TYPE_READ, XCP_MEMORY_AREA_SCOPE_DAQ_STIM, NULL_PTR);
#endif
    ...
    ((XCP_ENABLE_DAQ_OPTIMIZATION == STD_ON) && (XCP_ENABLE_BIT_OFFSET == STD_ON)))
        ...
        if (bitOffset != XCP_BIT_OFFSET_IGNORE_VALUE)
        {
          ...
          if (((*(uint8 *)XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress)) & bitMask) > 0U )
          {
          ...
          Xcp_MemoryProxyEnqueueRequest(DaqId, DaqListPtr->EventId, OdtEntryLength, DTOMirrorWritePos, MemoryArea->ApplicationId, Type, XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress), DummyBitMask);
        }
        else
#endif
        {
          Xcp_MemoryProxyEnqueueRequest(DaqId, DaqListPtr->EventId, OdtEntryLength, DTOMirrorWritePos, MemoryArea->ApplicationId, XCP_MEMORYPROXYDATA_SAMPLE, XCP_APPL_GET_ADDRESS( 0U, odtEntryAddress), DummyBitMask);
        }
        SchM_Exit_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
...
#if ((XCP_MEMORY_AREAS_ACCESS_SUPPORTED == STD_ON) && (XCP_BSW_DISTRIBUTION_MEMORY_ACCESS_AREAS == STD_OFF) && (XCP_BSW_DISTRIBUTION_ENABLED == STD_OFF))
#if (XCP_ENABLE_DAQ_OPTIMIZATION == STD_OFF)
    ...
    ((XCP_ENABLE_DAQ_OPTIMIZATION == STD_ON) && (XCP_ENABLE_BIT_OFFSET == STD_ON)))
        {
          ...
          /* Return TRUE when bit is set */
#if (XCP_ENABLE_DAQ_OPTIMIZATION == STD_OFF)
          if (((*(uint8 *)XCP_APPL_GET_ADDRESS( 0U, OdtEntryPtr[OdtEntryCount].Address)) &  bitMask) > 0U )
#else
          /* Deviation CERTC-1 */
          if (((*(uint8*)XCP_APPL_GET_ADDRESS( 0U, (uint8*)(*((uint32*)Xcp_GetOdtEntryData(OdtEntryPtr,OdtEntryCount,XCP_ODT_ENTRY_FIELD_ADDRESS))))) &  bitMask) > 0U )
#endif
          {
            /* !LINKSTO Xcp.ASAM.BITWISE_STIMULATION.002,1 */
...
#endif /* XCP_BSW_DISTRIBUTION_DAQSTIM_MEMORY_ACCESS_AREAS == STD_OFF */
    }
    else
    {
...
#endif
  {
    ...
    Xcp_DaqPrioQueueOfRequest.NoOfEntries[DaqListPtr->EventId]++;
...
FUNC(void, XCP_CODE) Xcp_ResetRequestData(void)
{
  ...
  for (EvIdx = 0U; EvIdx < XCP_MAX_EVENT_CHANNEL; EvIdx++)
  {
    ...
    {
      Xcp_RequestQueue.EventId[EvIdx] = XCP_INVALID_EVENT_CHANNEL;
    }
  }
...
{
  Xcp_RequestForEvent[EventId].EntriesForAppIdAvailable[AppId] = TRUE;
...
STATIC FUNC(boolean, XCP_CODE) Xcp_IsPosibleToEnqueueEvent(uint16 EventID)
{
  ...
  if ((Xcp_RequestQueue.WriteIdx == Xcp_RequestQueue.ReadIndex) &&
      (Xcp_RequestQueue.EventId[Xcp_RequestQueue.ReadIndex] != XCP_INVALID_EVENT_CHANNEL))
  {
    RetValue = FALSE;
  }
  else
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessQueuedEvents(void)
{
  ...
  if (EventId != XCP_INVALID_EVENT_CHANNEL)
  {
    /* enter critical section */
    SchM_Enter_Xcp_SCHM_XCP_EXCLUSIVE_AREA_XCP_INTERNALS();
...
#if(XCP_NUMBER_OF_QUEUED_EVENTS > 1U)
    /* update the read index in the Request Queue */
    if ((Xcp_RequestQueue.ReadIndex + 1U) < XCP_NUMBER_OF_QUEUED_EVENTS)
    {
    ...
    Xcp_RequestForEvent[EventId].RequestStatus = XCP_PROCESS_ON_MEMPROXY_REQUESTED;
    Xcp_RequestQueue.EventId[OldReadIdx] = XCP_INVALID_EVENT_CHANNEL;
    Xcp_MemoryProxySetNewState(XCP_MEMORY_PROXY_LOCKED);
...
STATIC FUNC(void, XCP_CODE) Xcp_ProcessLocalMemoryProxy(Xcp_MemoryProxyDataType* data)
{
  ...
  for (Idx = 0U; Idx < data->size; Idx++)
  {
    ...
    {
      data->RequestBuffer[ReadIdx].address2[0]  = 0U;
    }
    else if (data->RequestBuffer[ReadIdx].type == XCP_MEMORYPROXYDATA_SAMPLE_1)
    {
      data->RequestBuffer[ReadIdx].address2[0] = 1U;
    }
    else
    {
...
)
{
  ...
  Xcp_DaqPrioQueueIndexType QueueInsertPosition;
  /* Initialize the DAQ Prio Queue count to zero */
  Xcp_DaqPrioQueueIndexType DaqPrioQueueCount;
  /* The DAQ list to be sampled */
  P2VAR(Xcp_DaqType, AUTOMATIC, XCP_VAR_CLEARED) DaqListPtr = XCP_GET_DAQ_LIST_PTR(DaqId);
    /* current Ids*/
