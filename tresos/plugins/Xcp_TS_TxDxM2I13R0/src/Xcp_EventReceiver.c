...
#if (!((XCP_EVENT_TRIGGERED_MAINFUNC == STD_ON) && (XCP_MAX_MF_PROCESSED_EVENTS == 0U)))
...
FUNC(void, XCP_CODE) Xcp_SetCyclicEvents(void)
{
  ...
  if (Xcp_ConnectionStatus.State != XCP_STATE_DISCONNECTED)
  {
    ...
    /* Deviation TASKING-1 */
    for( EventChannelNumber = 0U; EventChannelNumber < XCP_MAX_CYCLIC_EVENT_CHANNEL; EventChannelNumber++ )
    {
