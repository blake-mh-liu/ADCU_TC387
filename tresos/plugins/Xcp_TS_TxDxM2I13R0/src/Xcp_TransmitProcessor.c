...
/** \brief data type containing information regarding tx LPdu */
typedef struct
{
...
#if ((XCP_ON_CAN_MAX_DLC_REQUIRED == STD_ON) || (XCP_ON_CANFD_ENABLED == STD_ON))
/** \brief Add fill bytes to the CAN or CAN-FD frame
 **
 ** \param[out] TxBufferPtr Pointer to the TxFrame, to which the header and tail shall be added.
 ...
 */
STATIC FUNC(void, XCP_CODE) Xcp_AddCanFill
(
  P2VAR(uint8, AUTOMATIC, XCP_APPL_DATA) TxBufferPtr,
 ...
 */
STATIC FUNC(void, XCP_CODE) Xcp_AddFlexRayMultiPduInFrameLastMessage
(
  PduLengthType AvailableTxSpace,
  uint8** TxBufferPtr,
  P2VAR(PduLengthType, AUTOMATIC, AUTOMATIC) TxPayloadSegmentSizePtr
);
...
FUNC(void, XCP_CODE) Xcp_InitTransmitProcessor(void)
{
  ...
  {
    if (XCP_IS_FLAG_BIT_SET(XCP_BUFFER_DIRECTION_TX_BIT, Xcp_FlxBufConfig[Iterator].Flags, uint8) == TRUE)
    {
...
)
{
  ...
  P2CONST(Xcp_ConnectionCfgType, AUTOMATIC, XCP_APPL_CONST) ConnectionCfgPtr;
  uint8 connectionId = Xcp_ConnectionStatus.ConnectionId;
...
#endif
  {
...
#endif /* (XCP_NO_OF_CONNECTIONS > 1U) */
    {
      ...
      /* !LINKSTO Xcp.SWS_Xcp_00849,1 */
      /* !LINKSTO Xcp.SWS_Xcp_00850,1 */
...
#endif
      {
        ...
        ConnectionCfgPtr = &Xcp_ConnectionCfg[connectionId];
        ...
        for (TxPduId = 0U; TxPduId < ConnectionCfgPtr->TxPduMax; TxPduId++)
        {
          ...
          if ((TRUE == XCP_IS_FLAG_BIT_SET(XCP_DYNAMIC_TX_PDU_BIT,
          ...
          else if (FALSE == XCP_IS_FLAG_BIT_SET(XCP_DYNAMIC_TX_PDU_BIT,
            Xcp_TxLPduIdConf[ConnectionCfgPtr->TxPduIdList[TxPduId]].Properties, uint8))
          {
            LocalReturnValue = E_OK;
          }
...
#endif /* (XCP_DYNAMIC_PDU_SUPPORTED == STD_ON) */
          {
...
#endif
            {
              ...
              Xcp_UpdateAvailableTxBuffers(TxPduId, ConnectionCfgPtr);
              /* send available Transmit Pdus */
              Xcp_TransmitAvailableTxPdus(SidId, TxPduId, ConnectionCfgPtr);
            }
          }
        }
      }
...
#if(XCP_NO_OF_CONNECTIONS > 1U)
#if(XCP_IMPLEMENT_MULTICAST_COMMANDS == STD_ON)
    if (XCP_IS_FLAG_BIT_SET(XCP_MULTICAST_TRANSMIT_MODE_BIT, Xcp_ConnectionStatus.Flags, uint8) == FALSE)
#endif
    {
...
)
{
  ...
  *IsDisconnectTriggered = FALSE;
  ...
  if (XCP_IS_FLAG_BIT_SET(XCP_AUTONOMOUS_DISCONNECT_BIT,
...
#if (XCP_IMPLEMENT_MULTICAST_COMMANDS == STD_ON)
    if ((XCP_IS_FLAG_BIT_SET(XCP_AUTONOMOUS_DISCONNECT_BIT, Xcp_ConnectionStatus.Flags, uint8) == FALSE) &&
      (XCP_IS_FLAG_BIT_SET(XCP_MULTICAST_TRANSMIT_MODE_BIT, Xcp_ConnectionStatus.Flags, uint8) == TRUE))
    {
...
#if(XCP_IMPLEMENT_MULTICAST_COMMANDS == STD_ON)
  if (XCP_IS_FLAG_BIT_SET(XCP_MULTICAST_TRANSMIT_MODE_BIT, Xcp_ConnectionStatus.Flags, uint8) == TRUE)
  {
...
)
{
  ...
  if ((TRUE == isTxIdle) &&
      (TRUE == XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_TX_FROM_CONFIRMATION_BIT,
...
#endif
    {
      P2CONST(Xcp_ConnectionCfgType, AUTOMATIC, XCP_APPL_CONST) ConnectionCfgPtr = &Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId];
      /* update buffers and trigger transmission for same PDU */
      Xcp_UpdateAvailableTxBuffers(IntPduId, ConnectionCfgPtr);
      /* send available Transmit Pdus */
      Xcp_TransmitAvailableTxPdus(XCP_SID_IF_TX_CONFIRMATION, IntPduId, ConnectionCfgPtr);
    }
  }
#endif
  ...
  /* CHECK: PARSE */
  {
    ...
    for (Iterator = 0U; Iterator < XCP_NO_OF_TX_PDU_IDS; Iterator++)
    {
      if (Xcp_TxLPduIdConf[Iterator].IntConnectionId == ConnectionId)
      {
        ...
        if ((TRUE == XCP_IS_FLAG_BIT_SET
                    (XCP_DYNAMIC_TX_PDU_BIT, Xcp_TxLPduIdConf[Iterator].Properties,uint8)) &&
            (TRUE == XCP_IS_FLAG_BIT_SET
                    (XCP_ENABLE_STATE_DYNAMIC_TX_PDU_BIT, Xcp_TxLPduIdConf[Iterator].Properties,uint8)))
        {
        ...
        {
#if (XCP_RESOURCE_DAQ == XCP_RESOURCE_DAQ_MASK)
          if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_DAQ_DTO_PDU_BIT, Xcp_TxLPduIdConf[Iterator].Properties, uint8) == TRUE)
          {
...
#endif
#if (XCP_EVENT_PACKET_ENABLED == STD_ON)
          if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_EVSERV_CTO_PDU_BIT, Xcp_TxLPduIdConf[Iterator].Properties, uint8) == TRUE)
          {
...
FUNC(Std_ReturnType, XCP_CODE) Xcp_SetDefaultLpduToBuffer(uint8 bufferId, uint8 bufferLength)
{
  ...
  if (XCP_IS_FLAG_BIT_SET(XCP_BUFFER_PRECONFIGURED_MASK_BIT, Xcp_FlxBufConfig[bufferId].Flags, uint8) == TRUE)
  {
...
STATIC FUNC(void, XCP_CODE) Xcp_MonitorFlexRayBuffers(void)
{
  ...
  if (FrIf_GetGlobalTime(0U, &FrIf_Cycle, &FrIf_MacroTick) == E_OK)
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_FLEXRAY_BUFFERS_READY_BIT, Xcp_ConnectionStatus.Flags, uint8) == FALSE)
    {
...
STATIC FUNC(uint16, XCP_CODE) Xcp_FlxCalculateHeaderCrc(uint16 FrameId, uint8 PayloadLength)
{
  ...
  HeaderCRC = Xcp_FlxCrcCalcNibble(HeaderCRC, (uint8)(Header & 0x0FU));
...
)
{
  ...
  if (XCP_IS_BYTE_FLAG_SET(IntTxPduId, Xcp_TxLPduData.IsBufferFullFlag) == FALSE)
  {
    ...
    if (E_OK == Xcp_PackTxBufferData(IntTxPduId, ConnectionCfgPtr))
    {
...
)
{
  ...
  if (PduTxStatus == E_OK)
  {
    ...
    if ((XCP_IS_BYTE_FLAG_SET(IntTxPduId, Xcp_TxLPduData.IsMulticastFlag) == TRUE) &&
      (XCP_IS_FLAG_BIT_SET(XCP_MULTICAST_TRANSMIT_MODE_BIT, Xcp_ConnectionStatus.Flags, uint8) ==
        FALSE))
    {
...
STATIC FUNC(boolean, XCP_CODE) Xcp_IsTxPacketSizeOk(uint8 HandlerId)
{
  ...
  {
    PduLengthType pduLength = Xcp_TxPduDataHandlers[HandlerId]->GetPduLength();
    if (pduLength <= Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].Xcp_Max_Dto)
    {
      PacketSizeOk = TRUE;
    }
    else
...
)
{
...
#if (XCP_ON_FLEXRAY_ENABLED == STD_ON) && (XCP_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON)
  PduLengthType handleFirstPacketAlignment = TRUE;
  PduLengthType FirstPacketSize = 0U;
#endif
  ...
  Xcp_InitFlexRayBuffer(TxBufferPtr, ConnectionCfgPtr);
#endif
...
#if (XCP_ON_FLEXRAY_ENABLED == STD_ON) && (XCP_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON)
        if ((ConnectionCfgPtr->ConnectionType == XCP_CONNECTION_OVER_FLEXRAY) &&
          (XCP_IS_FLAG_BIT_SET(XCP_MULTI_PDUS_IN_FRAME_BIT, ConnectionCfgPtr->Properties, uint8) == TRUE)
          )
        {
          if (FirstPacket == TRUE)
          {
            FirstPacketSize = PackageLength;
          }
          else
          {
            PduLengthType currentPacketTailSize = (ConnectionCfgPtr->FlexRayPackageAlignment - (PackageLength % ConnectionCfgPtr->FlexRayPackageAlignment)) % (ConnectionCfgPtr->FlexRayPackageAlignment);
            PduLengthType firstPacketTailSize = (ConnectionCfgPtr->FlexRayPackageAlignment - (FirstPacketSize % ConnectionCfgPtr->FlexRayPackageAlignment)) % (ConnectionCfgPtr->FlexRayPackageAlignment);
            PduLengthType sizeSoFar = MessageLength + currentPacketTailSize;
            if (handleFirstPacketAlignment == TRUE)
            {
              sizeSoFar += firstPacketTailSize;
            }
            if (AvailableTxSpace >= sizeSoFar)
            {
              if (handleFirstPacketAlignment == TRUE)
              {
                handleFirstPacketAlignment = FALSE;
                TS_MemSet(TxBufferPtr, XCP_RESERVED_BYTE, firstPacketTailSize);
                TxBufferPtr = &TxBufferPtr[firstPacketTailSize];
                TxPayloadSegmentSize += firstPacketTailSize;
                AvailableTxSpace -= firstPacketTailSize;
              }
              packetFits = TRUE;
              TS_MemSet(&TxBufferPtr[MessageLength], XCP_RESERVED_BYTE, currentPacketTailSize);
              MessageLength += currentPacketTailSize;
            }
            else
            {
              packetFits = FALSE;
    ...
    ((XCP_ON_FLEXRAY_ENABLED == STD_ON) && \
        ((XCP_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON) || (XCP_FLEXRAY_SEQUENCE_CORRECTION_SUPPORT == STD_ON ))))
            ...
            if ((ConnectionCfgPtr->ConnectionType == XCP_CONNECTION_OVER_CANFD) ||
              (ConnectionCfgPtr->ConnectionType == XCP_CONNECTION_OVER_CAN))
#endif
            {
              /* add the required FILL bytes */
              Xcp_AddCanFill(TxBufferPtr, &MessageLength, AvailableTxSpace);
            }
...
#if (XCP_NO_OF_CONNECTIONS > 1U)
            if (XCP_IS_FLAG_BIT_SET(XCP_MULTI_PDUS_IN_FRAME_BIT,
  ...
  if (RetVal == E_OK)
  {
    ...
    Xcp_AddFlexRayMultiPduInFrameLastMessage(AvailableTxSpace, &TxBufferPtr, &TxPayloadSegmentSize);
...
)
{
  ...
  if (Xcp_ConnectionCfg[Xcp_ConnectionStatus.ConnectionId].ConnectionType == XCP_CONNECTION_OVER_FLEXRAY)
  {
    if (PackageType == XCP_SUPPORT_FOR_RESERR_CTO_PDU)
    {
      ...
      retValue = XCP_IS_FLAG_MASK_SET(PackageType, Xcp_TxLPduIdConf[TxPduId].Properties, uint8);
    }
...
)
{
  ...
  case XCP_CONNECTION_OVER_FLEXRAY:
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_SEQUENCE_CORRECTION_BIT,
      ConnectionCfgPtr->Properties,
      uint8) == TRUE)
    {
...
)
{
  ...
  if (TxStatus == E_OK)
  {
    ...
    if (XCP_IS_FLAG_BIT_SET(XCP_SUPPORT_FOR_TRIGGER_TRANSMIT_BIT,
...
)
{
  ...
  if (XCP_IS_FLAG_BIT_SET(XCP_MAX_DLC_REQUIRED_BIT, ConnectionCfgPtr->Properties, uint8) == TRUE)
  {
      ...
      (AvailableFrameLength >= (ConnectionCfgPtr->MaxDlcValue)))
    {
      ...
      /* !LINKSTO Xcp.CAN.MAX_DLC.Fill,1 */
  ...
  else if (ConnectionCfgPtr->ConnectionType == XCP_CONNECTION_OVER_CANFD)
  {
    ...
    if (length >= 8U)
    {
      ...
      const PduLengthType ValidDlcValues[8U] = { 8U, 12U, 16U, 20U, 24U, 32U, 48U, 64U };
      /* local variable used to calculate the valid DLC */
      PduLengthType ValidLength = 0U;
      uint8 index;
      for (index = 1U; index < 8U; index++)
      {
        /* if the length is in the range ValidDlcValues..[index-1]ValidDlcValues[index],
...
#endif /* #if ((XCP_ON_CAN_MAX_DLC_REQUIRED == STD_ON) || (XCP_ON_CANFD_ENABLED == STD_ON)) */
    ...
    ConnectionCfgPtr->Properties, uint8) == TRUE)
  {
...
#endif
    {
        ...
        /* CHECK: PARSE */
      {
        ...
        *TxPayloadSegmentSizePtr = (*TxPayloadSegmentSizePtr) + 1U;
