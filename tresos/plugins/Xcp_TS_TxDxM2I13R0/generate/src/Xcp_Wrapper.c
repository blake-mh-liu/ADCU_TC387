...
[!IF "node:existsAndTrue(XcpGeneral/XcpServiceWrapper)"!][!//
...
FUNC(void, XCP_WRAPPER_CODE) Xcp_Wrapper_MainFunction_[!"as:name(.)"!](void)
{
  DBG_XCP_WRAPPER_MAINFUNCTION_[!"text:toupper(as:name(.))"!]_ENTRY();
  /* Check if Xcp module shall be called */
    /* !LINKSTO Xcp.Disable.Wrapper.Functionality,1 */
  if (XCP_MODULE_DISABLED != App_Xcp_Wrapper_IsXcpUsed())
  {
  ...
  DBG_XCP_WRAPPER_MAINFUNCTION_[!"text:toupper(as:name(.))"!]_EXIT();
}
    [!ENDIF!]
  [!ENDLOOP!]
[!ENDIF!]
...
[!IF "node:existsAndTrue(../../../../XcpGeneral/XcpEventTriggerMainFunc)"!][!//
  [!IF "(node:exists(../../XcpEventChannel)) and (num:i(count(../../XcpEventChannel/eb-list::*)) > 0)"!][!//
    [!LOOP "../../XcpEventChannel/eb-list::*"!][!//
      [!IF "./XcpEventChannelTimeCycle > 0"!][!//
        [!IF "node:existsAndTrue(../../../../XcpGeneral/XcpServiceWrapper)"!][!//
FUNC(void, XCP_1_[!"$XcpConnectionOsAppRefName"!]_CODE) Xcp_1_[!"$XcpConnectionOsAppRefName"!]_Wrapper_MainFunction_[!"as:name(.)"!](void)
{
  Xcp_Wrapper_MainFunction_[!"as:name(.)"!]();
}
        [!ENDIF!][!//
      [!ENDIF!][!//
    [!ENDLOOP!][!//
