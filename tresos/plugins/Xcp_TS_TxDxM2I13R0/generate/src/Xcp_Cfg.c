...
[!ENDLOOP!][!//
#endif
[!// Macro needs to be called only with container XcpConfig/eb-list::*[1] selected
...
[!IF "node:exists(XcpOdtEntryBitOffset)"!][!//
[!IF "$member='all'"!][!WS "4"!][!ENDIF!][!//
...
[!ENDLOOP!]
[!ELSE!][!//
[!IF "XcpDaqListNumber = number($XcpMinDaq) and $member='all'"!][!//
    /* -- ODT Entries for all static configurable DAQ lists -- */
[!ENDIF!][!//
[!FOR "x" = "1" TO "num:i(XcpMaxOdt)"!]
[!IF "(XcpMaxOdtEntries > number($XcpAbsoluteMaxOdtEntrySize)) and (node:existsAndTrue($XcpTimestampIsEnabled)) and ($x = 1)"!][!//
...
[!ENDLOOP!]
[!AUTOSPACING!][!//
[!IF "$XcpBswDistributionMemoryAccessAreas"!]
...
[!IF "$XcpBswDistributionDaqStimMemoryAccessAreas = 'true'"!][!//
...
[!IF "count(XcpEventChannel/eb-list::*/XcpEventChannelTriggeredDaqListRef/eb-list::*) > 0"!][!//
...
{
[!LOOP "util:distinct(node:order(XcpEventChannel/eb-list::*, 'XcpEventChannelNumber'))"!]
  /* Event channel: [!"XcpEventChannelNumber"!] */
  [!LOOP "util:distinct(node:order(XcpEventChannelTriggeredDaqListRef/eb-list::*, '255 - as:ref(.)/XcpDaqListPriority'))"!]
...
[!IF "count(XcpEventChannel/eb-list::*) > 0"!]
#define XCP_START_SEC_CONST_8
#include <Xcp_MemMap.h>

[!LOOP "util:distinct(node:order(XcpEventChannel/eb-list::*, 'XcpEventChannelNumber'))"!]
/*  Event name of Event channel number : [!"XcpEventChannelNumber"!]  */
STATIC CONST(uint8, XCP_CONST) Xcp_EventName_[!"XcpEventChannelNumber"!][[!"num:i(string-length(as:name(.)))"!]] = {[!/*
*/!][!FOR "i" = "1" TO "num:i(string-length(as:name(.)))"!][!/*
*/!]'[!"substring(as:name(.), $i, 1)"!]'[!/*
*/!][!IF "$i != num:i(string-length(as:name(.)))"!],[!ENDIF!][!/*
*/!][!ENDFOR!]};
[!ENDLOOP!]
...
{
[!LOOP "util:distinct(node:order(XcpConnectionCfg/eb-list::*,'XcpConnectionId'))"!]
  /* Tx Pdu configuration for the [!"as:name(.)"!] connection */
...
[!IF "$XcpMaxDaq > 0 and $XcpMaxDaq > $XcpDaqCount and $XcpNoOfNonDynamicODTEntries > 0 and node:existsAndTrue(../../RamOptimizations/XcpEnableDaqOptimization)"!][!//
#define XCP_START_SEC_CONST_8
#include <Xcp_MemMap.h>
...
[!IF "node:existsAndTrue(../../RamOptimizations/XcpEnableBitOffset)"!][!//
...
[!IF "node:existsAndTrue(../../XcpGeneral/XcpSupportForPDURoutingTable)"!]
/*-----------------------Xcp Pdu router Info--------------------------------*/
[!IF "(node:existsAndTrue(../../XcpGeneral/XcpOnCanEnabled)) or (node:existsAndTrue(../../XcpGeneral/XcpOnCanFDEnabled))"!]
...
{
[!LOOP "util:distinct(node:order(XcpPdu/eb-list::*[node:name(.)='XcpRxPdu'], 'XcpRxPduId'))"!][!//
...
{
[!LOOP "util:distinct(node:order(XcpPdu/eb-list::*[node:name(.)='XcpTxPdu'], 'XcpTxPduId'))"!][!//
  [!CALL "GetSymbolName","ShortNameRef"="'.'","OldName"="as:name(.)"!]
...
[!LOOP "util:distinct(node:order(XcpPdu/eb-list::*[node:name(.)='XcpTxPdu'], 'XcpTxPduId'))"!][!//
  ...
  [!VAR "TxPduName" = "as:name(node:ref(XcpTxPduRef))"!]
  [!VAR "TxPduId" = "XcpTxPduId"!]
  [!IF "node:existsAndTrue(XcpTxPduSupportForCddTriggerTransmit)"!]
    [!VAR "TxPduCddTriggerTransmit" = "'true'"!]
  [!ELSE!]
    [!VAR "TxPduCddTriggerTransmit" = "'false'"!]
  [!ENDIF!]
  [!LOOP "../../XcpConnectionCfg/eb-list::*/XcpTxPduConnectionInfo/eb-list::*[../../XcpConnectionInterfaceType = 'XcpConnectionOverCDD' and node:refvalid(XcpMapTxPdu2Connection) and (node:ref(XcpMapTxPdu2Connection)/XcpTxPduId = number($TxPduId))]"!]
...
[!LOOP "util:distinct(node:order(XcpPdu/eb-list::*[node:name(.)='XcpTxPdu'], 'XcpTxPduId'))"!][!//
  ...
  [!ELSE!]
    [!VAR "TxPduCddTriggerTransmit" = "'false'"!]
  [!ENDIF!]
  [!LOOP "../../XcpConnectionCfg/eb-list::*/XcpTxPduConnectionInfo/eb-list::*[node:refvalid(XcpMapTxPdu2Connection) and (node:ref(XcpMapTxPdu2Connection)/XcpTxPduId = number($TxPduId))]"!][!//
    [!CALL "GetTxPduInfo","ConnectionPath"="node:path(../..)","TxPduName"="$TxPduName","TxPduId"="$TxPduId", "TxPduCddTriggerTransmit"="$TxPduCddTriggerTransmit","TxPduConnectionInfoPath"="node:path(.)"!]
  { /* Mapping information for [!"$SymbolName"!] Pdu. */
    [!IF "(count(../../../eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCAN']) > 0) or (count(../../../eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCANFD']) > 0)"!]
      [!IF "$CanTxId != ''"!]
    ...
    [!"../../XcpConnectionId"!]U,  /* Xcp connection Id([!"as:name(.)"!]) */
...
[!LOOP "util:distinct(node:order(XcpPdu/eb-list::*[node:name(.)='XcpRxPdu'], 'XcpRxPduId'))"!][!//
  [!CALL "GetSymbolName","ShortNameRef"="'.'","OldName"="as:name(.)"!]
  [!VAR "RxPduId" = "XcpRxPduId"!]
  [!VAR "RxPduName" = "as:name(node:ref(XcpRxPduRef))"!]
  [!SELECT "../../XcpConnectionCfg/eb-list::*[count(XcpRxPduConnectionInfo/eb-list::*[node:refvalid(XcpMapRxPdu2Connection) and (node:ref(XcpMapRxPdu2Connection)/XcpRxPduId = number($RxPduId))]) > 0]"!]
    [!CALL "GetRxPduInfo","ConnectionPath"="node:path(.)","RxPduId"="$RxPduId","RxPduName"="$RxPduName"!]
  { /* Mapping information for [!"$SymbolName"!] Pdu. */
    [!IF "(count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCAN']) > 0) or (count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCANFD']) > 0)"!]
      [!IF "$CanRxId != ''"!]
...
{
  [!LOOP "util:distinct(node:order(XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverEthernet'],'XcpConnectionId'))"!]
    [!VAR "SelectedConnectionId" = "XcpConnectionId"!]
    [!VAR "SelectedConnectionName"="as:name(.)"!]
    [!VAR "TxPduName"="as:name(node:ref(node:ref(XcpTxPduConnectionInfo/eb-list::*[1]/XcpMapTxPdu2Connection)/XcpTxPduRef))"!]
    [!IF "node:refvalid(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)"!]
      [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
        [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef) and (node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../../SoAdSocketProtocol = 'SoAdSocketTcp')]"!]
          [!SELECT "node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
            [!VAR "SoAdConnectionName" = "concat(as:name(../..),'_',as:name(.))"!]
  {
    ...
    [!"$SelectedConnectionId"!]U, /* Xcp connection Id([!"$SelectedConnectionName"!]) */
         ...
         [!ENDSELECT!]
       [!ENDSELECT!]
      [!ELSEIF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)"!]
...
[!LOOP "util:distinct(node:order(XcpConnectionCfg/eb-list::*,'XcpConnectionId'))"!]
  ...
  [!VAR "TxPduName"="as:name(node:ref(node:ref(XcpTxPduConnectionInfo/eb-list::*[1]/XcpMapTxPdu2Connection)/XcpTxPduRef))"!]
  [!VAR "RxPduName"="as:name(node:ref(node:ref(XcpRxPduConnectionInfo/eb-list::*[1]/XcpMapRxPdu2Connection)/XcpRxPduRef))"!]
  ...
  [!ELSEIF "XcpConnectionInterfaceType = 'XcpConnectionOverEthernet'"!]
    [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
      [!IF "count(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef) and (node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../../SoAdSocketProtocol = 'SoAdSocketTcp')]) > 0"!]
        [!VAR "ConnectionType"="'TCPIP'"!] /* TCP */
      [!ELSEIF "count(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef) and (node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../../SoAdSocketProtocol = 'SoAdSocketUdp')]) > 0"!]
        [!VAR "ConnectionType"="'UDPIP'"!] /* UDP */
      [!ENDIF!]
    [!ELSEIF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)"!]
      [!IF "count(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef) and (node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/../../SoAdSocketProtocol = 'SoAdSocketTcp')]) > 0"!]
        [!VAR "ConnectionType"="'TCPIP'"!]
      [!ELSEIF "count(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef) and (node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/../../SoAdSocketProtocol = 'SoAdSocketUdp')]) > 0"!]
  ...
  [!ELSEIF "XcpConnectionInterfaceType = 'XcpConnectionOverCDD'"!]
    ...
    [!VAR "TransmitFunction"="concat('&',XcpConnectionInterfaceType/eb-choice::*[1]/XcpCddInformation/XcpCddTransmitFunctionName)"!]
  [!ENDIF!]
  ...
  [!IF "$MaxDlcRequired = 'true'"!]
    [!VAR "ConnectionFlags"="concat($ConnectionFlags,'XCP_MAX_DLC_REQUIRED_FLAG',' ')"!]
  [!ENDIF!]
  /* configuration information for [!"as:name(.)"!] connection(Xcp over [!"$ConnectionType"!]). */
  {
  ...
  [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverCANFD' and node:existsAndTrue(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdMaxDlcRequired)"!]
    [!"XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdMaxDlc"!]U, /* PDU maximum length */
  [!ELSE!]
    [!"num:i($XcpPackagePduSize)"!]U, /* PDU maximum length */
  [!ENDIF!]
    [!"num:i($XcpPackageHeaderSize)"!]U, /* PDU header size */
    [!"num:i(count(XcpTxPduConnectionInfo/eb-list::*))"!]U, /* number of mapped Tx PDUs towards connection */
    [!"num:i(count(XcpRxPduConnectionInfo/eb-list::*))"!]U, /* number of mapped Rx PDUs towards connection */
  [!IF "(count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCAN']) > 0) or (count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCANFD']) > 0) or (count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCDD']) > 0) "!]
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverCAN'"!]
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCtoSlavePduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive all CMD's but GET_SLAVE_ID. */
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpBroadcastPduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive the broadcasted GET_SLAVE_ID CMD. */
    [!ELSEIF "XcpConnectionInterfaceType = 'XcpConnectionOverCANFD'"!]
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdCtoSlavePduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive all CMD's but GET_SLAVE_ID. */
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdBroadcastPduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive the broadcasted GET_SLAVE_ID CMD. */
    [!ELSEIF "XcpConnectionInterfaceType = 'XcpConnectionOverCDD'"!]
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCddCtoSlavePduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive all CMD's but GET_SLAVE_ID. */
    [!"node:ref(node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCddBroadcastPduRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive the broadcasted GET_SLAVE_ID CMD. */
    [!ELSE!]
    ...
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverCAN' or XcpConnectionInterfaceType = 'XcpConnectionOverCANFD' or XcpConnectionInterfaceType = 'XcpConnectionOverCDD'"!]
      [!IF "(node:existsAndTrue(../../../../XcpGeneral/XcpStimSupported)) and (node:existsAndTrue(../../../../XcpGeneral/XcpSupportForGetDaqId))"!]
    [!"node:ref(XcpRxPduConnectionInfo/eb-list::*[node:existsAndTrue(XcpPduSupportForStimDtoType)]/XcpMapRxPdu2Connection)/XcpRxPduId"!]U, /* Xcp PDU channel, used to receive STIM DTOs. */
      [!ENDIF!]
    [!ELSE!]
      [!IF "(node:existsAndTrue(../../../../XcpGeneral/XcpStimSupported)) and (node:existsAndTrue(../../../../XcpGeneral/XcpSupportForGetDaqId))"!]
    ...
    [!ENDIF!]
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverCAN' or XcpConnectionInterfaceType = 'XcpConnectionOverCANFD' or XcpConnectionInterfaceType = 'XcpConnectionOverCDD'"!]
      [!IF "(node:existsAndTrue(../../../../XcpGeneral/XcpDaqSupported)) and (node:existsAndTrue(../../../../XcpGeneral/XcpSupportForGetDaqId))"!]
    [!"node:ref(XcpTxPduConnectionInfo/eb-list::*[node:existsAndTrue(XcpPduSupportForDaqDtoType)]/XcpMapTxPdu2Connection)/XcpTxPduId"!]U, /* Xcp PDU channel, used to transmit Daq DTOs. */
      [!ENDIF!]
    [!ELSE!]
      [!IF "(node:existsAndTrue(../../../../XcpGeneral/XcpDaqSupported)) and (node:existsAndTrue(../../../../XcpGeneral/XcpSupportForGetDaqId))"!]
    ...
    [!ENDIF!]
  [!ENDIF!]
  [!IF "count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverEthernet']) > 0"!]
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverEthernet'"!]
      [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
        [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and (count(SoAdPduRouteDest/eb-list::*) > 0)]"!]
          [!IF "not(node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
            [!ERROR "Invalid value for node SoAdTxSocketConnectionRef: Invalid reference."!]
          [!ENDIF!]
          [!VAR "SoAdConnectionName" = "as:name(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef))"!]
          [!VAR "SoAdConnectionGroupName" = "as:name(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../..)"!]
    [!"node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/SoAdSocketId"!]U, /* Tx socket connection Id([!"$SoAdConnectionGroupName"!]_[!"$SoAdConnectionName"!]) */
        [!ENDSELECT!]
      [!ELSEIF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)"!]
        [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and (count(SoAdPduRouteDest/eb-list::*) > 0)]"!]
          ...
          [!ENDIF!]
          [!VAR "SoAdConnectionName" = "as:name(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef))"!]
          [!VAR "SoAdConnectionGroupName" = "as:name(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/../..)"!]
    [!"node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/SoAdSocketId"!]U, /* Tx socket connection Id([!"$SoAdConnectionGroupName"!]_[!"$SoAdConnectionName"!]) */
        [!ENDSELECT!]
      [!ENDIF!]
      [!IF "$SuportForSoAdSocketProtocolUdp = 'true'"!]
        [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdSocketRoute/eb-list::*/SoAdRxSocketConnectionRef)"!]
          [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdSocketRoute/eb-list::*[(count(SoAdSocketRouteDest/eb-list::*) > 0) and node:refvalid(SoAdSocketRouteDest/eb-list::*/SoAdRxPduRef) and (as:name(node:ref(SoAdSocketRouteDest/eb-list::*/SoAdRxPduRef)) = string($RxPduName))]"!]
            [!IF "not(node:refvalid(SoAdRxSocketConnectionRef))"!]
              [!ERROR "Invalid value for node SoAdRxSocketConnectionRef: Invalid reference."!]
            [!ENDIF!]
            [!VAR "SoAdConnectionName" = "as:name(node:ref(SoAdRxSocketConnectionRef))"!]
            [!VAR "SoAdConnectionGroupName" = "as:name(node:ref(SoAdRxSocketConnectionRef)/../..)"!]
    [!"node:ref(SoAdRxSocketConnectionRef)/SoAdSocketId"!]U, /* Rx socket connection Id([!"$SoAdConnectionGroupName"!]_[!"$SoAdConnectionName"!]) */
          [!ENDSELECT!]
        [!ELSEIF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdSocketRoute/eb-list::*/SoAdRxSocketConnOrSocketConnBundleRef)"!]
          [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdSocketRoute/eb-list::*[(count(SoAdSocketRouteDest/eb-list::*) > 0) and node:refvalid(SoAdSocketRouteDest/eb-list::*/SoAdRxPduRef) and (as:name(node:ref(SoAdSocketRouteDest/eb-list::*/SoAdRxPduRef)) = string($RxPduName))]"!]
  ...
  [!IF "count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverEthernet']) > 0"!]
    ...
    [!ENDIF!]
  [!ENDIF!]   
  [!IF "count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverFlexRay']) > 0"!]
    [!IF "(XcpConnectionInterfaceType = 'XcpConnectionOverFlexRay') and (node:existsAndTrue($SequenceCorrection))"!]
      [!VAR "ConnectionHasCounter" = "'true'"!]
    [!ENDIF!]
  [!ENDIF!]
  ...
  [!ELSE!]
    XCP_IGNORED_CNT_ID,  
  [!ENDIF!]
  [!IF "(count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCANFD']) > 0) or (count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverCAN' and node:existsAndTrue(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanMaxDlcRequired)]) > 0)"!]
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverCANFD'"!]
    [!"XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdMaxDlc"!]U,  /* MAX_DLC value */
    [!"XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFdFillValue"!]U,  /* Fill byte value */
    [!ELSEIF "XcpConnectionInterfaceType = 'XcpConnectionOverCAN'"!]
    8U,  /* MAX_DLC value */
    [!IF "node:existsAndTrue(XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanMaxDlcRequired)"!]
    [!"XcpConnectionInterfaceType/eb-choice::*[1]/XcpCanFillValue"!]U,  /* Fill byte value */
    [!ELSE!]
    0U, /* Fill byte value */
    [!ENDIF!]
    [!ELSE!]
  ...
  [!IF "count(../../XcpConnectionCfg/eb-list::*[XcpConnectionInterfaceType = 'XcpConnectionOverFlexRay']) > 0"!]
    [!IF "XcpConnectionInterfaceType = 'XcpConnectionOverFlexRay'"!]
    [!"XcpConnectionInterfaceType/eb-choice::*[1]/XcpFlxNodeAddress"!]U, /* configured NAX address for the slave */
...
[!IF "node:existsAndTrue($XcpTimestampIsEnabled)"!]
    [!"text:toupper(XcpTimestampEnabled)"!], /* flag indicating if DTO timestamps are enabled for this connection */
[!ENDIF!]
[!IF "node:existsAndTrue(../../../../XcpGeneral/XcpMasterBlockModeSupported)"!]
...
[!IF "../../../../XcpGeneral/XcpMasterBlockModePgmSupported"!]
    ...
    [!ENDIF!]
    [!//
    [!IF "XcpMaxBSPgm < number($XcpMaxBSPgmVar)"!]
...
[!ENDIF!]
[!ENDIF!] /* node:existsAndTrue(XcpPgmSupported) */
...
[!IF "node:existsAndTrue(../../../../XcpGeneral/XcpStimSupported)"!]
    ...
    [!ENDIF!]
[!ENDIF!]
[!ENDIF!] /* node:existsAndTrue(XcpDaqSupported) */
  },
  [!VAR "TxPduIndex" = "$TxPduIndex + count(XcpTxPduConnectionInfo/eb-list::*)"!]
[!ENDLOOP!]
};

[!VAR "XcpEventChannelMFEnabled"="(node:existsAndTrue(../../XcpGeneral/XcpEventTriggerMainFunc))"!]
...
[!VAR "Counter" = "$Counter + count(XcpEventChannelTriggeredDaqListRef/eb-list::*)"!]
[!ENDLOOP!]
};
...
[!IF "$XcpNoOfNonDynamicODTEntries > 0 and node:existsAndTrue(../../RamOptimizations/XcpEnableDaqOptimization)"!][!//
...
CONST(Xcp_NonDynamicDaqListsType, XCP_CONST) Xcp_DaqListsDefault =
{
  ...
  [!LOOP "util:distinct(node:order(XcpDaqList/eb-list::*, 'XcpDaqListNumber'))"!]
    ...
    /* DaqList configuration for the DaqListId = [!"num:i(XcpDaqListNumber)"!] */
    {
      ...
      [!VAR "XcpDaqListName" = "as:name(.)"!][!//
      [!VAR "XcpEventPresent" = "0"!]
      [!VAR "XcpDirectionSet" = "0"!]
      [!LOOP "../../XcpEventChannel/eb-list::*/XcpEventChannelTriggeredDaqListRef/eb-list::*[as:name(as:ref(.)) = $XcpDaqListName]"!]
      ...
      [!IF "(node:exists(./XcpDaqListPriority))"!]
      ...
      [!VAR "ODTEntryCnt" = "0"!]
      [!LOOP "./XcpOdt/eb-list::*"!]
        [!VAR "ODTEntryCnt" = "count(./XcpOdtEntry/eb-list::*)"!]
      ...
      [!"./XcpMaxOdtEntries"!]U,
      [!ENDIF!]
      [!IF "(node:exists(./XcpDaqListPrescaler))"!]
      ...
      [!IF "(XcpDaqListNumber < number($XcpMinDaq))"!]
      XCP_DAQ_LIST_DIRECTION_[!"XcpDaqListType"!][!IF "node:existsAndTrue(./XcpDaqEventFixed)"!] | XCP_MASK_DAQLIST_EVENT_FIXED[!ENDIF!] [!IF "XcpDaqListNumber < number($XcpMinDaq)"!] | XCP_DAQ_LIST_PREDEFINED[!ENDIF!],
      [!ELSE!]
      ...
      [!IF "(XcpDaqListNumber < number($XcpMinDaq)) and ($XcpEventPresent = 1) and ($XcpDirectionSet = 1)"!]
  ...
  [!IF "XcpDaqListNumber < number($XcpMinDaq)"!]
    [!VAR "CounterODT" = "$CounterODT + count(XcpOdt/eb-list::*)"!]
  [!ELSE!]
  ...
  [!LOOP "util:distinct(node:order(XcpDaqList/eb-list::*, 'XcpDaqListNumber'))"!]
    ...
    /* --- ODT configuration list for the DaqListId = [!"num:i(XcpDaqListNumber)"!] --- */
      [!LOOP "util:distinct(node:order(XcpOdt/eb-list::*, 'XcpOdtNumber'))"!]
    /* ODT configuration for the OdtID = [!"num:i(XcpOdtNumber)"!][!IF "(node:existsAndTrue($XcpTimestampIsEnabled)) and (XcpOdtNumber = 0)"!] (the resulting DTO for this ODT contains [!"num:i($XcpTimestampSize)"!] byte[!IF "$XcpTimestampSize > 1"!]s[!ENDIF!][!CR!]
     *                                      with the time stamp data) [!ENDIF!]*/
    {
      /* Pointer to OdtEntry array or Object Descriptor table */
[!IF "node:existsAndFalse(as:modconf('Xcp')[1]/RamOptimizations/XcpEnableDaqOptimization)"!][!//
...
[!ENDIF!][!//
      ...
      [!"num:i(count(XcpOdtEntry/eb-list::*))"!]U
    },
      [!VAR "CounterODTEntry" = "$CounterODTEntry + count(XcpOdtEntry/eb-list::*)"!]
    ...
    /* --- ODT configuration list for the DaqListId = [!"num:i(XcpDaqListNumber)"!] --- */
      [!FOR "x" = "1" TO "num:i(XcpMaxOdt)"!]
    /* ODT configuration for the OdtID = [!"num:i($x - 1)"!][!IF "(node:existsAndTrue($XcpTimestampIsEnabled)) and ($x = 1)"!] (the resulting DTO for this ODT contains [!"num:i($XcpTimestampSize)"!] byte[!IF "$XcpTimestampSize > 1"!]s[!ENDIF!][!CR!]
     *                                      with the time stamp data) [!ENDIF!]*/
       [!IF "(XcpMaxOdtEntries > number($XcpAbsoluteMaxOdtEntrySize)) and (node:existsAndTrue($XcpTimestampIsEnabled)) and ($x = 1)"!]
         [!VAR "XcpRelativeOdtEntrySize" = "$XcpAbsoluteMaxOdtEntrySize"!]
       [!ELSE!]
...
[!IF "count(../../XcpUserCommand/eb-list::*) > 0"!]
...
{
  [!LOOP "util:distinct(node:order(../../XcpUserCommand/eb-list::*, 'XcpSubCommandCode'))"!]
  {
    ...
    [!IF "node:exists(./XcpSubCommandSeedSupport)"!]
...
[!IF "num:i(count(XcpConnectionCfg/eb-list::*/XcpConnectionInterfaceType/eb-choice::*[1]/XcpFlexrayBufferCfg/eb-list::*)) > 0"!]

/** \brief Configured Flexray Buffers */
CONST(Xcp_FlxBufType, XCP_CONST) Xcp_FlxBufConfig[XCP_NO_OF_FLX_BUF_CONFIGURED] =
{
[!LOOP "util:distinct(node:order(XcpConnectionCfg/eb-list::*/XcpConnectionInterfaceType/eb-choice::*[1]/XcpFlexrayBufferCfg/eb-list::*, 'FLX_BUFCfg/XcpFlxBufId'))"!]
[!VAR "XcpBufferFlags" = "''"!]
[!VAR "XcpBufferFlagsAtLeastOneIsSet" = "'false'"!]
[!IF "node:refvalid(FLX_BUFCfg/XcpFrIfLPDURef) and (count(node:ref(node:ref(node:ref(node:ref(FLX_BUFCfg/XcpFrIfLPDURef)/FrIfVBTriggeringRef)/FrIfFrameStructureRef)/FrIfPdusInFrame/eb-list::*[1]/FrIfPduRef)/FrIfPduDirection/FrIfTxPduRef) > 0)"!]
  [!VAR "XcpBufferFlags" = "concat($XcpBufferFlags,'XCP_BUFFER_DIRECTION_TX_MASK',' ')"!]
  [!VAR "XcpBufferFlagsAtLeastOneIsSet" = "'true'"!]  
[!ENDIF!]
[!IF "((node:exists(FLX_BUFCfg/XcpFlxBufMaxLenInitValue)) and (node:exists(LPDU_IDCfg/XcpFlxSlotIdInitValue)) and (node:exists(LPDU_IDCfg/XcpFlxOffsetInitValue))and (node:exists(LPDU_IDCfg/XcpFlxCycleRepetitionInitValue))and (node:exists(LPDU_IDCfg/XcpFlxChannelInitValue)))"!]
...
[!VAR "pduIdxToCompare" = "as:ref(as:ref(FLX_BUFCfg/XcpFlxRxConnectionInfoRef)/XcpMapRxPdu2Connection)/XcpRxPduId"!]
  [!LOOP "util:distinct(node:order(../../../../../XcpPdu/eb-list::*[node:name(.)='XcpRxPdu'], 'XcpRxPduId'))"!]
      [!IF "number($pduIdxToCompare) = XcpRxPduId"!]
...
[!VAR "pduIdxToCompare" = "as:ref(as:ref(FLX_BUFCfg/XcpFlxTxConnectionInfoRef)/XcpMapTxPdu2Connection)/XcpTxPduId"!]
  [!LOOP "util:distinct(node:order(../../../../../XcpPdu/eb-list::*[node:name(.)='XcpTxPdu'], 'XcpTxPduId'))"!]
      [!IF "number($pduIdxToCompare) = XcpTxPduId"!]
        [!VAR "pduIdxToWrite" = "$pduIdxCount"!]
      [!ENDIF!]
      [!VAR "pduIdxCount" = "$pduIdxCount + 1"!]
  [!ENDLOOP!]
...
[!IF "(node:exists(LPDU_IDCfg/XcpFlxSlotIdInitValue))"!]
...
[!IF "(node:exists(LPDU_IDCfg/XcpFlxOffsetInitValue))"!]
...
[!IF "(node:exists(LPDU_IDCfg/XcpFlxCycleRepetitionInitValue))"!]
...
[!IF "(node:exists(LPDU_IDCfg/XcpFlxChannelInitValue))"!]
...
[!IF "num:i(count(XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*)) > 0"!]
...
{
[!LOOP "util:distinct(node:order(XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*))"!]
  {
...
[!LOOP "util:distinct(node:order(XcpConnectionCfg/eb-list::*,'XcpConnectionId'))"!]
  [!IF "node:refvalid(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)"!]
    ...
    [!VAR "IsIPv6" = "'false'"!]
    [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)"!]
      [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef) and node:refvalid(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../../SoAdSocketLocalAddressRef) and node:ref(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnectionRef)/../../SoAdSocketLocalAddressRef)/TcpIpDomainType = 'TCPIP_AF_INET6']"!]
        [!VAR "IsIPv6" = "'true'"!]
        [!BREAK!]
      [!ENDSELECT!]
    [!ENDIF!]
    [!IF "node:exists(as:modconf('SoAd')/SoAdConfig/eb-list::*/SoAdPduRoute/eb-list::*/SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)"!]
      [!SELECT "node:ref(XcpConnectionInterfaceType/eb-choice::*[1]/XcpConnectionSoAdConfigRef)/SoAdPduRoute/eb-list::*[node:refvalid(SoAdTxPduRef) and (as:name(node:ref(SoAdTxPduRef)) = string($TxPduName)) and node:refvalid(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef) and node:refvalid(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/../../SoAdSocketLocalAddressRef) and node:ref(node:ref(SoAdPduRouteDest/eb-list::*/SoAdTxSocketConnOrSocketConnBundleRef)/../../SoAdSocketLocalAddressRef)/TcpIpDomainType  = 'TCPIP_AF_INET6']"!]
        [!VAR "IsIPv6" = "'true'"!]
        [!BREAK!]
      [!ENDSELECT!]
    [!ENDIF!]
    ...
    [!ENDIF!]
  [!ENDIF!]
  [!IF "(num:mod(XcpConnectionId, 8.0)[1] = 7.0) and (XcpConnectionId < number($NoOfConnections - 1.0))"!]
  [!"$IPv6Flag"!]U,
    [!VAR "IPv6Flag" = "'0'"!]
  [!ENDIF!]
[!ENDLOOP!]
...
[!IF "(count(XcpEventChannel/eb-list::*) > 0) and ((count(XcpEventChannel/eb-list::*[XcpEventChannelTimeCycle = 0]) > 0) or (node:existsAndFalse(../../XcpGeneral/XcpEventTriggerMainFunc)))"!]
...
{
[!LOOP "util:distinct(node:order(XcpEventChannel/eb-list::*,'number(255 - XcpEventChannelPriority)','XcpEventChannelNumber'))"!]
  [!IF "(node:existsAndFalse(../../../../XcpGeneral/XcpEventTriggerMainFunc)) or (XcpEventChannelTimeCycle = 0)"!]
  [!"XcpEventChannelNumber"!]U,  /* event [!"as:name(.)"!] */
  [!ENDIF!]
[!ENDLOOP!]
};
[!ENDIF!]
...
{
[!LOOP "XcpEventChannel/eb-list::*[XcpEventChannelTimeCycle > 0]"!]
  [!"XcpEventChannelNumber"!]U,  /* event [!"as:name(.)"!] */
[!ENDLOOP!]
};
[!ENDIF!]
...
[!IF "($XcpMaxEventChannel > 0) and (num:i(sum(XcpEventChannel/eb-list::*/XcpEventChannelMaxDaqList)) > 0)"!]
...
{
[!LOOP "util:distinct(node:order(XcpEventChannel/eb-list::*, 'XcpEventChannelNumber'))"!]
  /* Daq list configuration for [!"as:name(.)"!]. */
  [!IF "XcpEventChannelMaxDaqList > 0"!]
    [!LOOP "util:distinct(node:order(XcpEventChannelTriggeredDaqListRef/eb-list::*, '255 - node:ref(.)/XcpDaqListPriority'))"!]
  [!"as:ref(.)/XcpDaqListNumber"!]U,
    [!ENDLOOP!]
    [!IF "count(./XcpEventChannelTriggeredDaqListRef/eb-list::*) < XcpEventChannelMaxDaqList"!]
      [!FOR "x" = "1" TO "num:i(XcpEventChannelMaxDaqList - count(./XcpEventChannelTriggeredDaqListRef/eb-list::*))"!]
  XCP_DUMMY_DAQ_LIST,
      [!ENDFOR!]
   [!ENDIF!]
  [!ENDIF!]
...
Xcp_SendMemoryProxyData_FuncPtr Xcp_SendMemoryProxyData[[!"$XcpNumOsApps"!]][[!"$XcpNumOsApps"!]] = {
  [!LOOP "(as:modconf('Os')[1]/OsApplication/eb-list::*)"!][!//
  {
    ...
    [!LOOP "(as:modconf('Xcp')[1]/XcpConfig/eb-list::*[1]/XcpConnectionCfg/eb-list::*)"!][!//
      [!IF "as:name(as:ref(./XcpConnectionApplicationRef)) = $PartitionName"!][!//
        [!VAR "NeedsEntry" = "'true'"!][!//
        [!BREAK!][!//
      [!ENDIF!][!//
    [!ENDLOOP!][!//
    [!IF "$NeedsEntry = 'true'"!]
      [!FOR "Index" = "0" TO "$XcpNumOsApps - 1"!][!//
        [!VAR "NeedsEntry" = "'false'"!][!//
        [!LOOP "(as:modconf('Xcp')[1]/XcpConfig/eb-list::*[1]/XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*)"!][!//
          [!VAR "XcpReceivingApplication" = "as:name(as:ref(./XcpMemoryAccessAreaApplicationRef))"!][!//
          [!IF "(node:pos(node:ref(./XcpMemoryAccessAreaApplicationRef))) = number($Index)"!][!//
...
Xcp_SendMemoryProxyNotification_FuncPtr Xcp_SendMemoryProxyNotification[[!"$XcpNumOsApps"!]][[!"$XcpNumOsApps"!]] = {
  [!LOOP "(as:modconf('Os')[1]/OsApplication/eb-list::*)"!][!//
  {
    ...
    [!VAR "PartitionName" = "as:name(.)"!][!//
    [!LOOP "(as:modconf('Xcp')[1]/XcpConfig/eb-list::*[1]/XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*)"!][!//
      [!IF "as:name(as:ref(./XcpMemoryAccessAreaApplicationRef)) = $PartitionName"!][!//
        [!VAR "NeedsEntry" = "'true'"!][!//
      [!ENDIF!][!//
    [!ENDLOOP!][!//
    [!IF "$NeedsEntry = 'true'"!]
      [!FOR "Index" = "0" TO "$XcpNumOsApps - 1"!][!//
        [!VAR "NeedsEntry" = "'false'"!][!//
        [!LOOP "(as:modconf('Xcp')[1]/XcpConfig/eb-list::*[1]/XcpConnectionCfg/eb-list::*)"!][!//
          [!VAR "XcpReceivingApplication" = "as:name(as:ref(./XcpConnectionApplicationRef))"!][!//
          [!IF "(node:pos(node:ref(./XcpConnectionApplicationRef))) = number($Index)"!][!//
            [!IF "not(contains($XcpReceivingApplicationList,concat(' ',$XcpReceivingApplication,' ')))"!][!//
              [!IF "not(contains($PartitionName,$XcpReceivingApplication))"!][!//
...
VAR(Xcp_DescriptorCalloutType, XCP_VAR) Xcp_DescriptorCalloutTypePtr =
{
...
[!IF "$XcpNoOfNonDynamicODTEntries > 0 and node:existsAndTrue(../../RamOptimizations/XcpEnableDaqOptimization)"!][!//
#define XCP_START_SEC_VAR_SAVED_ZONE_8
#include <Xcp_MemMap.h>
...
#define XCP_STOP_SEC_VAR_SAVED_ZONE_8
#include <Xcp_MemMap.h>
[!ENDIF!][!//
...
[!LOOP "util:distinct(node:order(XcpEventChannel/eb-list::*, 'XcpEventChannelNumber'))"!]
  ...
  [!IF "XcpEventChannelMaxDaqList > 0"!]
    /* The number of DAQ Lists of this event */
    [!"num:i(count(./XcpEventChannelTriggeredDaqListRef/eb-list::*))"!],
  [!ELSE!]
     /* The number of DAQ Lists of this event */
     0U,
  [!ENDIF!]
...
#define XCP_START_SEC_VAR_INIT_CONTROL_API_8
#include <Xcp_MemMap.h>

[!SELECT "../../XcpGeneral"!]
VAR(uint8, XCP_VAR) Xcp_ControlStateOfXcpModule =          [!//
[!IF "(node:existsAndTrue(XcpDefaultXcpModuleState))"!]
XCP_STATE_ACTIVE;
[!ENDIF!]
[!IF "(node:existsAndFalse(XcpDefaultXcpModuleState))"!]
XCP_STATE_INACTIVE;
[!ENDIF!]
[!ENDSELECT!]

#define XCP_STOP_SEC_VAR_INIT_CONTROL_API_8
#include <Xcp_MemMap.h>
...
[!VAR "XcpReceivingApplicationList" = "''"!]
[!LOOP "(XcpConnectionCfg/eb-list::*)"!]
  [!VAR "XcpSendingApplicationIterator" = "as:name(as:ref(./XcpConnectionApplicationRef))"!]
...
FUNC(void, XCP_1_XcpMemoryProxy_[!"$XcpReceivingApplication"!]_CODE) Xcp_1_XcpMemoryProxy_[!"$XcpReceivingApplication"!]_MemoryProxyHandler (void)
{
  ...
  {
  [!LOOP "text:split($XcpConnectionOsAppRefNameList, ' ')"!]
    [!IF ". != string($XcpReceivingApplication)"!]
    case [!"."!]:
...
[!VAR "XcpReceivingApplicationList" = "''"!]
[!LOOP "(XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*)"!]
  [!VAR "XcpSendingApplicationIterator" = "as:name(as:ref(./XcpMemoryAccessAreaApplicationRef))"!]
  [!IF "not(contains($XcpSendingApplicationIteratorList,concat(' ',$XcpSendingApplicationIterator,' ')))"!]

    [!VAR "XcpMemProxyOsAppRefNameList" = "''"!]
    [!LOOP "(../../../XcpConnectionCfg/eb-list::*)"!]
      [!VAR "XcpMemProxyOsAppRefName"="as:name(as:ref(./XcpConnectionApplicationRef))"!]
      [!IF "not(contains($XcpMemProxyOsAppRefNameList,concat(' ',$XcpMemProxyOsAppRefName,' ')))"!]
        [!VAR "XcpReceivingApplication" = "as:name(as:ref(./XcpConnectionApplicationRef))"!]
        [!IF "not(contains($XcpReceivingApplicationList,concat(' ',$XcpReceivingApplication,' ')))"!]
        [!VAR "XcpSendingApplicationList" = "''"!]
        [!LOOP "(../../XcpMemoryAccessArea/XcpMemoryAccessArea/eb-list::*)"!]
          [!VAR "XcpSendingApplication" = "as:name(as:ref(./XcpMemoryAccessAreaApplicationRef))"!]
...
[!IF "node:existsAndTrue(RamOptimizations/XcpEnableDaqOptimization) and ($XcpNoOfNonDynamicODTEntries > 0 or XcpGeneral/XcpDaqConfigType = 'DAQ_DYNAMIC')"!][!//
  ...
  else
  {
    ...
    switch(data.type)
    {
      case XCP_REMOTE_DISPATCH_SETEVENT:
        ...
        break;
[!IF "node:existsAndTrue(XcpGeneral/XcpSuppressTxSupport)"!]
      case XCP_REMOTE_DISPATCH_SETTRANSMISSIONMODE:
        sid = XCP_SID_SET_TRANSMISSION_MODE;
        break;
[!ENDIF!]
[!IF "node:existsAndTrue(XcpGeneral/XcpEnableXcpControlApi)"!]
...
[!IF "node:existsAndTrue(XcpGeneral/XcpEnableXcpControlApi)"!]
      case XCP_REMOTE_DISPATCH_CONTROL:
        ...
        break;
[!ENDIF!]
[!IF "(node:existsAndTrue(XcpGeneral/XcpCalPagStoreSupported))"!]
