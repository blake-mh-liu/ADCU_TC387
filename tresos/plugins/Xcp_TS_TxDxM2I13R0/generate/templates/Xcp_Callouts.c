...
#define XCP_START_SEC_CODE_APPL
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpBuildChecksumSupport)"
[!IF "(node:existsAndTrue(XcpGeneral/XcpUserTimestampSupported))"!]
...
[!ENDIF!][!// "(node:existsAndTrue(XcpGeneral/XcpUserTimestampSupported))"
[!IF "node:existsAndTrue(XcpGeneral/XcpCalPagSupported)"!]
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpCalPagSupported)"
[!IF "node:existsAndTrue(XcpGeneral/XcpAddressTranslationSupport)"!]
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpAddressTranslationSupport)"
[!IF "node:existsAndTrue(XcpConfig/eb-list::*[1]/XcpMemoryAccessArea/XcpMemoryAccessAreasCallout)"!]
...
[!ENDIF!][!// "node:existsAndTrue(XcpConfig/eb-list::*[1]/XcpMemoryAccessArea/XcpMemoryAccessAreasCallout)"
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpPgmSupported)

[!IF "(node:existsAndTrue(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism)) and (node:exists(XcpGeneral/XcpWriteMemoryRAMCallback)) and not(node:empty(XcpGeneral/XcpWriteMemoryRAMCallback))"!]
...
(
  ...
  P2CONST( uint8, AUTOMATIC, XCP_APPL_DATA ) DataPtr,
  uint8 DataLength
[!IF "node:exists(XcpBswDistribution)"!]  ,uint16 SessionState[!ENDIF!][!//
)
{
    Xcp_ApplReturnType RetValue = XCP_APPL_OK;
...
[!ENDIF!][!// "(node:existsAndTrue(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism)) and (node:exists(XcpGeneral/XcpReadMemoryRAMCallback)) and not(node:empty(XcpGeneral/XcpReadMemoryRAMCallback))

[!IF "(node:existsAndTrue(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism)) and (node:exists(XcpGeneral/XcpReadMemoryRAMCallback)) and not(node:empty(XcpGeneral/XcpReadMemoryRAMCallback))"!]
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpGetIdType1Callout)"

[!IF "node:existsAndTrue(XcpGeneral/XcpSynchReceivedCallout)"!]
...
/*-----------[Callout for retrieving identification information for GET_ID command, generic type]-------*/
FUNC(Xcp_ApplReturnType, XCP_APPL_CODE) Xcp_ApplGenericGetIdentification
(
  uint8 Type,
  uint8* IdBufferPtr,
  uint8* IdLength
)
{
  ...
  DBG_XCP_APPLGENERICGETIDENTIFICATION_ENTRY(Type, IdBufferPtr, IdLength);

  TS_PARAM_UNUSED(Type);                      /* Remove if parameter is used */
  TS_PARAM_UNUSED(IdBufferPtr);               /* Remove if parameter is used */
  TS_PARAM_UNUSED(IdLength);                  /* Remove if parameter is used */

  /* insert your code here */

  DBG_XCP_APPLGENERICGETIDENTIFICATION_Exit(Type, IdBufferPtr, IdLength, RetValue);
  return RetValue;
}
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpGetIdGenericTypeCallout)"

#define XCP_STOP_SEC_CODE_APPL
#include <Xcp_MemMap.h>
