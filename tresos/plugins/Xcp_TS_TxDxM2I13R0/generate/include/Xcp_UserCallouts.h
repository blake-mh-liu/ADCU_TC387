...
[!IF "(node:exists(XcpBswDistribution)) and (node:existsAndTrue(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism))"!]
 ...
 ** \param[in] SessionState Value uniquely identifying the request associated to this notification.
 **
...
[!IF "(node:exists(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism))"!]
...
[!ENDIF!][!// "(node:existsAndTrue(XcpGeneral/XcpBlockWriteReadMemoryRAMMechanism))

[!IF "node:existsAndTrue(XcpGeneral/XcpBuildChecksumSupport) and XcpGeneral/XcpBuildChecksumType = 'XCP_CRC_USER_CALLOUT'"!]
...
[!ENDIF!][!// "node:existsAndTrue(XcpGeneral/XcpBuildChecksumSupport) and XcpGeneral/XcpBuildChecksumType = 'XCP_CRC_USER_CALLOUT'"
 ...
 ** Current EB implementation of the Xcp slave only supports a maximum seed length of MAX_CTO - 2.
 **
 ** \param[in]     ResourceIdentifier    A Resource to be unlocked.
 ...
 ** Implementation of this function shall be provided by the user.
 **
 ** \param[out] IdBufferPtr - pointer where identification information shall be written.
...
/** \brief Callout function for command GET_ID, generic type
 **
 ** This callout function shall retrieve the identification information for the command
 ** GET_ID generic type.
 **
 ...
 ** Implementation of this function shall be provided by the user.
 **
 ** \param[in]  Type - identification type from the GET_ID command
 ** \param[out] IdBufferPtr - pointer where identification information shall be written.
 ** \param[out] IdLength - length of the provided identification information
 **
 ** \return Function result.
 ...
 ** \Synchronicity{Synchronous}
 */
extern FUNC(Xcp_ApplReturnType, XCP_APPL_CODE) Xcp_ApplGenericGetIdentification
(
  uint8 Type,
  uint8* IdBufferPtr,
  uint8* IdLength
