...
/* !LINKSTO Fee.ASR40.SWS_Fee_00048,1 */
...
/** \brief Macro to call Fee_LowerLayer_GetJobResult API if available and
...
#else
#define FEE_GET_FLASH_JOB_RESULT()  (Fee_Gv.FeeFlashJobResult = Fee_LowerLayer_GetJobResult())
...
#define FEE_CHECKSUM_NUMBER_OF_BYTES          (Fee_LowerLayer_LengthType)((FEE_BLOCKSTAMP_SIZE) - (FEE_CHECKSUM_SIZE))
...
#endif
/** \brief Macro function to circularly subtract section indexes */
...
#define FEE_SECTIONINCREMENT(x) (((x) == ((uint8)FEE_NUMBER_OF_SECTIONS - (uint8)1)) ? (uint8)0 : ((x) + (uint8)1))
...
#define FEE_ALLIGNED_TO_VIRTUAL_PAGE(Size)     (Fee_LowerLayer_AddressType)(Size)
#else
#define FEE_ALLIGNED_TO_VIRTUAL_PAGE(Size)  \
    (Fee_LowerLayer_AddressType)((Fee_LowerLayer_AddressType)((FEE_VIRTUAL_PAGE_SIZE - (Fee_LowerLayer_AddressType)((Fee_LowerLayer_AddressType)(Size) % FEE_VIRTUAL_PAGE_SIZE)) % FEE_VIRTUAL_PAGE_SIZE) + (Fee_LowerLayer_AddressType)(Size))
#endif
...
#endif /* (FEE_NUMBER_NOT_CONFIGURED_BLOCKS != 0U) */
...
#endif /* (FEE_CONSISTENCY_PATTERN_ENABLED == STD_ON) */

#if (FEE_IMMEDIATE_SPACE_RESERVED != 0U)
#if (defined FEE_IMM_RESERVED_SPACE_IN_USE)
#error FEE_IMM_RESERVED_SPACE_IN_USE already defined
#endif
/** \brief Macro used to detect the need of using the reserved space for immediate block
 **        If the "&" operation with this mask gives the following values it means:
 **        Value 0: The immediate reserved space is not being used.
 **        Value 1: The immediate reserved space is now being used.
 **/
#define FEE_IMM_RESERVED_SPACE_IN_USE                       0x0200U

#if (FEE_NUMBER_OF_SECTIONS == 2U)
#if (defined FEE_IMM_SWITCH_FROM_RESERVED_SPACE)
#error FEE_IMM_SWITCH_FROM_RESERVED_SPACE already defined
#endif
/** \brief Macro used to detect whether immediate blocks written in the reserved space need switching
 **        If the "&" operation with this mask gives the following values it means:
 **        Value 0: The immediate blocks don't need switching from the reserved space.
 **        Value 1: The immediate blocks need switching from the reserved space.
 **/
#define FEE_IMM_SWITCH_FROM_RESERVED_SPACE                       0x0400U
...
/** \brief  Function to merge the MemAcc returned types into MemIf types to comply with current implementation of Fee*/
extern FUNC(MemIf_JobResultType, FEE_CODE) Fee_MemAcc_GetConsolidatedJobResult(void);

/** \brief State function to wait for the asyncrounous completion of the cancel job */
extern FUNC(void, FEE_CODE) Fee_SfCancelOngoingState(void);
 ...
 */
extern FUNC(Fee_LowerLayer_LengthType, FEE_CODE) Fee_CalculateSSRequiredSpace(void);
...
#if (FEE_IMMEDIATE_SPACE_RESERVED != 0U)
/** \brief Prepares the reserved address space for the requested immediate block.
 **/
extern FUNC(void, FEE_CODE) Fee_WriteImmInReservedSpace(void);
/** \brief Gives priority to immediate job if the space was reserved for it.
 **/
extern FUNC(void, FEE_CODE) Fee_ManageImmBlockPriority(void);
#endif /* (FEE_IMMEDIATE_SPACE_RESERVED != 0U) */
...
/** \brief Variable which shows the next internal state of the module pending after the cancel is finished */
#if (FEE_USE_MEMACC == STD_ON)
extern VAR(Fee_State_t, FEE_VAR) Fee_PendingState_CancelOngoing;
#endif
