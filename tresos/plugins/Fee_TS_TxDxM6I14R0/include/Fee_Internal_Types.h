...
typedef struct
{
  ...
  Fee_LowerLayer_AddressType FeeCache[FEE_TOTAL_NUMBER_OF_BLOCKS];
  /* Array to store the addresses where the blocks are switched from */
  Fee_LowerLayer_AddressType FeeSwRestartCache[FEE_TOTAL_NUMBER_OF_BLOCKS];
  /* Considered written size(with inconsistent data), used to avoid flash overwrite */
  Fee_LowerLayer_AddressType FeeLostSectionSize;
  /* Address to the next block data to be find at init */
  Fee_LowerLayer_AddressType FeeWorkingDataAddress;
  /* Address to the next block info to be find at init */
  Fee_LowerLayer_AddressType FeeWorkingInfoAddress;
  /* Address where switch should read from */
  Fee_LowerLayer_AddressType FeeSwitchSourceAddress;
  /* Address to which next data block to be written */
  Fee_LowerLayer_AddressType FeeDataAddress;
  /* Address to which next block info to be written */
  Fee_LowerLayer_AddressType FeeInfoAddress;
#if (FEE_CRITICAL_BLOCK_NUMBER != 0U)
  /* Address to which critical block info to be written */
  Fee_LowerLayer_AddressType FeeInfoAddressCritical;
  /* Address to which critical block data to be written */
  Fee_LowerLayer_AddressType FeeDataAddressCritical;
#endif
#if (FEE_IMMEDIATE_SPACE_RESERVED != 0U)
  /* Address to which immediate block infos are written during highest priority switch */
  Fee_LowerLayer_AddressType FeeInfoAddressReserved;
  /* Address to which immediate blocks data is written during highest priority switch */
  Fee_LowerLayer_AddressType FeeDataAddressReserved;
#endif
  /* Space needed for blocks to be switched in the last empty section */
  Fee_LowerLayer_LengthType FeeSwitchRequiredSpace;
...
typedef struct
{
  Fee_LowerLayer_AddressType FeeSectionStartAddress; /**< The address where the section starts */
  Fee_LowerLayer_LengthType FeeSectionSize; /**< Size of the section in bytes */
