...
#endif
        {
          ...
          else
          {
            ...
            if(IntBlockIndex == 0xFFFFU)
            {
              ...
              {
                if((BlockSize > 0U) && (BlockSize <= FEE_NOT_CONFIGURED_BLOCKS_DATA_SIZE))
                {
...
#endif /* (FEE_CRITICAL_BLOCK_NUMBER != 0U) */
    {
      ...
      else
      {
...
#endif
        {
          ...
          {
#if (FEE_DEV_ERROR_DETECT == STD_ON)
            const Fee_BlockConfiguration_t* BlockConfPtr = &Fee_NotCfgBlocks[((IntBlockIndex) - FEE_NO_OF_CONFIGURED_BLOCKS)];
