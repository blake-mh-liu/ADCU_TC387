...
)
{
  ...
  const Fee_LowerLayer_AddressType TargetAddress = FEE_SECTION_START_ADDRESS(Fee_Gv.FeeActiveSectionIndex) + FEE_HEADER_ACTIVE_MARKER_OFFSET;

  /* Clear the content of the buffer */
  TS_MemSet(Fee_Gv.FeeBuffPtr, FEE_SECTION_STATUS_ACTIVE, FEE_SECTION_MARKER_SIZE_AL);

  if(E_OK == Fee_LowerLayer_Write(TargetAddress, Fee_Gv.FeeBuffPtr, FEE_SECTION_MARKER_SIZE_AL))
  {
    /* Reset retry counter */
...
{
  /* Calculate target address  to write */
  const Fee_LowerLayer_AddressType TargetAddress = FEE_FOOTER_ADDRESS(Fee_Gv.FeeActiveSectionIndex) + FEE_HEADER_ERASABLE_MARKER_OFFSET;
...
{
  Fee_LowerLayer_AddressType TargetAddress;
...
)
{
  ...
  if(E_OK == Fee_LowerLayer_Write(TargetAddress, Fee_Gv.FeeBuffPtr, FEE_SECTION_MANAGEMENT_OBJ_SIZE_AL))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
    Fee_Gv.FeeFlashJobResult = MEMIF_JOB_PENDING;
...
{
  /* Calculate target address  to write */
  const Fee_LowerLayer_AddressType TargetAddress = FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MANAGEMENT_OFFSET;
  ...
  else
  {
    ...
    if(Fee_Gv.Fee_SectionManagement[Fee_Gv.FeeWorkingSectionIndex].SectionStatus == FEE_SECTION_EMPTY)
    {
      ...
      if(Fee_Gv.FeeMode == FEE_MODE_SWITCH)
      {
        ...
        {
          if(FEE_SECTIONDIF(Fee_Gv.FeeActiveSectionIndex,Fee_Gv.FeeOldestSectionIndex) < (FEE_NUMBER_OF_SECTIONS / 2U))
          {
...
)
{
  ...
  if (E_OK == Fee_LowerLayer_Erase(FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex),
                        Fee_SectionCfg[Fee_Gv.FeeWorkingSectionIndex].FeeSectionSize
                       )
     )
  {
...
)
{
  ...
  if (MEMIF_JOB_PENDING == Fee_Gv.FeeFlashJobResult)
  {
        ...
        (!FEE_CHECK_FLAG(FEE_HIGHEST_INTERNAL_PRIORITY_MASK)))
    {
      ...
      /* A job has been requested. Cancel the ongoing flash erase! */
...
{
  Fee_LowerLayer_AddressType TargetAddress = (FEE_SECTION_START_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) +
                  (Fee_LowerLayer_AddressType)FEE_SECTION_MANAGEMENT_END_PATTERN_OFFSET);
  /* Write the end pattern for the Section Management Marker field */
  if(E_OK == Fee_LowerLayer_Write(TargetAddress, &Fee_ConsistencyPattern[0], FEE_CONSISTENCY_PATTERN_SIZE))
  {
    /* Reset retry counter */
    Fee_Gv.FeeRetryStateCounter = FEE_STATE_MAX_RETRIES;
    Fee_Gv.FeeFlashJobResult = MEMIF_JOB_PENDING;
...
{
  Fee_LowerLayer_AddressType TargetAddress = (Fee_LowerLayer_AddressType)FEE_FOOTER_ADDRESS(Fee_Gv.FeeWorkingSectionIndex) + FEE_SECTION_MANAGEMENT_END_PATTERN_OFFSET;
  /* Write the end pattern for the Section Management Marker field */
  if(E_OK == Fee_LowerLayer_Write(TargetAddress, &Fee_ConsistencyPattern[0], FEE_CONSISTENCY_PATTERN_SIZE))
  {
    /* Reset retry counter */
