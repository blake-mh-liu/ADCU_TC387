/* --------{ EB Automotive C Source File }-------- */

/* !LINKSTO EB_CRY_0009,1 */

/*==[Includes]================================================================*/

#include <Std_Types.h>
#include <Csm_Types.h>
#include <Cry_Base64.h>  /* !LINKSTO EB_CRY_0010,1 */

/*==[Macros]==================================================================*/

#if (defined CRY_BASE64_ENCODING_BLOCK_LEN)
#error CRY_BASE64_ENCODING_BLOCK_LEN is already defined
#endif
#define CRY_BASE64_ENCODING_BLOCK_LEN 4U

#if (defined CRY_BASE64_DECODED_BLOCK_LEN)
#error CRY_BASE64_DECODED_BLOCK_LEN is already defined
#endif
#define CRY_BASE64_DECODED_BLOCK_LEN  3U

/*==[Types]===================================================================*/

/*==[Declaration of functions with internal linkage]==========================*/

#define CRY_START_SEC_CODE
#include <MemMap.h>

/**  \brief  Decode one block of characters
 **
 ** This function decodes one block of Base64 encoded characters.
 **
 ** \param[in]    EncodingBlock        The block to be decoded
 ** \param[in]    EncodingLength       The length of the block to be decoded (in bytes)
 **
 ** \param[out]      ResultBlock       The decoded block
 ** \param[in]       ResultBlockLength The length of the decoded block (in bytes)
 **
 ** \returns  Error code
 **
 ** \retval   CSM_E_OK      Decoding was successful
 ** \retval   CSM_E_NOT_OK  The input did not conform to the expected format
 **/
static FUNC(Csm_ReturnType, CRY_CODE) Cry_Base64DecodeOneBlock
(
  P2CONST(uint8, AUTOMATIC, CRY_APPL_DATA) EncodingBlock,
  uint32 EncodingBlockLength,
  P2VAR(uint8, AUTOMATIC, CRY_APPL_DATA) ResultBlock,
  uint32 ResultBlockLength
);

#define CRY_STOP_SEC_CODE
#include <MemMap.h>

/*==[Constants with internal linkage]=========================================*/

#define CRY_START_SEC_CONST_8
#include <MemMap.h>

static CONST(uint8, CRY_CONST) Cry_Base64Mapping[] =
{
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x00 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x08 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x10 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x18 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x20 */
  0xFFU, 0xFFU, 0xFFU,   62U, 0xFFU, 0xFFU, 0xFFU,   63U, /* 0x28 */
    52U,   53U,   54U,   55U,   56U,   57U,   58U,   59U, /* 0x30 */
    60U,   61U, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x38 */
  0xFFU,    0U,    1U,    2U,    3U,    4U,    5U,    6U, /* 0x40 */
     7U,    8U,    9U,   10U,   11U,   12U,   13U,   14U, /* 0x48 */
    15U,   16U,   17U,   18U,   19U,   20U,   21U,   22U, /* 0x50 */
    23U,   24U,   25U, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x58 */
  0xFFU,   26U,   27U,   28U,   29U,   30U,   31U,   32U, /* 0x60 */
    33U,   34U,   35U,   36U,   37U,   38U,   39U,   40U, /* 0x68 */
    41U,   42U,   43U,   44U,   45U,   46U,   47U,   48U, /* 0x70 */
    49U,   50U,   51U, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x78 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x80 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x88 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x90 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0x98 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xA0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xA8 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xB0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xB8 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xC0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xC8 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xD0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xD8 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xE0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xE8 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, /* 0xF0 */
  0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU  /* 0xF8 */
};

#define CRY_STOP_SEC_CONST_8
#include <MemMap.h>

/*==[Variables with internal linkage]=========================================*/


/*==[Constants with external linkage]=========================================*/


/*==[Variables with external linkage]=========================================*/


/*==[Definition of functions with external linkage]===========================*/

#define CRY_START_SEC_CODE
#include <MemMap.h>

FUNC(Csm_ReturnType, CRY_CODE) Cry_Base64Decode
(
  P2CONST(uint8, AUTOMATIC, CRY_APPL_DATA) Encoding,
  uint32 EncodingLength,
  P2VAR(uint8, AUTOMATIC, CRY_APPL_DATA) Result,
  P2VAR(uint32, AUTOMATIC, CRY_APPL_DATA) ResultLengthPtr
)
{
  uint32 EncodingPosition;
  uint32 ResultPosition;
  uint32 EncodingBlockLength;
  uint32 EncodingBlockRealLength;
  uint32 ResultBlockLength;
  uint32 ResultBufferLength;
  uint32 TotalPaddingChars;
  uint32 PaddingCharsAtEnd;
  Csm_ReturnType RetVal;
  boolean Finished;

  /* sets the encoding length to zero if the complete base 64 encoded string
   * consists of '\n' and '\r' */
  EncodingPosition = EncodingLength;
  EncodingLength = 0;

  /* remove all trailing newline characters by recomputing
     EncodingLength */
  Finished = FALSE;
  while ((EncodingPosition > 0) && (Finished == FALSE))
  {
    EncodingPosition--;
    if (
         (Encoding[EncodingPosition] != '\n') &&
         (Encoding[EncodingPosition] != '\r')
       )
    {
      Finished = TRUE;

      /* set the new length of the encoding */
      EncodingLength = EncodingPosition + 1;
    }
  }

  /* count the padding characters at the end of the input */
  PaddingCharsAtEnd = 0;
  Finished = FALSE;
  EncodingPosition = EncodingLength;
  while ((EncodingPosition > 0) && (Finished == FALSE))
  {
    EncodingPosition--;

    if (Encoding[EncodingPosition] == '=')
    {
      PaddingCharsAtEnd++;
    }
    else
    {
      if (
           (Encoding[EncodingPosition] != '\n') &&
           (Encoding[EncodingPosition] != '\r')
         )
      {
        Finished = TRUE;
      }
    }
  }

  RetVal = CSM_E_OK;
  EncodingPosition = 0;
  ResultPosition = 0;
  TotalPaddingChars = 0;
  ResultBufferLength = *ResultLengthPtr;
  ResultBlockLength = 0;

  /* additionally initialize EncodingBlockLength outside the while loop
   * to make sure it is initialized in the last if statement */
  EncodingBlockRealLength = 0;
  while
  (
    (EncodingPosition < EncodingLength) &&
    (CSM_E_OK == RetVal)
  )
  {
    EncodingBlockLength = 0;
    EncodingBlockRealLength = 0;

    while
    (
      ((EncodingBlockLength + EncodingPosition) < EncodingLength) &&
      (EncodingBlockRealLength < CRY_BASE64_ENCODING_BLOCK_LEN)
    )
    {
      if
      (
        (Encoding[EncodingPosition + EncodingBlockLength] != '\r') &&
        (Encoding[EncodingPosition + EncodingBlockLength] != '\n') &&
        (Encoding[EncodingPosition + EncodingBlockLength] != '=')
      )
      {
        EncodingBlockRealLength++;
      }

      if (Encoding[EncodingPosition + EncodingBlockLength] == '=')
      {
        TotalPaddingChars++;
      }

      EncodingBlockLength++;
    }

    /* If the real block length is smaller than two, then the first ouput
     * byte of a block is not completely encoded (two bits are missing).
     * This is treated as an error */
    if((EncodingBlockRealLength < 2) || (TotalPaddingChars > PaddingCharsAtEnd))
    {
       RetVal = CSM_E_NOT_OK;
    }
    else
    {
      ResultBlockLength = EncodingBlockRealLength - 1;
    }

    if(CSM_E_OK == RetVal)
    {
      /* the output buffer must be large enough to accomodate the result */
      if ((ResultPosition + ResultBlockLength) <= ResultBufferLength)
      {
        RetVal = Cry_Base64DecodeOneBlock
                 (
                   &Encoding[EncodingPosition],
                   EncodingBlockLength,
                   &Result[ResultPosition],
                   ResultBlockLength
                 );

        EncodingPosition += EncodingBlockLength;
        ResultPosition += ResultBlockLength;
      }
      else
      {
        RetVal = CSM_E_SMALL_BUFFER;
      }
    }
  }

  /* If a padding was used, check if the right amount of padding characters
   * was used at the end */
  if ((RetVal == CSM_E_OK) && (PaddingCharsAtEnd > 0) &&
      (((PaddingCharsAtEnd + EncodingBlockRealLength) != CRY_BASE64_ENCODING_BLOCK_LEN) ||
       (PaddingCharsAtEnd > 2U)
      )
     )
  {
    RetVal = CSM_E_NOT_OK;
  }

  *ResultLengthPtr = ResultPosition;

  return RetVal;
}

#define CRY_STOP_SEC_CODE
#include <MemMap.h>

/*==[Definition of functions with internal linkage]===========================*/

#define CRY_START_SEC_CODE
#include <MemMap.h>

static FUNC(Csm_ReturnType, CRY_CODE) Cry_Base64DecodeOneBlock
(
  P2CONST(uint8, AUTOMATIC, CRY_APPL_DATA) EncodingBlock,
  uint32 EncodingBlockLength,
  P2VAR(uint8, AUTOMATIC, CRY_APPL_DATA) ResultBlock,
  uint32 ResultBlockLength
)
{
  uint32 DecodedBytes;
  Csm_ReturnType RetVal;
  uint32 i;
  uint32 j;
  uint8 SixDecodedBits;

  RetVal = CSM_E_OK;
  DecodedBytes = 0;

  i = 0;
  j = 0;
  while(i < EncodingBlockLength)
  {
    while
    (
      (i < EncodingBlockLength) &&
      (
        ('\r' == EncodingBlock[i]) ||
        ('\n' == EncodingBlock[i]) ||
        ('='  == EncodingBlock[i])
      )
    )
    {
      i++;
    }

    if(i < EncodingBlockLength)
    {
      SixDecodedBits = Cry_Base64Mapping[EncodingBlock[i]];

      /* characters not in the alphabet are treated as errors */
      if(0xFFU == SixDecodedBits)
      {
        RetVal = CSM_E_NOT_OK;
      }
      else
      {
        DecodedBytes |=
            ((uint32)SixDecodedBits << (6U * (CRY_BASE64_ENCODING_BLOCK_LEN - (j + 1U))));
      }

      i++;
      j++;
    }
  }

  if(CSM_E_OK == RetVal)
  {
    for(i = 0; i < ResultBlockLength; i++)
    {
      ResultBlock[i] = (uint8) (DecodedBytes >> (8U * (CRY_BASE64_DECODED_BLOCK_LEN - (i + 1U))));
    }
  }

  return RetVal;
}

#define CRY_STOP_SEC_CODE
#include <MemMap.h>
