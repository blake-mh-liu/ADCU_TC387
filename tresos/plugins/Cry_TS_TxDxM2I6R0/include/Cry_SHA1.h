...
/* !LINKSTO EB_CRY_0001,1, CS_SECURE_CRY_0001_SHA1,1 */
...
#include <Cry_SHA1Config.h> /* !LINKSTO EB_CRY_0002,1 */
...
/** \brief  Start SHA1 computation
 **
 ** This function requests the start of the SHA computation for the
 ** given configuration. The start is performed in Cry_SHA1MainFunction()
 **
 ...
 **
 **/
extern FUNC(Csm_ReturnType, CRY_APPL_CODE) Cry_SHA1Start
(
  P2CONST(void, AUTOMATIC, CRY_APPL_DATA) cfgPtr
);

/** \brief  Update SHA1 computation.
 **
 ** This function requests the update of the SHA computation for the
 ** given data. The update is performed in Cry_SHA1MainFunction().
 **
 ...
 ** \retval   CSM_E_NOT_OK  If no SHA computation has been started via Cry_SHA1Start(), yet.
 **
 **/
extern FUNC(Csm_ReturnType, CRY_APPL_CODE) Cry_SHA1Update
(
  P2CONST(uint8, AUTOMATIC, CRY_APPL_DATA) dataPtr,
  uint32                                   dataLength
);

/** \brief  Finish SHA1 computation
 **
 ...
 ** is performed in Cry_SHA1MainFunction().
 **
 ...
 **
 **/
extern FUNC(Csm_ReturnType, CRY_APPL_CODE) Cry_SHA1Finish
(
 ...
 **  inform the CSM of the result. If the complete SHA1 computation has
 **  finished, additionally the function Csm_HashServiceFinishNotification() is
 **  called.
 **
 **/
extern FUNC(void, CRY_APPL_CODE) Cry_SHA1MainFunction
(
  void
);
 ...
 **
 **/
extern FUNC(P2CONST(uint8, AUTOMATIC, CRY_APPL_DATA), CRY_CODE) Cry_SHA1AlgorithmId
(
  P2CONST(void, AUTOMATIC, CRY_APPL_DATA) cfgPtr,
...
#endif /* #if (CRY_SHA1_ENABLED == STD_ON) */
