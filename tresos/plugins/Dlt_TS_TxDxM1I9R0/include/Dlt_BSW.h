 ...
 ** \retval E_NOT_OK  LogChannelNames could not be returned
 **
 ** ServiceID{::DLT_SID_GetLogChannelNames}
 ** Reentrancy{Non Reentrant}
 ** Synchronicity{Synchronous}
 */
#elif (DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_20_11)
/** \brief Get Log Channel Names
 **
 ** Returns the filter threshold for the given LogChannel.
 **
 ** \param[in,out] numberOfLogChannels Contains the number of requested LogChannels names. On Return it holds the number of configured LogChannels
 ** \param[out]    logChannelNames     Returns a list of configured LogChannel names
 **
 ** \return Result of changing the log level
 ** \retval E_OK      No error occurred
...
/* !LINKSTO Dlt.EB_Ref.SWS_Dlt_00732.Signature, 1 */
/** \brief Get log info
 *
...
#endif /* (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) || (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_20_11) */

#if (DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_431)
#if (DLT_POLL_BUFFER_SPACE == STD_ON)
/** \brief API used for calculating the remaining free space in the internal buffer.
 ** This Api is available only when AUTOSAR_431 or AUTOSAR_20_11 is configured
 **
 ** \param[in]  logChannelName  The name of the log channel for which we check the
 **             buffer size
 ** \param[out] AvailableBufferSize  Calculated size
 **
 ** \return Result of buffer free space calculation
 ** \retval DLT_E_NOT_OK The required operation failed.
 ** \retval DLT_E_OK The required operation succeeded.
 **
 ** ServiceID{::DLT_SID_GetRemainingBufferSize}
 ** Reentrancy{Reentrant}
 ** Synchronicity{Synchronous}
 */
extern FUNC(Std_ReturnType, DLT_CODE) Dlt_GetRemainingBufferSize(
  P2CONST(uint8, AUTOMATIC, DLT_APPL_CONST) logChannelName,
