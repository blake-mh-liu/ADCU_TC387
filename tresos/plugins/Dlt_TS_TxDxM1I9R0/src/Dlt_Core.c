...
#endif
  {
    ...
    else
    {
...
#if (DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_431)
#if ((DLT_DEFAULT_ASR_RETVAL == AUTOSAR_22_11) || (DLT_PROTOCOL_VERSION == 1U))
      Dlt_WriteDwordToByteArray(&NvMBufferPtr[DLT_NVM_APPID_INDEX], LocalCoreContext.AppIdTable[Dlt_AppIdIdx].AppId);
#else
      TS_MemCpy(&NvMBufferPtr[DLT_NVM_APPID_INDEX], &LocalCoreContext.AppIdTable[Dlt_AppIdIdx].AppId, DLT_APPID_LENGTH);
#endif
#else
      Dlt_WriteDwordToByteArray(&NvMBufferPtr[DLT_NVM_APPID_INDEX], Dlt_AppIdTable[Dlt_AppIdIdx].AppId);
#endif
...
#if (DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_431)
#if ((DLT_DEFAULT_ASR_RETVAL == AUTOSAR_22_11) || (DLT_PROTOCOL_VERSION == 1U))
      Dlt_WriteDwordToByteArray(&NvMBufferPtr[DLT_NVM_CTXID_INDEX], LocalCoreContext.ContextIdTable[accessContextTableIdx].ContextId);
#else
      TS_MemCpy(&NvMBufferPtr[DLT_NVM_CTXID_INDEX], &LocalCoreContext.ContextIdTable[accessContextTableIdx].ContextId, DLT_CTXID_LENGTH);
#endif
      ...
      ** */
      ...
      NvMBufferPtr[DLT_NVM_UPDATE_INDEX] |= (uint8)(LocalCoreContext.ContextIdTable[accessContextTableIdx].OsCoreId << 1U);
#endif
      ...
      {
        NvMBufferPtr[DLT_NVM_CHANNEL_NAME_INDEX + chnIdx] = (uint8)LocalCoreContext.ContextIdTable[accessContextTableIdx].ChannelsIdx[chnIdx];
      }
#else
...
FUNC(Std_ReturnType, DLT_CODE) Dlt_NvMInitDataSetBlockCbk(void)
{
  ...
  if (data_set_index < DLT_STORE_DATASET_NVRAM_BLOCK_NUMBER)
  {
    ...
    Dlt_Internal_ApplicationIDType app_id_Internal = {DLT_APP_ID_NULL};
    Dlt_Internal_ContextIDType context_id_Internal = {DLT_CTX_ID_NULL};
#else
    ...
    if (DLT_APPL_OK == Dlt_ApplGetConfigFactoryDefault
         (data_set_index, &session_id, &app_id_Internal, &context_id_Internal, &trace_status) )
#else
    if (DLT_APPL_OK == Dlt_ApplGetConfigFactoryDefault
  ...
  P2CONST(void, AUTOMATIC, NVM_APPL_DATA) NvMBuffer)
{
...
#endif
  {
        ...
        (Dlt_NvMInitCbkDataSetFlag == FALSE))
    {
...
#if (((DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) || (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_20_11)) && (DLT_PROTOCOL_VERSION == 2U))
      TS_MemCpy(&Dlt_NvMApplicationID, &NvMBufferPtr[DLT_NVM_APPID_INDEX], DLT_APPID_LENGTH);
      TS_MemCpy(&Dlt_NvMContextID, &NvMBufferPtr[DLT_NVM_CTXID_INDEX], DLT_CTXID_LENGTH);
#else
      ...
      TS_MemBZero(&ContextIdTable.ContextDescription.CtxtDesc[0U], sizeof(ContextIdTable.ContextDescription.CtxtDesc));
#endif
      Dlt_ReadDwordFromByteArray(&Dlt_NvMSessionId, &NvMBufferPtr[DLT_NVM_SESSIONID_INDEX]);
#if ((DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_431) && (DLT_ENABLE_BSW_DISTRIBUTION == STD_ON))
      ContextIdTable.OsCoreId = ((Dlt_CoreIdType)NvMBufferPtr[DLT_NVM_UPDATE_INDEX] & DLT_NVM_OS_CORE_ID_BIT_MASK) >> 1U;
#endif
...
#if (DLT_DEFAULT_ASR_RETVAL >= AUTOSAR_431)
      {
        ...
        for (chnIdx = 0; chnIdx < DLT_TXPDU_NO; chnIdx ++)
        {
          localContext.ContextIdTable->ChannelsIdx[chnIdx] = (boolean)NvMBufferPtr[DLT_NVM_CHANNEL_NAME_INDEX + chnIdx];
        }
      }
#endif
