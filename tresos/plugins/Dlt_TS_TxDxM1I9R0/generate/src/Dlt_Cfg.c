...
[!IF "as:modconf('Dlt')/DltGeneral/DltRteUsage = 'true'"!][!//
#include <Rte_Dlt.h>
...
#define DLT_VFB_START_CTX_ID 0x10010101U
[!ELSE!][!//
...
STATIC VAR(uint8, DLT_VAR) Dlt_MessageBuffer_[!"node:name(.)"!][[!"num:i(./DltLogChannelBufferSize)"!]U];
[!ENDLOOP!][!//
...
[!IF "(node:exists(DltSwc/*)) and (num:i(count(DltSwc/*)) > 0)"!][!//
  ...
  [!ENDFOR!][!//
[!ENDIF!][!//
VAR(Dlt_CtxtPerApplType, DLT_VAR) Dlt_MaxCtxtPerApplication[MAX_APP_NO] =
{
    ...
    [!IF "$CountForAppl != 0"!][!//
  {
    ...
    [!IF "((num:i($DltServiceAPI) = $AUTOSAR_431) or (num:i($DltServiceAPI) = $AUTOSAR_20_11))"!][!//
      [!CALL "DltPrepareAppId","APPTOSHOW"="$APPTOSHOW"!]
    {[!"$ApplicationId"!]}, /* AppId */
    [!ELSE!][!//
    [!"asc_dlt:getDecimalRepresentationOfCharacters(string($APPTOSHOW))"!]U, /* AppId - "[!"string($APPTOSHOW)"!]" */
    [!ENDIF!][!//
    [!ELSE!][!//
    [!"$APPTOSHOW"!]U, /* AppId */
    [!ENDIF!][!//
  ...
  {
    DLT_DET_APP_ID_INTERNAL,
    DLT_DET_CONTEXT_COUNT
  },
  [!ENDIF!][!//
...
/** \brief Constant structure containing threshold value for each Log Channel */
CONST(uint32, DLT_CONST) Dlt_CfgLogChannelThreshold[DLT_TXPDU_NO] =
{
  [!LOOP "as:modconf('Dlt')/DltConfigSet/*/DltLogOutput/*/DltLogChannel/*"!][!//
    [!IF "node:exists(./DltBufferThresholdForNotification)"!][!//
      [!"num:i(./DltBufferThresholdForNotification)"!]U,
    [!ELSE!][!//
      0U,
    [!ENDIF!][!//
  [!ENDLOOP!][!//
...
[!IF "(node:exists(DltMultipleConfigurationContainer/DltBswDistribution/DltEnableBswDistribution)) and DltMultipleConfigurationContainer/DltBswDistribution/DltEnableBswDistribution = 'true'"!][!//
/** \brief Constant which contains all the CoreIds for the DLT satellites */
CONST(Dlt_CoreIdType, DLT_CONST) Dlt_CoreIds[DLT_NUMBER_OF_SATELLITES + 1U] =
{
[!FOR "SatIndex" = "0" TO "node:value(as:modconf('Os')/OsOS/OsNumberOfCores)"!]
[!IF "node:exists(DltMultipleConfigurationContainer/DltBswDistribution/DltSatelliteCore/*[node:ref(DltSatelliteCoreId)/OsCoreId = number($SatIndex)])"!]
  [!"$SatIndex"!],
[!ENDIF!]
[!ENDFOR!]
  [!"node:ref(DltMultipleConfigurationContainer/DltBswDistribution/DltMasterCore)/OsCoreId"!]
};
[!ENDIF!]
#define DLT_STOP_SEC_CONST_8
#include <Dlt_MemMap.h>
  ...
  {
    {
      ...
      [!IF "node:value(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcApplicationId) != 0 "!]
        [!IF "(as:modconf('Dlt')/DltGeneral/DltProtocolVersion = 2)"!]
          [!IF "((num:i($DltServiceAPI) = $AUTOSAR_431) or (num:i($DltServiceAPI) = $AUTOSAR_20_11))"!]
            [!CALL "DltGetAppId","DltSwcIdx"="$DltSwcIdx", "DltSwcContextIdx"="$DltSwcContextIdx"!]
      /* !LINKSTO Dlt.SWS_Dlt_00647, 1 */
      {[!"$ApplicationId"!]},
          [!ELSE!]
      /* !LINKSTO Dlt.SWS_Dlt_00647, 1 */
      [!"asc_dlt:getDecimalRepresentationOfCharacters(string(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcApplicationId))"!]U, /* AppId - "[!"string(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcApplicationId)"!]" */
          [!ENDIF!]
        [!ELSE!]
      /* !LINKSTO Dlt.SWS_Dlt_00647, 1 */
      ...
      [!IF "node:value(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcContextId) != 0 "!]
        [!IF "(as:modconf('Dlt')/DltGeneral/DltProtocolVersion = 2)"!]
          [!IF "((num:i($DltServiceAPI) = $AUTOSAR_431) or (num:i($DltServiceAPI) = $AUTOSAR_20_11))"!]
            [!CALL "DltGetCtxId","DltSwcIdx"="$DltSwcIdx", "DltSwcContextIdx" = "$DltSwcContextIdx"!]
      {[!"$ContextId"!]}
          [!ELSE!]
      [!"asc_dlt:getDecimalRepresentationOfCharacters(string(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcContextId))"!]U /* CtxId - "[!"string(DltSwc/*[num:i($DltSwcIdx)]/DltSwcContext/*[num:i($DltSwcContextIdx)]/DltSwcContextId)"!]" */
          [!ENDIF!]
        [!ELSE!]
    ...
    DLT_DEFAULT_MAX_LOG_LEVEL,
      [!ENDIF!]
    {
      /* !LINKSTO Dlt.IsLogChannelAssigned.DefaultLogChannel,1, Dlt.SWS_Dlt_00664,1 */
...
[!IF "(node:exists(as:modconf('Dlt')/DltGeneral/DltDetForwardErrorTrace)) and (as:modconf('Dlt')/DltGeneral/DltDetForwardErrorTrace = 'true')"!][!//
STATIC CONST(Dlt_SwCContextType, DLT_CONST) Dlt_SwCContextInfo_Det[1U] =
{
  {
    {
      DLT_DET_APP_ID_INTERNAL,
      DLT_DET_CONTEXT_ID_INTERNAL
    },
    DLT_DEFAULT_MAX_LOG_LEVEL,
    {
...
[!IF "(num:i($VfbTraceFunctionCount) > 0)"!][!//
[!VAR "contextIdByte0"="16"!]
[!VAR "contextIdByte1"="1"!]
[!VAR "contextIdByte2"="1"!]
[!VAR "contextIdByte3"="1"!]
...
[!IF "((num:i($DltServiceAPI) = $AUTOSAR_431) or (num:i($DltServiceAPI) = $AUTOSAR_20_11)) and (as:modconf('Dlt')/DltGeneral/DltProtocolVersion = 2)"!][!//
  ...
  [!FOR "index" = "5" TO "num:i(node:value(DltGeneral/DltApplicationIdLength))"!][!//
    [!VAR "NullValue" = "concat($NullValue, ",'\0'")"!][!//
  [!ENDFOR!][!//
      {[!"num:i($contextIdByte0)"!], [!"num:i($contextIdByte1)"!], [!"num:i($contextIdByte2)"!], [!"num:i($contextIdByte3)"!][!"$NullValue"!]}
      [!VAR "contextIdByte3"="$contextIdByte3 + 1"!][!//
      [!IF "$contextIdByte3 = 0"!][!//
        [!VAR "contextIdByte3"="$contextIdByte3 + 1"!][!//
        [!VAR "contextIdByte2"="$contextIdByte2 + 1"!][!//
        [!IF "$contextIdByte2 = 0"!][!//
          [!VAR "contextIdByte2"="$contextIdByte2 + 1"!][!//
          [!VAR "contextIdByte1"="$contextIdByte1 + 1"!][!//
          [!IF "$contextIdByte1 = 0"!][!//
            [!VAR "contextIdByte1"="$contextIdByte1 + 1"!][!//
            [!VAR "contextIdByte0"="$contextIdByte0 + 1"!][!//
          [!ENDIF!][!//
        [!ENDIF!][!//
      [!ENDIF!][!//
[!ELSE!][!//
[!IF "$DltRteVfbContext = 1"!][!//
      DLT_VFB_START_CTX_ID,
[!ELSE!][!//
...
CONST(Dlt_SwCLogType, DLT_CONST) Dlt_SwCLogInfo[DLT_SWC_REGISTERED_NO] =
{
  ...
  {
    DLT_DET_MODULE_ID,
    100U, /* 100 bytes are enough to contain the Det error tracing data in the sent log message */
    0U,   /* The Det SessionId, ApplicationId and ContextId are not used for trace messages, so value is 0 */
    FALSE
  },
  [!ENDIF!][!//
[!IF "(num:i($VfbTraceFunctionCount) > 0)"!][!//
...
/** \brief Constant structure containing pointers to callback function for notification of each Software Component */
CONST(Dlt_UserThresholdReachedType, DLT_CONST) Dlt_ThresholdReachedNotification[DLT_BUFFER_THRSH_SWCS_TO_NOTIFY] =
{
[!IF "num:i($DltNumSwcsForThresholdNotification) > 0"!]
  [!LOOP "DltSwc/*"!][!//
    [!IF "(node:exists(./DltProvideBufferThresholdCallbackPort)) and (./DltProvideBufferThresholdCallbackPort = 'true')"!][!//
  {Rte_Call_BufferThresholdCallback_[!"node:name(.)"!]_SendNotification},
    [!ENDIF!][!//
  [!ENDLOOP!][!//
[!ELSE!][!//
  {[!"DltGeneral/DltThresholdInternalBufferUsageCallout"!]}
[!ENDIF!][!//
};
[!ENDIF!][!//
[!ENDIF!][!//
 ...
 CONST(Dlt_LogChannelType, DLT_CONST) Dlt_CfgLogChannel[DLT_TXPDU_NO] =
{
  [!LOOP "as:modconf('Dlt')/DltConfigSet/*/DltLogOutput/*/DltLogChannel/*"!]
  {
    ...
    {[!/*
    */!][!FOR "index" = "1" TO "num:i(string-length(node:value(./DltLogChannelId)))"!][!/*
    */!]'[!"substring(node:value(./DltLogChannelId), $index, 1)"!]'[!/*
    */!][!IF "$index != num:i(string-length(node:value(./DltLogChannelId)))"!],[!ENDIF!][!/*
    */!][!ENDFOR!]},
...
[!IF "(node:exists(DltMultipleConfigurationContainer/DltBswDistribution/DltEnableBswDistribution)) and DltMultipleConfigurationContainer/DltBswDistribution/DltEnableBswDistribution = 'true'"!][!//
  [!FOR "Index" = "0" TO "node:value(as:modconf('Os')/OsOS/OsNumberOfCores) - 1"!]
    [!IF "node:ref(DltMultipleConfigurationContainer/DltBswDistribution/DltMasterCore)/OsCoreId = number($Index)"!]
