 ...
 *
 *    Reason: Current implementation of MemIf uses Macros to export function names.
 *            As this is already used for 10+ years and has proof in field usage it
 *            is considered acceptable by the architect to violate this MISRA warning.
 *
...
#include <MemIf_Cfg.h>          /* external configuration */
...
/* !LINKSTO MemIf.ASR40.SWS_MemIf_00030,1 */
...
#define MemIf_Read(DeviceIndex, BlockNumber, BlockOffset, DataBufferPtr, Length) \
    (((void) (DeviceIndex)),MemIf_ReadFctPtr[0U]((BlockNumber), (BlockOffset), (DataBufferPtr), (Length)))

#define MemIf_Write(DeviceIndex, BlockNumber, DataBufferPtr) \
    (((void) (DeviceIndex)),MemIf_WriteFctPtr[0U]((BlockNumber),(DataBufferPtr)))

#define MemIf_Cancel(DeviceIndex) \
    (((void) (DeviceIndex)),MemIf_CancelFctPtr[0U]())

#define MemIf_GetStatus(DeviceIndex) \
    (((void) (DeviceIndex)),MemIf_GetStatusFctPtr[0U]())

#define MemIf_GetJobResult(DeviceIndex) \
    (((void) (DeviceIndex)),MemIf_GetJobResultFctPtr[0U]())

#define MemIf_InvalidateBlock(DeviceIndex, BlockNumber) \
    (((void) (DeviceIndex)),MemIf_InvalidateBlockFctPtr[0U]((BlockNumber)))

#define MemIf_EraseImmediateBlock(DeviceIndex, BlockNumber) \
    (((void) (DeviceIndex)),MemIf_EraseImmediateBlockFctPtr[0U]((BlockNumber)))

#if (MEMIF_MEMACC_USAGE == STD_OFF)
#define MemIf_SetMode(Mode) \
    (MemIf_SetModeFctPtr[0U]((Mode)))
#endif
...
extern CONST(MemIf_GetStatusFctPtrType, MEMIF_CONST)
  MemIf_GetStatusFctPtr[MEMIF_MAXIMUM_NUMBER_OF_DEVICES];

#if (MEMIF_MEMACC_USAGE == STD_OFF)
extern CONST(MemIf_SetModeFctPtrType, MEMIF_CONST)
  MemIf_SetModeFctPtr[MEMIF_MAXIMUM_NUMBER_OF_DEVICES];
#endif
