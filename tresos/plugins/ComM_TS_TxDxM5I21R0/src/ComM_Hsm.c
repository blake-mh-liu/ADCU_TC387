 ...
 * Ternary operation guardCond?"TRUE":"FALSE" is used on the context of printf() and
 * Misra tool sees "TRUE" as char[5] and "FALSE" as a char[6], hence the essential type category difference.
 */
  ...
  const ComM_HsmEventType         event)
{
  ...
  COMM_CHANNELPNC_ENTER_EXCLUSIVE_AREA(sc, COMM_INST(instIdx));
  /* in case the event has already been queued don't add it again to the queue*/
  if ( COMM_EV_COUNTER_ENTRY(sc, instIdx, event) < 1U)
  {
    COMM_EV_QUEUE_ENTRY(sc, instIdx, (uint16)inst->evQueueFill) = event;
    inst->evQueueFill++;
  ...
  COMM_CHANNELPNC_EXIT_EXCLUSIVE_AREA(sc , COMM_INST(instIdx));
  ...
  const ComM_HsmEventType         event)
{
  ...
  if ( COMM_EV_COUNTER_ENTRY(sc, instIdx, event) < 1U)
  {
    ...
    ++inst->evQueueFill;
    ++COMM_EV_COUNTER_ENTRY(sc, instIdx, event);
    COMM_EV_QUEUE_ENTRY(sc, instIdx, (uint16)inst->evQueueInsertEvNo) = event;
  ...
  const uint8                     queueEntryNo)
{
  ...
  ComM_HsmEventType event = COMM_EV_QUEUE_ENTRY(sc, instIdx, (uint16)queueEntryNo);
  --COMM_EV_COUNTER_ENTRY(sc, instIdx, event);
...
/* CHECK: NOPARSE */
  ...
  */
           ...
           && (FALSE == foundEventToDispatch))
    {
      ...
      ev        = COMM_EV_QUEUE_ENTRY(sc, instIdx, (uint16)entryNo);
