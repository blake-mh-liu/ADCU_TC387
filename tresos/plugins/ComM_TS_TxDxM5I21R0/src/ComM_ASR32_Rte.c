...
#if ((COMM_INCLUDE_RTE == STD_ON) && (COMM_OS_APPLICATION_MULTICORE_ENABLED == STD_OFF))
...
#endif /* ((COMM_INCLUDE_RTE == STD_ON) && (COMM_OS_APPLICATION_MULTICORE_ENABLED == STD_OFF)) */
...
)
{
  ...
  /* CHECK: NOPARSE */
  /* When COMM_NUM_USERS is 0 the function ComM_ASR32_GetCurrentComMode will always
   * return E_NOT_OK whcih will make the macros COMM_EB_DEFAULT_RTE_NOTIFY and
   * COMM_EB_ASR32_RTE_NOTIFY and COMM_EB_RTE_NOTIFY not to execute any of the
   * RTE notify functions (ComM_RteNotify, ComM_ASR32_RteNotify unreachable)
   */
#else
...
#include <ComM_MemMap.h>
     ...
     (COMM_OS_APPLICATION_MULTICORE_ENABLED == STD_ON)\
     )
...
#endif /* ((COMM_INCLUDE_RTE == STD_OFF) || (COMM_ENABLE_ASR32_SERVICE_API == STD_OFF) || COMM_OS_APPLICATION_MULTICORE_ENABLED == STD_ON) */
