...
[!LOOP "as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*"!]
  [!IF "node:exists(ComMManageReference/*[1])"!]
    ...
    [!LOOP "as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMUser/*"!]
      ...
      [!IF "num:i(count(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*[number(ComMChannelId) = $managedchannelid][util:distinct(node:refs(ComMUserPerChannel/*/ComMUserChannel))[number(ComMUserIdentifier) = $userid]])) > 0"!]
        [!VAR "manageduserfound" = "'true'"!]
      [!ENDIF!]
      [!IF "num:i(count(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*[util:distinct(node:refs(ComMChannelPerPnc/*))[number(ComMChannelId) = $managedchannelid]][util:distinct(node:refs(ComMUserPerPnc/*))[number(ComMUserIdentifier) = $userid]])) > 0"!]
        [!VAR "manageduserfound" = "'true'"!]
      [!ENDIF!]
      [!IF "$manageduserfound = 'true'"!]
        [!IF "num:i(count(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*[number(ComMChannelId) = $managingchannelid][util:distinct(node:refs(ComMUserPerChannel/*/ComMUserChannel))[number(ComMUserIdentifier) = $userid]])) > 0"!]
          [!VAR "managinguserfound" = "'true'"!]
        [!ENDIF!]
        [!IF "num:i(count(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*[util:distinct(node:refs(ComMChannelPerPnc/*))[number(ComMChannelId) = $managingchannelid]][util:distinct(node:refs(ComMUserPerPnc/*))[number(ComMUserIdentifier) = $userid]])) > 0"!]
          [!VAR "managinguserfound" = "'true'"!]
        [!ENDIF!]
        [!IF "$manageduserfound != $managinguserfound"!]
          [!/* !LINKSTO ComM.SWS_ComM_CONSTR_00002,1 */!]        
...
[!IF "as:modconf('ComM')[1]/ComMGeneral/ComMPncSupport ='true'"!]
    ...
    [!ELSE!]
        [!/* !LINKSTO ComM.EB.PartialNetworkOffset_MinComMPncId,1 */!]
        [!IF "not( ($macroRetValGetPnOffset > 0) and ($macroRetValGetPnOffset <= 63))"!]
                [!VAR "errMessage" = "concat('Retrived PN offset is ',string($macroRetValGetPnOffset))"!]
                [!ERROR!][!"$errMessage"!]: . Allowed range is [1:63]. Review ComMPnInfoOffset and busNm PnOffset.[!ENDERROR!][!//
        [!ENDIF!]
    [!ENDIF!]

    [!VAR "minPnId" = "num:i(8 * $macroRetValGetPnOffset)"!]
    [!IF "count(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*[number(ComMPncId) < $minPnId ]) > 0"!]
...
[!IF "as:modconf('ComM')[1]/ComMGeneral/ComMPncSupport ='true'"!]
  ...
  [!ENDIF!]
  [!/* !LINKSTO ComM.EB.PartialNetworkOffset_MaxComMPncId,1 */!]
...
[!IF "as:modconf('ComM')[1]/ComMGeneral/ComMMultiCoreSupport = 'true'"!]
  ...
  [!IF "count(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (string(OsAppEcucPartitionRef) = $MasterPartitionRef)]/OsApplicationCoreRef) = 0"!]
    [!IF "count(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (string(OsAppEcucPartitionRef) = $MasterPartitionRef)]/OsApplicationCoreAssignment) = 0"!]
      [!/* !LINKSTO ComM.EB.MasterCoreOsApplicationCoreId,1 */!]
      [!ERROR!]"At least one OsApplication shall reference an EcuPartition configured in ComMMasterCoreEcuCPartitionRef parameter and OsApplicationCoreRef or OsApplicationCoreAssignment shall be enabled"
      [!ENDERROR!]
    [!ENDIF!]
  [!ENDIF!]
  ...
  [!IF "num:i(count(text:split($MasterCoreId,'_'))) > 1"!]
    [!/* !LINKSTO ComM.EB.MasterCoreMultipleOsApplicationCoreIds,1 */!]
  ...
  [!IF "as:modconf('ComM')[1]/ComMGeneral/ComMPncSupport = 'true'"!]
    [!LOOP "util:distinct(node:order(util:distinct(node:refs(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalDirection='RX']/ComMPncComSignalRef)), 'ComHandleId'))"!]
      ...
      [!IF "num:i(count(as:modconf('Com')[1]/ComConfig/*/ComIPdu/*/ComIPduSignalRef/*[string(.) = $SignalRef])) = 0"!]
        [!/* !LINKSTO ComM.EB.SignalMappedToPDU,1 */!]
        [!ERROR!]"Signal [!"node:name(.)"!] is not maped to an IPDU"
        [!ENDERROR!]
      [!ENDIF!]

      [!IF "not(node:refvalid(as:modconf('Com')[1]/ComConfig/*/ComIPdu/*/ComIPduSignalRef/*[string(.) = $SignalRef]/../../ComMainFunctionRef))"!]
      ...
      [!VAR "MainFunctionRef" = "as:modconf('Com')[1]/ComConfig/*/ComIPdu/*/ComIPduSignalRef/*[string(.) = $SignalRef]/../../ComMainFunctionRef"!]
      [!VAR "PartitionRef" = "node:ref($MainFunctionRef)/ComPartitionRef"!]
      [!IF "count(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (string(OsAppEcucPartitionRef) = $PartitionRef)]/OsApplicationCoreRef) = 0"!]
        [!IF "count(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (string(OsAppEcucPartitionRef) = $PartitionRef)]/OsApplicationCoreAssignment) = 0"!]
          [!/* !LINKSTO ComM.EB.SignalMappedToOsApplicationCoreId,1 */!]
          [!ERROR!]"At least one OsApplication shall reference an EcuPartition configured in IPDU which contains Signal [!"node:name(.)"!] and OsApplicationCoreRef or OsApplicationCoreAssignment shall be enabled"
          [!ENDERROR!]
        [!ENDIF!]
      [!ENDIF!]
      ...
      [!IF "num:i(count(text:split($FoundCore,'_'))) > 1"!]
        [!/* !LINKSTO ComM.EB.SignalMappedToMultipleOsApplicationCoreIds,1 */!]
        [!ERROR!]"Signal [!"node:name(.)"!] shall be mapped to a single core (OsApplicationCoreRef or OsApplicationCoreAssignment)"
        [!ENDERROR!]
      [!ENDIF!]
    [!ENDLOOP!]
