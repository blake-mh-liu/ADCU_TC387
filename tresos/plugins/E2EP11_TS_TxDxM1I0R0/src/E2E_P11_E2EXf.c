 ...
 *
 * Reason:
 * Declaration has been done to prevent empty translation of unit.
*/

/*==================[inclusions]=============================================*/

#include <E2E_P11_E2EXf.h>  /* E2EXf APIs for E2E Profile 5 */
...
/* Call P11 dependent check functions */
static FUNC(Std_ReturnType, E2E_CODE) E2EXf_Inv_P11Check
(
...
/*------------------- E2EXf_P11InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P11InPlace_Transformer
(
...
{
  uint8 RetVal = E_SAFETY_HARD_RUNTIMEERROR;
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      ...
      CONSTP2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_CONST) ProfileCfgPtr =
          &(ConfigPtr->P11Config);
      ...
      /* set buffer length */
      BufferLength = (uint16)((InputBufferLength +
          ((E2EXF_XFRMBUFFERLENGTH_TYPE)PXXConfigPtr->HeaderLength / 8U)) & 0xFFFFU);

      *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE) BufferLength;

      /* call protection function */
      RetValProt = E2E_P11Protect(ProfileCfgPtr, ProtectStatePtr, BufferPtr, BufferLength);
...
/*------------------- E2EXf_P11OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P11OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
...
/*------------------- E2EXf_Inv_P11InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P11InPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P11Check(BufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
      ...
      {
        *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE)(InputBufferLength -
            ((E2EXF_XFRMBUFFERLENGTH_TYPE)PXXConfigPtr->HeaderLength / 8U));
      }
    }
  }
...
/*------------------- E2EXf_Inv_P11OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P11OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) InputBufferPtr,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P11Check(InputBufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
...
)
{
  ...
  /* get config addresses */
  CONSTP2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_CONST) ProfileConfigPtr =
      &(ConfigPtr->P11Config);
  CONSTP2CONST(E2E_SMConfigType, AUTOMATIC, E2E_CONST) SMConfigPtr =
      &(ConfigPtr->SMConfig);
  /* get RAM addresses */
  CONSTP2VAR(E2E_P11CheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr =
      &(CheckStatesPtr->P11CheckState);
  CONSTP2VAR(E2E_SMCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) SMCheckStatePtr =
      &(CheckStatesPtr->SMCheckState);

  RetValCheck = E2E_P11Check(ProfileConfigPtr, CheckStatePtr, BufferPtr, BufferLength);

  /* get profile independent state status */
  RetValMapStatusToSM = E2E_P11MapStatusToSM(RetValCheck, CheckStatePtr->Status);
