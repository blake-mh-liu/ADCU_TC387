...
[!MACRO "GetPncShutdownChannelsTypeCount"!][!//
  ...
  [!SELECT "as:modconf('CanNm')[1]/CanNmGlobalConfig/eb-list::*[1]/CanNmChannelConfig/eb-list::*
                [(node:existsAndTrue(CanNmSynchronizedPncShutdownEnabled))]
                [node:exists(node:ref(CanNmComMNetworkHandleRef)/ComMPncGatewayType) and (node:ref(CanNmComMNetworkHandleRef)/ComMPncGatewayType = 'COMM_GATEWAY_TYPE_ACTIVE')]
  "!][!//
    [!VAR "PncShutdownActiveChannels" = "num:i($PncShutdownActiveChannels + num:i(count(as:modconf('ComM')[1]/ComMConfigSet/eb-list::*[1]/ComMPnc/eb-list::*/ComMChannelPerPnc/eb-list::*[node:value(.) = node:value(node:current()/CanNmComMNetworkHandleRef)])))"!][!//
  [!ENDSELECT!][!//
  [!SELECT "as:modconf('CanNm')[1]/CanNmGlobalConfig/eb-list::*[1]/CanNmChannelConfig/eb-list::*
                [(node:existsAndTrue(CanNmSynchronizedPncShutdownEnabled))]
                [node:exists(node:ref(CanNmComMNetworkHandleRef)/ComMPncGatewayType) and (node:ref(CanNmComMNetworkHandleRef)/ComMPncGatewayType = 'COMM_GATEWAY_TYPE_PASSIVE')]
  "!][!//
    [!VAR "PncShutdownPassiveChannels" = "num:i($PncShutdownPassiveChannels + num:i(count(as:modconf('ComM')[1]/ComMConfigSet/eb-list::*[1]/ComMPnc/eb-list::*/ComMChannelPerPnc/eb-list::*[node:value(.) = node:value(node:current()/CanNmComMNetworkHandleRef)])))"!][!//
  [!ENDSELECT!][!//
[!ENDMACRO!][!//
[!ENDNOCODE!][!//
[!ENDIF!]
