...
=== Sum of FrTSynGlobalTimeSequenceCounterHysteresis of all time domains ===
*/!][!VAR "FrTSyn_AllHysteresisSum" = "0"!]
[!IF "num:i($FrTSyn_TimeDomainSlavesSize) > num:i(0)"!][!//
    [!LOOP "as:modconf('FrTSyn')[1]/FrTSynGlobalTimeDomain/*"!]
      [!IF "node:exists(./FrTSynGlobalTimeSlave)"!]
        [!VAR "FrTSyn_AllHysteresisSum" = "num:i($FrTSyn_AllHysteresisSum + node:value(./FrTSynGlobalTimeSlave/FrTSynGlobalTimeSequenceCounterHysteresis))"!]
      [!ENDIF!]
    [!ENDLOOP!]
[!ENDIF!][!/*
...
*/!][!MACRO "GetFrTSynTimeDomainsList", "Type"="'ALL'"!][!NOCODE!]
  [!IF "not(var:defined('sortedTimeDomainsListMasters'))"!]
    [!VAR "sortedTimeDomainsListMasters"="text:join(node:paths(util:distinct(node:order(as:modconf('FrTSyn')[1]/FrTSynGlobalTimeDomain/*[FrTSynGlobalTimeMaster],'FrTSynGlobalTimeMaster/FrTSynGlobalTimeMasterPdu/FrTSynGlobalTimeMasterHandleId', 'FrTSynGlobalTimeDomainId'))),',')"!]
  [!ENDIF!]
  [!IF "not(var:defined('sortedTimeDomainsListSlaves'))"!]
    [!VAR "sortedTimeDomainsListSlaves" = "text:join(node:paths(util:distinct(node:order(as:modconf('FrTSyn')[1]/FrTSynGlobalTimeDomain/*[FrTSynGlobalTimeSlave], 'FrTSynGlobalTimeSlave/FrTSynGlobalTimeSlavePdu/FrTSynGlobalTimeSlaveHandleId','FrTSynGlobalTimeDomainId'))),',')"!]
  [!ENDIF!]
  [!IF "$Type = 'SLAVE'"!]
...
<!-- !LINKSTO FrTSyn.SWS_FrTSyn_00072,1 -->
...
[!LOOP "text:split($timeMasterTxTypes, ';')"!]
  [!IF "string($timeMasterPath) = text:split(., ':')[1]"!]
...
*********************************************************************************************************************
<!-- !LINKSTO FrTSyn.SWS_FrTSyn_00018,1 -->
