...
/* !LINKSTO FrTSyn.SWS_FrTSyn_00062,1 */
...
[!IF "num:i($FrTSyn_AllHysteresisSum) > num:i(0)"!]
  CONST(uint8, TYPEDEF) scHysteresis; /* The SequenceCounterHysteresis is the number of consecutive valid SYNC required in Timeout until a Time Tuple is forwarded to the StbM.*/
[!ENDIF!][!//
...
/* !LINKSTO FrTSyn.SWS_FrTSyn_00061,1 */
typedef struct
{
...
typedef struct
{
  ...
  VAR(uint16, TYPEDEF) MacroticksPerCycle; /* MacroticksPerCycle from FrIf */
  VAR(uint8, TYPEDEF) seqCount; /* Received SC */
  VAR(uint8, TYPEDEF) assocIdx; /* Idx of OFS or self */
[!IF "num:i($FrTSyn_AllHysteresisSum) > num:i(0)"!]
  VAR(uint8, TYPEDEF) hysteresisCounter; /*Hysteresis Counter*/
[!ENDIF!][!//
