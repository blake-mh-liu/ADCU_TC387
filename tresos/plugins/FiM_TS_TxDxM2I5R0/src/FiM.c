...
/* patch version check */
#if (FIM_SW_PATCH_VERSION < 12U)
#error FIM_SW_PATCH_VERSION wrong (< 12U)
#endif
...
/* !LINKSTO FiM.SWS_Fim_00065_Implicit2,1 */
...
FIM_STATIC VAR(uint8, FIM_VAR) FiM_MessageStatusCounter[FIM_MESSAGE_MAX];
...
#endif
  {
    if (0U == FiM_MessageStatusCounter[MessageId])
    {
      *Status = FALSE;
    }
    else
...
STATIC FUNC(void, FIM_CODE) FiM_HandleFID(FiM_FunctionIdType Fid)
{
  ...
  if (FiM_MessageHandling != FiM_FidMessageFlag[Fid])
  {
    ...
    {
      FiM_MessageStatusCounter[FiM_FidData.MessageId]++;
    }
    else
    {
      FiM_MessageStatusCounter[FiM_FidData.MessageId]--;
...
#if (FIM_EXTENDED_LEVEL != FIM_NO_EXTENSION)
  TS_MemSet(FiM_MessageStatusCounter, 0U, (uint16)(sizeof(FiM_MessageStatusCounter)));
...
STATIC FUNC(void, FIM_CODE) FiM_SetFidData(FiM_FunctionIdType Fid)
{
  ...
  if (((FiM_ExtendedConfiguration[(FidIndex)] >> FIM_EXT_CONFIG_SYMPTOM_OFFSET)
      & FIM_EXT_CONFIG_SYMPTOM_MASK)
      != 0U)
  {
    FiM_FidData.SymptomBit = TRUE;
  }
  else
  {
    FiM_FidData.SymptomBit = FALSE;
  }
