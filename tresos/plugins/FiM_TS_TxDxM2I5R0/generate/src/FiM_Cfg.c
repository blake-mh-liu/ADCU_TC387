...
CONST(FiM_ConfigType, FIM_CONST) [!"as:name(FiMConfigSet/eb-list::*[1])"!] = 0U;
    ...
    CONST(FiM_FidConfigDataType, FIM_CONST) FiM_FidConfigData[FIM_FID_CONFIGDATA_NUM] =
    {
      [!INDENT "2"!]
        [!LOOP "util:distinct(node:order(FiMConfigSet/eb-list::*/FiMFID/eb-list::*, 'FiMFunctionId'))"!]
          ...
          /* ----- Events associated with FID '[!"as:name(node:current())"!]' ----- */
          [!LOOP "../../FiMInhibitionConfiguration/eb-list::*[text:split(FiMInhFunctionIdRef, '/')[last()] = as:name(node:current())]"!]
            [!VAR "InhInhibitionMask" = "FiMInhInhibitionMask"!]
            [!IF "FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceDemRef'"!]
              /* single Dem event link */
              [!"$InhInhibitionMask"!]_CFG_MASK | DemConf_DemEventParameter_[!"text:split(FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhEventRef, '/')[last()]"!],
            [!ELSE!][!//
              /* Dem event links of the event summary [!"as:name(as:ref(FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1]))"!] */
              [!/* loop over all Dem events of the event summary referring to this inhibition config */!][!//
              [!LOOP "../../FiMEventSummary/eb-list::*[FiMOutputSumEventRef = node:current()/FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1]]"!]
                [!"$InhInhibitionMask"!]_CFG_MASK | DemConf_DemEventParameter_[!"text:split(FiMInputSumEventRef/eb-list::*[1], '/')[last()]"!],
              [!ENDLOOP!]
            [!ENDIF!]
          [!ENDLOOP!]
        [!ENDLOOP!]
      ...
      CONST(FiM_FidExtendedCfgType, FIM_CONST) FiM_ExtendedConfiguration[FIM_EXT_CONFIG_NUM] =
      {
        [!INDENT "2"!]
          [!LOOP "util:distinct(node:order(FiMConfigSet/eb-list::*/FiMFID/eb-list::*, 'FiMFunctionId'))"!]
            ...
            [!ENDIF!]
            [!IF "node:existsAndTrue(FiMSetSymptom)"!]
              [!VAR "SymptomBit" = "'1'"!]
            [!ENDIF!]
          ...
          [!LOOP "as:modconf('FiM')[1]/FiMConfigSet/eb-list::*/FiMInhibitionConfiguration/eb-list::*"!]
            [!IF "node:existsAndTrue(FiMHighPrioInhibition)"!]
              [!IF "FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceDemRef'"!]
                ...
                /* Single Dem Event */
                DemConf_DemEventParameter_[!"text:split(FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhEventRef, '/')[last()]"!],
              [!ELSEIF "FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceSumRef'"!][!//
                [!VAR "eventSumName" = "text:split(FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1], '/')[last()]"!]
                [!LOOP "../../FiMEventSummary/eb-list::*[string(text:split(FiMOutputSumEventRef, '/')[last()]) = $eventSumName]"!]
                  ...
                  DemConf_DemEventParameter_[!"text:split(FiMInputSumEventRef/eb-list::*[1], '/')[last()]"!],
                [!ENDLOOP!]
              [!ENDIF!]
            [!ENDIF!]
          [!ENDLOOP!]
