...
[!ENDIF!]
[!LOOP "FiMConfigSet/eb-list::*/FiMFID/eb-list::*"!]
  [!/* count single Dem events of this FID */!][!//
  [!VAR "EventPerFidCount" = "count(../../FiMInhibitionConfiguration/eb-list::*[text:split(FiMInhFunctionIdRef, '/')[last()] = as:name(node:current()) and FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceDemRef'])"!]
  [!/* count individual Dem events of all event summaries of this FID */!][!//
  [!LOOP "../../FiMInhibitionConfiguration/eb-list::*[text:split(FiMInhFunctionIdRef, '/')[last()] = as:name(node:current()) and FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceSumRef']"!]
    [!VAR "EventPerFidCount" = "$EventPerFidCount + count(../../FiMEventSummary/eb-list::*[FiMOutputSumEventRef = node:current()/FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1]])"!]
  [!ENDLOOP!]
  [!IF "$EventPerFidCount > 255"!]
    [!ERROR!]
      More than 255 events (single Dem events and individual Dem events from event summaries) are assigned to the FID '[!"as:name(.)"!]'. Please reduce the number of assigned Dem events.
    [!ENDERROR!]
  [!ENDIF!]
  [!IF "$ExtendedLevel != 'FIM_NO_EXTENSION'"!]
    ...
    [!IF "$EventPerFidCount > $MaxHandledFidEventLinksPerScheduling"!]
      [!ERROR!]
        More than [!"$MaxHandledFidEventLinksPerScheduling"!] events (single Dem events and individual Dem events from event summaries) are assigned to the FID '[!"as:name(.)"!]'. Please reduce the number of assigned Dem events or increase the number of handled events per schedule, see 'FiMGeneral/FiMMaxHandledFidEventLinksPerScheduling'.
      [!ENDERROR!]
    [!ENDIF!]
    [!VAR "HighPrioEntry" = "0"!]
    [!LOOP "as:modconf('FiM')[1]/FiMConfigSet/eb-list::*/FiMInhibitionConfiguration/eb-list::*"!]
      [!IF "node:existsAndTrue(FiMHighPrioInhibition)"!]
        ...
        [!ELSEIF "FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceSumRef'"!][!//
          [!VAR "eventSumName" = "as:name(as:ref(FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1]))"!]
          [!LOOP "../../FiMEventSummary/eb-list::*[string(text:split(FiMOutputSumEventRef, '/')[last()]) = $eventSumName]"!]
            [!VAR "HighPrioEntry" = "$HighPrioEntry + 1"!]
          [!ENDLOOP!]
        [!ENDIF!]
      [!ENDIF!]
...
#define FIM_CFG_SIGNATURE [!"asc:getConfigSignature(node:difference(as:modconf('FiM')[1]//
                            *[not(child::*) and(node:configclass() = 'PreCompile')],
                            as:modconf('FiM')[1]/FiMConfigSet/eb-list::*))"!]U

/*------------------[API configuration options]------------------------------*/

/* !LINKSTO FiM.ASR40.SWS_Fim_00033,1 */
...
[!IF "node:existsAndTrue(FiMGeneral/FiMVersionInfoApi)"!]
...
[!IF "node:existsAndTrue(FiMGeneral/FiMDevErrorDetect)"!]
 ...
 ** deactivated for FiM */
[!IF "node:existsAndTrue(FiMGeneral/FiMEventUpdateTriggeredByDem)"!]
...
[!IF "node:existsAndTrue(FiMGeneral/FiMRteUsage)"!]
...
[!IF "node:existsAndTrue(FiMGeneral/FiMMeasurementSupport)"!]
...
[!INDENT "0"!]
  ...
  [!IF "$ExtendedLevel = 'FIM_NO_EXTENSION'"!]
    [!IF "node:existsAndFalse(FiMGeneral/FiMEventUpdateTriggeredByDem)"!]
...
#define FIM_FID_NUM    [!"num:i(count(FiMConfigSet/eb-list::*/FiMFID/eb-list::*) + 1)"!]U

[!INDENT "0"!]
  [!VAR "ConfigDataNum" = "0"!]
  [!VAR "ConfigStartIndex" = "1"!]
  [!IF "node:existsAndTrue(FiMGeneral/FiMEventUpdateTriggeredByDem) and $FixedData = 'true'"!]
    [!//
  ...
  [!ELSE!]
    [!VAR "ConfigStartIndex" = "num:i(count(FiMConfigSet/eb-list::*/FiMFID/eb-list::*) + 1)"!]
    [!LOOP "util:distinct(node:order(FiMConfigSet/eb-list::*/FiMFID/eb-list::*, 'FiMFunctionId'))"!]
      [!LOOP "../../FiMInhibitionConfiguration/eb-list::*[text:split(FiMInhFunctionIdRef, '/')[last()] = as:name(node:current())]"!]
        [!IF "FiMInhEventId/FiMInhRefChoice = 'FiMInhChoiceDemRef'"!]
         [!VAR "ConfigDataNum" = "$ConfigDataNum + 1"!]
        [!ELSE!][!//
          [!LOOP "../../FiMEventSummary/eb-list::*[FiMOutputSumEventRef = node:current()/FiMInhEventId/FiMInhRefChoice/eb-choice::*[1]/FiMInhSumRef/eb-list::*[1]]"!]
            [!VAR "ConfigDataNum" = "$ConfigDataNum + 1"!]
          [!ENDLOOP!]
        [!ENDIF!]
      [!ENDLOOP!]
    ...
    #define FIM_EXT_CONFIG_NUM        [!"num:i(count(FiMConfigSet/eb-list::*/FiMFID/eb-list::*))"!]U

    #if (defined FIM_MESSAGE_MAX)
    #error FIM_MESSAGE_MAX already defined
    #endif
    /** \brief Maximal number of messages */
    #define FIM_MESSAGE_MAX   [!"num:i(count(FiMConfigSet/eb-list::*/FiMMessage/eb-list::*) + 1)"!]U
  [!ENDIF!]
[!ENDINDENT!]
...
[!INDENT "0"!]
  [!LOOP "util:distinct(node:order(FiMConfigSet/eb-list::*/FiMFID/eb-list::*, 'FiMFunctionId'))"!]
    [!VAR "Indention" = "''"!]
    [!IF "string-length($Spaces31) > string-length(as:name(.))"!]
      [!VAR "Indention" = "substring($Spaces31, string-length(as:name(.)))"!]
    [!ENDIF!]

    #if (defined FiMConf_FiMFID_[!"as:name(.)"!])
    #error FiMConf_FiMFID_[!"as:name(.)"!] already defined
    #endif
    /** \brief Export symbolic name value */
    #define FiMConf_FiMFID_[!"as:name(.)"!][!"$Indention"!] [!"FiMFunctionId"!]U
    ...
    #define [!"as:name(.)"!]               [!"$Indention"!] [!"FiMFunctionId"!]U

    #if (defined FiM_[!"as:name(.)"!])
    #error FiM_[!"as:name(.)"!] already defined
    #endif
    /** \brief Export symbolic name value with module abbreviation as prefix
     [!WS!]** only (3.1 rev4 < AUTOSAR version <= AUTOSAR 4.0 rev2) */
    #define FiM_[!"as:name(.)"!]           [!"$Indention"!] [!"FiMFunctionId"!]U
  ...
  #define FIM_MAX_DEM_EVENT_ID  [!"num:max(as:modconf('Dem')[1]/DemConfigSet/eb-list::*/DemEventParameter/eb-list::*/DemEventId)"!]U
[!ENDINDENT!]
...
[!INDENT "0"!]
  [!LOOP "util:distinct(node:order(FiMConfigSet/eb-list::*/FiMSummaryEventId/eb-list::*, 'FiMEventSumId'))"!]
    [!VAR "Indention" = "''"!]
    [!IF "string-length($Spaces31) > string-length(as:name(.))"!]
      [!VAR "Indention" = "substring($Spaces31, string-length(as:name(.)))"!]
    [!ENDIF!]

    #if (defined FiMConf_FiMSummaryEventId_[!"as:name(.)"!])
    #error FiMConf_FiMSummaryEventId_[!"as:name(.)"!] already defined
    #endif
    /** \brief Export symbolic name value */
    #define FiMConf_FiMSummaryEventId_[!"as:name(.)"!][!"$Indention"!] [!"FiMEventSumId"!]U
    ...
    #endif
    /** \brief Export symbolic name value without prefix (AUTOSAR version <= 3.1 rev4) */
    #define [!"as:name(.)"!]                          [!"$Indention"!] [!"FiMEventSumId"!]U
    ...
    /** \brief Export symbolic name value with module abbreviation as prefix
     [!WS!]** only (3.1 rev4 < AUTOSAR version <= AUTOSAR 4.0 rev2) */
    #define FiM_[!"as:name(.)"!]                      [!"$Indention"!] [!"FiMEventSumId"!]U
    #endif /* ifndef FIM_DONT_PROVIDE_LEGACY_SYMBOLIC_NAMES */
  [!ENDLOOP!]
[!ENDINDENT!]
...
extern CONST(FiM_ConfigType, FIM_CONST) [!"as:name(FiMConfigSet/eb-list::*[1])"!];
