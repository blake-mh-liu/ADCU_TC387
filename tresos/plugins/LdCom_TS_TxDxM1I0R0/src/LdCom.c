 ...
 *
 *   Reason:
 *   This violation occurs since no internal header file for the object declaration is intended
 *   due to specific testing scenarios.
 */
...
/* !LINKSTO LdCom.SWS_LDCOM_00001,1 */
...
/* !LINKSTO LdCom.SWS_LdCom_00050,1 */
...
/* patch version check */
#if (LDCOM_SW_PATCH_VERSION < 31U)
#error LDCOM_SW_PATCH_VERSION wrong (< 31U)
#endif
...
/** \brief Define LDCOM_TP_TRANSMIT_HANDLEID_START_0 macro for handling the TP handle id's */
#if(LDCOM_TP_TRANSMIT_HANDLEID_POLICY == STD_ON)
#define LDCOM_TP_TRANSMIT_HANDLEID_START_0 0U
#else
#define LDCOM_TP_TRANSMIT_HANDLEID_START_0 LDCOM_IFTX_FUNCTION_PTR_COUNT
#endif
...
/* permanently defined API for use in PduR module independent on Rx or Tx transmission */
...
#if((LDCOM_TP_TRANSMIT_HANDLEID_POLICY == STD_OFF) && (LDCOM_IFTXAPIS_AVAILABLE == STD_ON))
  /* range check of PDU-ID */
  else if (Id < LDCOM_TP_TRANSMIT_HANDLEID_START_0)
  {
      LDCOM_DET_REPORT_ERROR(LDCOM_SID_TPTRANSMIT, LDCOM_E_INVALID_PDU_SDU_ID);
  }
#endif /* (LDCOM_TP_TRANSMIT_HANDLEID_POLICY == STD_OFF) */
  /* range check of PDU-ID */
  else if (Id >= (LdCom_GConfigPtr->TpTxPathConfig.TxPathCount + LDCOM_TP_TRANSMIT_HANDLEID_START_0))
  {
      LDCOM_DET_REPORT_ERROR(LDCOM_SID_TPTRANSMIT, LDCOM_E_INVALID_PDU_SDU_ID);
  }
  ...
  else
  {
    ...
    RetVal = PduR_LdComTpTransmit(PduIdRef[(Id - LDCOM_TP_TRANSMIT_HANDLEID_START_0)], PduInfoPtr);
  }
...
/* permanently defined API for use in PduR module independent on Rx or Tx transmission */
...
#endif  /* #if(LDCOM_TP_TRANSMIT_HANDLEID_POLICY == STD_OFF) */
  /* range check of PDU-ID */
  else if (Id >= (LdCom_GConfigPtr->TpTxPathConfig.TxPathCount + LDCOM_TP_TRANSMIT_HANDLEID_START_0))
  {
      LDCOM_DET_REPORT_ERROR(LDCOM_SID_COPYTXDATA, LDCOM_E_INVALID_PDU_SDU_ID);
  ...
  {
    /* get target API */
    const LdCom_RteCbkCopyTxDataFpType RteCbkCopyTxDataFp = LdCom_TpTxApi[Id - LDCOM_TP_TRANSMIT_HANDLEID_START_0].CopyTxDataFp;
...
/* permanently defined API for use in PduR module independent on Rx or Tx transmission */
  ...
  {
    /* get target API */
    const LdCom_RteCbkTpTxConfirmationFpType RteCbkTpTxConfirmationFp = LdCom_TpTxApi[Id - LDCOM_TP_TRANSMIT_HANDLEID_START_0].TpTxConfFp;
