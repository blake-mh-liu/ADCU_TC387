...
[!// at least one SYNC/Pure TimeBase has StbMSourceTimeBase refernced --> enable Clone feature
[!VAR "StbMTimeCloneUsed" = "false()"!]
[!// loop through all SYNC/Pure StbM time bases
[!LOOP "as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[(node:value(./StbMSynchronizedTimeBaseIdentifier) < 16) or  (node:value(./StbMSynchronizedTimeBaseIdentifier) > 31)]"!]
  [!IF "(node:exists(./StbMSourceTimeBase))"!]
    [!VAR "StbMTimeCloneUsed" = "true()"!]
    [!BREAK!]
  [!ENDIF!]
[!ENDLOOP!]
...
[!/* This list contains the following information:
      ...
      column 22: is Clone activated on this time base
...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*, './StbMSynchronizedTimeBaseIdentifier'))"!]
  ...
  [!VAR "isCloneEnabled" = "(node:exists(./StbMSourceTimeBase))"!]
      ...
      [!LOOP "asc:getNodesFromAbsXPathGeneric($BswModuleName, 'TimeBaseRefsSlave', $TimeBaseId)"!]
        [!IF "node:refvalid(.)"!]
          ...
          [!IF "$BswModuleName = 'CanTSyn'"!]
            ...
            [!IF "(((node:exists(./../EthTSynPortRole/CanTSynGlobalTimeRxIcvVerification)) and (node:exists(./../EthTSynPortRole/CanTSynGlobalTimeRxIcvVerification/CanTSynIcvVerificationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/CanTSynGlobalTimeRxIcvVerification/CanTSynIcvVerificationFvIdRef)))"!]
          ...
          [!IF "$BswModuleName = 'FrTSyn'"!]
            ...
            [!IF "(((node:exists(./../EthTSynPortRole/FrTSynGlobalTimeRxIcvVerification)) and (node:exists(./../EthTSynPortRole/FrTSynGlobalTimeRxIcvVerification/FrTSynIcvVerificationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/FrTSynGlobalTimeRxIcvVerification/FrTSynIcvVerificationFvIdRef)))"!]
              [!VAR "isSlaveReferringFreshnessValue" = "true()"!]
            [!ENDIF!]
          [!ENDIF!]
      ...
      [!LOOP "asc:getNodesFromAbsXPathGeneric($BswModuleName, 'TimeBaseRefsMaster', $TimeBaseId)"!]
        [!IF "node:refvalid(.)"!]
          ...
          [!IF "$BswModuleName = 'CanTSyn'"!]
            ...
            [!IF "(((node:exists(./../EthTSynPortRole/CanTSynGlobalTimeTxIcvGeneration)) and (node:exists(./../EthTSynPortRole/CanTSynGlobalTimeTxIcvGeneration/CanTSynIcvGenerationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/CanTSynGlobalTimeTxIcvGeneration/CanTSynIcvGenerationFvIdRef)))"!]
          ...
          [!IF "$BswModuleName = 'FrTSyn'"!]
            ...
            [!IF "(((node:exists(./../EthTSynPortRole/FrTSynGlobalTimeTxIcvGeneration)) and (node:exists(./../EthTSynPortRole/FrTSynGlobalTimeTxIcvGeneration/FrTSynIcvGenerationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/FrTSynGlobalTimeTxIcvGeneration/FrTSynIcvGenerationFvIdRef)))"!]
              [!VAR "isMasterReferringFreshnessValue" = "true()"!]
            [!ENDIF!]
          [!ENDIF!]
...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[count(./StbMNotificationCustomer/*) > 0], './StbMSynchronizedTimeBaseIdentifier'))"!]
  ...
  [!LOOP "util:distinct(node:order(./StbMNotificationCustomer/*, './StbMNotificationCustomerId'))"!]
    ...
    [!VAR "notificationCustomerName" = "as:name(.)"!][!//
...
[!// check if there is at least 1 master or gateway time base which has StbMAllowSystemWideGlobalTimeMaster or StbMIsSystemWideGlobalTimeMaster set to TRUE
[!VAR "atLeastOneMasterTimeBaseWithAllowSystemWideUsedOrIsSystemWideUsed" = "false()"!]
[!// loop over all elements from ListOfBusConnectionsToTimeBase
[!LOOP "text:split($ListOfBusConnectionsToTimeBase, ';')"!]
  [!VAR "masterConnected" = "text:split( ., ':' )[2]"!]
  [!VAR "gatewayConnected" = "text:split( ., ':' )[4]"!]
  [!VAR "allowSystemWideUsed" = "text:split( ., ':' )[16]"!]
  [!VAR "isSystemWideUsed" = "text:split( ., ':' )[17]"!]
  [!// if if there is at least 1 master or gateway time base which has StbMAllowSystemWideGlobalTimeMaster or StbMIsSystemWideGlobalTimeMaster set to TRUE
  [!IF "((($masterConnected = 'isMaster') or ($gatewayConnected = 'true')) and (($allowSystemWideUsed = 'true') or ($isSystemWideUsed = 'true')))"!]
    [!VAR "atLeastOneMasterTimeBaseWithAllowSystemWideUsedOrIsSystemWideUsed" = "true()"!]
    [!BREAK!]
  [!ENDIF!]
[!ENDLOOP!]
...
[!// check if Client-Server Interface GetTxFreshnessTruncData is used for Freshness Value
[!VAR "isGetTxFreshnessTruncDataCSInterfaceUsed" = "false()"!]

[!IF "(node:value(StbMFreshnessValueInformation/StbMQueryFreshnessValue) != 'NONE')"!]
  [!VAR "StbMFreshnessUsed" = "true()"!]
  [!// check if Callbacks or Client-Server Interfaces are used for Freshness Value
  [!IF "(node:value(StbMFreshnessValueInformation/StbMQueryFreshnessValue) = 'CFUNC')"!]
  ...
  [!ELSE!]
    ...
    [!LOOP "as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMFreshnessValue/*"!]
      ...
      [!IF "($freshnessValueLength > $freshnessValueTruncLength)"!]
        [!// activate GetTxFreshnessTruncData CS Interface usage only if freshnessValueLength > freshnessValueTruncLength
