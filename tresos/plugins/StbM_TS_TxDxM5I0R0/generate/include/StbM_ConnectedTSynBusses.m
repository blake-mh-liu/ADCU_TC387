  ...
  [!IF "$timeDomainId < 16"!]
    [!IF "node:value(./EthTSynSynchronizedTimeBaseRef) != ''"!]
      [!IF "node:refvalid(./EthTSynSynchronizedTimeBaseRef)"!]
        [!VAR "referencedSyncTimeBase" = "as:name(as:ref(./EthTSynSynchronizedTimeBaseRef))"!]
    ...
    [!IF "node:value(./EthTSynSynchronizedTimeBaseRef) != ''"!]
      [!IF "node:refvalid(./EthTSynSynchronizedTimeBaseRef)"!]
        [!IF "node:exists(node:value(as:ref(./EthTSynSynchronizedTimeBaseRef)/StbMOffsetTimeBase))"!]
          ...
          [!VAR "referencedSyncTimeBase" = "as:name(node:value(as:ref(./EthTSynSynchronizedTimeBaseRef)/StbMOffsetTimeBase))"!]
...
[!/* ListOfBusConnectionsToFreshness_[Bus]TSyn[Role] (where [Bus] can be Can/Fr/Eth and [Role] can be Master/Slave) list contains the following information:
      column 1: freshnessValueID
      column 2: timeDomainID
      
     and it looks like this:
     
     1                  2                           
     freshnessValueID : timeDomainID
e.g.         4                5      
     (StbMFreshnessValue with ID = 4 has a Master CAN connected with ID 5)

*/!]
[!VAR "ListOfBusConnectionsToFreshness_CanTSynMaster" = "''"!]
[!VAR "ListOfBusConnectionsToFreshness_FrTSynMaster" = "''"!]
[!VAR "ListOfBusConnectionsToFreshness_EthTSynMaster" = "''"!]
[!VAR "ListOfBusConnectionsToFreshness_CanTSynSlave" = "''"!]
[!VAR "ListOfBusConnectionsToFreshness_FrTSynSlave" = "''"!]
[!VAR "ListOfBusConnectionsToFreshness_EthTSynSlave" = "''"!]

[!VAR "isMasterReferringFreshnessValue" = "false()"!]
[!VAR "isSlaveReferringFreshnessValue" = "false()"!]

[!VAR "isFirstTimeBaseIteration" = "true()"!]
...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*, './StbMSynchronizedTimeBaseIdentifier'))"!]
      ...
      [!LOOP "asc:getNodesFromAbsXPathGeneric($BswModuleName, 'TimeBaseRefsSlave', $TimeBaseId)"!]
        [!IF "node:refvalid(.)"!]
          ...
          [!ENDIF!]
            ...
            [!// in case BswModule is CanTSyn
            [!IF "$BswModuleName = 'CanTSyn'"!] 
              [!// check that at least 1 CanTSyn SLAVE time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../CanTSynGlobalTimeSlave/CanTSynGlobalTimeRxIcvVerification)) and (node:exists(./../CanTSynGlobalTimeSlave/CanTSynGlobalTimeRxIcvVerification/CanTSynIcvVerificationFvIdRef))) and (node:refvalid(./../CanTSynGlobalTimeSlave/CanTSynGlobalTimeRxIcvVerification/CanTSynIcvVerificationFvIdRef)))"!]
            
                [!VAR "isSlaveReferringFreshnessValue" = "true()"!]
            
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../CanTSynGlobalTimeSlave/CanTSynGlobalTimeRxIcvVerification/CanTSynIcvVerificationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../CanTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_CanTSynSlave" = "concat($ListOfBusConnectionsToFreshness_CanTSynSlave, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
                  
                  [!VAR "freshnessValueId" = "'255'"!]
                  [!VAR "timeDomainId" = "'280'"!]
                [!ENDIF!]
                
              [!ENDIF!]
            [!ENDIF!]
            
            [!// in case BswModule is FrTSyn
            [!IF "$BswModuleName = 'FrTSyn'"!]
              [!// check that at least 1 FrTSyn SLAVE time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../FrTSynGlobalTimeSlave/FrTSynGlobalTimeRxIcvVerification)) and (node:exists(./../FrTSynGlobalTimeSlave/FrTSynGlobalTimeRxIcvVerification/FrTSynIcvVerificationFvIdRef))) and (node:refvalid(./../FrTSynGlobalTimeSlave/FrTSynGlobalTimeRxIcvVerification/FrTSynIcvVerificationFvIdRef)))"!]
            
                [!VAR "isSlaveReferringFreshnessValue" = "true()"!]
            
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../FrTSynGlobalTimeSlave/FrTSynGlobalTimeRxIcvVerification/FrTSynIcvVerificationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../FrTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_FrTSynSlave" = "concat($ListOfBusConnectionsToFreshness_FrTSynSlave, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
            ...
            [!// in case BswModule is EthTSyn
            [!IF "$BswModuleName = 'EthTSyn'"!]
              [!// check that at least 1 EthTSyn SLAVE time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../EthTSynPortRole/EthTSynGlobalTimeRxIcvVerification)) and (node:exists(./../EthTSynPortRole/EthTSynGlobalTimeRxIcvVerification/EthTSynIcvVerificationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/EthTSynGlobalTimeRxIcvVerification/EthTSynIcvVerificationFvIdRef)))"!]
            
                [!VAR "isSlaveReferringFreshnessValue" = "true()"!]
            
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../EthTSynPortRole/EthTSynGlobalTimeRxIcvVerification/EthTSynIcvVerificationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../EthTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_EthTSynSlave" = "concat($ListOfBusConnectionsToFreshness_EthTSynSlave, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
      ...
      [!LOOP "asc:getNodesFromAbsXPathGeneric($BswModuleName, 'TimeBaseRefsMaster', $TimeBaseId)"!]
        [!IF "node:refvalid(.)"!]
          ...
          [!ENDIF!]
            ...
            [!// in case BswModule is CanTSyn
            [!IF "$BswModuleName = 'CanTSyn'"!]
              [!// check that at least 1 CanTSyn MASTER time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../CanTSynGlobalTimeMaster/CanTSynGlobalTimeTxIcvGeneration)) and (node:exists(./../CanTSynGlobalTimeMaster/CanTSynGlobalTimeTxIcvGeneration/CanTSynIcvGenerationFvIdRef))) and (node:refvalid(./../CanTSynGlobalTimeMaster/CanTSynGlobalTimeTxIcvGeneration/CanTSynIcvGenerationFvIdRef)))"!]
            
                [!VAR "isMasterReferringFreshnessValue" = "true()"!]
                
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../CanTSynGlobalTimeMaster/CanTSynGlobalTimeTxIcvGeneration/CanTSynIcvGenerationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../CanTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_CanTSynMaster" = "concat($ListOfBusConnectionsToFreshness_CanTSynMaster, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
            ...
            [!// in case BswModule is FrTSyn
            [!IF "$BswModuleName = 'FrTSyn'"!]
              [!// check that at least 1 FrTSyn MASTER time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../FrTSynGlobalTimeMaster/FrTSynGlobalTimeTxIcvGeneration)) and (node:exists(./../FrTSynGlobalTimeMaster/FrTSynGlobalTimeTxIcvGeneration/FrTSynIcvGenerationFvIdRef))) and (node:refvalid(./../FrTSynGlobalTimeMaster/FrTSynGlobalTimeTxIcvGeneration/FrTSynIcvGenerationFvIdRef)))"!]
            
                [!VAR "isMasterReferringFreshnessValue" = "true()"!]
            
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../FrTSynGlobalTimeMaster/FrTSynGlobalTimeTxIcvGeneration/FrTSynIcvGenerationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../FrTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_FrTSynMaster" = "concat($ListOfBusConnectionsToFreshness_FrTSynMaster, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
            ...
            [!// in case BswModule is EthTSyn
            [!IF "$BswModuleName = 'EthTSyn'"!]
              [!// check that at least 1 EthTSyn MASTER time domain is referring a StbM FreshnessValue
              [!IF "(((node:exists(./../EthTSynPortRole/EthTSynGlobalTimeTxIcvGeneration)) and (node:exists(./../EthTSynPortRole/EthTSynGlobalTimeTxIcvGeneration/EthTSynIcvGenerationFvIdRef))) and (node:refvalid(./../EthTSynPortRole/EthTSynGlobalTimeTxIcvGeneration/EthTSynIcvGenerationFvIdRef)))"!]
            
                [!VAR "isMasterReferringFreshnessValue" = "true()"!]
            
                [!IF "$isFirstTimeBaseIteration = 'true'"!]
                  [!VAR "freshnessValueId" = "node:value(node:ref(./../EthTSynPortRole/EthTSynGlobalTimeTxIcvGeneration/EthTSynIcvGenerationFvIdRef)/StbMFreshnessValueId)"!]
                  [!VAR "timeDomainId" = "node:value(./../EthTSynGlobalTimeDomainId)"!]
                  [!VAR "ListOfBusConnectionsToFreshness_EthTSynMaster" = "concat($ListOfBusConnectionsToFreshness_EthTSynMaster, ' ', $freshnessValueId, ':', $timeDomainId, ';')"!]
  ...
  [!VAR "ListOfBusConnectionsToTimeBase" = "concat($ListOfBusConnectionsToTimeBase, ' ', $TimeBaseId, ':', $isMasterConnected, ':', $isSlaveConnected, ':', $isGateway, ':', $foundInMasterCanTSyn, ':', $foundInMasterFrTSyn, ':', $foundInMasterEthTSyn, ':', $foundInMasterEthTSynPDelayResp, ':', $foundInSlaveCanTSyn, ':', $foundInSlaveFrTSyn, ':', $foundInSlaveEthTSyn, ':', $foundInSlaveEthTSynPDelayInit, ':', as:name(.), ':', $isTimeValidationEnabled, ':', $isTimeRecordingEnabledOnTimeBase, ':', $allowSystemWideGlobalMasterForMasterUsed, ':', $isSystemWideGlobalMasterForMasterUsed, ':', $isTimeRecordingUsedOnTimeBase, ':', $allowSystemWideGlobalMasterForMasterEnabled, ':', $isStatusNotificationUsedOnTimeBase, ':', $isAllowMasterRateCorrectionActivatedOnTimeBase, ';')"!]

  [!VAR "isFirstTimeBaseIteration" = "false()"!]
