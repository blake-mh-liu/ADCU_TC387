...
[!VAR "StbMOsCounterId" = "as:name(node:ref($StbMOsCounter))"!]
...
/** \brief This typedef defines the version1 - type of the Shared Memory of all time bases
 */
typedef struct
{
...
/** \brief Freshness value config
 **
 ** This type contains the precompile time configuration
 ** of a freshness value
 */
typedef struct
{
  CONST(uint16, TYPEDEF) freshnessValueId;  /**< ID of the freshness value */
#if (STBM_FRESHNESSVALUE_TRUNCATION_ENABLED == STD_ON)
  CONST(boolean, TYPEDEF) isFreshnessValueTruncatedByStbM;  /**< Enables/Disables the truncation of the complete freshness value by StbM */
  CONST(uint8, TYPEDEF) freshnessValueTruncLength; /**< Truncated freshness value */
#endif /* (STBM_FRESHNESSVALUE_TRUNCATION_ENABLED == STD_ON) */
} StbM_FreshnessValueCfgType;
...
/** \brief Freshness value ids precompile time configuration structure */
extern CONST(StbM_FreshnessValueCfgType, STBM_APPL_CONST)
  StbM_FreshnessValueIdsCfg[STBM_NUMBER_OF_FRESHNESSVALUES];

#define STBM_STOP_SEC_CONFIG_DATA_16
#include <StbM_MemMap.h>

#endif /* #if (STBM_FRESHNESS_USED == STD_ON) */
 ...
 **
 ** This generated function returns the freshness value from the Most Significant Bits in the first byte, of the
 ...
 **                     be provided.
 */
extern FUNC(Std_ReturnType, STBM_CODE) StbM_GetTxFreshnessLcl
(
 ...
 **
 ** This generated function is used by the StbM to obtain the current freshness value. The interface function
 ...
 **                     be provided.
 */
extern FUNC(Std_ReturnType, STBM_CODE) StbM_GetTxFreshnessTruncDataLcl
(
  uint16 StbMFreshnessValueId,
  P2VAR(uint32,  AUTOMATIC, STBM_APPL_DATA) StbMFreshnessValueLength,
 ...
 **
 ** This generated function is used by the StbM to indicate that the Secured Time Synchronization Message has been initiated for transmission.
 **
 ** \param[in] StbMFreshnessValueId - Holds the identifier of the freshness value.
 **
 */
extern FUNC(void, STBM_CODE) StbM_SPduTxConfirmationLcl
(
  uint16 StbMFreshnessValueId
);
[!ENDIF!]
 ...
 **
 ** This generated function is used by the StbM to query the current freshness value.
 **
 ** \param[in] StbMFreshnessValueId - Holds the identifier of the freshness value.
 ...
 **                     be provided.
 */
extern FUNC(Std_ReturnType, STBM_CODE) StbM_GetRxFreshnessLcl
(
  uint16 StbMFreshnessValueId,
