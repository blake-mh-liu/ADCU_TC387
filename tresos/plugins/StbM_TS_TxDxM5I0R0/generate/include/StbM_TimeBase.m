...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[node:exists(./StbMOffsetTimeBase)], './StbMSynchronizedTimeBaseIdentifier'))"!]
  ...
  [!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[(not(node:exists(./StbMOffsetTimeBase))) and (node:value(./StbMSynchronizedTimeBaseIdentifier) >= 32)], './StbMSynchronizedTimeBaseIdentifier'))"!]
    [!VAR "PureTimeBaseName" = "as:name(.)"!]
...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[not(node:value(./StbMSynchronizedTimeBaseIdentifier) >= 16)], './StbMSynchronizedTimeBaseIdentifier'))"!]
    [!VAR "TimeBaseId" = "node:value(./StbMSynchronizedTimeBaseIdentifier)"!]
    [!CALL "GetConnectedBusTSynModule", "timeBase" = "node:path(.)"!]
...
[!LOOP "util:distinct(node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[not(node:value(./StbMSynchronizedTimeBaseIdentifier) >= 32)], './StbMSynchronizedTimeBaseIdentifier'))"!]
    [!VAR "TimeBaseId" = "node:value(./StbMSynchronizedTimeBaseIdentifier)"!]
    [!CALL "GetConnectedBusTSynModule", "timeBase" = "node:path(.)"!]
...
[!IF "(node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMQueryFreshnessValue) = 'CFUNC')"!]
  ...
  [!// ===== ERROR CHECKS FOR FRESHNESS master, when CFUNC is used ======
  [!// If there is no master <bus>TSyn time domain reffering an StbMFreshnessValue and master freshness callback GetTxFreshnessFct is configured, throw an error
  [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.CFUNC.GetTxFreshnessFctConfigured.NoMasterReferringFreshnessValue,1 */!]
  [!IF "(($isMasterReferringFreshnessValue = 'false') and (node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMGetTxFreshnessValueFuncName) != ''))"!]
    [!ERROR!] In order to be able to use MASTER freshness callback GetTxFreshnessFct, please make sure that at least 1 MASTER <bus>TSyn time domain refers an StbMFreshnessValue! [!ENDERROR!]
  [!ENDIF!]

  [!// ===== ERROR CHECKS FOR FRESHNESS master, when CFUNC is used ======
  [!// If there is no master <bus>TSyn time domain reffering an StbMFreshnessValue and master freshness callback GetTxFreshnessTruncDataFct is configured, throw an error
  [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.CFUNC.GetTxFreshnessTruncDataFctConfigured.NoMasterReferringFreshnessValue,1 */!]
  [!IF "(($isMasterReferringFreshnessValue = 'false') and (node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMGetTxTruncFreshnessValueFuncName) != ''))"!]
    [!ERROR!] In order to be able to use MASTER freshness callback GetTxFreshnessTruncDataFct, please make sure that at least 1 MASTER <bus>TSyn time domain refers an StbMFreshnessValue! [!ENDERROR!]
  [!ENDIF!]

  [!// ===== ERROR CHECKS FOR FRESHNESS master, when CFUNC is used ======
  [!// If there is no master <bus>TSyn time domain reffering an StbMFreshnessValue and master freshness callback SPduTxConfirmationFct is configured, throw an error
  [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.CFUNC.SPduTxConfirmationFctConfigured.NoMasterReferringFreshnessValue,1 */!]
  [!IF "(($isMasterReferringFreshnessValue = 'false') and (node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMGetTxConfFreshnessValueFuncName) != ''))"!]
    [!ERROR!] In order to be able to use MASTER freshness callback SPduTxConfirmationFct, please make sure that at least 1 MASTER <bus>TSyn time domain refers an StbMFreshnessValue! [!ENDERROR!]
  [!ENDIF!]


  [!// ===== ERROR CHECKS FOR FRESHNESS slave, when CFUNC is used ======
  [!// If there is no slave <bus>TSyn time domain reffering an StbMFreshnessValue and slave freshness callback GetTxFreshnessFct is configured, throw an error
  [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.CFUNC.GetRxFreshnessFctConfigured.NoSlaveReferringFreshnessValue,1 */!]
  [!IF "(($isSlaveReferringFreshnessValue = 'false') and (node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMGetRxFreshnessValueFuncName) != ''))"!]
    [!ERROR!] In order to be able to use SLAVE freshness callback GetRxFreshnessFct, please make sure that at least 1 SLAVE <bus>TSyn time domain refers an StbMFreshnessValue! [!ENDERROR!]
  [!ENDIF!]


  [!// ===== ERROR CHECKS in case FRESHNESS VALUE TRUNCATION IN STBM is used ======
  [!// loop through all StbM freshness values
  [!LOOP "as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMFreshnessValue/*"!]

    [!VAR "freshnessValueId" = "node:value(./StbMFreshnessValueId)"!]
    [!CALL "GetFreshnessValue_Role", "freshnessValue" = "node:path(.)"!]

    [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.CFUNC.StbMProvideTxTruncatedFreshnessValue.FALSE.NoGetTxFreshnessFctConfigured,1 */!]
    [!IF " (($freshnessValueRole = 'master') and ((node:exists(./StbMProvideTxTruncatedFreshnessValue) = 'true') and (node:value(./StbMProvideTxTruncatedFreshnessValue) = 'false')) and (node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMGetTxFreshnessValueFuncName) = ''))"!]
      [!ERROR!] In order for StbM to be able to get the complete freshness value from FvM and afterwards to truncate it, please make sure that StbMGetTxFreshnessValueFuncName is configured! [!ENDERROR!]
      [!BREAK!]
    [!ENDIF!]
  [!ENDLOOP!]
...
[!IF "(node:value(as:modconf('StbM')[1]/StbMFreshnessValueInformation/StbMQueryFreshnessValue) != 'NONE')"!]
  ...
  [!// ===== ERROR CHECKS FOR FRESHNESS master and slave, when CFUNC or SERVICE is used ======
  [!// If there is no master <bus>TSyn time domain reffering an StbMFreshnessValue and StbMQueryFreshnessValue is not NONE, throw an error
  [!/* !LINKSTO StbM.EB.Check.StbMQueryFreshnessValue.NotNONE.NoMasterOrSlaveReferringFreshnessValue,1 */!]
  [!IF "(($isMasterReferringFreshnessValue = 'false') and ($isSlaveReferringFreshnessValue = 'false'))"!]
    [!ERROR!] StbMQueryFreshnessValue is != NONE, but no TimeSync module is referencing any Freshness Value so it should be disabled. [!ENDERROR!]
  [!ENDIF!]
