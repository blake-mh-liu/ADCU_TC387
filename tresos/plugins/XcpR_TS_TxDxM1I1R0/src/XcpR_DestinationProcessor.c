 ...
 *
 * Reason:
 * In the "for (TxSrcIterator = XCPR_NO_OF_RX_SRC_PDU_IDS; TxSrcIterator < XCPR_NO_OF_SRC_PDU_IDS; TxSrcIterator++)" expression context
 ...
 ** \param[out] isBufferFullAtFirstIterationPtr Value indicating when attempting to pack first packet, buffer is already full.
 **
...
(
  ...
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) isBufferFullAtFirstIterationPtr
);
 ...
 ** \param[in] Pid_or_PackageResponse Packet identifier data.
 ...
 **/
STATIC FUNC(Std_ReturnType, XCPR_CODE) XcpR_ProcessDestFromLinkedMasterXcpSource
(
 ...
 ** \param[in] SidId                 Function ID from where the function is called.
 **/
STATIC FUNC(Std_ReturnType, XCPR_CODE) XcpR_ProcessDestFromLinkedRemoteXcpSource
(
  PduIdType Conf_SrcInternalPduId,
 ...
 ** \param[in] SidId                 Function ID from where the function is called.
 **/
STATIC FUNC(Std_ReturnType, XCPR_CODE) XcpR_ProcessDestFromLinkedLocalXcpSource
(
  PduIdType Conf_SrcInternalPduId,
  uint8 PackageResponse,
  uint8 SidId
...
(
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) ReportDetInternalBusPtr,
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) ReportDetExternalBusPtr,
  PduIdType Conf_DestInternalPduId
);
 ...
 ** \param[in]  IsFirstPacket          First packet indicator.
 ** \param[in]  SidId                  Function ID from where the function is called.
 ** \param[out] IsLastPacketInFramePtr Indicator to mark the current message as the last one in the frame.
...
(
  ...
  boolean IsFirstPacket,
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) IsLastPacketInFramePtr,
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) MessagePtr,
  uint8 SidId
);
...
(
  ...
  PduLengthType PackageLength,
  boolean IsFirstPacket,
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) AvailableSpaceForMessagePtr,
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) MessagePtr,
  uint8 SidId
);
 ...
 ** \param[in]  DataLength             Message length.
 ** \param[in]  Conf_DestInternalPduId Configured destination PDU-ID.
 ** \param[out] XcpRDestBufferPtr      Pointer to the buffer, to which the header and tail shall be added.
 **/
 ...
 ** \param[in]  Conf_DestInternalPduId Configured destination PDU-ID.
 ** \param[out] XcpRDestBufferPtr      Pointer to the TxFrame, to which the header and tail shall be added.
 **/
...
(
  ...
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) DestBufferPtr
);
#endif
...
(
  P2VAR(PduIdType, AUTOMATIC, XCPR_APPL_DATA) DestInternalPduIdPtr
);
...
#if ((XCPR_TX_BUS_RETRY_COUNT > 0U) || (XCPR_TX_RETRY_COUNT > 0))
/** \brief Function used for re-trigger transmission of destinations which have been selected for retry transmission.
 **
 ** \param[in] ServiceID The function Id from where the function is called.
 **/
...
(
  P2VAR(PduLengthType, AUTOMATIC, XCPR_APPL_DATA) ExtractedPackageLengthPtr,
  P2VAR(PduIdType, AUTOMATIC, XCPR_APPL_DATA) ExtractedSrcPduIdPtr,
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) ExtractedSrcPduTypePtr
);

/** \brief Function used to extract multiple values from the XcpR queue
 **
 ** \param[in] DestInternalPduId                Internal ID for the selected Pdu channel.
 ** \param[in] isBufferFullAtFirstIterationPtr  Value indicating when attempting to pack first packet, buffer is already full.
 ** \param[in] PreviousDestInternalPduId        Internal ID for the previous selected Pdu channel.
...
(
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) isFirstPacketPtr,
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) ContinueProcessingPtr,
  PduIdType DestInternalPduId,
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) isBufferFullAtFirstIterationPtr,
  PduIdType PreviousDestInternalPduId
);

/** \brief Function used to find the destination linked to the current source, based on the connect mode.
 **
 ** \param[in]  extracted_SrcPduType    Extracted source PDU type (Rx/Tx).
 ** \param[in]  extracted_SrcPduId      Extracted source PDU-ID.
 ** \param[in]  extracted_PackageLength Package length of the extracted packet.
 **/
STATIC FUNC(Std_ReturnType, XCPR_CODE) XcpR_HandleLinkedDestination
(
...
/** \brief Function used to determine if the source originates from the Master or a Slave and to handle Disconnect flags.
 **
...
(
  ...
  P2VAR(boolean, AUTOMATIC, XCPR_APPL_DATA) RetryTransmissionThisCyclePtr
);
#endif
 ...
 **  Index 1: Counter for runtime errors of type     XCPR_E_INVALID_CONNECT_MODE
 **  Index 2: Counter for development errors of type XCPR_E_INVALID_LENGTH
 */
...
{
  Std_ReturnType HandleLinkedDestinationResult = E_NOT_OK;
  PduIdType Iterator;
#if (XCPR_TRANSMISSION_OF_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON)
  uint16 PackedFrame_Iterator;
  PduIdType PackedFrame[XCPR_NO_OF_DEST_MULTI_PDUS_IN_FRAME] = {0};
#endif
  ...
  while (XcpR_IsMsgQueueEmpty() == FALSE)
  {
    ...
    HandleLinkedDestinationResult = XcpR_HandleLinkedDestination(extracted_SrcPduType, extracted_SrcPduId, extracted_PackageLength);
    XcpR_GetDestInternalPduId(&DestInternalPduId);

    if( (DeQueueCounter != 0U) && (HandleLinkedDestinationResult == E_OK))
    {
    ...
    /* Check if processing continues - unexpected messages might be consumed while searching for a Dest in XcpR_HandleLinkedDestination() -> GetActiveDest() */
    if ( (XcpR_IsMsgQueueEmpty() == FALSE) && (HandleLinkedDestinationResult == E_OK))
    {
     ...
     && (XCPR_TRANSMISSION_OF_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON))
...
#endif /* XCPR_TRANSMISSION_OF_MULTI_PDUS_IN_FRAME_SUPPORT == STD_ON */
          {
            ...
            {
              XcpR_ProcessActiveDestination(ServiceID, DestInternalPduId);
#if ((XCPR_TX_BUS_RETRY_COUNT > 0U) || (XCPR_TX_RETRY_COUNT > 0))
  ...
  {
#if ((XCPR_TX_BUS_RETRY_COUNT > 0U) || (XCPR_TX_RETRY_COUNT > 0))
    XcpR_HandlePackedDataProcessing(ServiceID, PackedFrame[PackedFrame_Iterator], &RetryTransmissionThisCycle);
#else
    XcpR_HandlePackedDataProcessing(ServiceID, PackedFrame[PackedFrame_Iterator], NULL_PTR);
#endif
  }
#endif
  ...
  {
    XcpR_ProcessTxRetryDestinations(ServiceID);
  }
#endif
...
)
{
  ...
  {
    XcpR_ProcessActiveDestination(ServiceID, PackedFrame);
...
{
  Std_ReturnType FindDestinationPduIdResult = E_NOT_OK;
  /* Config iterators */
...
#endif
  {
    FindDestinationPduIdResult = XcpR_FindDestinationPduId(PidOrPackageResponse, ConnectMode, Conf_SrcInternalPduId, extracted_SrcPduId, extracted_SrcPduType, XCPR_SID_PROCESS_DESTINATIONS);
  }
#if (XCPR_MULTIPLE_ROUTING_PER_CONNECTION == STD_OFF)
  else
  {
    FindDestinationPduIdResult = E_OK;
  }
#endif
  ...
  return FindDestinationPduIdResult;
}

/*------------------[XcpR_FindDestinationPduId]---------------*/

FUNC(Std_ReturnType, XCPR_CODE) XcpR_FindDestinationPduId
(
  uint8 Pid_or_PackageResponse,
  uint8 ConnectMode,
  PduIdType Conf_SrcInternalPduId,
...
{
  Std_ReturnType ProcessDestResult = E_NOT_OK;
  ...
  if (XcpR_SrcConf[Conf_SrcInternalPduId].IsMasterConnection == TRUE)
  {
    ...
    ProcessDestResult = XcpR_ProcessDestFromLinkedMasterXcpSource(extracted_SrcPduId, Conf_SrcInternalPduId, Pid_or_PackageResponse, ConnectMode, SidId);
  }
  else /* (XcpR_SrcConf[Conf_SrcInternalPduId].IsMasterConnection == FALSE) */
  {
    ...
    {
      ProcessDestResult = XcpR_ProcessDestFromLinkedRemoteXcpSource(Conf_SrcInternalPduId, Pid_or_PackageResponse, SidId);
    }
    else /* extracted_SrcPduType == XCPR_TX_SRC_TYPE */ /* source is XcpR_Transmit() */
    {
      ProcessDestResult = XcpR_ProcessDestFromLinkedLocalXcpSource(Conf_SrcInternalPduId, Pid_or_PackageResponse, SidId);
    }
  }

  return ProcessDestResult;
}
...
)
{
  ...
  if (XcpR_SrcConf[Conf_SrcInternalPduId].IsMasterConnection == TRUE)
  {
    ...
    /* source from master Xcp is always of type Rx */
    if (Pid_or_PackageResponse == 0xFEU) /* PID is disconnect */
    {
  ...
  else /* source is XcpR_Transmit() or source is XcpR_RxIndication() from remote Xcp */
  {
    ...
    if (XcpR_DisconnectCommandPending == TRUE)
    {
      ...
      if (Pid_or_PackageResponse == 0xFFU)
      {
...
{
  Std_ReturnType GetActiveDestResult = E_NOT_OK;
  ...
  if ((Pid_or_PackageResponse == 0xFFU) && (XcpR_Status.isPreviousCommandConnect == FALSE))
  {
  ...
  {
    GetActiveDestResult = XcpR_GetActiveDestination(Conf_SrcInternalPduId, XCPR_MODE_REMOTE_ECU, SidId);
  }
  else /* if (ConnectMode == XCPR_MODE_LOCAL_ECU) */
  {
    GetActiveDestResult = XcpR_GetActiveDestination(Conf_SrcInternalPduId, XCPR_MODE_LOCAL_ECU, SidId);
  }
...
)
{
  ...
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) XcpRDestBufferPtr = XcpR_GetDestPduDataPtr(DestInternalPduId, IsFirstPacket);
#endif
...
{
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) retValue;
...
)
{
  ...
  P2VAR(uint8, AUTOMATIC, XCPR_APPL_DATA) LocalXcpRDestBufferPtr = XcpRDestBufferPtr;
...
#endif
  {
    ...
    if (length >= 8U)
    {
      for (index = 1U; index < 8U; index++)
      {
        ...
        if ((length > ValidDlcValues[index - 1U]) && (length <= ValidDlcValues[index]))
        {
...
)
{
  ...
  if (XcpR_Status.ConnectionStatus == XCPR_STATE_DISCONNECTED)
  {
    ...
    /* If the PID in the transmitted package is Connect */
    if ((XcpRDestBufferPtr[Conf_DestPduHeaderLength] == 0xFFU) && (XcpR_Status.isPreviousCommandConnect == FALSE))
    {
      /* !LINKSTO XcpR.dsn.InternalState.PreviousCommandConnect,1 */
      XcpR_Status.isPreviousCommandConnect = TRUE;
    }
    /* If a CONNECT positive response is received from the Xcp slave */
    else if ((XcpRDestBufferPtr[Conf_DestPduHeaderLength] == 0xFFU) &&
             (XcpR_Status.isPreviousCommandConnect == TRUE) &&
  ...
  else /* if (DestPduType == XCPR_TX_PDU_DIRECTION) */
  {
    ...
    if (XcpR_Status.ConnectionStatus == XCPR_STATE_DISCONNECTED)
    {
      if (XcpRDestBufferPtr[Conf_DestPduHeaderLength] == 0xFFU)
      {
    ...
    {
#if (XCPR_DET_RUNTIME_CHECKS == STD_ON)
      XCPR_DET_REPORT_RUNTIME_ERROR(ServiceID, XCPR_E_INTERNAL_BUS);
#endif
    }
    if (ReportDetExternalBus == TRUE)
    {
#if (XCPR_DET_RUNTIME_CHECKS == STD_ON)
      XCPR_DET_REPORT_RUNTIME_ERROR(ServiceID, XCPR_E_EXTERNAL_BUS);
#endif
    }
...
)
{
  ...
  if ((TRUE == isTxIdle) && (XcpR_DestConf[XcpRTxPduId].ProcessingType == XCPR_TRANSMISSION_ON_TX_CONFIRMATION))
  {
    ...
    XcpR_ProcessDestinations(XCPR_SID_IF_TX_CONFIRMATION);
  }
#endif
}
#if ((XCPR_TX_BUS_RETRY_COUNT > 0U) || (XCPR_TX_RETRY_COUNT > 0))
...
#endif
      SchM_Exit_XcpR_SCHM_XCPR_EXCLUSIVE_AREA_0();
      XcpR_ProcessActiveDestination(ServiceID, Process_Iterator);
    }
  }
}
#endif
...
FUNC(void, XCPR_CODE)  XcpR_EmitSessionTerminatedEvent(void)
{
  ...
  (void)XcpR_DestConf[XcpR_Status.DestinationFromSlaveInternalId].LLTransmitPtr(XcpR_Status.DestinationFromSlave.Id, &TxPduInfo);
}
#endif
...
)
{
  ...
  {
    XCPR_DET_REPORT_RUNTIME_ERROR(ServiceID, XCPR_E_INVALID_LENGTH);
  }
#endif
