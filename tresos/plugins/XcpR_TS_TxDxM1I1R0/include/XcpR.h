...
#if (defined XCPR_E_ZERO_RX_PDU_LENGTH)
#error XCPR_E_ZERO_RX_PDU_LENGTH already defined
#endif
/** \brief DET Error Code.
 **
 ** The error is triggered by XcpR_RxIndication() if the length of the received Pdu is 0 size.
 **/
#define XCPR_E_ZERO_RX_PDU_LENGTH                0x31U

#if (defined XCPR_E_ZERO_TX_PDU_LENGTH)
#error XCPR_E_ZERO_TX_PDU_LENGTH already defined
#endif
/** \brief DET Error Code.
 **
 ** The error is triggered by XcpR_Transmit() if the length of the received Pdu is 0 size.
 **/
#define XCPR_E_ZERO_TX_PDU_LENGTH                0x32U

#if (defined XCPR_E_INVALID_CONNECT_REMOTE_XCP)
#error XCPR_E_INVALID_CONNECT_REMOTE_XCP already defined
#endif
/** \brief DET Error Code.
 **
 ** The error is triggered by XcpR_MainFunction() or XcpR_RxIndication(), when processing an XcpR source,
 ** if there is an attempt to connect to the remote XcpR even though there is no remote Xcp configured.
 **/
#define XCPR_E_INVALID_CONNECT_REMOTE_XCP        0x33U

#if (defined XCPR_E_INVALID_CONNECT_LOCAL_XCP)
#error XCPR_E_INVALID_CONNECT_LOCAL_XCP already defined
#endif
/** \brief DET Error Code.
 **
 ** The error is triggered by XcpR_MainFunction() or XcpR_RxIndication(), when processing an XcpR source,
 ** if there is an attempt to connect to the local XcpR even though there is no local Xcp configured.
 **/
#define XCPR_E_INVALID_CONNECT_LOCAL_XCP         0x34U
