 ...
 *
 * Reason:
 * False-positive from the Tasking v6.3r1 compiler (TCVX-41885)
 *
 * TASKING-2) Deviated Rule: W549
 * W549: condition is always true
 *
 * Reason:
 * This is done as defensive check
 *
...
#define SOMEIPXF_STRUCT_TLV_OPT_MEMBER_CONTEXT_TAG_SIZE 3U
...
#define SOMEIPXF_CLEAR_StructWithTLVContext_StructWithTLV_InfoType(nested_level) \
  do{ \
      ...
      TLVContextPtr->TLV_Info_LEVEL[(nested_level)].offsetAvailabilitybitfield = 0U; \
...
(
   ...
   P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr
);
...
(
   ...
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
   P2VAR(StructWithLf_NESTED_LEVEL_Type, AUTOMATIC, AUTOMATIC) StructWithLfContextPtr,
   P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr,
...
(
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
   P2VAR(FSA_NESTED_LEVEL_Type, AUTOMATIC, AUTOMATIC) FSAContextPtr,
   P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr
);
...
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_DeserializeExtFSAOfBasicTypeWithBufferLengthCheck
(
   P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) BufPtr,
   P2VAR(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) DataBytePtr,
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
...
(
   ...
   P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) DataBytePtr,
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
   P2VAR(VSA_NESTED_LEVEL_Type, AUTOMATIC, AUTOMATIC) VSAContextPtr,
   P2VAR(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, AUTOMATIC) CfgElemHandledPtr,
   P2VAR(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, AUTOMATIC) VSA_LF_BuffPos,
   P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr
);
...
#if (SOMEIPXF_DT_WITH_LF == STD_ON)
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_DeserializeBasicType_BufferLengthCheck
(
   P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) BufPtr,
   P2VAR(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) DataBytePtr,
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
...
#endif /* (SOMEIPXF_DT_WITH_LF == STD_ON) */
...
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_DeserializeFSABasicType_BufferLengthCheck
(
   P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) BufPtr,
   P2VAR(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) DataBytePtr,
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
...
(
   ...
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
   P2VAR(uint32, AUTOMATIC, AUTOMATIC) LF_ValuePtr,
...
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_Inv_GetNextArg (
  P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) BufPtr,
  P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
  P2VAR(uint32, AUTOMATIC, AUTOMATIC) TLV_ARG_BufPosPtr,
  P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr,
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufferLength,
...
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_TLV_SetUpContext (
  P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
  P2VAR(TLV_Nested_Type, AUTOMATIC, AUTOMATIC) TLVContextPtr,
  P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr
);
...
STATIC FUNC(uint8, SOMEIPXF_CODE) SomeIpXf_Inv_Get_NextDataIdPos (
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufferLength,
  P2CONST(uint8, AUTOMATIC, SOMEIPXF_APPL_DATA) BufPtr,
  P2VAR(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, AUTOMATIC) BytesCopiedPtr,
  P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, AUTOMATIC) BufPosOffsetPtr,
  P2VAR(TLV_Nested_Type, AUTOMATIC, AUTOMATIC) TLVContextPtr,
  P2VAR(SomeIpXf_DeSerInfoType, AUTOMATIC, AUTOMATIC) DeSerContextPtr
);
...
STATIC FUNC(void, SOMEIPXF_CODE) SomeIpXf_ValidateReceivedDataId(
  P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
  P2VAR(boolean, AUTOMATIC, AUTOMATIC) DataID_validPtr,
  P2VAR(TLV_Nested_Type, AUTOMATIC, AUTOMATIC) TLVContextPtr,
...
(
   ...
   P2CONST(SOMEIPXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, SOMEIPXF_APPL_CONST) ConfigPtr,
   P2VAR(SomeIpXf_AdminDataType, AUTOMATIC, AUTOMATIC) AdminData
)
{
...
#endif /* (SOMEIPXF_VSA_AVAILABLE == STD_ON) */
#if (SOMEIPXF_TLV_STRUCT_AVAILABLE == STD_ON)
   P2VAR(TLV_Nested_Type, AUTOMATIC, AUTOMATIC) TLVContextPtr = AdminData->TLVContextPtr;
#endif /* (SOMEIPXF_TLV_STRUCT_AVAILABLE == STD_ON) */
...
/* Deviation CODEMETRIC-1, CODEMETRIC-3 */
...
#endif /* (SOMEIPXF_TLV_ARGUMENT_AVAILABLE == STD_ON) */
   SOMEIPXF_XFRMBUFFERLENGTH_TYPE CfgElemHandled = DeSerContextPtr->CfgElemHandled;
   SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufferLengthCheck = DeSerContextPtr->BufferLengthCheck;
...
/* CHECK: PARSE */
   {
      switch ((ConfigPtr[CfgElemHandled] & BufferLengthCheck))
      {
/*=================================================================*/
/*               B A S I C   T Y P E S                             */
/*=================================================================*/
...
#if (SOMEIPXF_DT_WITH_LF == STD_ON)
         case SOMEIPXF_BDT_WITH_BUFFERLENGTH_CHECK_START:
         {
           /* deserialize basic data type, returns E_OK */
           RetVal = SomeIpXf_DeserializeBasicType_BufferLengthCheck (
              &BufPtr[0U], /* in: points to current output buffer position */
              &DataBytePtr[0U],  /* out: uint8 representation of the data element */
              &ConfigPtr[CfgElemHandled], /* in: points to current configuration element */
              DeSerContextPtr, /* in/out: general de-/serialization context */
...
/*=================================================================*/
         ...
         case SOMEIPXF_FSA_OF_BDT_WITH_BUFFERLENGTH_CHECK_START:
         {
            /* deserialize an array of basic data type with fix size, returns E_OK */
            RetVal = SomeIpXf_DeserializeFSABasicType_BufferLengthCheck(
               &BufPtr[0U], /* in: points to current output buffer position */
               &DataBytePtr[0U], /* out: uint8 representation of the data element */
               &ConfigPtr[CfgElemHandled], /* in: points to current configuration element */
               DeSerContextPtr, /* in/out: overall number of bytes copied to the buffer */
         ...
         case SOMEIPXF_FSA_OF_BDT_WITH_BUFFERLENGTH_CHECK_LF_START:
         {
           /* deserialize an array of basic data type with fix size with bufferlength check, returns E_OK */
           RetVal = SomeIpXf_DeserializeExtFSAOfBasicTypeWithBufferLengthCheck(
              &BufPtr[0U], /* in: points to current output buffer position */
              &DataBytePtr[0U],  /* out: uint8 representation of the data element */
              &ConfigPtr[CfgElemHandled], /* in: points to current configuration element */
              DeSerContextPtr,  /* in/out: overall number of bytes copied to the buffer */
...
)
{
  ...
  SOMEIPXF_PARAM_UNUSED(SomeIpXf_InvXfrmInfoPtr);
  ...
  if(SOMEIPXF_DE_SER == TLV_Arg_De_Seral)
  {
     ...
     */
    AdminData->TLV_ARG_BufPos = SomeIpXf_InvXfrmInfoPtr->BytesCopiedFromBuffer;
  }
#endif /* (SOMEIPXF_TLV_ARGUMENT_AVAILABLE == STD_ON) */
...
)
{
  ...
  if(SOMEIPXF_DE_SER == De_Seral)
  {
    DeSerContextPtr->BufferLengthCheck = SomeIpXf_InvXfrmInfoPtr->BufferLengthCheck;
    DeSerContextPtr->BufPosOffset = SomeIpXf_InvXfrmInfoPtr->BytesCopiedFromBuffer;
  }
  else
  {
    DeSerContextPtr->BufferLengthCheck = 0U;
    DeSerContextPtr->BufPosOffset = SOMEIPXF_HEADER_LENGTH;
  }
#if ((SOMEIPXF_TLV_ARGUMENT_AVAILABLE == STD_ON) || (SOMEIPXF_TLV_STRUCT_AVAILABLE == STD_ON))
...
)
{
  ...
  FSAContextPtr->FSA_RootEntryDataByteOffset = 0U;
...
(
   P2VAR(VSA_NESTED_LEVEL_Type, AUTOMATIC, AUTOMATIC) VSAContext,
...
)
{
  ...
  VSAContext->VSA_RootEntryDataByteOffset = 0U;
...
)
{
  ...
  /* CHECK: PARSE */
  {
    ...
    if(TRUE == con_valid_StructWithLf_De_Seral)
    {
      ...
      uint32 Continue_BufPos = ((uint32) SOMEIPXF_GET_StructWithLfContext_StructWithLf_InfoType(LF_BufPos) +
          (uint32) DeSerContextPtr->BufPosOffset + (uint32) SOMEIPXF_GET_StructWithLfContext_StructWithLf_InfoType(LFsize));

      /* postcondition overflow check */
      if(Continue_BufPos < (uint32) SOMEIPXF_GET_StructWithLfContext_StructWithLf_InfoType(LF_BufPos))
      {
        /* invalid LF_BufPos */
        RetVal = E_SER_MALFORMED_MESSAGE;
      }
      /* check if LF_BufPos is already outside of the BufferLength */
      else if(BufferLength <= Continue_BufPos)
      {
...
)
{
  ...
  const SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufPosOffset = DeSerContextPtr->BufPosOffset;

  const uint32 NextElementBufferPos = LF_Value + LF_BufPos + LFsize;

  /* postcondition overflow check */
  if(NextElementBufferPos < LF_Value)
  {
    /* invalid LF_Value */
    RetVal = E_SER_MALFORMED_MESSAGE;
  }
  else if(NextElementBufferPos > BytesCopied)
  {
    ...
    /* check if less data than expected via BufferLength are handed over for deserialization */
    if(((uint32)BytesCopied + (uint32)BufPosOffset) > BufferLength)
    {
      RetVal = E_SER_MALFORMED_MESSAGE;
    }
    else if((BytesCopied + NumSkipBytes + BufPosOffset) > BufferLength)
    {
      /* BytesCopied gets set to bufferlenght since skipping excceds bufferlenght */
      DeSerContextPtr->BytesCopied = BufferLength - BufPosOffset;
    }
    else
    {
      /* skipping behavior */
    ...
    /* Overflow check is enabled before deserializing for basic data type */
    DeSerContextPtr->BufferLengthCheck = SOMEIPXF_BDT_WITH_BUFFERLENGTH_CHECK;
  }
  /* check if less data than expected via LF value are handed over for deserialization */
  else if(NextElementBufferPos < BytesCopied)
  {
    RetVal = E_SER_MALFORMED_MESSAGE;
  }
...
)
{
  ...
  if( (TRUE == FSAContextPtr->FSA_Root) )
  {
    ...
    /* set data byte offset for FSA root entry */
    FSAContextPtr->FSA_RootEntryDataByteOffset = DeSerContextPtr->DataByteOffset;
  }
  else
  {
    /* CHECK: NOPARSE */
  ...
  /* CHECK: PARSE */
  {
    ...
    /* CHECK: PARSE */
    {
      ...
      for(dim_idx=0U; dim_idx<SOMEIPXF_GET_FSAContext_FSA_InfoType(FSA_max_config_DIM); dim_idx++)
      {
        ...
        SOMEIPXF_SET_FSAContext_DIM_LEVEL(FSA_LF_BufPos,
             DeSerContextPtr->BytesCopied + DeSerContextPtr->BufPosOffset);
        ...
        if(TRUE == con_valid_FSA_De_Seral)
        {
         ...
         uint32 Continue_BufPos = ((uint32) DeSerContextPtr->BytesCopied + (uint32) DeSerContextPtr->BufPosOffset +
             + (uint32) SOMEIPXF_GET_FSAContext_FSA_InfoType(FSA_LFsize));

          /* postcondition overflow check */
          if(Continue_BufPos < (uint32) DeSerContextPtr->BytesCopied)
          {
            /* invalid LF_BufPos */
            RetVal |= E_SER_MALFORMED_MESSAGE;
          }
          ...
          else
          {
            ...
            RetVal |= SomeIpXf_GetLFValues(
                ...
                (SOMEIPXF_GET_FSAContext_DIM_LEVEL(FSA_LF_BufPos) - DeSerContextPtr->BufPosOffset),
...
)
{
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufPosCount = 0U; /* position of serialized data */
  ...
  do{
     ...
     do{
        DataBytePtr[ConfigPtr[ConfPos] + ConfPosOffset + DeSerContextPtr->DataByteOffset] = BufPtr[DeSerContextPtr->BytesCopied + BufPosCount];
        BufPosCount++;
        ConfPos++;
  ...
  /* update number of bytes serialized and number of configuation elements handled */
  DeSerContextPtr->BytesCopied += BufPosCount;
  DeSerContextPtr->CfgElemHandled += ConfPos;
...
)
{
  ...
  uint32 Continue_BufPos = (uint32)DeSerContextPtr->BytesCopied +
      (uint32)DeSerContextPtr->BufPosOffset + ((uint32)NumOfArrayElem * SizeOfElem);
  /* postcondition overflow check */
  /* SomeIpXf_SetUpExtFSAContext detect already invalid LF value positions */
  /* CHECK: NOPARSE */
  if(Continue_BufPos < (uint32)DeSerContextPtr->BytesCopied)
  {
    /* invalid BytesCopied */
  ...
  /* CHECK: PARSE */
  /* FSA of BDT size check required due to previous possible relocation of the buffer pos by skipping of an
   * ext. DT, VSA or TLV */
  else if(Continue_BufPos > BufferLength)
  {
...
)
{
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE Element_ByteLength = (DeSerContextPtr->BytesCopied) -
      ((FSA_LF_BufPos - DeSerContextPtr->BufPosOffset) + FSA_LFsize);
#else
  ...
  SOMEIPXF_PARAM_UNUSED(FSA_Dim);
    ...
    {
      RetVal = SomeIpXf_Set_LF(BufPtr, ConfigPtr, (FSA_LF_BufPos - DeSerContextPtr->BufPosOffset), Element_ByteLength,
...
#endif /* (SOMEIPXF_FSA_MULTI_DIM_ENABLED == STD_ON) */
    {
      ...
      {
        /* set data byte offset to FSA root entry byte offset */
        DeSerContextPtr->DataByteOffset = FSAContextPtr->FSA_RootEntryDataByteOffset;
...
)
{
...
#if (SOMEIPXF_FSA_MULTI_DIM_ENABLED == STD_ON)
  {
    ...
    for(dim_idx = Current_DIM_LEVEL;dim_idx < Max_DIM_LEVEL; dim_idx++)
    {
      ...
      SOMEIPXF_SET_FSAContext_DIM_LEVEL(FSA_LF_BufPos, DeSerContextPtr->BytesCopied + DeSerContextPtr->BufPosOffset);
...
)
{
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE BytesCopiedBuf = DeSerContextPtr->BytesCopied + DeSerContextPtr->BufPosOffset;
  ...
  else if((NextElementBufferPos) > (BytesCopiedBuf))
  {
    /* LF_Value is greater than de-seralized bytes, apply skipping
     * of excess bytes */
    const uint32 NumSkipBytes = (uint32) ((NextElementBufferPos) - BytesCopiedBuf);

    /* check if less data than expected via BufferLength are handed over for deserialization */
    if((uint32) (BytesCopiedBuf) > BufferLength)
    {
      RetVal = E_SER_MALFORMED_MESSAGE;
    }
    else if((BytesCopiedBuf + NumSkipBytes) > BufferLength)
    {
      /* BytesCopied gets set to bufferlenght since skipping excceds bufferlenght */
      DeSerContextPtr->BytesCopied = BufferLength - DeSerContextPtr->BufPosOffset;
    }
    else
    {
      /* skipping behavior */
...
#if (SOMEIPXF_FSA_WITH_LF_AVAILABLE == STD_OFF)
  SOMEIPXF_PARAM_UNUSED(BufferLength);
#else
    if(SOMEIPXF_GET_FSAContext_FSA_InfoType(FSAWithLf) == TRUE)
    {
...
)
{
  ...
  for(dim_idx=0U;dim_idx<SOMEIPXF_GET_VSAContext_VSA_Info_LEVEL(VSA_max_config_DIM);dim_idx++)
  {
    ...
    /* set byte buffer pos of LF per dim */
    SOMEIPXF_SET_VSAContext_DIM_LEVEL(VSA_LF_BufPos,
        VSA_LF_BuffPos_loc + DeSerContextPtr->BytesCopied + DeSerContextPtr->BufPosOffset);
    ...
    else
    {
      ...
      uint32 Continue_BufPos = ((uint32) DeSerContextPtr->BytesCopied + (uint32) DeSerContextPtr->BufPosOffset +
          (uint32) VSA_LF_BuffPos_loc + (uint32) SOMEIPXF_GET_VSAContext_VSA_Info_LEVEL(VSA_LFsize));
      ...
      else
      {
        ...
        RetVal |= SomeIpXf_GetLFValues(
            &BufPtr[DeSerContextPtr->BytesCopied],
            ConfigPtr,
        ...
        /* CHECK: PARSE */
        {
          ...
          {
            /* following de-serialization of remaining VSA elements could either
             * enable skipping after max elements reached or exceeds bufferlenght
             * which leads to less data then expected by enabling overflow check of
             * basic data types */
            DeSerContextPtr->BufferLengthCheck = SOMEIPXF_BDT_WITH_BUFFERLENGTH_CHECK;
          }
...
)
{
  ...
  if( (TRUE == VSAContextPtr->VSA_Root) )
  {
    ...
    /* set data byte offset to VSA root entry */
    VSAContextPtr->VSA_RootEntryDataByteOffset = DeSerContextPtr->DataByteOffset;
  }
  else
  {
    /* CHECK: NOPARSE */
...
)
{
  ...
  /* calculate the current element byte length based from the LF position */
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE Element_ByteLength = (DeSerContextPtr->BytesCopied) -
      ((VSA_LF_BufPos - DeSerContextPtr->BufPosOffset) + VSA_LFsize);
  ...
  {
    RetVal = SomeIpXf_Set_LF(BufPtr, ConfigPtr, (VSA_LF_BufPos - DeSerContextPtr->BufPosOffset), Element_ByteLength,
...
#endif /* (SOMEIPXF_VSA_MULTI_DIM_ENABLED == STD_ON) */
    {
      ...
      {
        /* set data byte offset to VSA root entry byte offset */
        DeSerContextPtr->DataByteOffset = VSAContextPtr->VSA_RootEntryDataByteOffset;
  ...
  else /* CurrElement < Elements */
  {
...
#if (SOMEIPXF_VSA_MULTI_DIM_ENABLED == STD_ON)
    {
      ...
      for(dim_idx = Current_DIM_LEVEL;dim_idx < Max_DIM_LEVEL; dim_idx++)
      {
        ...
        /* save LF buffer pos of next dim */
        SOMEIPXF_SET_VSAContext_DIM_LEVEL(VSA_LF_BufPos, DeSerContextPtr->BytesCopied + DeSerContextPtr->BufPosOffset);
...
#endif /* (SOMEIPXF_VSA_MULTI_DIM_ENABLED == STD_ON) */
  {
    /* PRS_SOMEIP_00919: if skipping applied over BufferLength via LF of outermost dimension
     * set BytesCopied to BufferLength since otherwise less data as expected check
     * (BytesCopiedFromBuffer > BufferLength) after Xf_Inv call returns malfromed */
    if(((uint32)DeSerContextPtr->BytesCopied + (uint32)DeSerContextPtr->BufPosOffset) > BufferLength)
    {
      /* BytesCopied gets set to bufferlenght since skipping excceds bufferlenght */
      DeSerContextPtr->BytesCopied = BufferLength - DeSerContextPtr->BufPosOffset;
    }
    ...
    /* set CfgElemHandled to next tag after SOMEIPXF_VSA_STOP */
    SOMEIPXF_SET_CONFIGPOS(RetVal, CfgElemHandled, SOMEIPXF_VSA_STOP, VSA_ConfigPosVSAStop);
...
)
{
  ...
  if(TRUE == con_skipping_LF_larger_max_elem)
  {
    ...
    uint32 Continue_BufPos = ((uint32)DeSerContextPtr->BytesCopied + (uint32)DeSerContextPtr->BufPosOffset + NumSkipBytes);

    /* postcondition overflow check */
    if(Continue_BufPos < NumSkipBytes)
    {
      /* invalid LF_Value */
      RetVal = E_SER_MALFORMED_MESSAGE;
    }
    else
    {
      ...
      /* postcondition overflow check */
      if(DeSerContextPtr->BytesCopied < NumSkipBytes)
      {
        /* invalid LF_Value */
        RetVal = E_SER_MALFORMED_MESSAGE;
      }
  ...
  else
  {
...
#if (SOMEIPXF_VSA_MULTI_DIM_ENABLED == STD_ON)
    {
      ...
      for(dim_idx = Current_DIM_LEVEL;dim_idx < Max_DIM_LEVEL; dim_idx++)
      {
        ...
        Continue_BufPos = ((uint32) DeSerContextPtr->BytesCopied + (uint32) DeSerContextPtr->BufPosOffset +
            (uint32) SOMEIPXF_GET_VSAContext_VSA_Info_LEVEL(VSA_LFsize));
        ...
        else
        {
          ...
          RetVal |= SomeIpXf_GetLFValues(
              ...
              &ConfigPtr[0U],
              SOMEIPXF_GET_ADDR_VSAContext_DIM_LEVEL(VSA_LF_Value),
              SOMEIPXF_GET_VSAContext_DIM_LEVEL(VSA_LF_BufPos) - DeSerContextPtr->BufPosOffset,
...
)
{
  ...
  /* CHECK: PARSE */
  {
    ...
    /* set availability byte offset */
    SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(offsetAvailabilitybitfield) = ConfigPtr[CfgElemHandled + 3U];
...
)
{
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE CfgElemHandled = DeSerContextPtr->CfgElemHandled;
  const SOMEIPXF_XFRMBUFFERLENGTH_TYPE OptionalMemberIndex_AVB = ConfigPtr[CfgElemHandled + 4U];
  ...
  const SOMEIPXF_XFRMBUFFERLENGTH_TYPE byte_pos = (SOMEIPXF_XFRMBUFFERLENGTH_TYPE)
      ((OptionalMemberIndex_AVB / 8U) + SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(offsetAvailabilitybitfield)
      + DeSerContextPtr->DataByteOffset);
...
)
{
  ...
  else
  {
    ...
    /* CHECK: PARSE */
    {
      ...
      if( BufferLength < ((uint32)BytesCopied + (uint32)BufPosOffset +
  ...
  if(E_OK == RetVal)
  {
    ...
    const uint32 NextTLVMemberBufferPos = ((uint32)BytesCopied + SOMEIPXF_TLV_WIRETYPE_DATAID_SIZE +
        (uint32)TLV_static_lenghtfield_bytesize + DataMemSize);

    /* postcondition overflow check */
    if(NextTLVMemberBufferPos < DataMemSize)
    {
      /* invalid LF_Value */
      RetVal |= E_SER_MALFORMED_MESSAGE;
    }
    /* check if buffer pos after skipp is not eual or not within TLVStruct_LFsize */
    else if(NextTLVMemberBufferPos > TLVStruct_LFsize)
    {
      /* invalid LF value since skipping exceeds TLVStruct_LFsize */
      RetVal |= E_SER_MALFORMED_MESSAGE;
    }
    else
    {
      /* check if buffer pos after skipp excceds bufferlenght */
      if((NextTLVMemberBufferPos + BufPosOffset) > BufferLength)
      {
        /* BytesCopied gets set to bufferlenght since skipping excceds bufferlenght */
        BytesCopied = BufferLength - BufPosOffset;
      }
      else
      {
        /* skipping behavior according to wireType to next TLV tag of member */
        BytesCopied = (SOMEIPXF_XFRMBUFFERLENGTH_TYPE) NextTLVMemberBufferPos;
      }
      ...
      if(NextTLVMemberBufferPos == TLVStruct_LFsize)
      {
      ...
      /* decrease CfgElemHandled once for reenter of SomeIpXf_Inv_TlvStart for next TLV tag */
...
)
{
  ...
  const SOMEIPXF_XFRMBUFFERLENGTH_TYPE CfgElemHandled = DeSerContextPtr->CfgElemHandled;
  ...
  /* CHECK: PARSE */
  {
    ...
    /* CHECK: PARSE */
    {
      if(0U == DeSerContextPtr->TLV_LFSize_WireType567)
      {
        /* set static LF byte size */
        SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLV_static_lenghtfield_bytesize) = ConfigPtr[CfgElemHandled + 2U];
      }
      ...
      {
        SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLV_static_lenghtfield_bytesize) = DeSerContextPtr->TLV_LFSize_WireType567;
        DeSerContextPtr->TLV_LFSize_WireType567 = 0U;
      }
...
)
{
      ...
      (FALSE == TLVContextPtr->TLV_Struct_Deserial_FirstElement))
  {
    ...
    /* CHECK: PARSE */
    {
      ...
      if( BufferLength < ((uint32)DeSerContextPtr->BytesCopied + (uint32)DeSerContextPtr->BufPosOffset +
      ...
      else
      {
        ...
        /* CHECK: PARSE */
        {
          ...
          SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLVStruct_LFsize) =
              LFValue + SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLV_static_lenghtfield_bytesize) + DeSerContextPtr->BytesCopied;

          /* postcondition overflow check */
          if(SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLVStruct_LFsize) < LFValue)
          {
            /* invalid LF_BufPos */
            RetVal |= E_SER_MALFORMED_MESSAGE;
          }
          ...
          DeSerContextPtr->BytesCopied = DeSerContextPtr->BytesCopied + SOMEIPXF_TLV_STRUCT_WITH_INFO_TYPE(TLV_static_lenghtfield_bytesize);
        }
      }
    }
  }
  ...
  if(E_OK == RetVal)
  {
    /* Check TLV tag is within the recevied buffer */
    boolean TLVTagRead = ( BufferLength <= ((uint32)DeSerContextPtr->BytesCopied +
                          (uint32)DeSerContextPtr->BufPosOffset + SOMEIPXF_TLV_WIRETYPE_DATAID_SIZE)
                          );
    ...
    if(  /* LFValue is not zero else set TLV stop idx */
         ...
         /* and TLV tag read is not out of received buffer */
         && (TLVTagRead != TRUE))
    {
...
)
{
  ...
  /* check if buffer pos exceeds TLVStruct_LFsize */
  if(BytesCopied > TLVStruct_LFsize)
  {
    /* buffer pos exceeds TLVStruct_LFsize */
    RetVal |= E_SER_MALFORMED_MESSAGE;
  }
  /* check if end of TLV struct is already reached */
  ...
  /* check if end of BufferLength for next TLV member is already reached */
  else if(BufferLength < ((uint32)BytesCopied + (uint32)BufPosOffset +
  ...
  else /* BytesCopied < TLVStruct_LFsize */
  {
    /* TLV members left in the byte stream */
...
)
{
  ...
  const SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufPosOffset = *BufPosOffsetPtr;
...
)
{
  ...
  if(RetVal == E_OK)
    /* CHECK: PARSE */
  {
    /* arg buffer pos after skip */
    const uint32 NextTLVArgBufferPos = *TLV_ARG_BufPosPtr + SOMEIPXF_TLV_WIRETYPE_DATAID_SIZE +
        DataArgSize + (uint32)TLV_static_lenghtfield_bytesize;

    /* postcondition overflow check */
    if(NextTLVArgBufferPos < DataArgSize)
    {
      /* invalid LF_Value */
      RetVal |= E_SER_MALFORMED_MESSAGE;
    }
    ...
    {
      /* check if arg buffer pos after skip excceds bufferlenght */
      if(NextTLVArgBufferPos > BufferLength)
      {
        /* madatory missing TLV argument within stream */
        RetVal |= E_SER_MALFORMED_MESSAGE;
      }
      else
      {
        /* skipping behavior to next TLV tag of argument */
        *TLV_ARG_BufPosPtr = NextTLVArgBufferPos;

        DeSerContextPtr->BytesCopied = (SOMEIPXF_XFRMBUFFERLENGTH_TYPE) ((uint32)DeSerContextPtr->BytesCopied +
            SOMEIPXF_TLV_WIRETYPE_DATAID_SIZE + DataArgSize + (uint32)TLV_static_lenghtfield_bytesize);
      }
...
)
{
  ...
  uint32 Continue_BufPos = *TLV_ARG_BufPosPtr + SOMEIPXF_TLV_WIRETYPE_DATAID_SIZE;
  ...
  /* postcondition overflow check */
  if(Continue_BufPos < *TLV_ARG_BufPosPtr)
  {
    /* invalid TLV_ARG_BufPosPtr */
    RetVal = E_SER_MALFORMED_MESSAGE;
  }
  /* This check ensures that data id will not bre read from invalid address */
  else if(Continue_BufPos > BufferLength)
  {
    /* TLV_ARG_BufPosPtr has reached the maximum length */
    RetVal = E_SER_MALFORMED_MESSAGE;
  }
  else
  {
...
)
{
  ...
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE BufPosCount = 0U; /* indicates position of serialized data */
  SOMEIPXF_XFRMBUFFERLENGTH_TYPE ConfPos = 2U; /* neglects initial SOMEIPXF_BASIC_DATA_TYPE_START and
   SOMEIPXF_BASIC_DATA_TYPE_CONFIG */

  do{
    DataBytePtr[ConfigPtr[ConfPos] + DeSerContextPtr->DataByteOffset] = BufPtr[DeSerContextPtr->BytesCopied + BufPosCount];
    BufPosCount++;
    ConfPos++;
...
)
{
  ...
  uint32 Continue_BufPos = (uint32)DeSerContextPtr->BytesCopied +
      (uint32)DeSerContextPtr->BufPosOffset + BasicTypeSize;
  ...
  /* BDT size check required due to previous possible relocation of the buffer pos by skipping of an
   * ext. DT,VSA or TLV */
  else if(Continue_BufPos > BufferLength)
  {
    /* LF skipping value was larger then expected (bufferlength) */
...
)
{
  ...
  do{
    ...
    do{
      DataBytePtr[ConfigPtr[ConfPos] + ConfPosOffset + DeSerContextPtr->DataByteOffset] =
          BufPtr[DeSerContextPtr->BytesCopied + BufPosCount];
      BufPosCount++;
      ConfPos++;
