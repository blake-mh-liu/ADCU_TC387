...
[!VAR "ASR32ServiceAPIEnabled" = "node:existsAndTrue(DetServiceAPI/DetEnableASR32ServiceAPI)"!]
[!VAR "ASR40ServiceAPIEnabled" = "node:existsAndTrue(DetServiceAPI/DetEnableASR40ServiceAPI)"!]
[!VAR "ASR43ServiceAPIEnabled" = "node:existsAndTrue(DetServiceAPI/DetEnableASR43ServiceAPI)"!]
...
#error DET_FORWARD_TO_DLT already defined
#endif
#define DET_FORWARD_TO_DLT                            [!IF "(node:exists(as:modconf('Det')/DetGeneral/DetForwardToDlt) and (as:modconf('Det')/DetGeneral/DetForwardToDlt = 'true'))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 **
 ** En- or disable the availability of Det_GetVersionInfo() */
#define DET_VERSION_INFO_API                [!//
[!IF "node:existsAndTrue(DetVersionInfoApi)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "node:existsAndTrue(DetRteUsage)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
 ...
 ** analyses the situation in a debugger/emulator. */
...
[!IF "node:existsAndTrue(KeepFirst)"!]DET_KEEP_FIRST_REPORTS[!ELSE!]DET_KEEP_LAST_REPORTS[!ENDIF!]
[!ENDIF!][!// /* Logging mode */
...
[!IF "node:existsAndTrue(EnableReportErrorStopExecution)"!]TRUE[!ELSE!]FALSE[!ENDIF!]
[!ENDSELECT!][!//
...
[!IF "node:existsAndTrue(DetDefensiveProgrammingEnabled)"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(DetDefensiveProgrammingEnabled)) and (node:existsAndTrue(DetPreconditionAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(DetDefensiveProgrammingEnabled)) and (node:existsAndTrue(DetPostconditionAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(DetDefensiveProgrammingEnabled)) and (node:existsAndTrue(DetStaticAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(DetDefensiveProgrammingEnabled)) and (node:existsAndTrue(DetUnreachableCodeAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "(node:existsAndTrue(DetDefensiveProgrammingEnabled)) and (node:existsAndTrue(DetInvariantAssertEnabled))"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
