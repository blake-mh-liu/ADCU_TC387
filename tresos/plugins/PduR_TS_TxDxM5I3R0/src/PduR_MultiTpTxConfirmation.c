...
)
{
...
#endif /* if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      if(PDUR_MULTITPTX_NO_PENDING_TX_MSK == PendingTxCount)
      {
         ...
         /* get local Result value, so that the session can be released before Up_TpTxConfirmation is called,
            this allows calling PduR_UpTpTransmit in the same context by the upper layer module */
         const NotifResultType LocalResult = pMultiTpTxSessionInfo[SessionIndex].ResultTxConf;
         ...
         TxConfirmationFp(TargetPduId, LocalResult);
      }
   }
