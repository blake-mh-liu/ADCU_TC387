...
)
{
  ...
  else
  {
    ...
    CONSTP2CONST(uint16, AUTOMATIC, PDUR_APPL_CONST) pDestPduBufferIdRef =
        PDUR_GET_CONFIG_ADDR(uint16, pRPGDestBufConfig->DestPduBufferIdRef);
#endif
    ...
    do
    {
      ...
      case PDUR_DEST_SINGLE_BUFFER_DYNAMIC_PAYLOAD:
        /* single buffer is set to default value (for trigger transmit data provision) */
      ...
      case PDUR_DEST_TRIGGERED_FIFO:
        /* FIFO flushed, first buffer set to default value */
...
/* Single buffers without default value are used by inter-core communication for a
 * - non-TP destination PDU that goes to a lower layer module with direct data provision in a (multicast) gateway path
 * - non-TP destination PDU that goes to an upper layer module in a multicast gateway path */
#if (PDUR_MULTICORE_SUPPORT == STD_ON)
  if (pDefaultValues != NULL_PTR)
...
typedef void PduR_PreventEmptyTranslationUnit_DisableRoutingType;
