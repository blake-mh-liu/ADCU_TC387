...
)
{
...
#endif
  {
    ...
    /* for multicore, the input parameter of PduR_GateTpCopyTxData is GMPduIdTx (not GMPduIdRx) */
#if (PDUR_MULTICORE_SUPPORT == STD_ON)
    const PduR_PduIdType GMPduId = PduR_GetGMPduIdTxFromGMPduIdRx(*pGMPduIdMem);
#else
    const PduR_PduIdType GMPduId = *pGMPduIdMem;
#endif /* #if (PDUR_MULTICORE_SUPPORT == STD_ON) */

    RetVal = PduR_GateTpCopyTxData(GMPduId, PduInfoPtr, RetryInfoPtr, AvailableDataPtr);
  }
...
typedef void PduR_PreventEmptyTranslationUnit_Nto1GateTpCopyTxDataType;
