...
FUNC(void, PDUR_CODE) PduR_GateIfSbNoInitMCoreRxIndication
(
    PduIdType GPduId,
    P2VAR(PduInfoType, AUTOMATIC, PDUR_APPL_DATA) PduInfoPtr
)
{
   DBG_PDUR_GATEIFSBNOINITMCORERXINDICATION_ENTRY(GPduId, PduInfoPtr);

#if (PDUR_DEV_ERROR_DETECT == STD_ON)
   /* check if input parameter is NULL */
   if (NULL_PTR == PduInfoPtr->SduDataPtr)
   {
       PDUR_DET_REPORT_ERROR(PDUR_SID_GATEIF_SBNOINIT_MCORE_RXIND, PDUR_E_NULL_POINTER);
   }
   else
   ...
   DBG_PDUR_GATEIFSBNOINITMCORERXINDICATION_EXIT(GPduId, PduInfoPtr);
}
...
typedef void PduR_PreventEmptyTranslationUnit_GateIfSbNoInitMCoreRxIndicationType;
