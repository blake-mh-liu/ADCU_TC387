...
)
{
   ...
   /* HeadIdx for later check on preemption if still the same queue element is considered */
   PduR_GateTpQueueIndexType HeadIndexInUse = PDUR_NO_GATETP_QUEUEING_POSSIBLE;
...
#endif /* if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      /* get HeadIdx for later check on preemption if still the same queue element is considered */
      HeadIndexInUse = pGateTpQueueInfo[GMPduId].HeadIdx;
#else
      HeadIndexInUse = PDUR_GATETPQUEUE_FIRSTINDEX;
   ...
   if(CallLoTpTransmit)
   {
      ...
      /* TpRxIndication preempted by final TpTxConfirmation(s) of same routing path
       * The buffer might be reassigned to another GMPduId already and the session released */
      if(SessionIndex == pGateTpSessionIndex[HeadIndexInUse])
      {
...
typedef void PduR_PreventEmptyTranslationUnit_GateTpRxIndicationType;
