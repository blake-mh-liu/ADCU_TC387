...
FUNC(void,PDUR_CODE) PduR_GateIfDfMCoreRxIndication
(
    PduIdType GPduId,
    P2VAR(PduInfoType, AUTOMATIC, PDUR_APPL_DATA) PduInfoPtr
)
{
   DBG_PDUR_PDUR_GATEIFDFMCORERXINDICATION_ENTRY(GPduId, PduInfoPtr);

#if (PDUR_DEV_ERROR_DETECT == STD_ON)
   /* check if input parameter is NULL */
   if (NULL_PTR == PduInfoPtr->SduDataPtr)
   {
      PDUR_DET_REPORT_ERROR(PDUR_SID_GATEIF_DF_MCORE_RXIND, PDUR_E_NULL_POINTER);
   }
   else
#endif /* #if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      /* actually used SduLength of the PDU passed to destination
       * initialize SDU length value from PduInfoPtr: the SduLength data type (fix PduLengthTypeEnum
      ...
      uint8 ActUsedSduLength = (uint8)PduInfoPtr->SduLength;
      ...
      /* overwrite length value of PDU to be transmitted */
      if(PduInfoPtr->SduLength > LenDfBuffer)
      {
         /* use buffer length configured */
         ActUsedSduLength = LenDfBuffer;
      }

      /* store length value of PDU to be transmitted */
      pUsedSduLength[TailPos] = ActUsedSduLength;

      /* Copy data to Tail of D-FIFO buffer of PDU Router */
      TS_MemCpy(&pBufMem[TailPos * LenDfBuffer], PduInfoPtr->SduDataPtr, ActUsedSduLength);
      ...
      if (Transmit == TRUE)
      {
         ...
         /* initialized that way since ActUsedSduLength is not computable at load time */
         PduR_PduInfo.SduLength = ActUsedSduLength;
      ...
      {
         /* Report flush of FIFO to Det */
         PDUR_DET_REPORT_ERROR(PDUR_SID_GATEIF_DF_MCORE_RXIND, PDUR_E_PDU_INSTANCES_LOST);
      }
#endif /* (PDUR_DEV_ERROR_DETECT == STD_ON) */
   }

   DBG_PDUR_GATEIFDFMCORERXINDICATION_EXIT(GPduId, PduInfoPtr);
}
...
typedef void PduR_PreventEmptyTranslationUnit_GateIfDfMCoreRxIndicationType;
