 ...
 **
 **/
extern FUNC(Std_ReturnType,PDUR_CODE) PduR_MCastIfMCoreUpTransmit
(
   PduIdType MPduId,
   P2CONST(PduInfoType,AUTOMATIC,PDUR_APPL_DATA) PduInfoPtr
);
 ...
 ** \brief Auxiliary TP gateway handler function
 **
 ** Auxiliary function that releases the TP gateway session queue for GMPduId.
 **
 ...
 ** \brief Auxiliary TP gateway handler function
 **
 ** Auxiliary function that determines the GMPduIdTx, the ID used for accessing GTabTpTx,
 ** based on GMPduIdRx (the ID used for accessing GTabTpRx). In a multicore scenario the GMPduIdTx
 ** is used as input parameter to PduR_GateTpCopyTxData and PduR_GateTpTxConfirmation.
 **
 ** \param[in]  GMPduIdRx   Identification of the gateway (multicast) PDU.
 **
 ** \return GMPduIdTx, the ID used for accessing GTabTpTx
 **/
TS_MOD_PRIV_DECL FUNC(PduR_PduIdType, PDUR_CODE) PduR_GetGMPduIdTxFromGMPduIdRx
(
 ...
 **                           pointer to data buffer.
 **/
extern FUNC(void,PDUR_CODE) PduR_GateIfSbNoInitMCoreRxIndication
(
   PduIdType  GPduId,
   P2VAR(PduInfoType,AUTOMATIC,PDUR_APPL_DATA) PduInfoPtr
);
 ...
 **                           pointer to data buffer.
 **/
extern FUNC(void,PDUR_CODE) PduR_GateIfSbNoInitMCoreUpRxIndication
(
   PduIdType  GPduId,
   P2VAR(PduInfoType,AUTOMATIC,PDUR_APPL_DATA) PduInfoPtr
);
#endif /* (PDUR_MULTICAST_FROMIF_SUPPORT == STD_ON) */
 ...
 **                           pointer to data buffer.
 **/
extern FUNC(void,PDUR_CODE) PduR_GateIfDfMCoreRxIndication
(
   PduIdType  GPduId,
   P2VAR(PduInfoType,AUTOMATIC,PDUR_APPL_DATA) PduInfoPtr
);
 ...
 ** \param[in]  GPduId  ID of If-gateway PDU
 **/
extern FUNC(void,PDUR_CODE) PduR_GateIfDfMCoreTxConfirmation
(
    PduIdType  GPduId
);
#endif /* (PDUR_MULTICORE_SUPPORT == STD_ON) */
