...
#if(((ETHTSYN_HW_TIMESTAMP_SUPPORT == STD_ON) || (ETHTSYN_SWITCH_USED == STD_ON) && (ETHTSYN_HW_TIMESTAMP_SUPPORT == STD_ON) || (ETHTSYN_TIME_RECORDING == STD_ON)))
...
#endif /* (((ETHTSYN_HW_TIMESTAMP_SUPPORT == STD_ON) || (ETHTSYN_SWITCH_USED == STD_ON) && (ETHTSYN_HW_TIMESTAMP_SUPPORT == STD_ON) || (ETHTSYN_TIME_RECORDING == STD_ON))) */
...
)
{
  ...
  uint8 Idx;
  Eth_TimeStampType TempTimeStamp = {0U, 0U, 0U};
  ...
  /* Add remaining timestamp from NanosecondsLo */
  TempTimeStamp.nanoseconds += VirtualLocalTimePtr->nanosecondsLo;

  EthTSyn_AddTimeStamps(EthTimePtr, &TempTimeStamp);
}
