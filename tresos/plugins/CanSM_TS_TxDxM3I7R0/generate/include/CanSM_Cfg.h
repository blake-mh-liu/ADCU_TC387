...
#if (defined [!"text:toupper(as:name(.))"!]_ID)
#error [!"text:toupper(as:name(.))"!]_ID is already defined
#endif
#define [!"text:toupper(as:name(.))"!]_ID   [!"$PartitionID"!]U
  [!ENDLOOP!][!//
...
/** \brief Enable Channels initialization distribution support*/
#if (defined CANSM_DISTRIBUTED_CHANNEL_INIT_ENABLED)
#error CANSM_DISTRIBUTED_CHANNEL_INIT_ENABLED already defined
#endif
#define CANSM_DISTRIBUTED_CHANNEL_INIT_ENABLED           [!//
[!IF "as:modconf('CanSM')[1]/CanSMGeneral/CanSMDistributedChannelInitEnable = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]

[!IF "($ChannelsDistributionSupport = 'true')"!][!/*
*/!][!IF "$ChannelsInitDistributionSupport = 'true'"!]
/** \brief Enable Channels initialization distribution support*/
#if (defined CANSM_MULTI_INIT_COUNT)
#error CANSM_MULTI_INIT_COUNT already defined
#endif
#define CANSM_MULTI_INIT_COUNT                           [!CALL "NumberOfPartitions"!]U
[!ENDIF!][!/*
*/!][!ENDIF!]
...
[!IF "($ChannelsDistributionSupport = 'true')"!][!/*
*/!][!IF "$ChannelsInitDistributionSupport = 'true'"!]
/** \brief Number of CanSM_Init() calls in case of Multi-Partitioned/Multi-CoreProcessor . */
extern VAR(uint8, CANSM_VAR) CanSM_MultiCoreInitCount;
[!ENDIF!][!/*
*/!][!ENDIF!]
...
[!IF "($ChannelsDistributionSupport = 'true')"!][!/*
*/!][!IF "$ChannelsInitDistributionSupport = 'true'"!]
/** \brief SpinLock for handling race condition that might happens in CanSM_Init() in
 * case of Multi-Partitioned/Multi-CoreProcessor .
 */
extern VAR(Atomic_t, CANSM_VAR) CanSM_InitInProgress;
[!ENDIF!][!/*
*/!][!ENDIF!]
