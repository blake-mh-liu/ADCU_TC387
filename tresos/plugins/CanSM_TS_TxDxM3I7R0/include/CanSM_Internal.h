...
#if(CANSM_DISTRIBUTED_CHANNEL_INIT_ENABLED == STD_ON)

#if (defined CANSM_INIT_SPIN_LOCKED_WAIT)      /* To prevent double declaration */
#error CANSM_INIT_SPIN_LOCKED_WAIT already defined!
#endif /* #if (defined CANSM_INIT_SPIN_LOCKED_WAIT) */
/** \brief Wait till the lock becomes free and acquire the Lock.
 * \param[in,out] Lock The pointer to the Lock.
 * \param[in] SelectionMask The bit mask to select one bit in the Lock.
 */
#define CANSM_INIT_SPIN_LOCKED_WAIT(Lock,SelectionMask)\
  do \
  { \
    boolean isLocked = (boolean) Atomics_TestAndSetFlag(&(Lock),(Atomic_value_t)(SelectionMask)); \
    if(isLocked == FALSE) \
    { \
      break; \
    } \
