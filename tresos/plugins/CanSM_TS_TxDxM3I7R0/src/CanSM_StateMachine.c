...
/**
 * \brief Calls CanSM_WaitForModeIndication waiting for a mode indication.
 *
 * \param[in] nw Network index
 * \param[in] internalState Copy of CanSM_MainFunctionInternal local variable
 * \param[in,out] nextTransition If a transition is triggered, the subroutine sets the parameter to
 * the corresponding value
 */
STATIC FUNC(void, CANSM_CODE) CanSM_WaitForModeIndication
(
  const CanSM_NetworkIndexType nw,
  const CanSM_InternalStateType internalState,
  CONSTP2VAR(CanSM_TransitionType, AUTOMATIC, AUTOMATIC) nextTransition
...
)
{
  ...
  if((state == CANSM_S_NOIN) || (state == CANSM_S_NOCO) ||
     (state == CANSM_S_SICO) || (state == CANSM_S_FUCO_BUS_OFF_CHECK) ||
     (state == CANSM_S_FUCO_NO_BUS_OFF) || (state == CANSM_S_FUCO_TX_OFF))
  {
    /* do nothing */
  }
#if ( (CANSM_PNSUPPORT == STD_ON) && (CANSM_USE_TRANSCEIVER == STD_ON) )
  else if((state == CANSM_S_PN_CLEAR_WUF) || (state == CANSM_S_PN_CC_STOPPED) ||
          (state == CANSM_S_PN_TRCV_NORMAL) || (state == CANSM_S_PN_TRCV_STANDBY) ||
          (state == CANSM_S_PN_CC_SLEEP) || (state == CANSM_S_PN_CHECK_WFLAG_IN_CC_SLEEP) ||
          (state == CANSM_S_PN_CHECK_WFLAG_IN_NOT_CC_SLEEP))
  {
    result = TRUE;
  }
#endif /* ( (CANSM_PNSUPPORT == STD_ON) && (CANSM_USE_TRANSCEIVER == STD_ON) ) */
...
)
{
  ...
  if (DoMainFunctionProcessing == TRUE)
  {
    ...
    if ( nextTransition == CANSM_T_NONE )
    {
      ...
      {
        CanSM_WaitForModeIndication(nw, internalState, &nextTransition);
      }
      else
      {
        /* do nothing */
...
)
{
  ...
  if (CANSM_NW_INFO_STATE(nw) == (uint8)CANSM_UNINITED)
  {
...
#if (CANSM_DEV_ERROR_DETECT == STD_ON)
        CANSM_DET_WITH_INSTANCEID(CANSM_NW_HANDLE_FROM_IDX(nw),
                                  CANSM_SID_MAINFUNCTION,
