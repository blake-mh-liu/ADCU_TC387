...
/* !LINKSTO Ea.ASR40.SWS_Ea_00049,1, Ea.SWS_Ea_91001,1 */
...
/** \brief Calls the function Memory Driver Cancel.
 **
 ** This function calls the Memory Driver Cancel() to cancel ongoing jobs.
 **
 ** \ServiceID{4}
 ** \Reentrancy{non reentrant}
 ** \Synchronicity{asynchronous}
 ...
 **
 ** Calls the GetStatus function of the underlying Memory Driver
 ...
 ** one if none is ongoing. Calls the function GetJobResult of the
 ** Memory Driver. If it returns MEMIF_JOB_OK, the result from the Memory
 ** Abstraction module is returned, otherwise, the result from the
 ** Memory Driver is returned.
 **
 ...
 ** Driver function Memory Driver Write.
 **
 ** \param[in]  BlockNumber  Identifier of the block (from the configuration).
 **
 ** \return Standard Return Code
 ...
 ** written during the next write operation. Uses the Memory Driver function
 ** Erase.
