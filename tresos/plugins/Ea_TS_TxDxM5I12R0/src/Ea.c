...
/*==================[inclusions]============================================*/
#include <Ea_Trace.h>
/* !LINKSTO Ea.ASR40.SWS_Ea_00113,1 */
...
/* patch version check */
#if (EA_SW_PATCH_VERSION < 23U)
#error EA_SW_PATCH_VERSION wrong (< 23U)
#endif
...
/* !LINKSTO Ea.SWS_Ea_00007,1 */
#define EA_CALC_ADDRESS(block_number, offset, current_copy) \
   (Ea_LowerLayer_AddressType)(Ea_BlockConfig[(block_number)].StartAddress + \
   (((Ea_LowerLayer_LengthType)(current_copy) - 1U) * Ea_BlockConfig[(block_number)].BlockPageSize) + \
   (offset))
...
#define EA_GET_BLOCK_SIZE(block_number) \
  ((Ea_LowerLayer_LengthType)(Ea_BlockConfig[(block_number)].BlockPageSize \
...
#define EA_NEXT_VLEC(value) \
   ((((value) + 1U) != EA_LOWERLAYER_ERASE_VALUE) ? ((value) + 1U) : ((value) + 2U))
...
#if (EA_USE_MEMACC == STD_ON)
/** \brief Gets the consolidate Job result in case MemAcc is used.
 **    This is needed for legacy reasons as MemAcc_GetJobResult
 **    has a different behavior then Eep_GetJobResult.
 **
 */
STATIC FUNC(MemIf_JobResultType, EA_CODE) Ea_MemAcc_GetConsolidatedJobResult(void);
#endif
...
VAR(Ea_VLEBlockInfoType, EA_VAR) Ea_VLEBlockInfo[EA_NUMBEROFBLOCKS];
...
#if (EA_ALIGNMENT_VALUE == 1)
#define EA_START_SEC_VAR_CLEARED_8
#include <Ea_MemMap.h>
#elif (EA_ALIGNMENT_VALUE == 2)
#define EA_START_SEC_VAR_CLEARED_16
#include <Ea_MemMap.h>
#elif (EA_ALIGNMENT_VALUE == 4)
#define EA_START_SEC_VAR_CLEARED_32
#include <Ea_MemMap.h>
#else
#define EA_START_SEC_VAR_CLEARED_64
#include <Ea_MemMap.h>
#endif

VAR(uint8, EA_VAR) Ea_TempBuffer[EA_TEMPBUFFER_LENGTH];
...
#include <Ea_MemMap.h>
#else
#define Ea_STOP_SEC_VAR_CLEARED_64
#include <Ea_MemMap.h>
#endif
/*==================[internal data]=========================================*/
...
STATIC VAR(uint8, EA_VAR) Ea_SearchPreviousCopy;
/** \brief Information for VLE copy search: current copy. */
STATIC VAR(uint8, EA_VAR) Ea_SearchCurrentCopy;
/** \brief Information for VLE copy search: previous VLE Counter. */
STATIC VAR(uint8, EA_VAR) Ea_SearchPreviousVLEC;
/** \brief Information for VLE copy search: current VLE Counter. */
STATIC VAR(uint8, EA_VAR) Ea_SearchCurrentVLEC;
...
STATIC VAR(uint16, EA_VAR) Ea_InternalBlockNumber;

/** \brief Information for current job: offset in the block. */
STATIC VAR(uint16, EA_VAR) Ea_BlockOffset;

/** \brief Information for current job: length to read. */
STATIC VAR(uint16, EA_VAR) Ea_Length;
...
/*--------------------------[EA_MemAcc_GetConsolidatedJobResult]--------------------------------*/
/* !LINKSTO Ea.EaMemAccUsage.GetJobResultCompatibility,1 */
#if (EA_USE_MEMACC == STD_ON)
STATIC FUNC(MemIf_JobResultType, EA_CODE) Ea_MemAcc_GetConsolidatedJobResult
(
  void
)
{
  ...
  if(Ea_LowerLayer_GetStatus() == MEMACC_JOB_PENDING)
  {
    Return = MEMIF_JOB_PENDING;
  }
  else
  {
    switch (MemAcc_GetJobResult(EA_MEMACC_AREA_ID))
    {
      case MEMACC_MEM_OK:
...
)
{
  ...
  else
  {
    ...
    {
#endif
      /* Cancel memory driver */
      Ea_LowerLayer_Cancel();
...
)
{
  ...
  /* Ea_LowerLayer_GetStatus() is not called, Ea internal status is returned. */
...
#endif
  {
    /* Ea_LowerLayer_GetJobResult() is not called, Ea internal job result is returned */
...
)
{
  ...
  TS_MemSet(Ea_TempBuffer, EA_LOWERLAYER_ERASE_VALUE, EA_TEMPBUFFER_LENGTH);
  ...
  if ( EA_TOTAL_BLOCK_SIZE(Ea_InternalBlockNumber) <= EA_TEMPBUFFER_LENGTH )
  {
    ...
    Ea_TempBuffer[EA_INVALID_FLAG_POS(Ea_InternalBlockNumber)] = EA_LOWERLAYER_ERASE_VALUE;

    /* Call EEP to write block */
    retValue = Ea_LowerLayer_Write(
...
{
  MemIf_JobResultType jobResult = Ea_LowerLayer_GetJobResult();
  ...
  else if ( MEMIF_JOB_OK == jobResult )
  {
    ...
    if (E_OK == Ea_LowerLayer_Write(
        ...
        (Ea_LowerLayer_LengthType)length))
    {
      /* Switch to last state */
...
)
{
  ...
  else if (MEMIF_JOB_OK == jobResult)
  {
    ...
    if (Ea_TempBuffer[0] != EA_LOWERLAYER_ERASE_VALUE)
    {
      /* Check if block is valid (was not invalidated or erased) */
      if (Ea_TempBuffer[2U*EA_VLEC_LENGTH] == EA_LOWERLAYER_ERASE_VALUE)
      {
        ...
        if (E_OK == Ea_LowerLayer_Read(
            ...
            (Ea_LowerLayer_LengthType)Ea_Length))
        {
...
)
{
  ...
  else if (MEMIF_JOB_OK == jobResult)
  {
    ...
    /* Check if Block is not erased */
    if (Ea_TempBuffer[EA_STARTMARKER_POS] != EA_LOWERLAYER_ERASE_VALUE)
    {
      if (Ea_TempBuffer[EA_INVALID_FLAG_POS(Ea_InternalBlockNumber)] == EA_LOWERLAYER_ERASE_VALUE)
      {
...
)
{
  ...
  else if (MEMIF_JOB_OK == jobResult)
  {
    ...
    if (E_OK == Ea_LowerLayer_Erase(EA_CALC_ADDRESS(Ea_InternalBlockNumber, 0U, EA_NEXT_COPY(Ea_InternalBlockNumber)), EA_TOTAL_BLOCK_SIZE(Ea_InternalBlockNumber)))
    {
...
)
{
  ...
  else if (MEMIF_JOB_OK == jobResult)
  {
    ...
    if (Ea_SearchCurrentVLEC == Ea_TempBuffer[0])
    {
      ...
      if (Ea_SearchCurrentVLEC != EA_LOWERLAYER_ERASE_VALUE)
      {
