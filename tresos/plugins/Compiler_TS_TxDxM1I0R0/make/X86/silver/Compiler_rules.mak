...
define build-exe-file
	$(info  Generating sbs file)
	$(create_sbs_file)
	$(info  Executing sbsBuild tool from Silver)
	$(SBS_BUILD) silver_vecu.sbs --output ./../output/bin/$(TARGET)_$(DERIVATE)_$(PROJECT_NAME) -d ./../output/temp -K --automation\
	&& echo === BUILD DONE ===
endef
...
define build-cpp-file
	...
	$(file > $(DEPEND_DIR)/$(notdir $(basename $@)).d, DUMMY_VAR=compile error in c file $(notdir $<))
	$(EB_VERBOSE)$(GPP) -pipe @$(MAKE_INCPP) -MMD -MF $(DEPEND_DIR)/$(notdir $(basename $@)).d -MT $$(OBJ_DIR)/$(notdir $(basename $@)).o -c -o $(OBJ_DIR)/$(notdir $(basename $@)).o $<
endef
