...
define build-cpp-file
	...
	$(file > $(DEPEND_DIR)/$(notdir $(basename $@)).d, TEST=compile error in cpp file $(notdir $<))
	$(EB_VERBOSE)$(CC) --pass-c++ --dep-file=$(DEPEND_DIR)/$(notdir $(basename $<).d)  -f $(MAKE_INCPP) -co $< -o $(OBJ_DIR)/$(notdir $(basename $<).o)
endef
