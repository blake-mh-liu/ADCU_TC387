...
define build-exe-file
	$(info Linking $(notdir $@))
	$(EB_VERBOSE)$(LINK) -Wl,-Map=$(MAPFILE) @$(subst \,/,$(LNK_INC)) -Wl,-T $(GET_LOC_FILE) -o $(subst \,/,$@)\
	&& echo === BUILD DONE ===
endef
...
define build-c-file
	...
	$(EB_VERBOSE)$(CC) @$(MAKE_INC) -MT $(OBJ_DIR)/$(notdir $(basename $<).o) -MM @$(DEPEND_INC) $< -MF $(DEPEND_DIR)/$(notdir $(basename $@)).d
