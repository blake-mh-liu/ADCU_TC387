...
define build-exe-file
	...
	$(EB_VERBOSE)$(call runIn,$(OUTPUT_DIR)/,$(LINK) @$(LNK_INC) -o $(call cygpath_m,$@) -Wl$(COMMA)--start-group $(addprefix -l, $(notdir $(basename $(LIBRARIES_TO_BUILD) $(LIBRARIES_LINK_ONLY)))) -Wl$(COMMA)--end-group $(LINK_OPT_POST))\
	&& echo === BUILD DONE ===
endef
...
define build-postbuild-file
	...
	&& $(OBJCOPY) -S -X -O binary $(basename $@).temp $(call cygpath_m,$@)\
...
define build-c-file
	...
	$(EB_VERBOSE)$(call runIn,$(OUTPUT_DIR)/,$(CC) -pipe @$(MAKE_INC) $(addprefix -I,$(USER_INCLUDE_PATH)) -MMD -MF $(DEPEND_DIR)/$(notdir $(basename $@)).d -MT "$$(OBJ_DIR)/$(notdir $(basename $@)).o" -o $(OBJ_DIR)/$(notdir $(basename $@)).o) -c $(call cygpath_m,$<)
endef

define build-cpp-file
	$(info Compiling cpp: $(notdir $<))
	$(file > $(DEPEND_DIR)/$(notdir $(basename $@)).d, DUMMY_VAR=compile error in c file $(notdir $<))
	$(EB_VERBOSE)$(call runIn,$(OUTPUT_DIR)/,$(GPP) -pipe @$(MAKE_INCPP) $(addprefix -I%,$(USER_INCLUDE_PATH)) -MMD -MF $(DEPEND_DIR)/$(notdir $(basename $@)).d -MT "$$(OBJ_DIR)/$(notdir $(basename $@)).o" -o $(OBJ_DIR)/$(notdir $(basename $@)).o) -c $(call cygpath_m,$<)
endef

define build-prep-file
	$(info Preprocessing: $(notdir $<))
	$(EB_VERBOSE)$(CC) -E @$(DEPEND_INC) $(call cygpath_m,$<) > $(LIST_DIR)/$(notdir $(basename $@)).$(PREP_FILE_SUFFIX)
endef
...
define build-lib-file
	$(call SC_CMD_LISTPERFORM,SC_CMD_AR_HELPER,\
		...
		$(EB_VERBOSE)cd $(call fixCmdPaths,$(OBJ_DIR)) && $(CCLIB) crus $(call cygpath_m,$@),50,51, $(subst $(OBJ_DIR)/,,$?))
endef
