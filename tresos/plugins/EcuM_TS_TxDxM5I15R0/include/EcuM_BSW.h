...
/* !LINKSTO EcuM.ASR403.EcuM2991,1 */
...
#define ECUM_CONTINUE_SYNC                              0x00U
#endif
...
/* !LINKSTO EcuM.ASR403.EcuM2676,1 */
...
/** \brief Select target state for shutdown sequence using AUTOSAR R20-11 API.
 **
 ** This function doesn't initiate any shutdown activities but only stores the
 ** target state for later use when performing a shutdown sequence. During
 ** SHUTDOWN a change of the shutdown target from and to the value
 ** ECUM_SHUTDOWN_TARGET_SLEEP is not allowed.
 **
 ** Development Error Detection
 ** - ECUM_E_STATE_PAR_OUT_OF_RANGE: On invalid value for parameter \p
 **   shutdownTarget.
 **
 ** \param[in] shutdownTarget The selected shutdown target. Only the following subset
 ** of the EcuM_StateType value range is accepted:
 **   - ECUM_SHUTDOWN_TARGET_SLEEP
 **   - ECUM_SHUTDOWN_TARGET_RESET
 **   - ECUM_SHUTDOWN_TARGET_OFF
 ** \param[in] shutdownMode An index like value which can be dereferenced to a sleep
 ** mode (EcuM_SleepModeConfigType). Available sleep modes are defined at
 ** configuration time and are stored in the EcuM_SleepMode list. The shutdownMode
 ** parameter is only used if the target parameter equals to
 ** ECUM_SHUTDOWN_TARGET_SLEEP. In all other cases, it is ignored.
 **
 ** \return Standard Return Code
 ** \retval E_OK No Errors.
 ** \retval E_NOT_OK The new shutdown target was not set.
 ...
 ** \Reentrancy{Reentrant}
 ** \Synchronicity{Synchronous} */
extern FUNC(Std_ReturnType, ECUM_CODE) EcuM_ASR_R2011_SelectShutdownTarget
(
...
#if  (ECUM_EB_DEFAULT_ASR_SERVICE_API == ECUM_EB_SERVICE_API_ASR_R2011)
/** \brief Get target state for shutdown sequence.
 **
 ** This function always returns the selected shutdown target as set by
 ** EcuM_SelectShutdownTarget.
 **
 ...
 **   - ECUM_SHUTDOWN_TARGET_RESET
 **   - ECUM_SHUTDOWN_TARGET_OFF
 ** \param[out] shutdownMode If the return parameter is ECUM_SHUTDOWN_TARGET_SLEEP, then
 ** this output parameter identifies which of the configured sleep modes was
 ** actually chosen (index into EcuM_SleepMode). If the return parameter is
 ** ECUM_SHUTDOWN_TARGET_RESET, this out parameter tells which of the configured reset
 ** modes was actually chosen.
 **
 ** \return Standard Return Code
 ** \retval E_OK No Errors. (Always).
 ...
 **   - ECUM_SHUTDOWN_TARGET_RESET
 **   - ECUM_SHUTDOWN_TARGET_OFF
 ** \param[out] shutdownMode If the return parameter is ECUM_SHUTDOWN_TARGET_SLEEP, this
 ** output parameter tells which of the configured sleep modes was actually
 ** chosen (index into EcuM_SleepMode). If the return parameter is
 ** ECUM_SHUTDOWN_TARGET_RESET, this out parameter tells which of the configured reset
 ** modes was actually chosen.
