...
#endif
[!VAR "DccmMainFuncTime" = "node:value(DccmGeneral/DccmTaskTime)"!][!//
...
#endif
[!IF "node:existsAndTrue(DccmGeneral/Dccm_Dev_Error_Detect)"!][!//
...
#endif
[!IF "node:existsAndTrue(DccmGeneral/Dccm_Buffer_Streaming)"!][!//
#define DCCM_BUFFER_STREAMING         STD_ON
[!ELSE!][!//
#define DCCM_BUFFER_STREAMING         STD_OFF
[!ENDIF!][!//

/** \brief Switch for Keep-Alive-Logic: Dccm to notify (or not) the SWC after each sending of the functionally addressed valid TesterPresent
message with SPRMIB=true */
#if (defined DCCM_TO_NOTIFY_KEEP_ALIVE_LOGIC)
#error DCCM_TO_NOTIFY_KEEP_ALIVE_LOGIC is already defined
#endif
[!IF "DccmGeneral/Dccm_NotifySwcForKeepAliveLogic = 'true'"!][!//
#define DCCM_TO_NOTIFY_KEEP_ALIVE_LOGIC         STD_ON
[!ELSE!][!//
#define DCCM_TO_NOTIFY_KEEP_ALIVE_LOGIC         STD_OFF
[!ENDIF!][!//

/** \brief Switch for functional communication: when there is time-out for a functional request
 * with SuppressPosRspMsgIndication bit set to TRUE, Dccm will notify (or not) the SWC
 * about time-out for each physical communication protocol that is still waiting for a response. */
#if (defined DCCM_FUNCREQ_SPRMIBTRUE_NOTIFY_PHYSPROTO)
#error DCCM_FUNCREQ_SPRMIBTRUE_NOTIFY_PHYSPROTO is already defined
#endif
[!IF "DccmGeneral/Dccm_FunctionalComm_SPRMIbTrue_TimeoutNotificationOnPhysicalProtocols = 'true'"!][!//
#define DCCM_FUNCREQ_SPRMIBTRUE_NOTIFY_PHYSPROTO         STD_ON
[!ELSE!][!//
#define DCCM_FUNCREQ_SPRMIBTRUE_NOTIFY_PHYSPROTO         STD_OFF
[!ENDIF!][!//
