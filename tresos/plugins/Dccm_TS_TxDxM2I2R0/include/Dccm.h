...
/**
 *
 * \brief No response from server during the P2Client timeout,
 * but the request was sent with suppressPosRspMsgIndicationBit set to TRUE.
 *
 * Dccm::Dccm_ResponseCode value provided to the upper module in the callback
 *
 */
#if (defined DCCM_RSP_TIMEOUT_P2CLIENT_SPRMIB_TRUE)
  #error DCCM_RSP_TIMEOUT_P2CLIENT_SPRMIB_TRUE is already defined
#endif
#define DCCM_RSP_TIMEOUT_P2CLIENT_SPRMIB_TRUE        0x0AU

/**
 *
 * \brief No response from server during the P6Client timeout,
 * but the request was sent with suppressPosRspMsgIndicationBit set to TRUE.
 *
 * Dccm::Dccm_ResponseCode value provided to the upper module in the callback
 *
 */
#if (defined DCCM_RSP_TIMEOUT_P6CLIENT_SPRMIB_TRUE)
  #error DCCM_RSP_TIMEOUT_P6CLIENT_SPRMIB_TRUE is already defined
#endif
#define DCCM_RSP_TIMEOUT_P6CLIENT_SPRMIB_TRUE        0x0BU
...
#if (defined DCCM_SID_PREPARERXBUFFERFORFUNCREQ)
  #error DCCM_SID_PREPARERXBUFFERFORFUNCREQ is already defined
#endif
/** \brief Dccm_PrepareRxBufferForFuncReq() service ID */
#define DCCM_SID_PREPARERXBUFFERFORFUNCREQ        0x04U
 ...
 ** \Synchronicity{Synchronous}
 **/
extern FUNC(Std_ReturnType, DCCM_CODE) Dccm_AllocateDiagnosticProtocol
(
  uint16 TxPduId,
  uint16 RxPduId,
...
/* Deviation MISRAC2012-1 <START> */
/** \brief Prepare to receive a functional response on a physical Diagnostic Protocol
 **
 ** This function is used to prepare a physical Diagnostic Protocol
 ** to receive a response from the diagnostic server that comes as result
 ** of a functional request. The Diagnostic Protocol
 ** should be in the READY state otherwise the call will return with error.
 **
 ** Configuration: The maximum number of parallel diagnostic protocols
 ** can be configured but is limited to maximum 255. The PduIds should be
 ...
 **       in the communication with the server.
 **
 ** \param[out] Buffer A pointer to the start of the buffer where the data
 **       received from the server will be stored.
 **
 ** \param[in]  BufferLength Size of the buffer.
 **
 ** \param[out]  DataLengthPtr A pointer to return the number of bytes
 **       received from the server.
 **
 ** \return Std_ReturnType
 ** \retval E_OK Request was successful.
 ** \retval E_NOT_OK Request was not successful.
 ** \retval DCCM_STD_E_BUSY A request is already active on the communication protocol ProtocolId.
 **
 ** \ServiceID{::DCCM_SID_PREPARERXBUFFERFORFUNCREQ}
 ** \Reentrancy{Reentrant for different PduIds. Non reentrant for the same PduId.}
 ** \Synchronicity{Asynchronous}
 **/
extern FUNC(Std_ReturnType, DCCM_CODE) Dccm_PrepareRxBufferForFuncReq
(
  Dccm_ProtocolIdType ProtocolId,
  P2VAR(uint8, AUTOMATIC, DCCM_APPL_DATA) Buffer,
  uint16 BufferLength,
 ...
 **                   or the protocol identified with ProtocolId was not allocated for functional communication.
 **
