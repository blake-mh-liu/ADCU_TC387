...
)
{
  ...
  Dccm_ProtocolIdType ProtocolIdTempCounter = 0U;
  boolean ExtendedWaitTimeBecauseAResponsePending = FALSE;
  ...
  else if ( (Dccm_DiagnosticProtocols[ProtocolId].PreparedToReceiveFunctionalResponse == TRUE)
       && (Dccm_FunctionalRequestHelper.FunctionalRequestOngoingOnProtocolId == DCCM_INVALID_PROTOCOL_ID)
  )
  {/* This physical protocol was prepared to receive functional response.
      But the functional request had a timeout, the FunctionalHelper was reset to default,
      and Dccm was configured to not send notification for physical protocols that
         were prepared for functional response when the functional request was sent with
         SuppressPositiveResponse set to TRUE.
      The client of Dccm should have released this physical protocol when it was notified
        with the timeout notification for the functional request - if this was not done, it must be handled here.
    */
    DCCM_DET_REPORT_ERROR(DCCM_SID_RX, DCCM_E_FUNCTIONAL_ADDRESSING);
  }
  ...
  {
    /* everything is OK */
    /* Note: the response can be only on a physical communication protocol (i.e.: the response can not be on a functional communication protocol). */

    if (Dccm_DiagnosticProtocols[ProtocolId].PreparedToReceiveFunctionalResponse == FALSE)
    {
      /* This is the "usual use-case": receive a physical response because of a physical request on the same communication protocol.*/
    ...
    } /* Receive a physical response because of a physical request on the same communication protocol. */
    else /* use-case: (Dccm_DiagnosticProtocols[ProtocolId].PreparedToReceiveFunctionalResponse == TRUE) */
    {
      /* Receive a "functional response" on a physical communication protocol, because of a functional request on a different communication protocol. */
      /* In this use-case, there is no need to track the InternalTimeout,
       * because the modified behavior for P2_Client/P2*_Client (see UDS ISO 14229_2 for details) covers the time-out
       * that can appear during the receiving of the responses. */
      /* Because of the checks from Dccm_PrepareRxBufferForFuncReq(),
       * the protocol can be only PHYSICAL_ADDRESSING. */

      /* If a ResponsePending was received on the current communication protocol before the current receiving of data,
       * now in StartOfReception() Dccm must clear that information (set it to FALSE).
       * As a run-time optimization: no check will be done before setting to FALSE: this clearing (set to FALSE)
       * can be done also if there was no ResponsePending received before: it will just have no impact. */
      Dccm_DiagnosticProtocols[ProtocolId].ResponsePendingWasReceived = FALSE;

      /* Search in all communication protocols:
       * is there at least one physical protocol prepared to receive "functional response"
       * having "ResponsePendingWasReceived set on TRUE"? */
      /* if yes: time-out counter for functional communication must be reinitialized with P2*_Client */
      /* if no: time-out counter for functional communication must be reinitialized with P2_Client */
      /* Deviation TASKING-1 */
      for (ProtocolIdTempCounter = 0U; ProtocolIdTempCounter < DCCM_MAX_DIAGNOSTIC_PROTOCOLS; ProtocolIdTempCounter++)
      {
        if ( /* XXX is this really needed? check also the status of the protocol?: to be RECEIVING? */
            (Dccm_DiagnosticProtocols[ProtocolIdTempCounter].Status == DCCM_DIAGNOSTIC_PROTOCOL_STATUS_RECEIVE)
            /* PreparedToReceiveFunctionalResponse can be TRUE only on PHYSICAL_ADDRESSING (this is
             * because of the checks from Dccm_PrepareRxBufferForFuncReq()); */
            && (Dccm_DiagnosticProtocols[ProtocolIdTempCounter].PreparedToReceiveFunctionalResponse == TRUE)
            && (Dccm_DiagnosticProtocols[ProtocolIdTempCounter].ResponsePendingWasReceived == TRUE)
        )
        {
          ExtendedWaitTimeBecauseAResponsePending = TRUE;
          break;
        }
      } /* for */
      /* ENTER critical section */
      SchM_Enter_Dccm_SCHM_DCCM_EXCLUSIVE_AREA_0();
      if (ExtendedWaitTimeBecauseAResponsePending == FALSE)
      {
      /* Re-start the time-out tracking using P2_Client: not in it's own ProtocolId,
       * but in the helper structure for functional request handling. */
#if (DCCM_P2CLIENT_ENABLED == STD_ON)
        Dccm_FunctionalRequestHelper.Timeout = Dccm_FunctionalRequestHelper.P2ClientConfigurationValue + DCCM_EXE_INTERVAL;
        ...
        /* Dccm_FunctionalRequestHelper.Timeout = Dccm_FunctionalRequestHelper.P6ClientConfigurationValue + DCCM_EXE_INTERVAL;*/
#endif
      }
      else /* use-case: ExtendedWaitTimeBecauseAResponsePending == TRUE */
      {
#if (DCCM_P2CLIENT_ENABLED == STD_ON)
        Dccm_FunctionalRequestHelper.Timeout = Dccm_FunctionalRequestHelper.P2StarClientConfigurationValue + DCCM_EXE_INTERVAL;
#elif (DCCM_P6CLIENT_ENABLED == STD_ON)
        /* Currently, P6Client is out of scope for functional request processing. */
        /* To be updated at the implementation for P6_Client/P6*_Client. */
        /* Dccm_FunctionalRequestHelper.Timeout = Dccm_FunctionalRequestHelper.P6StarClientConfigurationValue + DCCM_EXE_INTERVAL;*/
#endif
      }
      /* LEAVE critical section */
      SchM_Exit_Dccm_SCHM_DCCM_EXCLUSIVE_AREA_0();
    } /* receive a "functional response" on a physical communication protocol */
      ...
      case NTFRSLT_OK:
        ...
        if (Dccm_DiagnosticProtocols[ProtocolId].Dccm_PduInfo.PduInfo.SduLength > 0U)
        {
          ...
          if (TRUE == ret)
          {
            ...
            {
              /* it was a valid response pending -> start the Timeout counter */
              /* depending on the configuration the Timeout counter will be loaded with P2StarClient or P6StarClient */
              /* ENTER critical section */
              SchM_Enter_Dccm_SCHM_DCCM_EXCLUSIVE_AREA_0();
              if (Dccm_DiagnosticProtocols[ProtocolId].PreparedToReceiveFunctionalResponse == FALSE)
              {
                /* this is the "usual use-case" (i.e.: receive a response that IS NOT a "functional response" on a physical communication protocol) */
#if (DCCM_P2CLIENT_ENABLED == STD_ON)
              ...
              else /* use-case: (Dccm_DiagnosticProtocols[ProtocolId].PreparedToReceiveFunctionalResponse == TRUE) */
              {
                ...
                /* because of the checks from Dccm_PrepareRxBufferForFuncReq(),
                 *  the protocol can be only PHYSICAL_ADDRESSING */
                Dccm_DiagnosticProtocols[ProtocolId].ResponsePendingWasReceived = TRUE;

                /* Start the time-out tracking: not in it's own ProtocolId,
                 * but in the helper structure for functional request handling. */
#if (DCCM_P2CLIENT_ENABLED == STD_ON)
                Dccm_FunctionalRequestHelper.Timeout = Dccm_FunctionalRequestHelper.P2StarClientConfigurationValue + DCCM_EXE_INTERVAL;
#elif (DCCM_P6CLIENT_ENABLED == STD_ON)
              ...
              /* if SPRMIB was saved at the beginning of the request, now set the SPRMIB to FALSE, regardless of it's value;
               * reason: if SPRMIB was originally TRUE, it's value must be considered FALSE because NRC 0x78 ResponsePending was received.*/
            ...
            } /* it was a valid response pending */
          } /* it was response pending */
          else
          {
...
)
{
  ...
  /* If no buffer is available to contain the TesterPresent request (eg. the TesterPresent was disabled in the meantime) */
    ...
    else /* (Dccm_DiagnosticProtocols[ProtocolId].Status == DCCM_DIAGNOSTIC_PROTOCOL_STATUS_TRANSMIT) */
    {
      ...
      {
        if (RetryInformationProcessing == ((Std_ReturnType) BUFREQ_E_BUSY))
        {
            retVal = BUFREQ_E_BUSY;
        }
      }
  ...
  else
  {
      switch(Result)
      {
        case NTFRSLT_OK:
          ...
          {
            /* use-case: (Dccm_DiagnosticProtocols[ProtocolId].TesterPresentInitiatedByDccm == FALSE) */
            /* Prepare the receiving of the response from the diagnostic server. */
            ...
            else /* use-case: (Dccm_DiagnosticProtocols[ProtocolId].AddressingType == FUNCTIONAL_ADDRESSING) && [ProtocolId].(TesterPresentInitiatedByDccm == FALSE)*/
            {
              /* Load the relevant data in the helper structure for the functional request.
               * The time-out tracking for the functional request will be done in the helper structure for the functional request,
               * not in the structure with all the protocols, because a functional TesterPresent can be in progress during the receiving of the "functional responses".*/
              /* ENTER critical section */
              SchM_Enter_Dccm_SCHM_DCCM_EXCLUSIVE_AREA_0();
#if (DCCM_P2CLIENT_ENABLED == STD_ON)
              Dccm_FunctionalRequestHelper.Timeout                        = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P2ClientConfigurationValue + DCCM_EXE_INTERVAL;
              Dccm_FunctionalRequestHelper.P2ClientConfigurationValue     = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P2ClientConfigurationValue;
              Dccm_FunctionalRequestHelper.P2StarClientConfigurationValue = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P2StarClientConfigurationValue;
#elif (DCCM_P6CLIENT_ENABLED == STD_ON)
              /* To be updated when "functional request handling" for P6_Client/P6Star_Client will be implemented.
               * Dccm_FunctionalRequestHelper.Timeout                        = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P6ClientConfigurationValue + DCCM_EXE_INTERVAL;
               * Dccm_FunctionalRequestHelper.P6ClientConfigurationValue     = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P6ClientConfigurationValue;
               * Dccm_FunctionalRequestHelper.P6StarClientConfigurationValue = Dccm_DiagnosticProtocols[ProtocolId].Timeouts.P6StarClientConfigurationValue;*/
#endif
              /* LEAVE critical section */
              SchM_Exit_Dccm_SCHM_DCCM_EXCLUSIVE_AREA_0();

              /* Prepare all eligible physical protocols to receive the functional response.*/
              /* Deviation TASKING-1 */
              for (PhysicalProtocolId = 0U; PhysicalProtocolId < DCCM_MAX_DIAGNOSTIC_PROTOCOLS; PhysicalProtocolId++)
              {
                if ((Dccm_DiagnosticProtocols[PhysicalProtocolId].Status == DCCM_DIAGNOSTIC_PROTOCOL_STATUS_READY)
                    /* Because of the checks from Dccm_PrepareRxBufferForFuncReq(),
                     * PreparedToReceiveFunctionalResponse == TRUE only when AddressingType == PHYSICAL_ADDRESSING.*/
                    && (Dccm_DiagnosticProtocols[PhysicalProtocolId].PreparedToReceiveFunctionalResponse == TRUE)
                )
                {
                  /* Change status of the physical protocol to RECEIVE. */
                  Dccm_DiagnosticProtocols[PhysicalProtocolId].Status = DCCM_DIAGNOSTIC_PROTOCOL_STATUS_RECEIVE;
                  /* give to the physical protocol the needed data from the functional protocol */
                  Dccm_DiagnosticProtocols[PhysicalProtocolId].RequestInfo.ServiceID             = Dccm_DiagnosticProtocols[ProtocolId].RequestInfo.ServiceID;
                  Dccm_DiagnosticProtocols[PhysicalProtocolId].RequestInfo.isServiceIDsaved      = Dccm_DiagnosticProtocols[ProtocolId].RequestInfo.isServiceIDsaved;
                  Dccm_DiagnosticProtocols[PhysicalProtocolId].RequestInfo.SuppressPosRsp        = Dccm_DiagnosticProtocols[ProtocolId].RequestInfo.SuppressPosRsp;
                  Dccm_DiagnosticProtocols[PhysicalProtocolId].RequestInfo.isSuppressPosRspSaved = Dccm_DiagnosticProtocols[ProtocolId].RequestInfo.isSuppressPosRspSaved;
                  /* When receiving the "functional response", no time-tracking will be done on the physical communication protocol. */
                }
              } /* for each PhysicalProtocolId */
            } /* if (Dccm_DiagnosticProtocols[ProtocolId].AddressingType == FUNCTIONAL_ADDRESSING) */
          } /* there is also reception of response */
          break;
