 ...
 *
 *    Reason:
 *    False positive. CanTp_RxIndication() function can't be defined twice
 *    since CANTP_JUMPTABLE_SUPPORT enables one version of the function extension.
 *
...
/** \brief CanTp_ProcessReceivedFFSub()
 **
 ** This function processes a received FF from CanIf
 **
 ** \param[in] CanTpRxPduPtr  pointer to received data and length
 ** \param[in] ChannelPtr  pointer to channel data structure in RAM
 ** \param[in] ValidFirstFrame  Indication for valid first frame
 ** \param[in] FirstFrame_NPCI_Size  Number of bytes required for N_PCI
 */
STATIC FUNC(void, CANTP_CODE) CanTp_ProcessReceivedFFSub
(
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_CONST) CanTpRxPduPtr,
  P2VAR(CanTp_ChannelType, AUTOMATIC, CANTP_VAR) ChannelPtr,
 ...
 */
#if (CANTP_METADATA_CHANNELS == STD_OFF)
STATIC FUNC(boolean, CANTP_CODE) CanTp_FindNSduId
(
  PduIdType CanTpRxPduId,
  P2CONST(uint8, AUTOMATIC, CANTP_APPL_CONST) CanDataPtr,
  P2VAR(PduIdType, AUTOMATIC, CANTP_APPL_DATA) NSduId,
  P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) FrameType
);
#else
STATIC FUNC(boolean, CANTP_CODE) CanTp_FindNSduId_WithMetadata
(
  PduIdType CanTpRxPduId,
  P2CONST(uint8, AUTOMATIC, CANTP_APPL_CONST) CanDataPtr,
  P2VAR(PduIdType, AUTOMATIC, CANTP_APPL_DATA) NSduId_old,
  P2VAR(PduIdType, AUTOMATIC, CANTP_APPL_DATA) NSduId,
  P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) FrameType
);
#endif /*(CANTP_METADATA_CHANNELS == STD_OFF)*/
 ...
 ** \param[in] CanTpRxPduPtr, pointer to received data and length
 */
STATIC FUNC(void, CANTP_CODE) CanTp_RxIndicationSub
(
  PduIdType CanTpRxPduId,
...
)
{
  ...
  if((CANTP_CHECK_FLAG(ChannelPtr->PostFlag,CANTP_POST_RX_IND_MASK)) != 0U)
  {
...
{
  /* SWS_CanTp_00290: Terminate current Rx but continue Tx */
  switch(CANTP_CH_GET_MODE(ChannelPtr))
  {
    ...
    case CANTP_CH_MODE_RX:
    {
      ...
      {
        /* !LINKSTO CanTp.SWS_CanTp_00057_FullDuplex_6, 1, CanTp.SWS_CanTp_00057_FullDuplex_7, 1 */
      ...
      {
        /* !LINKSTO CanTp.SWS_CanTp_00057_HalfDuplex_6, 1, CanTp.SWS_CanTp_00057_HalfDuplex_7, 1 */
...
)
{
  ...
  *NSduId = 0;
  *FrameType = CANTP_GET_N_PCI_TYPE(CanDataPtr[0]); /* index 0 means addressing STANDARD */

  if(*FrameType != CANTP_N_PCI_TYPE_FC)
  {
    CanTp_RxNPduLookupEntryType Index;
    PduIdType MetaDataLLPduId;
    PduIdType MetadataULPduId;
    PduIdType MetadataULPduId_local = 0U;
    PduInfoType MetadataPduInfo;
    uint32 Metadata_NPdu = 0U;
    uint32 Metadata_NSdu = 0U;
    PduInfoType CanTpRxPdu;
    P2VAR(CanTp_ChannelType, AUTOMATIC, CANTP_VAR) ChannelPtr;
    CanTp_ChType Channel;
    Std_ReturnType Result = E_NOT_OK;
    uint8 metadata_found = FALSE;
    uint8 metadata_process = FALSE;

    P2CONST(CanTp_RxNSduConfigType, AUTOMATIC, CANTP_APPL_CONST) RxNSduCfgPtr;

    for(Index = 0U; Index < CanTp_CfgPtr->NumberRxNSdus; Index++)
    {
      /* Pointer to N-Sdu config */
      if(CANTP_CFG_GET_RXNSDU_CONFIG(Index).NPduIdxF == CanTpRxPduId)
      {
        *NSduId_old = Index;
        RxNSduCfgPtr = &CANTP_CFG_GET_RXNSDU_CONFIG(Index);
        MetaDataLLPduId = RxNSduCfgPtr->MetadataLLPduId;
        /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_Reception, 1 */
        (void)EcuC_GetMetaDataCanId(MetaDataLLPduId, &CanTpRxPdu, &Metadata_NPdu); /*get metadata for Pdu*/
        indexFound = TRUE;
        break;
      }
    }
    ...
    if(indexFound)
    {
      for(Index = 0U; Index < CanTp_CfgPtr->NumberRxNSdus; Index++)
      {
        CANTP_GET_RX_CH_INDEX(Channel, Index, Result); /* index is the same thing as NSduId and is used to get the rx channel */
        ChannelPtr = &CanTp_Channel[Channel];
        MetadataULPduId = CANTP_CFG_GET_RXNSDU_CONFIG(Index).MetadataULPduId;

        if((ChannelPtr->State != CANTP_CH_IDLE) && (CANTP_CH_GET_MODE(ChannelPtr) == CANTP_CH_MODE_RX))  /* check if a reception is ongoing on each channel */
        {
          /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_Reception, 1 */
          (void)EcuC_GetMetaDataCanId(MetadataULPduId, &CanTpRxPdu, &Metadata_NSdu); /*get metadatada for Sdu*/

          if(Metadata_NPdu == Metadata_NSdu)  /* check if the received metadata on Pdu is the same as the one from Sdu */
          {
            /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_RxSameMetadata, 1 */
            metadata_found = TRUE;
            *NSduId = Index;
            FoundFlag = TRUE; /* this reception will overwrite the ongoing one */
            break;
          }
          else
          {
            /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_RxNSduNotAvailable, 1 */
            /* do nothing, received NPdu has a different Metadata than this NSdu */
          }
        }
        else /* if at least one channel is available and received metadata doesn't match with the one from existing Sdu*/
        {
          *NSduId = Index;
          MetadataULPduId_local = MetadataULPduId;
          metadata_process = TRUE;
        }
      }
    }

    if((metadata_process == TRUE) && (metadata_found == FALSE))
    {
      if(*FrameType != CANTP_N_PCI_TYPE_CF)
      {
        /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_RxDifferentMetadata, 1 */
        (void)EcuC_SetMetaDataCanId(MetadataULPduId_local, &MetadataPduInfo, Metadata_NPdu); /* Set on Sdu ID the received metadata from Pdu */
      }
      FoundFlag = TRUE;
    }
  }
  ...
  {
    PduIdType MetadataLLFcPduId;
    PduInfoType CanTpRxFcPdu;
    uint32 Metadata_Fc = 0U;
    P2VAR(CanTp_ChannelType, AUTOMATIC, CANTP_VAR) ChannelPtr;
    uint8 index = 0U;
    P2CONST(CanTp_TxNSduConfigType, AUTOMATIC, CANTP_APPL_CONST) TxNSduCfgPtr;

    /* use index variable to go through all tx channels */
    for(index = 0U; index < MAX_NUMBER_OF_TX_CHANNELS; index++)
    {

      if(CANTP_CFG_GET_TXNSDU_CONFIG(index).NPduIdFc == CanTpRxPduId)
      {
        TxNSduCfgPtr = &CANTP_CFG_GET_TXNSDU_CONFIG(index);
        MetadataLLFcPduId = TxNSduCfgPtr->MetadataLLFcPduId;

        /* !LINKSTO CanTp.EB.CanTpMetaDataParallelHandling_FC_Reception, 1 */
        (void)EcuC_GetMetaDataCanId(MetadataLLFcPduId, &CanTpRxFcPdu, &Metadata_Fc); /*get metadata for RxFcNPdu */
        indexFound = TRUE;
        break;
      }
    }
    ...
    if(indexFound)
    {
      for(index = 0U; index < MAX_NUMBER_OF_TX_CHANNELS; index++)
      {
        ChannelPtr = &CanTp_Channel[index];
        /* check the reveived metadata against the metadata set for related RxFc at FF transmission */
        if((ChannelPtr->MetadataFc == Metadata_Fc) && (CANTP_CH_GET_MODE(ChannelPtr) == CANTP_CH_MODE_TX))
        {
          *NSduId = ChannelPtr->NSduId;
          FoundFlag = TRUE; /*if metadata matches, process received RxFcNPdu */
          break;
        }
      }
    }
...
)
{
  ...
  if((ChannelPtr->State == CANTP_CH_RX_RECEIVE_CF) || (ChannelPtr->State == CANTP_CH_RX_CONFIRM_FC_CTS))
  {
    ...
    {
      /* CanTp.ASR40.SWS_CanTp_00314: check correctness of SN, abort with
...
)
{
  ...
  switch(ChannelPtr->State)
  {
    ...
    case CANTP_CH_TX_CONFIRM_CF:
    {
      ...
      {
        /* ptr to can data */
        CONSTP2CONST(uint8, AUTOMATIC, CANTP_APPL_CONST) CanDataPtr
        ...
        {
          /* CanTp.ASR40.SWS_CanTp_00309: FC OVFLW received -> report to PduR
        ...
        default:
        {
          /* CanTp.ASR40.SWS_CanTp_00317: on invalid flow control status, stop channel
...
#endif /* CANTP_DYNAMIC_ID_SUPPORT == STD_ON */
    {
      if((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags, CANTP_RXCHANN_LOCK_MASK)) == 0U)
      {
...
)
{
  ...
  PduIdType               NSduId_old;       /* N-Sdu Id related to the receiving Pdu */
#endif /* #if (CANTP_METADATA_CHANNELS == STD_ON) */
...
#else
  NSduId_old = 0U;
  if(CanTp_FindNSduId_WithMetadata(CanTpRxPduId, CanTpRxPduPtr->SduDataPtr, &NSduId_old, &NSduId, &FrameType))
  {
#endif
    P2CONST(CanTp_RxNPduTableType, AUTOMATIC, CANTP_APPL_CONST)
              RxNPduTableEntry = &(CANTP_CFG_GET_RXNPDU_TABLE(CANTP_CFG_GET_RXNPDU_LOOKUPTABLE(CanTpRxPduId)));
...
#if (CANTP_METADATA_CHANNELS == STD_ON)
        P2CONST(CanTp_RxNSduConfigType, AUTOMATIC, CANTP_APPL_CONST) RxNSduCfgPtr =
                                                          &CANTP_CFG_GET_RXNSDU_CONFIG(NSduId_old);
        ChannelPtr->FcPduId = RxNSduCfgPtr->LSduIdFc;
        ChannelPtr->MetadataLLFcPduId = RxNSduCfgPtr->MetadataLLFcPduId;
#endif /* #if (CANTP_METADATA_CHANNELS == STD_ON) */
...
#endif
      {
        ...
        if(IgnoreMsgFlag == (boolean)FALSE)
        {
          ...
          if(
              ...
              ((FrameType == CANTP_N_PCI_TYPE_SF) && ((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_TXLASTFRAME_MASK)) != 0U)) ||
              ((FrameType == CANTP_N_PCI_TYPE_FF) && ((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_TXLASTFRAME_MASK)) != 0U))))
          )
          {
            CANTP_LOCK_CHANNEL(ChannelPtr);
...
#if(CANTP_STALLHANDLING_SUPPORT == STD_ON)
#if(CANTP_DEV_ERROR_DETECT == STD_ON)
            if((FrameType == CANTP_N_PCI_TYPE_CF) && ((CANTP_CHECK_FLAG(ChannelPtr->PostFlag,CANTP_POST_RX_IND_MASK)) != 0U))
            {
              OverwrittenCF = TRUE;
            }
#endif /* (CANTP_DEV_ERROR_DETECT == STD_ON) */
