 ...
 *
 *    Reason:
 *    False positive. CanTp_TxConfirmation() function can't be defined twice
...
)
{
  ...
  if((CANTP_CHECK_FLAG(ChannelPtr->PostFlag,CANTP_POST_TX_CONF_MASK)) != 0U)
  {
...
)
{
  ...
  if(CanTpTxNPduConfId == SearchTxNPduId)
  {
    ...
    case CANTP_CH_TX_CONFIRM_FF:
    {
      ...
      if ((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_FCRECEIVED_MASK)) == 0U)
      {
    ...
    case CANTP_CH_RX_CONFIRM_FC_WT:
    {
      ...
      if((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_FCWAIT_REPEATED_MASK)) != 0U)
      {
    ...
    case CANTP_CH_RX_CONFIRM_FC_OVFLW:
    {
      /* Abort reception don't inform Upper layer and send FC(OVFL)
           (see CanTp.ASR40.SWS_CanTp_00318).
       */
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if ((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_STMIN_ROUNDUP_MASK)) != 0U)
        {
