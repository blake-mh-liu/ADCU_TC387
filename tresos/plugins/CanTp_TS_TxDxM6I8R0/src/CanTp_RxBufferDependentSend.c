...
/** \brief Rx state handling CANTP_CH_RX_BUFREQ_FF
 **
 ** This function handles the internal CANTP_CH_RX_BUFREQ_FF states.
 **
 ** \param[in] ChannelPtr  Pointer to channel data structure to ease lookups
 */
STATIC FUNC(void, CANTP_CODE) CanTp_RxStateHandling_FF
(
...
)
{
  ...
  /* SWS_CanTp_00079 Inform upper layer about Rx frame */
...
)
{
  switch(ChannelPtr->State)
  {
    ...
    {
      CanTp_RxStateHandling_FF(ChannelPtr);
      break;
    }
