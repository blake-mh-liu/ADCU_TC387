...
/** \brief CanTp_BufReqOkAction()
 **
 **   This function performs neccessary padding and initiates the frame transmission
 ** when the buffer request returns BUFREQ_OK.
 **
 ** \param[in] ChannelPtr  pointer to channel data structure to ease lookups
 */
STATIC FUNC(void, CANTP_CODE) CanTp_BufReqOkAction
(
  P2VAR(CanTp_ChannelType, AUTOMATIC, CANTP_VAR) ChannelPtr
);

/** \brief CanTp_TxStateHandling_SF_FF_CF()
 **
 ** This function handles the internal TX states for
 **  -CANTP_CH_TX_BUFREQ_SF
 **  -CANTP_CH_TX_BUFREQ_FF
 **  -CANTP_CH_TX_BUFREQ_CF
 **
 ** \param[in] ChannelPtr  pointer to channel data structure to ease lookups
 */
STATIC FUNC(void, CANTP_CODE) CanTp_TxStateHandling_SF_FF_CF
(
  P2VAR(CanTp_ChannelType, AUTOMATIC, CANTP_VAR) ChannelPtr,
...
)
{
  switch(ChannelPtr->State)
  {
    ...
    {
      CanTp_TxStateHandling_SF_FF_CF(ChannelPtr, CFBufferRequested);
      break;
    }
    ...
    {
      BufReq_ReturnType DataReqRetVal = BUFREQ_E_NOT_OK;

      /* Request buffer for next CF. */
      TS_AtomicAssign8(ChannelPtr->State, CANTP_CH_TX_BUFREQ_CF);

      DataReqRetVal = CanTp_RequestTxFrameData(ChannelPtr);

      switch(DataReqRetVal)
      {
        case BUFREQ_OK:
        {
...
)
{
  ...
  if(CFBufferRequested != TRUE)
  {
    ...
    switch(DataReqRetVal)
    {
      case BUFREQ_OK:
      {
        CanTp_BufReqOkAction(ChannelPtr);
        break;
      }
      case BUFREQ_E_BUSY:
      {
