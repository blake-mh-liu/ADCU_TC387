...
)
{
  ...
  if(IsValidAddInfo)
  {
    ...
    if(FCBufferPtr!= NULL_PTR)
    {
       ...
       (void)EcuC_GetMetaDataCanId(CANTP_CFG_GET_RXNSDU_CONFIG(NSduId).MetadataLLFcPduId, &MetadataPduInfo, &Metadata_NSdu); /*get metadata for Fc Pdu*/
       (void)EcuC_SetMetaDataCanId(ChannelPtr->MetadataLLFcPduId, &MetadataPduInfo, Metadata_NSdu); /*set metadata for Fc Pdu*/
    }
#endif
    ...
    {
      /* !LINKSTO CanTp.SWS_CanTp_00343, 1 */
