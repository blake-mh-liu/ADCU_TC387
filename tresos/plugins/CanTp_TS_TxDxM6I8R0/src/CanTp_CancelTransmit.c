...
/** \brief Process Cancel a pending transfer
 **
 ...
 **
 */
STATIC FUNC(Std_ReturnType, CANTP_CODE) CanTp_ProcessCancelTransmit
(
...
#endif
  {
    ...
    if((boolean)TRUE == CANTP_CFG_GET_TXNSDU_CONFIG(CanTpTxSduId).CanTpTc)
    {
...
#endif /* CANTP_PARALLEL_CHANNELS_ENABLED == STD_ON */
      {
        ...
        {
         ret_result  = CanTp_ProcessCancelTransmit(CanTpTxSduId, ChannelPtr);
        }
      }
#if(CANTP_PARALLEL_CHANNELS_ENABLED == STD_ON)
      else /* all channels locked with other NSdus */
      {
        ...
        /* !LINKSTO CanTp.ASR40.SWS_CanTp_00254_2, 1 */
#if(CANTP_DEV_ERROR_DETECT == STD_ON)
        CANTP_DET_REPORT_ERROR(
            CANTP_MODULE_ID,
            CANTP_INSTANCE_ID,
...
)
{
  ...
  if(ChannelPtr->NSduId == CanTpTxSduId)
  {
    ...
    if(E_OK == ret_result)
    {
      /* !LINKSTO CanTp.SWS_CanTp_00256, 1 */
      CanTp_StopChannel(
          ChannelPtr,
          CANTP_ULIND_STATUS,
          NTFRSLT_E_NOT_OK
