...
/* CanTp_Cfg.h shall define constant and customizable data for module configuration
   at pre-compile time.
 */
/* !LINKSTO CanTp.ASR40.SWS_CanTp_00001, 1 */
/*==================[includes]===============================================*/
...
[!IF "node:contains(util:distinct(node:refs(as:modconf('PbcfgM')/PbcfgMBswModules/*/PbcfgMBswModuleRef)), as:modconf('CanTp')) = 'true'"!]
...
#if (defined CANTP_METADATA_CHANNELS) /* to prevent double declaration */
#error CANTP_METADATA_CHANNELS is already defined
#endif /* if (defined CANTP_METADATA_CHANNELS) */

[!IF "CanTpGeneral/CanTpMetaDataParallelHandling = 'true'"!][!//
#define CANTP_METADATA_CHANNELS          STD_ON
[!ELSE!][!//
#define CANTP_METADATA_CHANNELS          STD_OFF
[!ENDIF!][!//
...
#define CANTP_CFG_SIGNATURE [!"asc:calculateConfigSignature(as:modconf('CanTp')[1], 'PreCompile', as:modconf('CanTp')[1]/CanTpJumpTable/*)"!]U
...
#define CANTP_PUBLIC_INFO_SIGNATURE [!"asc:calculateConfigSignature(as:modconf('CanTp')[1]/CommonPublishedInformation, 'PublishedInformation', as:modconf('CanTp')[1]/CommonPublishedInformation/Release)"!]U
