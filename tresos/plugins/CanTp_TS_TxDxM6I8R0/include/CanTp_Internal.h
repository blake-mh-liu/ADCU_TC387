...
#if((CANTP_DYNAMIC_ID_SUPPORT == STD_ON)||(CANTP_METADATA_CHANNELS == STD_ON))
#include <EcuC.h>
#endif

/* !LINKSTO CanTp.ASR40.SWS_CanTp_00165,1 */
...
#if (defined INVALID_TX_CHANNEL_ID) /* to prevent double declaration */
#error INVALID_TX_CHANNEL_ID is already defined
#endif /* if (defined INVALID_TX_CHANNEL_ID) */

#define INVALID_TX_CHANNEL_ID    (CanTp_CfgPtr->InvalidTxChID)

#if (defined INVALID_RX_CHANNEL_ID) /* to prevent double declaration */
#error INVALID_RX_CHANNEL_ID is already defined
#endif /* if (defined INVALID_RX_CHANNEL_ID) */

#define INVALID_RX_CHANNEL_ID    (CanTp_CfgPtr->InvalidRxChID)
