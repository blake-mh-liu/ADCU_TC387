...
if strcmp(TargetECU,'MCU_B_R'); TargetECU = 'MCU'; end
...
for i = 1:length(R_SG_ALL)
    ...
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/' char(extractBefore(R_SG_ALL(i),'_SG')) '/ConstantSpecification/Init_' char(R_SG_ALL(i))];
...
for i = 1:length(P_SG_ALL)
    ...
    h = find(contains(tmpCell,'<CONSTANT-REF DEST="CONSTANT-SPECIFICATION">'));
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/' char(extractBefore(P_SG_ALL(i),'_SG')) '/ConstantSpecification/Init_' char(P_SG_ALL(i))];
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /CAN1/ConstantSpecification/Init_CAN1_SG_VCU1
...
for i = 1: length(R_SG_APP)
    ...
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/Comp_' TargetECU '_ARPkg/Comp_' TargetECU '_main/SWC_' SWC];
    ...
    OldString = extractBetween(tmpCell(h),'>','<');
    NewString = ['/SWC_' SWC '_ARPkg/SWC_' SWC '_type/R_' erase(char(R_SG_APP(i)),'_SG')];
...
for i = 1: length(P_SG_APP)
    ...
    if contains(SGName,'_OTA1')
	    ...
	    h = find(contains(tmpCell,'<TARGET-P-PORT-REF DEST="P-PORT-PROTOTYPE">'));
	    OldString = extractBetween(tmpCell(h),'>','<');
	    NewString = ['/SWC_' SWC '_ARPkg/SWC_' SWC '_type/P_' erase(char(P_SG_APP(i)),'_SG')];
