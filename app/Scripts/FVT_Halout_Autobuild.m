...
%% Read SWC_APP.arxml for output port order
...
tmpCell1 = extractBetween(Target_arxml(Raw_start+h-1,1),'SWC_APP_type/','</PORT-PROTOTYPE-REF>');
...
%                      end
                    ...
                    for k = 1:str2double(DLC)
                        ...
                        for m = 1:length(buf)
                            ...
                            elseif ismissing(string(MessageLink(idx,2)))
                                 OUTPSignal = '';
                            else
                                ...
                                if ~any(idx) ||...
                                    ~any(strcmp(OutputSignal(OutputSignal_idy,OutputSignal_idx),MsgName))
...
function Copy_DD_to_other_car_models(car_model, Common_Scripts_path, arch_Path)
    ...
    source_dd_Path = [arch_Path '/hal/halout'];
    target_dd_Path = [project_path '/software/sw_development/arch/hal/halout'];
    copyfile(source_dd_Path, target_dd_Path);
end