function FVT_INP_Autobuild()
%% Initial settings
% ref_car_model = {'d21lawd-fdc2','d21lrwd-fdc2', 'd31hawd-fdc2', 'd31hrwd-fdc2'};
% car_model = ref_car_model{1};
is_Calibration_Data_From_Another_File = boolean(0);

Common_Scripts_path = pwd;
if ~contains(Common_Scripts_path, 'app\Scripts'), error('current folder is not under app\Scripts'), end
...
if strcmp(TargetNode,'FUSION')
    channel_list = {'CAN1','CAN2','CAN3','CAN4','CAN5','CAN6','CAN8','GPIO'};
...
for i = 1:Num_CANChn
    ...
    CANChannel = char(channel_list(q(i)));
...
%             set_param(h,'LinkStatus', 'inactive');
    ...
    for k = 1:Numb_categty
        ...
        for j = m:n
            ...
            Subsubmodule = extractBefore(str_unit,'_');
            if strcmp(CANChannel,'CAN8')
                srcT = 'FVT_lib/inp/can_signal_template_without_ovrd';
            else
...
%                         Arry_Cal(Numb_TotalCal,8) = num2cell(10);
                     case {2}
                        ...
                        Arry_Cal(Numb_TotalCal,7) = cellstr('N/A');
                        Arry_Cal(Numb_TotalCal,8) = num2cell(0);
                        if contains(Arry_Cal(Numb_TotalCal,1),{'KINP_MCUBF1CSErr_flg_ovrdflg';'KINP_MCUBR1CSErr_flg_ovrdflg';'KINP_BMS1CSErr_flg_ovrdflg'...
                                                              ;'KINP_BMS6CSErr_flg_ovrdflg';'KINP_FCM1Checksum_enum_ovrdflg';'KINP_FCM1LifeCount_enum_ovrdflg'})
                            Arry_Cal(Numb_TotalCal,8) = num2cell(1);
                        end
                     case {3}
                        Arry_Cal(Numb_TotalCal,2) = cellstr('internal'); 
...
%
    ...
    set_param(h,'OutDataTypeStr',['Bus:  BINP_' CANChannel '_outputs']);
    ...
    for k = 1:Numb_Outputs
        ...
        set(h,'name',char(str));
        if ~strcmp(CANChannel,'CAN8') || contains(char(str),'MsgValid')
...
% buildbus(DD_file,DD_path,signal_table,calibration_table,verctrl,ref_car_model,arch_Path);
buildbus(DD_file,DD_path,signal_table,calibration_table,verctrl,arch_Path);
...
%     if ~strcmp(module_name,'INP_CAN8')
