...
for k = 1:length(System_arxml)
    if contains(strip(char(System_arxml(k)),'left'),'<SHORT-NAME>MAP_R_CAN')
        ...
        if strcmp(Channel,'CAN')
            if contains(System_arxml(k),'CAN_MCU_R1')
                Channel = cellstr('CAN_MCU_R1');
            elseif contains(System_arxml(k),'CAN_MCU_F1')
                Channel = cellstr('CAN_MCU_F1');
            else
                error('Unable to identify CAN channel');
            end
        end
        MsgName = cellstr(extractBetween(System_arxml(k),['<SHORT-NAME>MAP_R_' char(Channel) '_'],'</SHORT-NAME>'));
        cntR = cntR + 1;
        R_SG_APP(cntR,1) = MsgName;
        R_SG_APP(cntR,2) = Channel;
    end
end
...
%         cntR = cntR + 1;
%         R_SG_APP{cntR,1} = char(Rx_MsgLink(i));
%         R_SG_APP{cntR,2} = Channel;
%     end
% 
...
for i = 1:length(Channel_list)
    ...
    for k = 1:length(DBC.Messages)
        ...
        % 
        %     R_SG_APP(end+1,1) = cellstr(DBC.MessageInfo(k).Name);
        %     R_SG_APP(end,2) = Channel_list(i);
        % end
...
idx = contains(HOH_arxml(h1),{'_Xcp';'HOH_TX'});
h1(idx) = [];
...
for i = 1:length(R_SG_APP(:,1))
    h = find(strcmp(HOH_Array(:,2),R_SG_APP(i,1)));
    ...
    if any(h)
        ...
        disp(['CAUTION: ' char(R_SG_APP(i,2)) '::' char(R_SG_APP(i,1)) ' is new Rx PDU.']);
        HOH_Array(end+1,1) = cellstr(['HOH_999_' char(R_SG_APP(i,2))]);
        HOH_Array(end,2) = R_SG_APP(i,1);
        HOH_Array(end,3) = ValueTable(strcmp(ValueTable(:,1),R_SG_APP(i,1)),2); % New Rx PDU ID
        HOH_Array(end,4) = cellstr('/Llce_Af/Llce_Af/LlceAfGeneral/AF_Host');
        HOH_Array(end,5) = cellstr('1'); % Rx PDU CanHwObjectCount
...
cnt = 0;
for i = 1:length(HOH_Array(:,1))
    if ~strcmp(HOH_Array(i,3),'0') && endsWith(string(HOH_Array(i,4)),'/AF_Host') && ~any(strcmp(R_SG_APP(:,1),HOH_Array(i,2))) % Not Frame routing, have ID, didn't exist in Messagelink input 
       cnt = cnt+1;
...
for i = 1:length(Channel_list)
    ...
    if i == 1
        Final_HOH_Array(1:length(HOH_Array_CAN(:,1)),:) = HOH_Array_CAN(1:end,:);
...
for i = 1:length(Final_HOH_Array(:,1))
    tmpCell = template;
    NmPDU_flg = boolean(0);
...
for i = 1:length(Else_HOH_cell(:,1))
    ...
    NewString = mat2str(length(Final_HOH_Array(:,1))+i -1);
    HOH_arxml(ECUC_start + h) = strrep(HOH_arxml(ECUC_start + h),OldString,NewString);

end

% Add in HOH arxml
h = find(contains(HOH_arxml,'<SHORT-NAME>HOH_1_CAN_Xcp</SHORT-NAME>'));
ECUC_end_array = find(contains(HOH_arxml(:,1),'</ECUC-CONTAINER-VALUE>'));
ECUC_end = max(ECUC_end_array(ECUC_end_array<h));
