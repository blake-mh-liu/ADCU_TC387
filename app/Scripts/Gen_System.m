...
for k = 1:length(Source_arxml)
    if (strcmp(strip(char(Source_arxml(k)),'left'),'<R-PORT-PROTOTYPE>') ||...
        ...
        Port_SWC(cnt,3) = cellstr(['/' char(extractBetween(Source_arxml(k+2),'IF_' ,'_SG')) '/SignalGroup/' char(extractBetween(Source_arxml(k+2),'/IF_' ,'<'))]);
    end
end

% Add FD_OTA1 in Port_SWC
Port_OTA = unique(DTCell(endsWith(DTCell(:,1),'_OTA1')));
for i = 1:length(Port_OTA)
    cnt = cnt+1;
    Channel = char(extractBetween(Port_OTA(i),'_','_SG'));
    Port_SWC(cnt,1) = cellstr(['P_' char(erase(Port_OTA(i),{'DT_';'SG_'}))]);
    Port_SWC(cnt,2) = cellstr(['/CANInterface_ARPkg/IF_' char(erase(Port_OTA(i),'DT_')) '/' char(erase(Port_OTA(i),'DT_'))]);
    Port_SWC(cnt,3) = cellstr(['/' Channel '/SignalGroup/' char(erase(Port_OTA(i),'DT_'))]);
end
...
% for i = 1:length(Channel_list)
% Channel = char(Channel_list(i));
% if any(strcmp(Channel,{'CAN6';'CAN7'}))
%     continue
% end
% Raw_start = find(contains(Target_arxml,'<FIBEX-ELEMENT-REF-CONDITIONAL>'),1,'first');
% Raw_end = find(contains(Target_arxml,'</FIBEX-ELEMENT-REF-CONDITIONAL>'),1,'first');
...
for i = 1:length(NmPDUCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(NmPDUCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
    end
...
for i = 1:length(FrameCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(FrameCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(IPDUCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(IPDUCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(NPDUCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(NPDUCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(DCMIPDUCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(DCMIPDUCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(SGCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(SGCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(SignalCell)
    ...
    if ~any(strcmp(Channel_list,Channel))
        for k = 1:length(Channel_list(:,1))
            if contains(SignalCell(i),Channel_list(k,1))
                Channel = char(Channel_list(k,1));
                break
            end
        end
...
for i = 1:length(Port_SWC(:,1))
    ...
    if endsWith(char(Port_SWC(i,1)),'_OTA1')
        ...
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['/SWC_VES_ARPkg/SWC_VES_type/' char(Port_SWC(i,1))];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_FDC_ARPkg/SWC_FDC_type/P_CAN1_FD_BCM1
    else
        h = find(contains(tmpCell,'<CONTEXT-COMPONENT-REF DEST="SW-COMPONENT-PROTOTYPE">'),1,'first');
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['/Comp_' TargetECU_Abb '_ARPkg/Comp_' TargetECU_Abb '_main/SWC_APP'];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /Comp_FDC_ARPkg/Comp_FDC_main/SWC_FDC
        ...
        h = find(contains(tmpCell,'<CONTEXT-PORT-REF DEST="P-PORT-PROTOTYPE">'),1,'first');
        OldString = extractBetween(tmpCell(h),'>','<');
        NewString = ['/SWC_APP_ARPkg/SWC_APP_type/' char(Port_SWC(i,1))];
        tmpCell(h) = strrep(tmpCell(h),OldString,NewString); % /SWC_FDC_ARPkg/SWC_FDC_type/P_CAN1_FD_BCM1
    end
    ...
    for m = 1:length(idx_DTCell)
        ...
        NewString = ['/' char(extractBetween(DTCell(idx_DTCell(m),1),'_','_SG')) '/ImplementationDataTypes/' char(DTCell(idx_DTCell(m),1)) '/' char(DTCell(idx_DTCell(m),2))];
        ...
        OldString = extractBetween(tmpCell2(h),'>','<');
        NewString = ['/' char(extractBetween(DTCell(idx_DTCell(m),1),'DT_','_SG')) '/Signal/' char(DTCell(idx_DTCell(m),2))];
