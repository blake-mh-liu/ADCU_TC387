...
if ~contains(Common_Scripts_path, 'app\Scripts'), error('current folder is not under common\Scripts'), end
library_path = [Common_Scripts_path '/../library'];
project_path = char(strcat(Common_Scripts_path, '/../'));
...
addpath([project_path '/../common/Models'], '-end');

%% Select Project
ECU_list = {'FUSION','ZONE_DR','ZONE_FR','MCU_B_R'};
...
if strcmp(TargetNode,'FUSION')
    channel_list = {'CAN1','CAN2','CAN3','CAN4','CAN5','CAN6','CAN8'};
    busList =      {'1','2','3','4','6','7','14'};
    NUM_CAN_CHANNEL = '6';
    NUM_LIN_CHANNEL = '0';
...
elseif strcmp(TargetNode,'ZONE_FR')
    ...
    NUM_CAN_CHANNEL = '2';
    NUM_LIN_CHANNEL = '2';
elseif strcmp(TargetNode,'MCU_B_R')
    channel_list = {'CAN_MCU_R1','CAN3'};
    busList =      {'1','1'};
    NUM_CAN_CHANNEL = '2';
    NUM_LIN_CHANNEL = '0';
else
...
%% Read MessageLink
path = [project_path '\documents\MessageLink'];
filenames = dir(path);
filenames = string({filenames.name});
...
if Autosar_flg
    ...
    for j = 1:length(childNamesList)
        if  any(contains(MessageLink(2:end,3),extractAfter(childNamesList(j),'SG_')))
...
%     end
    ...
    if find(ismissing(string(MessageLink(:,1:3))))
    ...
    for j = 1:length(DBC.Messages)
        if ~contains(DBC.MessageInfo(j).Name,'CCP')...
                ...
                || (strcmp(Channel,'CAN8') && startsWith(DBC.MessageInfo(j).Name,'SRR')) % for D21M SRR
    ...
    DD_cell = cell(MsgCnt + Numb_array + 5000,16);
    ...
    if Autosar_flg
        ...
        for k = 1:Autosar_input_msg_CAN_num
            ...
            BlockName = [extractAfter(BlockName,'SG_') '_Status']; 
            srcT = 'simulink/Sources/In1';
            dstT = [TargetModel '/' BlockName];
            block_x = block_x;
            block_y = block_y + 50;
            ...
            inPos  = get_param(inport.Outport,'Position');
            ...
            dstT = [TargetModel '/' 'goto'];    
            h = add_block(srcT,dstT,'MakeNameUnique','on');     
            set_param(h,'position',[block_x,block_y,block_w,block_h]);
            set_param(h,'GotoTag',BlockName,'ShowName','off');
            goto = get_param(h,'PortHandles');
            add_line(TargetModel,inport.Outport(1),goto.Inport(1));
        ...
        BlockName = 'CheckTimeoutSubsystem';
        block_x = original_x - 1000;
        block_y = original_y + 50;
        block_w = block_x + 250;
        block_h = block_y + 80*Autosar_input_msg_CAN_num;
        srcT = 'built-in/SubSystem';
        dstT = [TargetModel '/' BlockName];    
        h = add_block(srcT,dstT);     
        set_param(h,'position',[block_x,block_y,block_w,block_h]);
        set_param(h,'ContentPreviewEnabled','off','BackgroundColor','LightBlue');
        ChkTimeoutModel = [TargetModel '/' BlockName];

        for k = 1:Autosar_input_msg_CAN_num
            BlockName = [extractAfter(char(Autosar_input_msg_CAN(k)),'SG_') '_Status'];
            srcT = 'simulink/Sources/In1';
            dstT = [ChkTimeoutModel '/' BlockName]; 
            block_x = original_x ;
            block_y = original_y + 200*k;
            block_w = block_x + 30;
            block_h = block_y + 13;
            Inp = add_block(srcT,dstT,'MakeNameUnique','on');  
            set_param(Inp,'position',[block_x,block_y,block_w,block_h]);
            InPort = get_param(Inp,'PortHandles');
            InPos  = get_param(InPort.Outport,'Position');

            BlockName = 'Equal'; 
            srcT = 'simulink/Quick Insert/Logic and Bit Operations/Equal';
            dstT = [ChkTimeoutModel '/' BlockName]; 
            block_x = InPos(1) +200;
            block_y = InPos(2) -25;
            block_w = block_x + 30;
            ...
            h = add_block(srcT,dstT, 'MakeNameUnique','on');  
            set_param(h,'position',[block_x,block_y,block_w,block_h]);
            equalPort = get_param(h,'PortHandles');
            equalPos  = get_param(equalPort.Inport,'Position');
            equalOutPos = get_param(equalPort.Outport,'Position');
            add_line(ChkTimeoutModel,InPort.Outport(1),equalPort.Inport(1));

            BlockName = 'constant';
            block_x = equalPos{2}(1) - 250;
            block_y = equalPos{2}(2) - 20;
            block_w = block_x + 150;
            block_h = block_y + 40;
            ...
            set_param(constant,'Value','TimeoutOccurred','ShowName', 'off');
            add_line(ChkTimeoutModel,constantPort.Outport(1),equalPort.Inport(2));

            BlockName = ['VHAL_CANMsgInvalid' char(erase(extractAfter(Autosar_input_msg(XX(k)),'SG_'),'_')) '_flg'];
            block_x = equalOutPos(1) + 150;
            block_y = equalOutPos(2) -5;
            block_w = block_x + 30;
            block_h = block_y + 13;
            srcT = 'simulink/Sinks/Out1';
            dstT = [ChkTimeoutModel '/' BlockName];    
            ...
            line = add_line(ChkTimeoutModel,equalPort.Outport(1),OutPort.Inport(1));
            set_param(line, 'Name', BlockName);  
            set(line,'MustResolveToSignalObject',1)
            
        end
        
        ChkTimeoutModelPort = get_param(ChkTimeoutModel,'Porthandles');
        ChkTimeoutModelInPos = get_param(ChkTimeoutModelPort.Inport,'Position');
        ChkTimeoutModelOutPos = get_param(ChkTimeoutModelPort.Outport,'Position');
        ChkTimeoutModelInport = find_system(ChkTimeoutModel, 'SearchDepth', 1, 'BlockType', 'Inport');
        ChkTimeoutModelOutport = find_system(ChkTimeoutModel, 'SearchDepth', 1, 'BlockType', 'Outport');
        inportNames = get_param(ChkTimeoutModelInport, 'Name');
        outportNames = get_param(ChkTimeoutModelOutport, 'Name');
        
        for k = 1: length(inportNames)
            srcT = 'simulink/Signal Routing/From';
            dstT = [TargetModel '/' char(inportNames(k))]; 
            if isscalar(ChkTimeoutModelInPos(:,1))
                block_x = ChkTimeoutModelInPos(1) - 300;
                block_y = ChkTimeoutModelInPos(2) - 20;
            else
                block_x = ChkTimeoutModelInPos{k}(1) - 300;
                block_y = ChkTimeoutModelInPos{k}(2) - 20;    
            end
            block_w = block_x + 200;
            block_h = block_y + 35;
            from = add_block(srcT,dstT,'MakeNameUnique','on');  
            set_param(from,'position',[block_x,block_y,block_w,block_h]);
            set_param(from,'Gototag',char(inportNames(k)))
            fromPort = get_param(from,'PortHandles');
            fromPos  = get_param(fromPort.Outport,'Position');
            add_line(TargetModel,fromPort.Outport,ChkTimeoutModelPort.Inport(k))
        end

        for k = 1: length(outportNames)
            srcT = 'simulink/Signal Routing/Goto';
            dstT = [TargetModel '/' char(outportNames(k))]; 
            if isscalar(ChkTimeoutModelOutPos(:,1))
                block_x = ChkTimeoutModelOutPos(1) + 100;
                block_y = ChkTimeoutModelOutPos(2) - 20;
            else
                block_x = ChkTimeoutModelOutPos{k}(1) + 100;
                block_y = ChkTimeoutModelOutPos{k}(2) - 20;
            end
            block_w = block_x + 200;
            block_h = block_y + 35;
            goto = add_block(srcT,dstT,'MakeNameUnique','on');  
            set_param(goto,'position',[block_x,block_y,block_w,block_h]);
            set_param(goto,'Gototag',char(outportNames(k)))
            gotoPort = get_param(goto,'PortHandles');
            gotoPos  = get_param(fromPort.Outport,'Position');
            add_line(TargetModel,ChkTimeoutModelPort.Outport(k),gotoPort.Inport(1))
        end
        % Outside channel block       
        targetport = get_param([new_model '/' Channel],'PortHandles');
        targetModelInport = find_system(TargetModel, 'SearchDepth', 1, 'BlockType', 'Inport');
        inportNames = get_param(targetModelInport, 'Name');
        for k = 1:length(inportNames)
            targetpos = get_param(targetport.Inport(k),'Position');
            BlockName = char(inportNames(k));

            if contains(BlockName,'_Status')
        ...
        h = add_block(srcT,dstT);  
        set_param(h,'position',[block_x,block_y,block_w,block_h]);
        set_param(h,'UseBusObject','on','BusObject',['BHAL_' Channel '_outputs']);
        targetport = get_param(h,'PortHandles');
    ...
    for j = 2: MsgCnt+1
        ...
        % For CAN8 SRR
        if strcmp(Channel,'CAN8') && startsWith(MsgName,'SRR')
            SignalName_app = cell(length(DBC.MessageInfo(cell2mat(RxMsgTable(j,strcmp(RxMsgTable(1,:),'DBCidx')))).Signals(:,1)),1);
            SignalName_app(:,1) = DBC.MessageInfo(cell2mat(RxMsgTable(j,strcmp(RxMsgTable(1,:),'DBCidx')))).Signals(:,1);
            Signal_match_array = 1:length(SignalName_app(:,1));
        end     
...
%         end
     %%
        ...
        if exist('Detect_AppMsg','var') && ~isempty(Detect_AppMsg) && ~isempty(Detect_APP_signal_can) || (strcmp(Channel,'CAN8') && startsWith(MsgName,'SRR'))
...
%         end      
    ...
    sourceport = get_param(h,'PortHandles');
    targetpos = get_param(sourceport.Outport(1),'Position');
    set_param(h,'OutDataTypeStr',['Bus:  BHAL_' Channel '_outputs']);
...
%              end
            ...
            if ~strcmp(Channel,'CAN8') || contains(Signalname,'CANMsgInvalid')
...
%         end        
     ...
     % buildbus(File_name,DD_path,DD_table,DD_table_cal,verctrl,ref_car_model,arch_Path);
     buildbus(File_name,DD_path,DD_table,DD_table_cal,verctrl,arch_Path);
     cd(arch_Path);
     disp([Channel ' Done!']);
...
Unit_modify = Rx_Signal_unit;
    ...
    switch Rx_Signal_unit
        ...
        case {'rad/s'}
            Unit_modify = cellstr('radps');
        case {'1/m'}
            Unit_modify = cellstr('1pm');
        case {'1/m^2'}
            Unit_modify = cellstr('1pm2');
    end    
end

function buildbus(FileName,PathName,signal_table,calibration_table,verctrl,ref_car_model,arch_Path)
...
%     cd(arch_Path);
%     car_model = ref_car_model{car_model_indx};
%     project_path = char([extractBefore(pwd,'software') '/../' car_model]);
...
if contains(Module,'HAL_CAN') && ~strcmp(Module,'HAL_CAN8')
    ARXML_Name = 'HALIN';
elseif contains(Module,'INP_')
    ARXML_Name = 'INP';
elseif strcmp(Module,'HAL_CAN8')
    ARXML_Name = 'ELK';
...
for i = 1:length(h)
    ...
    elseif strcmp(Signal_type,'sint16')
        Signal_type = cellstr('int16'); 
    end
