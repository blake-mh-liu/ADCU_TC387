...
for i = 1:length(Channel_list)
    ...
    % modify R_SG_APP name
    R_SG_APP(:,1) = erase(R_SG_APP(:,1),['HALR_' Channel '_']);
    ...
    for k = 1:length(DBC.Messages)
        ...
        if startsWith(MsgName,'NMm_MCU_B_R') || startsWith(MsgName,'Diag')
        ...
        elseif any(strcmp(R_SG_APP(:,1),MsgName)) && ~strcmp(TxNode,'MCU_B_R')
...
%             % P_SG_APP{h,6} = TxNode;
        elseif ~any(strcmp(Routing_RxMsg,MsgName)) && strcmp(TxNode,'MCU_B_R')
            cnt = cnt +1;
...
for i = 1:length(HOH_Array(:,1))
    ...
    Channel = upper(extractAfter(HOH_Name,extractBefore(HOH_Name,'CAN')));

    if contains(HOH_Name,'CAN_Xcp')
        Channel = cellstr('CAN_Xcp');
    elseif strcmp(Channel,'CAN_MCU_R1')
    ...
    % HOH CanIfCtrlDrvCfg
    h = find(contains(tmpCell,'/CanIf/CanIf/CanIfCtrlDrvCfg_0/'));
    OldString = extractBetween(tmpCell(h),'CanIfCtrlDrvCfg_0/','</VALUE-REF>');
    NewString = Channel;
    tmpCell(h) = strrep(tmpCell(h),OldString,NewString);
...
for i = 1:length(R_SG_APP(:,1))
    ...
    h = find(contains(tmpCell,'/CanIf/CanIf/CanIf_InitCfg/CanIfInitHohCfg/'));
...
for i = 1:length(P_SG_APP(:,1))
    ...
    if startsWith(P_SG_APP(i,1),'XCP_') || startsWith(P_SG_APP(i,1),'Diag')
        ...
        if contains(tmpCell(h+1,1),'<VALUE>')
            ...
            NewString = ['                      <VALUE>' char(string(cnt)) '</VALUE>'];
            tmpCell = [tmpCell(1:h);NewString;tmpCell(h+1:end)];
        end
    else
        h = find(contains(tmpCell,'/CanIfTxPduSourcePduID</DEFINITION-REF>'));
    ...
    % CanIfRxPduId
    h = find(contains(tmpCell,'/CanIfTxPduId</DEFINITION-REF>'));

    if isempty(h)
        h = find(contains(tmpCell,'/CanIfTxPduUserTxConfirmationUL</DEFINITION-REF>'));
        tmpCell_PduId = tmpCell(h-1:h+2);

        h = find(contains(tmpCell_PduId,'/CanIfTxPduUserTxConfirmationUL</DEFINITION-REF>'));
        OldString = extractBetween(tmpCell_PduId(h),'CanIfInitCfg/CanIfTxPduCfg/','</DEFINITION-REF>');
        NewString = cellstr('CanIfTxPduId');
        tmpCell_PduId(h) = strrep(tmpCell_PduId(h),OldString,NewString);
    ...
    OldString = extractBetween(tmpCell(h+1),'/CanIf/CanIf_InitCfg/','</VALUE-REF>');
    if strcmp(P_SG_APP(i,1),'XCP_')
        idx = find(strcmp(TxHOH_Array(:,4),'CAN_Xcp'));
    elseif strcmp(Channel,'CAN_MCU_R1')
        idx = strcmp(TxHOH_Array(:,4),'CAN_EOP');
    else
