...
Script_Path = pwd;
arch_Path= [Script_Path '\..\software\sw_development\arch'];
...
for i  = 1:Numb_inputs
    ...
    out_name = strcat("VOUTP_",out_name);            
     ...
     block_h=block_y+30;
     Hal = add_block(srcT,dstT);  
     set_param(Hal,'position',[block_x,block_y,block_w,block_h ],'showname','off','OutDataTypeStr',str_unit);
     ...
     for j = 1:length(k_value)
         ...
         elseif (j==2)
            Arry_cal(Numb_Cal,3) = cellstr(str_unit);
            if (str_unit == "single")
            ...
            elseif (str_unit == "single")
                ...
                Arry_cal(Numb_Cal,7) = cellstr('N/A');
                Arry_cal(Numb_Cal,8) = num2cell(0);
            elseif (str_unit == "uint16")
                Arry_cal(Numb_Cal,5) = num2cell(65535);
                Arry_cal(Numb_Cal,6) = cellstr(sign_unit);
                Arry_cal(Numb_Cal,7) = cellstr('N/A');
                Arry_cal(Numb_Cal,8) = num2cell(0);
           elseif (str_unit == "uint32")
...
dstT = [new_model '/' char(Bus_name)]; 
Hal = add_block(srcT,dstT);
set_param(Hal,'position',[block_x,block_y,block_w,block_h],'UseBusObject','on','BusObject',Bus_name);  
dstT = [new_model];
