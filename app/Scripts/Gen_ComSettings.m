...
if strcmp(TargetECU,'MCU_B_R'); TargetECU_Abb = 'MCU'; end
...
R_port_msg = extractAfter(erase(R_port_msg, [Channel_list; {'HALR_'}]),'_');

h = find(contains(Composition_arxml,'<P-PORT-PROTOTYPE>')) + 1;
P_port_msg = extractBetween(Composition_arxml(h),'<SHORT-NAME>','</SHORT-NAME>');
P_port_msg = extractAfter(erase(P_port_msg, [Channel_list; {'HALP_'}]),'_');
    ...
    for k = 1:length(DBC.Messages)
        ...
        elseif sum(strcmp(erase(P_port_msg,'CGW_'),MsgName)) == 2 && strcmp(DBC.MessageInfo(k).TxNodes,TargetECU)
            ...
            for g = 1:length(h)
                ...
                for n = 1:1 %length(DBC.MessageInfo(k).Signals)
                    SignalName = char(DBC.MessageInfo(k).Signals(n));
        ...
        elseif strcmp(DBC.MessageInfo(k).TxNodes,TargetECU) && ~any(strcmp(Routing_RxMsg,MsgName)) && ~contains(DBC.MessageInfo(k).Name,{'Diag';'NMm'})
            MsgName = char(DBC.MessageInfo(k).Name);
            MessageID = num2str(DBC.MessageInfo(k).ID);
            CycleTime_ms = DBC.MessageInfo(k).AttributeInfo(strcmp(DBC.MessageInfo(k).Attributes(:,1),'GenMsgCycleTime')).Value;
            MsgSendtype = DBC.MessageInfo(k).AttributeInfo(strcmp(DBC.MessageInfo(k).Attributes(:,1),'GenMsgSendType')).Value;
...
Msg_idx(end+1,1) = length(ValueTable_R(:,1));
...
for i = 1:length(Msg_idx)-1
    ...
    Channel = char(ValueTable_R(Msg_idx(i),9));
...
h = find(contains(COM_arxml,'T</SHORT-NAME>'));
h1 = find(contains(COM_arxml,'<SHORT-NAME>GRCAN'));
h = intersect(h,h1);

% Get Tx ComsignalGroup template
ECUC_end_array = find(contains(COM_arxml,'</SUB-CONTAINERS>'));
ECUC_start = h(1)-1;
ECUC_end = min(ECUC_end_array(ECUC_end_array>h(1)))+1;
...
Msg_idx(end+1,1) = length(ValueTable_P(:,1));
...
for i = 1:length(Msg_idx)-1
    ...
    Channel = char(ValueTable_P(Msg_idx(i),9));
    ...
    % ComSignalGroupDirection
    h = find(contains(Template_Comsigroup_P,'/ComSignalGroupDirection</DEFINITION-REF>'), 1);

    if isempty(h)
        h = find(contains(Template_Comsigroup_P,'/ComTransferProperty</DEFINITION-REF>'));
        tmpCell_Direct = Template_Comsigroup_P(h-1:h+2);

        h = find(contains(tmpCell_Direct,'/ComTransferProperty</DEFINITION-REF>'));
        OldString = extractBetween(tmpCell_Direct(h),'/ComSignalGroup/','</DEFINITION-REF>');
        NewString = cellstr('ComSignalGroupDirection');
        tmpCell_Direct(h) = strrep(tmpCell_Direct(h),OldString,NewString);

        OldString = extractBetween(tmpCell_Direct(h+1),'<VALUE>','</VALUE>');
        NewString = 'SEND';
        tmpCell_Direct(h+1) = strrep(tmpCell_Direct(h+1),OldString,NewString);

        % Add into <PARAMETER-VALUES>
        h = find(contains(Template_Comsigroup_P,'</PARAMETER-VALUES>'));
        Template_Comsigroup_P = [Template_Comsigroup_P(1:h-1);tmpCell_Direct;Template_Comsigroup_P(h:end)];
    end
...
for i = 1:length(Msg_idx)
    ...
    NewString = [TargetECU_Abb '_' Channel '_Rx' ];
    Template_ComIPDU_R(h) = strrep(Template_ComIPDU_R(h),OldString,NewString);
...
for i = 1:length(Msg_idx)
    ...
    NewString = [TargetECU_Abb '_' Channel '_Tx' ];
    Template_ComIPDU_P(h) = strrep(Template_ComIPDU_P(h),OldString,NewString);
...
ECUC_end = find(contains(COM_arxml,'<SHORT-NAME>MCU_CAN'),1,'first')-1;
