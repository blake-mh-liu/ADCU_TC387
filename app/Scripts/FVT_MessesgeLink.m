...
%% Initial settings
% ref_car_model = {'d21lawd-fdc2','d21lrwd-fdc2', 'd31hawd-fdc2'};
...
ECU_list = {'ZONE_DR','ZONE_FR', 'ZONE_RR_TA2', 'ZONE_FR_TA2', 'FUSION','MCU_B_R'};
...
MsgLinkFolder = [project_path '\app\documents\MessageLink'];
...
for i = 1:length(MsgLinkInArray)
    ...
    for j = 1:length(sig_index)
        ...
        MsgLinkInArray(i,j+3) = table2cell(MsgLinkInTable(i,3)); 
    end
end
...
if contains(TargetNode,'MCU')
    cd ([project_path '\app\documents\MessageLink']);
...
if isempty(RoutingTableName)
    ...
    can4_file = char(dbc_files(contains({dbc_files.name}, 'CAN3')).name);
