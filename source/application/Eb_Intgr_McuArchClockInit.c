...
FUNC(void,MCU_APPL_CODE) Eb_Intgr_McuArchClockInit_Init(void)
{
...
#if (( MCU_NO_PLL == STD_OFF )||( MCU_DISTRIBUTE_PLL_CLOCK_API == STD_ON ))
  ...
  while (MCU_PLL_STATUS_UNDEFINED != Mcu_GetPllStatus())
  {
    ;
  }

#endif /* #if (( MCU_NO_PLL == STD_OFF )||( MCU_DISTRIBUTE_PLL_CLOCK_API == STD_ON )) */
#endif /* #if (( MCU_INIT_CLOCK_API == STD_ON )||( MCU_INIT_CLOCK == STD_ON )) */
