...
FUNC(void, BSWM_APPL_CODE) Eb_Intgr_BswM_DriverInitTwo(void)
{
  ...
  /*Initilization of the Ethernet controller (HW)*/
  ETH_EB_MIIINIT();
#endif
...
/* Initialization of the Ethernet controller so that most APIs are available (after 
activation of the Eth controller all APIs are available). */
#if(BASE_ETH_ENABLED==STD_ON)
#if defined(ETH_EB_CONTROLLERINIT)
