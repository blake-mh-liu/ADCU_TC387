...
#
# $Id: genld-TRICORE-Mk.pl 39057 2020-02-07 11:13:37Z brha2357 $
#
...
for (my $coreId = 0; $coreId < $sysdesc->{'-numcores'}; ++$coreId)
{
	...
	for my $name (@stacks)
	{
		...
		$LdBackend->printOutputSectionWithGap($stacksym, 'STACK', [$file], undef, "MK_RSA_MK_$stack", "MK_RLA_MK_$stack");
	}
	...
	foreach my $stack (@threadStacks) {
		...
		$LdBackend->printOutputSectionWithGap($stacksym, 'STACK', undef, [".bss.$stacksym"], "MK_RSA_$stack", "MK_RLA_$stack");
	}

	# for single core systems no core specific regions are needed
	if ($sysdesc->{'-numcores'} > 1)
	{

	$LdBackend->printComment(
		"--------------------------------------------------------",
			"Memory regions for the MK and counter subsystem data on core $coreId",
		"--------------------------------------------------------"
	);
	...
	$LdBackend->printOutputSectionWithGap("MK_Ram_bss_C$coreId", 'BSS',
		["Mk*_c${coreId}_*", "Ioc_data_kern_c${coreId}_*"],
	...
	$LdBackend->printOutputSectionWithGap("MK_OsRam_bss_C$coreId", 'BSS', undef,
...
# Common MK and IOC data
my @ram_selectors = ("Mk_*", "Ioc_data_kern_shared*");
# For single core systems also include core specific data since no extra core region is needed
push (@ram_selectors, ("Ioc_data_kern*")) if $sysdesc->{'-numcores'} == 1;

$LdBackend->beginMemoryBlockMappings("int_ram_core0");
$LdBackend->printOutputSection("MK_Ram_data", 'DATA',
	\@ram_selectors, undef, "MK_RSA_MK_Ram", undef, "MK_RDA_MK_Ram");
$LdBackend->printOutputSectionWithGap("MK_Ram_bss", 'BSS',
	\@ram_selectors, undef, "MK_BSA_MK_Ram", "MK_RLA_MK_Ram");

if ($sysdesc->{'-numcores'} == 1)
{
# Common QM-OS data
my @osram_selectors = ("kern-*", "TRICORE-*", "Os_*");
# For single core systems also include core specific data since no extra core region is needed
	push (@osram_selectors, ("core*", "shared.core*"));

$LdBackend->printOutputSection("MK_OsRam_data", 'DATA',
	\@osram_selectors, undef, "MK_RSA_MK_OsRam", undef, "MK_RDA_MK_OsRam");
$LdBackend->printOutputSectionWithGap("MK_OsRam_bss", 'BSS',
	\@osram_selectors, undef, "MK_BSA_MK_OsRam", "MK_RLA_MK_OsRam");
}
...
sub printPrivateObjectRegion
{
	...
	if ( @$files == 0 ) {
		...
		$LdBackend->printOutputSectionWithGap("bss_C$coreId.$name", 'BSS', $files, undef, $bss, $dend);
	}
