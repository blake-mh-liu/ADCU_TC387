...
sub needsLoadFlash
{
	my $self = shift;
	my ($osecType) = @_;
	my $mydisableRomInit = $GenldCommon::sysdesc{'-disableRomInit'};
	return ($osecType eq 'DATA') && ($mydisableRomInit ne '-disableRomInit');
}
...
sub endMemoryBlockMappings
{
	...
	{
		$temp = "DATA";
		$type1 = join("_","_","GLBL","$temp","START"); 
		$type2 = join("_","_","GLBL","$temp","END"); 
	}
	...
	if ($block eq 'RAM')
	{
		GenldCommon::printOut("} run=$block RUN_START($type1), RUN_END($type2)");
	}
	else
	{
...
sub printSymbolDefinition
{
	my $self = shift;
	my ($symbol, $value) = @_;
	GenldCommon::printOut("	SYMBOL_DEF_${symbol} : { ${symbol} = ${value}; }\n");
