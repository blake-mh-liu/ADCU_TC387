...
sub parseArgs
{
	...
	elsif ($arg eq '-mapMpuCacheToRom')
	{
		GenldCommon::setArg($sysdesc, $arg, $$argv_position);
		$sysdesc->{$arg} = 1;
	}
	elsif ($arg eq '-mapMpuCacheToRam')
	{
		GenldCommon::setArg($sysdesc, $arg, $$argv_position);
		$sysdesc->{$arg} = 1;
	}
...
Additional options specific to CORTEXM:
  ...
  -mapMpuCacheToRom:
  Put the region cache into the MK_Rom memory region. This assumes, that MK_Rom is mapped into RAM,
  as the region cache is written to during startup before memory protection is turned on.
  -mapMpuCacheToRam:
  Put the region cache into the RAM memory region.
  The region cache is written to during startup before memory protection is turned on.
EOUSAGE
