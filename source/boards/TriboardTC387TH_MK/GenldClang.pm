...
=head1 The C<GenldClang> package

This package implements the L<GenldBackend> interface for the Clang ld linker that is typically used
with Clang-based toolchains.
...
The following attributes are expected by the C<GenldClang> back end in the output section type
specifications:

=over

=item C<_clang_extraattr>: Optional extra attributes for an output section definition
...
push @GenldCommon::usage_fns, \&clang_usage;
...
sub printOutputSection
{
	...
	# To discard the unwanted input section
	if ($osecName =~ /^DISCARD$/)
	{
		...
		GenldCommon::printOut("*($isectSelector)");
		GenldCommon::printOut("}");
		return;
	}
	...
	{
		$startAlign = _val_to_dec($startAlign);
	}

	# Override end alignment from architecture defintion
	if (defined $endAlign)
	{
		$endAlign = _val_to_dec($endAlign);
	}
	...
	if($inFiles)
	{
		...
		{
			# Clang wants the full path name, which we do not support. We prepend a wildcard
	...
	#This is currently Clang only: Explicitly specify segments in the ELF header.
...
# For normal Clang LDs, aligning the section using ALIGN inside the section specification suffices.
# Make this behavior overrideable for derived backends based on the Clang syntax.
...
Additional options specific to Clang:
  -emit-load-segements:
  Explicitly place output sections in loadable segments. This option expects to find a suitable
  PHDRS section in the board-specific LDscript snippet that also specifies the memory regions. You
  need to specify 'text' and 'data' segments there.
  This is meant as a workaround for versions of the Clang toolchain where the linker generates
  binaries that have overlapping segements, although the output sections in the resulting binary do
  not overlap.
EOUSAGE
