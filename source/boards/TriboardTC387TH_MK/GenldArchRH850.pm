...
(
	...
	'BSS'            =>
	{
		align        => 4,
		endalign     => 4,
		isectDefault => [qw(.bss .zbss .sbss COMMON)],
...
sub parseArgs
{
	...
	elsif($arg eq '-use-common-vector-table')
	{
		GenldCommon::setArg($sysdesc, $arg);
	}
	else
...
Additional options specific to RH850:
  ...
  -use-common-vector-table:
   This avoids creation of reset vectors for different cores and uses the common vector table
   Do not use this option on derivatives that do not have a common vector table.
