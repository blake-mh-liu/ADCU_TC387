...
sub parseArgs
{
	...
	elsif ($arg eq '-use-Synoposys-VDK-Instrumentation')
	{
		$sysdesc->{$arg} = 1;
	}
	else
...
Additional options specific to TriCore:
  ...
  -hv:
   This option adds a TMPL_StartupHv section at the beginning of the entry code.
   This is necessary for derivatives that start in hypervisor mode.

  -use-Synoposys-VDK-Instrumentation:
   This option creates sections necessary for the use of the Synopsys VDK Instrumetation that allows
   the use of certain features of the Synopsys simulator.
