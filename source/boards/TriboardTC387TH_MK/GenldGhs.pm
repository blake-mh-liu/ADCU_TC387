...
sub printOutputSection
{
	...
	if($inFiles)
	{
		...
		{
			# We add the appropriate Glob operators for this file.
	...
	foreach my $file (@files)
	{
		...
		{
			GenldCommon::printOut("\"$file($selector)$post\"");
		}
	}
	...
	if(defined $endSymbol)
	{
		...
		# Must be done outside the section as the "." evaluates to relative addresses inside the section.
